...
```md
В начале изучения HTML, CSS и JavaScript будут полезны наши руководства по [Изучению веб-разработки](/ru/docs/Learn).
```

### Внешние ссылки

При переводе ссылок на сторонние сайты необходимо придерживаться следующих правил:
...
## Адаптация при переводе личностных обращений

Для более эффективной передачи смысла оригинального документа следует использовать неопределенно-личные высказывания и избегать прямых обращений.

Примеры использования обращения к читателю:
...
При переводе на русский язык лучше использовать безличные обороты:

```md
Руководство показывает, как использовать JavaScript и даёт общее представление о языке.
Исчерпывающую информацию о возможностях языка можно найти в справочнике по JavaScript.
```

Если избежать использования обращения нельзя, то используйте «_вы_» и его производные со строчной буквы.
...
- Используйте кавычки-ёлочки (`«` и `»`), за исключением примеров кода.
...
- Перевод некоторых терминов можно найти в [словарях «Веб-стандартов»](https://github.com/web-standards/dictionary) и [глоссарии](#Глоссарий).
- При переводе зарубежных имён используйте [правила практической транскрипции](https://ru.wikipedia.org/wiki/Категория:Практическая_транскрипция_с_русским_языком).
- Переводите примеры кода (как комментарии, так и вывод), за исключением ошибок.
- Не переводите названия компаний, библиотек и т.п.
- Не переводите аббревиатуры (API, DOM, UI и т.п.).
...
| ------------------- | -------------------------------------------------------------------- |
| cache               | кеш                                                                  |
| feature             | возможность (менее предпочтительный синоним — _функциональность_)    |
...
\* — по возможности лучше избежать прямого перевода, используя синонимы или перефразирование.
