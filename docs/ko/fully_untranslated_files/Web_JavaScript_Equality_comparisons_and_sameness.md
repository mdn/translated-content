...
3. 피연산자 중 하나가 객체이고 다른 하나가 원시 값인 경우 [객체를 원시 값으로 변환](/en-US/docs/Web/JavaScript/Data_structures#primitive_coercion)합니다.
4. 이 단계에서 두 피연산자는 원시 값(문자열, 숫자, 불리언, Symbol, BigInt 중 하나)로 변환됩니다. 나머지 변환은 사항별로 수행됩니다.
   ...
   - 피연산자 중 하나가 불리언이고 다른 피연산자는 아닌 경우, [불리언을 숫자로 변환](/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number#number_coercion)합니다. `true`는 1로 변환되고 `false`는 0으로 변환됩니다. 그런 다음 두 피연산자를 다시 느슨하게 비교합니다.
