...
本文是針對正體中文（ zh-TW ）地區的翻譯指南。如果你所在的地區尚未有文件指南而你又想新建的話，歡迎你開始建立或[跟我們負責該地區的團隊](https://github.com/mdn/translated-content/blob/docs-readme/README.md)聯絡。

同樣的，如果你對一般指導原則有一些不錯的想法想要加入，不用客氣，去發起一個問題（ issue ）來跟我們談談吧。以下進入正體中文翻譯指導原則的主要內容。
...
然而，不要對實際的程式碼術語進行翻譯，好比語法。你要確保範例在經過翻譯後仍可正常執行。
...
基於閱讀及選取操作（連點游標可選取該詞組—— word ）上的便利性，在翻譯時，中外文（拉丁語系）交界處使用一個半形空格進行修飾是常見的作法。但譯者往往發生風格不一致、過度修飾等問題。
...
- 過度修飾。在中文與中文標點符號之間也進行修飾，要知道中文標點符號既中文字。
  - 正確：版本控制系統—— Git 。
  - 不正確：版本控制系統 —— Git。
...
英文有主動語態（active voice）及被動語態（passive voice）。中文雖然也具備主被動語態的觀念，但主要仍以主動語態為主。因此在翻譯時應盡可能的使用主動語態以避免造成閱讀與理解上的困難，最典型的就是「被」的濫用以及照搬英文的倒裝句。
...
在英文的文章中，通常會以第二人稱的方式進行敘述。但在翻譯為中文時這種過度出現的代名詞實際上會使得閱讀變得不通順。適度地減少代名詞地出現並不為過，甚至能夠讓中文的閱讀更加順暢。
...
- 宜：在完成操作指南後，在有序清單之後給予結語或說明有哪些在完成操作指南後應期待出現的結果。
- 不宜：在你完成你的操作指南之後，在有序清單之後給予結語或說明有哪些在完成操作指南後應期待出現的結果。
