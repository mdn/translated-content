...
이 장은 당장 알아야 하는 기본적인 부분만 다룹니다.

### 숫자의 종류

프로그래밍에서는 우리 모두가 잘 알고 있는 단순한 십진수 체계도 생각보다 복잡합니다. 예를 들어, 우리는 다양한 유형의 십진수를 설명하기 위해 다양한 용어를 사용합니다.

- **정수**는 분수가 없는 부동 소수점 숫자입니다. 예를 들어 10, 400 또는 -5와 같이 양수 또는 음수가 될 수 있습니다.
- **부동 소수점 숫자**(floats)에는 소수점과 소수 자릿수가 있습니다(예: 12.5, 56.7786543).
- **Doubles**는 표준 부동 소수점 숫자보다 정밀도가 높은 특정 유형의 부동 소수점 숫자입니다(즉, 소수점 이하 자릿수까지 정확하다는 의미).

숫자 체계에도 다양한 유형이 있습니다! 10진수는 10을 기본으로 하지만(각 자릿수에 0-9를 사용한다는 의미), 다음과 같은 것도 있습니다.
...
> [!NOTE]
> 사실 JavaScript에는 {{jsxref("BigInt")}}라는 또 다른 숫자 타입이 있습니다. 정수를 나타내는 데 사용되는 데이터 타입이지만 이 글에서는 다루지 않습니다.
...
2. 숫자는 따옴표가 없습니다. 계속 하기 전에 여러 개의 변수를 선언하고 숫자를 초기화 해주세요.
3. 우리들의 변수들의 데이터 타입을 확인합니다. JavaScript에서는 데이터 타입을 확인하기 위해 {{jsxref("Operators/typeof", "typeof")}} 라는 키워드를 사용합니다. 아래와 같이 입력해 주세요:
   ...
   `"number"` 는 정수와 실수인 경우에 나옵니다. 이것들은 정수와 실수가 다른 데이터 타입일 때 보다 다루기 쉽게 해줍니다. 그리고 다른 데이터 타입일 때 다른 방법으로 다뤄야만 합니다. 휴!
...
> [!NOTE]
> 연산에 관계된 수를 {{Glossary("Operand", "피연산자")}}라고 부릅니다.
...
> [!NOTE]
> JavaScript의 연산자와 연산자 우선순위에 대해 더 알고 싶다면 [연산자 우선순위](/ko/docs/Web/JavaScript/Reference/Operators/Operator_precedence) 문서를 참고하세요.
...
> [!NOTE]
> 변수 앞에 연산자를 사용하면 브라우저가 변수를 먼저 증가/감소시키고 값을 반환합니다. 이번에는 `++num1`과 `--num2`를 사용하고 위의 예제를 다시 실행해 보세요.
...
또한 오른쪽 표현식에서 다른 변수를 사용할 수 있습니다. 다음의 예시를 확인해보세요.
...
> [!NOTE]
> 일부 사람들은 `==`와 `!=`를 동등성과 불일치성을 테스트하는 데 사용합니다. 이 연산자들은 JavaScript에서 유효한 연산자이지만, `===`/`!==`와는 다릅니다. 전자는 값이 동일한지는 테스트하지만, 값의 데이터 유형이 동일한지의 여부는 테스트하지 않습니다. 후자의 경우 엄격한 버전은 값과 값의 데이터 유형의 동일성을 모두 테스트합니다. 엄격한 버전이 오류가 적은 편이기 때문에, 이를 사용하는 것을 권장합니다.
...
> [!NOTE]
> 두 상태 사이를 전환하는 이러한 컨트롤을 일반적으로 **토글(toggle)** 이라고 합니다. 토글은 조명 켜기, 조명 끄기 등 한 상태와 다른 상태 사이를 전환합니다.

## 실력 테스트!

이 글의 마지막까지 읽었지만 가장 중요한 정보를 기억할 수 있나요? 다음 단계로 넘어가기 전에 이 정보를 기억하고 있는지 확인할 수 있는 몇 가지 추가 테스트가 있습니다. [실력 테스트: 수학](/ko/docs/Learn/JavaScript/First_steps/Test_your_skills:_Math)을 참고하세요.
