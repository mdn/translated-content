...
- **전체 로드시간 단축하기**: 사용자 컴퓨터로 웹 사이트를 렌더링하기 위해 필요한 파일을 다운로드 하는 데 시간이 얼마나 걸리나요? 이는 파일 크기, 파일 개수, [대기시간](/ko/docs/Web/Performance/Guides/Understanding_latency), 그 외 다른 요인에 의해 영향을 받습니다. 일반적인 개선 방법으로는 가능한 파일을 작게 만들고 HTTP 요청 수를 줄이고 가능한 파일을 바로 사용할 수 있도록 [preload](/ko/docs/Web/HTML/Attributes/rel/preload)와 같은 현명한 로딩 기법을 적용하는 것입니다.
