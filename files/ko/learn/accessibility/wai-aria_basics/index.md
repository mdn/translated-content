...
- 상태
  - : 입력 양식이 현재 비활성화 되어 있음을 뜻하는 `aria-disabled="true"`등과 같이 요소의 현재 상태를 지정하여 스크린리더에 전달하는 특수 속성입니다. 속성은 앱의 수명 주기 동안 변경되지 않는 반면 상태는 일반적으로 자바스크립트를 통해 프로그래밍 방식으로 변화시킬 수 있다는 점에서 속성과 차이가 있습니다.
...
WAI-ARIA가 만들어진 몇 가지 문제 상황에 대해 앞서 이야기 해 보았습니다만 핵심적으로는 다음의 네 가지 영역에서 유용하게 사용될 수 있습니다.

- 이정표/랜드마크
  - : ARIA의 [역할(`role`)](/ko/docs/Web/Accessibility/ARIA/Roles) 속성 값은 HTML 요소(예: {{htmlelement("nav")}})의 의미를 복제하거나 HTML 의미를 넘어 검색(`search`), 탭 목록(`tablist`), 탭(`tab`), 목록 상자(`listbox`) 등 다양한 기능 영역에 대한 이정표를 제공하는 랜드마크 역할을 할 수 있습니다.
- 동적 콘텐츠 업데이트
  - : 스크린리더는 지속적으로 변화하는 콘텐츠를 알리는 데 어려움을 겪습니다. ARIA의 `aria-live`를 사용하면 콘텐츠 영역이 동적으로 업데이트될 때(예를 들어, 페이지 내의 자바스크립트를 이용하여 [새로운 콘텐츠를 서버에서 가져와 DOM에 업데이트 하기](/ko/docs/Learn/JavaScript/Client-side_web_APIs/Fetching_data)) 스크린리더를 통해 알림을 받을 수 있습니다.
- 키보드 접근성 향상
  - : 네이티브 HTML 요소들은 기본적으로 키보드 접근성을 포함하고 있는데, 자바스크립트와 함께 다른 요소들을 사용하여 유사한 상호작용을 시뮬레이션하면 결과적으로 키보드 접근성 및 스크린리더 보고에 문제가 발생합니다. 하지만 이러한 상황이 불가피하다면 WAI-ARIA는 다른 요소가 포커스를 받을 수 있도록 하는 수단( `tabindex` 이용)을 제공할 수 있습니다.
- 시맨틱이 결핍된 컨트롤의 접근성
  - : 여러개의 중첩된 `<div>`를 CSS/JavaScript와 함께 사용하여 복잡한 UI 기능을 만들거나 네이티브 컨트롤이 JavaScript를 통해 크게 향상/변경되는 경우 접근성이 저하될 수 있습니다. 스크린리더 사용자는 의미나 기타 단서가 없으면 해당 기능이 무엇을 하는지 알아내기 어렵습니다. 이러한 상황에서 ARIA는 버튼(`button`), 목록 상자(`listbox`) 또는 탭 목록(`tablist`)과 같은 역할과 `aria-required` 또는 `aria-posinset`과 같은 속성을 조합하여 기능에 대한 추가 단서를 제공하는 데 도움을 줄 수 있습니다.

한 가지 기억해야 할 점은 **꼭 필요한 경우에만 WAI-ARIA를 사용해야 한다는 것입니다!** 이상적으로는 _항상_ 네이티브 HTML 기능을 사용하여 스크린리더가 사용자에게 무슨 일이 일어나고 있는지 알려주는 데 필요한 시맨틱을 제공해야 합니다. 그러나 코드를 제어할 수 있는 권한이 제한되어 있거나 간단한 HTML 요소로는 쉽게 만들기 어려운 복잡한 것을 구현할 때는 이것이 불가능합니다. 이러한 경우 WAI-ARIA는 접근성을 향상시키는 유용한 도구가 될 수 있습니다.

그러나 다시 한 번 강조하지만, 꼭 필요한 경우에만 사용하세요!

> [!NOTE]
> 또한 비장애인, 스크린 리더를 사용하는 사람, 키보드 탐색을 사용하는 사람 등 다양한 _실제_ 사용자를 대상으로 사이트를 테스트해 보세요. 이들은 사이트가 얼마나 잘 작동하는지에 대해 여러분보다 더 나은 통찰력을 가지고 있을 것입니다.
