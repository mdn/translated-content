...
| [`<dimension>`](/en-US/docs/Web/CSS/dimension) | `<dimension>` 은 예를 들어 `45deg`, `5s` 또는 `10px`. 과 같은 단위가 붙어있는 `<number>` 입니다. `<dimension>` 은 [`<length>`](/en-US/docs/Web/CSS/length), [`<angle>`](/en-US/docs/Web/CSS/angle), [`<time>`](/en-US/docs/Web/CSS/time) 및 [`<resolution>`](/en-US/docs/Web/CSS/resolution) 의 종류를 포함하는 카테고리입니다[.](/ko/docs/Web/CSS/resolution) |
