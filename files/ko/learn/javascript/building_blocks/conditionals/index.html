...
<p>여러분이 자바스크립트에서 쓸 단연코 가장 일반적인 형태의 조건문을 살펴봅시다 — 변변찮은 <code><a href="/ko/docs/Web/JavaScript/Reference/Statements/if...else">if ... else</a></code><a href="/ko/docs/Web/JavaScript/Reference/Statements/if...else"> 문</a>입니다.</p>
...
<p>이 코드는 사람이 읽을 수 있습니다. — 이것은 "<strong>만약</strong> <strong>조건</strong>이 <code>true</code>면, 코드 A를 실행하고, <strong>아니면</strong> 코드 B를 실행한다." 라고 말합니다.</p>
...
<p>하지만, 여기서 조심해야 할 점 — 위의 경우, 코드의 두 번째 블록은 조건문에 의해서 제어되지 않아서, 조건이 <code>true</code>나 <code>false</code>를 리턴하는 것에 관계없이 <strong>항상</strong> 동작합니다. 이것이 반드시 나쁜 것은 아니지만, 여러분이 원하는 것이 아닐 지도 모릅니다. — 종종 여러분은 둘 다가 아니라, 코드의 한 블럭 또는 다른 블럭을 실행하기를 원합니다.</p>
...
<p>추가로 선택/결과를 <code>if...else</code>에 연결하는 방법이 있습니다. — <code>else if</code>를 사용해서 말입니다. 각 추가 선택은 <code>if() { ... }</code>와 <code>else { ... }</code>사이에 넣을 추가적인 블록을 필요로 합니다. 간단한 날씨 예보 어플리케이션의 일부가 될 수 있는 다음의 좀 더 관련된 예시를 확인하세요:</p>
...
<ol>
 ...
 <li>자바스크립트 코드에서 {{htmlelement("select")}} 와 {{htmlelement("p")}} 요소를 모두 저장하고 있고, 값이 변할 때 <code>setWeather()</code>함수가 동작하도록 <code>&lt;select&gt;</code> 요소에 이벤트 리스너를 추가했습니다.</li>
...
<p>위 예시에서, 첫 번째 코드 블록은 <code>choice === 'sunny'</code><em>와</em> <code>temperature &lt; 86</code>가 오직 <code>true</code>를 리턴해야만 실행될 것입니다.</p>
...
<p>그러한 경우에 <a href="/ko/docs/Web/JavaScript/Reference/Statements/switch"><code>switch</code>문</a>은 여러분의 친구입니다. 이는 입력으로 하나의 표현식/값을 받고, 값과 일치하는 하나를 찾을 때까지 여러 항목을 살펴보고 그에 맞는 코드를 실행합니다. 여러분에게 아이디어를 주기 위해, 여기 몇몇 많은 의사 코드가 있습니다:</p>
