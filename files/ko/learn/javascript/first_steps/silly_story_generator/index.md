...
5. 두번째 `if` 블록에서는 `uk` 라디오 버튼이 선택되었는지 확인합시다. `uk` 라디오 버튼이 눌려졌다면, 이야기 상의 무게와 온도('temperature') 값 들인 파운드('pounds')와 화씨온도(Fahrenheit)를 스톤('stone')과 섭씨온도('centigrade')로 바꾸어야 합니다. 필요한 것은 아래와 같습니다.
