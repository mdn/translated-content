...
- **부동 소수점 숫자**(부동 소수점)에는 소수점과 소수 자릿수가 있습니다(예: 12.5, 56.7786543).
- **복소**는 표준 부동 소수점 숫자보다 정밀도가 높은 특정 유형의 부동 소수점 숫자입니다(즉, 소수점 이하 자릿수까지 정확하다는 의미).
...
> **참고:** JavaScript의 연산자와 연산자 우선순위에 대해 더 알고 싶다면 [연산자 우선순위](/ko/docs/Web/JavaScript/Reference/Operators/Operator_precedence) 문서를 참고하세요.
...
> **참고:** 두 상태 사이를 전환하는 이러한 컨트롤을 일반적으로 토글이라고 합니다. 토글은 조명 켜기, 조명 끄기 등 한 상태와 다른 상태 사이를 전환합니다.

## 실력을 시험해 보세요!

이 글의 마지막까지 읽었지만 가장 중요한 정보를 기억할 수 있나요? 다음 단계로 넘어가기 전에 이 정보를 기억하고 있는지 확인할 수 있는 몇 가지 추가 테스트가 있습니다. [실력 테스트: 수학](/en-US/docs/Learn/JavaScript/First_steps/Test_your_skills:_Math)
