...
위 코드는 텍스트 단락을 선택(1번 줄)해서 이벤트 리스너를 첨부(3번 줄)하여, 사용자가 단락을 클릭하면 `updateName()` 코드 블록(5번 ~ 8번 줄)을 실행하도록 합니다. `updateName()` 코드 블록(이렇게 재사용 가능하도록 나눠놓은 코드 블록을 "함수"라고 합니다)은 새 이름을 요청한 다음 해당 이름을 단락에 삽입하여 표시를 업데이트합니다.
...
`para` 객체가 아직 존재하지 않으므로 이벤트 리스너를 추가할 수 없기 때문입니다.
...
이건 이벤트, 그중에서도 HTML 본문 전체를 불러와 읽었다는 것을 나타내는 브라우저의 `DOMContentLoaded` 이벤트를 수신하는 이벤트 리스너입니다.
...
/*
  ...
  2. 버튼 각각을 순회하면서 클릭 이벤트 리스너를 추가합니다.
