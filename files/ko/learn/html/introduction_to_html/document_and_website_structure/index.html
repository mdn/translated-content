...
<p>이유는 시각 장애인과 관련이 있습니다. 시각 장애인에게는 색상,글꼴이 중요하지 않습니다. 화면이 전체 스토리를 말해주지 않기 때문입니다.</p>

<div class="note">
<p><strong>참고</strong>: 색맹은 세계 인구의<a href="http://www.color-blindness.com/2006/04/28/colorblind-population/">8% 를 차지 합니다</a>. 시각 장애인은 세계 인구의 약 4-5%를 차지합니다 (2012년에는 <a href="https://en.wikipedia.org/wiki/Visual_impairment">전 세계적으로 2억 8,500만명</a>), 총 인구는 <a href="https://en.wikipedia.org/wiki/World_human_population#/media/File:World_population_history.svg">70억 이었습니다</a>.)</p>
</div>

<p>HTML 코드에서 기능에 따라 콘텐츠 섹션을 마크업할 수 있습니다. 위에서 설명한 콘텐츠 섹션을 나타내는 요소를 명확하게 사용할 수 있으며 스크린리더와 같은 보조 기술은 이러한 요소를 인식하고 "기본 탐색 찾기" 또는 "주요 콘텐츠 찾기" 과정의 앞부분에서 언급했듯이 올바른 작업에 올바른 요소 구조와 의미 체계를 사용하지 않으면 여러 가지 올바르지 않은 결과가 발생합니다.</a>.</p>

<p>이러한 시맨틱 마크업을 구현하기 위해 HTML은 이러한 섹션을 나타내는 데 사용할 수 있는 전용 태그를 제공합니다. 예시:</p>

<ul>
 <li><strong>header: </strong>{{htmlelement("header")}}.</li>
 <li><strong>navigation bar: </strong>{{htmlelement("nav")}}.</li>
 <li><strong>main content: </strong>{{htmlelement("main")}}, {{HTMLElement("article")}}, {{htmlelement("section")}},{{htmlelement("div")}} 의 다양한 하위 섹션이 있습니다.</li>
 <li><strong>sidebar: </strong>{{htmlelement("aside")}}; 종종 {{htmlelement("main")}}안에 위치합니다.</li>
 <li><strong>footer: </strong>{{htmlelement("footer")}}.</li>
</ul>

<h3 id="Active_learning_exploring_the_code_for_our_example">능동적 학습: 예제 코드 탐색</h3>

<p>위에서 본 예제는 다음 코드로 표시됩니다(<a href="https://github.com/mdn/learning-area/blob/master/html/introduction-to-html/document_and_website_structure/index.html ">Github 리포지토리에서 예제를 찾아보세요</a>). 위의 예를 살펴본 다음 아래 목록을 살펴보고 어떤 부분이 시각적 개체의 어떤 섹션을 구성하는지 확인하시기 바랍니다.</p>
...
<p>코드를 살펴보고 이해하세요. 코드 내부의 주석도 코드를 이해하는 데 도움이 될 것입니다. 문서 레이아웃을 이해하는 열쇠는 건전한 HTML 구조를 작성한 다음 CSS로 레이아웃하는 것이기 때문에 이 기사에서 다른 많은 작업을 요구하지 않습니다. CSS 주제의 일부로 CSS 레이아웃을 연구하기 시작할 때까지 기다리겠습니다.</p>

<h2 id="HTML_레이아웃_요소의_세부_사항">HTML 레이아웃 요소의 세부 사항</h2>

<p>모든 HTML 섹션 요소의 전반적인 의미를 자세히 이해하는 것이 좋습니다. 이것은 웹 개발에 대한 더 많은 경험을 얻기 시작하면서 점차적으로 작업하게 될 것입니다. HTML 요소 참조를 읽으면 많은 세부 정보를 찾을 수 있습니다. 지금으로서는 다음과 같은 주요 정의를 이해하려고 노력해야 합니다:</p>
...
<h3 id="Non-semantic_wrappers">의미 없는 래퍼</h3>

<p>때로는 일부 항목을 함께 그룹화하거나 일부 콘텐츠를 래핑하는 데 이상적인 의미론적 요소를 찾을 수 없는 상황이 발생합니다. 때로는 요소 집합을 그룹화하여 일부 요소가 있는 단일 엔터티로 모든 요소에 영향을 미치기를 원할 수 있습니다.{{glossary("CSS")}} 또는  {{glossary("JavaScript")}}. 이와 같은 경우 HTML은 다음을 제공합니다. {{HTMLElement("div")}} 과 {{HTMLElement("span")}} 엘리먼트. 당신은 적합하게 이것들을 사용해야 합니다 {{htmlattrxref('class')}}속성은 쉽게 타겟팅할 수 있도록 일종의 레이블을 제공합니다.</p>

<p>{{HTMLElement("span")}} 엘리먼트는 내용을 감싸는 더 나은 의미 체계 텍스트 요소가 생각나지 않거나 특정 의미를 추가하고 싶지 않은 경우에만 사용해야 하는 인라인 비의미적 요소입니다. For example:</p>
...
<p>이것은 페이지의 주요 콘텐츠와 반드시 관련이 있는 것은 아니기 때문에 실제로 <aside>가 아닙니다(어디서나 볼 수 있기를 원합니다.) 그것은 일부가 아니기 때문에 <code>&lt;section&gt;</code>을 사용하는 것을 특별히 보증하지도 않습니다. 페이지의 주요 내용입니다. 따라서 이 경우 <code>&lt;div&gt;</code>가 좋습니다. 스크린리더 사용자가 쉽게 찾을 수 있도록 푯말로 제목을 포함했습니다.</p>

<div class="warning">
<p><strong>경고</strong>: Div는 사용하기 너무 편리해서 너무 많이 사용하기 쉽습니다. 의미론적 가치가 없기 때문에 HTML 코드를 어지럽힐 뿐입니다. 더 나은 의미론적 솔루션이 없을 때만 사용하도록 주의하고 사용을 최소로 줄이십시오. 그렇지 않으면 문서를 업데이트하고 유지 관리하는 데 어려움을 겪을 수 있습니다.</p>
</div>

<h3 id="Line_breaks_and_horizontal_rules">Line breaks and horizontal rules</h3>

<p>줄바꿈에 사용하는 두 가지 요소는 다음과 같습니다. {{htmlelement("br")}} and {{htmlelement("hr")}}:</p>

<p><code>&lt;br&gt;</code> 단락에 줄 바꿈을 만듭니다. 우편 주소나 시와 같이 일련의 고정된 짧은 줄을 원하는 상황에서 단단한 구조를 강제하는 유일한 방법입니다. For example:</p>
...
<p><code>&lt;br&gt;</code> elements, the paragraph would just be rendered in one long line (as we said earlier in the course, <a href="/en-US/Learn/HTML/Introduction_to_HTML/Getting_started#Whitespace_in_HTML">HTML ignores most whitespace</a>); with them in the code, the markup renders like this:</p>
...
<p><code>&lt;hr&gt;</code> 요소는 텍스트의 주제 변경(예: 주제 또는 장면의 변경)을 나타내는 문서에 수평선을 만듭니다. 시각적으로 수평선처럼 보입니다. As an example:</p>
...
<p>간단한 웹페이지의 콘텐츠를 계획했다면 다음 논리적 단계는 최고의 사용자 경험을 위해 전체 웹사이트에 어떤 콘텐츠를 넣고 싶은지, 어떤 페이지가 필요한지, 어떻게 정렬하고 서로 연결해야 하는지 알아내는 것입니다. 이를 {{glossary("Information architecture")}}라고 합니다. 크고 복잡한 웹 사이트에서는 이 프로세스에 많은 계획이 포함될 수 있지만 몇 페이지로 된 간단한 웹 사이트에서는 상당히 간단하고 재미있을 수 있습니다!</p>
...
<h3 id="Active_learning_create_your_own_sitemap">능동적인 학습: 나만의 사이트맵 만들기</h3>

<p>무엇에 대한 사이트를 만들고 싶습니까? 자신이 만든 웹사이트에 대해 위의 예를 수행해 보십시오.</p>
...
<p>이 시점에서 웹 페이지/사이트를 구성하는 방법에 대해 더 잘 알고 있어야 합니다. 이 모듈의 마지막 기사에서는 HTML을 디버그하는 방법을 연구할 것입니다.</p>
