...
사용자가 웹사이트를 열고 상호작용할 때, HTML, JavaScript, CSS, 이미지, 폰트 및 앱이 명시적으로 요청한 모든 데이터를 포함한 웹사이트에 필요한 모든 리소스는 HTTP(S) 요청을 통해 가져옵니다. PWA의 가장 기본적인 기능 중 하나는 앱의 일부 리소스를 기기에 명시적으로 캐싱할 수 있는 능력으로, 이는 네트워크에 요청을 보내지 않고도 해당 리소스를 가져올 수 있음을 의미합니다.

리소스를 로컬에 캐싱하는 데는 두 가지 주요 이점이 있습니다: **오프라인 동작**과 **반응성**입니다.
...
물론 주요 단점은 **최신성**입니다. 최신 상태를 유지해야 하는 리소스에는 캐싱이 적합하지 않습니다. 또한 {{HTTPMethod("POST")}} 요청과 같은 특정 유형의 요청에는 캐싱이 절대 적합하지 않습니다.

이는 리소스를 캐싱할지 여부와 시점이 해당 리소스에 크게 좌우되며, PWA는 일반적으로 리소스별로 다른 전략을 채택함을 의미합니다. 본 가이드에서는 PWA의 일반적인 캐싱 전략을 살펴보고, 어떤 전략이 어떤 리소스에 적합한지 알아보겠습니다.
...
- **서비스 워커의 `install` 이벤트 핸들러에서 (프리캐싱)**: 서비스 워커가 설치되면 브라우저는 서비스 워커의 전역 범위에서 {{domxref("ServiceWorkerGlobalScope/install_event", "install")}} 이벤트를 발생시킵니다. 이때 서비스 워커는 리소스를 네트워크에서 가져와 캐시에 저장하는 방식으로 리소스를 _프리캐싱_할 수 있습니다.
...
이 전략에서는 일부 리소스를 미리 캐싱한 후, 해당 리소스에 대해서만 "캐시 우선" 전략을 구현합니다. 즉:
