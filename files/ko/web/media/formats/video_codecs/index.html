...
<table class="standard-table">
 ...
 <tbody>
  ...
  <tr>
   <th scope="row"><a href="#mpeg-1">MPEG-1</a></th>
  ...
  <tr>
   <th scope="row"><a href="#mpeg-2">MPEG-2</a></th>
...
<table class="standard-table">
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>이미지 상에 보여지는 움직임이 얼마나 부드러운지에 크게 영향을 끼칩니다. 프레임 레이트가 높을 수록 움직임이 더 부드럽고 현실에 가까워 집니다. 계속 높이다보면 압축의 의미가 없어지는 지점에 도달하게 되죠. 아래의 <a href="#frame_rate">Frame rate</a> 절을 참조하세요.</td>
 ...
 <tbody>
  <tr>
   <td>
    ...
    <p><strong><a href="https://en.wikipedia.org/wiki/Moir%C3%A9_pattern">모아레 현상-Moiré pattern</a></strong>는 소스 이미지의 패턴과 인코더의 동작 방식이 공간적으로 약간 정렬되어 있지 않을 때 발생하는 대규모 공간 간섭 패턴입니다. 인코딩 시 발생한 아티팩트를 디코딩 하면 소스 이미지에 이상한 회오리 무늬가 생깁니다.</p>
   </td>
  ...
  <tr>
   <td>
    ...
    <p><strong>마차 바퀴 현상</strong> (또는 <strong><a href="https://ko.wikipedia.org/wiki/%EC%8A%A4%ED%8A%B8%EB%A1%9C%EB%B3%B4_%ED%9A%A8%EA%B3%BC">스트로보 효과</a></strong>)은 필름에서 흔히 볼 수 있는 시각적 효과로 프레임 레이트와 압축 알고리즘에 의해 회전하는 바퀴가 느리거나 빠르게 혹은 아예 반대 방향으로 보이는 것을 의미합니다. 이는 철도 노선의 침목이나 도로변의 기둥 등 일정한 패턴으로 움직이는 것이라면 어디서든 볼 수 있습니다. 이는 시간적 위신호 이슈로;압축 또는 인코딩 시 샘플링 레이트가 회선 속도에 간섭하여 발생합니다.</p>
   </td>
...
<p>손실 압축 알고리즘은 링잉 효과 <strong><a href="https://en.wikipedia.org/wiki/Ringing_artifacts">ringing artifacts</a></strong>를 일으킬 수 있습니다. 링잉 효과는 압축 알고리즘에 의해 오브젝트의 경계면에 픽셀이 오염되는 현상을 의미합니다. 압축 알고리즘이 오브젝트와 배경의 경계면이 포함된 블럭을 사용했을 때 발생할 수 있습니다. 보통 압축율이 높을 때 주로 발생합니다.</p>
...
<p>위 별 모양의 경계 부분에 파랑 및 분홍 부분을 보세요 (계단 현상 등 다른 압축 아티팩트도 나타남). 저 부분이 링잉 효과입니다. 링잉은 어떤 면에서는 <a href="#mosquito_noise">mosquito noise</a>와 비슷합니다, 다만 모기 효과는 일렁거리거나 움직이는데 반해 링잉 효과는 정지한 채로 변하지 않습니다.</p>
...
<p><strong>모스키토 노이즈</strong>는 시간적 아티팩트 중 하나로 배경과 물체의 경계면의 차이가 큰 부분에서 노이즈나 <strong>edge busyness</strong>가 흐릿하게 일렁거리는 현상을 의미합니다. 시각적으로는  <a href="#ringing">ringing</a> 효과와 유사합니다.</p>
...
<p><strong><a href="https://ko.wikipedia.org/wiki/%EC%9B%80%EC%A7%81%EC%9E%84_%EB%B3%B4%EC%83%81">움직임 보상</a></strong>은 물체가 각각의 방향으로 얼마만큼 많은 픽셀이 이동했는지 움직임(카메라 자체의 이동 또는 프레임 상의 물체의 이동)을 추적하는 기술입니다.  그리고 단순 움직임 만으로는 설명할 수 없는 픽셀의 추가 정보와 함께 움직임을 저장합니다. 요약하자면 인코더가 움직이는 물체를 찾아낸 후 원본과 동일해 보이지만 새로운 위치로 이동한 인터널 프레임을 생성하는 방식입니다. 이론적으로는 새로운 프레임이 나타난 것과 거의 동일합니다. 새 프레임에 남아있는 다른 차이점이 발견된다면 물체의 움직임과 픽셀 차이점을 저장하여 작업을 마무리 합니다. 이렇게 움직임과 픽셀 차이점이 기록된 물체를 <strong>residual frame</strong>이라 부릅니다.</p>
...
<p><a id="av1-foot-1" name="av1-foot-1">[1]</a> <a href="https://en.wikipedia.org/wiki/ISO_base_media_file_format">ISO Base Media File Format</a></p>
...
<table class="standard-table">
 <tbody>
  ...
  <tr>
   <th scope="row">HDR 지원</th>
   <td>예; <a href="https://en.wikipedia.org/wiki/Hybrid_log%E2%80%93gamma">Hybrid Log-Gamma</a> 또는 Advanced HDR/SL-HDR; 모두  ATSC의 파트</td>
  </tr>
  <tr>
   <th scope="row">가변 프레임 레이트 (VFR) 지원</th>
   <td>예</td>
...
<p><strong><a href="http://hevc.info/">High Efficiency Video Coding</a></strong> (<strong>HVEC</strong>) 코덱은 ITU의 <strong>H.265 </strong>및 MPEG-H Part 2 (MPEG-4 후속 작업으로 개발 진행 중). HEVC는 현대 프로세서의 특정점을 십분 활용하여 소프트웨어가 (8K 비디오를 포함한) 초고해상도 비디오 인코딩/디코딩을 효율적으로 할 수 있도록 설계하였습니다. 이론적으로 HEVC는 <a href="#avc">AVC</a>와 유사한 품질을 유지하면서 절반 크기로 압축할 수 있습니다.</p>
...
<p>MPEG-2는 서로 다른 스펙을 가진 여러 프로파일이 있습니다. 각 프로파일은 4개의 레벨을 가지고 있으며 프레임 레이트, 해상도, 비트레이트등의 비디오 속성 값을 증가시킬 수 있습니다. 대부분의 프로파일은 Y'CbCr 4:2:0 크로마 서브샘플링을 쓰지만 더 상위의 프로파일은 4:2:2를 지원한다든가 말입니다. 추가로 대형 프레임 크기 및 비트레이트 지원을 위한 4개의 레벨이 있습니다. 예를 들어 북미 지역의 <a href="https://en.wikipedia.org/wiki/ATSC_standards">ATSC standards</a> TV 스펙은 Main Profile at high Level을 통해 1920 x 1080 (30 FPS) 및 1280 x 720 (60 FPS)의 고화질을 최대 80 Mbps 비트레이트로 지원합니다.</p>
...
<p><strong>Video Processor 8</strong> (<strong>VP8</strong>) 코덱은 최초 On2 Technologies가 개발했습니다. Google은 On2 인수 후, VP8 관련된 특허와 무관하게 완전한 오픈 로열티-프리 라이선스로 출시했습니다. 압축률과 품질의 면에서 VP8은 <a href="#avc">AVC</a>에 견줄만 합니다.</p>
...
<p>VP9는 광범위한 브라우저가 지원하고 있으며 하드웨어 구현체로 상당히 퍼져있습니다. VP9는 <a href="/en-US/docs/Web/Media/Formats/Containers#WebM">WebM</a>에서만 사용할 수 있는 두 코덱 중 하나입니다(나머지 하나는 <a href="#vp8">VP8</a>). 하지만 Safari는 WebM 및 VP9 모두 지원하지 않으므로 VP9를 사용할 경우 iPhone, iPad, Mac에서 AVC나 HEVCS등 다른 포맷을 대체할 수 있도록 만들어야 합니다.</p>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>
    ...
    <p>지원 색 공간: <a href="https://en.wikipedia.org/wiki/Rec._601">Rec. 601</a>, <a href="https://en.wikipedia.org/wiki/Rec._709">Rec. 709</a>, <a href="https://en.wikipedia.org/wiki/Rec._2020">Rec. 2020</a>, <a href="https://en.wikipedia.org/wiki/NTSC#SMPTE_C">SMPTE C</a>, SMPTE-240M (obsolete; replaced by Rec. 709), <a href="https://en.wikipedia.org/wiki/SRGB">sRGB</a>.</p>
   </td>
  </tr>
  <tr>
   <th scope="row">HDR 지원</th>
...
<ol>
 <li>
  <p><strong><a href="/en-US/docs/Web/Media/Formats/Containers#WebM">WebM</a></strong> 컨테이너에 비디오는 <strong><a href="#vp8">VP8</a>, 오디오는</strong> <strong><a href="/en-US/docs/Web/Media/Formats/Audio_codecs#Opus">Opus</a></strong> 코덱을 사용합니다. 이들은 로열티-프리 오픈 포맷이지만 최근 브라우저에서만 폭넓게 지원하는 경향이 있어 폴백 지원이 필수적입니다.</p>

  <pre class="brush: js">&lt;video controls src="filename.webm"&gt;&lt;/video&gt;
</pre>
 </li>
 <li>
  <p><strong><a href="/en-US/docs/Web/Media/Formats/Containers#MP4">MP4</a></strong> 컨테이너에 비디오 코덱은 <strong><a href="#avc">AVC</a></strong> (<strong>H.264</strong>) 오디오 코덱은 <strong><a href="/en-US/docs/Web/Media/Formats/Audio_codecs#AAC">AAC</a></strong> 를 사용합니다. MP4 컨테이너에 AVC, AAC 조합은 모든 주요 브라우저에서 폭넓게 지원하는 조합이며 대부분의 유즈케이스에서 좋은 품질을 보여주기 때문입니다. 하지만 라이선스 요구사항에 대해 컴플라이언스 이슈는 없는 지 확인이 필요하죠.</p>
...
<ol>
 ...
 <li>
  <p>MP4 컨테이너에 비디오 코덱으로 <a href="#hevc">HEVC</a> 를 쓰되 프로파일은 Main 4:2:2 10/12 bit 색 깊이, 최대 Main 4:4:4 16 bit 색 깊이 수준의 고급 Main 프로파일을 사용합니다. 비트레이트를 높이면 놀라운 색 재현과 훌륭한 그래픽 퀄리티를 보여줄 것입니다. 또한 하이 다이나믹 레인지 비디오를 위한 HDR 메타데이터도 추가할 수 있습니다. 오디오는 ACC 인코딩 시 높은 샘플링 레이트(최소 48 kHz, 96 kHz 권장)에 fast-encoding이 아닌 complex-encoding을 사용합니다.</p>
...
<p>여러분의 웹 사이트나 앱 외부 영역에 보존 목적의 비디오라면 무압축 원본 비디오 데이터를 압축하는 유틸리티를 사용하세요. 예를들어 <a href="https://www.videolan.org/developers/x264.html">x264</a> 유틸리티는 매우 높은 비트레이트로 <a href="#avc">AVC</a> 인코딩을 할 수 있습니다:</p>
...
<p>무손실에 가까운 비디오를 보여줘야 한다는 제약이 있다면, <a href="#avc">AVC</a> 또는 <a href="#av1">AV1</a>를 고려해 볼 필요가 있습니다. 예를들어 비디오를 녹화하기 위해 <a href="/en-US/docs/Web/API/MediaStream_Recording_API">MediaStream Recording API</a>를 사용한다면, {{domxref("MediaRecorder")}} 객체를 생성하는 코드는 아래와 같습니다:</p>
...
<table class="standard-table">
 ...
 <tbody>
  <tr>
   ...
   <td>4문자 코드 (4CC-4 Character Code) <a href="#av1">AV1</a> 코덱을 의미합니다.</td>
  </tr>
  <tr>
   <td><code>2</code></td>
