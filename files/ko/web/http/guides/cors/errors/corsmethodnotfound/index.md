...
> [!NOTE]
> 서버의 `Access-Control-Allow-methods` 헤더에 인식되지 않거나 정의되지 않은 메서드 이름이 포함된 경우, 다른 오류가 발생합니다. [`Reason: invalid token 'xyz' in CORS header 'Access-Control-Allow-Methods'`](/ko/docs/Web/HTTP/Guides/CORS/Errors/CORSInvalidAllowMethod).
