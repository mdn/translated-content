...
`POST` 요청은 보통 [HTML 양식](/ko/docs/Learn_web_development/Extensions/Forms)을 통해 서버에 전송하며, 서버에 변경사항을 만듭니다. 이 경우의 콘텐츠 유형(`Content-Type`)은 _{{HTMLElement("form")}} 요소의 [`enctype`](/ko/docs/Web/HTML/Element/form#enctype) 특성이나 {{HTMLElement("input") }}, {{HTMLElement("button")}}_ 요소의 _[`formenctype`](/ko/docs/Web/HTML/Element/input#formenctype) 특성 안에 적당한 문자열을 넣어 결정합니다._
