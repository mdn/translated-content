...
`<details>` 요소는 HTML 요소에서 지원하는 일반적인 이벤트 외에도 열림과 닫힘 상태가 변경될 때마다 `<details>` 요소로 전송하는 {{domxref("HTMLElement/toggle_event", "toggle")}} 이벤트를 지원합니다. 이 이벤트는 상태가 변경된 _후에_ 전송되지만, 브라우저가 이벤트를 전송하기 전에 상태가 여러 번 변경되는 경우, 이벤트는 하나로 합쳐져 하나만 전송됩니다.
...
> [!NOTE]
> {{cssxref(":open")}} 가상 요소를 지원하지 않는 브라우저에서는, `details[open]` 특성 선택자를 사용해 `<details>` 요소가 열린 상태일 때 스타일을 지정할 수 있습니다.
