...
**HTML `<abbr>` 요소**는 준말 또는 머리글자를 나타냅니다. 선택 속성인 [`title`](/ko/docs/Web/HTML/Reference/Global_attributes#title)을 사용하면 준말의 전체 뜻이나 설명을 제공할 수 있습니다. `title` 속성은 전체 설명만을 가져야 하며 다른건 포함할 수 없습니다.
...
이 요소는 [전역 특성](/ko/docs/Web/HTML/Reference/Global_attributes)만 포함합니다. 단, 다른 요소와 달리 `<abbr>` 요소에서의 [`title`](/ko/docs/Web/HTML/Reference/Global_attributes#title) 특성은 특정한 의미를 가지며, `title`은 준말에 대한 설명 혹은 확장 형태를 사람이 읽을 수 있는 형태를 값으로 가져야 합니다. 브라우저는 `title`의 값을 보통 마우스 커서를 올렸을 때 나타나는 툴팁으로 보여줍니다.
...
- 사용한 준말과 머리글자를 문서 콘텐츠 플로우 밖에서 풀어서 설명하고 싶으면 적절한 [`title`](/ko/docs/Web/HTML/Reference/Global_attributes#title) 특성을 지정한 `<abbr>` 요소를 사용하세요.
...
[`title`](/ko/docs/Web/HTML/Reference/Global_attributes#title) 특성을 사용하면 준말과 머리글자를 펼친 원래 형태를 보여줄 수 있습니다.
