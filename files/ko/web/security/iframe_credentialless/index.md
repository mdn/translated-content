...
교차 출처 격리를 선택하려면 `same-origin`(공격자로부터 출처를 보호) 값을 갖는 {{httpheader("Cross-Origin-Opener-Policy")}}와 `credentialless`나 `require-corp`(원본으로부터 피해자를 보호) 값을 갖는 {{httpheader("Cross-Origin-Embedder-Policy")}}를 사용하여 리소스를 제공해야 합니다. 후자는 {{httpheader("Cross-Origin-Resource-Policy")}} 또는 [교차 출처 리소스 공유](/ko/docs/Web/HTTP/Guides/CORS)를 사용하여 문서에 명시적으로 권한을 부여하지 않은 인증된 교차 출처 리소스를 문서에 로드하지 못하도록 합니다.
...
- [교차 출처 리소스 공유](/ko/docs/Web/HTTP/Guides/CORS)
