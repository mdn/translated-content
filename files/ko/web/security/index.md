...
- [콘텐츠 보안 정책 (CSP)](/ko/docs/Web/HTTP/Guides/CSP)
...
- [전송 계층 보안 (TLS)](/ko/docs/Web/Security/Defenses/Transport_Layer_Security)
...
- [HTTP Strict-Transport-Security](/ko/docs/Web/HTTP/Reference/Headers/Strict-Transport-Security)
...
- [혼합 콘텐츠](/ko/docs/Web/Security/Defenses/Mixed_content)
...
- [혼합 콘텐츠가 차단된 웹사이트를 고치는 방법](/ko/docs/orphaned/Web/Security/Mixed_content/How_to_fix_website_with_mixed_content)
  - : 웹 사이트에가 HTTPS 페이지를 제공한다면, HTTP를 통해 제공되는 모든 [혼합 활성 콘텐츠](/ko/docs/Web/Security/Defenses/Mixed_content)는 기본적으로 차단됩니다. 결과적으로 여러분의 웹사이트는 사용자에게 깨진 것처럼 보일 수 있습니다(iframe 또는 플러그인이 로드되지 않는 경우 등). [혼합 패시브 콘텐츠](/ko/docs/Web/Security/Defenses/Mixed_content)는 기본적으로 표시되지만 사용자는 이러한 유형의 콘텐츠를 차단하도록 기본 설정을 지정할 수도 있습니다. 이 페이지에서는 웹 개발자로서 알아야 할 사항을 살펴봅니다.
...
- [HTTP 쿠키 사용](/ko/docs/Web/HTTP/Guides/Cookies)
...
- [리퍼러 헤더 정책: 개인 정보 보호 및 보안 문제](/ko/docs/Web/Privacy/Guides/Referer_header:_privacy_and_security_concerns)
  - : [Referer HTTP 헤더](/ko/docs/Web/HTTP/Reference/Headers/Referer)와 관련된 개인 정보 및 보안 위험이 있습니다. 이 문서에서는 이에 대해 설명하고 이러한 위험을 완화하는 방법에 대한 조언을 제공합니다.
- [자격 증명 없는 IFrame](/ko/docs/Web/HTTP/Guides/IFrame_credentialless)
...
- [하위 리소스 무결성](/ko/docs/Web/Security/Defenses/Subresource_Integrity)
...
- [HTTP Access-Control-Allow-Origin](/ko/docs/Web/HTTP/Reference/Headers/Access-Control-Allow-Origin)
...
- [HTTP X-Content-Type-Options](/ko/docs/Web/HTTP/Headers/X-Content-Type-Options)
  - : **`X-Content-Type-Options`** HTTP 응답 헤더는 {{HTTPHeader("Content-Type")}} 헤더에 광고된 [MIME 타입](/ko/docs/Web/HTTP/Guides/MIME_types) 유형을 변경허나 따르지 않아야 함을 나타내기 위해 서버에서 사용하는 마커입니다. 이는 [MIME 스니핑](/ko/docs/Web/HTTP/Guides/MIME_types#mime_스니핑)을 빠져나오는 방법입니다. 즉, MIME 유형이 의도적으로 구성되었음을 나타냅니다.

## 클릭재킹 보호

{{Glossary("Clickjacking", "클릭재킹")}}은 사용자가 기대하는 것과 다른 작업을 수행하는 UI 요소를 클릭하도록 속입니다.

- [HTTP X-Frame-Options](/ko/docs/Web/HTTP/Reference/Headers/X-Frame-Options)
