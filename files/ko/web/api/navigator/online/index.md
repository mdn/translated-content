...
[`navigator.onLine`](/ko/docs/Web/API/Navigator/onLine)는 `true`/`false`(온라인은 `true`, 오프라인은 `false`)를 보관하는 속성입니다. 이 속성은 사용자가 해당 메뉴 항목(파일 -> 오프라인 작업)을 선택하여 "오프라인 모드"로 전환할 때마다 갱신됩니다.
...
- `window`, `document`, `document.body`에서 [`addEventListener`](/ko/docs/Web/API/EventTarget/addEventListener)를 이용
