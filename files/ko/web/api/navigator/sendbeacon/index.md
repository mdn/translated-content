...
분석 정보를 보낼 때의 문제는, 대개 사이트에서는 사용자가 다른 페이지로 이동할 때와 같이 사용자가 페이지를 이탈할 때 전송하길 바랍니다. 하지만 이탈 전송의 경우 브라우저가 페이지를 언로드하고자 하는 순간일 수도 있으므로, 브라우저가 재량에 따라 비동기적인 {{domxref("XMLHttpRequest")}} 요청을 보내지 않을 수도 있습니다.

과거 웹 페이지들은 데이터를 모두 보낼 수 있을 정도로 페이지의 언로딩을 지연시키기도 했습니다. 정확한 방법에는 아래와 같은 기법이 있었습니다.
