...
Web Audio API의 `AudioParam` 인터페이스는 오디오에 관련된 매개변수를 나타내는데, 보통 ({{ domxref("GainNode.gain") }}과 같은) {{domxref("AudioNode")}}의 매개변수입니다.
...
두 종류의 `AudioParam` 이 있는데, 그것은 _a-rate_ 와 _k-rate_ 매개변수입니다. 각 {{domxref("AudioNode")}}는 명세에서 이것의 매개변수들 중 어떤 것이 _a-rate_ 이고 어떤 것이 _k-rate_ 인지 정의합니다.

### a-rate

_a-rate_ `AudioParam` 은 오디오 신호의 각 [샘플 프레임](/ko/docs/Web/API/Web_Audio_API/Basic_concepts_behind_Web_Audio_API#audio_buffers.3a_frames.2c_samples_and_channels)에 대해 현재의 오디오 매개변수 값을 취합니다.

### k-rate

_k-rate_ `AudioParam` 은 처리되는 전체 블럭에 대해 같은 초기 오디오 매개변수 값을 사용합니다; 즉, 128 샘플 프레임입니다. 다른 말로 하자면, 오디오가 노드에 의해 처리되는 동안 같은 값이 오디오에 있는 모든 프레임에 적용됩니다.
...
- {{domxref("AudioParam.maxValue")}} {{readonlyInline}}
  - : 매개변수의 명목상의 (효과적인) 범위에 대한 최대로 가능한 값을 나타냅니다.
- {{domxref("AudioParam.minValue")}} {{readonlyinline}}
  - : 매개변수의 명목상의 (효과적인) 범위에 대한 최소로 가능한 값을 나타냅니다.
- {{domxref("AudioParam.value")}}
  - : 현재 시간에 대한 매개변수의 현재 값을 나타냅니다; 초기적으로는 {{domxref("AudioParam.defaultValue", "defaultValue")}}.의 값으로 설정됩니다.
