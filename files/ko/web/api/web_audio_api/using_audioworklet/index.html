...
<p>Web Audio API가 처음 브라우저에 소개되었을 때, 이것은 실시간으로 오디오를 조작하는 사용자 정의 오디오 프로세서를 생성하기 위해 JavaScript 코드를 사용하고 있었습니다. <code>ScriptProcessorNode</code>의 문제점은 단순했습니다: 이것은 메인 스레드에서 실행되기 때문에 실행을 마치기 전까지 다른 모든 동작을 막았습니다. 이러한 방식은 이상적이지 않았습니다. 특히 오디오 프로세싱같이 계산적으로 비용이 많이 들 수 있는 무언가에 대해서 말입니다.</p>
...
<ol>
 ...
 <li>{{domxref("AudioWorkletNode")}}가 필요로 하는, 또는 여러분이 설정하기를 원하는 오디오 파라미터들을 설정합니다. 이 파라미터들은 오디오 worklet 프로세서 모듈 내에 정의되어 있습니다.</li>
...
<p><a href="https://mdn.github.io/webaudio-examples/audioworklet/"><strong>직접 시도해보기</strong></a></p>
...
<p>오디오 프로세서 클래스의 가장 기본적인 프레임워크는 다음과 같습니다:</p>
...
<h2 id="Supporting_audio_parameters">오디오 파라미터 지원하기</h2>

<p>다른 Web Audio 노드들처럼, {{domxref("AudioWorkletNode")}}은 파라미터를 지원하는데, 이는 실제 작업을 하는 {{domxref("AudioWorkletProcessor")}}와 공유됩니다.</p>

<h3 id="Adding_parameter_support_to_the_processor">프로세서에 파라미터 지원 추가하기</h3>

<p>{{domxref("AudioWorkletNode")}}에 파라미터를 추가하기 위해서, 여러분은 모듈 내의 {{domxref("AudioWorkletProcessor")}} 기반의 프로세서 클래스 내에서 파라미터들을 정의할 필요가 있습니다. 이 작업은 static getter {{domxref("AudioWorkletProcessor.parameterDescriptors", "parameterDescriptors")}}를 클래스에 추가함으로써 이루어질 수 있습니다. 이 함수는 {{domxref("AudioParam")}} 객체들의 배열을 반환해야 하는데, 이 객체는 프로세서에 의해 지원되는 각 파라미터에 대해 하나씩입니다.</p>
...
<p>프로세서 노드의 파라미터들에 접근하는 것은 그것들을 {{domxref("AudioWorkletProcessor.process", "process()")}}의 구현에 전달된 <code>parameters</code> 객체에서 찾는 것만큼이나 간단합니다. <code>parameters</code> 객체 내부에 있는 것은 배열들인데, 각 파라미터 당 하나이고, 파라미터로서 같은 이름을 공유합니다.</p>

<dl>
 <dt>A-rate 파라미터</dt>
 <dd>a-rate 파라미터에 대해서—시간에 따라 값이 자동적으로 변하는 파라미터—<code>parameters</code> 객체에서 파라미터의 엔트리는 {{domxref("AudioParam")}} 객체들의 배열인데, 처리되고 있는 블록의 각 프레임 당 하나입니다. 이 값들은 해당하는 프레임들에 적용될 것입니다.</dd>
 <dt>K-rate 파라미터</dt>
 <dd>반면에 K-rate 파라미터는, 블럭당 한 번만 변할 수 있어서, 파라미터의 배열은 오직 하나의 엔트리를 가집니다. 블럭의 모든 프레임에 대해 이 값을 사용하십시오.</dd>
</dl>
...
<p>여기서, 만약 <code>gain.length</code>가 <code>gain</code> 파라미터의 값들의 배열에 오직 하나의 값만이 있다는 것을 나타내면, 배열에 있는 첫번째 엔트리가 블럭의 모든 프레임에 적용됩니다. 그렇지 않으면, 블럭의 각 프레임에 대해, <code>gain[]</code>에 있는 해당하는 엔트리가 적용됩니다.</p>

<h3 id="Accessing_parameters_from_the_main_thread_script">메인 스레드 스크립트에서 파라미터 접근하기</h3>

<p>여러분의 메인 스레드 스크립트는 다른 노드들이 할 수 있는 것과 마찬가지로 파라미터에 접근할 수 있습니다. 이렇게 하기 위해서는, 첫째로 여러분은 {{domxref("AudioWorkletNode")}}의 {{domxref("AudioWorkletNode.parameters", "parameters")}} 속성의 {{domxref("AudioParamMap.get", "get()")}} 메서드를 호출함으로써 파라미터에 대한 참조를 얻을 필요가 있습니다.</p>

<pre class="brush: js">let gainParam = myAudioWorkletNode.parameters.get("gain");
</pre>

<p><code>gainParam</code>에 반환되고 저장된 값은 <code>gain</code> 파라미터를 저장하기 위해 사용된 {{domxref("AudioParam")}}입니다. 여러분은 그리고 나서 {{domxref("AudioParam")}} 메서드 {{domxref("AudioParam.setValueAtTime", "setValueAtTime()")}}을 사용하여 주어진 시간의 값을 효과적으로 바꿀 수 있습니다.</p>
...
<p>파라미터의 값을 읽는 것은 {{domxref("AudioParam.value", "value")}} 속성을 보는 것 만큼이나 간단합니다:</p>
...
<ul>
 ...
 <li><a href="https://developers.google.com/web/updates/2017/12/audio-worklet">Enter Audio Worklet</a> (Google Developers 블로그)</li>
</ul>
