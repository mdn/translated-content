...
- `value`
  - : 필드의 값입니다. 파라미터가 두 개인 버전에서는 {{jsxref("String")}}이고, 그렇지 않은 경우 문자열로 변환됩니다. 파라미터가 세 개인 버전에서는 {{domxref("Blob")}}, {{domxref("File")}}, {{jsxref("String")}}이 될 수 있습니다. 이들 중 어느것도 지정되지 않으면 값은 문자열로 변환됩니다.
- `filename` {{optional_inline}}
  - : {{domxref("Blob")}} 또는 {{domxref("File")}} 이 두 번째 매개 변수로 전달될 때, 서버에 리포트하는 파일 이름(a {{jsxref("String")}})입니다. {{domxref("Blob")}} 객체의 기본 파일 이름은 "blob"입니다.
