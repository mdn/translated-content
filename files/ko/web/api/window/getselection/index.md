...
위 예제에서 selObj를 {{domxref("window.alert()")}}의 인자로 전달하면 `selObj.toString()`가 자동적으로 호출됩니다. 하지만, selObj에 대해서 [`length`](/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/length "JS/String.length") 나 [`substr`](/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/substr "JS/String.substr") 같은 JavaScript [String](/ko/docs/Web/JavaScript/Reference/Global_Objects/String "JS/String")의 속성이나 메소드를 사용하면 객체에 그러한 속성이나 메소드가 없기 때문에 에러나 예상치 못한 결과가 발생합니다. `Selection` 객체를 문자열로 사용하려면 다음처럼 직접 `toString()` 메소드를 호출해야 합니다:
