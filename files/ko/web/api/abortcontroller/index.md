...
- {{domxref("AbortController.abort()")}}
  - : DOM 요청이 완료되기 전에 취소한다. 이를 통해 [fetch 요청](/ko/docs/Web/API/Window/fetch), 모든 응답 {{domxref("Body")}} 소비, 스트림을 취소할 수 있다.
...
> [!NOTE]
> `abort()`가 호출되면, `fetch()` promise는 `AbortError`으로 명명된 `DOMException`과 함께 reject된다.
