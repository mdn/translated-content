...
가비지 콜렉션 알고리즘의 핵심 개념은 *참조*입니다. A라는 메모리를 통해 (명시적이든 암시적이든) B라는 메모리에 접근할 수 있다면 "B는 A에 참조된다" 라고 합니다. 예를 들어 모든 자바스크립트 오브젝트는 [prototype](/en/JavaScript/Guide/Inheritance_and_the_prototype_chain "en/JavaScript/Guide/Inheritance_and_the_prototype_chain") 을 암시적으로 참조하고 그 오브젝트의 속성을 명시적으로 참조합니다.
