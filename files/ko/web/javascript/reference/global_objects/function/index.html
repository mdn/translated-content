...
<p><strong><code>Function</code> 생성자</strong>는 새 <code>Function</code> 객체를 만듭니다. 이 생성자를 직접 호출하여 동적으로 함수를 생성할 수도 있으나, 보안 문제 및 {{jsxref("eval")}}과 유사한(그러나 훨씬 덜 심각한) 성능 문제가 발생할 수 있습니다. 하지만 <code>eval</code>과 달리, <code>Function</code> 생성자는 전역 범위로 한정된 함수만 생성합니다.</p>
...
<dl>
 ...
 <dt><s class="obsoleteElement">{{jsxref("Function.arity")}} {{deprecated_inline}}</s></dt>
