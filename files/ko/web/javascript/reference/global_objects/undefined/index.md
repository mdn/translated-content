...
> **참고:** `undefined`는 [예약어](/ko/docs/Web/JavaScript/Reference/Lexical_grammar#reserved_words)가 아니기 때문에 전역 범위 외에서 {{Glossary("Identifier", "식별자")}}(변수 이름)로 사용할 수 있습니다. 그러나 유지보수와 디버깅 시 어려움을 낳을 수 있으므로 반드시 피해야 합니다.
>
> ```js example-bad
> // DON'T DO THIS
>
...
> **참고:**: 동등 연산자가 아니라 일치 연산자를 사용해야 합니다. 동등 연산자일 때 `x == undefined`는 `x`가 `null`일 때도 참이기 때문입니다. 즉 `null`은 `undefined`와 동등하지만, 일치하지는 않습니다.자세한 내용은 [Equality comparison and sameness](/ko/docs/Web/JavaScript/Equality_comparisons_and_sameness) 문서를 확인하세요.
...
{{jsxref("Operators/typeof", "typeof")}}를 사용하는 이유 중 하나는 선언하지 않은 변수를 사용해도 오류를 던지지 않기 때문입니다.
