...
> [!NOTE]
> `Date` 객체의 중심을 구성하는 시간 값은 UTC 기준이지만, 날짜와 시간 등 구성 요소를
...
- {{jsxref("Date.parse()")}}
  - : 날짜를 나타내는 문자열을 분석한 후, 해당 날짜와 1970년 1월 1일 00:00:00 UTC의 시간 차이를 밀리초 단위의 숫자 값으로 반환합니다.
    ...
    > [!NOTE]
    > `Date.parse()`를 사용한 날짜 분석은 브라우저 간 차이 및 일관적이지 못한 동작을
...
> [!NOTE]
> `Date` 생성자(및 동일한 동작의 `Date.parse()`)를 사용한 날짜 분석은
...
> [!NOTE]
> {{domxref("Window.performance", "Web Performance API", "", 1)}}의 고해상도 시간 기능을
