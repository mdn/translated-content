...
`union()`은 `other` 매개변수로 [유사 집합](/ko/docs/Web/JavaScript/Reference/Global_Objects/Set#set-like_objects) 객체를 받습니다. 사용자 코드를 호출하지 않고 `this`에 저장된 기본 데이터를 직접 검색하기 때문에 {{jsxref("this")}}는 실제 {{jsxref("Set")}} 인스턴스여야합니다. 그런 다음 `keys()` 메서드를 호출하여 `other`을 순회하고 `this`의 모든 요소와 `other`에 없는 모든 요소로 새로운 집합을 구성합니다.
