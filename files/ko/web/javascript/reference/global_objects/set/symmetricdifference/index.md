...
수학적 표기법으로는 대칭차(symmetric difference)은 아래와 같이 표현할 수 있습니다.
...
`symmetricDifference()`는 `other` 매개변수로 [유사 Set](/ko/docs/Web/JavaScript/Reference/Global_Objects/Set#set-like_objects) 객체를 허용합니다. 어떠한 사용자 코드 호출없이 `this`에 저장된 데이터를 직접 검색하기 때문에 실제 {{jsxref("Operators/this", "this")}} 인스턴스여야 합니다. 그 다음 `keys()` 메서드를 호출하여 `other`를 순회하고 `other`에서 보이지 않는 `this`의 모든 요소와 `this`에서 보이지 않는 `other`의 모든 요소로 새 집합을 구성합니다.
