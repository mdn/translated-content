...
`union()`은 `other` 매개변수로 [유사 집합](/ko/docs/Web/JavaScript/Reference/Global_Objects/Set#set-like_objects) 객체를 받습니다. 사용자 코드를 호출하지 않고 `this`에 저장된 기본 데이터를 직접 검색하기 때문에 {{jsxref("this")}}는 실제 {{jsxref("Set")}} 인스턴스여야합니다. 그 다음의 행동은 `this`와 `other`의 크기에 달려있습니다.
