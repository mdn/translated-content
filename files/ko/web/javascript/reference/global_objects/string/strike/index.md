...
> **참고:** 모든 [HTML 래퍼 메서드](/ko/docs/Web/JavaScript/Reference/Global_Objects/String#html_wrapper_methods)는 더 이상 사용되지 않으며 호환성 목적으로만 표준화되었습니다. `strike()`의 경우, `<strike>` 요소 자체가 HTML 명세에서 제외되었기에 더 이상 사용해서는 안됩니다. 웹 개발자들은 삭제된 컨텐츠에는 {{HTMLElement("del")}}를, 더 이상 정확하지 않거나 연관되지 않은 컨텐츠에는 {{HTMLElement("s")}}를 대신 사용해야 합니다.
