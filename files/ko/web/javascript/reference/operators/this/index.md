...
이 예제에서, `f` 속성을 가지고 있지 않은 변수 `p`가 할당된 객체는, 프로토타입으로 부터 상속받는다. 그러나 그것은 결국 `o`에서 `f` 이름을 가진 멤버를 찾는 되는 문제가 되지 않는다 ; `p.f`를 찾아 참조하기 시작하므로, 함수 내부의 `this`는` p` 처럼 나타나는 객체 값을 취한다. 즉, `f`는 `p`의 메소드로서 호출된 이후로, `this`는 `p`를 나타낸다. 이것은 JavaScript의 프로토타입 상속의 흥미로운 기능이다.
