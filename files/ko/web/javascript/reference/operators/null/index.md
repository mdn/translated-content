...
`null`은 리터럴로서 `null`이라 씁니다. `null`은 {{jsxref("undefined")}}과 같이 글로벌 객체의 속성에 대한 식별자가 아닙니다. 대신 `null`은 식별되지 않은 것을 표현합니다. 즉, 변수가 아무런 객체를 가리키지 않음을 표현합니다. API에서는 `null`을 종종 관련된 객체가 존재하지 않을 때 그 객체 대신 사용합니다.
