...
`false`로 변환할 수 있는 표현식은 평가 결과가 {{jsxref("Operators/null", "null")}}, `0`, {{jsxref("NaN")}}, 빈
...
(`??`)를 사용할 수도 있습니다. 널 병합 연산자는 첫 번째 연산자가 "[널과 유사](/ko/docs/Glossary/Nullish)"한, 즉 {{jsxref("Operators/null", "null")}}이거나 {{jsxref("undefined")}}일 때만 두 번째 피연산자를 반환합니다. `''`와
