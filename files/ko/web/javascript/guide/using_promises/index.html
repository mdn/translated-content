...
<pre class="brush: js"><code>function successCallback(result) {
...
<pre class="brush: js"><code>createAudioFileAsync(audioSettings).then(successCallback, failureCallback);</code></pre>

<p>…조금 더 간단하게 써보자면:</p>

<pre class="brush: js"><code>const promise = createAudioFileAsync(audioSettings);
...
<p>보통 두 개 이상의 비동기 작업을 순차적으로 실행해야 하는 상황을 흔히 보게 됩니다. 순차적으로 각각의 작업이 이전 단계 비동기 작업이 성공하고 나서 그 결과값을 이용하여 다음 비동기 작업을 실행해야 하는 경우를 의미합니다. 우리는 이런 상황에서 <strong>promise chain</strong>을 이용하여 해결하기도 합니다.</p>
...
<p>'콜백 지옥'에서 <code>failureCallback</code>이 3번 발생한 것을 기억할 것입니다. promise chain에서는 단 한 번만 발생하는 것과 비교되죠.</p>

<pre class="brush: js"><code>doSomething()
.then(result =&gt; doSomethingElse(result))
.then(newResult =&gt; doThirdThing(newResult))
...
<pre class="brush: js"><code>async function foo() {
  try {
...
<p>이것은 promise를 기반으로 합니다. <code>doSomething()</code>은 이전 함수와 같습니다. 문법은 <a href="https://developers.google.com/web/fundamentals/getting-started/primers/async-functions">이곳</a>에서 확인할 수 있습니다.</p>
...
<p>특히 유용한 사례 : {{Glossary("Node.js")}}로 코드를 작성할 때, 흔히 프로젝트에서 사용하는 모듈이 reject된 프로미스를 처리하지 않을 수 있습니다. 이런 경우 노드 실행 시 콘솔에 로그가 남습니다. 이를 수집에서 분석하고 직접 처리할 수도 있습니다. 아니면 그냥 콘솔 출력을 어지럽히는 것을 막기 위해 그럴 수도 있죠. 이런 식으로 {{domxref("Window.unhandledrejection_event", "unhandledrejection")}}(<a href="/en-US/docs/Web/API/Window/unhandledrejection_event">영어</a>) 이벤트를 처리하는 핸들러를 추가하면 됩니다.</p>

<pre class="brush: js"><code>window.addEventListener("unhandledrejection", event =&gt; {
...
<pre class="brush: js"><code>setTimeout(() =&gt; saySomething("10 seconds passed"), 10000);</code>
</pre>
...
<pre class="brush: js"><code>const wait = ms =&gt; new Promise(resolve =&gt; setTimeout(resolve, ms));
...
<pre class="brush: js"><code>Promise.all([func1(), func2(), func3()])
.then(([result1, result2, result3]) =&gt; { /* use result1, result2 and result3 */ });</code></pre>

<p>고급진 JavaScript를 사용하여 순차적 구성이 가능합니다.</p>

<pre class="brush: js"><code>[func1, func2, func3].reduce((p, f) =&gt; p.then(f), Promise.resolve())
...
<pre class="brush: js"><code>const applyAsync = (acc,val) =&gt; acc.then(val);
...
<pre class="brush: js"><code>const transformData = composeAsync(func1, func2, func3);
const result3 = transformData(data);</code>
</pre>

<p>ECMAScript 2017에서는 async / await를 사용하여 순차적 구성을 보다 간단하게 수행할 수 있습니다.</p>

<pre class="brush: js"><code>let result;
...
<p>놀라움(역자 주. 에러가 난다거나, 코드가 문제가 생긴다거나..했을때의 그 놀라움..)을 피하기 위해 <code><a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/then">then()</a></code>에 전달된 함수는 already-resolved promise에 있는 경우에도 동기적으로 호출되지 않습니다.</p>

<pre class="brush: js"><code>Promise.resolve().then(() =&gt; console.log(2));
...
<p>간단한 promise 체인은 평평하게 유지하는 것이 가장 좋습니다. 중첩된 체인은 부주의한 구성의 결과일 수 있습니다. <a href="https://developer.mozilla.org/ko/docs/Web/JavaScript/Guide/Using_promises#Common_mistakes">common mistakes</a>를 참조하십시오.</p>
...
<pre class="brush: js"><code>doSomethingCritical()
...
<p>inner neutralizing <code>catch</code> 문은 <code>doSomethingOptional()</code>및 <code>doSomethingExtraNice()</code>에서 발생한 오류를 catch 한 후에 코드가 <code>moreCriticalStuff()</code>로 다시 시작됩니다. 중요한 것은 <code>doSomethingCritical()</code>이 실패하면 해당 오류는 최종 (외부) <code>catch</code>에서만 포착된다는 것입니다.</p>
...
<pre class="brush: js"><code>// Bad example! Spot 3 mistakes!
...
<p>두 번째 실수는 불필요하게 중첩되어 첫 번째 실수를 가능하게 만드는 것입니다. 또한 중첩은 내부 오류 처리기의 범위를 제한하며, 의도하지 않은 에러가 캐치되지 않는 오류가 발생할 수 있습니다. 이 변형은 <a href="https://stackoverflow.com/questions/23803743/what-is-the-explicit-promise-construction-antipattern-and-how-do-i-avoid-it">promise constructor anti-pattern</a>입니다. 이 패턴은 이미 약속을 사용하는 코드를 감싸기 위해 promise 생성자의 중복 사용과 중첩을 결합합니다.</p>
...
<p><a href="/ko/docs/Web/JavaScript/Reference/Statements/async_function"><code>async</code>/<code>await</code></a>를 사용하면 대부분의 문제를 해결할 수 있습니다. 이러한 문법의 가장 흔한 실수는 <a href="/ko/docs/Web/JavaScript/Reference/Statements/async_function"><code>await</code></a>키워드를 빼먹는 것입니다.</p>

<h2 id="promise와_작업이_충돌할_때">promise와 작업이 충돌할 때</h2>

<p>예측할 수 없는 순서로 실행되는 promise 및 작업(예: 이벤트 또는 콜백)이 있는 상황에 직면하면 마이크로 태스크를 사용하여 상태를 확인하거나 promise가 조건부로 생성될 때 promise의 균형을 맞추는 것이 좋습니다.</p>

<p>마이크로 태스크가 이 문제를 해결하는 데 도움이 될 수 있다고 생각되면, <a href="/ko/docs/Web/API/queueMicrotask">queueMicrotask()</a>를 사용해서 함수를 마이크로 태스크로 대기열에 넣는 방법을 <a href="/ko/docs/Web/API/HTML_DOM_API/Microtask_guide">마이크로 태스크 가이드</a>에서 자세히 알아보십시오.</p>
