...
JavaScript의 스크립트 소스는 왼쪽에서 오른쪽으로 탐색하면서 "토큰", "제어 문자", "줄바꿈 문자", "주석"이나 {{glossary("whitespace", "공백")}}으로 이루어진 입력 요소의 시퀀스로 변환됩니다. (스페이스, 탭, 줄바꿈 문자는 공백으로 간주됩니다.)
...
- {{jsxref("Statements/var", "var")}} 키워드로. 예를 들어, `var x = 42`. 이 구문은 "실행 맥락"에 따라  **지역 및 전역 변수**를 선언하는데 모두 사용될 수 있습니다.
- {{jsxref("Statements/const", "const")}} 혹은 {{jsxref("Statements/let", "let")}} 키워드로. 예를 들어, `let y = 13`. 이 구문은 블록 스코프 지역 변수를 선언하는데 사용될 수 있습니다. 아래 [변수 스코프](#변수_스코프)를 참고하세요.
...
`undefined` 값은 불리언 문맥(context)에서 사용될 때 `false`로 동작합니다. 예를 들어, 아래 코드는 `myArray` 요소가 `undefined`이므로 `myFunction` 함수를 실행합니다.
...
{{jsxref("null")}} 값을 평가할 때, 수치 문맥에서는 `0`으로, 불리언 문맥에서는 `false`로 동작합니다. 예를 들면,
...
예를 들어서 아래의 코드는 `5`라는 로그가 남는데. `x`의 스코프가 전역 문맥 (혹은 코드가 함수의 일부분이라면 함수 문맥)이기 때문입니다. `x`의 스코프는 `if`문 블록에 제한되지 않습니다.
...
함수에서는 단지 함수 "선언"만 상단으로 끌어올려집니다. 함수 "표현식"은 그렇지 "않습니다".
...
전역 변수는 사실 "전역 객체"의 속성(property)입니다. 

웹 페이지에서 전역 객체는 {{domxref("window")}} 이므로, `windows.variable` 구문을 통해 전역 변수를 설정하고 접근할 수 있습니다.

그 결과, window 혹은 frame의 이름을 지정하여 한 window 혹은 frame에서 다른 window 혹은 frame에 선언된 전역 변수에 접근할 수 있습니다. 예를 들어, `phoneNumber` 라는 변수가 문서에 선언된 경우, `iframe`에서 `parent.phoneNumber`로 이 변수를 참조할 수 있습니다.
...
JavaScript에서 값을 나타내기 위해 "리터럴"을 사용합니다. 이는 "말 그대로" 스크립트에 부여한 고정 값으로, 변수가 아닙니다. 이 구획에서는 다음과 같은 형태의 리터럴을 설명합니다.
...
정수와 {{jsxref("BigInt")}} 리터럴은 10진, 16진, 8진 및 2진수로 표현될 수 있습니다.

- "10진" 정수 리터럴은 선행 `0` (zero)이 아닌 숫자열로 이루어집니다.
- 정수 리터럴에서 선행 `0` (zero)이나 선행 `0o` (혹은 `0O`)은 "8진수"임을 나타냅니다. 8진 정수는 오직 숫자 `0`-`7`만 포함할 수 있습니다.
- 선행 `0x`(나 `0X`)는 "16진" 정수 리터럴 임을 나타냅니다. 16진 정수는 숫자 (`0`-`9`) 및 문자 `a`-`f`, `A`-`F`를 포함할 수 있습니다. (문자의 대소문자는 그 값을 변경하지 않습니다. 그러므로 `0xa` = `0xA` = `10` 그리고 `0xf` = `0xF` = `15` 입니다.)
- 선행 `0b` (나 `0B`)는 "2진" 정수 리터럴 임을 나타냅니다. 2진 정수 리터럴은 오직 숫자 `0`과 `1`만 포함할 수 있습니다.
- 정수 리터럴의 후행 `n` 접미사는 {{jsxref("BigInt")}} 리터럴 임을 나타냅니다. 정수 리터럴은 위의 어떤 진수든 사용할 수 있습니다. `0123n`과 같은 선행 `0` 8진수 구문은 허용되지 않으나, `0o123n`은 허용됩니다. 
...
또한 유효한 식별자가 아닌 속성명은 점(`.`) 속성으로 접근할 수 없습니다. 대신 배열 같은 표기법("`[]`")으로 접근하고 값을 설정"할 수 있습니다".
...
전행 백슬래시와 함께 문자열 안에 따옴표를 사용할 수 있습니다. 이것을 따옴표 "이스케이프"라고 합니다. 예를 들어,
