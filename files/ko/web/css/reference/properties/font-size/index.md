...
- {{cssxref("&lt;length&gt;")}}
  - : {{cssxref("&lt;length&gt;")}}는 항상 양수입니다. 만약 단위가 `em` 이나 `ex` 로 표현되어 있으면, 부모 엘리먼트의 크기에 상대적으로 정해집니다.

    만약 단위가 `rem` 으로 지정되어 있다면, 폰트 크기는 {{HTMLElement("html")}} (루트) 엘리먼트의 폰트 크기에 따라 상대적으로 정해집니다.
...
키워드는 웹의 폰트 크기를 정하는 좋은 방법입니다. 키워드로 {{HTMLElement("body")}} 엘리먼트의 폰트 크기를 정하면 웹 페이지의 모든 페이지의 폰트 크기가 상대적으로 지정되어 쉽게 전체 페이지의 폰트 크기를 늘이거나 줄일 수 있습니다.
...
폰트 크기를 설정하는 또 하나의 방법은 `em` 값으로 설정하는 것입니다. `em` 값은 동적입니다. `font-size` 속성을 정의할 때, em 은 부모 엘리먼트의 폰트 크기와 같습니다.
만약, 전체 페이지에서 폰트 크기를 정의하지 않으면 높은 가능성으로 브라우저 기본 `font-size`인 `16px`로 지정됩니다. 따라서 기본 값일때 1em = 16px 이고 2em = 32px 를 나타냅니다. 만약 `<body>` 엘리먼트의 `font-size` 를 20px로 정한 경우, `<p>` 엘리먼트에서 1em = 20px 이고 2em = 40px 입니다. 2 라는 값은 현재 크기의 배수를 나타냄을 주의해야 합니다.
