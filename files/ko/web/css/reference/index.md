...
**CSS 참고서**를 이용해 [알파벳 순서로 정리한](#키워드_색인) 모든 표준 [CSS](/ko/docs/Web/CSS) 속성, [의사 클래스](/ko/docs/Web/CSS/Reference/Selectors/Pseudo-classes), [의사 요소](/ko/docs/Web/CSS/Reference/Selectors/Pseudo-elements), [CSS 자료형](/ko/docs/Web/CSS/Reference/Values/Data_types)과 [@규칙](/ko/docs/Web/CSS/Guides/Syntax/At-rules)을 찾아보세요. 또한 [유형별로 정리한 CSS 선택자](#선택자)와 [주요 CSS 개념](#개념)도 찾아볼 수 있습니다. 추가로 간단한 [DOM-CSS / CSSOM](#dom-css_cssom) 참조도 들어 있습니다.
...
CSS 선택자 구문을 설명하는 입문자 단계의 소개 부분은 [이 자습서](/ko/docs/Learn/CSS/Introduction_to_CSS/%EC%84%A0%ED%83%9D%EC%9E%90)에서 찾아볼 수 있습니다. 규칙 정의에서 [구문](/ko/docs/Web/CSS/Guides/Syntax/Introduction) 오류가 하나라도 발생하면 규칙 전체가 유효하지 않다는 점을 명심하세요. 유효하지 않은 규칙은 브라우저가 무시합니다. CSS 규칙 정의는 모두 (ASCII) [텍스트에 기반](https://www.w3.org/TR/css-syntax-3/#intro)하지만, DOM-CSS / CSSOM (규칙 관리 시스템)은 [객체에 기반](https://www.w3.org/TR/cssom/#introduction)합니다.

### @규칙 구문

@규칙 체계는 매우 다양하므로 필요한 구체적인 구문을 찾으려면 [@규칙](/ko/docs/Web/CSS/Guides/Syntax/At-rules)을 봐주세요.
...
다음은 [선택자](/ko/docs/Web/CSS/Guides/Selectors) 목록입니다. 선택자를 사용하면 DOM 요소의 다양한 기능에 기반한 조건을 통해 스타일을 입힐 수 있습니다.

### 기본 선택자

**기본 선택자**는 선택자의 기초를 이루며, 조합을 통해 더 복잡한 선택자를 생성합니다.

- [전체 선택자](/ko/docs/Web/CSS/Reference/Selectors/Universal_selectors) `*`, `ns|*`, `*|*`, `|*`
- [태그 선택자](/ko/docs/Web/CSS/Reference/Selectors/Type_selectors) _`elementname`_
- [클래스 선택자](/ko/docs/Web/CSS/Reference/Selectors/Class_selectors) `.classname`
- [ID 선택자](/ko/docs/Web/CSS/Reference/Selectors/ID_selectors) `#idname`
- [속성 선택자](/ko/docs/Web/CSS/Reference/Selectors/Attribute_selectors) `[attr=value]`

### 그룹 선택자

- [선택자 목록](/ko/docs/Web/CSS/Reference/Selectors/Selector_list) `A, B`
  - : A와 B 요소를 모두 선택합니다. 일치하는 여러가지 요소를 선택할 때 사용합니다.

### 결합자

결합자는 "*`A`*는 *`B`_의 자식", "_`A`*는 *`B`*와 인접 요소"처럼, 두 개 이상의 선택자끼리 관계를 형성합니다.
...
- [일반 형제 결합자](/ko/docs/Web/CSS/Reference/Selectors/Subsequent-sibling_combinator) `A ~ B`
  - : 요소 *`A`*와 *`B`*가 같은 부모를 가지며 *`B`*가 *`A`*를 뒤따라야 하도록 지정합니다. 그러나 *`B`*가 *`A`*의 바로 옆에 위치해야 할 필요는 없습니다.
- [자식 결합자](/ko/docs/Web/CSS/Reference/Selectors/Child_combinator) `A > B`
  - : 요소 *`B`*가 *`A`*의 바로 밑에 위치해야 하도록 지정합니다.
- [자손 결합자](/ko/docs/Web/CSS/Reference/Selectors/Descendant_combinator) `A B`
  - : 요소 *`B`*가 *`A`*의 밑에 위치해야 하도록 지정합니다. 그러나 *`B`*가 *`A`*의 바로 아래에 있을 필요는 없습니다.
- [열 결합자](/ko/docs/Web/CSS/Reference/Selectors/Column_combinator) `A || B` {{experimental_inline}}
...
- [CSS 구문](/ko/docs/Web/CSS/Guides/Syntax/Introduction)
...
- [쌓임 맥락](/ko/docs/Web/CSS/Guides/Positioned_layout/Stacking_context)
