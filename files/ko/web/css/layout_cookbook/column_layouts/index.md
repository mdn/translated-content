...
{{cssxref("display", "display: flex;")}}를 설정하여 부모 요소를 플렉스 컨테이너로 만들면, 플렉스박스를 사용해 콘텐츠를 여러 열로 나눌 수 있습니다. 이 속성 하나만 추가해도 모든 자식 요소(요소 노드, 가상 요소, 텍스트 노드)가 단일 행에 배치된 플렉스 요소으로 변환됩니다. 또한 {{cssxref("flex")}} 단축 속성에 단일 숫자 값을 지정하면 사용 가능한 공간이 모든 요소에 균등하게 분배되어, 일반적으로 콘텐츠가 줄바꿈되지 않는 한 모든 플렉스 요소의 크기가 동일하게 유지됩니다.
