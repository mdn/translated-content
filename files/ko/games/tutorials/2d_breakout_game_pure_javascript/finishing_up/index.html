...
<p>{{Previous("Games/Tutorials/2D_Breakout_game_pure_JavaScript/Mouse_controls")}}</p>

<div class="summary">
<p><a href="/ko/docs/Games/Tutorials/2D_Breakout_game_pure_JavaScript">Gamedev Canvas tutorial</a>의 <strong>10개의 레슨 중 마지막 단계</strong>입니다. 이 수업을 마친 후 비교하여 살펴볼 소스코드는 <a href="https://github.com/end3r/Gamedev-Canvas-workshop/blob/gh-pages/lesson10.html">Gamedev-Canvas-workshop/lesson10.html</a>에서 찾아볼 수 있습니다.</p>
</div>

<p><span class="seoSummary">우리가 쓰는 모든 게임에 항상 개선해야 할 것이 있습니다. 예를 들자면, 우리는 플레이어에게 한 개보다 많은 라이프를 제공할 수 있습니다. 플레이어들은 한 번 또는 두 번의 실수를 만들어도 여전히 게임을 끝낼 수 있습니다. 우리는 또한 우리의 코드 렌더링을 개선할 수 있습니다.</span></p>

<h2 id="플레이어에게_생명을_부여하기">플레이어에게 생명을 부여하기</h2>

<p>생명을 구현하는 방법은 아주 간단합니다. 먼저 변수를 선언했던 곳과 같은 장소에서 변수를 추가하여 생명 수를 저장합니다.</p>
...
<p>자, 이제 공이 화면의 맨 아래 가장자리에 맞으면 생명(<code>lives</code>)을 하나 뺍니다. 생명이 없다면 게임이 끝나고 아직 남아있다면 공의 위치와 패들의 위치가 재설정 됩니다.</p>
...
<p>이제 게임 메커니즘과 연결이 되지 않지만, 그것이 렌더링되는 방식에 대해 연구해 봅시다. {{domxref("window.requestAnimationFrame", "requestAnimationFrame")}}은 우리가 현재 {{domxref("windowTimers.setInterval()", "setInterval()")}}을 사용하여 구현한 고정된 프레임레이트(fixed framerate)보다 더 낫게 게임을 렌더링합니다. 다음 줄을,</p>
...
<p>그것이 이 레슨의 전부입니다. 게임의 최종 버전이 준비되어 있고 시작할 준비가 되어 있습니다!</p>

<p>{{JSFiddleEmbed("https://jsfiddle.net/raymondjplante/dfh2tpu1/","","395")}}</p>

<div class="note">
<p><strong>Exercise</strong>: 생명의 수와 공이 패들에서 튕겨나가는 각도를 변경해 보세요.</p>
</div>

<h2 id="게임_오버_-_마무리!">게임 오버 - 마무리!</h2>

<p>축하합니다! 당신은 모든 수업을 완수하였습니다! 이쯤 되면 이제 캔버스의 조작 기초와 간단한 2D 게임 구현 방법을 알게 되었을 것입니다. 이제 몇몇 프레임워크를 배우고 게임 개발을 계속하기에 좋은 시간입니다. 이 시리즈의 <a href="/ko/docs/Games/Tutorials/2D_breakout_game_Phaser">2D breakout game using Phaser</a> 또는 <a href="/ko/docs/Games/Tutorials/HTML5_Gamedev_Phaser_Device_Orientation">Cyber Orb built in Phaser</a> 튜토리얼을 확인해 볼 수 있습니다. <a href="/ko/docs/Games">Games section on MDN</a>의 섹션에서 더 많은 지식과 영감을 얻어보세요.</p>

<p>당신은 또한 <a href="/ko/docs/Games/Tutorials/2D_Breakout_game_pure_JavaScript">이 튜토리얼 시리즈의 메인 페이지</a>로 돌아갈 수 있습니다. 즐겁게 코딩하세요!</p>
