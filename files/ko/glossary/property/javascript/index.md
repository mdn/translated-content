...
**JavaScript 속성**은 키와 값을 연결하는 [객체](/ko/docs/Web/JavaScript/Guide/Data_structures#objects)의 구성원입니다. JavaScript 객체는 속성 모음을 저장하는 데이터 구조입니다.
...
[접근자 속성](/ko/docs/Web/JavaScript/Guide/Data_structures#accessor_property)에는 실제 "값"이 없습니다. 값은 값을 읽을 때 호출되는 하나 (getter), 그리고 값을 설정할 때 호출되는 하나 (setter)로 한 쌍의 함수를 통해 간접적으로 표현됩니다. 그러나 접근자 속성은 표면적으로는 일반적인 데이터 속성처럼 동작하는데, getter와 setter 함수가 자동으로 호출되고 숨겨져 있기 때문입니다.
...
[클래스](/ko/docs/Web/JavaScript/Reference/Classes) 속성 맥락은 각 인스턴스가 소유하는 "인스턴스 속성"과 클래스가 소유하고 모든 인스턴스에 공통 데이터를 보유하는 "정적 속성"으로 나눌 수 있습니다. [상속](/ko/docs/Web/JavaScript/Guide/Inheritance_and_the_prototype_chain) 맥락에서 속성은 객체 자체가 소유하는 "고유 속성"과 객체의 프로토타입 체인에 있는 객체가 소유하는 "상속 속성"으로 나눌 수 있습니다.
...
- [객체 속성](/ko/docs/Web/JavaScript/Guide/Data_structures#properties)
- [속성의 열거 가능성 및 소유권](/ko/docs/Web/JavaScript/Guide/Enumerability_and_ownership_of_properties)
