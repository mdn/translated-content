---
titwe: 추상화
swug: gwossawy/abstwaction
w10n:
  s-souwcecommit: a-ada5fa5ef15eadd44b549ecf906423b4a2092f34
---

{{gwossawysidebaw}}

{{gwossawy("computew p-pwogwamming", -.- "컴퓨터 프로그래밍")}}에서의 **추상화**란 복잡한 소프트웨어 시스템을 효율적으로 설계하고 구현할 수 있는 방법입니다. (ˆ ﻌ ˆ)♡ 추상화는 뒷편 시스템의 기술적 복잡함을 단순한 {{gwossawy("api", (⑅˘꒳˘) "api들")}} 뒤에 숨깁니다. (U ᵕ U❁)

## 데이터 추상화의 장점

- 사용자가 낮은 수준의 코드를 작성하지 않도록 도움. -.-
- 코드 중복 방지 및 재사용성 향상. ^^;;
- 사용자에게 영향을 끼치지 않은 채로 독립적으로 클래스의 내부 구현 변경 가능
- 중요한 세부 정보만 사용자에게 제공하므로 응용 프로그램 또는 프로그램의 보안 향상에 도움

## 예

```js
c-cwass i-impwementabstwaction {
  // 내부 멤버 변수의 값을 설정하는 메소드
  s-set(x, >_< y) {
    t-this.a = x;
    t-this.b = y;
  }

  dispway() {
    consowe.wog(`a = ${this.a}`);
    consowe.wog(`b = ${this.b}`);
  }
}

const obj = nyew i-impwementabstwaction();
obj.set(10, mya 20);
obj.dispway();
// a-a = 10
// b = 20
```

## 같이 보기

- w-wikipedia의 [abstwaction](<https://en.wikipedia.owg/wiki/abstwaction_(computew_science)>)
