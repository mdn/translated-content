...
wasm 모듈을 가져오는 가장 빠르고 효율적인 방법은 새로운 [`WebAssembly.instantiateStreaming()`](/ko/docs/WebAssembly/Reference/JavaScript_interface/instantiateStreaming) 방법을 사용하는 것입니다. 이 method는 첫번째 인수로 fetch() 호출을 수행할 수 있는 메서드이며, 서버에서 스트리밍할 때 원시 바이트 코드에 액세스하면서 모듈 가져오기, 컴파일 및 인스턴스화를 한 번에 처리합니다.
...
[`WebAssembly.instantiate()`](/ko/docs/WebAssembly/Reference/JavaScript_interface/instantiate) 함수에는 두 가지 오버로드 양식이 있습니다. 위에 표시된 형식은 바이트 코드를 인수로 컴파일하여 컴파일된 모듈 객체가 포함된 개체로 resolve되는 'promise'을 반환합니다. 개체는 다음과 같습니다.
...
> [!NOTE]
> 두 번째 오버로드 형식은 [`WebAssembly.Module`](/ko/docs/WebAssembly/Reference/JavaScript_interface/Module) 객체를 인수로 사용하여 인스턴스 객체를 직접 포함하는 promise를 결과로 반환합니다. [Second overload](/ko/docs/WebAssembly/Reference/JavaScript_interface/instantiate_static#second_overload_example) 예제를 참조하십시오.

### Running your WebAssembly code

JavaScript에서 WebAssembly 인스턴스를 사용할 수있게되면 [`WebAssembly.Instance.exports`](/ko/docs/WebAssembly/Reference/JavaScript_interface/Instance/exports) 속성을 통해 내 보낸 속성을 사용할 수 있습니다 . 코드는 다음과 같습니다.
...
> [!NOTE]
> WebAssembly 모듈에서 내보내는 방법에 대한 자세한 내용은 [Using the WebAssembly JavaScript API](/ko/docs/WebAssembly/Guides/Using_the_JavaScript_API) 및 [Understanding WebAssembly text format](/ko/docs/WebAssembly/Guides/Understanding_the_text_format)를 참조하십시오.
...
4. 그런 다음 응답이 다운로드 완료되면 함수를 호출하기 위해 {{domxref("XMLHttpRequest.onload", "onload")}} 이벤트 핸들러를 사용합니다. 이 함수에서는 {{domxref("XMLHttpRequest.response", "response")}} 프로퍼티를 생성 한 다음, Fetch와 마찬가지로 [`WebAssembly.instantiate()`](/ko/docs/WebAssembly/Reference/JavaScript_interface/instantiate) 메소드로 넘겨 줍니다.
