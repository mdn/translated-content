...
**`WebAssembly.compileStreaming()`**함수는 스트림 된 원본 소스에서 직접 [`WebAssembly.Module`](/ko/docs/WebAssembly/Reference/JavaScript_interface/Module)을 컴파일합니다. 이 함수는 모듈을 인스턴스화하기 전에 컴파일해야하는 경우 유용합니다 (그렇지 않으면 [`WebAssembly.instantiateStreaming ()`](/ko/docs/WebAssembly/Reference/JavaScript_interface/instantiateStreaming) 함수를 사용해야합니다).
...
다음 예제 (GitHub의 [compile-streaming.html](https://github.com/mdn/webassembly-examples/blob/master/js-api-examples/compile-streaming.html) 데모 및 [라이브보기](https://mdn.github.io/webassembly-examples/js-api-examples/compile-streaming.html))에서 기본 소스의 .wasm 모듈을 직접 스트리밍 한 다음 [`WebAssembly.Module`](/ko/docs/WebAssembly/Reference/JavaScript_interface/Module) 객체로 컴파일합니다. `compileStreaming()` 함수는 {{domxref ( "Response")}} 객체에 대한 promise를 받으므로 직접 {{domxref("fetch()")}} 호출을 전달할 수 있습니다.
...
결과 모듈 인스턴스는 [`WebAssembly.instantiate ()`](/ko/docs/WebAssembly/Reference/JavaScript_interface/instantiate)를 사용하여 인스턴스화되고 내 보낸 함수가 호출됩니다.
