...
> [!NOTE]
> 텍스트 형식 파일은 대개 `.wat` 확장자로 저장됩니다. 때로는 `.wast`도 사용됩니다.이 파일은 변환 할 때 `.wast`으로 변환되지 않는 파일에 허용되는 추가 테스트 명령을 포함하는 파일을 가리 킵니다 - assertions 등
...
> [!NOTE]
> wasm2wat 도구를 사용하여 어셈블리를 다시 텍스트 표현으로 변환 할 수도 있습니다. 예 : `wasm2wat simple.wasm -o text.wat`.
