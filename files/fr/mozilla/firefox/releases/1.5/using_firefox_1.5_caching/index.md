...
- le site est HTTPS et la page définit au moins un des éléments suivants&nbsp;:
  - "Cache-Control: no-cache"
  - "Pragma: no-cache"
  - avec "Expires: 0" ou "Expires" avec une valeur de date antérieure par rapport à la valeur de l'en-tête "Date" (sauf si "Cache-Control: max-age=" est également défini)&nbsp;;
- la page n'est pas complètement chargée lorsque l'utilisateur·ice s'en va ou a des requêtes réseau en attente pour d'autres raisons (par exemple, `XMLHttpRequest`)&nbsp;;
- la page a des transactions IndexedDB en cours&nbsp;;
- la page de niveau supérieur contient des cadres (par exemple, {{HTMLElement("iframe")}}) qui ne peuvent pas être mis en cache pour l'une des raisons listées ici&nbsp;;
- la page est dans un cadre et l'utilisateur·ice charge une nouvelle page dans ce cadre (dans ce cas, lorsque l'utilisateur·ice navigue hors de la page, le contenu qui a été chargé en dernier dans les cadres est celui qui est mis en cache).
...
- savoir qu'une page a été accédée (lorsqu'elle est chargée depuis le cache de l'utilisateur·ice)
- définir le comportement d'une page lorsque l'utilisateur·ice la quitte (tout en lui permettant d'être mise en cache)
...
Si la page contient des scripts ou d'autres actions déclenchées au chargement qui doivent continuer à s'exécuter lorsque l'utilisateur·ice affiche la page, ou si vous voulez savoir si un·e utilisateur·ice a consulté une page en cache, utilisez le nouvel évènement `pageshow`.

Si vous avez des actions devant s'exécuter lorsque l'utilisateur·ice quitte une page, mais désirez profiter de la nouvelle fonctionnalité de mise en cache, donc sans pouvoir employer le gestionnaire `unload`, utilisez le nouvel évènement `pagehide`.
...
- lorsque l'utilisateur·ice quitte la page mise en cache, l'évènement `pagehide` de chaque cadre se déclenche avant l'évènement `pagehide` du document principal.
...
- Le curseur est placé dans le champ Nom du formulaire au premier chargement de la page. Dans Firefox 1.5, lorsque l'utilisateur·ice revient sur la page, le curseur reste dans le champ dans lequel il se trouvait lorsqu'il l'a quittée. Dans les autres navigateurs, le curseur retourne dans le champ Nom.
...
<head>
  <script>
    function onLoad() {
      ...
      // Calcule la date et l'heure actuelles
      var currentTime = new Date();
      var year = currentTime.getFullYear();
      var month = currentTime.getMonth() + 1;
      var day = currentTime.getDate();
...
Les [extensions](/fr/docs/Mozilla/Add-ons) pour Firefox 1.5 doivent prendre en compte cette fonctionnalité de mise en cache. Si vous développez une extension pour Firefox et que vous désirez qu'elle soit compatible à la fois avec la 1.5 et les versions antérieures, assurez-vous qu'elle écoute l'évènement `load` pour les déclencheurs qui peuvent être mis en cache et écoute l'évènement `pageshow` pour les déclencheurs qui ne doivent pas être mis en cache.
