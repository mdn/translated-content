...
- **Les tableaux JavaScript ne sont pas des tableaux associatifs** et donc, les éléments d'un tableau ne peuvent pas être accédés avec des chaînes de caractères arbitraires comme index, mais uniquement avec des entiers qui ne sont pas négatifs (ou leur forme en chaîne de caractères correspondante).
- **Les tableaux JavaScript sont [indexés à partir de zéro <sup>(angl.)</sup>](https://en.wikipedia.org/wiki/Zero-based_numbering)**&nbsp;: le premier élément d'un tableau a pour indice `0`, le second `1`, etc. — et le dernier élément est à la position donnée par la propriété {{JSxRef("Array/length", "length")}} du tableau moins `1`.
...
Les objets `Array` ne peuvent pas utiliser de chaînes de caractères arbitraires comme indices d'éléments (comme dans un [tableau associatif](https://fr.wikipedia.org/wiki/Tableau_associatif)), mais doivent utiliser des entiers qui ne sont pas négatifs (ou leur forme en chaîne de caractères). Définir ou accéder à un élément par une valeur qui n'est pas entière ne définit ni ne récupère un élément de la liste du tableau, mais définit ou accède à une variable associée à la [collection de propriétés de l'objet](/fr/docs/Web/JavaScript/Guide/Data_structures#propriétés) de ce tableau. Les propriétés d'objet du tableau et la liste des éléments du tableau sont séparées, et les [opérations de parcours et de modification](/fr/docs/Web/JavaScript/Guide/Indexed_collections#méthodes_des_tableaux) du tableau ne s'appliquent pas à ces propriétés nommées.
...
Le `2` dans `annees[2]` est converti en chaîne de caractères par le moteur JavaScript avec une conversion implicite avec `toString`. Ainsi, `'2'` et `'02'` font référence à deux emplacements différents dans l'objet `annees`, et l'exemple suivant peut donc retourner `true`&nbsp;:

```js
console.log(annees["2"] !== annees["02"]);
```

Seul `annees['2']` est un véritable indice de tableau. `annees['02']` est une propriété de chaîne de caractères arbitraire qui n'est pas parcourue lors de l'itération sur le tableau.
...
Lorsque l'on définit une propriété sur un tableau JavaScript, si la propriété est un indice de tableau valide et que cet indice est en dehors des limites actuelles du tableau, le moteur met à jour la propriété {{JSxRef("Array/length", "length")}} du tableau en conséquence&nbsp;:
...
```js
fruits.length = 10;
console.log(fruits); // ['banane', 'pomme', 'pêche', empty x 2, 'mangue', empty x 4]
console.log(Object.keys(fruits)); // ['0', '1', '2', '5']
...
Toutes les méthodes itératives peuvent [copier](#méthodes_de_copie_et_méthodes_mutantes) et sont [génériques](#méthodes_de_tableau_génériques), bien qu'elles se comportent différemment avec [les emplacements vides](#méthodes_de_tableau_et_emplacements_vides).
...
```js
function methode(foncRappel, argumentThis) {
  const longueur = this.length;
  for (let i = 0; i < longueur; i++) {
    if (i in this) {
      const resultat = foncRappel.call(argumentThis, this[i], i, this);
      // Faire quelque chose avec resultat&nbsp;; peut-être retourner tôt
    }
  }
}
```
...
Les méthodes de tableau sont toujours génériques — elles n'accèdent à aucune donnée interne de l'objet tableau. Elles accèdent uniquement aux éléments du tableau avec la propriété `length` et les éléments indexés. Cela signifie qu'elles peuvent aussi être appelées sur des objets ressemblant à des tableaux.
...
- {{JSxRef("Array.isArray()")}}
  - : Cette méthode retourne `true` si la variable est un tableau, `false` sinon.
- {{JSxRef("Array.of()")}}
...
- {{JSxRef("Array.prototype.filter()")}}
  - : Retourne un nouveau tableau qui contient tous les éléments du tableau courant pour lesquels la fonction de filtre passée en argument a retourné `true`.
...
- {{JSxRef("Array.prototype.includes()")}}
  - : Détermine si le tableau contient une valeur et retourne `true` ou `false` selon le cas de figure.
...
- {{JSxRef("Array.prototype.join()")}}
  - : Retourne une nouvelle chaîne de caractères qui est la concaténation de tous les éléments de ce tableau, séparés par des virgules ou par une chaîne de caractères de séparateur définie.
...
- {{JSxRef("Array.prototype.pop()")}}
  - : Retire le dernier élément du tableau et retourne cet élément.
- {{JSxRef("Array.prototype.push()")}}
  - : Ajoute un ou plusieurs éléments à la fin du tableau et retourne la nouvelle longueur (`length`) du tableau.
...
- {{JSxRef("Array.prototype.shift()")}}
  - : Retire le premier élément du tableau et retourne cet élément.
- {{JSxRef("Array.prototype.slice()")}}
  - : Extrait une section du tableau courant et retourne un nouveau tableau.
...
- {{JSxRef("Array.prototype.sort()")}}
  - : Trie les éléments du tableau à même le tableau et retourne le tableau.
...
Cet exemple utilise la méthode {{JSxRef("Array.prototype.join", "join()")}} pour créer une chaîne de caractères à partir du tableau `fruits`.
...
Cet exemple montre comment accéder aux éléments du tableau `fruits` en définissant l'indice de leur position dans le tableau.
...
Cet exemple utilise la méthode {{JSxRef("Array.prototype.indexOf", "indexOf()")}} pour trouver la position (indice) de la chaîne de caractères `"Banane"` dans le tableau `fruits`.
...
Cet exemple montre deux façons de vérifier si le tableau `fruits` contient `"Banane"` et `"Cerise"`&nbsp;: d'abord avec la méthode {{JSxRef("Array.prototype.includes", "includes()")}}, puis avec la méthode {{JSxRef("Array.prototype.indexOf", "indexOf()")}} pour tester si l'indice retourné n'est pas `-1`.
...
Cet exemple utilise la méthode {{JSxRef("Array.prototype.push", "push()")}} pour ajouter une nouvelle chaîne de caractères au tableau `fruits`.
...
Cet exemple utilise la méthode {{JSxRef("Array.prototype.pop", "pop()")}} pour retirer le dernier élément du tableau `fruits`.
...
Cet exemple utilise la méthode {{JSxRef("Array.prototype.splice", "splice()")}} pour retirer les 3 derniers éléments du tableau `fruits`.
...
Cet exemple utilise la méthode {{JSxRef("Array.prototype.splice", "splice()")}} pour tronquer le tableau `fruits` à ses 2 premiers éléments.
...
Cet exemple utilise la méthode {{JSxRef("Array.prototype.shift", "shift()")}} pour retirer le premier élément du tableau `fruits`.
...
Cet exemple utilise la méthode {{JSxRef("Array.prototype.splice", "splice()")}} pour retirer les 3 premiers éléments du tableau `fruits`.
...
Cet exemple utilise la méthode {{JSxRef("Array.prototype.unshift", "unshift()")}} pour ajouter, à l'indice `0`, un nouvel élément au tableau `fruits` — il devient alors le premier élément du tableau.
...
Cet exemple utilise la méthode {{JSxRef("Array.prototype.splice", "splice()")}} pour retirer la chaîne de caractères `"Banane"` du tableau `fruits` — en définissant la position de l'indice de `"Banane"`.
...
Cet exemple utilise la méthode {{JSxRef("Array.prototype.splice", "splice()")}} pour retirer les chaînes de caractères `"Banane"` et `"Fraise"` du tableau `fruits` — en définissant la position de l'indice de `"Banane"` et le nombre total d'éléments à retirer.
...
Cet exemple utilise la méthode {{JSxRef("Array.prototype.splice", "splice()")}} pour remplacer les 2 derniers éléments du tableau `fruits` par de nouveaux éléments.
...
Mais `for...of` n'est qu'une des nombreuses façons d'itérer sur un tableau&nbsp;; pour d'autres méthodes, voir les [boucles et itérations](/fr/docs/Web/JavaScript/Guide/Loops_and_iteration), ainsi que la documentation des méthodes {{JSxRef("Array.prototype.every", "every()")}}, {{JSxRef("Array.prototype.filter", "filter()")}}, {{JSxRef("Array.prototype.flatMap", "flatMap()")}}, {{JSxRef("Array.prototype.map", "map()")}}, {{JSxRef("Array.prototype.reduce", "reduce()")}} et {{JSxRef("Array.prototype.reduceRight", "reduceRight()")}} — et voir l'exemple suivant qui utilise la méthode {{JSxRef("Array.prototype.forEach", "forEach()")}}.

### Appeler une fonction sur chaque élément d'un tableau

Cet exemple utilise la méthode {{JSxRef("Array.prototype.forEach", "forEach()")}} pour appeler une fonction sur chaque élément du tableau `fruits`&nbsp;; la fonction affiche chaque élément dans la console, ainsi que son indice.
...
Cet exemple utilise la méthode {{JSxRef("Array.prototype.concat", "concat()")}} pour fusionner le tableau `fruits` avec un tableau `autresFruits` et produire un nouveau tableau `fruitsCombines`. Remarquez que `fruits` et `autresFruits` restent inchangés.
...
Cet exemple montre trois façons de créer un nouveau tableau à partir du tableau existant `fruits`&nbsp;: d'abord en utilisant la [syntaxe de l'opérateur de propagation](/fr/docs/Web/JavaScript/Reference/Operators/Spread_syntax), puis en utilisant la méthode {{JSxRef("Array.from", "from()")}}, et enfin en utilisant la méthode {{JSxRef("Array.prototype.slice", "slice()")}}.

```js
const fruits = ["Fraise", "Mangue"];
...
const fruitsCopie2 = Array.from(fruits);
// ["Fraise", "Mangue"]

// Créer une copie en utilisant la méthode slice().
const fruitsCopie3 = fruits.slice();
// ["Fraise", "Mangue"]
```

Toutes les opérations natives de copie de tableau ([syntaxe de l'opérateur de propagation](/fr/docs/Web/JavaScript/Reference/Operators/Spread_syntax), {{JSxRef("Array.from()")}}, {{JSxRef("Array.prototype.slice()")}} et {{JSxRef("Array.prototype.concat()")}}) créent des [copies superficielles](/fr/docs/Glossary/Shallow_copy). Si vous souhaitez plutôt une [copie profonde](/fr/docs/Glossary/Deep_copy) d'un tableau, vous pouvez utiliser {{JSxRef("JSON.stringify()")}} pour convertir le tableau en chaîne de caractères JSON, puis {{JSxRef("JSON.parse()")}} pour convertir la chaîne de caractères en un nouveau tableau totalement indépendant du tableau d'origine.

```js
const copieProfondeFruits = JSON.parse(JSON.stringify(fruits));
```
...
fruits.unshift("Pomme", "Banane");
console.log(fruits);
// ['Pomme', 'Banane', 'Fraise', 'Mangue']
...
L'exemple suivant crée un échiquier sous forme de tableau à deux dimensions de chaînes de caractères. Le premier coup est effectué en copiant le `'p'` de `tableau[6][4]` vers `tableau[4][4]`. L'ancienne position `[6][4]` est rendue vide.
...
Le résultat d'une correspondance entre un {{JSxRef("RegExp")}} et une chaîne de caractères peut créer un tableau JavaScript qui possède des propriétés et des éléments fournissant des informations sur la correspondance. Un tel tableau est retourné par {{JSxRef("RegExp.prototype.exec()")}} et {{JSxRef("String.prototype.match()")}}.
...
Les [méthodes itératives](#méthodes_itératives) ne modifient pas le tableau sur lequel elles sont appelées, mais la fonction fournie en tant que `callbackFn` peut le faire. Le principe clé à retenir est que seuls les indices entre 0 et `arrayLength - 1` sont visités, où `arrayLength` est la longueur du tableau au moment où la méthode a été appelée, mais l'élément passé à la fonction de rappel est la valeur au moment où l'indice est visité. Ainsi&nbsp;:

- `callbackFn` ne visite aucun élément ajouté au-delà de la longueur initiale du tableau au début de l'appel de la méthode itérative.
- Les modifications des indices déjà visités ne provoquent pas un nouvel appel de `callbackFn` sur ceux-ci.
- Si un élément existant mais pas encore visité du tableau est modifié par `callbackFn`, la valeur passée à la fonction de rappel est celle au moment où cet élément est visité. Les éléments supprimés ne sont pas visités.
...
function testSideEffect(effect) {
  ...
  console.log(`Tableau final : [${arr.join(", ")}]`);
}
```

La modification d'indices pas encore visités est visible une fois l'indice atteint&nbsp;:
...
// Tableau final : [e1, e2*, e3*, e4*]
```

La modification d'indices déjà visités ne change pas le comportement de l'itération, même si le tableau est différent après&nbsp;:
...
L'insertion de _n_ éléments à des indices non encore visités et inférieurs à la longueur initiale du tableau les fait être visités. Les _n_ derniers éléments du tableau d'origine qui ont maintenant un indice supérieur à la longueur initiale ne sont pas visités&nbsp;:
...
// Tableau final : [e1, e2, new, e3, e4]
// e4 n'est pas visité, car il a maintenant l'indice 4
```

L'insertion de _n_ éléments à un indice supérieur à la longueur initiale du tableau ne les fait pas être visités&nbsp;:
...
L'insertion de _n_ éléments à des indices déjà visités ne les fait pas être visités, mais décale les éléments restants de _n_ vers l'arrière, donc l'indice courant et les _n - 1_ éléments précédents sont visités à nouveau&nbsp;:
...
La suppression de _n_ éléments à des indices non encore visités fait qu'ils ne sont plus visités. Comme le tableau a rétréci, les _n_ dernières itérations visitent des indices hors limites. Si la méthode ignore les indices inexistants (voir [méthodes de tableau et trous](#méthodes_de_tableau_et_emplacements_vides)), les \_n\* dernières itérations sont ignorées&nbsp;; sinon, elles reçoivent `undefined`&nbsp;:
...
La suppression de _n_ éléments à des indices déjà visités ne change pas le fait qu'ils ont été visités avant d'être supprimés. Comme le tableau a rétréci, les _n_ éléments suivants après l'indice courant sont ignorés. Si la méthode ignore les indices inexistants, les _n_ dernières itérations sont ignorées&nbsp;; sinon, elles reçoivent `undefined`&nbsp;:
