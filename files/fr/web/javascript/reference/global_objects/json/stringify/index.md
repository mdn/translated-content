...
- `space` {{Optional_Inline}}
  - : Une chaîne de caractères ou un nombre utilisé pour insérer des espaces (y compris l'indentation, les caractères de saut de ligne, etc.) dans la chaîne de caractères JSON produite afin d'en faciliter la lecture.
    ...
    Si c'est un nombre, il indique le nombre d'espaces à utiliser pour l'indentation, limité à 10 (c'est-à-dire que toute valeur supérieure à `10` est traitée comme si elle vaut `10`). Les valeurs inférieures à 1 signifient qu'aucun espace n'est utilisé.
...
- Les nombres {{JSxRef("Infinity")}} et {{JSxRef("NaN")}}, ainsi que la valeur [`null`](/fr/docs/Web/JavaScript/Reference/Operators/null), sont tous considérés comme `null`. (Mais contrairement aux valeurs du point précédent, elles ne sont jamais omises.)
...
- Pour les autres objets&nbsp;:
  - Toutes les propriétés dont la clé est un {{JSxRef("Symbol")}} sont complètement ignorées, même lors de l'utilisation du paramètre [`replacer`](#le_paramètres_replacer).

  - Si la valeur possède une méthode `toJSON()`, c'est à elle de définir quelles données sont sérialisées. Au lieu de sérialiser l'objet, la valeur retournée par la méthode `toJSON()` lors de son appel est sérialisée. `JSON.stringify()` appelle `toJSON` avec un paramètre, `key`, qui a la même sémantique que le paramètre `key` de la fonction [`replacer`](#le_paramètres_replacer)&nbsp;:
    ...
    - si `JSON.stringify()` a été appelé directement sur cet objet, une chaîne de caractères vide

    Tous les objets {{JSxRef("Temporal")}} implémentent la méthode `toJSON()`, qui retourne une chaîne de caractères (identique à l'appel de `toString()`). Ainsi, ils sont sérialisés comme des chaînes de caractères. De même, les objets {{JSxRef("Date")}} implémentent `toJSON()`, qui retourne la même chose que [`toISOString()`](/fr/docs/Web/JavaScript/Reference/Global_Objects/Date/toISOString).

  - Seules les [propriétés propres énumérables](/fr/docs/Web/JavaScript/Guide/Enumerability_and_ownership_of_properties) sont parcourues. Cela signifie que {{JSxRef("Map")}}, {{JSxRef("Set")}}, etc. deviennent `"{}"`. Vous pouvez utiliser le paramètre [`replacer`](#le_paramètres_replacer) pour les sérialiser de manière plus utile.

    Les propriétés sont parcourues en utilisant le même algorithme que [`Object.keys()`](/fr/docs/Web/JavaScript/Reference/Global_Objects/Object/keys), qui a un ordre bien défini et stable entre les implémentations. Par exemple, `JSON.stringify` sur le même objet produit toujours la même chaîne de caractères, et `JSON.parse(JSON.stringify(obj))` produit un objet avec le même ordre de clés que l'original (à condition que l'objet soit entièrement sérialisé en JSON).
...
La fonction `replacer` est également appelée pour l'objet initial à convertir, auquel cas la clé (`key`) est une chaîne de caractères vide (`""`). Elle est ensuite appelée pour chaque propriété de l'objet ou du tableau à convertir. Les indices de tableau sont fournis sous forme de chaîne de caractères comme `key`. La valeur de la propriété courante est remplacée par la valeur de retour de la fonction `replacer` pour la conversion en chaîne de caractères. Cela signifie&nbsp;:

- Si vous retournez un nombre, une chaîne de caractères, un booléen ou `null`, cette valeur est directement sérialisée et utilisée comme valeur de la propriété. (Retourner un `BigInt` lève également une exception.)
...
> [!NOTE]
> Lors de l'analyse du JSON généré avec des fonctions `replacer`, vous souhaitez probablement utiliser le paramètre [`reviver`](/fr/docs/Web/JavaScript/Reference/Global_Objects/JSON/parse#utiliser_le_paramètre_reviver) pour effectuer l'opération inverse.

En général, l'indice des éléments du tableau ne change jamais (même si l'élément est une valeur invalide comme une fonction, il devient `null` au lieu d'être omis). Utiliser la fonction `replacer` vous permet de contrôler l'ordre des éléments du tableau en retournant un tableau différent.

### Le paramètre `space`

Le paramètre `space` peut être utilisé pour contrôler les espacements dans la chaîne de caractères finale.

- S'il s'agit d'un nombre, chaque niveau d'imbrication dans la conversion a autant d'espaces que ce nombre.
- S'il s'agit d'une chaîne de caractères, chaque niveau d'imbrication est indenté avec cette chaîne de caractères.

Chaque niveau d'indentation ne dépasse jamais 10. Les valeurs numériques de `space` sont limitées à 10, et les chaînes de caractères sont tronquées à 10 caractères.
...
Si vous souhaitez que le `replacer` distingue l'objet initial d'une propriété dont la clé est une chaîne de caractères vide (puisque les deux donnent la chaîne de caractères vide comme clé et potentiellement un objet comme valeur), vous devez suivre le nombre d'itérations (si celui-ci dépasse la première itération, il s'agit d'une véritable clé vide).
...
Comme le [format JSON <sup>(angl.)</sup>](https://www.json.org/) ne prend pas en charge les références d'objet (bien qu'un brouillon IETF existe), une erreur {{JSxRef("TypeError")}} est levée si l'on tente d'encoder un objet contenant des références circulaires.
...
Pour sérialiser des références circulaires, vous pouvez utiliser une bibliothèque qui les prend en charge (par exemple, [cycle.js<sup>(angl.)</sup>](https://github.com/douglascrockford/JSON-js/blob/master/cycle.js) de Douglas Crockford) ou implémenter vous‑même une solution, ce qui nécessite de trouver et remplacer (ou supprimer) les références cycliques par des valeurs qui peuvent être sérialisés.
...
Dans le cas où vous souhaitez stocker un objet créé par votre utilisateur·ice et permettre sa restauration même après la fermeture du navigateur, l'exemple suivant illustre l'utilisation de `JSON.stringify()`&nbsp;:
...
Les moteurs implémentant la [spécification JSON.stringify bien formée <sup>(angl.)</sup>](https://github.com/tc39/proposal-well-formed-stringify) convertissent les substituts isolés (tout point de code compris entre U+D800 et U+DFFF) en chaînes de caractères à l'aide de séquences d'échappement Unicode plutôt que littéralement (en produisant des substituts isolés). Avant cette modification, ces chaînes de caractères ne peuvent pas être encodées en UTF-8 ou UTF-16 valides&nbsp;:
...
Cette modification doit être rétrocompatible tant que vous transmettez le résultat de `JSON.stringify()` à des API telles que `JSON.parse()` qui acceptent tout texte JSON valide, car elles traitent les échappements Unicode des substituts isolés comme identiques aux substituts isolés eux-mêmes. Ce n'est que si vous interprétez directement le résultat de `JSON.stringify()` que vous devez traiter avec soin les deux encodages possibles de ces points de code par `JSON.stringify()`.
...
- [La prothèse d'émulation du comportement moderne de `JSON.stringify` (symbol, unicode bien formé, JSON brut) dans `core-js` <sup>(angl.)</sup>](https://github.com/zloirock/core-js#ecmascript-json)
