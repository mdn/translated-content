...
isNaN(""); // false : la chaîne de caractères vide est convertie en 0 qui n'est pas NaN
isNaN(" "); // false : une chaîne de caractères avec des espaces est convertie en 0 qui n'est pas NaN

// Dates
isNaN(new Date()); // false ; Les objets Date peuvent être convertis en nombre (timestamp)
isNaN(new Date().toString()); // true ; la représentation sous forme de chaîne de caractères d'un objet Date ne peut pas être analysée comme un nombre
