---
title: "Temporal.PlainTime : propriété millisecond"
...
La propriété d'accesseur **`millisecond`** des instances de {{JSxRef("Temporal.PlainTime")}} retourne un entier de 0 à 999 représentant la composante milliseconde (10<sup>-3</sup> seconde) de cette heure.

Le mutateur d'accesseur de `millisecond` est `undefined`. Vous ne pouvez pas modifier cette propriété directement. Utilisez la méthode {{JSxRef("Temporal/PlainTime/with", "with()")}} pour créer un nouvel objet `Temporal.PlainTime` avec la nouvelle valeur souhaitée.
...
### Changer la valeur de `millisecond`
...
Vous pouvez également utiliser {{JSxRef("Temporal/PlainTime/add", "add()")}} ou {{JSxRef("Temporal/PlainTime/subtract", "subtract()")}} pour déplacer un certain nombre de millisecondes à partir de l'heure actuelle.
