...
let arr = new Float32Array([21,31]);
console.log(arr[1]); // 31

// À partir d'un autre tableau typé
let x = new Float32Array([21, 31]);
let y = new Float32Array(x);
console.log(y[0]); // 21

// À partir d'un ArrayBuffer
let buffer = new ArrayBuffer(16);
let z = new Float32Array(buffer, 0, 4);

// À partir d'un itérable
let iterable = function*(){ yield* [1,2,3]; }();
let float32 = new Float32Array(iterable);
// Float32Array[1, 2, 3]
```
...
```js example-bad
let dv = Float32Array([1, 2, 3]);
// TypeError: calling a builtin Float32Array constructor
// without new is forbidden
```

```js example-good
let dv = new Float32Array([1, 2, 3]);
```
