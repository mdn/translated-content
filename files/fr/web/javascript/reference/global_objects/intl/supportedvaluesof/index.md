...
Les types ci-dessous sont définis dans CLDR mais n'ont pas d'implémentations distinctes des calendriers ci-dessus dans les navigateurs.
...
| `dict`     | Ordre de type dictionnaire (pour le cinghalais). Également reconnu comme `dictionary`.                                                                                                                                                  |
| `emoji`    | Ordre recommandé pour les caractères emoji. La langue demandée n'a aucun effet, donc combiner cela avec une langue qui nécessite des règles spécifiques à la langue perturbe le tri du texte dans cette langue&nbsp;!                   |
| `eor`      | Règles de tri européennes (ENV 13710). Cet ordre est conçu pour ne pas être spécifique à une langue. Bien que l'API exige de l'attacher à une langue, la langue demandée n'a aucun effet sur l'ordre.                                   |
| `phonebk`  | Ordre de style annuaire téléphonique (pour l'allemand). Également reconnu comme `phonebook`.                                                                                                                                            |
| `phonetic` | Tri phonétique (tri basé sur la prononciation&nbsp;; pour le lingala)                                                                                                                                                                   |
| `pinyin`   | Ordre pinyin pour le latin et pour les caractères Han (pour le chinois)                                                                                                                                                                 |
...
| `stroke`   | Ordre pinyin pour le latin, ordre par traits pour les caractères Han (pour le chinois)                                                                                                                                                  |
| `trad`     | Ordre de style traditionnel (comme en espagnol). Également reconnu comme `traditional`.                                                                                                                                                 |
| `unihan`   | Ordre radical-trait pour les caractères Han (pour le chinois, le japonais et le coréen), ordre pinyin pour le latin pour le chinois et l'ordre standard kana pour le japonais                                                           |
| `zhuyin`   | Ordre pinyin pour le latin, ordre zhuyin pour Bopomofo et les caractères Han (pour le chinois)                                                                                                                                          |
...
| `search`                         | Type de collation spécial pour la recherche de chaînes de caractères                                                            | Ne pas utiliser comme type de collation, car dans {{JSxRef("Intl.Collator")}}, cette collation est activée avec l'option `usage: "search"`. Il n'existe actuellement aucune API pour la recherche de sous-chaînes, donc cela ne sert actuellement qu'à filtrer une liste de chaînes en essayant une correspondance exacte de la clé avec chaque élément de la liste. |
| `standard`                       | Ordre par défaut pour chaque langue, sauf le chinois (et, auparavant, le suédois)                                               | Ne pas utiliser explicitement. En général, il est inutile de définir cela explicitement et le définir pour le suédois est problématique en raison de la signification différente pour le suédois dans le passé.                                                                                                                                                      |
...
Il existe trois valeurs spéciales&nbsp;: `native`, `traditio` et `finance`, dont la signification dépend de la locale, et qui sont résolues vers le bon système selon la locale. Par conséquent, les méthodes `resolvedOptions()` ne retournent jamais ces valeurs, mais `Intl.Locale.prototype.numberingSystem` le fait (si elles sont fournies en entrée).
...
Pour obtenir les valeurs prises en charge pour les représentations calendaires, on peut appeler la méthode avec la clé `"calendar"` et parcourir le tableau obtenu&nbsp;:
