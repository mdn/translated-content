...
- `numberingSystem`
  - : La valeur fournie pour cette propriété dans l'argument `options`, ou en utilisant la clé d'extension Unicode `"nu"`, avec la valeur par défaut complétée si nécessaire. Il s'agit d'un [système de numération](/fr/docs/Web/JavaScript/Reference/Global_Objects/Intl/supportedValuesOf#types_de_systèmes_de_numération_pris_en_charge) pris en charge pour cette locale. La valeur par défaut dépend de la locale.
- `style`
