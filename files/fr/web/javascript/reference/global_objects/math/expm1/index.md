...
La fonction **`Math.expm1()`** renvoie `e^x` - 1, avec `x` l'argument donné et {{jsxref("Global_Objects/Math/E","e")}} la base du logarithme nepérien.
...
Cette fonction peut être émulée en utilisant la fonction {{jsxref("Global_Objects/Math/exp", "Math.exp()")}} :
