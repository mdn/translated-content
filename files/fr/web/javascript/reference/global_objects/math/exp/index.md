...
La fonction **`Math.exp()`** renvoie l'exponentielle d'un nombre (donnée par `e^x`, où `x` est la valeur passée en argument et `e` la valeur du {{jsxref("Math.E","nombre d'Euler (parfois appelé constante de Napier)","",1)}}.
