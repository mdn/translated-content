...
Deux facteurs sont à l'origine de la conception des fonctions fléchées&nbsp;: une syntaxe plus courte et l'absence de `this` spécifique à la fonction. Sur ce dernier point, cela signifie qu'une fonction fléchée ne lie pas son propre {{jsxref("this")}} au sein de la fonction (il en va de même avec {{jsxref("Functions/arguments","arguments")}}, {{jsxref("Operators/super","super")}} ou {{jsxref("Operators/new.target","new.target")}}).
...
Jusqu'a l'apparition des fonctions fléchées, chaque nouvelle fonction définissait son propre {{jsxref("this")}} :
