...
- {{jsxref("Global_Objects/Proxy/Proxy","gestionnaire (handler)","",1)}}
...
{{jsxref("Reflect")}} est un objet natif qui fournit des méthodes pour les opérations JavaScript qui peuvent être interceptées. Ces méthodes sont les mêmes que celles gérées par les {{jsxref("Global_Objects/Proxy/Proxy","gestionnaires de proxy","",1)}}. `Reflect` n'est pas un constructeur et ne peut pas être utilisé comme une fonction !
