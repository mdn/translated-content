...
Le CSS inclut le [sélecteur d'attribut](/fr/docs/Web/CSS/Attribute_selectors) `[open]`, qui ne correspond que lorsque l'attribut `open` est présent (c'est-à-dire lorsque l'élément `<details>` est ouvert). Les pseudo-classes {{cssxref(":first-of-type")}} et {{cssxref(":last-of-type")}} ciblent respectivement le premier et le dernier élément du même type. Nous avons inclus le pseudo-élément préfixé `-webkit-` dans une pseudo-classe {{cssxref(":is()")}} car elle accepte une [liste de sélecteurs tolérante](/fr/docs/Web/CSS/Selector_list#forgiving_selector_list)&nbsp;: si le pseudo-élément préfixé est invalide dans un navigateur, tout le bloc de sélecteurs ne sera pas invalidé. Nous avons aussi utilisé la [syntaxe imbriquée de CSS](/fr/docs/Web/CSS/Nesting_selector). Voir le module [Sélecteurs CSS](/fr/docs/Web/CSS/CSS_selectors).
