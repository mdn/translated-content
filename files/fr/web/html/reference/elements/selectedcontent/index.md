...
> [!NOTE]
> Si les éléments `<button>` et/ou `<selectedcontent>` ne sont pas inclus dans `<select>`, le navigateur crée un `<button>` implicite pour afficher le contenu de l'élément `<option>` sélectionné. Ce bouton de secours ne peut pas être ciblé avec CSS avec les sélecteurs de type `button` ou `selectedcontent`.
