...
- `sizes`
  - : Une ou plusieurs valeurs séparées par des virgules, qui peuvent être des tailles de source ou le mot-clé `auto`.
    ...
    Une valeur de taille de source peut être n'importe quelle [longueur](/fr/docs/Web/CSS/length) non négative. Elle ne doit pas utiliser de fonctions CSS autres que les [fonctions mathématiques](/fr/docs/Web/CSS/CSS_Values_and_Units/CSS_Value_Functions#math_functions). Les unités sont interprétées de la même manière que dans les [requêtes média](/fr/docs/Web/CSS/CSS_media_queries), ce qui signifie que toutes les unités de longueur relative sont relatives à la racine du document et non à l'élément `<img>`. Par exemple, une valeur en `em` est relative à la taille de police racine, et non à la taille de police de l'image. Les valeurs en [pourcentage](/fr/docs/Web/CSS/percentage) ne sont pas autorisées.

    Le mot-clé `auto` peut remplacer toute la liste des tailles ou la première entrée de la liste. Il n'est valide que lorsqu'il est combiné avec `loading="lazy"`, et il correspond à la [taille concrète](/fr/docs/Web/CSS/image) de l'image. Comme la taille intrinsèque de l'image n'est pas encore connue, il est recommandé de spécifier aussi les attributs `width` et `height` (ou leurs équivalents CSS) pour éviter que le navigateur n'assume une largeur d'image par défaut de 300px.
