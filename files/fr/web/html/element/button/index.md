...
- `popovertargetaction`
  ...
  - : Définit l'action à effectuer sur l'élément popover cible lorsqu'un bouton est activé. Les valeurs possibles sont :
    ...
    - `"toggle"`
      - : Le bouton affichera l'élément popover cible s'il est masqué, ou le masquera s'il est affiché. Si `popovertargetaction` n'est pas défini, le bouton se comportera comme s'il avait la valeur `"toggle"`.
...
Le clique sur un {{HTMLElement("button")}} ou un {{HTMLElement("input")}} de type bouton, peut, selon le navigateur et l'OS lui donner le focus. Il s'agit du comportement par défaut le plus courant, mais ce n'est [intentionnellement pas le cas de Safari](https://webkit.org/b/22261).
