...
On insère une image [de la même façon que d'habitude](/fr/docs/Learn_web_development/Core/Structuring_content/HTML_images#comment_intégrer_une_image_à_une_page_web) (avec un élément {{HTMLElement("img")}} et un texte dans l'attribut [`alt`](/fr/docs/Web/HTML/Reference/Elements/img#alt)). Si l'image n'est présente qu'à des fins de navigations, `alt` peut être laissé vide : `alt=""`, si les valeurs pour les différents [`alt`](/fr/docs/Web/HTML/Reference/Elements/area#alt) sont bien renseignés dans les éléments {{HTMLElement('area')}} que nous allons présenter.
