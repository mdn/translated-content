...
La propriété [CSS](/fr/docs/Web/CSS) **`corner-inline-start-shape`** définit la forme des deux coins sur le bord supérieur d'une boîte, à l'intérieur de leur zone {{CSSxRef("border-radius")}}.
...
La propriété `corner-inline-start-shape` est une forme raccourcie pour les propriétés logiques suivantes&nbsp;:
...
La propriété `corner-inline-start-shape` se définit avec une ou deux valeurs {{CSSxRef("&lt;corner-shape-value&gt;")}}&nbsp;:

- Si **une valeur** est utilisée, elle définit la forme des **deux coins gauche**.
...
### Utilisation simple de `corner-inline-start-shape`
...
Nous appliquons à la boîte quelques styles de base, qui sont masqués ici pour plus de clarté. Nous appliquons aussi un {{CSSxRef("box-shadow")}}, un `border-radius` de 60 pixels, et un `corner-inline-start-shape` de `square bevel`.
...
- Les propriétés {{CSSxRef("corner-block-start-shape")}}, {{CSSxRef("corner-block-end-shape")}} et {{CSSxRef("corner-inline-end-shape")}}
...
- Les propriétés {{CSSxRef("border-start-start-radius")}} et {{CSSxRef("border-end-start-radius")}}
