...
En outre, un état de départ pour l'animation est défini à l'intérieur de la règle {{CSSxRef("@starting-style")}}. C'est nécessaire pour éviter un comportement inattendu. Par défaut, les transitions ne sont pas déclenchées lors des premières mises à jour de style des éléments, ou lorsque le type `display` passe de `none` à un autre type. `@starting-style` permet de remplacer ce comportement par défaut de manière contrôlée. Sans cela, l'animation d'entrée ne se produirait pas et la fenêtre contextuelle apparaîtrait simplement.
