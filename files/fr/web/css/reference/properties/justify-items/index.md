...
L'effet de cette propriété est différent selon le type de disposition utilisé&nbsp;:

- Pour les dispositions de bloc, cette propriété aligne les éléments à l'intérieur de leur bloc englobant sur l'axe en incise.
- Pour les éléments positionnés de façon absolue, cette propriété aligne les éléments à l'intérieur de leur bloc englobant sur l'axe en incise, en tenant compte des valeurs de décalage haut, gauche, bas et droite.
...
- Pour les dispositions flexibles, cette propriété est _ignorée_ (voir [L'alignement des boîtes flexibles](/fr/docs/Web/CSS/Guides/Box_alignment/In_flexbox))
- Pour les grilles, cette propriété aligne les éléments à l'intérieur de leur zone de grille sur l'axe en incise (voir [L'alignement des boîtes en disposition grille](/fr/docs/Web/CSS/Guides/Box_alignment/In_grid_layout))
...
- `normal`
  - : Ce mot-clé a un sens différent selon le mode de disposition utilisé&nbsp;:
    ...
    - Pour les dispositions qui utilisent une grille, ce mot-clé un comportement proche de `stretch`, sauf pour les boîtes qui ont des dimensions intrinsèques, dans ce cas, il est synonyme de `start`.
- `start`
  - : Les éléments sont regroupés vers le début du conteneur pour l'axe en incise.
- `end`
  - : Les éléments sont regroupés vers la fin du conteneur pour l'axe en incise.
- `flex-start`
...
- `self-start`
  - : Les éléments sont alignés sur le bord du conteneur au début de l'axe en incise.
- `self-end`
  - : Les éléments sont alignés sur le bord du conteneur à la fin de l'axe en incise.
- `center`
  - : Les éléments sont regroupés au centre du conteneur dans le sens de l'axe en incise.
- `left`
  - : Les éléments sont accolés les uns aux autres vers le bord gauche du conteneur d'alignement. Si l'axe de la propriété n'est pas parallèle à l'axe en incise, cette valeur se comporte comme `start`.
- `right`
  - : Les éléments sont accolés les uns aux autres vers le bord droit du conteneur d'alignement sur l'axe approprié. Si l'axe de la propriété n'est pas parallèle à l'axe en incise, cette valeur se comporte comme `start`.
- `baseline`, `first baseline`, `last baseline`
  - : Ces valeurs permettent de définir l'alignement par rapport à la ligne de base pour l'élément du conteneur avec la ligne de base la plus haute ou la plus basse.
    Si `first baseline` n'est pas prise en charge, la valeur correspond à `start`, si `last baseline` n'est pas prise en charge, la valeur correspond à `end`.
- `stretch`
...
- `anchor-center`
  - : Dans le cas des éléments [positionnés avec une ancre](/fr/docs/Web/CSS/Guides/Anchor_positioning), aligne les éléments au centre de l'élément d'ancre associé dans la direction en incise (<i lang="en">inline</i> en anglais). Voir [Centrer sur l'ancre avec `anchor-center`](/fr/docs/Web/CSS/Guides/Anchor_positioning/Using#centrer_sur_lancre_avec_anchor-center).
- `safe`
  - : Si la taille de l'élément dépasse du conteneur pour la valeur d'alignement indiquée, l'élément est alors aligné comme si la valeur `start` a été utilisée.
- `unsafe`
...
- `legacy`
  - : La valeur est héritée par les éléments descendants de la boîte. Si un élément descendant a `justify-self: auto`, le mot-clé `legacy` n'est pas pris en compte mais uniquement la valeur `left`, `right`, ou `center`.
