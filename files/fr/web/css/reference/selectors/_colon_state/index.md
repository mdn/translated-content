...
Cet exemple montre comment la pseudo-classe `:state()` peut être utilisée dans la fonction pseudo-classe {{CSSxRef(":host_function", ":host()")}} pour correspondre à des états personnalisés dans l'implémentation d'un élément personnalisé.
...
### Correspondance d'un état personnalisé dans une partie d'ombre

Cet exemple montre comment la pseudo-classe `:state()` peut être utilisée pour cibler les [parties d'ombre (<i lang="en">shadow parts</i> en anglais)](/fr/docs/Web/CSS/CSS_shadow_parts) d'un élément personnalisé.

Les parties d'ombre sont définies et nommées à l'aide de l'attribut [`part`](/fr/docs/Web/HTML/Reference/Global_attributes/part).
Par exemple, considérons un élément personnalisé nommé `<question-box>` qui utilise un élément personnalisé `<labeled-checkbox>` comme partie d'ombre nommée `checkbox`&nbsp;:

```js
shadowRoot.innerHTML = `<labeled-checkbox part='checkbox'>Yes</labeled-checkbox>`;
```

Le CSS ci-dessous montre comment le pseudo-élément {{CSSxRef("::part()")}} peut être utilisé pour correspondre à la partie d'ombre de `'checkbox'`.
...
Pour un exemple en direct de ce code en action, consultez l'exemple [Correspondance d'un état personnalisé dans une partie d'ombre d'un élément personnalisé](/fr/docs/Web/API/CustomStateSet#correspondance_dun_état_personnalisé_dans_une_partie_dombre_dun_élément_personnalisé) sur la page `CustomStateSet`.
...
- L'interface API {{DOMxRef("CustomStateSet")}}
