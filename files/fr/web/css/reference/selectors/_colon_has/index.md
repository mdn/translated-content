...
Si la pseudo-classe `:has()` n'est pas prise en charge par un navigateur, l'ensemble du bloc sélecteur échouera, sauf si `:has()` figure dans une liste de sélecteurs tolérants, comme dans {{CSSxRef(":is()")}} et {{CSSxRef(":where()")}}.
