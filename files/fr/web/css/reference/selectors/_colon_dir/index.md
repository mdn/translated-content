...
`:dir()` ne prend en compte que la valeur _sémantique_ de la direction, celle qui est définie par le document (la plupart du temps un document HTML). Elle ne tiendra pas compte de la direction liée à la mise en forme, purement stylistique, modifiée par des propriétés telles que [`direction`](/fr/docs/Web/CSS/Reference/Properties/direction).

> [!NOTE]
> La pseudo-classe `:dir()` ne fonctionne pas de la même façon que [le sélecteur d'attribut](/fr/docs/Web/CSS/Reference/Selectors/Attribute_selectors) `[dir=…]`. Ce dernier utilise la valeur de l'attribut [`dir`](/fr/docs/Web/HTML/Reference/Elements/html#dir) et il n'y a aucune correspondance lorsque l'attribut n'est pas défini (même si l'élément HTML hérite de la valeur de son élément parent). De la même façon `[dir=rtl]` ou `[dir=ltr]` ne pourront pas correspondre à la valeur `auto` qui peut être utilisée sur l'attribut `dir`. Au contraire, `:dir()` calculera la correspondance en fonction de la valeur utilisée par l'agent utilisateur (qu'elle soit héritée ou qu'elle vaille `auto`).

> [!NOTE]
> En HTML, la direction est définie grâce à l'attribut [`dir`](/fr/docs/Web/HTML/Reference/Elements/html#dir). Pour les autres types de document, cela peut être déterminé autrement.
