...
La valeur du descripteur `range` peut être soit `auto`, soit une liste séparée par des virgules de plages avec bornes inférieure et supérieure, définies à l'aide d'entiers négatifs ou positifs ou du mot-clé `infinite`.
...
Lorsque la plage est définie par des entiers (plutôt que par `auto`), la valeur `infinite` peut être utilisée pour désigner l'infini. Si <i lang="en">infinite</i> est défini comme première valeur d'une plage, il est interprété comme l'infini négatif. S'il est utilisé comme borne supérieure, la seconde valeur de la paire, il est pris comme l'infini positif.
...
{{EmbedLiveSample("Définir un style de compteur sur une plage")}}
