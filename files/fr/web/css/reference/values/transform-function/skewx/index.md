...
{{InteractiveExample("Démonstration CSS&nbsp;: skewX()")}}
...
Cette transformation est un cisaillement ([transvection](https://fr.wikipedia.org/wiki/Transvection)) qui déforme chaque point d'un élément selon un certain angle dans la direction horizontale. L'abscisse (coordonnée horizontale, x) de chaque point est modifiée d'une valeur proportionnelle à l'angle défini et à la distance à l'origine&nbsp;; ainsi, plus un point est éloigné de l'origine, plus la valeur ajoutée sera grande.
...
- `a`
  - : Un {{CSSxRef("&lt;angle&gt;")}} représentant l'angle à utiliser pour déformer l'élément selon l'abscisse (coordonnée horizontale, x).
