...
- `multiply`
  - : La couleur finale est obtenue en multipliant les couleurs du premier plan et de l'arrière-plan.
    Si une des couches est noire, le résultat obtenu est une image noire. Si une des couches est blanche, cela ne modifie pas l'autre image. On obtient ainsi un effet semblable à la superposition de deux transparents.
- `screen`
  - : La couleur finale est obtenue en&nbsp;: inversant les couleurs des deux plans, en les multipliant et en inversant la couleur obtenue. Si une des couches est noire, le résultat obtenu est l'image de l'autre couche. Si une des couches est blanche, l'image obtenue est blanche. L'effet obtenu est semblable à la projection de deux images sur un écran.
- `overlay`
...
- `color-dodge`
  - : La couleur finale est obtenue en divisant la couleur de l'arrière-plan avec l'inverse de la couleur du premier plan.
    Un premier-plan noir n'entraîne aucun changement.
- `color-burn`
  - : La couleur finale est obtenue en inversant la couleur de l'arrière-plan qui est divisée par la couleur du premier-plan puis inversée. Un premier-plan blanc n'entraîne aucun changement et un premier-plan qui possède exactement l'inverse de l'arrière-plan entraîne une image noire.
- `hard-light`
...
- `difference`
  - : La couleur finale est obtenue en soustrayant la couleur la plus sombre à la couleur la plus claire. Une couche noire n'a aucun effet et une couche blanche inverse l'autre image.
- `exclusion`
  - : La couleur finale obtenue est semblable à `difference` avec moins de contraste. Comme pour `difference`, une couche noire n'a aucun effet et une couche blanche inverse l'autre couche.
- `hue`
  - : La couleur finale a la teinte de la couleur du premier-plan et la saturation et la luminosité de la couleur de l'arrière-plan.
- `saturation`
  - : La couleur finale a la saturation de la couleur du premier plan et la teinte et la luminosité de la couleur de l'arrière-plan.
- `color`
  - : La couleur finale a la teinte et la saturation du premier plan et la luminosité de l'arrière-plan. Cet effet conserve les niveaux de gris et peut être utilisé pour coloriser le premier plan.
- `luminosity`
  - : La couleur finale a la luminosité de la couleur du premier plan et la teinte et la saturation de la couleur de l'arrière-plan.
...
{{EmbedLiveSample("Exemple avec `normal`", "100%", 300)}}
...
{{EmbedLiveSample("Comparaison des différentes valeurs de `background-blend-mode`", "100%", 780)}}
...
.isolate {
  isolation: isolate; /* Sans isolation, la couleur de fond est prise en compte */
  position: relative;
}
```
...
{{EmbedLiveSample("Comparaison des différentes valeurs de `mix-blend-mode`", "100%", 820)}}
...
{{EmbedLiveSample("Comparaison visuelle des modes de fusion", "100%", 400)}}
