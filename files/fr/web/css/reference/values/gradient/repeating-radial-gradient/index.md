---
title: Fonction CSS `repeating-radial-gradient()`
...
À chaque répétition, les positions des arrêts de couleur sont décalés d'un multiple du dégradé unitaire (ce dernier couvrant la distance entre le premier et le dernier arrêt de couleur). Ainsi, la position de chaque arrêt de couleur final coïncide avec l'arrêt de couleur initial qui suit&nbsp;: si les couleurs sont différentes, on a une transition abrupte. Pour ne pas obtenir cet effet, on peut réutiliser la couleur du premier arrêt de couleur pour le dernier arrêt de couleur.

Comme les autres dégradés, un dégradé radial répété [n'a pas de dimensions intrinsèques](/fr/docs/Web/CSS/Reference/Values/image#description), c'est-à-dire qu'il n'a pas de taille ou de proportions préférées. Sa taille réelle correspond à la taille de l'élément auquel il est appliqué.
...
- `<shape>`
  - : La forme du contour du dégradé. Elle vaut soit `circle` (la forme du dégradé est un cercle de rayon constant) ou `ellipse` (la forme est une ellipse alignée sur les axes). La valeur par défaut est `ellipse`.
- `<extent-keyword>`
  - : Un mot-clé décrivant la taille de la forme de terminaison. Les valeurs possibles sont&nbsp;:
    ...
    | `farthest-side`   | Cette valeur fonctionne de façon semblable à `closest-side`, sauf que ce sont les côtés les plus éloignés qui sont utilisés.                                                                       |
    | `farthest-corner` | La valeur par défaut. Cette valeur fonctionne de façon semblable à `closest-corner`, sauf que c'est le coin le plus éloigné du centre qui est utilisé.                                             |
...
- `<color-stop>`
  - : Une valeur d'arrêt de couleur, composée d'une valeur {{CSSxRef("&lt;color&gt;")}}, suivie d'une ou deux positions d'arrêt optionnelles (définies par une valeur {{CSSxRef("&lt;percentage&gt;")}} ou {{CSSxRef("&lt;length&gt;")}} le long de l'axe du dégradé). Un pourcentage à `0%`, ou une longueur de `0`, représente le centre du dégradé&nbsp;; la valeur `100%` représente l'intersection de la forme du contour avec la ligne d'évolution du dégradé. Les valeurs de pourcentages intermédiaire sont positionnées linéairement le long du dégradé.
...
Le dégradé elliptique a son centre situé à 20% du coin supérieur gauche et se répète 10 fois entre le centre et le coin le plus éloigné (situé donc dans le coin inférieur droit). Les navigateurs qui prennent en charge les arrêts de couleur multiples affichent des bandes elliptiques rouges et vertes. Les navigateurs qui n'implémentent pas cette fonctionnalité affichent un dégradé qui va du rouge au noir puis du bleu au vert.
