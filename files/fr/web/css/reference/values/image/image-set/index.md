...
Les navigateurs ne fournissent pas d'informations particulières aux outils d'assistance quant aux outils d'assistance. Ainsi, les lecteurs d'écran ne peuvent pas annoncer d'informations utiles si l'image contient des informations essentielles à la compréhension de la page. Il faut inclure une description sémantique sur l'image pour que tous puissent en bénéficier.
...
{{EmbedLiveSample("image-set-example", "", 250)}}

### Utiliser `image-set()` pour fournir des formats d'image alternatifs

Dans l'exemple qui suit, la fonction `type()` est utilisée afin de servir une image aux formats AVIF et JPEG. Si le navigateur prend en charge le format AVIF, il choisit cette version, sinon il utilise la version JPEG.
...
{{EmbedLiveSample("image-set-type-example", "", 250)}}
