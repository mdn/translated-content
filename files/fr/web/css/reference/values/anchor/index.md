...
Retourne une valeur de type {{CSSxRef("&lt;length&gt;")}}.
...
```css
.fenetreContextuellePositionnee {
  margin: 0;
  inset: auto;
}
```
...
Bien qu'un élément positionné puisse être placé par rapport à plusieurs éléments d'ancre, il n'est toujours associé qu'à une seule ancre, définie avec sa propriété {{CSSxRef("position-anchor")}} (ou l'attribut HTML [`anchor`](/fr/docs/Web/HTML/Reference/Global_attributes/anchor)). C'est cette ancre qui défile avec l'élément lors du défilement de la page&nbsp;; elle peut aussi servir à contrôler quand l'élément est [masqué conditionnellement](/fr/docs/Web/CSS/Guides/Anchor_positioning/Try_options_hiding#masquer_conditionnellement_des_éléments_positionnés_avec_une_ancre).
...
Nous incluons un élément HTML {{HTMLElement("div")}} qui est défini comme notre ancre, et un élément HTML {{HTMLElement("p")}} que nous positionnons par rapport à cette ancre&nbsp;:

```html
<div class="ancre">⚓︎</div>

<p class="elementPositionne">Ceci est un élément positionné.</p>
```
...
<form>
  <div>
    <label for="ancre-cote-haut">
      Définir une valeur verticale <code>anchor()</code>&nbsp;:
    </label>
    <select id="ancre-cote-haut" name="ancre-cote-haut">
  ...
  <div>
    <label for="ancre-cote-gauche">
      Définir une valeur horizontale <code>anchor()</code>&nbsp;:
    </label>
    <select id="ancre-cote-gauche" name="ancre-cote-gauche">
...
Nous définissons une `<div>` `ancre` comme élément d'ancre en lui attribuant un nom d'ancre avec la propriété {{CSSxRef("anchor-name")}}. Nous l'associons ensuite à l'élément positionné en définissant la même valeur pour sa propriété {{CSSxRef("position-anchor")}}. `top: anchor(--mon-ancre bottom)` place le bord supérieur de la boîte d'information contre le bord inférieur de son ancre, tandis que `left: anchor(right)` place le bord gauche de la boîte d'information contre le bord droit de son ancre. Cette position initiale est modifiée lorsque différentes valeurs sont choisies dans les menus déroulants.
...
```js
const boiteInfo = document.querySelector(".boite-info");
const selecteurHaut = document.querySelector("#ancre-cote-haut");
const selecteurGauche = document.querySelector("#ancre-cote-gauche");

selecteurHaut.addEventListener("change", (e) => {
  const coteAncre = e.target.value;
  boiteInfo.style.top = `anchor(--mon-ancre ${coteAncre})`;
});

selecteurGauche.addEventListener("change", (e) => {
  const coteAncre = e.target.value;
  boiteInfo.style.left = `anchor(${coteAncre})`;
});
```
...
Nous définissons au total trois éléments HTML {{HTMLElement("div")}}. Les deux premiers ont la classe `ancre` et sont définis comme ancres&nbsp;; chacun possède un `id` individuel qui sert à leur fournir des informations de positionnement différentes. Le dernier `<div>` a la classe `boite-info` et est défini comme l'élément positionné. Nous incluons l'attribut [`tabindex`](/fr/docs/Web/HTML/Reference/Global_attributes/tabindex) pour leur permettre de recevoir la sélection clavier.

```html
<div id="ancre1" class="ancre" tabindex="0">⚓︎1</div>

<div id="ancre2" class="ancre" tabindex="0">⚓︎2</div>
...
#ancre1 {
  anchor-name: --mon-ancre1;
  top: 50px;
  left: 100px;
}

#ancre2 {
  anchor-name: --mon-ancre2;
  top: 200px;
  left: 350px;
}
```
...
.boite-info {
  ...
  position: fixed;
  top: anchor(--mon-ancre1 100%);
  left: anchor(--mon-ancre1 100%);
  bottom: anchor(--mon-ancre2 0%);
  right: anchor(--mon-ancre2 0%);
}
```
...
ancres.forEach((ancre) => rendreDeplacable(ancre));

function rendreDeplacable(elem) {
  let pos1, pos2, pos3, pos4;

  elem.onmousedown = deplacementAppuisSouris;
  ...
  function arreterDeplacerElement() {
