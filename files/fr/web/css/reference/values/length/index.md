...
### Valeurs définies et valeurs calculées

La [valeur définie](/fr/docs/Web/CSS/Guides/Cascade/Property_value_processing#valeur_définie) d'une longueur (_longueur définie_) est représentée par sa quantité et son unité. La [valeur calculée](/fr/docs/Web/CSS/Guides/Cascade/Property_value_processing#valeur_calculée) d'une longueur (_longueur calculée_) est la longueur définie résolue en une longueur absolue, et son unité n'est pas distinguée.

Pour certaines propriétés, telles que `border-width`, `outline-width`, `column-rule-width` et `outline-offset`, les valeurs calculées de `<length>` sont arrondies à un nombre entier de {{Glossary("device pixel", "pixels de l'appareil")}} pour garantir un affichage visuel raisonnable&nbsp;:

- Une valeur non nulle inférieure à 1 pixel de l'appareil est arrondie à l'unité supérieure.
- Une valeur supérieure à 1 pixel de l'appareil est arrondie à l'unité inférieure la plus proche.

Par exemple, sur un écran avec un {{DOMxRef("Window.devicePixelRatio", "devicePixelRatio")}} de 3, `border-width: 1.5px` est calculé à environ `1.33px` (arrondi à l'unité inférieure de 4,5 à 4 pixels de l'appareil), et `outline-width: 0.2px` est calculé à environ `0.33px` (arrondi à l'unité supérieure de 0,6 à 1 pixel de l'appareil).
...
> [!NOTE]
> Les éléments enfants n'héritent pas des valeurs relatives telles que définies pour leur parent&nbsp;; ils héritent des valeurs calculées.
...
Ces unités définissent une longueur relative aux dimensions du conteneur de requête.
...
- `cqw`
  - : Représente un pourcentage de la largeur du conteneur de requête. `1cqw` correspond à 1% de la largeur du conteneur de requête. Par exemple, si la largeur du conteneur de requête est de `800px`, une valeur de `50cqw` pour une propriété donne `400px`.

- `cqh`
  - : Représente un pourcentage de la hauteur du conteneur de requête. `1cqh` correspond à 1% de la hauteur du conteneur de requête. Par exemple, si la hauteur du conteneur de requête est de `300px`, une valeur de `10cqh` pour une propriété donne `30px`.

- `cqi`
  - : Représente un pourcentage de la taille en ligne du conteneur de requête. `1cqi` correspond à 1% de la taille en ligne du conteneur de requête. Par exemple, si la taille en ligne du conteneur de requête est de `800px`, une valeur de `50cqi` pour une propriété donne `400px`.

- `cqb`
  - : Représente un pourcentage de la taille de bloc du conteneur de requête. `1cqb` correspond à 1% de la taille de bloc du conteneur de requête. Par exemple, si la taille de bloc du conteneur de requête est de `300px`, une valeur de `10cqb` pour une propriété donne `30px`.

- `cqmin`
  - : Représente un pourcentage de la plus petite valeur entre la taille en ligne ou la taille de bloc du conteneur de requête. `1cqmin` correspond à 1% de la plus petite valeur entre la taille en ligne ou la taille de bloc du conteneur de requête. Par exemple, si la taille en ligne du conteneur de requête est de `800px` et sa taille de bloc de `300px`, une valeur de `50cqmin` pour une propriété donne `150px`.

- `cqmax`
  - : Représente un pourcentage de la plus grande valeur entre la taille en ligne ou la taille de bloc du conteneur de requête. `1cqmax` correspond à 1% de la plus grande valeur entre la taille en ligne ou la taille de bloc du conteneur de requête. Par exemple, si la taille en ligne du conteneur de requête est de `800px` et sa taille de bloc de `300px`, une valeur de `50cqmax` pour une propriété donne `400px`.
...
Lorsqu'elles sont animées, les valeurs du type de données `<length>` sont interpolées comme des nombres réels à virgule flottante. {{Glossary("interpolation", "L'interpolation")}} s'effectue sur la valeur calculée. La vitesse de l'interpolation est déterminée par la {{CSSxRef("easing-function","fonction de temporisation","",1)}} associée à l'animation.

## Exemples

### Comparer différentes unités de longueur

L'exemple suivant vous propose un champ de saisie dans lequel vous pouvez entrer une valeur `<length>` (par exemple&nbsp;: `300px`, `50%`, `30vw`) pour définir la largeur d'une barre de résultat qui apparaît en dessous après avoir appuyé sur la touche <kbd>Entrée</kbd> ou <kbd>Retour arrière</kbd>.
