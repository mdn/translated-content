...
1. [Sélecteurs de type](/en-US/docs/Web/CSS/Type_selectors) (ex.&nbsp: `h1`) et pseudo-éléments (ex.&nbsp: `::before`).
2. [Sélecteurs de classe](/en-US/docs/Web/CSS/Class_selectors) (ex.&nbsp: `.exemple`), sélecteurs d’attributs (ex.&nbsp: `[type="radio"]`) et pseudo-classes (ex.&nbsp: `:hover`).
3. [Sélecteurs d’ID](/en-US/docs/Web/CSS/ID_selectors) (ex.&nbsp: `#exemple`).

Sélecteur universel ({{CSSxRef("Universal_selectors", "*")}}), combinateurs ({{CSSxRef("Adjacent_sibling_combinator", "+")}}, {{CSSxRef("Child_combinator", "&gt;")}}, {{CSSxRef("General_sibling_combinator", "~")}}, [" "](/en-US/docs/Web/CSS/Descendant_combinator), {{CSSxRef("Column_combinator", "||")}}) et pseudo-classe de négation ({{CSSxRef(":not", ":not()")}}) n’ont aucun effet sur la spécificité (cependant, les sélecteurs déclarés _à l’intérieur de_ `:not()` ont un effet).

Pour davantage d’informations, veuillez visiter&nbsp;: ["Spécificité" dans "Cascade et héritage"](/en-US/docs/Learn/CSS/Building_blocks/Cascade_and_inheritance#Specificity_2), vous pouvez également visiter&nbsp;: <https://specifishity.com>
...
```html
<div class="foo" style="color: red;">De quelle couleur suis-je ?</div>
```
...
Comment faites-vous pour rendre les paragraphes de classe `awesome` toujours en rouge, même ceux qui se trouvent dans `#someElement`&nbsp;? Sans `!important`, la première règle aura davantage de spécificité et prendra le dessus sur la seconde règle.

#### Prendre le dessus sur `!important`

A) Ajouter une autre règle CSS avec `!important` et, soit donner au sélecteur une spécificité plus haute (en ajoutant une balise, ID ou classe au sélecteur), soit ajouter une règle CSS avec le même sélecteur plus loin dans le fichier CSS. Cela fonctionne car lorsqu’on est en présence d’une égalité de spécificité, la dernière règle définie est prise en compte.
...
Inclure une ID comme un sélecteur d’attribut au lieu d’un sélecteur d’ID (`#someElement`) lui donne la même spécificité qu’une classe. Les deux sélecteurs ci-dessus ont à présent la même importance. Lors d’une égalité de spécificité, la dernière règle définie est prise en compte.
...
La spécificité est basée sur la forme d’un sélecteur. Dans le cas suivant, le sélecteur `*[id="foo"]` compte pour un sélecteur d’attribut lorsqu’on détermine la spécificité du sélecteur, même s’il cible une ID.
...
```html
<p id="foo">Je suis un texte d’exemple.</p>
```
...
Cela est dû au fait qu’il corresponde au même élément mais que le sélecteur d’ID a une spécificité plus haute.
...
| {{SpecName("CSS3 Selectors", "#specificity", "Calculer la spécificité d’un sélecteur")}}         | {{Spec2("CSS3 Selectors")}} | Ajout des [pseudo-éléments](/en-US/docs/Web/CSS/Pseudo-elements).                           |
| {{SpecName("CSS2.1", "cascade.html#specificity", "Calculer la spécificité d’un sélecteur")}}     | {{Spec2("CSS2.1")}}             | Ajout des [pseudo-classes](/en-US/docs/Web/CSS/Pseudo-classes).                             |
| {{SpecName("CSS1", "#cascading-order", "Ordre dans la cascade")}}                                             | {{Spec2("CSS1")}}             | Définition initiale.                                                                   |

## Voir également

- Calculateur de spécificité&nbsp;: un site web interactif permettant de tester et comprendre vos propre règles CSS - <https://specificity.keegan.st/>
