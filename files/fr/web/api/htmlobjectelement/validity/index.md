---
title: "HTMLObjectElement : propriété validity"
...
La propriété en lecture seule **`validity`** de l'interface {{DOMxRef("HTMLObjectElement")}} retourne un objet {{DOMxRef("ValidityState")}} qui représente les états de validité de cet élément. Bien que les éléments {{HTMLElement("object")}} ne soient jamais candidats à la [validation de contraintes](/fr/docs/Web/HTML/Guides/Constraint_validation), l'état de validité peut tout de même être invalide si un message de validité personnalisé a été défini.
...
L'exemple suivant montre qu'un `<object>` est dans un état invalide lorsqu'un {{DOMxRef("ValidityState/customError", "customError")}} est défini&nbsp;; dans cet état, {{DOMxRef("HTMLObjectElement/checkValidity", "checkValidity()")}} retourne `true` tandis que la propriété `valid` de `validityState` est `false`.
