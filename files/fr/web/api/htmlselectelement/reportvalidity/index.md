...
La méthode **`reportValidity()`** de l'interface {{DOMxRef("HTMLSelectElement")}} effectue les mêmes étapes de vérification de validité que la méthode {{DOMxRef("HTMLSelectElement.checkValidity", "checkValidity()")}}. De plus, si l'évènement {{DOMxRef("HTMLElement/invalid_event", "invalid")}} n'est pas annulé, le navigateur affiche le problème à l'utilisateur·ice.
