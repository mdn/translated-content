...
> [!NOTE]
> Il est en fait possible de modifier les définitions et les orientations de ces systèmes de coordonnées grâce à des propriétés CSS telles que {{CSSxRef("transform")}}. Toutefois, nous évoquons uniquement le système de coordonnées standard.
...
Le coin supérieur gauche de la fenêtre est toujours `(0, 0)`, quel que soit le contenu du document ou tout défilement qui a pu avoir lieu. Autrement dit, le défilement du document modifie les coordonnées de la zone d'affichage d'une position donnée dans le document.
...
Cela signifie qu'un point donné sur un élément conserve les mêmes coordonnées sur la page (sauf si l'élément est déplacé avec un changement de position ou à cause de l'ajout d'autres éléments sur la page ou à cause d'un redimensionnement d'un autre élément par exemple).
...
Pour le système de coordonnées lié à l'écran, l'origine est situé dans le coin supérieur gauche de l'écran. Cela signifie que la position d'un point donné évolue si l'utilisateur·ice déplace la fenêtre du navigateur ou s'il change de résolution (voire s'il ajoute des écrans).
...
Dans cet exemple simple, nous allons créé un ensemble de boîtes imbriquées. Lorsque la souris entre sur la surface de ces boîtes, se déplace ou quitte la surface correspondante, l'évènement déclenché est géré afin de mettre à jour les messages informatifs au sein de la boîte pour afficher les différentes coordonnées du pointeur selon les quatre systèmes de coordonnées.
