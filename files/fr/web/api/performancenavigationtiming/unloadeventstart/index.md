---
title: "PerformanceNavigationTiming : propriété unloadEventStart"
...
La propriété en lecture seule **`unloadEventStart`** retourne un objet {{DOMxRef("DOMHighResTimeStamp")}} représentant le temps immédiatement avant que le gestionnaire d'évènements {{DOMxRef("Window/unload_event", "unload")}} du document précédent ne commence.

## Valeur

La propriété `unloadEventStart` peut avoir les valeurs suivantes&nbsp;:

- Un objet {{DOMxRef("DOMHighResTimeStamp")}} représentant le temps immédiatement avant que le gestionnaire d'évènements {{DOMxRef("Window/unload_event", "unload")}} du document précédent ne démarre.
...
La propriété `unloadEventStart` peut être utilisée pour mesurer combien de temps il faut pour traiter le gestionnaire d'évènements {{DOMxRef("Window/unload_event", "unload")}}.
