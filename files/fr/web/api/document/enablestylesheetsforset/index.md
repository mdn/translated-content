...
La méthode **`enableStyleSheetsForSet()`** de l'interface {{DOMxRef("Document")}} active les feuilles de styles correspondant au nom défini dans l'ensemble de feuilles de styles en cours et désactive toutes les autres (à l'exception de celles sans titre qui sont toujours activées).
...
- `name`
  - : Le nom des feuilles de style à activer. Toutes les feuilles de style dont le titre correspond à ce nom sont activées, tandis que toutes les autres qui ont un titre sont désactivées.
