...
- Valeur contenant une ou plusieurs URL, par exemple&nbsp;:
  ...
  Ceci est utile lorsque la ressource demandée n'est pas sur un serveur que vous contrôlez, ou si vous souhaitez gérer l'enregistrement de la source d'attribution sur un autre serveur. Dans ce cas, vous pouvez définir une ou plusieurs URL comme valeur de `attributionSrc`. Lors de la requête de la ressource, l'en-tête {{HTTPHeader("Attribution-Reporting-Eligible")}} est envoyé aux URL définies dans `attributionSrc` en plus de l'origine de la ressource. Ces URL peuvent alors répondre avec un en-tête {{HTTPHeader("Attribution-Reporting-Register-Source")}} ou {{HTTPHeader("Attribution-Reporting-Register-Trigger")}} selon le cas pour compléter l'enregistrement.
