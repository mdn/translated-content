...
- `windowFeatures` {{Optional_Inline}}
  - : Une chaîne de caractères contenant une liste de fonctionnalités de fenêtre, séparées par des virgules, de la forme `nom=valeur`. Les valeurs booléennes peuvent être définies sur true en utilisant l'une des options suivantes&nbsp;: `nom`, `nom=yes`, `nom=true` ou `nom=n` où `n` est un entier non nul. Ces fonctionnalités incluent des options telles que la taille et le positionnement par défaut de la fenêtre, s'il faut ouvrir une fenêtre popup minimale, etc. Les options suivantes sont prises en charge&nbsp;:
    ...
    - `popup`
      - : Par défaut, `window.open` ouvre la page dans un nouvel onglet. Si `popup` est défini sur true, il est demandé d'utiliser une fenêtre intrusive minimale. Les fonctionnalités de l'interface utilisateur incluses dans la fenêtre intrusive sont automatiquement décidées par le navigateur, généralement en incluant uniquement une barre d'adresse. Si `popup` est présent et défini sur faux, un nouvel onglet est toujours ouvert.

        Il existe quelques fonctionnalités héritées, qui contrôlent autrefois les fonctionnalités de l'interface utilisateur de la fenêtre ouverte. Dans les navigateurs modernes, elles n'ont pour effet que de demander une fenêtre intrusive. Si `popup` n'est pas défini, et que `windowFeatures` contient des fonctionnalités (y compris non reconnues) autres que `noopener`, `noreferrer` ou `attributionsrc`, la fenêtre est également ouverte en tant que fenêtre intrusive si l'une des conditions suivantes s'applique&nbsp;:
    ...
    - `left` ou `screenX`
      - : Définit la distance, exprimée en pixels, depuis le bord gauche de la zone de travail définie par le système d'exploitation, à laquelle la fenêtre est ouverte.

    - `top` ou `screenY`
      - : Définit la distance, exprimée en pixels, depuis le bord haut de la zone de travail définie par le système d'exploitation, à laquelle la fenêtre est ouverte.

    - `noopener`
      - : Si cette fonctionnalité est utilisée, la nouvelle fenêtre n'a pas accès à la fenêtre d'origine avec {{DOMxRef("Window.opener")}} (qui retourne alors `null`).
    ...
    - `noreferrer`
      - : Si cette fonctionnalité est utilisée, le navigateur ne fournit pas d'en-tête {{HTTPHeader("Referer")}} et fixe d'office `noopener` à `true`. Voir [`rel="noreferrer"`](/fr/docs/Web/HTML/Reference/Attributes/rel#noreferrer) pour plus d'informations.

    Une valeur {{JSxRef("null")}} est traitée de la même manière que la chaîne de caractères vide (`""`).

> [!NOTE]
> Les valeurs de position demandées (`top`, `left`) et de dimension demandées (`width`, `height`) dans `windowFeatures` **sont corrigées** si l'une de ces valeurs demandées ne permet pas à la fenêtre intrusive du navigateur d'être entièrement rendue dans la zone de travail des applications du système d'exploitation de l'utilisateur·ice. En d'autres termes, aucune partie de la nouvelle fenêtre intrusive ne peut être initialement positionnée hors écran.
...
La méthode `open()` de l'interface {{DOMxRef("Window")}} prend comme paramètre une URL et charge la ressource correspondante dans un nouveau contexte de navigation ou un contexte existant.
Le paramètre `target` détermine la fenêtre, l'onglet ou le cadre dans lequel charger la ressource, et le paramètre `windowFeatures` peut être utilisé pour contrôler les fonctionnalités de la nouvelle fenêtre, comme s'il s'agit d'un onglet ou d'une fenêtre contextuelle avec une interface utilisateur minimale, sa taille et sa position, etc.

Lorsque `window.open()` crée un nouveau contexte de navigation (c'est-à-dire lorsqu'aucune fenêtre existante portant ce nom n'est trouvée), la fenêtre contient initialement `about:blank`.
Si une URL différente a été fournie, elle est chargée de manière asynchrone, et l'objet global est réutilisé pour cette navigation si elle est de même origine — donc toutes les propriétés définies sur la fenêtre avant le chargement peuvent persister.
Si `target` fait référence à un contexte navigable existant (`_self`, `_parent`, `_top` ou un nom de fenêtre connu), aucune phase `about:blank` ne se produit — le navigateur navigue directement dans le contexte existant.

Les navigateurs modernes ont des politiques strictes de blocage des fenêtres contextuelles. Les fenêtres contextuelles doivent être ouvertes en réponse directe à une action de l'utilisateur·ice, et un évènement de geste utilisateur·ice distinct est requis pour chaque appel à `Window.open()`. Cela empêche les sites de spammer les utilisateur·ice·s avec de nombreuses fenêtres. Cependant, cela pose un problème pour les applications multi-fenêtres. Pour contourner cette limitation, vous pouvez concevoir vos applications de manière à&nbsp;:
...
if (!handle) {
  ...
  // C'est probablement dû aux bloqueurs de fenêtres contextuelles
  // intégrés.
  // …
}
```
...
Dans certains cas, JavaScript est désactivé ou indisponible et `window.open()` ne fonctionne pas. Au lieu de se fier uniquement à la présence de cette fonctionnalité, nous pouvons fournir une solution alternative afin que le site ou l'application continue de fonctionner.

### Fournir des alternatives lorsque JavaScript est désactivé

Si le support de JavaScript est désactivé ou inexistant, l'agent utilisateur crée une fenêtre secondaire en conséquence ou rend la ressource référencée selon sa gestion de l'attribut `target`. L'objectif et l'idée sont de fournir (et _non d'imposer_) à l'utilisateur·ice un moyen d'ouvrir la ressource référencée.
...
```js
let windowObjectReference = null; // variable globale
let previousURL; /* variable globale qui stocke l'URL
...
Si le contexte de navigation nouvellement ouvert ne partage pas la même {{Glossary("Origin", "origine")}}, le script d'ouverture ne peut pas interagir (lecture ou écriture) avec le contenu du contexte de navigation.
...
Si nécessaire, utilisez un élément HTML {{HTMLElement("button")}} à la place. En général, _vous ne devez utiliser un lien que pour la navigation vers une URL réelle_.

### Toujours identifier les liens menant à une fenêtre secondaire

Identifiez les liens qui ouvrent de nouvelles fenêtres d'une manière qui aide à la navigation pour les utilisateur·ice·s.
...
Lorsque des changements extrêmes de contexte sont explicitement identifiés avant qu'ils ne se produisent, les utilisateur·ice·s peuvent déterminer s'ils souhaitent continuer ou se préparer au changement&nbsp;: non seulement ils ne sont pas confus ou désorientés, mais les utilisateur·ice·s plus expérimenté·e·s peuvent mieux décider comment ouvrir de tels liens (dans une nouvelle fenêtre ou non, dans la même fenêtre, dans un nouvel onglet ou non, en «&nbsp;arrière-plan&nbsp;» ou non).
