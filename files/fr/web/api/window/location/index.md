---
title: "Window : propriété location"
...
### Exemple 2 : recharger la page courante
...
### Exemple 4 : afficher les propriétés de l'URL courante

```js
function showLoc() {
  const logLines = [
    "Propriété (Typeof): Valeur",
...
// dans le html : <button onclick="showLoc();">Afficher les propriétés de location</button>
```

### Exemple 5 : envoyer une chaîne de données au serveur en modifiant la propriété `search`
...
<p id="myBookmark1">
  [&nbsp;<span class="intLink">Aller à la marque #2</span>&nbsp;]
</p>
<p>
...
<p id="myBookmark2">
  [&nbsp;<span class="intLink">Aller à la marque #1</span>
  |
  <span class="intLink">
    Aller à la marque #1 sans utiliser location.hash
  </span>
  |
  <span class="intLink">Aller à la marque #3</span>&nbsp;]
</p>
<p>
...
<p id="myBookmark3"><em>Voici la marque #3</em></p>
<p>
...
…la même chose mais avec un défilement de page animé&nbsp;:
...
const showBookmark = (() => {
  ...
  // duration: la durée en millisecondes de chaque frame
  // frames: nombre de frames pour chaque défilement
  let duration = 200;
  let frames = 10;
...
- L'interface qui décrit la valeur retournée par cette propriété, {{DOMxRef("Location")}}
- Une information similaire, mais attachée au document courant, {{DOMxRef("Document.location")}}
