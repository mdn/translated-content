...
Les fenêtres qui font partie d'un groupe d'agents indexé par origine sont envoyées à certaines restrictions supplémentaires, par rapport aux fenêtres qui n'en font pas partie. En particulier, elles ne peuvent pas&nbsp;:
...
- Envoyer des objets [`WebAssembly.Module`](/fr/docs/WebAssembly/Reference/JavaScript_interface/Module) à d'autres pages inter-origine d'un même site avec {{DOMxRef("Window.postMessage()")}}.
...
Notez que la fonctionnalité de groupe d'agents indexé par origine n'est prise en charge que dans les {{Glossary("Secure Context", "contextes sécurisés")}}. Si un site n'est pas un contexte sécurisé, `window.originAgentCluster` retourne toujours `false`.
