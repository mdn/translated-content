---
title: "HTMLMediaElement : évènement stalled"
short-title: stalled
...
L'évènement `stalled` est déclenché lorsque l'agent utilisateur tente de récupérer des données multimédias, mais que les données ne sont pas disponibles de manière inattendue.
...
Ces exemples ajoutent un écouteur d'évènement pour l'évènement `stalled` de l'élément HTMLMediaElement, puis envoient un message lorsque ce gestionnaire d'évènement a réagi au déclenchement de l'évènement.
...
video.addEventListener("stalled", (event) => {
  console.log("Échec de la récupération des données, mais tentative en cours.");
});
```

Utiliser la propriété de gestionnaire d'évènement `onstalled`&nbsp;:
