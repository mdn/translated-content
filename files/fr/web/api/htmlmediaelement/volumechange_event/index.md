---
title: "HTMLMediaElement : évènement volumechange"
...
L'évènement `volumechange` est déclenché lorsque l'attribut {{DOMxRef("HTMLMediaElement.volume", "volume")}} ou l'attribut {{DOMxRef("HTMLMediaElement.muted", "muted")}} a changé.
...
Ces exemples ajoutent un écouteur d'évènement pour l'évènement `volumechange` de l'HTMLMediaElement, puis affichent un message lorsque ce gestionnaire d'évènement a réagi au déclenchement de l'évènement.
...
video.addEventListener("volumechange", (event) => {
  console.log("Le volume a changé.");
});
```

Utiliser la propriété de gestionnaire d'évènement `onvolumechange`&nbsp;:
