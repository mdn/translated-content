...
3. Le paramètre `scope` est optionnel et peut être utilisé pour définir le sous-ensemble de votre contenu que vous souhaitez que le <i lang="en">service worker</i> contrôle. Dans ce cas, nous avons indiqué `'/'`, ce qui signifie tout le contenu sous l'origine de l'application. Si vous l'omettez, cette valeur est utilisée par défaut, mais nous l'avons précisée ici à des fins d'illustration.
...
1. On ajoute un gestionnaire d'évènement pour `install` sur le <i lang="en">service worker</i> (représenté par `self`) et on chaîne un appel à la méthode {{DOMxRef("ExtendableEvent.waitUntil()")}} lors de la réception de l'évènement, pour s'assurer que l'installation du <i lang="en">service worker</i> ne commence pas avant que le code contenu dans `waitUntil()` ait été exécuté.
2. Dans la fonction `addResourcesToCache()`, on utilise la méthode {{DOMxRef("CacheStorage.open", "caches.open()")}} afin de créer un nouveau cache intitulé `v1`, qui est la première version de notre cache pour les ressources de notre site. On appelle ensuite une fonction `addAll()` sur le cache ainsi créé et qui prend en paramètre un tableau d'URL pour les différentes ressources qu'on souhaite mettre en cache. Les URL sont relatives à la {{DOMxRef("WorkerGlobalScope.location", "location", "", 1)}} du <i lang="en">worker</i>.
3. Si la promesse échoue, l'installation échoue et le <i lang="en">service worker</i> ne fait rien. Il est toujours possible de corriger le code et de réessayer lors du prochain enregistrement.
4. Après une installation réussie, on passe à l'activation du <i lang="en">service worker</i>. Lors d'une première installation, cela peut ne pas avoir beaucoup d'intérêt, mais cela s'avère utile lors des mises à jour du <i lang="en">service worker</i> (voir la section [Mettre à jour le <i lang="en">service worker</i>](#mettre_à_jour_le_service_worker) ci-après).
...
`caches.match(event.request)` fonctionne à merveille s'il y a une ressource correspondante dans le cache du <i lang="en">service worker</i>, mais que se passe-t-il si ce n'est pas le cas&nbsp;? Si on ne fournit pas de gestion d'erreur, la promesse est résolue avec `undefined` et rien n'est retourné.
...
Une stratégie plus raffinée est de mettre en cache les ressources que nous récupérons depuis le réseau afin qu'elles puissent être réutilisées hors ligne par la suite. Dans notre exemple, cela signifie que si nous ajoutons de nouvelles images à la galerie, notre application peut automatiquement les récupérer la première fois et les mettre en cache. Voici un fragment de code qui implémente cette méthode&nbsp;:
...
Cloner la réponse est nécessaire, car les flux de requête et de réponse ne peuvent être lus qu'une seule fois. Afin de fournir la réponse au navigateur et la mettre en cache, il faut la cloner. La version originale est fournie au navigateur et le clon'est mis en cache. Chaque réponse est lue une seule fois.
...
Le problème restant est que si la requête ne correspond à rien en cache et que le réseau n'est pas disponible, la requête échoue. Voyons comment fournir un contenu par défaut dans ce cas-là&nbsp;:
...
Ensuite, on utilise {{DOMxRef("FetchEvent/preloadResponse", "event.preloadResponse")}} pour attendre que le téléchargement de la ressource préchargée soit terminé dans le gestionnaire d'évènement `fetch`.
...
const cacheFirst = async ({
  ...
  const responseFromCache = await caches.match(request);
  if (responseFromCache) {
    // Garde la requête de préchargement à la navigation active même si nous n'utilisons pas sa réponse.
...
Notez que dans cet exemple qu'on télécharge et met en cache les mêmes données pour la ressource qu'elle soit téléchargée normalement ou préchargée. On peut aussi choisir de télécharger et de mettre en cache une ressource différente lors du préchargement. Pour plus d'informations, voir [`NavigationPreloadManager` > réponses personnalisées](/fr/docs/Web/API/NavigationPreloadManager#réponses_personnalisées).
...
Comme nous l'avons vu dans la section précédente, lorsqu'on met à jour un <i lang="en">service worker</i> avec une nouvelle version, on peut créer un nouveau cache avec le gestionnaire d'évènement `install`. Tant qu'il y a des pages ouvertes qui sont contrôlées par l'ancienne version, il faut conserver les deux caches, car la version précédente utilise cette version précédente du cache. L'évènement `activate` peut ensuite être utilisé pour retirer des données des caches précédents.

Les promesses passées à `waitUntil()` bloquent les autres évènements tant qu'elles ne sont pas terminées. Cela permet de s'assurer que les étapes de nettoyage ont été réalisées lorsque le premier évènement `fetch` parvient au nouveau <i lang="en">service worker</i>.
...
- [Edge <sup>(angl.)</sup>](https://learn.microsoft.com/en-us/microsoft-edge/devtools/progressive-web-apps/#service-workers)
