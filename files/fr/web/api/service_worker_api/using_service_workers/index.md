...
2. Dans la fonction `addResourcesToCache()`, on utilise la méthode {{DOMxRef("CacheStorage.open", "caches.open()")}} afin de créer un nouveau cache intitulé `v1`, qui sera la première version de notre cache pour les ressources de notre site. On appelle ensuite une fonction `addAll()` sur le cache ainsi créé et qui prend en paramètre un tableau d'URL pour les différentes ressources qu'on souhaite mettre en cache. Les URL sont relatives à la {{DOMxRef("WorkerGlobalScope.location", "location", "", 1)}} du <i lang="en">worker</i>.
