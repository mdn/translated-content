...
La propriété **`referrerPolicy`** de l'interface {{DOMxRef("HTMLIFrameElement")}} reflète l'attribut HTML [`referrerpolicy`](/fr/docs/Web/HTML/Reference/Elements/iframe#referrerpolicy) de l'élément HTML {{HTMLElement("iframe")}}, définissant le référent envoyé lors de la récupération de la ressource.
...
- `unsafe-url`
  - : Envoie une URL complète lors d'une requête de même origine ou inter-origines.
    ...
    > [!NOTE]
    > Cette politique divulgue les origines et les chemins des ressources protégées par TLS vers des origines non sécurisées. Veuillez bien considérer l'impact de ce paramètre.
