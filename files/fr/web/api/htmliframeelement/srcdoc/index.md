...
La propriété **`srcdoc`** de l'interface {{DOMxRef("HTMLIFrameElement")}} permet d'obtenir ou de définir le balisage HTML en incise du document du cadre.
...
Tout balisage pris en charge par le navigateur est analysé/sérialisé, y compris les {{Glossary("shadow tree", "arbre d'ombre")}}.

Notez que si cette propriété est définie, elle écrase toute valeur définie dans la propriété {{DOMxRef("HTMLIFrameElement.src", "src")}}.

### Considérations de sécurité

La propriété `srcdoc` permet par défaut à n'importe quel balisage HTML de s'exécuter dans un cadre.
Si le cadre n'est pas isolé à l'aide de la directive [`sandbox`](/fr/docs/Web/HTTP/Reference/Headers/Content-Security-Policy/sandbox) de la politique de sécurité du contenu (CSP) (ou s'il est isolé mais inclut la valeur [`allow-same-origin`](/fr/docs/Web/HTTP/Reference/Headers/Content-Security-Policy/sandbox#allow-same-origin)), il est de même origine que le parent.
Cela signifie que le cadre a un accès complet au DOM et aux ressources du parent, et inversement.

Il s'agit d'un vecteur important pour les attaques par [script inter-site (XSS)](/fr/docs/Web/Security/Attacks/XSS) si des chaînes de caractères potentiellement non sûres fournies par un·e utilisateur·ice sont injectées dans un cadre sans être d'abord assainies.
Considérez le code suivant où une chaîne de caractères HTML provenant d'un·e utilisateur·ice peut être passée dans un cadre qui est ensuite ajouté au document.
...
Le cadre est alors traité comme une ressource inter-origines et les attaques sont fortement limitées.
...
### Remplacer la source en incise du cadre

Dans cet exemple, nous allons remplacer le document d'un cadre en affectant du HTML à sa propriété `srcdoc`.
Pour atténuer le risque de XSS, nous allons d'abord créer un objet `TrustedHTML` à partir de la chaîne de caractères contenant le HTML, puis affecter cet objet à `srcdoc`.

Les types de confiance ne sont pas encore pris en charge sur tous les navigateurs, donc nous définissons d'abord la [petite prothèse d'émulation des types de confiance](/fr/docs/Web/API/Trusted_Types_API#petite_prothèse_démulation_des_types_de_confiance).
...
Ensuite, nous créons une {{DOMxRef("TrustedTypePolicy")}} qui définit une méthode {{DOMxRef("TrustedTypePolicy/createHTML", "createHTML()")}} pour transformer une chaîne de caractères d'entrée en instance de {{DOMxRef("TrustedHTML")}}.
...
Nous utilisons ensuite cet objet `policy` pour créer un objet `TrustedHTML` à partir de la chaîne de caractères d'entrée potentiellement non sûre, et affectons le résultat à l'élément&nbsp;:
...
> [!WARNING]
> Bien que vous puissiez affecter directement une chaîne de caractères à `srcdoc`, cela représente un [risque de sécurité](#considérations_de_sécurité) si la chaîne de caractères à insérer peut contenir du contenu potentiellement malveillant.
> Vous devez utiliser `TrustedHTML` pour vous assurer que le contenu est assaini avant d'être inséré, et définir un en-tête CSP pour [imposer les types de confiance](/fr/docs/Web/API/Trusted_Types_API#utiliser_une_csp_pour_appliquer_des_types_de_confiance).
