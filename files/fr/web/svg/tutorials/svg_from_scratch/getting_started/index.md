...
- La première chose importante à noter est l'ordre de rendu des éléments. La règle générale pour les fichiers SVG est que les éléments _plus tardifs_ sont rendus _au-dessus des précédents_. Plus un élément est bas dans le code, plus il sera visible.
- Les fichiers SVG peuvent être affichés directement dans un navigateur ou même être incorporés directement dans un fichier HTML, en suivant plusieurs méthodes&nbsp;:
  - Si le HTML est du XHTML et est distribué avec le type `application/xhtml+xml`, le SVG peut être directement intégré dans la source XML.
  - Le SVG peut aussi être intégré directement dans le HTML.
  - Une balise `img` peut être utilisée.
  - Le fichier SVG peut être référencé avec un élément `object`&nbsp;:
  ...
  - Enfin, SVG peut être créé dynamiquement avec JavaScript et injecté dans le DOM HTML.
...
Vous pouvez vérifier que votre serveur envoie le bon en-tête HTTP avec vos fichiers SVG en utilisant [le Moniteur réseau <sup>(angl.)</sup>](https://firefox-source-docs.mozilla.org/devtools-user/network_monitor/index.html#headers) ou un site comme [websniffer.com <sup>(angl.)</sup>](https://websniffer.com/). Fournissez l'URL d'un de vos fichiers SVG et regardez les en-têtes HTTP de la réponse. Si vous remarquez que votre serveur n'envoie pas les en-têtes avec les valeurs ci-dessus, vous devriez contacter votre hébergeur. Si vous avez du mal à le convaincre de configurer correctement leurs serveurs pour le SVG, il y a peut-être moyen de le faire vous-même. Regardez [la page de configuration d'un serveur <sup>(angl.)</sup>](https://www.w3.org/services/svg-server/) sur le site w3.org pour quelques solutions simples.
