...
L'entête **`X-Content-Type-Options`** est un marqueur utilisé par le serveur pour indiquer que les [types MIME](/fr/docs/Web/HTTP/Guides/MIME_types) annoncés dans les en-têtes {{HTTPHeader("Content-Type")}} ne doivent pas être modifiés ou et suivis. Cela permet de se détacher du sniffing de type [MIME](/fr/docs/Web/HTTP/Guides/MIME_types#détection_de_type_mime), ou, en d'autres termes, c'est une façon de dire que les webmasters savaient ce qu'ils faisaient.
