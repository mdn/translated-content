...
{{Glossary("response header", "L'en-tête de réponse")}} HTTP **`Content-DPR`** est utilisé pour confirmer le rapport de pixels de l'appareil pour une _image_ (DPR) dans les requêtes où l'indication client {{HTTPHeader("DPR")}} de l'écran a été utilisée pour sélectionner une ressource image.
...
Si l'indication client `DPR` est utilisée pour sélectionner une image, le serveur doit définir `Content-DPR` dans la réponse.
