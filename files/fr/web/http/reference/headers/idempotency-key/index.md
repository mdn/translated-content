...
{{Glossary("request header", "L'en-tête de requête")}} HTTP **`Idempotency-Key`** peut être utilisé pour rendre les requêtes {{Glossary("idempotent", "idempotentes")}} {{HTTPMethod("POST")}} et {{HTTPMethod("PATCH")}}.

Cela permet aux clients de retourner des requêtes non reconnues sans avoir à se soucier du fait que la requête a déjà été reçue et traitée par le serveur.
...
Cela signifie que vous pouvez envoyer un message avec ces méthodes autant de fois que vous le souhaitez et que l'état du serveur n'est pas modifié, ou est modifié de la même manière à chaque réception du message.
Par exemple, si vous envoyez plusieurs fois le même message `PUT`, il met à jour la même ressource sur le serveur à chaque fois, avec la même valeur.
...
L'idempotence est importante dans les cas où un client ne reçoit pas de réponse, car cela signifie qu'il peut retourner la requête sans craindre d'effets secondaires.
...
Le client peut alors retourner la même requête plusieurs fois, et le serveur sait qu'il ne doit effectuer l'action qu'une seule fois.
...
Une clé unique doit être utilisée pour chaque nouvelle requête envoyée, et la même clé doit être utilisée si cette requête est retournée.
...
- Si c'est le cas, il ne doit pas effectuer l'opération, mais doit répondre comme s'il l'a fait.
...
C'est utilisé pour répondre par une erreur si une clé et une empreinte ultérieures ne correspondent pas.
...
Dans le cas d'une réponse `409 Conflict`, les clients doivent attendre avant de réessayer.
Pour toutes les autres erreurs, les clients doivent modifier les requêtes avant de les retourner.
...
Si aucune réponse n'est reçue, le client peut retourner exactement la même requête en toute sécurité. Si le serveur n'a pas reçu la requête précédente, il agit sur celle-ci&nbsp;; s'il l'a déjà reçue, la seconde est ignorée, mais le serveur répond comme s'il a traité la seconde normalement.

Si le client retourne la requête trop rapidement, il peut recevoir une réponse d'erreur comme celle-ci.
