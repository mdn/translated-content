...
Certains clients et serveurs peuvent souhaiter être compatibles avec les approches précédentes des connexions persistantes, et peuvent le faire avec un en-tête de requête `Connection: keep-alive`.
...
- `<parameters>`
  - : Une liste de paramètres séparés par des virgules, chacun consistant en un identifiant et une valeur séparés par le signe égal (`=`).
    ...
    - `timeout`
      - : Un entier qui est le temps en secondes pendant lequel l'hôte autorise une connexion inactive à rester ouverte avant d'être fermée.
    ...
    - `max`
      - : Un entier qui est le nombre maximal de requêtes pouvant être envoyées sur cette connexion avant de la fermer.
        Sauf si la valeur est `0`, cette valeur est ignorée pour les connexions non pipeline, car une autre requête est envoyée dans la réponse suivante.
