...
{{Glossary("request header", "L'en-tête de requête")}} HTTP **`ECT`** est utilisé dans les [indications client](/fr/docs/Web/HTTP/Guides/Client_hints) pour indiquer le {{Glossary("effective connection type", "type de connexion effectif")}}&nbsp;: `slow-2g`, `2g`, `3g` ou `4g`.
...
Ensuite, lors des requêtes suivantes, le client peut retourner un en-tête `ECT`&nbsp;:
