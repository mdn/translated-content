...
L'en-tête HTTP {{HTTPHeader("Permissions-Policy")}} avec la directive **`language-detector`** contrôle l'accès à la fonctionnalité de détection de la langue des [API Translator et Language Detector](/fr/docs/Web/API/Translator_and_Language_Detector_APIs).

Plus précisément, lorsqu'une politique définie bloque l'utilisation de cette fonctionnalité, toute tentative d'appel des méthodes de détection de la langue de l'API échouera avec une {{DOMxRef("DOMException")}} de type `NotAllowedError`.
