...
Une requête `POST` est habituellement envoyée via un [formulaire HTML](/fr/docs/conflicting/Learn_web_development/Extensions/Forms) et a pour résultat un changement sur le serveur. Dans ce cas, le type du contenu est sélectionné en mettant la chaîne de caractères adéquate dans l'attribut _[`enctype`](/fr/docs/Web/HTML/Reference/Elements/form#enctype) de l'élément {{HTMLElement("form")}} ou dans l'attribut [`formenctype`](/fr/docs/Web/HTML/Reference/Elements/input#formenctype) de l'élément {{HTMLElement("input") }}, voir celui des éléments {{HTMLElement("button")}}_&nbsp;:
