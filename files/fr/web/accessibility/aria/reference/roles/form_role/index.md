...
- `onsubmit`
  - : Le gestionnaire d'évènements onSubmit gère l'évènement déclenché lorsque le formulaire est envoyé. Tout ce qui n'est pas un `<form>` ne peut pas être envoyé, vous devez donc utiliser JavaScript pour créer un mécanisme d'envoi de données alternatif, par exemple avec {{DOMxRef("Window/fetch", "fetch()")}}.
...
Les lecteurs d'écran annoncent le type de rôle que le repère a. De ce fait, vous n'avez pas besoin de décrire ce que le repère est dans son étiquette. Par exemple, une déclaration de `role="form"` avec un `aria-label="Contact form"` peut être annoncée de manière redondante comme, "formulaire de contact form".

## Bonnes pratiques

### Préférer HTML

L'utilisation de l'élément {{HTMLElement("form")}} communique automatiquement que l'élément a un rôle de formulaire (`form`). Si possible, privilégiez l'utilisation de l'élément sémantique `<form>` plutôt que le rôle `form`.
