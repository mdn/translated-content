...
Un élément avec `role="table"` est une structure tabulaire statique avec des lignes contenant des cellules. Les cellules ne sont ni sélectionnées ni sélectionnables, bien que des composants à l'intérieur de chaque cellule puissent être interactifs. Il est fortement recommandé d'utiliser l'élément HTML natif {{HTMLElement("table")}} chaque fois que possible.
