...
> [!NOTE]
> Gardez à l'esprit qu'un titre de dialogue et un texte descriptif n'ont pas besoin d'être sélectionnables pour être perçus par les lecteurs d'écran fonctionnant en mode non virtuel. La combinaison du rôle de dialogue ARIA et des techniques d'étiquetage doit permettre au lecteur d'écran d'annoncer les informations du dialogue lorsque la sélection est déplacée à l'intérieur.
...
- Les dialogues doivent toujours avoir au moins un contrôle sélectionnable. Pour de nombreux dialogues, il y a un bouton comme «&nbsp;Fermer&nbsp;», «&nbsp;OK&nbsp;» ou «&nbsp;Annuler&nbsp;». En plus du contrôle nécessaire, les dialogues peuvent contenir un nombre quelconque d'éléments sélectionnables, même des formulaires entiers ou d'autres composants de conteneurs comme des onglets.
- Lorsque le dialogue apparaît à l'écran, la sélection au clavier (dont le contrôle dépend de l'objectif du dialogue) doit être déplacé vers le contrôle sélectionnable par défaut à l'intérieur du dialogue. Pour les dialogues qui ne fournissent qu'un message de base, cela peut être un bouton «&nbsp;OK&nbsp;». Pour les dialogues contenant un formulaire, cela peut être le premier champ du formulaire.
...
- [`aria-labelledby`](/fr/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-labelledby)
  - : Utilisez cet attribut pour étiqueter le dialogue. Souvent, la valeur de l'attribut `aria-labelledby` est l'identifiant de l'élément utilisé pour titrer le dialogue.
