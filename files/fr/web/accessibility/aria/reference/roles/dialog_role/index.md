...
Un dialogue a des exigences particulières concernant la gestion du focus clavier&nbsp;:
...
- Lorsque le dialogue apparaît à l'écran, le focus clavier (dont le contrôle dépend de l'objectif du dialogue) doit être déplacé vers le contrôle focalisable par défaut à l'intérieur du dialogue. Pour les dialogues qui ne fournissent qu'un message de base, cela pourrait être un bouton «&nbsp;OK&nbsp;». Pour les dialogues contenant un formulaire, cela pourrait être le premier champ du formulaire.
- Après la fermeture du dialogue, le focus clavier doit être déplacé vers l'endroit où il se trouvait avant d'entrer dans le dialogue. Sinon, le focus peut être perdu au début de la page.
...
> [!NOTE]
> Quand il est possible d'empêcher les utilisateur·ice·s de clavier de déplacer le focus vers des éléments en dehors de la boîte de dialogue, les utilisateur·ice·s de lecteurs d'écran peuvent toujours être en mesure de naviguer vers ce contenu à l'aide du curseur virtuel de leur lecteur d'écran.
