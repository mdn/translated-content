...
Lorsqu'une interface à onglets est initialisée, un `tabpanel` est affiché et son onglet associé est mis en forme pour indiquer qu'il est actif, reflétant son état programmatique. Tous les éléments `tabpanel` inactifs doivent être masqués pour tous les utilisateur·ice·s, le plus souvent avec la propriété CSS `display: none`.
