...
- le rôle [`menu`](/fr/docs/Web/Accessibility/ARIA/Reference/Roles/menu_role)
  - : Un composant fournissant une liste de choix. Rôle de contexte requis (ou `menubar`).
...
- [`aria-haspopup`](/fr/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-haspopup)
  - : Indique la disponibilité et le type de fenêtre intrusive interactive qui peut être déclenché par le `menuitem`

### Interactions au clavier

- <kbd>Entrée</kbd> et <kbd>Espace</kbd>
  - : Si le `menuitem` a un sous-menu, ouvre le sous-menu et place la sélection sur son premier élément. Sinon, active l'élément et ferme le menu.
- <kbd>Flèche vers le bas</kbd>
  - : Sur un `menuitem` qui a un sous-menu dans un `menubar`, ouvre le sous-menu et place la sélection sur le premier élément du sous-menu. Sinon, déplace la sélection sur l'élément suivant, en option de l'enveloppement du dernier au premier.
- <kbd>Flèche vers le haut</kbd>
  - : Déplace la sélection vers l'élément précédent, en option de l'enveloppement du premier au dernier. En option, si le `menuitem` est dans un menubar et a un sous-menu, ouvre le sous-menu et place la sélection sur le dernier élément du sous-menu.
- <kbd>Flèche vers la droite</kbd>
  - : Si dans un `menu` ouvert avec un menubutton et pas dans un `menubar`, si le menuitem n'a pas de sous-menu, ne fait rien. Lorsque la sélection est dans un `menubar`, déplace la sélection vers l'élément suivant, en option de l'enveloppement du dernier au premier. Lorsque la sélection est dans un `menu` et sur un `menuitem` qui a un sous-menu, ouvre le sous-menu et place la sélection sur son premier élément. Lorsque la sélection est dans un `menu` et sur un élément qui n'a pas de sous-menu, ferme le sous-menu et tous les menus parents, déplace la sélection vers l'élément suivant dans le `menubar`, et, si la sélection est maintenant sur un `menuitem` avec un sous-menu, ouvre soit le sous-menu de ce `menuitem` sans déplacer la sélection dans le sous-menu, soit ouvre le sous-menu de ce `menuitem` et place la sélection sur le premier élément du sous-menu.
- <kbd>Flèche vers la gauche</kbd>
  - : Lorsque la sélection est dans un `menubar`, déplace la sélection vers l'élément précédent, en option de l'enveloppement du premier au dernier. Lorsque la sélection est dans un sous-menu d'un élément dans un menu, ferme le sous-menu et retourne la sélection au `menuitem` parent. Lorsque la sélection est dans un sous-menu d'un élément dans un `menubar`, ferme le sous-menu, déplace la sélection vers l'élément précédent dans le `menubar`, et, si la sélection est maintenant sur un `menuitem` avec un sous-menu, ouvre soit le sous-menu de ce `menuitem` sans déplacer la sélection dans le sous-menu, soit ouvre le sous-menu de ce `menuitem` et place la sélection sur le premier élément du sous-menu.
- <kbd>Début</kbd>
  - : Si l'enveloppement des touches fléchées n'est pas pris en charge, déplace la sélection vers le premier élément du `menu` ou du `menubar` actuel.
- <kbd>Fin</kbd>
  - : Si l'enveloppement des touches fléchées n'est pas pris en charge, déplace la sélection vers le dernier élément du `menu` ou du `menubar` actuel.
- Toute touche correspondant à un caractère imprimable (Optionnel)
  - : Déplace la sélection vers l'élément suivant dans le menu actuel dont l'étiquette commence par ce caractère imprimable.
- <kbd>Échap</kbd>
  - : Ferme le menu qui contient la sélection et retourne la sélection à l'élément ou au contexte, par exemple, le bouton de menu ou le `menuitem` parent, à partir duquel le menu a été ouvert.
- <kbd>Tab</kbd>
  - : Déplace la sélection vers l'élément suivant dans la séquence de tabulation, et si l'élément qui avait la sélection n'est pas dans un menubar, ferme son menu et tous les conteneurs de menu parents ouverts.
- <kbd>Maj + Tab</kbd>
  - : Déplace la sélection vers l'élément précédent dans la séquence de tabulation, et si l'élément qui avait la sélection n'est pas dans un menubar, ferme son menu et tous les conteneurs de menu parents ouverts.

Si un menu est ouvert ou qu'une barre de menu reçoit la sélection à la suite d'une action contextuelle, <kbd>Échap</kbd> ou <kbd>Entrée</kbd> peut renvoyer la sélection au contexte d'invocation.
