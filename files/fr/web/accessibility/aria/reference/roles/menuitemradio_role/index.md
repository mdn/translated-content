...
L'élément `menuitemradio` peut contenir du contenu phrastique, mais ne peut pas avoir de contenu interactif comme descendants ni de descendants avec un attribut `tabindex` défini.
...
- Rôle [`menu`](/fr/docs/Web/Accessibility/ARIA/Reference/Roles/menu_role)
  - : Composant qui propose une liste d'actions ou de fonctions courantes que l'utilisateur·ice peut invoquer.
...
Lorsqu'un `menu` s'ouvre ou qu'une `menubar` reçoit la sélection, la sélection clavier est placé sur le premier élément. Tous les éléments dans les deux sont sélectionnables, y compris tous les éléments `menuitemradio`.
...
- <kbd>Flèche vers la droite</kbd>
  - : Ferme le sous-menu. Dans une menubar, déplace la sélection vers l'élément suivant de la menubar, en ouvrant le sous-menu s'il y en a un.
- <kbd>Flèche vers la gauche</kbd>
  - : Ferme le menu. Dans une menubar, déplace la sélection vers l'élément précédent de la menubar, en ouvrant le sous-menu s'il y en a un.
- <kbd>Flèche vers le bas</kbd>
  - : Déplace la sélection vers l'élément suivant du menu. Si la sélection est sur le dernier élément, la sélection passe au premier élément.
- <kbd>Flèche vers le haut</kbd>
  - : Déplace la sélection vers l'élément précédent du menu. Si la sélection est sur le premier élément, la sélection passe au dernier élément.
- <kbd>Début</kbd>
  - : Déplace la sélection vers le premier élément du menu.
- <kbd>Fin</kbd>
  - : Déplace la sélection vers le dernier élément du menu.
- <kbd>Caractère</kbd>
  - : Déplace la sélection vers le prochain élément dont le nom commence par le caractère saisi. Si aucun élément ne commence par ce caractère, la sélection ne bouge pas.
...
N'utilisez pas la propriété CSS raccourcie {{CSSxRef("background")}}, car cela écrase la propriété CSS {{CSSxRef("background-clip")}} utilisée pour créer l'effet bouton radio.
