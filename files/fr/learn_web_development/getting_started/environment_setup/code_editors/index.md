...
Précédemment, nous vous avons demandé d'installer un éditeur de code, car vous en avez besoin pour suivre ce parcours. Dans cet article, nous examinons les éditeurs de code plus en détail, afin de vous donner une idée de ce qu'ils peuvent vous apporter.
...
Nous recommandons Visual Studio Code (VSCode), car c'est l'éditeur que nous utilisons principalement. Si vous ne l'avez pas encore (ou un autre éditeur de code), vous devez [l'installer avant de continuer <sup>(angl.)</sup>](https://code.visualstudio.com/).
...
> [!NOTE]
> Si vous n'utilisez que le clavier, sachez que VSCode propose de nombreux raccourcis. Voir la [référence des raccourcis clavier par défaut de VSCode <sup>(angl.)</sup>](https://code.visualstudio.com/docs/reference/default-keybindings).

### Ouvrir et éditer des fichiers

Cela peut sembler évident, mais installer un éditeur de code est utile, car il vous donne une application unique pour ouvrir tous les fichiers de code dont vous avez besoin. Rien de plus agaçant que de double-cliquer sur un fichier et de le voir s'ouvrir dans une application inadaptée, ou que le système d'exploitation ne le reconnaisse pas.

Cela doit se configurer automatiquement lors de l'installation de VSCode, mais si vous avez encore des soucis avec certains types de fichiers, vous pouvez les associer manuellement à l'application. La méthode varie selon le système d'exploitation, donc cherchez «&nbsp;choisir l'application qui ouvre un type de fichier &lt;nom-et-version-OS&gt;&nbsp;» — par exemple, «&nbsp;choisir l'application qui ouvre un type de fichier windows 11&nbsp;» si vous êtes sous Windows 11.

Vous pouvez trouver plus d'informations sur l'ouverture et l'édition de fichiers et dossiers dans l'article suivant.

### Coloration syntaxique

Les éditeurs de code comme VSCode proposent la coloration syntaxique — c'est-à-dire que les éléments du code sont affichés en différentes couleurs selon leur rôle. Cela rend le code bien plus lisible que s'il était tout monochrome. Prenons cet exemple JavaScript&nbsp;:
...
Essayez cet exercice dans VSCode&nbsp;:

1. Copiez l'exemple ci-dessus (les blocs de code MDN ont une icône de copie en haut à droite).
2. Ouvrez VSCode et créez un nouveau fichier avec _Fichier_ > _Nouveau fichier..._
...
4. Collez le code pour voir la coloration syntaxique JavaScript de VSCode.

VSCode propose d'autres fonctionnalités syntaxiques&nbsp;:

- Vous voyez une fine ligne verticale reliant le mot-clé `function` à l'accolade fermante (`}`). Ces lignes marquent les niveaux d'indentation, ce qui aide à repérer où commencent et finissent les blocs.
- Essayez aussi de placer le curseur sur une accolade ouvrante ou fermante (`{` ou `}`)&nbsp;: les deux sont surlignées. Cela aide à repérer les blocs, surtout dans des structures imbriquées. Ce surlignage fonctionne aussi avec les parenthèses (`(` et `)`) et crochets (`[` et `]`).
...
3. Commencez à taper `function` — une liste d'options doit apparaître.
4. Sélectionnez l'option _function_ avec _Function Statement_ à droite. Cela insère&nbsp;:
...
2. La petite croix à droite du bloc indique un mauvais exemple (et c'est justifié)&nbsp;: il y a trois erreurs dans ce code&nbsp;! Regardez comment VSCode les met en évidence, puis corrigez-les avec nous.
3. Première erreur&nbsp;: on utilise `nom` à la première ligne, mais `Nom` à la seconde pour la même variable. JavaScript est sensible à la casse, donc il les considère comme différentes. VSCode colore `nom` en gris foncé (valeur déclarée mais jamais utilisée, souvent signe d'une faute de frappe) et met trois points sous `Nom` (suggestion de correction, ici «&nbsp;vouliez-vous écrire `nom`&nbsp;?&nbsp;»). Corrigez en remplaçant `Nom` par `nom`.
   > [!NOTE]
   > Survolez les surlignages pour plus d'infos.
4. Deuxième erreur, ligne 6&nbsp;: on écrit `"Chris`. En JavaScript, une chaîne de caractères (**string**) doit être entourée de deux guillemets, or il en manque un. VSCode souligne l'erreur d'une vague rouge, comme pour les fautes d'orthographe dans Word. Corrigez en mettant `"Chris"` à la place de `"Chris`.
...
1. Revenez à votre fichier JavaScript et ouvrez le panneau de recherche avec _Édition_ > _Rechercher_.
...
3. Passez en mode remplacement avec _Édition_ > _Remplacer_ ou en cliquant sur la flèche à gauche de la zone _Rechercher_.
...
VSCode propose de nombreuses options avancées — voir [Rechercher et remplacer <sup>(angl.)</sup>](https://code.visualstudio.com/docs/editing/codebasics#_find-and-replace).
...
- Activer la complétion, le <i lang="en">linting</i> ou le débogage pour des langages non pris en charge, ou ajouter des fonctions supplémentaires.
...
3. Installons plutôt une extension simple et utile pour tout fichier de code. Tapez «&nbsp;Prettier&nbsp;» dans la zone de recherche et cliquez sur _Prettier - code formatter_. Une fois l'extension [Prettier <sup>(angl.)</sup>](https://prettier.io/) installée, elle peut formater votre code à chaque enregistrement, le rendant plus lisible.
4. Cliquez sur _Installer_ dans l'onglet Extension. Fermez l'onglet une fois l'installation terminée.
5. Pour activer Prettier, modifiez quelques réglages. Ouvrez les paramètres de VSCode (_Code_ > _Paramètres..._ > _Paramètres_ sur macOS, _Fichier_ > _Préférences_ > _Paramètres_ sur Windows).
...
3. Enregistrez à nouveau&nbsp;: Prettier doit reformater le code ainsi&nbsp;:
