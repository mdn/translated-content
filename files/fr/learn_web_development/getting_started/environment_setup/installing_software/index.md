...
Disposer de navigateurs web modernes est essentiel pour le développement web afin de pouvoir tester vos sites ou applications sur les navigateurs que vos visiteur·euse·s utilisent. Il est aussi important de garder vos navigateurs à jour pour qu'ils prennent en charge les dernières technologies web et bénéficient des correctifs de sécurité les plus récents.
...
Pour l'instant, vous devez installer quelques navigateurs de bureau et mobiles/alternatifs pour tester votre code. Installez des navigateurs basés sur au moins deux moteurs de rendu différents (par exemple Chromium et Gecko), afin de ne pas tester uniquement sur des navigateurs utilisant le même moteur de rendu. C'est important, car votre code peut contenir des bugs qui n'affectent qu'un seul moteur de rendu.

Les navigateurs basés sur WebKit ne sont pas disponibles pour les systèmes d'exploitation Windows, Linux et Android. Si vous souhaitez tester votre code sur les trois principaux moteurs de rendu et que votre ordinateur est sous Windows, vous devez accéder à un appareil de test macOS ou iOS ou utiliser une solution logicielle telle qu'une machine virtuelle ou une plateforme de test. Vous n'avez pas besoin de vous inquiéter de tests complets à ce stade — reconnaître que vous devez tester votre code sur différents moteurs de rendu et obtenir un peu de pratique est suffisant pour l'instant.

Vous examinez les stratégies de test plus en détail dans notre module [des tests](/fr/docs/Learn_web_development/Extensions/Testing).
...
1. Ouvrez le panneau _Extensions_ avec le menu _Affichage_ > _Extensions_.
2. Dans la zone «&nbsp;Rechercher...&nbsp;» en haut du panneau, tapez «&nbsp;live preview&nbsp;». Le premier résultat doit être l'extension [_Live Preview_](https://marketplace.visualstudio.com/items?itemName=ms-vscode.live-server) créée par Microsoft.
...
Nous vous recommandons de n'installer un éditeur graphique que lorsque vous en avez besoin plus tard dans votre apprentissage. N'investissez pas dans un produit commercial coûteux à moins d'en avoir vraiment l'utilité.

Il existe de nombreux outils gratuits et services en ligne qui suffisent largement pour l'instant, par exemple&nbsp;:
