...
- [TC39 <sup>(angl.)</sup>](https://tc39.es/) et [ECMA <sup>(angl.)</sup>](https://ecma-international.org/) définissent et publient la norme ECMAScript, sur laquelle repose le JavaScript moderne.
...
1. Quelqu'un remarque le besoin d'une nouvelle fonctionnalité standard du Web qui facilite la vie des développeur·euse·s. Par exemple, il existe peut-être un motif courant utilisé dans les interfaces utilisateur web, mais difficile à implémenter. Une fonctionnalité CSS dédiée rend cela beaucoup plus simple. Ce «&nbsp;quelqu'un&nbsp;» peut être n'importe qui — un·e développeur·euse individuel·le ou un·e ingénieur·e travaillant pour une grande entreprise technologique.
2. La personne discute de cette fonctionnalité avec d'autres développeur·euse·s, des ingénieur·e·s navigateur, etc., et commence à susciter de l'intérêt pour l'implémentation de la fonctionnalité. Généralement, elle rédige un document explicatif qui présente le besoin et le fonctionnement de la fonctionnalité, ainsi qu'une démonstration de code montrant à quoi cela ressemble en pratique.
...
9. Finalement, si tout se passe bien, la fonctionnalité est implémentée dans tous les navigateurs et peut commencer à être utilisée lors de la création de sites web.
...
Nous avons brièvement parlé des technologies que vous allez utiliser pour créer des sites web. Parlons maintenant des bonnes pratiques que les développeur·euse·s web emploient généralement pour s'assurer que leurs sites soient utilisables par le plus grand nombre de personnes possible.

Lorsque vous développez pour le web, la principale source d'incertitude vient du fait que vous ne savez pas quelle combinaison de technologies chaque utilisateur·ice utilise pour consulter votre site&nbsp;:
