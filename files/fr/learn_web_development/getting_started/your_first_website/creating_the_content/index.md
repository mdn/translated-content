...
Certains attributs n'ont pas de valeur, comme [`required`](/fr/docs/Web/HTML/Reference/Attributes/required).
...
On peut également placer des éléments à l'intérieur d'autres éléments&nbsp;: c'est ce qu'on appelle **l'imbrication**. Si on souhaitait indiquer que le chat était **très** grincheux, on pourrait placer le mot «&nbsp;très&nbsp;» dans un élément [`<strong>`](/fr/docs/Web/HTML/Reference/Elements/strong), indiquant l'accentuation sur ce mot&nbsp;:

```html
<p>Mon chat est <strong>très</strong> grincheux.</p>
```

Il faut en revanche s'assurer que les éléments sont correctement imbriqués. Dans l'exemple précédent, on commence par ouvrir l'élément [`<p>`](/fr/docs/Web/HTML/Reference/Elements/p), puis l'élément [`<strong>`](/fr/docs/Web/HTML/Reference/Elements/strong). Il faut donc commencer par fermer l'élément `<strong>`, puis l'élément `<p>`. Ce qui suit est incorrect&nbsp;:
...
Certains éléments n'ont pas de contenu, on les qualifie d'[éléments vides](/fr/docs/Glossary/Void_element). Prenons l'exemple de l'élément [`<img>`](/fr/docs/Web/HTML/Reference/Elements/img) que nous avons sur notre page HTML&nbsp;:
...
- `<html></html>`
  - : L'élément [`<html>`](/fr/docs/Web/HTML/Reference/Elements/html) est celui qui contient tout le reste de la page. On l'appelle aussi l'élément racine. Il contient ici l'attribut `lang` qui indique la langue principale du contenu du document.
- `<head></head>`
  - : L'élément [`<head>`](/fr/docs/Web/HTML/Reference/Elements/head) sert de conteneur pour tout ce qu'on veut inclure dans une page HTML qui n'est _pas_ du contenu à afficher à l'écran. Cela inclut [les mots-clés](/fr/docs/Glossary/Keyword) et une description de la page destinée aux moteurs de recherches, les liens vers le CSS qui mettra en forme notre contenu, les déclarations pour les jeux de caractères utilisés, etc.
- `<meta charset="utf-8">`
...
- `<meta name="viewport" content="width=device-width">`
  - : Cet [élément `viewport`](/fr/docs/Web/CSS/CSSOM_view/Viewport_concepts#zones_daffichage_sur_mobiles) permet que la page soit affichée avec la bonne largeur par rapport à la zone d'affichage, empêchant les navigateurs sur mobile d'afficher la page plus largement que la zone d'affichage avant de la réduire.
- `<title></title>`
  - : L'élément [`<title>`](/fr/docs/Web/HTML/Reference/Elements/title) définit le titre de la page, qui apparaîtra dans l'onglet sur lequel la page est chargée. Il sert aussi à décrire la page lorsqu'on l'ajoute aux favoris ou qu'elle est listée dans les résultats d'un moteur de recherche.
- `<body></body>`
  - : L'élément [`<body>`](/fr/docs/Web/HTML/Reference/Elements/body) contient _tout_ le contenu qu'on veut afficher aux utilisatrices et utilisateurs web lorsqu'ils visitent la page, que ce soit du texte, des vidéos, des jeux, des pistes audio, etc.

## Images

Revenons sur l'élément [`<img>`](/fr/docs/Web/HTML/Reference/Elements/img)&nbsp;:
...
Nous avons également inclus un attribut `alt` (pour texte alternatif). [L'attribut `alt`](/fr/docs/Web/HTML/Reference/Elements/img#écrire_des_descriptions_alternatives_significatives), permet d'indiquer un texte descriptif pour les personnes qui ne peuvent pas voir l'image&nbsp;:
...
> [!NOTE]
> Pour en savoir plus sur l'accessibilité, n'hésitez pas à consulter [notre module d'apprentissage sur l'accessibilité](/fr/docs/Learn_web_development/Core/Accessibility).
...
Essayez maintenant d'ajouter un titre pertinent à votre page HTML avant l'élément [`<img>`](/fr/docs/Web/HTML/Reference/Elements/img).

> [!NOTE]
> Vous pourrez voir que le titre de niveau 1 possède une mise en forme implicite, mais il ne faut pas utiliser ces éléments pour uniquement afficher du texte en gras ou plus grand. En effet, les éléments ont un sens, utilisé pour [l'accessibilité](/fr/docs/Learn_web_development/Core/Accessibility/HTML#contenus_textuels) et [le référencement](/fr/docs/Learn_web_development/Core/Structuring_content/Headings_and_paragraphs#pourquoi_faut-il_structurer_un_document). Essayez de créer une structure de titres pertinente pour vos pages, sans sauter de niveaux intermédiaires.

### Paragraphes

Comme expliqué plus tôt, les éléments [`<p>`](/fr/docs/Web/HTML/Reference/Elements/p) forment des paragraphes de texte. Cet élément est souvent utilisé pour baliser du texte normal du contenu d'une page&nbsp;:

```html
<p>Voici un paragraphe simple</p>
```

Ajoutez votre texte d'échantillon (celui créé à la lecture de [_À quoi ressemblera votre site web&nbsp;?_](/fr/docs/Learn_web_development/Getting_started/Your_first_website/What_will_your_website_look_like)) dans un ou plusieurs paragraphes, juste après l'élément [`<img>`](/fr/docs/Web/HTML/Reference/Elements/img).
...
- Les listes non-ordonnées
  - : Utilisées pour les listes où l'ordre des éléments n'a pas d'importance, comme une liste de courses. Pour celles-ci, on utilise un élément [`<ul>`](/fr/docs/Web/HTML/Reference/Elements/ul).
- Les listes ordonnées
  - : Utilisées pour les listes où l'ordre des éléments est important, comme une recette de cuisine. Pour celles-ci, on utilise un élément [`<ol>`](/fr/docs/Web/HTML/Reference/Elements/ol).

Chaque élément d'une liste est placé dans un élément [`<li>`](/fr/docs/Web/HTML/Reference/Elements/li).
...
Les liens sont importants, ce sont eux qui forment la toile qu'est le Web&nbsp;! Pour ajouter un lien, on utilisera un élément [`<a>`](/fr/docs/Web/HTML/Reference/Elements/a) («&nbsp;a&nbsp;» correspondant à «&nbsp;ancre&nbsp;»). Pour placer un lien dans votre paragraphe, suivez les étapes suivantes&nbsp;:

1. Identifiez le texte voulu (ici nous prendrons le texte «&nbsp;Manifeste Mozilla&nbsp;»).
2. Entourez-le avec les balises ouvrantes et fermantes d'un élément [`<a>`](/fr/docs/Web/HTML/Reference/Elements/a) comme suit&nbsp;:

   ```html
   <a>Manifeste Mozilla</a>
   ```

3. Ajoutez un attribut `href` à l'élément [`<a>`](/fr/docs/Web/HTML/Reference/Elements/a)&nbsp;:
...
Nous n'avons fait qu'effleurer la surface de HTML. Pour approfondir, n'hésitez pas à consulter la thématique [Apprendre HTML](/fr/docs/Learn_web_development/Core/Structuring_content).
