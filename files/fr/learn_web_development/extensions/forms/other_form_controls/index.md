...
Un champ texte sur plusieurs lignes se crée à l'aide d'un élément [`<textarea>`](/fr/docs/Web/HTML/Reference/Elements/textarea) plutôt qu'avec un élément [`<input>`](/fr/docs/Web/HTML/Reference/Elements/input).
...
`<textarea>` utilise également une balise fermante et le texte contenu entre ces deux balises sera utilisé comme contenu par défaut. [`<input>`](/fr/docs/Web/HTML/Reference/Elements/input) est quant à lui un élément vide sans balise fermante et la valeur par défaut est portée par l'attribut [`value`](/fr/docs/Web/HTML/Reference/Elements/input#value).

Bien qu'on puisse mettre n'importe quoi à l'intérieur d'un élément `<textarea>` (y compris d'autres éléments HTML, CSS, et JavaScript), étant donné sa nature, tout le contenu sera affiché à l'écran comme du texte simple (on utilisera [`contenteditable`](/fr/docs/Web/HTML/Reference/Global_attributes/contenteditable) sur des éléments qui ne sont pas des contrôles de formulaire pour fournir une API permettant de capturer du contenu riche/complexe plutôt que du texte simple).
...
On crée une boîte de sélection à l'aide d'un élément [`<select>`](/fr/docs/Web/HTML/Reference/Elements/select) qui contient un ou plusieurs éléments [`<option>`](/fr/docs/Web/HTML/Reference/Elements/option) comme enfants, chacun de ces derniers fournissant une valeur possible.
...
Les éléments [`<option>`](/fr/docs/Web/HTML/Reference/Elements/option) peuvent être imbriqués dans des éléments [`<optgroup>`](/fr/docs/Web/HTML/Reference/Elements/optgroup) pour créer des groupes de valeurs distincts&nbsp;:
...
Si un élément [`<option>`](/fr/docs/Web/HTML/Reference/Elements/option) possède explicitement un attribut `value`, c'est cette valeur qui sera envoyée avec le formulaire lors de l'envoi si cette option est sélectionnée. Si l'attribut `value` est absent (à l'instar des exemples précédents), c'est le contenu de l'élément [`<option>`](/fr/docs/Web/HTML/Reference/Elements/option) qui est utilisé comme valeur. Aussi, les attributs `value` ne sont pas strictement nécessaires mais peuvent s'avérer utiles lorsqu'on souhaite fournir une valeur raccourcie ou normalisée au serveur et qui est différente de la valeur affichée dans la boîte.
...
Par défaut la taille de la boîte de sélection permet d'afficher une seule valeur. L'attribut optionnel [`size`](/fr/docs/Web/HTML/Reference/Attributes/size) permet de contrôler le nombre d'options visibles lorsque la boîte de sélection n'a pas le focus.
...
Les navigateurs qui prennent en charge l'élément [`<datalist>`](/fr/docs/Web/HTML/Reference/Elements/datalist) ignoreront tous les éléments enfants qui ne sont pas des éléments [`<option>`](/fr/docs/Web/HTML/Reference/Elements/option), la liste fonctionnant alors comme on le souhaite. Les navigateurs qui ne prennent pas en charge [`<datalist>`](/fr/docs/Web/HTML/Reference/Elements/datalist) afficheront le libellé et la boîte de sélection.
...
Pour les navigateurs qui prennent en charge [`<datalist>`](/fr/docs/Web/HTML/Reference/Elements/datalist) et [`<input type="color">`](/fr/docs/Web/HTML/Reference/Elements/input/color), ils devraient afficher une palette de couleurs personnalisée par défaut, tout en gardant la palette complète disponible.
...
Le contenu à l'intérieur d'un élément [`<progress>`](/fr/docs/Web/HTML/Reference/Elements/progress) agit comme contenu alternatif pour les navigateurs qui ne prennent pas en charge cet élément et pour les outils de lecture d'écran qui doivent vocaliser l'information.
...
Pour créer une telle jauge, on utilise l'élément [`<meter>`](/fr/docs/Web/HTML/Reference/Elements/meter). Ce dernier permet d'implémenter n'importe quel type de jauge, par exemple une barre représentant l'espace total utilisé sur un disque, qui devient rouge lorsque l'espace vient à manquer.
...
Le contenu situé à l'intérieur d'un élément [`<meter>`](/fr/docs/Web/HTML/Reference/Elements/meter) est utilisé comme contenu alternatif par les navigateurs qui ne prennent pas en charge cet élément et par les technologies d'assistance qui doivent vocaliser cette information.

La prise en charge de [`<progress>`](/fr/docs/Web/HTML/Reference/Elements/progress) et [`<meter>`](/fr/docs/Web/HTML/Reference/Elements/meter) est plutôt correcte, seul Internet Explorer ne prend pas en charge ces fonctionnalités.
