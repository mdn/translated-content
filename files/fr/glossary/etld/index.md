...
Un domaine de plus haut niveau (<i lang="en">top level domain</i>, ou TLD) correspond au domaine qui suit le point final. Ainsi, le domainde de plus haut niveau pour `crookedtimber.org` est `org`.
...
Toutfois, cela ne fonctionne pas toujours ainsi et certains bureaux d'enregistrement permettent aux organisations d'enregistrer des domaines en dessous du niveau le plus haut. Par exepmle, `sussex.ac.uk` et `aber.ac.uk` sont enregistrés par des organisations différentes.
