...
Dans notre application de liste de courses, le modèle définit quelles données doivent être présentes dans chaque élément de la liste — article, prix, etc. — et quels éléments sont déjà présents.

### La Vue

La vue définit comment les données de l'application doivent être affichées.

Dans notre application de liste de courses, la vue définit comment la liste est présentée à l'utilisateur·ice et reçoit les données à afficher depuis le modèle.
...
Par exemple, notre liste de courses peut comporter des formulaires et des boutons permettant d'ajouter ou de supprimer des articles. Ces actions nécessitent la mise à jour du modèle, donc la saisie est envoyée au contrôleur, qui manipule alors le modèle comme il convient, puis envoie les données mises à jour à la vue.

Vous pouvez aussi vouloir simplement mettre à jour la vue pour afficher les données dans un format différent, par exemple trier les articles par ordre alphabétique ou du prix le plus bas au plus élevé. Dans ce cas, le contrôleur peut gérer cela directement sans avoir besoin de mettre à jour le modèle.

## MVC sur le Web

En tant que développeur·euse web, ce modèle vous est probablement familier même si vous ne l'avez jamais utilisé consciemment. Votre modèle de données est probablement contenu dans une base de données (qu'il s'agisse d'une base traditionnelle côté serveur comme MySQL, ou d'une solution côté client telle que [IndexedDB](/fr/docs/Web/API/IndexedDB_API)). Le code de contrôle de votre application est probablement écrit en HTML/JavaScript, et votre interface utilisateur en HTML/CSS ou tout autre langage. Cela ressemble beaucoup à MVC, mais MVC impose à ces composants de suivre un schéma plus rigide.

Aux débuts du Web, l'architecture MVC était principalement implémentée côté serveur, le client demandant des mises à jour avec des formulaires ou des liens, et recevant des vues mises à jour à afficher dans le navigateur. Aujourd'hui, la logique est de plus en plus poussée côté client avec l'apparition des bases de données locales et de [l'API Fetch](/fr/docs/Web/API/Fetch_API) permettant des mises à jour partielles de page selon les besoins.

Des frameworks web comme [AngularJS <sup>(angl.)</sup>](https://fr.wikipedia.org/wiki/AngularJS) et [Ember.js <sup>(angl.)</sup>](https://fr.wikipedia.org/wiki/Ember.js) implémentent tous une architecture MVC, bien que de façon légèrement différente.
