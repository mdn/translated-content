...
![Diagramme circulaire des vulnérabilités les plus courantes : l'injection SQL est responsable de 50 % des vulnérabilités, le script inter-site de 42% et la divulgation de code source de 7%.](sql_inj_xss.gif)
...
Supposons maintenant que l'utilisateur·ice entre le nom d'utilisateur·ice&nbsp;: admin et le mot de passe&nbsp;: mdp123, après avoir cliqué sur le bouton Connexion, la requête SQL s'exécute comme suit&nbsp;:
...
Ainsi, en raison de (1) chaque guillemet simple (`'`) dans la chaîne de caractères d'entrée est remplacé par des guillemets (`"`), et en raison de (2) avant chaque (`'`) est ajouté un (`/`). La chaîne de caractères magique contrôlée échoue à contourner l'authentification et votre base de données reste sécurisée.
