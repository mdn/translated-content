...
_Three_ est l'une des bibliothèques [WebGL](/fr/docs/Web/API/WebGL_API) les plus populaires, bien que nous ne disions pas qu'elle soit meilleure qu'une autre bibliothèque WebGL, et vous devez vous sentir libre d'essayer d'autres bibliothèques.

> [!NOTE]
> Ce guide a été mis à jour pour la dernière fois en Août 2026, et est compatible avec la version `r185` de Three.js.
...
Sinon, vous pouvez télécharger la [dernière version de la bibliothèque Three.js <sup>(angl.)</sup>](https://github.com/mrdoob/three.js/archive/master.zip) et copier la version minifiée de Three.js depuis l'archive décompressée à `build/three.webgpu.js` dans votre projet.
Gardez à l'esprit que les archives incluent les fichiers source, ce qui rend la taille du téléchargement d'environ 350 Mo.

Quelle que soit la méthode que vous choisissez pour commencer, assurez-vous d'avoir la [documentation de Three.js <sup>(angl.)</sup>](https://threejs.org/docs/) ouverte quelque part pendant pour référence quand vous travaillez.
...
<html lang="fr">
  ...
  <body>
    ...
    <script type="module">
      ...
      /* tout notre code JavaScript va ici */
    </script>
  </body>
</html>
```

Cette structure contient des informations de base comme le {{HTMLElement("title")}} du document, ainsi que du CSS pour définir la `width` et la `height` de l'élément HTML {{HTMLElement("canvas")}} que Three.js ajoute à la page en 100% pour remplir tout l'espace disponible de la zone d'affichage. Le premier élément HTML {{HTMLElement("script")}} inclut la bibliothèque Three.js dans la page, et nous écrivons notre code d'exemple dans le second. Deux variables utilitaires sont déjà incluses, elles stockent la `width` et la `height` de la fenêtre.
...
Un moteur de rendu est un outil qui affiche des scènes directement dans votre navigateur. Il existe plusieurs moteurs de rendu&nbsp;: WebGL est celui par défaut, mais on en trouve d'autres, tels que WebGPU, Canvas, SVG, CSS et DOM. Ils diffèrent par leur manière de rendre les éléments, de sorte que l'implémentation WebGL produit un rendu différent de celle en CSS. Malgré la diversité des méthodes utilisées pour atteindre cet objectif, l'expérience est identique pour l'utilisateur·ice. Grâce à cette approche, une solution de repli peut être utilisée si la technologie souhaitée n'est pas prise en charge par le navigateur. Par exemple, le moteur de rendu WebGPU utilise par défaut un arrière-plan WebGPU et un arrière-plan WebGL 2 comme solution de repli.

Le code ci-dessous crée un nouveau moteur de rendu WebGPU, définit sa taille pour occuper tout l'espace disponible à l'écran, et ajoute la structure DOM à la page.
...
Plus tard, nous utilisons la méthode `add()` pour ajouter des objets à cette scène.
...
- La valeur que nous fixons pour le champ de vision, `70`, est quelque chose que nous pouvons ajuster&nbsp;: plus la valeur est élevée, plus la quantité de scène que la caméra affiche est grande. Imaginez une vue de caméra normale par opposition à un effet «&nbsp;œil de poisson&nbsp;» (<i lang="en">fisheye</i> en anglais), qui permet de voir beaucoup plus. La valeur par défaut est 50.
...
- La position `z`, avec une valeur de 50 unités, est la distance entre la caméra et le centre de la scène sur l'axe `z`. Ici, nous reculons la caméra, afin que les objets de la scène puissent être visualisés. 50 est une valeur correcte. Ce n'est ni trop près, ni trop loin, et la taille des objets leur permet de rester sur la scène, dans le champ de vision donné. Les valeurs `x` et `y`, si elles ne sont pas spécifiées, sont définies par défaut à 0.

N'hésitez pas à modifier ces valeurs et observer les effets de ces changements sur la scène.
Les valeurs de distance (par exemple pour la position z de la caméra) sont sans unité et peuvent correspondre à toute unité que vous utilisez pour l'ensemble des objets de la scène (millimètres, mètres, etc.).

## Rendu de la scène

Tout est prêt, mais on ne voit toujours rien. Bien que nous ayons configuré le moteur de rendu, nous devons toujours effectuer le rendu. Notre fonction `render()` fait ce travail, avec un peu d'aide de {{DOMxRef("Window/requestAnimationFrame", "requestAnimationFrame()")}}, ce qui signifie que le rendu de scène est calculé à chaque image&nbsp;:
...
À chaque nouvelle image, la fonction `render` est appelée, et le `renderer` effectue le rendu de la `scene` et de la `camera`. Juste après la déclaration de la fonction, nous l'appelons pour lancer la boucle, après quoi elle s'exécute en continu.

Là encore, ajoutez ce nouveau code sous vos précédents ajouts. Enregistrez votre fichier et ouvrez-le dans votre navigateur. Vous devez voir une fenêtre grise. Félicitations&nbsp;!

## Géométrie

Maintenant que notre scène s'affiche correctement, nous pouvons commencer à ajouter des formes en 3D. Pour accélérer le développement, Three.js fournit toute une série de primitives prédéfinies, que vous pouvez utiliser pour créer instantanément des formes en une seule ligne de code. Des cubes, des sphères, des cylindres et des formes plus complexes sont disponibles. Les détails tels que le tracé des sommets et des faces nécessaires à une forme donnée sont gérés par Three.js, ce qui nous permet de nous concentrer sur le codage de plus haut niveau. Commençons par définir la géométrie d'un cube en ajoutant le code suivant juste au-dessus de la fonction `render()`&nbsp;:

```js
const boxGeometry = new THREE.BoxGeometry(10, 10, 10);
```

Dans ce cas, nous définissons un cube simple de 10 x 10 x 10 unités. Cette géométrie n'est pourtant pas suffisante, car nous avons besoin d'un matériau qui est utilisé pour le rendu de notre forme.

## Matériau

Un matériau est ce qui recouvre un objet, les couleurs, ou les textures sur sa surface. Dans notre cas, nous allons choisir une simple couleur bleue pour peindre notre boîte. Différents matériaux prédéfinis peuvent être utilisés&nbsp;: `Basic`, `Phong`, et `Lambert`. Nous voyons les deux derniers plus tard, pour l'instant, commençons par utiliser le matériau `Basic`&nbsp;:
...
Pour appliquer le matériau à une géométrie, on utilise un maillage. Ce maillage épouse la forme, et ajoute le matériau défini sur chaque face&nbsp;:
...
Félicitations.
Votre code doit ressembler à l'exemple interactif suivant.
...
Ajoutons maintenant plus de formes et de matériaux. Que doit-il arriver si vous ajoutez un tore, enveloppé dans un matériau `Phong`&nbsp;? Essayez d'ajouter les lignes suivantes, juste sous les lignes qui définissent le cube.
...
Ces lignes vont ajouter une géométrie de tore&nbsp;; les paramètres de la méthode `TorusGeometry()` définissent les paramètres&nbsp;: `radius`, `tube diameter`, `radial segment count`, et `tubular segment count`. Le matériau Phong doit paraître plus brillant que le matériau Basic simple de la boîte, même si pour l'instant notre tore apparaît noir.
...
Il est aussi possible d'agrandir ou de faire rétrécir un objet. En appliquant une valeur constante, on change sa taille une seule fois. Pour que l'effet soit plus intéressant, on déclare une variable utilitaire `t` qui sert à mesurer le temps écoulé. Ajoutons-la avant la fonction `render()`&nbsp;:
...
Nous utilisons `Math.sin`, ce qui donne un résultat assez intéressant. Cela permet de mettre le tore à l'échelle, en répétant le processus, puisque `sin` est une fonction périodique. Nous encapsulons la valeur d'échelle dans `Math.abs`, afin de ne conserver que les valeurs absolues supérieures ou égales à 0. Comme la fonction sin prend des valeurs comprises entre -1 et 1, les valeurs négatives peuvent donner un rendu étrange à notre tore. Dans ce cas, il apparaît noir la moitié du temps.
...
Cela permet de faire monter et descendre le dodécaèdre en appliquant la valeur de la fonction `sin()` à l'axe y à chaque image, avec un léger ajustement pour rendre l'effet plus esthétique. Essayez de modifier ces valeurs pour voir comment cela influence les animations.
...
Vous pouvez également essayer d'apprendre WebGL et WebGPU, pour mieux comprendre ce qui se passe en dessous.
Consultez notre [documentation WebGL](/fr/docs/Web/API/WebGL_API) et [documentation WebGPU](/fr/docs/Web/API/WebGPU_API) pour plus d'informations.
