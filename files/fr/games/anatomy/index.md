...
Cet article se concentre sur l'anatomie et le flux de la plupart des jeux video à partir d'un point de vue technique, concernant la manière dont la boucle principale doit tourner. Cela aide les débutants dans l'arène du développement à comprendre ce qui est requis quand il est question de bâtir un jeu et comment les standards du web comme JavaScript leur est offert comme outil. Les programmeurs de jeux expérimentés et nouveaux dans le développement web peuvent aussi en tirer bénéfice.

## Présenter, accepter, interpréter, calculer, répéter

Le but de chaque jeu vidéo est de **présenter** à (aux) utilisateur·ice(s) une situation, **accepter** leur entrée, **interpréter** ces signaux en actions, et **calculer** une nouvelle situation résultant de ces actes. Les jeux bouclent continuellement à travers ces niveaux, jusqu'à ce qu'une condition finale arrive (comme gagner, perdre, ou quitter le jeu). Sans surprise, ce modèle correspond à la manière dont un moteur de jeu est programmé.
...
Le JavaScript moderne — comme décrit dans les prochaines sections — heureusement, facilite le développement d'une boucle efficace exécutée une fois par seconde. Bien sûr, votre jeu est optimisé au long de sa conception. Si quelque chose doit s'apparenter à un évènement peu fréquent alors il est préférable de briser la boucle principale (mais pas tout le temps).
...
Temporiser la boucle principale avec le rafraîchissement du navigateur permet à votre boucle d'être exécutée aussi fréquemment que le navigateur à besoin de rafraîchir l'écran. Cela vous permet de contrôler chaque image de votre animation. C'est aussi beaucoup plus simple, car `main()` est la seule fonction qui est bouclée. Dans un jeu de tir à la première personne (ou un jeu équivalent) on présente une nouvelle scène à chaque image. Vous ne pouvez donc pas obtenir quelque chose de plus fluide que cela.

Pourtant n'imaginez pas que les animations requièrent un contrôle image-par-image. De simples animations peuvent être exécutées, même avec une accélération matérielle, avec des animations CSS et d'autres outils inclus dans le navigateur. Bon nombre vont vous faciliter la vie.

## Construire une meilleure boucle principale en Javascript

Il y a deux problèmes évidents avec notre boucle principale précédente: `main()` pollue l'objet {{DOMxRef("window")}}` (où sont stockées toutes les variables globales) et l'exemple donné ne nous permet pas de _stopper_ la boucle à moins que l'onglet du navigateur ne soit fermé ou rafraîchit. Pour le premier problème, si vous désirez que la boucle principale tourne simplement sans y accéder directement, vous pouvez en créer une fonction à accès immédiat (FAI ou «&nbsp;Immediately-Invoked Function Expression - IIFE&nbsp;»).

```js
/*
 * Débuter avec le point virgule au cas où le code qui réside au-dessus de cet exemple
...
Quand le navigateur passe à travers la FAI, cela va définir votre boucle principale et immédiatement la mettre en file d'attente pour la prochaine image. Cela n'est attaché à aucun objet et `main` (ou `main()` pour les méthodes) est un nom valide inutilisé dans le reste de l'application, libre d'être défini comme quelque chose d'autre.

> [!NOTE]
> En pratique, il est plus courant de prévenir le prochain `requestAnimationFrame()` avec une déclaration `if`, plutôt que d'appeler `cancelAnimationFrame()`.

Pour le second problème, arrêter la boucle principale, vous avez besoin d'annuler l'appel à `main()` avec {{DOMxRef("window.cancelAnimationFrame()")}}. Vous avez besoin de passer la valeur donnée par `cancelAnimationFrame()` en référence à `requestAnimationFrame()` quand elle a été appelée en dernier. Assumons que vos fonctions de jeu et les variables sont bâties dans un espace de nom appelé `MonJeu`. Avec notre dernier exemple étendu, la boucle principale a maintenant l'air de ceci&nbsp;:
 ...
 * marque le début de notre nouvelle ligne si la précédente n'était pas vide ou complétée.
 *
 * Assumons aussi que MonJeu est défini précédemment.
 */

(() => {
  function main() {
    MonJeu.stopMain = window.requestAnimationFrame(main);
...
Nous avons maintenant une variable déclarée dans l'espace de nom `MonJeu`, que nous appelons `stopMain`, qui contient la valeur de l'appel de notre boucle principale `requestAnimationFrame()` la plus récente. À tout moment, nous pouvons stopper la boucle principale en disant au navigateur d'annuler la requête qui correspond à notre valeur.

```js
window.cancelAnimationFrame(MonJeu.stopMain);
```

La clé pour programmer une boucle principale, en JavaScript, est d'attacher n'importe quel évènement qui doit diriger votre action et porter attention aux systèmes interconnectés. Vous pouvez avoir différents composants dirigés par différents évènements. Cela paraît comme d'une complexité inutile mais cela peut être une bonne optimisation (pas nécessairement, bien sûr). Le problème c'est que vous ne programmez pas une boucle principale typique. En Javascript, vous utilisez la boucle principale du navigateur et vous essayez de le faire avec efficacité.

## Construire une boucle principale encore plus optimisée en JavaScript

En fin de compte, en JavaScript, le navigateur exécute sa propre boucle principale et votre code existe dans certaines de ses étapes. Les sections ci-dessus décrivent des boucles principales qui évitent de retirer le contrôle au navigateur. Ces méthodes principales s'attachent à `window.requestAnimationFrame()`, qui demande au navigateur le contrôle de la prochaine image. C'est au navigateur de décider comment relier ces requêtes à sa propre boucle principale. La [spécification HTML <sup>(angl.)</sup>](https://html.spec.whatwg.org/multipage/imagebitmap-and-animations.html#dom-animationframeprovider-requestanimationframe) ne définit pas vraiment à quel moment les navigateurs doivent exécuter les fonctions de rappel de `requestAnimationFrame`. Cela peut être un avantage, car cela laisse les éditeurs de navigateurs libres d'expérimenter les solutions qu'ils jugent les meilleures et de les ajuster au fil du temps.
...
3. Passer le ramasse-miettes et autres tâches par-images quand les retours ci-dessous cessent de contrôler le fil principal.
4. Se mettre en veille (à moins qu'un évènement interrompe la sieste du navigateur) jusqu'à ce que le moniteur ne soit prêt pour votre image ([VSync <sup>(angl.)</sup>](https://en.wikipedia.org/wiki/Screen_tearing#Vertical_synchronization)) et répète.

Vous pouvez considérer que développer une application en temps réel est comme avoir un temps donné pour faire le travail. Toutes les étapes ci-dessus doivent prendre effet toutes les 16.5 millisecondes pour fonctionner avec un affichage de 60 Hz. Les navigateurs invoquent leur code aussitôt que possible pour donner un maximum de temps aux calculs. Votre fil principal va souvent débuter par les tâches qui ne sont même pas dans le fil principal (tel que la rasterisation ou les ombrages en WebGL). Les longs calculs peuvent être fait par un Web Worker ou une accélération matérielle en même temps que le navigateur utilise son propre fil principal pour passer le ramasse-miette, ces autres tâches, ou gérer les évènements asynchrones.

Bien que nous ne discutons pas du gain de temps, plusieurs navigateur ont un outil appelé _Temps Haute Résolution_. L'objet {{JSxRef("Date")}} n'est plus la méthode reconnue pour temporiser les évènements, car elle est très imprécise et peut être modifiée par l'horloge système. Le Temps Haute Résolution, d'un autre côté, compte le nombre de millisecondes depuis `navigationStart` (quand le document précédent est déchargé). Cette valeur est retournée en un nombre décimal précis au millième de seconde. Il est connu comme étant {{DOMxRef("DOMHighResTimeStamp")}} mais, à toutes fins utiles, considérez le comme un nombre décimal à virgule flottante.

> [!NOTE]
> Les systèmes (matériels ou logiciels) qui ne sont pas capables d'avoir une précision à la microseconde sont autorisés à fournir une précision à la milliseconde au minimum. Sinon, ils doivent fournir une précision de 0.001ms s'ils en sont capables.
...
Retournons sur le sujet de la boucle principale. Il vous arrive souvent de vouloir savoir quand votre boucle principale a été invoquée. Parce que cela est commun, la fonction `window.requestAnimationFrame()` fournit toujours un `DOMHighResTimeStamp` avec un argument de retour quand elles sont exécutées. Cela mène à une amélioration de notre boucle précédente.
...
Plusieurs autres optimisations sont possibles et cela dépend vraiment de ce que votre jeu tente d'accomplir. Le genre de votre jeu va visiblement faire la différence mais il peut être aussi subtil que cela. Vous pouvez dessiner un pixel à la fois sur un canvas ou vous pouvez étager des éléments du DOM (incluant de multiples canvas de WebGL avec des arrières-plans transparents si vous le désirez) en une hiérarchie complexe. Chacune de ces solutions mène à des contraintes et opportunités différentes.

## Il est temps de la… décision

Vous avez besoin de faire un choix difficile concernant votre boucle principale: comment simuler l'évolution du temps. Si vous désirez un contrôle par image alors vous avez besoin de déterminer combien est-t-il nécessaire à votre jeu d'être remis à jour et dessiné. Vous pouvez même vouloir une mise à jour et dessiner à différents taux (de rafraîchissement). Vous avez aussi besoin de considérer combien comment votre jeu échoue gracieusement si le système de l'utilisateur·ice ne peut soutenir la charge de travail. Commençons par considérer que vous êtes capables de gérer les entrées de l'utilisateur·ice et de mettre à jour l'état du jeu à chaque fois que vous dessinez. Nous ramifierons après.

> [!NOTE]
> Changer la manière dont votre boucle principale gère le temps est un cauchemar de débogage, partout. Pensez à vos besoins, précautionneusement, avant de travailler sur votre boucle principale.

### De quoi les jeux dans le navigateur doivent avoir l'air
...
Si le maximum du taux de rafraîchissement ne peut être atteint, les paramètres de qualités peuvent être mis à jour pour rester sous notre gain en temps. L'exemple le plus célèbre de ce concept est le jeu de id Software, RAGE. Ce jeu a retiré à l'utilisateur·ice le contrôle afin de conserver son temps de calcul à environ 16ms (ou environ 60ips). Si le calcul prenait trop de temps alors la résolution est diminuée, les textures et autres éléments échouent au chargement et à l'affichage, et ainsi de suite. Ce cas (non-web) a créé plusieurs hypothèses et faits&nbsp;:
...
- Permettant à l'utilisateur·ice le contrôle à travers des paramètres est un cauchemar.
- Des moniteur différents apportent des taux de rafraîchissement différents: 30 FPS, 75 FPS, 100 FPS, 120 FPS, 144 FPS, etc.
- Des systèmes qui ne sont pas capables de fonctionner avec 60 FPS vont perdre en qualité pour permettre au jeu de rouler à une vitesse optimale (éventuellement, il échoue complètement si cela devient trop bas).
...
Une technique commune est de mettre à jour la simulation à une fréquence constante et dessiner autant (ou au moins) que le taux actuel le permet. Cette méthode de mise à jour peut continuer à boucler sans se soucier de ce que l'utilisateur·ice voit. Cette méthode peut voir la dernière mise à jour, et quand elle est arrivée. Quand le dessin sait quand il est représenté, et le temps simulé pour la dernière mise à jour, il peut prédire une image plausible à dessiner. Cela n'a pas d'importance si c'est plus fréquent que la mise à jour officielle (ou même moins fréquente). La méthode de mise à jour établis des points de contrôle, autant que le système le permet, la méthode de rendu va dessiner autour de ces instants de temps. Il y a plusieurs manières de séparer la méthode de mise à jour dans les standards du web&nbsp;:

- Dessiner à chaque `requestAnimationFrame()` et mettre à jour {{DOMxRef("Window.setInterval", "setInterval()")}} ou {{DOMxRef("Window.setTimeout", "setTimeout()")}}.
...
- Dessiner à chaque `requestAnimationFrame()` et mettre à jour sur un `setInterval` ou `setTimeout` dans un [Web Worker](/fr/docs/Web/API/Web_Workers_API/Using_web_workers).
  - C'est la même chose que ci-dessus, excepté que la mise à jour ne monopolise pas le fil principal (ni le fil principal ne le monopolise). C'est une solution plus complexe, et ce peut être trop de travail pour de simples mises à jours.
...
- Dessiner à chaque `requestAnimationFrame()` et l'utiliser pour solliciter un Web Worker qui contient la méthode de mise à jour avec la quantité de temps à calculer, s'il y a lieu.
  - Cela se met en veille jusqu'à ce que `requestAnimationFrame()` est appelée et ne pollue pas le fil principal, et de plus vous ne vous reposez pas sur d'anciennes méthodes. À nouveau, c'est un peu plus complexe que les deux premières options, et _débuter_ chaque mise à jour est bloqué tant que le navigateur ne décide de lancer les retours rAF.

Chacune de ces méthodes ont un compromis similaire&nbsp;:

- Les utilisateur·ice·s peuvent éviter le rendu d'images ou interpoler celles en sus dépendant de leurs performances.
- Vous pouvez compter sur tous les utilisateur·ice·s mettant à jours les variables non-cosmétiques à la même fréquence constante, moins quelques hoquets.
- Beaucoup plus compliquée à programmer que la boucle de base que nous avons vu précédemment.
- Les entrées utilisateur·ice·s sont complètement ignorées jusqu'à la prochaine mise à jour (même si l'utilisateur·ice à un système rapide).
- L'interpolation obligatoire à un défaut de performance obligatoire.

Une méthode séparée de mise à jour et de dessin peut avoir l'air de l'exemple suivant. Pour les besoins de la démonstration, l'exemple est basé sur le troisième point, sans l'utilisation des Web Workers par soucis de lecture (et, soyons honnête, pour faciliter l'écriture).

> [!WARNING]
> Cet exemple spécifiquement, a besoin d'une relecture technique.
...
*
*
* MonJeu.lastRender fait le suivi du dernier poinçon de temps fourni par requestAnimationFrame.
* MonJeu.lastTick fait le suivi de la dernière mise à jour. Toujours incrémenté par tickLength.
* MonJeu.tickLength est à quelle fréquence le jeu est mis à jour. C'est 20 Hz (50ms) ici.
*
* timeSinceTick est le temps entre un retour de requestAnimationFrame et la dernière mise à jour.
* numTicks est combien de mises à jour ont dû avoir lieu entre 2 rendus d'images.
*
* render() se voit passé tFrame, car il est considéré que la méthode de rendu va calculer
            combien de temps s'est écoulé depuis la mise à jour la plus récente pour
            extrapolation (purement cosmétique pour des systèmes rapides). La scène est dessinée.
*
* update() calcule l'état du jeu comme point donné dans le temps. Ça doit toujours être
            ...
            la dernière mise à jour + MonJeu.tickLength qu'une pause ne soit ajoutée, etc.)
*
* setInitialState() réalise n'importe quel tâche mise de côté avant que la boucle principale ne doive tourner.
*                   Ceci est juste un exemple générique d'une fonction que vous devez ajouter.
*/

(() => {
  function main(tFrame) {
    MonJeu.stopMain = window.requestAnimationFrame(main);
    const nextTick = MonJeu.lastTick + MonJeu.tickLength;
    let numTicks = 0;
    ...
    //Note: Comme nous le mentionnons dans le sommaire, vous devez faire un suivi de la taille de numTicks.
    //S'il est important, alors soit votre jeu est en veille, soit votre machine ne répond plus.
    if (tFrame > nextTick) {
      const timeSinceTick = tFrame - MonJeu.lastTick;
      numTicks = Math.floor(timeSinceTick / MonJeu.tickLength);
    }

    queueUpdates(numTicks);
    render(tFrame);
    MonJeu.lastRender = tFrame;
  }

  function queueUpdates(numTicks) {
    for (let i = 0; i < numTicks; i++) {
      MonJeu.lastTick = MonJeu.lastTick + MonJeu.tickLength; // Maintenant lastTick est ce tick.
      update(MonJeu.lastTick);
    }
  }

  MonJeu.lastTick = performance.now();
  MonJeu.lastRender = MonJeu.lastTick; //Prétendre que le premier dessin était sur la première mise à jour.
  MonJeu.tickLength = 50; //Cela positionne votre simulation pour tourner à 20Hz (50ms)
