...
Cet article explique comment créer une nouvelle page de référence pour un en-tête HTTP.
...
- Les en-têtes HTTP existants sont documentés [dans la référence HTTP](/fr/docs/Web/HTTP/Reference/Headers).
...
- Vous pouvez soit partir de notre [modèle de page d'en-tête HTTP](/fr/docs/MDN/Writing_guidelines/Page_structures/Page_types#page_de_référence_den-tête_http), soit utiliser une structure copiée à partir de l'un des documents d'en-tête HTTP existants que vous avez trouvés à l'étape 2. C'est à vous de choisir.
