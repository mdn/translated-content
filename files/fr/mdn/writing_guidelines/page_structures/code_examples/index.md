...
Sur MDN, vous trouvez de nombreux exemples de code qui montrent comment utiliser les fonctionnalités de la plateforme Web que nous documentons.
...
La macro `InteractiveExample` accepte un titre pour l'exemple sous forme d'une chaîne de caractères, suivi d'un mot-clé pour définir la hauteur de l'exemple.
...
Une \{{EmbedGHLiveSample}} prend le contenu à une URL définies (qui doit être un dépôt GitHub **MDN**), et l'insère dans la page dans un {{HTMLElement("iframe")}}.
...
2. La largeur d'un `<iframe>`, qui peut être exprimée en pourcentage ou en pixels.
3. La hauteur d'un `<iframe>`, qui peut être exprimée en pourcentage ou en pixels.
