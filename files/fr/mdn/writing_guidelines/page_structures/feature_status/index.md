...
> [!WARNING]
> La macro `\{{Deprecated_Header}}` n'est plus nécessaire pour générer une bannière **Statut obsolète**. La plateforme génère désormais automatiquement la bannière en utilisant la propriété front-matter [`status`](#icônes_de_statut_dans_les_barres_latérales) ou les données de [web-features <sup>(angl.)</sup>](https://github.com/web-platform-dx/web-features).
...
Les macros suivantes servent à afficher les icônes de statut en incise à côté des éléments de liste de définitions&nbsp;:
...
Si une page de fonctionnalité web possède une bannière de statut, alors les macros en incise du même statut sont utilisées explicitement pour chaque membre ou valeur de la fonctionnalité dans la liste de définitions.
