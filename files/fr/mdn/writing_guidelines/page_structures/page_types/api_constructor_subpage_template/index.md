---
title: Modèle de sous-page de constructeur d'API
...
> Les valeurs doivent être mises à jour de manière appropriée pour le constructeur.
>
> ```md
> ---
> title: NameOfTheParentInterface : constructeur NameOfTheConstructor()
...
>     Format&nbsp;: `NameOfTheParentInterface : constructeur NameOfTheConstructor()`.
>     Par exemple, le constructeur [Request()](/fr/docs/Web/API/Request/Request) a un _titre_ de `Request : constructeur Request()`.
> - **slug**
>   - : La fin du chemin URL après `https://developer.mozilla.org/fr/docs/`.
>     C'est formaté comme `Web/API/NameOfTheParentInterface/NameOfTheConstructor`.
>     Notez que le nom de la fonction constructeur dans le slug omet les parenthèses (il se termine par `NameOfTheConstructor` et non `NameOfTheConstructor()`).
> - **page-type** (pages anglaises uniquement)
>   - : La clé `page-type` pour les constructeurs Web/API est toujours `web-api-constructor`.
...
>   - : Remplacez la valeur de l'espace réservé `path.to.feature.NameOfTheConstructor` par la chaîne de caractères de requête pour le constructeur dans le [dépôt de données de compatibilité des navigateurs <sup>(angl.)</sup>](https://github.com/mdn/browser-compat-data).
...
>
>     Notez que vous devez peut-être d'abord créer/met à jour une entrée pour le constructeur API dans notre [dépôt de données de compatibilité des navigateurs <sup>(angl.)</sup>](https://github.com/mdn/browser-compat-data), et l'entrée pour l'API doit inclure des informations sur la spécification.
...
>   Pour générer la barre latérale correcte pour votre API, vous devez ajouter une entrée `GroupData` à notre dépôt GitHub, et inclure le nom de l'entrée dans l'appel de macro à la place de _GroupDataName_.
...
Vous pouvez copier la plupart de ces informations à partir du résumé du constructeur sur la page de référence API correspondante.
...
- `parameter1` {{Optional_Inline}}
  - : Incluez une brève description du paramètre et de ce qu'il fait ici. Incluez un terme et une définition pour chaque paramètre.
    Si le paramètre n'est pas optionnel, supprimez l'appel de macro \\{{Optional_Inline}}.
- `parameter2`
  - : etc.

### Valeur de retour

Incluez une description de la valeur de retour du constructeur, y compris le type de données et ce qu'il représente.
Ceci est normalement juste "Une instance de l'objet `\{{DOMxRef("NameOfTheParentInterface")}}`."

_Pour utiliser cette macro, supprimez les accents inversés et l'antislash dans le fichier markdown._

### Exceptions

Incluez une liste de toutes les exceptions que le constructeur peut lever. Incluez un terme et une définition pour chaque exception.
...
Notez que nous avons deux types d'exceptions&nbsp;: les objets {{DOMxRef("DOMException")}} et les exceptions JavaScript régulières, comme {{JSxRef("TypeError")}} et {{JSxRef("RangeError")}}. Un·e développeur·euse web doit savoir&nbsp;:
...
> [!NOTE]
> Parfois, vous allez vouloir créer un lien vers des exemples donnés sur une autre page.
>
...
>
> Ne ajoutez pas de titres H3&nbsp;; ajoutez simplement les liens directement sous le titre H2 "Exemples". Par exemple&nbsp;:
>
> ```md
> ## Exemples
>
