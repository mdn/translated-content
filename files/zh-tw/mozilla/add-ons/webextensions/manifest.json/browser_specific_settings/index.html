...
<table class="fullwidth-table standard-table">
 <tbody>
  ...
  <tr>
   <th scope="row">強制</th>
   <td>通常是不強制（請看<a href="/en-US/Add-ons/WebExtensions/WebExtensions_and_the_Add-on_ID#When_do_you_need_an_add-on_ID">你什麼時候會需要 Add-on ID？</a>）。在 Firefox 48（桌面）前以及Android版Firefox 是強制的。</td>
  </tr>
  <tr>
   <th scope="row">範例</th>
   <td>
...
<ul>
 <li><code>id</code> 是套件ID。Firefox 48 以後爲選擇性，48前爲強制。需要什麼來指定 add-on ID 請查看<a href="/en-US/docs/Mozilla/Add-ons/WebExtensions/WebExtensions_and_the_Add-on_ID">套件與 Add-on ID</a>。</li>
 ...
 <li><code>update_url</code> 是<a href="/en-US/Add-ons/Updates">套件更新 manifest</a>。備：連結必須以 "https" 開頭。這個鍵用來自己管理套件更新（不透過套件管理器）。</li>
</ul>

<p>查看<a href="https://addons.mozilla.org/en-US/firefox/pages/appversions/">可用Gecko版本</a>。</p>

<h3 id="套件ID格式">套件ID格式</h3>

<p>套件ID格式必須是下列其中一種：</p>

<ul>
 <li><a href="https://en.wikipedia.org/wiki/Universally_unique_identifier">GUID</a></li>
 <li>寫得像信箱地址的字串：<code>extensionname@example.org</code></li>
</ul>
...
<pre class="brush: json">"id": "extensionname@example.org",
