...
<p>This documentation is not yet complete. Want to help document Firefox 10? See the <a href="http://beta.elchi3.de/doctracker/#list=fx&amp;version=10.0">list of bugs that need to be written about</a> and pitch in!</p>

<div class="note"><p><strong>備註：</strong> Firefox 10 is the first release of this browser with two digits. This may lead to problem with some UA-sniffing scripts. Be sure to check them, and those contained in 3rd-party software you embed in your pages, like libraries. For more information about this, look at the <a href="http://hacks.mozilla.org/2012/01/firefox-goes-2-digit-time-to-check-your-ua-sniffing-scripts/">Firefox goes 2-digit article on hack.mozilla.org</a>.</p></div>
...
<ul>
 <li>The method <code><a href="/en/JavaScript/Reference/Global_Objects/WeakMap">WeakMap</a>.set()</code> now returns <em><code>undefined</code></em>, instead of itself.</li>
 ...
 <li>You can no longer use <a href="/en/E4X">E4X</a> syntax while in <a href="/en/JavaScript/Reference/Functions_and_function_scope/Strict_mode">ECMAScript 5 strict mode</a> (that is, after <code>"use strict;"</code>).</li>
</ul>
...
<ul>
 <li>The DOM Event method <a href="/en/DOM/event.stopImmediatePropagation"><code>event.stopImmediatePropagation</code></a> has been implemented.</li>
...
<ul>
 <li>The <a href="/en/DOM/Using_the_Page_Visibility_API">Page Visibility API</a> has been implemented (prefixed): <code>document.mozHidden</code>, <code>document.mozVisibilityState</code> are available and the event <code>mozvisibilitychanged</code> is sent when the state is modified.</li>
</ul>
...
<ul>
 <li>The <a href="/en/DOM/CanvasRenderingContext2D#createPattern%28%29"><code>createPattern()</code></a> method now throws an exception if a zero-sized source canvas is specified.</li>
 <li>If you use a non-finite value for any of the numeric parameters to <a href="/en/DOM/CanvasRenderingContext2D#putImageData%28%29"><code>putImageData()</code></a>, the call is now silently ignored instead of throwing an exception, in keeping with the specification.</li>
</ul>

<h4 id="WebGL">WebGL</h4>

<ul>
 <li>Firefox 10 now supports the <a href="http://www.khronos.org/registry/webgl/extensions/OES_standard_derivatives/"><code>OES_standard_derivatives</code></a> extension.</li>
 <li><a href="/en/WebGL#WebGL_debugging_and_testing">New preferences have been added</a> to help test WebGL code for compatibility with minimally-capable devices on your full development platform.</li>
</ul>

<h4 id="Web_Workers">Web Workers</h4>

<ul>
 <li>The attribute <code>XMLHttpRequest.responseType</code> and <code>XMLHttpRequest.response</code> are now available from inside <a href="/en/DOM/Worker/Functions_available_to_workers#section_2">Workers</a>.</li>
 <li>The <a href="/en/DOM/Worker#Worker()"><code>Worker()</code></a> constructor now accepts <a href="/en/data_URIs">data URIs</a>.</li>
</ul>
...
<ul>
 <li>The <a href="/en/IndexedDB/IDBIndex#count"><code>IDBIndex.count()</code></a> and <a href="/en/IndexedDB/IDBObjectStore#count"><code>IDBObjectStore.count()</code></a> methods have been added.</li>
 <li>The <a href="/en/IndexedDB/IDBCursor#advance"><code>IDBCursor.advance()</code></a> method has been added.</li>
 <li>When encountering an unknown optional parameter in <a href="/en/IndexedDB/IDBObjectStore#createIndex"><code>IDBObjectStore.createIndex()</code></a> or <a href="/en/IndexedDB/IDBDatabase#createObjectStore"><code>IDBDatabase.createObjectStore()</code></a>, Gecko will not fire an exception anymore, but simply ignore it.</li>
 <li>When <a href="/en/IndexedDB/IDBTransaction#abort%28%29"><code>IDBTransaction.abort()</code></a> is called, all pending <a href="/en/IndexedDB/IDBRequest"><code>IDBRequest</code></a> have their <code>errorCode</code> set to <code>ABORT_ERROR</code>.</li>
 <li>The methods <a href="/en/IndexedDB/IDBObjectStore#delete()"><code>IDBObjectStore.delete()</code></a> and <a href="/en/IndexedDB/IDBCursor#delete()"><code>IDBCursor.delete()</code></a> now set the <code>result</code> attribute of the returned <a href="/en/IndexedDB/IDBRequest"><code>IDBRequest</code></a> to <code>undefined</code>.</li>
 <li>The method <a href="/en/IndexedDB/IDBDatabase#setVersion()"><code>IDBDatabase.setVersion()</code></a> has been removed as it was removed from the latest spec. The version of the database is given through the <a href="/en/IndexedDB/IDBFactory#open"><code>IDBFactory.open()</code></a> method which has been updated and the <code>onupgradeneeded </code>callback allows the schema of the database to be upgraded. The version itself has been changed from a <code>DOMString</code> to an <code>unsigned long long</code>. The <a href="/en/IndexedDB/IDBVersionChangeRequest"><code>IDBVersionChangeRequest</code></a> interface has been removed and replaced by the new <a href="/en/IndexedDB/IDBOpenDBRequest"><code>IDBOpenDBRequest</code></a> interface.</li>
 <li>The method <a href="/en/IndexedDB/IDBFactory#deleteDatabase%28%29"><code>IDBFactory.deleteDatabase()</code></a> method has been added.</li>
</ul>

<h4 id="Other">Other</h4>

<ul>
 <li>When the proper MIME type is passed, <code>image/svg+xml</code>, <a href="/en/DOM/DOMParser#Parsing_a_SVG_document">the <code>DOMParser</code> now creates a <code>SVGDocument</code></a> when given a string with SVG.</li>
 ...
 <li>The {{ domxref("XMLHttpRequest") }} <code>responseType</code> "<code>moz-json</code>" <a href="/en/Firefox_9_for_developers#DOM">introduced in Firefox 9</a> has been updated to the latest draft of the specification and has been unprefixed. See {{ bug("707142#c13") }}</li>
</ul>
...
<ul>
 ...
 <li>The {{ cssxref("text-overflow") }} property's handling of cases in which the box overflows on both sides while the <code>text-overflow</code> property is set to overflow on only one <a href="/en/CSS/text-overflow#Gecko_notes">has been corrected</a>.</li>
...
<ul>
 ...
 <li>The new <a href="/en/Tools/Page_Inspector">Page Inspector</a> has been added, providing an excellent way to examine and manipulate the HTML and CSS behind your content.</li>
</ul>

<h2 id="Changes_for_Mozilla_and_add-on_developers">Changes for Mozilla and add-on developers</h2>

<p>For an overview of likely issues that may arise when updating your add-ons to support Firefox 10, see <a href="/en/Firefox/Updating_add-ons_for_Firefox_10">Updating add-ons for Firefox 10</a>.</p>

<div class="note"><p><strong>備註：</strong> The old <a href="/en/PRBool"><code>PRBool</code></a> data type has been retired! Anywhere in the documentation that refers to it now uses the standard C++ <code>bool</code> type instead. Documentation will be updated in the future, but for now, just keep this in mind.</p></div>

<h3 id="Manifests">Manifests</h3>

<ul>
 <li>Support for <a href="/en/Install_Manifests#strictCompatibility"><code>&lt;em:strictCompatibility&gt;</code></a> has been added to the install manifest. It allows add-ons authors to opt in to checking the maximum version of their extension. If set to <code>true</code> the add-on will be disabled if the application version is greater than <code>&lt;em:maxVersion&gt;</code>. Firefox 10 defaults to add-ons being compatible, regardless of their specified maximum version. This flag overrides that preference. You should set this if your add-on does things that are likely to be broken by Firefox updates, <strong>but not</strong> if your add-on has a binary component, since such add-ons always get strictly checked (remember that binary components must always be recompiled for each major Firefox release).</li>
...
<ul>
 <li>Bootstrapped add-ons using a <a href="/en/Chrome_Registration">chrome.manifest</a> file now have the manifest file registered automatically. See the section <a href="/en/Extensions/Bootstrapped_extensions#Adding_user_interface_with_a_chrome.manifest">Adding user interface with a chrome.manifest</a> for details.</li>
</ul>

<h3 id="XPConnect">XPConnect</h3>

<ul>
 <li>Several new properties and methods have been added to <a href="/en/Components.utils"><code>Components.utils</code></a>, granting access to assorted debugging-related information.</li>
</ul>

<h3 id="Interface_changes">Interface changes</h3>

<ul>
 <li>The <code>mozISpellCheckingEngine</code> and <code>nsIEditorSpellCheck</code> interfaces have been updated to allow restartless add-ons to add dictionaries to the spell checker. <strong>XXX need to <a href="/En/Using_an_External_Spell-checker">update docs</a> on how to actually do this.</strong></li>
 <li>The <code>nsIBrowserHistory.lastPageVisited</code> attribute has been removed.</li>
 <li>The <code>nsIDocumentViewer</code> interface has been merged into <code>nsIContentViewer</code>.</li>
 <li>The <code>nsIURIFixup</code> interface has a new flag, <code>FIXUP_FLAG_USE_UTF8</code>, which lets you tell it to use UTF-8 instead of the platform character set, when doing conversions.</li>
</ul>

<h3 id="Plug-in_changes">Plug-in changes</h3>

<ul>
 <li>The <a href="/en/Gecko_Plugin_API_Reference/Plug-in_Development_Overview#Working_with_URLs">new variable <code>NPNVdocumentOrigin</code></a> has been added; this returns the document origin, and is more secure than {{ domxref("window.location") }}.</li>
</ul>
...
<ul>
 <li><a href="/en/Firefox_9_for_developers">Firefox 9 for developers</a></li>
 <li><a href="/en/Firefox_8_for_developers">Firefox 8 for developers</a></li>
 <li><a href="/en/Firefox_7_for_developers">Firefox 7 for developers</a></li>
 <li><a href="/en/Firefox_6_for_developers">Firefox 6 for developers</a></li>
