...
**`:lang()`** [CSS](/zh-TW/docs/Web/CSS) [偽類別](/zh-TW/docs/Web/CSS/Pseudo-classes)會根據元素被判定的語言來匹配元素。
...
在此範例中，`:lang()` 偽類別應用於父元素，並使用[子組合器](/zh-TW/docs/Web/CSS/Child_combinator)來設定其內的引號元素（{{htmlElement("q")}}）樣式。請注意，這並非唯一的方法，最佳方法取決於文件類型。另請注意，{{glossary("Unicode")}} 值用於指定一些特殊引號字元。
