...
Firefox 可透過 Google 的定位服務 (Google Location Services，GLS)，根據使用者的 WiFi 資訊而找出使用者的位置。與 Google 之間所交換的資料，包含 WiFi 存取點 (Access Point) 資料、Access token (類似 2 個禮拜的 cookie)、使用者的 IP 位址。若需更多資訊，可參閱 [Mozilla 的隱私權政策](https://www.mozilla.org/en-US/privacy/)與 [Google 的隱私權政策](http://www.google.com/privacy/lsf.html)。其內將詳述資料的使用方式。
