...
<p><strong><code>Document</code></strong> 介面代表所有在瀏覽器中載入的網頁，也是作為網頁內容 <a href="/en-US/docs/Using_the_W3C_DOM_Level_1_Core">DOM 樹</a>（包含如 {{HTMLElement("body")}}、{{HTMLElement("table")}} 與其它的{{domxref("Element", "元素")}}）的進入點。<code>Document</code> 提供了網頁文件所需的通用函式，例如取得頁面 URL 或是建立網頁文件中新的元素節點等。</p>

<p>{{inheritanceDiagram}}</p>

<p><code>Document</code> 介面描述了各種類型文件的共同屬性與方法。根據文件的類型（如 <a href="/zh-TW/docs/HTML">HTML</a>、<a href="/zh-TW/docs/XML">XML</a>、SVG 等），也會擁有各自的 API：HTML 文件（content type 為 <code>text/html</code>）實作了 {{domxref("HTMLDocument")}} 介面，而 XML 及 SVG 文件實作了 {{domxref("XMLDocument")}} 介面。</p>
...
<dl>
 ...
 <dt>{{domxref("Document.mozFullScreenElement")}} {{readonlyinline}} {{non-standard_inline}}</dt>
 <dd>The element that's currently in full screen mode for this document.</dd>
 <dt>{{domxref("Document.mozFullScreenEnabled")}} {{readonlyinline}} {{non-standard_inline}}</dt>
...
<dl>
 ...
 <dt>{{domxref("document.querySelector","document.querySelector(String selector)")}}</dt>
 <dd>Returns the first Element node within the document, in document order, that matches the specified selectors.</dd>
 <dt>{{domxref("document.querySelectorAll","document.querySelectorAll(String selector)")}}</dt>
...
<dl>
 ...
 <dd>Compiles an <code><a href="/en-US/docs/XPathExpression">XPathExpression</a></code> which can then be used for (repeated) evaluations.</dd>
...
<dl>
 ...
 <dd>Returns size in bytes of the document. Starting with Internet Explorer 11, that property is no longer supported. See <a href="http://msdn.microsoft.com/en-us/library/ms533752%28v=VS.85%29.aspx">MSDN</a>. Internet Explorer does not support all methods from the <code>Node</code> interface in the <code>Document</code> interface:</dt>
