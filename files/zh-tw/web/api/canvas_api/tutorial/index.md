...
[**`<canvas>`**](/en-US/docs/HTML/Canvas) 是一個 [HTML](/en-US/docs/HTML) 元素，我們可以利用程式腳本在這個元素上繪圖（通常是用 [JavaScript](/en-US/docs/JavaScript)）。除了繪圖，我們還可以合成圖片或做一些簡單（或是[不那麼簡單](/en-US/docs/HTML/Canvas/A_Basic_RayCaster)）的動畫。右方的影像便是一些運用 \<canvas> 的例子，接下來我們將會在教學文件中一一說明。

本教學從基礎知識開始，描述如何利用 \<canvas> 進行 2D 繪圖。教學中的範例會讓各位清楚瞭解 \<canvas> 該如何運用，另外也會提供程式碼範例，讓大家嘗試製作自己的內容。

`<canvas>` 最早是由 Apple 為 Mac OS X Dashboard 所提出，之後 Safari 和 Google Chrome 也都採用。[Gecko](/en-US/docs/Gecko) 1.8 作基礎的瀏覽器，如 Firefox 1.5 也都提供了支援。`<canvas>` 元素是 [WhatWG Web applications 1.0](http://www.whatwg.org/specs/web-apps/current-work/)（也就是 HTML5）規範的一部分，目前所有主流的瀏覽器都已支援。
...
- [繪製文字](/en-US/docs/Web/API/Canvas_API/Tutorial/Drawing_text)
...
- [進階動畫](/en-US/docs/Web/API/Canvas_API/Tutorial/Advanced_animations)
- [像素控制](/en-US/docs/Web/API/Canvas_API/Tutorial/Pixel_manipulation_with_canvas)
- [Hit regions and accessibility](/en-US/docs/Web/API/Canvas_API/Tutorial/Hit_regions_and_accessibility)
- [最佳化 canvas](/zh-TW/docs/Web/Guide/HTML/Canvas_tutorial/Optimizing_canvas)
- [Finale](/en-US/docs/Web/API/Canvas_API/Tutorial/Finale)

## 參見

- [Canvas topic page](/en-US/docs/HTML/Canvas)
...
- [Canvas examples](/en-US/docs/tag/Canvas_examples)
- [HTML5 Tutorial](http://html5tutorial.com)
- [Drawing Text Using a Canvas](/en-US/docs/Drawing_text_using_a_canvas)
