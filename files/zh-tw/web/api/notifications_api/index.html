...
<p>Notifications API允許網頁控制向終端用戶顯示系統通知 — 它在網頁瀏覽器的視圖之外，因此使用者切換顯示卡或移至不同的應用程式也能顯示。此 API 設計為在相容於不同平台與現有的通知系統相容。</p>

<h2 id="概念與使用方法">概念與使用方法</h2>

<p>在受到支持的平台上，顯示系統通知通常涉及兩件事。 首先，用戶需要授予當前的來源權限才能顯示系統通知，通常在應用程式或網站初始化完成後使用{{domxref("Notification.requestPermission()")}} 方法。</p>
...
<p>這不僅是最佳實踐 — 您不應向用戶發送他們不同意的通知，並且在未來瀏覽器將明確禁止沒有響應用戶請求允許通知對話框的通知。Firefox 72 開始已遵循這項約定。使用此方法將產生一個請求對話框，如下所示：</p>
...
<p>用戶可以從此處選擇允許、屏蔽來自此來源的通知，也可以不做選擇。 一旦選定，該設置通常將在當前會話中保持不變。</p>
...
<p>另外，Notifications API規範指定了 <a href="/en-US/docs/Web/API/ServiceWorker_API">ServiceWorker API</a> 的許多附加功能，以允許服務人員觸發通知。</p>

<div class="note">
<p><strong>注意</strong>: 要了解有關在自己的應用程序中使用通知的更多信息，請閱讀 <a href="/en-US/docs/Web/API/Notifications_API/Using_the_Notifications_API">Using the Notifications API</a>。</p>
</div>
...
<dl>
 ...
 <dd>包含 {{domxref("ServiceWorkerRegistration.showNotification()")}} 和 {{domxref("ServiceWorkerRegistration.getNotifications()")}} 用於控制通知顯示的方法。</dd>
 <dt>{{domxref("ServiceWorkerGlobalScope")}}</dt>
 <dd>包含 {{domxref("ServiceWorkerGlobalScope.onnotificationclick")}} 點擊通知時觸發自定義函數的處理程序。</dd>
 <dt>{{domxref("NotificationEvent")}}</dt>
 <dd>基於 {{domxref("ExtendableEvent")}} 的特定類型的事件對象，它表示已觸發的通知。</dd>
</dl>
