...
當一個視窗是由另一個視窗所開啟（使用 {{domxref("Window.open")}} 或一個帶有 [`target`](/zh-TW/docs/Web/HTML/Reference/Elements/a#target) 屬性設定的連結），被開啟的這個視窗會於 **window\.opener** 保留開啟它的第一個視窗之參考。假如目前的視窗沒有開啟它的視窗，則會回傳 NULL。
