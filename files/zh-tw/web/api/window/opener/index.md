...
如果開啟者與目前頁面不同源，則開啟者物件的功能會受到限制。例如，無法存取視窗物件上的變數和函式。然而，可以導覽開啟者視窗，這表示被開啟的頁面可以在原始分頁或視窗中開啟一個 URL。在某些情況下，這可能導致網路釣魚攻擊，其中新開啟的頁面會將原始視窗中開啟的受信任頁面替換為釣魚頁面。

確切來說，對於跨來源的開啟者物件，可以使用以下屬性：
