...
- `code`
  - : 一種可選語法，允許使用字串代替函式，該字串會被編譯並在每個 `delay` 毫秒執行一次。
...
{{EmbedLiveSample("Example_2:_Alternating_two_colors", "從變數中釋放我們的 intervalID")}}
...
myArray.myMethod(); // 輸出 "zero,one,two"
myArray.myMethod(1); // 輸出 "one"
...
如果你的邏輯執行可能需要比間隔時間更長，建議你使用 {{domxref("Window.setTimeout", "setTimeout()")}} 遞歸調用一個命名函數。例如，如果使用 `setInterval()` 每 5 秒輪詢一次遠程伺服器，網路延遲、伺服器無反應以及其他問題可能會阻止請求在指定的時間內完成。因此，你可能會發現排隊的 XHR 請求未必會按順序返回。

在這些情況下，推薦使用遞歸的 `setTimeout()` 模式：
...
在上述程式碼片段中，宣告了一個命名函數 `loop()` 並立即執行。在執行完邏輯後，`loop()` 會在 `setTimeout()` 內部被遞歸調用。雖然這個模式無法保證在固定的間隔執行，但它能保證在遞歸之前，前一個間隔已經執行完成。
...
- [`core-js` 中的 `setInterval` Polyfill 允許將參數傳遞給回調函式。](https://github.com/zloirock/core-js#settimeout-and-setinterval)
