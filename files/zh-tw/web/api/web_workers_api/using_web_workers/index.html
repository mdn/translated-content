...
---
<div>
<p>Web Workers 提供簡單的方法讓網頁在背景執行緒 (Thread) 中執行程式，而不干擾使用者介面運行，另外，Worker也可以利用 <a href="/en/nsIXMLHttpRequest">XMLHttpRequest</a> 執行輸出/輸入(但是responseXML 和channel這兩個屬性為null)；一個worker可以藉由事件處理器來和 web worker 創造端互相傳送訊息，接下來本文會提供使用 web worker 的詳細說明。</p>
</div>
...
<p>worker 可以產生新 worker，只要新 worker 的來源 (origin) 和父頁面相同，也可以利用 <a href="/en/nsIXMLHttpRequest">XMLHttpRequest</a> 執行輸出/輸入(但是responseXML 和channel這兩個屬性為null)。</p>

<h2 id="Dedicated_workers">Dedicated workers</h2>

<p>dedicated worker 只能被產生它的檔案存取，下面我們先介紹簡單的 <a href="https://github.com/mdn/simple-web-worker">Basic dedicated worker example</a> (<a href="http://mdn.github.io/simple-web-worker/">run dedicated worker</a>) 範例。這個範例會將兩個數字送入 worker 相乘，然後再於前端頁面顯示相乘結果。</p>
...
<p>當執行時期錯誤發生時，onerror 事件處理器會被呼叫，onerror事件處理器會收到一名為 error 的事件物件 (實作 ErrorEvent Interface)，該事件不會 bubble 且可取消，如果要避免事件預設行為，可以呼叫 <a href="/en-US/docs/Web/API/Event/preventDefault"><code>preventDefault()</code></a>。</p>
...
<p>瀏覽器會載入並執行每個程式碼腳本，然後 worker 能夠存取程式碼腳本內定義的全域變數，若是腳本無法載入，會產生一個 NETWORK_ERROR，後續的程式碼不會被執行，但是先前執行過的程式碼或用 <a href="/en-US/docs/Web/API/window.setTimeout">window.setTimeout()</a> 延遲執行的程式碼依然有效，而 importScripts() 之後宣告的函數也一樣存在，因為這些程式碼總是在其他程式碼之前就解析過了。</p>
...
<p>shared worker 能夠被多個程式腳本存取，縱使跨越不同 window、iframe 或 worker。這邊的 <a href="https://github.com/mdn/simple-shared-worker">Basic shared worker example</a> (<a href="http://mdn.github.io/simple-shared-worker/">run shared worker</a>) 範例和 dedicated worker 範例類似，但多了兩個可以讓多個檔案存取的函數：<em>數字相乘以及數字平方</em>。</p>
...
<p><a href="/samples/workers/fibonacci">範例測試</a>。</p>

<h3 id="在背景中執行_web_IO">在背景中執行 web I/O</h3>

<p>範例請見 <a href="/en-US/docs/Using_workers_in_extensions">Using workers in extensions</a> 。</p>
...
<ul>
 ...
 <li>Chrome Workers 是 Firefox 唯一的 worker 類型，他們可以用在開發 add-ons，或是想要使用 <a href="/en/js-ctypes">js-ctypes</a>。詳情請見 {{domxref("ChromeWorker")}}。</li>
