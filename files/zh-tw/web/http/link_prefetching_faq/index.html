...
---
<h3 id="What_is_link_prefetching.3F">何謂預先取回連結 ?</h3>
...
<h3 id="What_are_the_prefetching_hints.3F">如何告訴瀏覽器預先取回資源 ?</h3>
...
<h3 id="Are_anchor_.28.3Ca.3E.29_tags_prefetched.3F">&lt;a&gt; 元素能被預先取回嗎 ?</h3>

<p>不行。目前只有 rel 屬性為 prefetch 的 link 元素 (或標頭) 所標示的資源能被預先取回。</p>

<h3 id="Is_link_prefetching_standards_compliant.3F">預先取回 (Prefetch) 符合標準嗎 ?</h3>
...
<h3 id="How_is_browser_idle_time_determined.3F">如何判定瀏覽器是否閒置 ?</h3>
...
<h3 id="What_happens_if_I_click_on_a_link_while_something_is_being_prefetched.3F">當預先取回執行中有連結被點擊時會如何 ?</h3>
...
<h3 id="What_if_I.27m_downloading_something_in_the_background.3F_Will_link_prefetching_compete_for_bandwidth.3F">當在背景下載檔案，預先取回作業會佔用頻寬嗎 ?</h3>

<p>不一定。Firefox 會先暫停預先取回作業直到背景下載任務結束，但如果是使用其他軟體下載檔案，那麼 Firefox 的預先取回作業並不會停止，未來 Firefox 計畫加入偵測作業系統的網路閒置時間 。</p>

<h3 id="Are_there_any_restrictions_on_what_is_prefetched.3F">預先取回有限制嗎 ?</h3>

<p>有。只有 http:// (and, starting in {{ Gecko("1.9.1") }} https://) URLs 能被預先取回，其它如 FTP 則無法。</p>

<h3 id="Will_Mozilla_prefetch_documents_from_a_different_host.3F">Mozilla 會預先取回不同網域的文件嗎 ?</h3>

<p>會，因為預先取回並沒有 same-origin (同源政策) 的限制，同源限定並不會加強瀏覽器安全性。</p>

<h3 id="Do_prefetched_requests_contain_a_Referer:_header.3F">預先取回請求帶有 Referer: header 嗎 ?</h3>
...
<h3 id="As_a_server_admin.2C_can_I_distinguish_prefetch_requests_from_normal_requests.3F">如何分辨來自一般和預先取回的請求 ?</h3>
...
<h3 id="Is_there_a_preference_to_disable_link_prefetching.3F">可以從偏好設定裡關閉預先取回嗎 ?</h3>
...
<h3 id="What_about_folks_who_pay-per-byte_for_network_bandwidth.3F">對頻寬使用量付費的使用者的影響</h3>
...
<h3 id="Which_browsers_support_link_prefetching.3F">那些瀏覽器支援預先取回 ?</h3>

<p>Firefox 和 Netscape 7.01+。 <a href="http://gemal.dk/browserspy/prefetch.php">測試</a>瀏覽器是否支援預先取回功能。</p>

<h3 id="Privacy_implications">隱私權議題</h3>
...
<h3 id="What_about....3F">還有... ?What about...?</h3>

<p>若是還有其他有關預先取回的問題，請不要客氣直接發問 :-)</p>

<h4 id="See_also...">延伸閱讀</h4>
