...
> [!NOTE]
> 如果你收到的回應未列於此處，那麼它是一個非標準回應，可能是伺服器軟體的自訂回應。
...
- {{HTTPStatus(202, "202 Accepted")}}
  - : 已接收請求，但尚未對其進行處理。由於在 HTTP 中沒有後續發送表示請求結果的非同步回應的方法，因此此回應是非承諾性的。它適用於另一個處理請求的進程或伺服器，或用於批次處理。
- {{HTTPStatus(203, "203 Non-Authoritative Information")}}
  - : 此回應代碼表示返回的後設資料與原始伺服器提供的資料不完全相同，而是從本地或第三方副本收集的。這主要用於另一個資源的鏡像或備份。除了這種特定情況外，{{HTTPStatus(200, "200 OK")}} 回應優先於此狀態。
- {{HTTPStatus(204, "204 No Content")}}
  - : 沒有內容可發送給此請求，但標頭很有用。用戶端可以使用新的標頭更新此資源的快取標頭。
- {{HTTPStatus(205, "205 Reset Content")}}
  - : 告訴用戶端重置發送此請求的文件。
- {{HTTPStatus(206, "206 Partial Content")}}
  - : 此回應代碼用於回應[範圍請求](/zh-TW/docs/Web/HTTP/Guides/Range_requests)，在用戶端請求資源的一部分或多個部分時使用。
- {{HTTPStatus(207, "207 Multi-Status")}}（{{Glossary("WebDAV")}}）
  - : 傳達有關多個資源的訊息，適用於可能適用多個狀態碼的情況。
...
- {{HTTPStatus(226, "226 IM Used")}}（[HTTP 增量編碼](https://datatracker.ietf.org/doc/html/rfc3229)）
  - : 伺服器已滿足對資源的 {{HTTPMethod("GET")}} 請求，並且回應是對當前實例應用的一個或多個實例操作的結果的表示。

## 重新導向訊息

- {{HTTPStatus(300, "300 Multiple Choices")}}
  - : 在[代理驅動的內容協商](/zh-TW/docs/Web/HTTP/Guides/Content_negotiation#代理驅動的內容協商)中，該請求有多個可能的回應，用戶代理或使用者應選擇其中一個。沒有標準化的方法讓用戶端自動選擇其中一個回應，因此很少使用。
- {{HTTPStatus(301, "301 Moved Permanently")}}
  - : 所請求的資源的 URL 已永久更改。新的 URL 在回應中給出。
- {{HTTPStatus(302, "302 Found")}}
  - : 此回應代碼表示所請求的資源的 URI 已*暫時*更改。將來可能對 URI 進行進一步更改，因此用戶端應在以後的請求中使用相同的 URI。
- {{HTTPStatus(303, "303 See Other")}}
  - : 伺服器發送此回應以指示用戶端使用 {{HTTPMethod("GET")}} 請求在另一個 URI 獲取所請求的資源。
- {{HTTPStatus(304, "304 Not Modified")}}
  - : 這用於緩存目的。它告訴用戶端回應未被修改，因此用戶端可以繼續使用回應的相同[快取](/zh-TW/docs/Web/HTTP/Guides/Caching)版本。
- `305 Use Proxy` {{deprecated_inline}}
  - : 在 HTTP 規範的先前版本中定義，表示必須通過代理訪問所請求的回應。由於關於代理的帶內設定的安全問題，它已被棄用。
- `306 unused`
  - : 此回應代碼不再使用；但它被保留了。它曾在 HTTP/1.1 規範的先前版本中使用過。
- {{HTTPStatus(307, "307 Temporary Redirect")}}
  - : 伺服器發送此回應以指示用戶端使用在先前請求中使用的相同方法，在另一個 URI 獲取所請求的資源。這與 `302 Found` HTTP 回應代碼具有相同的語義，唯一的區別在於用戶代理*不能*改變使用的 HTTP 方法：如果在第一個請求中使用了 {{HTTPMethod("POST")}}，則在重新導向的請求中必須使用 `POST`。
- {{HTTPStatus(308, "308 Permanent Redirect")}}
  - : 這意味著資源現在永久位於另一個 URI，由 {{HTTPHeader("Location")}} 回應標頭指定。這與 `301 Moved Permanently` HTTP 回應代碼具有相同的語義，唯一的區別在於用戶代理*不能*改變使用的 HTTP 方法：如果在第一個請求中使用了 {{HTTPMethod("POST")}}，則在第二個請求中必須使用 `POST`。
...
- {{HTTPStatus(402, "402 Payment Required")}}
  - : 此代碼的最初目的是用於數位支付系統，但此狀態碼極少使用，且沒有標準約定存在。
...
- {{HTTPStatus(405, "405 Method Not Allowed")}}
  - : 伺服器知道[請求方法](/zh-TW/docs/Web/HTTP/Reference/Methods)，但不支援目標資源。例如，API 可能不允許對資源使用 `DELETE`，或完全不允許使用 `TRACE` 方法。
- {{HTTPStatus(406, "406 Not Acceptable")}}
  - : 當網路伺服器在[伺服器驅動的內容協商](/zh-TW/docs/Web/HTTP/Guides/Content_negotiation#伺服器驅動的內容協商)後，找不到符合用戶代理給定標準的內容時，就會發送此回應。
- {{HTTPStatus(407, "407 Proxy Authentication Required")}}
  - : 這與 `401 Unauthorized` 類似，但需要代理進行驗證。
- {{HTTPStatus(408, "408 Request Timeout")}}
  - : 一些伺服器在閒置連接時發送此回應，即使用戶端之前沒有發送任何請求。這意味著伺服器希望關閉此未使用的連接。由於一些瀏覽器使用 HTTP 預連接機制加快瀏覽速度，因此此回應用得更多。有些伺服器可能會直接關閉連線而不發送此訊息。
- {{HTTPStatus(409, "409 Conflict")}}
  - : 當請求與伺服器的當前狀態存在衝突時，就會發送此回應。在 {{glossary("WebDAV")}} 遠端網頁編寫中，`409` 回應是發送給用戶端的錯誤，以便使用者可以解決衝突並重新提交請求。
- {{HTTPStatus(410, "410 Gone")}}
  - : 當所請求的內容已永久從伺服器中刪除且沒有轉發地址時，就會發送此回應。用戶端應該刪除其緩存和指向資源的鏈接。HTTP 規範打算將此狀態碼用於「有限時間的促銷服務」。API 不應感到有必要使用此狀態碼來指示已刪除的資源。
- {{HTTPStatus(411, "411 Length Required")}}
  - : 伺服器拒絕了請求，因為未定義 {{HTTPHeader("Content-Length")}} 標頭欄位，而伺服器需要它。
- {{HTTPStatus(412, "412 Precondition Failed")}}
  - : 在[條件式請求](/zh-TW/docs/Web/HTTP/Guides/Conditional_requests)中，用戶端在其標頭中指示了伺服器未達到的前提條件。
- {{HTTPStatus(413, "413 Content Too Large")}}
  - : 請求主體大於伺服器定義的限制。伺服器可能會關閉連接或返回 {{HTTPHeader("Retry-After")}} 標頭欄位。
- {{HTTPStatus(414, "414 URI Too Long")}}
  - : 用戶端所請求的 URI 長度超過伺服器願意解釋的範圍。
- {{HTTPStatus(415, "415 Unsupported Media Type")}}
  - : 所請求資料的媒體格式不受伺服器支援，因此伺服器拒絕該請求。
- {{HTTPStatus(416, "416 Range Not Satisfiable")}}
  - : 請求中 `Range` 標頭欄位指定的[範圍](/zh-TW/docs/Web/HTTP/Guides/Range_requests)無法滿足。可能是目標資源的資料大小超出了範圍。
- {{HTTPStatus(417, "417 Expectation Failed")}}
  - : 此回應代碼意味著伺服器無法滿足 {{HTTPHeader("Expect")}} 請求標頭欄位指示的期望。
...
- {{HTTPStatus(500, "500 Internal Server Error")}}
  - : 伺服器遇到一個它不知道如何處理的情況。此錯誤是通用的，表示伺服器無法找到更合適的 `5XX` 狀態碼來回應。
- {{HTTPStatus(501, "501 Not Implemented")}}
  - : 伺服器不支援請求方法，無法處理。伺服器需要支援的唯一方法（因此不應返回此代碼）是 {{HTTPMethod("GET")}} 和 {{HTTPMethod("HEAD")}}。
- {{HTTPStatus(502, "502 Bad Gateway")}}
  - : 此錯誤回應表示，伺服器在作為閘道器以獲取處理請求所需的回應時，收到了無效的回應。
- {{HTTPStatus(503, "503 Service Unavailable")}}
  - : 伺服器尚未準備好處理該請求。常見原因是伺服器正在進行維護或負載過重。需要注意的是，除了此回應外，還應該發送一個用戶友好的頁面來解釋問題。此回應應該用於暫時的情況，並且如果可能，{{HTTPHeader("Retry-After")}} HTTP 標頭應包含服務恢復之前的估計時間。網站管理員還必須注意與此回應一起發送的與緩存相關的標頭，因為這些暫時的狀態回應通常不應該被緩存。
- {{HTTPStatus(504, "504 Gateway Timeout")}}
  - : 當伺服器充當閘道器且無法及時獲得回應時，將提供此錯誤回應。
- {{HTTPStatus(505, "505 HTTP Version Not Supported")}}
  - : 請求中使用的 HTTP 版本不受伺服器支援。
- {{HTTPStatus(506, "506 Variant Also Negotiates")}}
  - : 伺服器存在內部組態錯誤：在內容協商期間，所選擇的變體被組態為自行參與內容協商，這在建立回應時會導致循環引用。
...
- {{HTTPStatus(510, "510 Not Extended")}}
  - : 用戶端請求宣告應使用一個 HTTP 擴充（{{RFC("2774")}}）來處理請求，但該擴充不被支援。
