...
IM 代表*實例操作*，指的是產生*增量*的演算法。

在增量編碼中，用戶端會發送一個帶有兩個標頭的 {{HTTPMethod("GET")}} 請求：`A-IM:`，表示對差異演算法的偏好；以及 {{HTTPHeader("If-None-Match")}}，指定其擁有的資源版本。

伺服器會以相對於給定基礎文件的差異來回應，而不是回傳完整的文件。

此回應使用 `226` 狀態碼、一個描述所用差異演算法的 `IM:` 標頭，並可能包含一個 `Delta-Base:` 標頭，其 {{HTTPHeader("ETag")}} 與增量關聯的基礎文件相符。
