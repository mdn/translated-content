...
<ul>
 <li><a href="#賦值運算子">賦值運算子</a></li>
 <li><a href="#比較運算子">比較運算子</a></li>
 <li><a href="#算術運算子">算術運算子</a></li>
 <li><a href="#位元運算子">位元運算子</a></li>
 <li><a href="#邏輯運算子">邏輯運算子</a></li>
 <li><a href="#字串運算子">字串運算子</a></li>
 <li><a href="#條件（三元）運算子">條件（三元）運算子</a></li>
 <li><a href="#逗點運算子">逗點運算子</a></li>
 <li><a href="#一元運算子">一元運算子</a></li>
 <li><a href="#關係運算子">關係運算子</a></li>
</ul>

<p>JavaScript 同時具有二元運算子及一元運算子， 以及一種特殊的 三元運算子，也就是 條件運算子。 一個二元運算子需要具備兩個運算元， 一個在運算元之前，一個在運算元之後:</p>

<pre class="syntaxbox"><em>運算元1</em> <em>運算子</em> <em>運算元2</em>
</pre>
...
<pre class="syntaxbox"><em>運算子</em> <em>運算元</em>
</pre>

<p>或</p>

<pre class="syntaxbox"><em>運算元 運算子</em>
</pre>
...
<table>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>假如運算元具有相同型態且等價則回傳True。參考 {{jsxref("Object.is")}} 及 <a href="/en-US/docs/Web/JavaScript/Equality_comparisons_and_sameness">JS中的等價性</a>。</td>
...
<div class="note">
<p><strong>備註：</strong> <code>=&gt;</code> 不是運算子，是 <a href="/en-US/docs/Web/JavaScript/Reference/Functions/Arrow_functions">箭頭函式</a>。</p>
</div>
...
<ul>
 <li>運算元被轉換為32 bits 的整數以二進位形式表示 (0 和 1)。大於 32 bits 的數字將被捨棄多出來的位元。例如， 下列整數大於32個bit但是會被轉換為32個bit的整數:
  <pre>轉換之前:  11100110111110100000000000000110000000000001
...
<pre class="brush: js">var a1 =  true &amp;&amp; true;     // t &amp;&amp; t 回傳 true
...
<pre class="brush: js">var o1 =  true || true;     // t || t 回傳 true
...
<pre class="brush: js">var n1 = !true;  // !t 回傳 false
...
<pre class="brush: js">console.log('我的 ' + '字串'); // 會印出 字串 "我的字串"。</pre>

<p>簡化的設定運算子 += 也能用於串接字串。</p>

<p>例如，</p>

<pre class="brush: js">var mystring = '字';
...
<pre class="syntaxbox"><em>條件</em> ? <em>值1</em> : <em>值2</em>
</pre>
...
<pre class="brush: js">delete 物件名稱;
delete 物件名稱.性質;
delete 物件名稱[索引];
...
<pre class="syntaxbox">typeof 運算元
typeof (運算元)
</pre>
...
<pre class="brush: js">typeof myFun;       // 回傳 "function"
...
<pre class="brush: js">typeof true; // 回傳 "boolean"
typeof null; // 回傳 "object"
</pre>

<p>對於字串或數字， <code>typeof</code> 運算子會回傳下列結果:</p>

<pre class="brush: js">typeof 62;            // 回傳 "number"
typeof 'Hello world'; // 回傳 "string"
</pre>

<p>對於特性，<code>typeof</code> 運算子會回傳 特性的值的類型:</p>

<pre class="brush: js">typeof document.lastModified; // 回傳 "string"
...
<pre class="brush: js">typeof blur;        // 回傳 "function"
...
<pre class="brush: js">typeof Date;     // 回傳 "function"
...
<pre class="brush: html">&lt;a href="javascript:void(0)"&gt;點擊這裡，甚麼都不會發生&lt;/a&gt;
</pre>
...
<pre class="brush: js">性質名稱 in 物件名稱
</pre>
...
<pre class="syntaxbox">物件名稱 instanceof 物件類型
</pre>
...
<ul>
 <li>算術: 解析出數字， 例如 3.14159。（通常使用<a href="#算術運算子">算術運算子</a>。）</li>
 <li>字串: 解析出字串， 例如 "Fred" or "234"。（通常使用<a href="#字串運算子">字串運算子</a>。）</li>
 <li>邏輯: 解析出 True 或 False（通常與<a href="#邏輯運算子">邏輯運算子</a>相關。）</li>
...
<pre class="brush: html">&lt;p&gt;請輸入一介於18 與 99 的數字:&lt;/p&gt;
...
<pre class="brush: js">var 物件名稱 = new 物件型態([參數1, 參數2, ..., 參數N]);
</pre>
...
<pre class="syntaxbox">super([參數]); // 呼叫父物件的建構子.
super.父物件的函式([參數]);
</pre>
...
<pre class="brush: js">var parts = ['肩膀', '膝蓋'];
