...
由於 `asinh()` 是 `Math` 的靜態方法，你必須使用 `Math.asinh()` 來呼叫它，而非呼叫你建立的 `Math` 物件的方法（`Math` 並非建構函式）。
...
- [在 `core-js` 中的 `Math.asinh` polyfill](https://github.com/zloirock/core-js#ecmascript-math)
- {{jsxref("Math.acosh()")}}
- {{jsxref("Math.atanh()")}}
- {{jsxref("Math.cosh()")}}
- {{jsxref("Math.sinh()")}}
