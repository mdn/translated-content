...
預設參數允許使用任何運算式，但不可使用 {{jsxref("Operators/await", "await")}} 或 {{jsxref("Operators/yield", "yield")}}，因為這些運算子會導致預設運算式的評估暫停。參數的初始化必須是*同步*完成的。
...
預設引數會在*呼叫時*進行評估。與 Python（例如）不同，每次呼叫函式時都會建立一個新的物件。
