...
- `size`
  - : 這個屬性藉由數字或相對值指定文字大小。數字是由最小的 `1` 到最大的 `7` 之間，且預設為 `3` 的數值表示。也可以用諸如 `+2` 或 `-3` 的相對值指定, which set it relative to the value of the [`size`](/zh-TW/docs/Web/CSS/CSS_Fonts#size) attribute of the {{HTMLElement("basefont")}} element, or relative to 3, the default value, if none does exist.
