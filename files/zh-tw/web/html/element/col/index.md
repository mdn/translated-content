...
**`<col>`** [HTML](/zh-TW/docs/Web/HTML) 元素用於定義由其父元素 {{HTMLElement("colgroup")}} 表示的行組中的一個或多個行。`<col>` 元素僅在未定義 [`span`](/zh-TW/docs/Web/HTML/Element/colgroup#span) 屬性的 {{HTMLElement("colgroup")}} 元素的情況下作為其子元素有效。
...
- `valign` {{deprecated_inline}}
  ...
  - ：指定每個行單元格的垂直對齊方式。可能的 {{Glossary("enumerated")}} 值包括 `baseline`、`bottom`、`middle` 和 `top`。請注意，此屬性會覆蓋其 {{HTMLElement("colgroup")}} 父元素的指定 [`valign`](/zh-TW/docs/Web/HTML/Element/colgroup#valign)。請改用 {{cssxref("vertical-align")}} CSS 屬性應用於 {{htmlelement("td")}} 和 {{htmlelement("th")}} 元素上，因為此屬性已棄用。
    ...
    >
    > 如果表格未使用 [`colspan`](/zh-TW/docs/Web/HTML/Element/td#colspan) 屬性，則可以使用 `td:nth-of-type(an+b)` CSS 選擇器。將 `a` 設置為零，將 `b` 設置為表格中行的位置，例如 `td:nth-of-type(2) { vertical-align: middle; }` 可以使第二行單元格垂直居中。
    >
    > 如果表格使用了 [`colspan`](/zh-TW/docs/Web/HTML/Element/td#colspan) 屬性，則可以通過組合適當的 CSS 屬性選擇器，如 `[colspan=n]` 來達到效果，雖然這不是一個簡單的過程。
...
- `<col>` 元素不會將行結構上分組在一起，這是 {{HTMLElement("colgroup")}} 元素的作用。
- 只有有限數量的 CSS 屬性會影響 `<col>`：
  - {{cssxref("background")}}：各種 `background` 屬性將為行內的單元格設置背景。由於行背景顏色繪製在表格和行組（{{HTMLElement("colgroup")}}）的頂部，但在應用於行組（{{htmlelement("thead")}}、{{htmlelement("tbody")}} 和 {{htmlelement("tfoot")}}）、行（{{htmlelement("tr")}}）和單元格（{{htmlelement("th")}} 和 {{htmlelement("td")}}）的背景色之前，因此只有在所有覆蓋它們頂部的層都具有透明背景時，才會看到應用於表行的背景。
  - {{cssxref("border")}}：各種 `border` 屬性適用，但僅當 `<table>` 具有 {{cssxref("border-collapse", "border-collapse: collapse")}} 設置時。
  - {{cssxref("visibility")}}：對於一行，`collapse` 值導致該行的所有單元格不被呈現，並且跨入其他行的單元格被剪切。這些行將佔用的空間被移除。但是，其他行的大小仍然被計算，就好像摺疊行中的單元格存在一樣。`visibility` 的其他值沒有效果。
  - {{cssxref("width")}}：`width` 屬性定義了行的最小寬度，就像 {{cssxref("min-width")}} 被設置一樣。

## 範例

有關介紹常見標準和最佳實踐的完整表格範例，請參見 {{HTMLElement("table")}}。

此示例演示了一個分為三個 `<col>` 元素的八行表格。

### HTML

一個 {{HTMLElement("colgroup")}} 元素提供了一個基本表格的結構，創建了一個隱式的單行組。三個 `<col>` 元素包含在 `<colgroup>` 內，創建了三個可樣式化的行。當省略時，[`span`](#span) 屬性指定了每個 `<col>` 應跨越的表格行數（默認為 `1`），從而使每個 `<col>` 中的屬性可以在其所跨越的行中共享。
...
我們使用 CSS 而不是已棄用的 HTML 屬性來為行提供背景顏色並對齊單元格內容：
...
- {{cssxref("background-color")}}：設置每個行單元格的背景顏色的 CSS 屬性
- {{cssxref("border")}}：控制行單元格邊框的 CSS 屬性
- {{cssxref("text-align")}}：水平對齊每個行單元格內容的 CSS 屬性
- {{cssxref("vertical-align")}}：垂直對齊每個行單元格內容的 CSS 屬性
- {{cssxref("visibility")}}：隱藏行的單元格的 CSS 屬性
- {{cssxref("width")}}：控制每個行的默認寬度的 CSS 屬性
- {{cssxref(":nth-of-type")}}、{{cssxref(":first-of-type")}}、{{cssxref(":last-of-type")}}：CSS 偽類，用於選擇所需的行單元格
