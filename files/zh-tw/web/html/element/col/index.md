...
**`<col>`** [HTML](/zh-TW/docs/Web/HTML) 元素用於定義由其父元素 {{HTMLElement("colgroup")}} 表示的列組中的一個或多個列。`<col>` 元素僅在未定義 [`span`](/zh-TW/docs/Web/HTML/Element/colgroup#span) 屬性的 {{HTMLElement("colgroup")}} 元素的情況下作為其子元素有效。
...
- `span`
  - ：指定 `<col>` 元素橫跨的連續列數。該值必須是大於零的正整數。如果未指定，則其默認值為 `1`。
...
- `valign` {{deprecated_inline}}
  ...
  - ：指定每個列單元格的垂直對齊方式。可能的 {{Glossary("enumerated")}} 值包括 `baseline`、`bottom`、`middle` 和 `top`。請注意，此屬性會覆蓋其 {{HTMLElement("colgroup")}} 父元素的指定 [`valign`](/zh-TW/docs/Web/HTML/Element/colgroup#valign)。請改用 {{cssxref("vertical-align")}} CSS 屬性應用於 {{htmlelement("td")}} 和 {{htmlelement("th")}} 元素上，因為此屬性已棄用。

    > **備註：** 在 `<col>` 元素上設置 `vertical-align` 不起作用，因為 `<col>` 沒有後代元素，因此沒有元素繼承自它。
    >
    > 如果表格未使用 [`colspan`](/zh-TW/docs/Web/HTML/Element/td#colspan) 屬性，則可以使用 `td:nth-of-type(an+b)` CSS 選擇器。將 `a` 設置為零，將 `b` 設置為表格中列的位置，例如 `td:nth-of-type(2) { vertical-align: middle; }` 可以使第二列單元格垂直居中。
    >
    > 如果表格使用了 [`colspan`](/zh-TW/docs/Web/HTML/Element/td#colspan) 屬性，則可以通過組合適當的 CSS 屬性選擇器，如 `[colspan=n]` 來達到效果，雖然這不是一個簡單的過程。
...
- `<col>` 元素不會將列結構上分組在一起，這是 {{HTMLElement("colgroup")}} 元素的作用。
- 只有有限數量的 CSS 屬性會影響 `<col>`：
  - {{cssxref("background")}}：各種 `background` 屬性將為列內的單元格設置背景。由於列背景顏色繪製在表格和列組（{{HTMLElement("colgroup")}}）的頂部，但在應用於行組（{{htmlelement("thead")}}、{{htmlelement("tbody")}} 和 {{htmlelement("tfoot")}}）、行（{{htmlelement("tr")}}）和單元格（{{htmlelement("th")}} 和 {{htmlelement("td")}}）的背景色之前，因此只有在所有覆蓋它們頂部的層都具有透明背景時，才會看到應用於表列的背景。
...
一個 {{HTMLElement("colgroup")}} 元素提供了一個基本表格的結構，創建了一個隱式的單列組。三個 `<col>` 元素包含在 `<colgroup>` 內，創建了三個可樣式化的列。當省略時，[`span`](#span) 屬性指定了每個 `<col>` 應跨越的表格列數（默認為 `1`），從而使每個 `<col>` 中的屬性可以在其所跨越的列中共享。
...
- {{cssxref("background-color")}}：設置每個列單元格的背景顏色的 CSS 屬性
- {{cssxref("border")}}：控制列單元格邊框的 CSS 屬性
- {{cssxref("text-align")}}：水平對齊每個列單元格內容的 CSS 屬性
- {{cssxref("vertical-align")}}：垂直對齊每個列單元格內容的 CSS 屬性
- {{cssxref("visibility")}}：隱藏列的單元格的 CSS 屬性
- {{cssxref("width")}}：控制每個列的默認寬度的 CSS 屬性
- {{cssxref(":nth-of-type")}}、{{cssxref(":first-of-type")}}、{{cssxref(":last-of-type")}}：CSS 偽類，用於選擇所需的列單元格
