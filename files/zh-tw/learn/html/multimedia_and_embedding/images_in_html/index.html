...
<p>最初的網頁最初的發展階段，只是文字。而只有文字想當然爾令網頁讀起來十分的枯燥乏味。然而幸運的是沒有多久，將圖片（以及其他更有趣的內容類型）嵌入網頁的功能就誕生了。 在多媒體嵌入網頁的學習中，從<a href="/en-US/docs/Web/HTML/Element/img"> element embeds an image into the document."><code>&lt;img&gt;</code></a>元素開始是相對適當，因為該元素用於在網頁中嵌入簡單的圖像。 在本文中，我們將研究如何深入使用它，包括在網頁中嵌入簡單圖像的基礎知識，使用&lt;figure&gt;增加標題說明以做註釋，以及詳細說明它與CSS背景圖片的關係。</p>
...
<div class="note">
<p><strong>備註：</strong> 在繼續之前，您應該閱讀有關URL和路徑的快速入門，以複習<a href="/en-US/docs/https://developer.mozilla.org/en-US/Learn/HTML/Introduction_to_HTML/Creating_hyperlinks#A_quick_primer_on_URLs_and_paths">相對路徑和絕對路徑URL</a>。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 搜索引擎還會讀取圖片名稱，並將其納入SEO中。 因此您應該為圖片提供一個描述性的檔名。 例如 dinosaur.jpg 的命名方式會比 img835.png 更好。</p>
</div>
...
<div class="warning">
<p><strong>警告：</strong> 大多數圖像均受版權保護。 請勿在你的網站上隨意顯示圖片，除非：</p>
...
<div class="note">
<p><strong>備註：</strong> &lt;img&gt;和&lt;video&gt;之類的元素有時也稱為替換元素。 這是因為元素的內容和圖片大小是由外部（例如圖片或影音檔）所定義的，而不是由元素的內容定義。</p>
</div>

<div class="note">
<p><strong>備註：</strong> 您可以從在<a href="/zh-TW/docs/https://mdn.github.io/learning-area/html/multimedia-and-embedding/images-in-html/index.html">Github</a>上找到本節完成的示例（參見<a href="/zh-TW/docs/https://github.com/mdn/learning-area/blob/master/html/multimedia-and-embedding/images-in-html/index.html">開源碼</a>。）</p>
</div>
...
<p>測試替代文字最簡單方法是故意拼錯檔名。 例如，如果我們的圖片名稱為dinosooooor.jpg，則瀏覽器將不會顯示該圖片，而是顯示alt文本：<img alt="The Images in HTML title, but this time the dinosaur image is not displayed, and alt text is in its place." src="alt-text.png"></p>
...
<div class="note">
<p><strong>備註：</strong> For more information, see our guide to <a href="/en-US/docs/Learn/Accessibility/HTML#Text_alternatives">Text Alternatives</a>.</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> If you do need to alter an image's size, you should use <a href="/en-US/docs/Learn/CSS">CSS</a> instead.</p>
</div>

<h3 id="圖片標題">圖片標題</h3>

<p>As <a href="/en-US/Learn/HTML/Introduction_to_HTML/Creating_hyperlinks#Adding_supporting_information_with_%3Ctitle%3E">with links</a>, you can also add <code>title</code> attributes to images, to provide further supporting information if needed. In our example, we could do this:</p>
...
<p>{{ EmbedLiveSample('實戰練習：嵌入圖片', 700, 350) }}</p>
...
<p>{{ EmbedLiveSample('實戰練習：建立圖文標註', 700, 350) }}</p>

<h2 id="CSS_背景圖片">CSS 背景圖片</h2>

<p>您還可以使用CSS將圖像嵌入網頁（JavaScript也可以，但這完全是另一回事了）。 CSS<a href="/en-US/docs/Web/CSS/background-image"><code>background-image</code></a>屬性和其他background- *屬性用於控制背景圖片的放置。 例如要將背景圖片放置在頁面的每個段落上，可以執行以下操作：</p>
...
<div class="note">
<p><strong>備註：</strong> 在我們的CSS主題中，您將學到更多關於CSS背景圖片的知識。</p>
</div>
