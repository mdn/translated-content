...
---
<p>這篇指南提供了一系列的文章，幫你掌握HTML表單的基本知識。對於與使用者互動，網站表單是一項十分有力的工具，最常使用於用戶數據蒐集，或控制使用者介面。但由於一些歷史與技術上的因素，並沒有顯著的方法發揮表單的潛力。在下面的指引中，我們將介紹網站表單所有基本面向，包括標記他們的HTML結構、設定控制器樣式、驗證數據及將數距提送至伺服器</p>

<h2 id="參考文章列表">參考文章列表</h2>

<ol>
 <li><a href="/zh-TW/docs/HTML/Forms/My_first_HTML_form">我的第一個HTML表單</a></li>
 <li><a href="/zh-TW/docs/HTML/Forms/How_to_structure_an_HTML_form">如何構建 HTML 表單</a></li>
 <li><a href="/zh-TW/docs/HTML/Forms/The_native_form_widgets">本機表單控件</a></li>
 <li>CSS和HTML表單
  <ol>
   <li><a href="/zh-TW/docs/HTML/Forms/Styling_HTML_forms">造型HTML表單</a></li>
 ...
 <li><a href="/zh-TW/docs/HTML/Forms/Sending_and_retrieving_form_data">發送和檢索表單數據</a></li>
 <li><a href="/zh-TW/docs/HTML/Forms/Data_form_validation">數據表單驗證</a></li>
 <li><a href="/zh-TW/docs/HTML/Forms/How_to_build_custom_form_widgets">如何創建自定義表單控件</a></li>
...
<table>
 ...
 <tbody>
  <tr>
   ...
   <td>該<code>按鈕</code>元素表示一個可點擊的按鈕。</td>
  </tr>
  <tr>
   <td>{{HTMLElement("datalist")}}</td>
   <td>{{domxref("HTMLDataListElement")}}</td>
   <td>該數據列表元素包含了一組  {{ HTMLElement("option") }}  表示對其他表單元素的值可能的選擇要素。</td>
  </tr>
  <tr>
   <td>{{HTMLElement("fieldset")}}</td>
   <td>{{domxref("HTMLFieldSetElement")}}</td>
   <td>該字段集是用來在表單中的組數表單元素。</td>
  </tr>
  <tr>
   <td>{{HTMLElement("form")}}</td>
   <td>{{domxref("HTMLFormElement")}}</td>
   <td>的<code>形式</code>元素表示的文件的一部分，它包含使用戶能夠提交信息給web服務器的交互元件。</td>
  </tr>
  <tr>
   <td>{{HTMLElement("input")}}</td>
   <td>{{domxref("HTMLInputElement")}}</td>
   <td>該  <code>輸入</code>元素用於創建表格的交互式控制。</td>
  </tr>
  <tr>
   <td>{{HTMLElement("keygen")}}</td>
   <td>{{domxref("HTMLKeygenElement")}}</td>
   <td>所述<code>凱基</code>元素存在，以促進生成的密鑰材料，並提交了公開密鑰的作為HTML形式的一部分</td>
  </tr>
  <tr>
   <td>{{HTMLElement("label")}}</td>
   <td>{{domxref("HTMLLabelElement")}}</td>
   <td>該<code>標籤</code>元素代表一個項目在用戶界面的標題</td>
  </tr>
  <tr>
   <td>{{HTMLElement("legend")}}</td>
   <td>{{domxref("HTMLLegendElement")}}</td>
   <td>在<code>傳說</code>元素代表一個標題為其父 {{ HTMLElement("fieldset") }} 的內容。</td>
  </tr>
  <tr>
   <td>{{HTMLElement("meter")}}</td>
   <td>{{domxref("HTMLMeterElement")}}</td>
   <td>所述<code>米</code>元素表示一個已知的範圍內的任一標量值或分數值。</td>
  </tr>
  <tr>
   <td>{{HTMLElement("optgroup")}}</td>
   <td>{{domxref("HTMLOptGroupElement")}}</td>
   <td>在<code>OPTGROUP</code>元素創建一個 {{ HTMLElement("select") }}  元素中的一組選項。</td>
  </tr>
  <tr>
   <td>{{HTMLElement("option")}}</td>
   <td>{{domxref("HTMLOptionElement")}}</td>
   <td>在HTML <code>選項</code>元素用於創建表示  {{ HTMLElement("select") }} ，一個 {{ HTMLElement("optgroup") }} 或 {{ HTMLElement("datalist") }} 元素中的項目的控制。<em> </em><code><em> </em></code></td>
  </tr>
  <tr>
   <td>{{HTMLElement("output")}}</td>
   <td>{{domxref("HTMLOutputElement")}}</td>
   <td>的<code>輸出</code>元素表示一個計算的結果。</td>
  </tr>
  <tr>
   <td>{{HTMLElement("progress")}}</td>
   <td>{{domxref("HTMLProgressElement")}}</td>
   <td>的<code>進展</code>元素用於查看任務的完成進度。</td>
  </tr>
  <tr>
   <td>{{HTMLElement("select")}}</td>
   <td>{{domxref("HTMLSelectElement")}}</td>
   <td>在<code>選擇</code>元素代表呈現一個選項菜單的控制。</td>
  </tr>
  <tr>
   <td>{{HTMLElement("textarea")}}</td>
   <td>{{domxref("HTMLTextAreaElement")}}</td>
   <td>該<code>textarea的</code>元素代表多行純文本編輯控制。</td>
  </tr>
 </tbody>
</table>

<div class="note">
<p><strong>備註：</strong> 所有的表單元素，因為所有的HTML元素，支持 {{domxref("HTMLElement")}} DOM接口。</p>
</div>
...
<ul>
 <li><a href="http://www.w3.org/html/wg/drafts/html/master/forms.html#forms">W3C HTML 5.1規範（表格）</a></li>
 <li><a href="http://www.whatwg.org/specs/web-apps/current-work/multipage/forms.html#forms">WHATWG HTML生活水平（表格）</a></li>
</ul>
