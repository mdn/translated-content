...
<div class="note">
<p><strong>備註：</strong> 這兩件事情並不互斥。我們可以用任務執行器來訪問服務。例如，你可以用 Sauce Labs 的 API 來跑跨瀏覽器測試，並顯示結果。我們會在下面解釋這件事。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 如果因為權限問題而失敗，<a href="https://docs.npmjs.com/getting-started/fixing-npm-permissions">Fixing npm permissions</a> 應該對你有所幫助。</p>
</div>
...
<ol>
 <li>輸入以下指令安裝：
  ...
  <div class="note">
  <p><strong>備註：</strong> <code>--save-dev</code> 會把此套件加到開發相依設定中。如果去看專案的 <code>package.json</code> 檔，你會在 <code>devDependencies</code> 屬性看到它被放在裡面。</p>
  </div>
 </li>
...
<ol>
 <li>輸入以下指令安裝：
  <pre class="brush: bash">npm install --save-dev gulp-babel babel-preset-es2015
npm install jshint gulp-jshint --save-dev
</pre>
 </li>
 <li>在 <code>gulpfile.js</code> 增加這個相依：
  <pre class="brush: js">var babel = require('gulp-babel');
...
<div class="note">
<p><strong>備註：</strong> <code>*</code> 是通配字符（wildcard character）－－這裡的意思是「當任何檔案被儲存的時候，執行這些任務」。你也可以在主要任務內使用通配，例如 <code>gulp.src('src/*.css')</code> 會抓取所有的 CSS 檔案並執行 piped task。</p>
</div>

<div class="note">
<p><strong>備註：</strong> 在我們的 watch 指令有個問題，那就是我們的 CSSLint/Autoprefixer combination throws full-blown errors when a CSS error is encountered, which stops the watch working. You'll have to restart the watch once a CSS error is encountered, or find another way to do this.</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> This is already very useful, and way more convenient than having to set all these emulators and virtual machines by yourself.</p>
</div>
...
<ol>
 ...
 <li>使用以下指令安裝 Node Sauce Labs wrapper：
  <pre class="brush: bash">npm install saucelabs</pre>
 </li>
