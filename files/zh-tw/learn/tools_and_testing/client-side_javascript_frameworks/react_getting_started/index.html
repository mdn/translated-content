...
<p><strong>備註：</strong> 前面程式碼中的括號<code>(</code>、<code>)</code>不是JSX獨有的，這對你的應用程式沒有任何影響。它們會向你（和你的電腦）發出訊號，表示括號其中的多行程式碼是同個表達式的一部分。你也可以像這樣編寫表達式：</p>
...
<div class="notecard note">
<p><strong>備註：</strong> 如果您已有安裝yarn套件管理器，<code>create-react-app</code>默認將使用它而不是使用npm。如果你已安裝yarn和npm兩個套件管理器並且堅持繼續使用npm，當你要執行<code>create-react-app</code>指令時，你可以添加指令<code>--use-npm</code></p>
...
<pre class="brush: plain">moz-todo-react
├── README.md
├── node_modules
├── package.json
├── package-lock.json
...
<div class="notecard note">
<p><strong>備註：</strong> In JSX, React components and HTML elements must have closing slashes. Writing just <code>&lt;App&gt;</code> or just <code>&lt;img&gt;</code> will cause an error.</p>
</div>
