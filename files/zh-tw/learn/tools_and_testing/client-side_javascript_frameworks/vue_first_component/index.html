...
<p><strong>備註：</strong> 如果你需要根據我們的版本來檢查你的程式碼，你可以從 <a href="https://github.com/mdn/todo-vue">todo-vue 存放庫</a>找到 Vue 範例應用程式最終版本的程式碼。有關實際運行的版本，請看 <a href="https://mdn.github.io/todo-vue/dist/">https://mdn.github.io/todo-vue/dist/</a> 。</p>
</div>
...
<p><img alt="目前應用程式的渲染狀態——包含一個標題（ To-Do List ），一個 checkbox 和 label" src="rendered-todoitem.png"></p>
...
<div class="notecard note">
<p><strong>備註：</strong> Prop validation only happens in development mode, so you can't strictly rely on it in production. Additionally, prop validation functions are invoked before the component instance is created, so they do not have access to the component state (or other props).</p>
</div>
...
<pre class="brush: plain">[Vue warn]: Missing required prop: "label"
...
<div class="notecard note">
<p><strong>備註：</strong> Because of the way that <code>this</code> works in arrow functions (binding to the parent’s context), you wouldn’t be able to access any of the necessary attributes from inside <code>data</code> if you used an arrow function. So don’t use an arrow function for the <code>data</code> property.</p>
</div>
...
<pre class="brush: plain">v-bind:attribute="expression"</pre>
...
<div class="notecard note">
<p><strong>備註：</strong> If you prefer yarn, you could instead use <code>yarn add lodash.uniqueid</code>.</p>
</div>
