...
---
<div>{{LearnSidebar}}{{PreviousMenuNext("Learn/JavaScript/Building_blocks/Return_values","Learn/JavaScript/Building_blocks/Image_gallery", "Learn/JavaScript/Building_blocks")}}</div>

<p>每一個 HTML 元素都可以放置事件屬性，以藉此於事件發生時能執行 JavaScript 程式。事件屬性的名稱都有一個前綴「on」，例如當使用者點選元素時要執行指定的 JavaScript，可以使用 <code>onclick</code> 屬性並把要執行的 JavaScript 當成屬性值。</p>
...
<h2 id="Example_using_event_attributes">Example using event attributes</h2>
