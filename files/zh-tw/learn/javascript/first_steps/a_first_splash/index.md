...
<div class="note">
<p><strong>備註：</strong> 你會在 JavaScript 看到許多跟其他程式語言一樣的特徵 — functions , loops 之類的 ，雖然程式語法看起來有差 ，但概念大部分都差不多 .</p>
</div>
...
<p>{{ EmbedLiveSample('範例 — 猜數字遊戲', '100%', 320) }}</p>
...
<div class="note">
<p><strong>備註：</strong> 從<a href="/en-US/docs/user:chrisdavidmills/variables">下一篇文章</a>開始，你會學到更多有關變數的事。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 你會在後續的課程中學習到更多關於函式的事。</p>
</div>
...
<p>{{ EmbedLiveSample('運算子', '100%', 300) }}</p>
...
<p>這裡我們為 <code>guessSubmit</code> 按鈕添加一個事件偵聽器。這是一個函式，它接受兩個輸入值（稱為<em>參數</em>） - 我們正在監聽的事件類型字串（本例中的 <code>click</code>），以及我們想要在事件發生時運行的程式碼（在這種情況下是<code>checkGuess()</code>函式） — 請注意，在編寫 <a href="/zh-TW/docs/Web/API/EventTarget/addEventListener"><code>addEventListener()</code></a> 內部時我們不需要為函式加上括號。</p>
