...
<p>在本單元的最後一篇文章中，我們將介紹陣列——一種在單個變數名下儲存資料項列表的簡潔方法。在這裡，我們看看為什麼這很有用，然後探討如何建立陣列，檢索、增加和刪除儲存在陣列中的項目等等。</p>
...
// shopping 回傳 [ "tahini", "milk", "cheese", "hummus", "noodles" ]</pre>
  ...
  <div class="note"><p><strong>備註：</strong> 前面有說過，但還是提醒下：電腦從 0 開始數！</p></div>
 </li>
...
<div class="note">
<p><strong>備註：</strong> 好的，在技術上它屬於字串的方法，而非陣列的方法。但因為它可以很順利地對陣列進行操作，因此我們把它列在這邊。</p>
</div>
...
<p>{{ EmbedLiveSample('Active learning: Printing those products', '100%', 730) }}</p>
...
<div class="note">
<p><strong>備註：</strong> In a real search app, you'd probably be able to click the previous search terms to return to previous searches, and it would display actual search results! We are just keeping it simple for now.</p>
</div>
...
<p>{{ EmbedLiveSample('Active learning: Top 5 searches', '100%', 700) }}</p>
