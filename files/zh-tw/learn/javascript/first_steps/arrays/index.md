...
你可以使用括號標記法存取個別單位，同時也可以[存取字串中的字母](/zh-TW/docs/Learn/JavaScript/First_steps/Useful_string_methods#retrieving_a_specific_string_character)。
...
4. As part of the above line of code, you'll also want to convert the price from a string to a number. If you can't remember how to do this, check out the [first strings article](/zh-TW/docs/Learn/JavaScript/First_steps/Strings#numbers_versus_strings).
5. There is a variable called `total` that is created and given a value of 0 at the top of the code. Inside the loop (below `// number 4`) we want you to add a line that adds the current item price to that total in each iteration of the loop, so that at the end of the code the correct total is printed onto the invoice. You might need an [assignment operator](/zh-TW/docs/Learn/JavaScript/First_steps/Math#assignment_operators) to do this.
