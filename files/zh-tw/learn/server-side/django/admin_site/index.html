...
<p>現在，我們已經為本地圖書館網站 <a href="/en-US/docs/Learn/Server-side/Django/Tutorial_local_library_website">LocalLibrary</a> 創建了模型，我們接下來使用 Django 管理網站，去添加 一些 “真實的“ 書本數據。首先，我們展示如何用管理網站註冊模型，然後展示如何登錄和創建一些數據。本文最後，我們介紹可以進一步改進管理網站的建議。</p>
...
<div class="note"><p><strong>備註：</strong> 如果你在上一章節最後有接受挑戰並建立一個書本的「語言模型」 (<a href="/zh-TW/docs/Learn/Server-side/Django/Models">查看模型教學文章</a>)，你必需也要導入並註冊該模型！</p></div>
...
<div class="note">
<p><strong>備註：</strong> 在這邊我們希望你花點時間在你的 app 中新增一些書本、作者和書及類型(例如：奇幻等)。請確保每位作者與每種書籍類型都分別關聯了一本以上的書(這在文章稍後的實作的時候，會讓你的列表與細節視圖更加豐富有趣)。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> Getting the <code>genre</code> may not be a good idea here, because of the "cost" of the database operation. We're showing you how because calling functions in your models can be very useful for other reasons — for example to add a <em>Delete </em>link next to every item in the list.</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 更新 <code>BookInstance</code> 模型列表用來顯示狀態和預期的返回日期是有價值的。 我們在本文結尾處添加了一個挑戰！</p>
</div>

<h3 id="加入列表過濾器_List_Filter">加入列表過濾器 (List Filter)</h3>

<p>當你的列表有很多個記錄時, 加入列表過濾器可以幫助你過濾想顯示的記錄。加入<code>list_filter</code>這個屬性就可以。請用以下的程式碼來取代原本的 <code>BookInstanceAdmin</code> 類別</p>
...
<div class="note">
<p><strong>備註：</strong> <em>LocalLibrary</em> 模型相對簡單，因此我們無須更改佈局。 但我們仍然會進行一些更改，向您展示如何進行。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 您還可以使用 <code>exclude</code> 屬性來聲明要從表單中排除的屬性列表（將顯示模型中的所有其他屬性）。</p>
</div>
