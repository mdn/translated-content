...
<ul>
 <li><code>max_length=20</code> — 表示此字段中值的最大長度為20個字符的狀態。</li>
 <li><code>help_text="Enter field documentation"</code> — 提供一個幫助用戶的文本標籤，讓用戶知道當前透過HTML表單輸入時要提供什麼值。</li>
</ul>
...
<ul>
 <li><a href="https://docs.djangoproject.com/en/1.10/ref/models/fields/#help-text">help_text</a> :提供HTML表單文本標籤(eg i在管理站點中),如上所述。</li>
 <li><a href="https://docs.djangoproject.com/en/1.10/ref/models/fields/#verbose-name">verbose_name</a> :字段標籤中的可讀性名稱，如果沒有被指定，Django將從字段名稱推斷默認的詳細名稱。</li>
 <li><a href="https://docs.djangoproject.com/en/1.10/ref/models/fields/#default">default</a> :該字段的默認值。這可以是值或可呼叫物件(callable object)，在這種情況下，每次創建新紀錄時都將呼叫該物件。</li>
 <li><a href="https://docs.djangoproject.com/en/1.10/ref/models/fields/#null">null</a>：如為 <code>True</code>，即允許 Django 於資料庫該欄位寫入 <code>NULL</code>（但欄位型態如為 <code>CharField</code> 則會寫入空字串）。預設值是 <code>False</code>。</li>
 <li><a href="https://docs.djangoproject.com/en/1.10/ref/models/fields/#blank">blank</a> :如果<code><strong>True</strong></code>，表單中的字段被允許為空白。默認是<code>False</code>，這意味著Django的表單驗證將強制你輸入一個值。這通常搭配 <code>NULL=True</code> 使用，因為如果要允許空值，你還希望數據庫能夠適當地表示它們。</li>
 <li><a href="https://docs.djangoproject.com/en/1.10/ref/models/fields/#choices">choices</a> :這是給此字段的一組選項。如果提供這一項，預設對應的表單部件是「該組選項的列表」，而不是原先的標准文本字段。</li>
 <li><a href="https://docs.djangoproject.com/en/1.10/ref/models/fields/#primary-key">primary_key</a> :如果是True，將當前字段設置為模型的主鍵（主鍵是被指定用來唯一辨識所有不同表記錄的特殊數據庫欄位(column)）。如果沒有指定字段作為主鍵，則Django將自動為此添加一個字段。</li>
</ul>
...
<ul>
 <li><a href="https://docs.djangoproject.com/en/1.10/ref/models/fields/#django.db.models.CharField">CharField</a> 是用來定義短到中等長度的字段字符串。你必須指定<code>max_length</code>要存儲的數據。</li>
 <li><a href="https://docs.djangoproject.com/en/1.10/ref/models/fields/#django.db.models.TextField">TextField </a>用於大型任意長度的字符串。你可以<code>max_length</code>為該字段指定一個字段，但僅當該字段以表單顯示時才會使用（不會在數據庫級別強制執行）。</li>
 <li><a href="https://docs.djangoproject.com/en/1.10/ref/models/fields/#django.db.models.IntegerField">IntegerField</a> 是一個用於存儲整數（整數）值的字段，用於在表單中驗證輸入的值為整數。</li>
 ...
 <li><a href="https://docs.djangoproject.com/en/1.10/ref/models/fields/#emailfield">EmailField</a> 用於存儲和驗證電子郵件地址。</li>
 <li><a href="https://docs.djangoproject.com/en/1.10/ref/models/fields/#filefield">FileField</a> 和<a href="https://docs.djangoproject.com/en/1.10/ref/models/fields/#imagefield">ImageField</a> 分別用於上傳文件和圖像（<code>ImageField</code> 只需添加上傳的文件是圖像的附加驗證）。這些參數用於定義上傳文件的存儲方式和位置。</li>
 <li><a href="https://docs.djangoproject.com/en/1.10/ref/models/fields/#autofield">AutoField</a> 是一種 <strong>IntegerField </strong>自動遞增的特殊類型。如果你沒有明確指定一個主鍵，則此類型的主鍵將自動添加到模型中。</li>
 <li><a href="https://docs.djangoproject.com/en/1.10/ref/models/fields/#foreignkey">ForeignKey</a> 用於指定與另一個數據庫模型的一對多關係（例如，汽車有一個製造商，但製造商可以製作許多汽車）。關係的“一”側是包含密鑰的模型。</li>
...
<div class="note">
<p><strong>備註：</strong> 假設你將使用URL<code>/myapplication/mymodelname/2</code> 來顯示模型的單個記錄（其中“2”是id特定記錄），則需要創建一個URL映射器來將響應和id傳遞給“模型詳細視圖” （這將做出顯示記錄所需的工作）。以上示例中，<code>reverse()</code>函數可以“反轉”你的url映射器（在上訴命名為“model-detail-view”的案例中，以創建正確格式的URL。</p>
...
<div class="note">
<p><strong>備註：</strong> 要用"抽象的"模型還有欄位說明怎麼搜尋紀錄可能會有點令人困惑。我們會以一個Book模型，其包含<code>title</code>與<code>genre</code>字段，而genre 也是一個僅有<code>name</code>一個字段的模型。</p>
</div>
