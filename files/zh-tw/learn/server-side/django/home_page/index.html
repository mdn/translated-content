...
<p>我們現在可以添加代碼，來顯示我們的第一個完整頁面 - <a href="/en-US/docs/Learn/Server-side/Django/Tutorial_local_library_website">LocalLibrary</a> 網站的主頁，顯示每個模型類型有多少條記錄，並提供我們其他頁面的側邊欄導航鏈接。一路上，我們將獲得編寫基本URL 地圖和視圖、從數據庫獲取記錄、以及使用模板的實踐經驗。</p>
...
<div class="note">
<p><strong>備註：</strong> Django允許您以自己喜歡的任何方式來構造URL-您可以如上所示在URL主體中編碼信息或使用URL <code>GET</code> 參數（例如<code>/book/?id=6</code>）。 無論使用哪種方法，都應保持URL的整潔，邏輯和可讀性(<a href="https://www.w3.org/Provider/Style/URI">在此處查看W3C建議</a>).</p>
...
<div class="note">
<p><strong>備註：</strong> 值得在本節中多加註意。大多數材料是所有頁面共有的。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 每當Django遇到導入函數 <code><a href="https://docs.djangoproject.com/en/2.1/ref/urls/#django.urls.include">django.urls.include()</a></code>時，它都會在指定的結束字符處分割URL字符串，並將剩餘的子字符串發送到所包含的<em>URLconf</em> 模塊以進行進一步處理。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 我們可以對上面的鏈接進行硬編碼 (例如<code>&lt;a href="<strong>/catalog/</strong>"&gt;Home&lt;/a&gt;</code>), 但是如果我們更改主頁的模式 (例如更改為 <code>/catalog/index</code>) 則模板將不再 正確鏈接。 使用反向URL映射更加靈活和健壯！</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> Django將根據項目的設置文件在許多位置查找模板（搜索已安裝的應用程序是默認設置！）。 您可以在 <a href="https://docs.djangoproject.com/en/2.0/topics/templates/">Templates</a> （Django docs）中找到有關Django如何查找模板及其支持的模板格式的更多信息。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 模板<em>tags</em> 類似於可以在模板中使用的功能，可以在模板中循環使用列表，基於變量的值執行條件操作等。除了模板標記之外，模板語法還允許您引用模板變量（傳遞給 模板），並使用<em>template filters</em>，該過濾器可重新格式化變量（例如，將字符串設置為小寫）。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 我們還引入了兩個附加的模板標籤：<code>url</code> 和<code>load static</code>。 這些將在以下各節中討論。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 因為變量具有雙括號 (<code>\{{ num_books }}</code>)，而標籤則用百分號括在單括號中擴展為 (<code>{% extends "base_generic.html" %}</code>)，所以您可以輕鬆識別是要處理模板變量還是模板標籤（函數）。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 上面的更改指定了文件的位置，但是Django默認不提供文件。創建網站框架時 (<a href="/en-US/docs/Learn/Server-side/Django/skeleton_website">created the website skeleton</a>)，雖然我們在全局URL映射器(<strong>/locallibrary/locallibrary/urls.py</strong>)中啟用了由開發Web服務器提供的服務，但您仍需要安排它們在生產中提供。 我們待會再看。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 您將無法使用<strong>All books</strong>和<strong>All authors</strong>鏈接，因為尚未定義這些頁面的路徑，視圖和模板（當前我們僅在<code>base_generic.html</code> html模板中插入了這些鏈接的佔位符）。</p>
</div>
...
<ol>
 <li>LocalLibrary <a href="#The_LocalLibrary_base_template">base template</a> 已定義<code>title</code> 欄。 在 <a href="#The_index_template">index template</a>中覆蓋此塊並為頁面創建一些新標題。
  ...
  <div class="note">
  <p><strong>備註：</strong> <a href="#Extending_templates">Extending templates</a> 部分介紹瞭如何創建塊並將其擴展到另一個模板中。<br>
    </p>
  </div>
 </li>
