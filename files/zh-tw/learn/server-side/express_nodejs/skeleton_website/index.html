...
<div class="note">
<p><strong>備註：</strong> Express Application Generator並非 Express 應用程序的唯一生成器，生成的項目不是構建文件和目錄的唯一可行方式。然而，生成的網站具有易於擴展和理解的模塊化結構。有關最小 Express 應用程序的信息，請參閱 <a href="https://expressjs.com/en/starter/hello-world.html">Hello world </a>示例（Express docs）。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 選擇模板引擎的其他選項（例如 <code>--hogan</code>, <code>--ejs</code>, <code>--hbs</code>等）已被棄用。請用 <code>--view</code> (或 <code>-v</code>)!</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 如果要使用生成器不支持的模板引擎，請參閱<a href="https://expressjs.com/en/guide/using-template-engines.html">在Express中使用模板引擎（Express文檔）</a>，並參閱目標視圖引擎的文檔。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 互聯網上有許多資源，可幫助您比較不同的視圖/模板引擎選擇！</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> CSS有一些限制，使某些任務變得困難。 CSS 樣式表引擎允許您使用更強大的語法來定義您的 CSS，然後將定義編譯為純粹的舊式 CSS，以供瀏覽器使用。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 您也可以使用 <code>npm start</code>命令啟動應用程序。如下圖所示，指定 DEBUG 變量可啟用控制台日誌記錄/調試。例如，當你訪問上面的頁面時，你會看到像這樣的調試輸出：</p>
...
<div class="note">
<p><strong>備註：</strong> 現在，如果您編輯項目中的任何文件，服務器將重新啟動（或者您可以隨時在命令提示符下，鍵入<code>rs</code>來重新啟動它）。您仍需要重新加載瀏覽器，以刷新頁面。</p>
...
<div class="note">
<p><strong>備註：</strong> <code>require()</code> 是一個全局 node 函數，用於將模塊導入當前文件。這裡我們使用相對路徑指定 <strong>app.js</strong> 模塊，並省略可選的（<strong>.js</strong>）文件擴展名。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 此時我們剛剛導入了模塊；我們還沒有真正使用過它的路由（在文件的更下方一點將使用到路由）。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 上面指定的路徑 ('/' and '<code>/users</code>')，被視為定義在導入文件中的路由前綴。因此，例如，如果導入的用戶模塊 <strong>users</strong>為<code>/profile</code>定義了路由，則可以在 <code>/users/profile</code>中訪問該路由。我們將在後面的文章中，詳細討論路由。</p>
</div>
...
<div class="note">
<p><strong>備註：</strong> 嘗試運行帶有 node 的服務器，並在瀏覽器中訪問以下 URL: <a href="http://localhost:3000/users/">http://localhost:3000/users/</a>。您應該看到一條消息：'respond with a resource'。</p>
</div>
