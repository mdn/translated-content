...
Firefox 33 は米国時間 2014 年 10 月 14 日にリリースされました。この記事では、ウェブ開発者だけでなく、Firefox および Gecko の開発者、アドオン開発者にも有益な主要な変更点を掲載しています。
...
- {{cssxref("@counter-style")}} ルールを実装しました ([Firefox バグ 966166](https://bugzil.la/966166))。
...
- 内容がないキーフレームルールを持つ CSS アニメーションでイベントが発生するように修正しました ([Firefox バグ 1004377](https://bugzil.la/1004377))。
...
- 標準外の `Number.toInteger()` メソッドを削除しました ([Firefox バグ 1022396](https://bugzil.la/1022396))。
...
- [デフォルト引数](/ja/docs/Web/JavaScript/Reference/Functions/Default_parameters)は関数の本体にある関数宣言より先に評価されますので、これらの関数はデフォルト引数から参照できなくなりました ([Firefox バグ 1022962](https://bugzil.la/1022962))。
...
- 標準外の `hasOwn` トラップを削除しました ([Firefox バグ 980565](https://bugzil.la/980565))。
...
- 標準外の (ただし他のブラウザーで実装されている) `DOMException.stack` プロパティを追加しました。これはスタックを可読性の高い書式 (既存の標準外プロパティ {{jsxref("Error.stack")}} と同じ書式) で表した文字列を返します ([Firefox バグ 857648](https://bugzil.la/857648))。
...
- 標準外の `XMLHttpRequest.mozBackgroundRequest()` はウェブサイトからアクセス不可になりました。Firefox 内部のコード (Chrome コード) のみ使用できます ([Firefox バグ 1035242](https://bugzil.la/1035242))。
...
- IndexedDB の `FileHandle` インターフェイスを `IDBMutableFile` にリネームしました ([Firefox バグ 1006485](https://bugzil.la/1006485))。
- IndexedDB の `LockedFile` インターフェイスを `IDBFileHandle` にリネームしました ([Firefox バグ 1006485](https://bugzil.la/1006485))。
- {{domxref("ServiceWorker")}} インターフェイスを実装しました。設定項目 `dom.serviceWorkers.enabled` で制御されます ([Firefox バグ 903441](https://bugzil.la/903441) で総括しています)。
...
- [CSP](/ja/docs/Web/HTTP/Guides/CSP) 1.1 の `frame-ancestors` [ディレクティブ](/ja/docs/Web/HTTP/Reference/Headers/Content-Security-Policy)に対応しました ([Firefox bug 846978](https://bugzil.la/846978))。

## アドオン開発者と Mozilla 開発者向けの変更点

- JavaScript Debugger Service (JSD) を削除しました。新たな [Debugger API](https://firefox-source-docs.mozilla.org/devtools-user/debugger-api/index.html) が好ましいためです ([Firefox バグ 800200](https://bugzil.la/800200))。
...
- `Panel` コンストラクターの新たなオプションにより、パネルのコンテキストメニューをサポートしました。
- `tab.readyState` をサポートしました。
- `BrowserWindow` 引数を `sidebar.show()` および `sidebar.hide()` に追加しました。サイドバーを表示または非表示にするウィンドウを制御するための引数です。
