...
- 接頭辞がない標準仕様の {{Cssxref("hyphens")}} をサポートしました ([Firefox バグ 953408](https://bugzil.la/953408))。
- {{cssxref("font")}} 一括指定プロパティを、{{cssxref("font-stretch")}} の値を受け入れるように更新しました ([Firefox バグ 1057680](https://bugzil.la/1057680))。
- 最新の仕様の更新に合致するよう、{{cssxref(":fullscreen")}} 擬似クラスはフルスクリーン時に、トップレベル要素だけでなく要素の集合全体を選択するようになりました ([Firefox バグ 1199522](https://bugzil.la/1199522))。
- {{cssxref("writing-mode")}} において非推奨な SVG 向けの値である `lr`、`lr-tb`、`rl`、`tb`、`tb-rl` を、標準プロパティのエイリアスとして CSS に追加しました ([Firefox バグ 1205787](https://bugzil.la/1205787))。

### HTML

- 複数のフレームを持つ ICO 画像を指定した {{htmlelement("img")}} 要素で、画像固有の寸法は最小フレームではなく最大フレームの寸法を設定するようになりました ([Firefox バグ 1201796](https://bugzil.la/1201796))。
- ドキュメントのビューポートの値 (`<meta name="viewport>` で定義) を、JavaScript で動的に変更できるようになりました ([Firefox バグ 976616](https://bugzil.la/976616))。

### JavaScript

#### 新規 API

- ES2016 の新規メソッドである {{jsxref("Array.prototype.includes()")}} および {{jsxref("TypedArray.prototype.includes()")}} を、デフォルトで有効にしました ([Firefox バグ 1070767](https://bugzil.la/1070767))。
...
- ロケールを明示したインデックスの作成を可能にする、[Locale-aware Sorting](/ja/docs/Web/API/IndexedDB_API/Using_IndexedDB#locale-aware_sorting) と呼ばれる新機能を追加しました。ロケールの規則に従ってデータをソートするために使用できます ([Firefox バグ 871846](https://bugzil.la/871846))。これは、非標準の Firefox 独自機能です。

#### サービスワーカー

- 仕様に従い、{{domxref("ExtendableEvent.waitUntil()")}} が {{domxref("ExtendableEvent")}} ハンドラーの外部で呼び出された場合に、Firefox は `InvalidStateError` を発生するようになりました。加えて、複数回呼び出した {{domxref("ExtendableEvent.waitUntil","waitUntil()")}} は積み上げられ、結果的に promise は [Extend Lifetime Promises](https://slightlyoff.github.io/ServiceWorker/spec/service_worker/index.html#dfn-extend-lifetime-promises) のリストに追加されます ([Firefox バグ 1180274](https://bugzil.la/1180274))。
- {{domxref("PushMessageData")}} メソッドを実装しました ([Firefox バグ 1149195](https://bugzil.la/1149195))。

#### WebRTC

- {{domxref("HTMLCanvasElement.captureStream()")}} メソッドを、デフォルトで有効にしました ([Firefox バグ 1177276](https://bugzil.la/1177276))。
...
- Linux 環境で、Windows 環境と同様に {{domxref("Event.timeStamp")}} が {{domxref("DOMHighResTimeStamp")}} を返すようになりました ([Firefox バグ 1026803](https://bugzil.la/1026803))。
