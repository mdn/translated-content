...
- Mac での HTML コンテンツの既定の [`accesskey`](/ja/docs/Web/HTML/Reference/Global_attributes) が Control + Option キーに変更されました。これは Mac での Webkit ベースブラウザーと同じです。
...
- `MozBlobBuilder` インターフェイスが {{domxref("Blob")}} コンストラクターの利用を促すために非推奨になりました。 `MozBlobBuilder` を利用した場合、ウェブコンソールに警告が表示されます。
...
- 新しく [`getLineStart()`](https://web.archive.org/web/20210620193439/https://developer.mozilla.org/en-US/docs/Mozilla/JavaScript_code_modules/source-editor.jsm#getLineStart%28%29) および [`getLineEnd()`](https://web.archive.org/web/20210620193439/https://developer.mozilla.org/en-US/docs/Mozilla/JavaScript_code_modules/source-editor.jsm#getLineEnd%28%29) メソッドが追加されました。
...
- `nsILocalFile` インターフェイスは `nsIFile` にマージされました ([bug 682360](https://bugzil.la/682360))。
...
- 辞書の名前が完全な {{glossary("BCP 47 language tag","BCP 47 言語タグ")}}で解釈されるようになりました ([bug 730209](https://bugzil.la/730209), [bug 741842](https://bugzil.la/741842))。開発者は辞書名での言語名をハードコーディングしないようにしてください。
