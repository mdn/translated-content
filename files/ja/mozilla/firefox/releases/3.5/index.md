...
- [HTML での音声と動画](/ja/docs/Learn_web_development/Core/Structuring_content/HTML_video_and_audio)
  - : Firefox 3.5 では HTML 5 の [`audio`](/ja/docs/Web/HTML/Reference/Elements/audio) および [`video`](/ja/docs/Web/HTML/Reference/Elements/video) 要素の対応が追加されます。
- Firefox でのオフラインリソース
...
- 長さの単位 `ch`
  - : 単位 `ch` が通常の[長さ](/ja/docs/Web/CSS/Reference/Values/length)として任意の場所で使えるようになりました。`1ch` は文字 "0" の横幅です。
- {{ cssxref("opacity") }}
...
- [CSS 座標変換の使用](/ja/docs/Web/CSS/Guides/Transforms/Using)
  - : Firefox 3.5 では CSS 座標変換に対応しています。詳細は {{ cssxref("-moz-transform") }} および {{ cssxref("-moz-transform-origin") }} を参照してください。
- {{ cssxref(":nth-child") }}, {{ cssxref(":nth-last-child") }}, {{ cssxref(":nth-of-type") }}, {{ cssxref(":nth-last-of-type") }}, {{ cssxref(":first-of-type") }}, {{ cssxref(":last-of-type") }}, {{ cssxref(":only-of-type") }}
  - : これらのセレクターがすべて Firefox 3.5 で新たに対応されます。

#### 新しい DOM の機能

- [localStorage](/ja/docs/Web/API/Window/localStorage)
  - : Firefox 3.5 では Web Storage の `localStorage` プロパティに対応します。これはウェブアプリケーションがクライアントのコンピューター上にローカルにデータを保存する方法を提供します。
- [ウェブワーカーの使用](/ja/docs/Web/API/Web_Workers_API/Using_web_workers)
  - : Firefox 3.5 ではウェブアプリケーションでの簡単なマルチスレッド対応を可能にするウェブワーカーに対応しています。
- [位置情報の使用](/ja/docs/Web/API/Geolocation_API)
  - : Firefox 3.5 では位置情報 API に対応しています。これによりウェブアプリケーションはユーザーの現在位置についての情報を提供するプロバイダーがインストールされ有効化されていれば、その情報を保持することができます。
- [セレクターを使用した DOM 要素の指定](/ja/docs/Web/API/Document_Object_Model/Selection_and_traversal_on_the_DOM_tree)
  - : セレクター API により与えられた選択ルールに一致する要素を指定するために文書を検索できます。
- [マウスジェスチャーイベント](https://web.archive.org/web/20210724051233/https://developer.mozilla.org/en-US/docs/Web/Events/Mouse_gesture_events)
...
- [JavaScript 1.8.1 の新機能](https://web.archive.org/web/20200805002842/https://developer.mozilla.org/en-US/docs/Archive/Web/JavaScript/New_in_JavaScript/1.8.1)
...
- ネイティブ JSON の使用
  - : Firefox 3.5 は JSON をネイティブで対応します。
- String オブジェクトの新しい trim メソッド
  - : [`String`](/ja/docs/Web/JavaScript/Reference/Global_Objects/String) オブジェクトに [`trim()`](/ja/docs/Web/JavaScript/Reference/Global_Objects/String/trim)、[`trimLeft()`](/ja/docs/Web/JavaScript/Reference/Global_Objects/String/trimStart)、[`trimRight()`](/ja/docs/Web/JavaScript/Reference/Global_Objects/String/trimEnd) メソッドが定義されました。

#### ネットワーク機能

- HTTP 向けのクロスサイトアクセス制御
  - : Firefox 3.5 では、サーバーが対応している場合に、[`XMLHttpRequest`](/ja/docs/Web/API/XMLHttpRequest) によるものも含む HTTP リクエストでドメインを越える動作が可能になりました。
- [`XMLHttpRequest` のための Progress イベント](/ja/docs/Web/API/XMLHttpRequest_API/Using_XMLHttpRequest#進捗の監視)
  - : Progress イベントが拡張がリクエストの進捗を監視できるようにするために提供されるようになりました。
- 同期 `XMLHttpRequest`サポートの改善
  - : [DOM Timeout](https://bugzil.la/340345) と [Input Events](https://bugzil.la/333198) が同期 `XMLHttpRequest` 中に抑制されるようになりました。
- DNS 先読みの制御
  - : Firefox 3.5 では DNS 先読みが提供され、それにより現在のページに含まれるリンクのドメイン名解決が事前に行われ、リンクが実際にクリックされたときの時間を節約しま す。この記事ではウェブサイト側で先読みを無効にする、もしくは、先読みの動作を調整する方法について説明しています。

#### 新しい Canvas の機能

- [`canvas` 要素向けの HTML 5 テキスト API](/ja/docs/Web/API/Canvas_API/Tutorial/Drawing_text)
  - : Canvas 要素が HTML5 テキスト API をサポートするようになりました。
- [`canvas` での影付き効果](/ja/docs/Web/API/Canvas_API/Tutorial/Applying_styles_and_colors#shadows)
...
- [`createImageData()`](/ja/docs/Web/API/Canvas_API/Tutorial/Pixel_manipulation_with_canvas#creating_an_imagedata_object)
  - : キャンバスの `createImageData()` メソッドがサポートされるようになりました。このメソッドを利用することで、`ImageData` オブジェクトを必要なときに自動的に作成させるのではなく、コードで明示的に作成することができます。オブジェクトを作成する必要性を無くすことができるので、このメソッドで他の `ImageData` を扱うメソッドのパフォーマンスを改善することができます。
- `moz-opaque` 属性
  - : [`moz-opaque`](/ja/docs/Web/HTML/Reference/Elements/canvas#moz-opaque) DOM 属性が追加されたことにより、キャンバスに半透明な要素があるかどうか知ることができます。キャンバスが半透明な要素がないことを知った場合、描画パフォーマンスが最適化されます。
...
- [Firefox での ICC カラー補正](/ja/docs/Mozilla/Firefox/Releases/3.5/ICC_color_correction_in_Firefox)
  - : Firefox 3.5 では タグ付けられた画像に対して ICC カラー補正が対応されています。
- `defer` 属性の [`script`](/ja/docs/Web/HTML/Reference/Elements/script) 要素での対応
  - : この属性はスクリプトが実行し終わるの待たずにブラウザーにパースし描画し続けることを選択させます。

### その他の改善

- Text ノードの [`wholeText`](/ja/docs/Web/API/Text/wholeText) プロパティ と `Text.replaceWholeText()` メソッドが実装されました。
- [`element.children`](/ja/docs/Web/API/Element/children) プロパティが追加されました。これは与えられた要素の子要素のコレクションを返します。
- {{domxref("HTMLElement.contentEditable")}} プロパティに対応するようになり、編集可能な要素に対応するようになりました。
- DOM の [Element](/ja/docs/Web/API/Element) オブジェクトで Element Traversal API がサポートされました。
- HTML の document ノードを [`cloneNode()`](/ja/docs/Web/API/Node/cloneNode) を用いて複製できるようになりました。
...
- クローム以外のスコープにおいて、例外で捕捉されるオブジェクトが、スローされたオブジェクトを含む [`XPConnect`](https://web.archive.org/web/20210423135742/https://developer.mozilla.org/en-US/docs/Mozilla/Tech/XPCOM/Language_bindings/XPConnect) ラッパーではなく実際にスローされたオブジェクトになりました。
...
- JavaScript においてオブジェクトおよび配列の初期化子によるプロパティの暗黙的な設定ではセッターの定義を行わないようになりました。
...
- [プライベートブラウジングモードのサポート](https://web.archive.org/web/20210620014429/https://developer.mozilla.org/en-US/docs/Archive/Mozilla/Supporting_private_browsing_mode)
  - : Firefox 3.5 ではプライベートブラウジングモードが提供されます。これはユーザーの活動を記録しません。拡張はこの記事で挙げるガイドラインに従ってプライベートブラウジングをサポートすることができます。
- [Firefox 3.5 でのセキュリティの変更](/ja/docs/Mozilla/Firefox/Releases/3.5/Security_changes)
  - : この記事は Firefox 3.5 でのセキュリティ関連の変更をカバーしています。
- [Firefox 3.5 でのテーマの変更](https://web.archive.org/web/20191004004454/https://developer.mozilla.org/en-US/docs/Archive/Themes/Theme_changes_in_Firefox_3.5)
  - : この記事は Firefox 3.5 でのテーマ関連の変更をカバーしています。
- Wi-Fi アクセスポイントのモニタリング
  - : UniversalXPConnect 特権を持つコードで有効なアクセスポイントの一覧がモニタリング可能になり、個々の SSIDs、MAC アドレス、シグナル強度の情報が取得できます。 これを Geolocation と連携して用いることで Wi-Fi ベースのロケーションサービスを提供できます。

#### 注目すべき変更と改善

- XUL の `textbox` ウィジェットが検索フィールドとして利用するための `search` 型を提供するようになりました。
...
- ストレージ API が非同期リクエストのサポートを提供するようになりました。
- `nsICookie2` インターフェイスに新しく `creationTime` 属性が追加され、Cookie が作成された時間を取得できるようになりました。
- プロトコルが登録することを許可されることを保証するためにクローム登録の間にチェックされる `nsIProtocolHandler` へのフラグが追加されました (`URI_IS_LOCAL_RESOURCE`)。
