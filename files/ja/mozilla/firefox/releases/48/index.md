...
[Firefox の最新の開発者向け機能を試すには、Firefox Developer Edition をインストールしてください。](https://www.firefox.com/ja/channel/desktop/developer/) Firefox 48 は、米国時間 2016 年 8 月 2 日にリリースされました。このページでは、開発者に影響する Firefox 48 の変更点をまとめています。
...
- コンソールの出力を消去するための {{domxref("console/clear_static", "console.clear()")}} を実装しました ([Firefox バグ 659625](https://bugzil.la/659625))。
...
- {{HTMLElement("details")}} および {{HTMLElement("summary")}} 要素を、Nightly および Aurora (DevTools) でデフォルトで有効にしました。Beta および Release では無効です。
  ...
  - `toggle` イベントが、{{HTMLElement("details")}} 要素を開いたり閉じたりしたときに送られるようになりました ([Firefox バグ 1225412](https://bugzil.la/1225412))。

- [`meta`](/ja/docs/Web/HTML/Reference/Elements/meta) 属性で、値 `no-referrer-when-downgrade` および `origin-when-cross-origin` に対応しました ([Firefox バグ 1178337](https://bugzil.la/1178337))。

### CSS

- 仕様書に準拠するよう、{{cssxref("calc", "calc()")}} を改良しました。
  - {{cssxref("line-height")}} プロパティで {{cssxref("calc", "calc()")}} に対応しました ([Firefox バグ 594933](https://bugzil.la/594933))。
  - CSS の {{cssxref("calc", "calc()")}} の入れ子に対応しました ([Firefox バグ 968761](https://bugzil.la/968761))。
...
- CSS グリッドの実験的な実装を更新しました。
  ...
  - {{HTMLElement("fieldset")}} でグリッドおよびフレックスボックスレイアウトに対応しました ([Firefox バグ 1230207](https://bugzil.la/1230207))。

- {{cssxref("mask-mode")}} で値 `luminance` に対応しました。また、仕様書に合わせて値 `auto` を `match-source` に改名しました ([Firefox バグ 1228354](https://bugzil.la/1228354))。
- CSS Animation および CSS Transition で、 {{cssxref("clip-path")}} の基本シェイプの補間処理に対応しました ([Firefox バグ 1110460](https://bugzil.la/1110460))。
- {{cssxref("text-combine-upright")}} プロパティの値 `all` で縦中横 (_horizontal-in-vertical_) テキストに対応しました ([Firefox バグ 1097499](https://bugzil.la/1097499)).
- 画像や背景の印刷をページから許可できるようにするための {{cssxref("print-color-adjust", "color-adjust")}} プロパティに実験的に対応しました ([Firefox バグ 1209273](https://bugzil.la/1209273))。
...
- いくつかの `-webkit` 接頭辞付きプロパティおよび値を、ウェブの互換性のためにサポートしました。設定項目 `layout.css.prefixes.webkit` で制御しており、既定値は `false` です:
  ...
  - `-webkit-box-flex`、`-webkit-box-ordinal-group`、`-webkit-box-align`、`-webkit-box-pack` プロパティおよび {{cssxref("display")}} プロパティの値 `-webkit-box` を、最新の [CSS フレックスボックス](/ja/docs/Web/CSS/Guides/Flexible_box_layout/Use_cases) の別名として追加しました ([Firefox バグ 1208635](https://bugzil.la/1208635))。
...
- {{cssxref("position")}} プロパティの値 `absolute` を、最上位レイヤーの要素で対応しました ([Firefox バグ 1236828](https://bugzil.la/1236828))。
- 設定を検出するため、{{cssxref("@supports")}} の内部利用限定の構文に対応しました ([Firefox バグ 1259889](https://bugzil.la/1259889))。
...
- ES2015 の {{jsxref("Symbol.unscopables")}} および [`Array.prototype[Symbol.unscopables]`](/ja/docs/Web/JavaScript/Reference/Global_Objects/Array/Symbol.unscopables) プロパティを実装しました ([Firefox バグ 1054759](https://bugzil.la/1054759) および [Firefox バグ 1258163](https://bugzil.la/1258163))。
- ES2015 の {{jsxref("Symbol.isConcatSpreadable")}} シンボルを実装しました ([Firefox バグ 1041586](https://bugzil.la/1041586))。
- ES2015 の [`Array[Symbol.species]`](/ja/docs/Web/JavaScript/Reference/Global_Objects/Array/Symbol.species) ゲッターを実装しました ([Firefox バグ 1165052](https://bugzil.la/1165052))。
- ES2015 の [`ArrayBuffer[Symbol.species]`](/ja/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer/Symbol.species) getter および [`TypedArray[Symbol.species]`](/ja/docs/Web/JavaScript/Reference/Global_Objects/TypedArray/Symbol.species) getter を実装しました ([Firefox バグ 1165053](https://bugzil.la/1165053))。
...
- 非推奨の [古い Proxy API](/ja/docs/Web/JavaScript/Reference/Deprecated_and_obsolete_features#proxy) (`Proxy.create` および `Proxy.createFunction()`) を廃止しました。代わりに標準の {{jsxref("Proxy")}} オブジェクトを使用してください ([Firefox バグ 892903](https://bugzil.la/892903))。
...
- `NavigatorConcurrentHardware` {{Glossary("mixin", "ミックスイン")}}を実装しました。これは、{{domxref("Navigator.hardwareConcurrency")}} プロパティを {{domxref("Navigator")}} インターフェイスに 追加します。このプロパティにより、{{domxref("Worker")}} を実行できるプロセッシングコアがいくつあるか (少なくとも概算値) をウェブサイトやアプリが取得できます ([Firefox バグ 1008453](https://bugzil.la/1008453))。
...
- [Web Crypto API](/ja/docs/Web/API/Web_Crypto_API) が[ウェブワーカー](/ja/docs/Web/API/Web_Workers_API)で使用可能になりました ([Firefox バグ 842818](https://bugzil.la/842818))。
- {{domxref("CustomEvent")}} インターフェイスが[ウェブワーカー](/ja/docs/Web/API/Web_Workers_API)で使用可能になりました ([Firefox バグ 1003432](https://bugzil.la/1003432))。
- `DOMApplicationsManager.getNotInstalled()` メソッドを削除しました ([Firefox バグ 1255036](https://bugzil.la/1255036))。
- 誤ってウェブに公開されていた Firefox OS の API を、本来すべきであったとおりに隠蔽しました。対象は `mozContact`、`MozContactChangeEvent`、`navigator.mozContacts`、`MozPowerManager`、`MozSettingsEvent` です ([Firefox バグ 1043562](https://bugzil.la/1043562)、[Firefox バグ 1256414](https://bugzil.la/1256414)、[Firefox バグ 1256046](https://bugzil.la/1256046))。
...
- キャッシュの動作を制御できるようにするため、{{domxref("Request.cache")}} プロパティを追加しました ([Firefox バグ 1120715](https://bugzil.la/1120715))。
...
- {{HTTPHeader("Upgrade-Insecure-Requests")}} ヘッダーに対応しました ([Firefox バグ 1243586](https://bugzil.la/1243586))。
- CSP ディレクティブ {{CSP("block-all-mixed-content")}} を実装しました ([Firefox バグ 1122236](https://bugzil.la/1122236))。

## アドオン開発者と Mozilla 開発者向けの変更点

- Social Worker API を削除しました。
- `-moz-bool-pref()` という [CSS](/ja/docs/Web/CSS) の {{CSSxRef("@supports")}} 関数を追加し、クロームスタイルシートの一部を論理環境設定に隠すことができるようになりました。 ([Firefox バグ 1259889](https://bugzil.la/1259889))
