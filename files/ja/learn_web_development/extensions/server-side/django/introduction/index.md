...
<table>
  <tbody>
    ...
    <tr>
      <th scope="row">目標:</th>
      <td>
        Django とは何か、提供する機能、 Django アプリケーションの主要要素に精通すること。
      </td>
    </tr>
  </tbody>
</table>
...
- 多様性
  - : Django を使って、コンテンツマネジメントシステムや Wiki からソーシャルネットワークやニュースサイトまで、ほとんど全てのタイプのウェブサイトを構築することができます（そして構築してきました）。どんなクライアントサイドフレームワークでも動作し、ほとんどどんな形式のコンテンツでも配信できます（HTML、RSS フィード、JSON、XML を含む）。
...
Django は、2008 年 9 月の最初のマイルストーンリリース (1.0) から、2023 年後半のバージョン 5.0 に至るまで、成長と改善を続けてきました。各リリースでは、新しい種類のデータベース、テンプレートエンジン、キャッシュ機能のサポートから、「ジェネリック」ビュー関数やクラスの追加（これにより、開発者がさまざまなプログラミングタスクのために記述しなければならないコード量が削減されます）に至るまで、新しい機能やバグ修正が盛り込まれてきました。
...
サーバーサイドフレームワークの人気度を即座に把握できる決定的な指標は存在しません（もっとも、各プラットフォームの GitHub プロジェクト数や Stack Overflow の質問数を数えるなどの方法で、人気度を推定することは可能です）。むしろ問うべきは、Django が「十分に人気がある」かどうか、つまり、人気のないプラットフォームが抱える問題を回避できるかどうかです。Django は進化し続けているでしょうか？ 必要な時に助けを得られるでしょうか？ Django を学べば、有償の仕事を得る機会はあるでしょうか？
...
Django のウェブアプリケーションは、モデルと呼ばれる Python オブジェクトを通じてデータを管理し、クエリーを実行します。モデルは、フィールドのタイプや、最大サイズ、デフォルト値、選択リストのオプション、ドキュメントのヘルプテキスト、フォームのラベルテキストなど、格納されるデータの構造を定義します。モデルの定義は、下層のデータベース（プロジェクト設定でいくつかから選択できます）からは独立しています。使用したいデータベースを選択したあとは、直接そのデータベースとやりとりする必要はありません。モデル構造と他のコードを書くだけで、Django はデータベースとのやりとりのすべての面倒な作業を処理します。
