...
9. 次に、drawImage の引数 9 個版を使用して画像を描画します。次のものを追加します。
...
12. 最後に、アニメーションをループさせるために、 `draw()` 関数の一番下に {{domxref("window.requestAnimationFrame", "requestAnimationFrame()")}} を呼び出す必要があります。
...
6. 次に、シーンを見るための**カメラ**が必要です。三次元画像の用語では、カメラは世界における視聴者の位置を表します。カメラを作成するには、以下の行を次に追加してください。
