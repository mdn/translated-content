...
次に、ウェブサーバーは {{Glossary("HTTP")}} (Hypertext Transfer Protocol) への対応を提供しています。名前にある通り、 HTTP はコンピューター間でハイパーテキスト（リンクされたウェブ文書）を転送する方法を定義しています。

{{Glossary("Protocol", "プロトコル")}}は 2 台のコンピューター間で通信する規則を設定します。 HTTP はテキストによる、ステートレス（下記参照）のプロトコルです。
...
HTTP はクライアントとサーバーがどのように通信するかについての明確なルールを提供しています。
もっと学びたい場合は、 [HTTP のドキュメント](/ja/docs/Web/HTTP)を参照してください。
とりあえず、いくつか注意すべきことがあります。

- **クライアント**は**サーバー**に HTTP リクエストを行います。サーバーはクライアントの HTTP リクエストに応答します。
...
3. どちらの処理もできない場合、ウェブサーバーは {{HTTPStatus("404", "404 Not Found")}} としてよく知られているエラーメッセージをブラウザーに返します。
   ...
   ![MDN 404ページはこのようなエラーページの例です](mdn-404.jpg)
