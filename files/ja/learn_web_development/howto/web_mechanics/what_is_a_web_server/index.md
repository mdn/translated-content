...
- 通常、**クライアント**だけが HTTP リクエストを、**サーバー**に対してのみ行います。サーバーはクライアントの HTTP リクエストに応答します。サーバーは、[サーバープッシュ](https://en.wikipedia.org/wiki/HTTP/2_Server_Push)と呼ばれるメカニズムによって、リクエストされる前にデータをクライアントのキャッシュに入れることもできます。
