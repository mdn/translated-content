...
- 使用するモバイル端末は、 CPU/GPU や利用できる帯域幅の点で低性能でした。 CSS はもちろん、 HTML さえ対応していないものもあり、その結果、端末が対応できるサイトを提供する前に、サーバー側でブラウザー検出を行い、端末/ブラウザーの種類を判別するのが一般的でした。モバイル端末では、実に単純で基本的な使い勝手しか提供されないことが多く、それが端末が処理することのできるすべてだったからです。最近では、モバイル端末はデスクトップコンピューターと同じ技術を処理することができるため、このような技術はあまり見かけなくなりました。
  ...
  - ユーザー体験は今なお課題です。例えば旅行サイトのモバイルユーザーは、単にフライト時刻や遅延情報を調べたいだけかもしれません。そんな時に、飛行経路や会社の歴史を示す 3D アニメーションの地球儀を表示させる必要はありません。
...
デフォルトのレスポンシブ動作は解決策が不要に思えるかもしれませんが、広いモニターで全画面表示された長いテキスト行は読みにくい場合があります。この問題は CSS で解決できます。例えば、行の長さを制限するために狭い列を生成する方法があります。しかし、これはブラウザーウィンドウを狭めるユーザーやモバイル端末でサイトを表示するユーザーにとって新たな問題を引き起こす可能性があります。列が押しつぶされたように見え、さらに読みにくくなるのです。
...
例えば、次のメディアクエリーは、現在のウェブページが画面メディアとして表示され（したがって印刷文書ではない）、ビューポートの幅が `80rem` 以上であるかどうかを検査します。 `.container` ルールは、これら 2 つのことが当てはまる場合にのみ適用されます。
...
スタイルシート内に複数のメディアクエリーを追加して、レイアウト全体またはその一部をさまざまな画面サイズに最適に調整することができます。メディアクエリーが導入されてレイアウトが変化するポイントは、ブレークポイント (breakpoints) と呼ばれます。
...
> [!NOTE]
> Scrimba には [Aside: Media queries](https://scrimba.com/frontend-path-c0j/~0j3?via=mdn) <sup>[_MDN 学主パートナー_](/ja/docs/MDN/Writing_guidelines/Learning_content#partner_links_and_embeds)</sup> というチュートリアルがあり、メディアクエリーのインタラクティブな入門と、基本を理解しているかテストするチャレンジを提供しています。（英語）
...
ブラウザーウィンドウのサイズを変更してください。上記の例が `600px` の幅のしきい値を超えると、レイアウトが単一列のレイアウトから 2 列レイアウトに切り替わります。
...
再度、ブラウザーウィンドウのサイズを変更してみてください。前回の例と同様に、`600px` の幅のしきい値でレイアウトが変化するはずです。
...
これは、メディア要素を拡大縮小して、コンテナーからあふれないことを保証します。
...
この例では、レベル 1 の見出しを `4rem` に設定します。これは基本フォントサイズの 4 倍ということです。実に大きな見出しです。この特大見出しは画面サイズが大きい場合にのみ適用したいので、まず見出しに `2rem` という小さいサイズを設定し、ユーザーの画面幅が少なくとも `1200px` であると判別できた場合にのみ、メディアクエリーを用いてより大きなサイズで上書きします。
...
次の例は、先ほどのレスポンシブグリッドの例を変更したもので、前述の方法を使用してレスポンシブ見出しを記載しています。モバイルでは見出しは小さくなりますが、デスクトップでは大きな見出しサイズが表示されます。
...
前回の例と同様に、ブラウザーウィンドウの幅を変更してみてください。`600px` の幅のしきい値でレイアウトが変化するだけでなく、見出しのサイズも変化することに注意してください。
...
{{cssxref("calc()")}} を使用するという解決策があります。 `em` や `rem` などの固定サイズを使用した値に `vw` 単位を加算しても、テキストはズーム可能です。次のように基本的に、`vw` 単位はズームした値に加算します。
...
以前と同様にブラウザーウィンドウのサイズを変更してみてください。今度は幅が変わるにつれて、見出しのサイズが徐々に大きくなることに注目してください。
...
`width=device-width` を設定することで、モバイル端末の既定値（iPhone の既定値 `width=980px` など）を実際の端末の幅で上書きすることになります。これがないと、ブレークポイントやメディアクエリーを使ったレスポンシブデザインがモバイルブラウザーで意図通りに動作しない可能性があります。ビューポートの幅が 480px 以下で動作する狭い画面レイアウトを取得していても、端末が 980px の幅だと言えば、そのユーザーには狭い画面用のレイアウトは表示されません。

**したがって、ビューポートメタタグを文書の先頭に常に含める必要があります**。

ビューポートメタタグの `content` 属性にはそれ以外にもいくつかのオプションを指定できます。詳細は [`<meta name="viewport">`](/ja/docs/Web/HTML/Reference/Elements/meta/name/viewport) の参照記事をご覧ください。

## まとめ

レスポンシブデザインとは、閲覧環境に応答するサイトやアプリケーションのデザインを指します。CSS と HTML の機能やテクニックを包括し、既定ではこれでウェブサイトを構築することができます。携帯電話で閲覧するサイトを考えてみましょう。デスクトップ版を縮小したサイトや、ものを探すのに横スクロールが必要なサイトに出会うのは、かなり珍しいことでしょう。これは、ウェブがレスポンシブデザインという手法に移行されたからです。

また、この記事で網羅したレイアウト手法を活用することで、レスポンシブデザインの実現が格段に容易になりました。今日、ウェブ開発を新しく始めた方は、レスポンシブデザインの初期の頃よりも自由に使えるツールがたくさんあります。そのため、使用している素材の古さを調べる価値があります。過去の記事は今でも有用ですが、現代の CSS と HTML を使用することで、訪問者がどんな端末でサイトを閲覧しても、エレガントで有益なデザインをはるかに簡単に作成することができます。
