...
既に定義済みのパラメーターがいくつかあります。`color`、`position`、`rotation` です。これらはとても分かりやすく、キューブの基本色、3D シーン内での位置、そしてキューブの回転を決めます。
...
まずシーンハンドラーへの参照を取得し、次に円柱要素を A-Frame エンティティとして作ります。その後は、`color`、`height`、`radius`、`position` といった適切な属性を決めるだけです。最後の行で、新しく作った円柱をシーンに加えます。これで、A-Frame を使って 3 種類の異なる形を作れました！
