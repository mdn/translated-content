...
> **Note:** このサイトを実行するには、ローカルのウェブサーバーを実行する必要があります。file:// URL はワーカーを読み込むことができないからです。ローカルテストサーバの設定] (/ja/docs/Learn/Common_questions/set_up_a_local_testing_server) のガイドを参照してください。これで、［素数の生成］をクリックすると、メインページが応答し続けるようになるはずです。
>
