...
ループの反復処理ごとに、現在の `newImage` にクリックリスナーを追加する必要があります。このリスナーは現在の画像の `src` 属性の値を見つけます。`displayed-img <img>` の `src` 属性の値をパラメータとして渡されたものの `src` 値へ設定します。次に `alt` 属性についても同じようにします。
