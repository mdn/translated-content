...
以下の図は、これらのレイヤーを示しています:![Diagram of the box model](https://mdn.mozillademos.org/files/16558/box-model.png)
...
標準のボックスモデルを使用してボックスが占めるスペースは、実際には 410px (350 + 25 + 25 + 5 + 5) で、高さは 210px (150 + 25 + 25 + 5 + 5) であり、padding と border は コンテンツボックスに使用される幅に追加されます。![Showing the size of the box when the standard box model is being used.](https://mdn.mozillademos.org/files/16559/standard-box-model.png)

> **Note:** **注:** マージンは、ボックスの実際のサイズにはカウントされません。確かに、ボックスがページ上で占める合計スペースに影響しますが、ボックスの外側のスペースにのみ影響します。ボックスの領域は境界線で停止します—マージンまでは達しません。
...
> **Note:** **注**: 興味深い歴史 — Internet Explorer は以前は代替ボックスモデルにデフォルト設定されていましたが、切り替え可能なメカニズムはありませんでした。
...
> **Note:** **注**: [ここ](https://github.com/mdn/css-examples/blob/master/learn/solutions.md#the-box-model)でこのタスクの解決策を見つけることができます。
