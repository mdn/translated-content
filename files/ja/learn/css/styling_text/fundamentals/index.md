...
> **メモ:** 要素内のテキストはすべて単一の実体として影響を受けることに注意してください。 テキストの一部分を（{{htmlelement("span")}} や {{htmlelement("strong")}} など）適切な要素で囲んだり、[::first-letter](/ja/docs/Web/CSS/::first-letter)（要素のテキストの最初の文字を選択）、[::first-line](/ja/docs/Web/CSS/::first-line)（要素のテキストの最初の行を選択）、[::selection](/ja/docs/Web/CSS/::selection)（現在カーソルで強調表示されているテキストを選択）のようなテキスト固有の疑似要素を使用しない限り、テキストの一部分を選択して装飾することはできません。
