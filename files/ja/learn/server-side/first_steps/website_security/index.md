...
> **Note:** これは導入トピックであり、ウェブサイトのセキュリティについて考え始めるのに役立つように設計されていますが、網羅的なものではありません。
...
> **Note:** XSS 脆弱性は、他のどの種類のセキュリティの脅威よりも歴史的に一般的です。
...
> **Note:** SQL ステートメントは、**'** 文字を文字列リテラルの開始と終了として扱います。この文字の前に円記号を入れる (**\\'**) ことで、シンボルをエスケープし、代わりにそれを文字 (文字列の一部) として扱うように SQL に指示します。
...
> **Note:** このセクションは[ここウィキペディア](https://en.wikipedia.org/wiki/SQL_injection)の情報に大きく依存しています。
...
> **Note:** ここでのトリックは、John がユーザーの cookie (またはアクセス資格情報) にアクセスする必要がないことです。ユーザーのブラウザーはこの情報を保存し、関連するサーバーへのすべてのリクエストに自動的に含めます。
