...
[前の章で述べたように、ルーティング](/ja/docs/Learn/Tools_and_testing/Client-side_JavaScript_frameworks/Introduction#routing)は Web エクスペリエンスの重要な部分です。多数のビューを備えた非常に複雑なアプリでの操作性の低下を避けるために、このモジュールで説明する各フレームワークは、開発者がアプリケーションにクライアント側ルーティングを実装するのに役立つライブラリ(または複数のライブラリ)を提供します。
