...
Netlify では、特に、デプロイ前のタスクを実行することもできます。これは、この場合、すべての実稼働コードのビルドプロセスを Netlify 内で実行でき、ビルドが成功すると、Web サイトの変更がデプロイされることを意味します。
...
1. プロジェクトのルートディレクトリにある `package.json` ファイルを開き、 `scripts` プロパティを見つけます。
