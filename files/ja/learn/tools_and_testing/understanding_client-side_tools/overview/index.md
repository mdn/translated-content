...
コード フォーマッターは、コードのエラーを指摘するのではなく、通常、スタイル ルールに従ってコードが正しくフォーマットされていることを確認し、理想的には見つけたエラーを自動的に修正する傾向があることを除いて、リンターに多少関連しています。

[Prettier](https://prettier.io/) はコード フォーマッターの非常に一般的な例であり、モジュールの後半で使用します。 
