...
{{Glossary("HTML")}} を使用しているとき、HTML タグを正しく使用する方法や適用方法を簡単に覚える方法があるととても便利です。MDN は広範にわたる [HTML ドキュメント](/ja/docs/Web/HTML/Element) と深い解説の [HTML ハウツー](/ja/docs/Learn/HTML/Howto)を提供しています。しかし、多くの場合、手っ取り早くヒントを得たいものです。そのため、早見表では、よく使用するコードの断片をすばやく正確に使用することができるようにしています。

> **メモ:** HTML タグは、見た目ではなく、意味づけのために使用しなければなりません。 常に {{Glossary("CSS")}} を使って指定されたタグの見た目をガラリと変えることが可能なので、HTML を使用する際は、見た目よりも意味を重視するようにしましょう。
...
> **メモ:** この早見表は、固有の構造を表したり、特別な意味づけをするいくつかの要素に限定しているため、[`div`](/ja/docs/Web/HTML/Element/div) 要素は意図的に含まれていません。`div` 要素は何らかの構造を表すものではなく、特別な意味づけを持っているわけでもありません。
