...
{{glossary("SSG", "静的サイト")}}は、 SSR と見なすことができ（ SSR インフラストラクチャを使用して生成することもできます）、微妙な違いがあります。静的サイトのコンテンツは、リクエスト時ではなくビルド時に生成されます。静的サイトは多くの場合、サーバーにデプロイする必要がなく、 {{glossary("CDN")}} から配信できます。
