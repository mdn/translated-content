...
**有効接続種別** (ECT) とは、測定されたネットワーク性能のことで、実際の接続がテザリングされたブロードバンドや WiFi であっても、ブラウザーのページのリクエストの時間と有効接続種別に基づいて、 3G などの携帯通信接続種別を返します。

観測されたラウンドトリップと下り接続速度の値を使用して、 '`slow-2g`', '`2g`', '`3g`', '`4g`' の値が特定されます。

| ECT       | 最小 [RTT](/ja/docs/Glossary/Round_Trip_Time) | 最大下り接続 | 説明                                                                         |
| --------- | --------------------------------------------- | ------------ | ---------------------------------------------------------------------------- |
