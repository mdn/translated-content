...
**JIT**（_実行時コンパイル_）とは、実行前ではなく、*実行時*にコードを高水準言語 (JavaScript など) や中間表現 (Java バイトコードなど)からマシンコードに変換する {{glossary("compile", "Compile (コンパイル)")}} 処理です。この手法は、インタープリター方式と AOT(Ahead-of-Time) (事前)コンパイルの両方の利点を兼ね備えています。
