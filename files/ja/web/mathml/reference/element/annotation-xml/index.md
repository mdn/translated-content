...
**`<annotation-xml>`** は [MathML](/ja/docs/Web/MathML) の要素で、{{glossary("XML")}} 形式（[内容 MathML](https://ja.wikipedia.org/wiki/MathML#内容MathML) や {{glossary("SVG")}} など）で記述された、MathML 式に対する注釈が含まれています。
...
- `encoding`
  - : この注釈の意味情報のエンコード形式（例: `"application/mathml+xml"`, `"application/mathml-presentation+xml"`, `"application/mathml-content+xml"`）
...
<math display="block">
  <semantics>
    ...
    <!--
      数学式の意味を表すための専用の XML 方言である
      内容 MathML を用いて注釈を付ける
    -->
