...
- `accentunder`
  - : 下付きのスクリプトをアクセントとして扱う（つまり、より大きく、かつベースとなる式により近づけて描く）かどうかを示す [`<boolean>`](/ja/docs/Web/MathML/Reference/Values#mathml-specific_types) です。
...
- {{ MathMLElement("mover") }} (上付きの注釈)
- {{ MathMLElement("munderover") }} (下付きと上付きの注釈の組)
