...
これで、path 要素の円弧を扱う必要なしに半円を作りました。クリッピング用に `clipPath` の内部にあるすべてのパスが、その線や座標変換と併せて調査および評価されます。そして適用先のオブジェクトのうち、`clipPath` の内容物で決められた透過領域にかかる部分は描画されません。色や不透明度などは、図形の一部を完全に消滅させない限り効果がありません。
