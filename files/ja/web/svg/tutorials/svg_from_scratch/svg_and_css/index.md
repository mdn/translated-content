...
このデモでは、 SVG 対応ブラウザーは既に SVG 要素の表示方法を知っています。このスタイルシートは特定の方法で表示を変更するだけです。これは HTML 文書にも当てはまります。しかし、要素の表示方法があらかじめ定義されていない汎用の XML 文書には CSS を使用することができます。次の [XML の紹介](/ja/docs/Web/XML/XML_introduction)のページでは、これをデモしています。
