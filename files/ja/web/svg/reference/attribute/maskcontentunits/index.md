...
**`maskContentUnits`** 属性は、 {{ SVGElement("mask") }} 要素のコンテンツに使用する座標系を示します。
...
{{SVGElement("mask")}} では、`maskContentUnits` は、要素のコンテンツに使用されている座標系を定義します。
...
- `userSpaceOnUse`
  - : この値は、この {{SVGElement('mask')}} 要素内のすべての座標が、マスクの作成時に定義されたユーザー座標系を参照することを示します。
- `objectBoundingBox`
  - : この値は、この {{SVGElement('mask')}} 要素内のすべての座標が、マスクが適用される要素の境界ボックスを基準としていることを示します。境界ボックスは、{{ SVGElement("mask") }} のコンテンツが `"0 0 1 1"` {{ SVGAttr("viewBox") }} に結び付けられている場合と同じと考えることができます。
