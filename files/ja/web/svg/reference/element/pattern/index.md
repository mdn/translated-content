...
- {{SVGAttr("href")}}
  - : この属性は `<pattern>` 属性のデフォルト値を提供するテンプレートパターンを参照します。
...
- {{SVGAttr("patternTransform")}}
  - : この属性は、パターン座標系からターゲット座標系への任意の追加変換の定義が入ります。
    _値の型_: **[\<transform-list>](/ja/docs/Web/SVG/Guides/Content_type#transform-list)**; _デフォルト値_: _恒等行列_; _アニメーション_: **可**
...
この要素は、{{domxref("SVGPatternElement")}} インターフェイスを実装しています。
