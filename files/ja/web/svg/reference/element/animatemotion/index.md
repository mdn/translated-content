...
- {{SVGAttr("path")}}
  - : この属性は、 {{SVGAttr('d')}} 属性と同じ構文を使用して、モーションパスを定義します。
    _値の型_: **\<string>**; _デフォルト値_: none; _アニメーション_: **不可**
- {{SVGAttr("rotate")}}
  - : この属性は、パスに沿ってアニメーションされた要素に適用される回転を定義します。
    _値の型_: [**\<number>**](/ja/docs/Web/SVG/Guides/Content_type#number) | `auto` | `auto-reverse`; _デフォルト値_: `0`; _アニメーション_: **不可**

> [!NOTE]
> `<animateMotion>` においては、 {{SVGAttr("calcMode")}} 属性のデフォルト値は `paced` です。
