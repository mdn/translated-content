...
- [`download`](/ja/docs/Web/HTML/Reference/Elements/a#download)
  - : 移動するのではなく {{Glossary("URL")}} をダウンロードするようブラウザーに指示しますので、ユーザーにローカルファイルとして保存するよう促します。
    _値の型_: **\<string>** ; _デフォルト値_: _none_; _アニメーション_: **不可**
- {{SVGAttr("href")}}
  - : ハイパーリンクが指す先の {{Glossary("URL")}} または URL フラグメントです。
    _値の型_: **[\<URL>](/ja/docs/Web/SVG/Guides/Content_type#url)** ; _デフォルト値_: _none_; _アニメーション_: **可**
- [`hreflang`](/ja/docs/Web/HTML/Reference/Elements/a#hreflang)
  - : ハイパーリンクが指す先の URL または URL フラグメントにおける人間の言語です。
    _値の型_: **\<string>** ; _デフォルト値_: _none_; _アニメーション_: **不可**
- [`interestfor`](/ja/docs/Web/HTML/Reference/Elements/a#interestfor) {{experimental_inline}} {{non-standard_inline}}
  - : `<a>` 要素を**関心インボーカー**として定義します。その値は、インボーカー要素に対する関心が示されたり失われたりした際に（例えば、ポインターをかざしたりやめたり、フォーカスを与えたり外したりして）、何らかの影響（通常は表示/非表示）を受ける対象要素の `id` です。詳細と例については、[関心インボーカーの使用](/ja/docs/Web/API/Popover_API/Using_interest_invokers)を参照してください。
    _値の型_: **\<string>** ; _デフォルト値_: _none_; _アニメーション_: **不可**
- [`ping`](/ja/docs/Web/HTML/Reference/Elements/a#ping) {{experimental_inline}}
  - : 空白区切りの URL のリストで、ハイパーリンクをたどる際に、 {{HTTPMethod("POST")}} リクエストで本文が `PING` であるものがブラウザーから（バックグラウンドで）送信されます。通常はトラッキングのために使用されます。同じ用途でもっと広く対応されている機能として、 {{domxref("Navigator.sendBeacon()")}} を参照してください。
    _値の型_: **[\<list-of-URLs>](/ja/docs/Web/SVG/Guides/Content_type#list-of-ts)**; _デフォルト値_: _none_; _アニメーション_: **不可**
- [`referrerpolicy`](/ja/docs/Web/HTML/Reference/Elements/a#referrerpolicy) {{experimental_inline}}
  - : どの[リファラー](/ja/docs/Web/HTTP/Reference/Headers/Referer)をアクセス先の {{Glossary("URL")}} に送信するかです。
    _値の型_: `no-referrer` | `no-referrer-when-downgrade` | `same-origin` | `origin` | `strict-origin` | `origin-when-cross-origin` | `strict-origin-when-cross-origin` | `unsafe-url`; _デフォルト値_: _none_; _アニメーション_: **不可**
- [`rel`](/ja/docs/Web/HTML/Reference/Elements/a#rel)
  - : 対象のオブジェクトとリンクしているオブジェクトの関係です。
    _値の型_: **[\<list-of-Link-Types>](/ja/docs/Web/HTML/Reference/Attributes/rel)**; _デフォルト値_: _none_; _アニメーション_: **可**
- {{SVGAttr("target")}}
  - : リンクされた {{Glossary("URL")}} の表示先です。
    _値の型_: `_self` | `_parent` | `_top` | `_blank` | **\<name>**; _デフォルト値_: `_self`; _アニメーション_: **可**
- [`type`](/ja/docs/Web/HTML/Reference/Elements/a#type)
  - : リンク先の URL の {{Glossary("MIME type", "MIME タイプ")}}です。
    _値の型_: **\<string>** ; _デフォルト値_: _none_; _アニメーション_: **可**
...
この要素は {{domxref("SVGAElement")}} インターフェイスを実装しています。
