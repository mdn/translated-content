...
- `size`
  - : フォントサイズとして、数値または相対値キーワードを指定します。 数値の場合の範囲は `1` から `7` で、`1` が最小、`3` が初期値となります。 `+2`、`-3` の様に、{{HTMLElement("basefont")}} 要素の [`size`](/ja/docs/Web/HTML/Element/basefont#size) 属性の値、もしくは初期値である `3` を基準とする相対値で記述する事も可能です。
