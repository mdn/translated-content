...
IDL 属性は JavaScript プロパティとしても知られています。これらは、 `element.foo` のような JavaScript プロパティを使って読み込んだり設定したりすることができます。 IDL 属性は常に、取得する際に値を返すために基盤となっているコンテンツ属性を使用し（変換することもあります）、設定するにはコンテンツ属性に何かを保存します。言い換えれば、 IDL 属性は要するにコンテンツ属性を反映します。
