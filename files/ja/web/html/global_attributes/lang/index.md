...
WCAG 成功基準 3.1.1 では、「プログラムによって決定される」方法で（つまり、**`lang`** 属性を介して）ページの言語を指定することを**必須としています**。

WCAG 成功基準 3.1.2 は、言語の異なる**部分**があるページには、それらの部分の言語も指定することを必須としています。ここでも、**`lang`** 属性がそのための正しいやり方です。
