...
1. **メディア条件** (`(width <= 600px)`) — これについては [CSS の記事](/ja/docs/Learn_web_development/Core/Styling_basics)で詳しく学びますが、今はメディア条件とは画面がなる可能性がある状態であるとだけ言っておきましょう。 この場合、「ビューポートの幅が 600 ピクセル以下であるとき」と言っています。
2. 空白
...
4. `srcset` リストで指定された画像のうち、スロットのサイズと一致するものを読み込みます。表示サイズに完全に一致する画像がない場合、ブラウザーは選択されたスロットのサイズよりも大きい最初の画像を選び、表示サイズに合わせて変倍します。

以上です！ この時点で、480px のビューポート幅を持つ対応しているブラウザーがこのページを読み込むと、`(width <= 600px)` メディア条件が真となるため、ブラウザーは `480px` のスロットを選択します。`elva-fairy-480w.jpg` の固有幅 (`480w`) が最も近いため、これが読み込まれます。 800px の画像は 128KB ですが、480px の画像は 63KB で 65KB の節約になります。 今、これが多くの写真があるページだったと想像してみてください。 この技術を使用することで、モバイルユーザーは多くの帯域幅を節約できます。
...
- `<source>` 要素にはメディア条件が含まれている `media` 属性が含まれています。 最初の srcset の例のように、これらの条件はどの画像が表示されるかを決定するテストであり、最初に真を返すものが表示されます。 この場合、ビューポート幅が 800 ピクセル未満の場合、最初の `<source>` 要素の画像が表示されます。 ビューポート幅が 800 ピクセル以上であれば、2 番目のものになります。
...
この演習では、勇気を出して、基本的には自分自身で取り組んでいただくことを期待しています。`<picture>` を使用して、自分自身でデザインした縦長画面／横長画面のスクリーンショットを実装し、さらに `srcset` を使用した解像度切り替えの例も作成してください。

1. 自分のコードを格納するための HTML を書きましょう（必要に応じて `not-responsive.html` を出発点として使用する）。
