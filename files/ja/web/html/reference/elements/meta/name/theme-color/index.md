...
**`theme-color`** の値は {{htmlelement("meta")}} 要素の [`name`](/ja/docs/Web/HTML/Reference/Elements/meta#name) 属性において、ユーザーエージェントがページやその周辺のユーザーインターフェイスの表示をカスタマイズするために使用すべき推奨色を示します。指定された場合、 [`content`](/ja/docs/Web/HTML/Reference/Elements/meta#content) 属性には有効な CSS の {{cssxref("&lt;color&gt;")}} が含まれていなければなりません。
