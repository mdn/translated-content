---
title: HTML `<input type="month">` 属性値
...
入力コントロールのデフォルト値は、次のように [`value`](/ja/docs/Web/HTML/Reference/Elements/input#value) 属性に年と月を入れることで設定することができます。
...
`step` 属性は値が準拠する粒度を指定する数値、または後述する特殊な値 `any` です。段階の基準値から整数倍の値のみが有効です。段階の基準値は（指定されていれば）[`min`](#min)、そうでなければ [`value`](/ja/docs/Web/HTML/Reference/Elements/input#value)、どちらも提供されていなければ `0` （UNIX 元期、`1970-01`）です。

`month` 入力の場合、`step` の値は月単位で指定されます。`step` のデフォルト値は 1 で、これは 1 ヶ月を示します。

文字列値の `any` は段階がないことを意味し、（[`min`](#min) や [`max`](#max) のようなほかの制約を除けば）どの値でも許可されます。実際には、ピッカー UI では 1 月単位での選択しかできないため、`month` 入力では `1` と同じ効果になります。
...
デフォルトで、 `<input type="month">` は入力された値の検証を行いません。
...
一方、デフォルトの設定では、 `text` 入力欄ではどの書式で入力されるかの認識がなく、以下のように人間が日付を書く様々な方法で入力される可能性があります。
