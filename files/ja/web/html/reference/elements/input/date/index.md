...
`step` 属性は値が準拠する粒度を指定する数値、または後述する特殊な値 `any` です。刻みの基準値から整数倍の値のみが有効です。刻みの基準値は（指定されていれば）[`min`](#min)、そうでなければ [`value`](/ja/docs/Web/HTML/Reference/Elements/input#value)、どちらも提供されていなければ `0` （UNIX 元期、`1970-01-01`）です。

`date` 入力の場合、`step` の値は日単位で指定され、`step` の値に 86,400,000 を掛けた値に相当するミリ秒数として扱われます（実際の数値はミリ秒単位です）。デフォルト値は 1 で、これは 1 日を示します。

文字列値の `any` は刻みがないことを意味し、（[`min`](#min) や [`max`](#max) のようなほかの制約を除けば）どの値でも許可されます。実際には、ピッカー UI では 1 日単位での選択しかできないため、`date` 入力では `1` と同じ効果になります。
