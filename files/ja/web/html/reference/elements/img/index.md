...
- `decoding`
  - : この属性は、ブラウザーが画像のデコードを他の DOM コンテンツのレンダリングと一緒に行い、より「正しく」見えるようにするのか (`sync`)、それとも他の DOM コンテンツを先にレンダリングして表示し、画像をデコードして後で表示するのか (`async`) のヒントを提供します。実際には、`async` は次の描画が画像のデコードを待たないことを意味します。
    ...
    - `auto`
      - : デコードモードの優先順位はありません。ブラウザーがユーザーにとって最適なものを決定します。これはデフォルト値です。
...
- `loading`
  - : ブラウザーがどのように画像を読み込むかを示します。
    - `eager`
      - : 画像が現在{{glossary("visual viewport", "視覚ビューポート")}}に入っているかどうかにかかわらず、直ちに画像を読み込みます（これがデフォルト値です）。
    - `lazy`
      - : 画像がブラウザーで定義されたビューポートからの距離に達するまで、画像の読み込みを遅延させます。

        遅延読み込みは、画像が必要になるとほぼ確実になるまで、その画像を処理するために必要なネットワーク帯域幅やストレージ帯域幅の使用を避けることができます。これにより、一般的な用途のほとんどでパフォーマンスが改善されます。

        レイアウトのずれを避けるため、すべての画像に対して明示的な [`width`](#width) および [`height`](#height) 属性の指定が推奨されていますが、これらは特に遅延読み込みされる画像において重要です。遅延読み込みされる画像は、たとえ読み込むことで表示領域が変化するとしても、要素の表示領域と重ならない限り読み込まれません。これは、読み込まれていない画像の `width` と `height` が `0` であるためです。読み込み中にビューポート内のコンテンツが再配置されると、使い勝手にさらに大きな支障をきたすことになります。

        視覚的ビューポート内にある遅延読み込み画像は、Window の {{domxref("Window.load_event", "load")}} イベントが発生した時点では、まだ表示されていないことがあります。これは、このイベントが即時読み込みされた画像に基づいて発生するためです。つまり、ページの初期読み込み時に視覚的ビューポート内にあったとしても、遅延読み込み画像は考慮されません。
...
- `referrerpolicy`
  - : リソースを読み込む際に、どのリファラーを使用するかを示す文字列です。
    ...
    - `strict-origin-when-cross-origin`（デフォルト値）: 同一オリジンリクエストを行う際には完全な URL を送信し、セキュリティレベルが同等 (HTTPS→HTTPS) の場合はオリジンのみを送信し、安全性の低い宛先 (HTTPS→HTTP) にはヘッダーを送信しません。
...
- `sizes`
  - : カンマで区切られた 1 つ以上の値。値としては、ソースサイズまたは `auto` キーワードを指定できます。
    仕様書では、`srcset` が幅記述子を使用している場合にのみ、`sizes` 属性が要求されています。
    - **ソースサイズ**
      - : **ソースサイズ**は次のものから構成されます。
        1. [メディア条件](/ja/docs/Web/CSS/Guides/Media_queries/Using#構文)で、リストの最後のアイテムでは省略されます。
        2. ソースサイズ値。

        例えば、以下のソースサイズでは、ビューポートの幅が 500px 以下の場合、幅 `1000px` の画像ソースを使用することを提案しています。

        ```css
        (width <= 500px) 1000px
        ```

        メディア条件は、{{glossary("viewport", "ビューポート")}}のプロパティを説明するものであり、画像のプロパティではありません。
        ソースサイズ記述子は、レイアウト時に画像に使用する幅を指定するため、メディア条件は通常（必ずしもそうとは限りませんが）、{{cssxref("@media/width")}} に基づいています。

        ソースサイズ値は、画像の表示サイズを指定するものです。
        {{glossary("User agent", "ユーザーエージェント")}}は `srcset` 属性で与えられたソースからひとつを選択するために、現在のソースサイズを使用します。そのとき、ソースは幅記述子 (`w`) を使用して説明します。
        sizes で定義された `w` の値は、画像のデフォルトのレイアウト幅を決定します。
        {{glossary("CSS")}} が指定されていない場合、ブラウザーはダウンロードされたファイルの実際のピクセルサイズに関係なく、画像をこのサイズで表示します。

        ソースサイズの値は、非負の任意の[長さ](/ja/docs/Web/CSS/Reference/Values/length)でなければなりません。
        また、[数学関数](/ja/docs/Web/CSS/Reference/Values/Functions#math_functions)以外の CSS 関数を使用してはなりません。
        単位は[メディアクエリー](/ja/docs/Web/CSS/Guides/Media_queries)と同様に解釈されます。つまり、すべての相対長の単位は `<img>` 要素ではなく文書ルートを基準とします。例えば `em` 値は画像のフォントサイズではなく、ルートフォントサイズを基準とします。[パーセント値](/ja/docs/Web/CSS/Reference/Values/percentage) は使用できません。`sizes` 属性が指定されていない場合、そのデフォルト値は `100vw`（ビューポートの幅）となります。

    - `auto`
      - : `auto` キーワードは、ブラウザーが要素の想定されるレイアウト幅に基づいて、表示させる画像を選択すべきであることを示します。
        つまり、HTML と CSS によるレイアウトが適用された後に計算される、画像の[具体的なサイズ](/ja/docs/Web/CSS/Reference/Values/image#concrete_size)を使用することになります。
        これは、`loading="lazy"` と組み合わせて使用する場合にのみ有効です。なぜなら、画像が読み込まれる時点では、ページにはすでに CSS やその他のレイアウト情報が用意されていることが想定されているためです。

        `auto` を使用すると、レイアウトのメディア条件を 2 回指定する手間が省けます。1 回目はレイアウト用、2 回目は取得・表示させる適切な画像の選択用です。

        `auto` が解決できない場合（ブラウザーが対応していないか、画像にレイアウトサイズがまだ設定されていないため）、ブラウザーは幅を決定するために、リスト内のソースサイズに代替し、次に要素に定義された `width`/`height` 属性に代替し、最後にユーザーエージェントスタイルシートで定義された `<img>` 要素のデフォルトの内在サイズ（300px × 150px）に代替します。

        `auto` に対応していないブラウザーとの下位互換性を高めるために、`sizes` 属性の `auto` の後に代替サイズを記載することができます。
        同時に、`srcset` 内の最も大きな画像の実際のサイズに合わせて、要素の `width` および `height` 属性を設定してください。そうすることで、ブラウザーは正しいアスペクト比で空間を確保できるようになります。
...
- `src`
  - : 画像の {{glossary("URL")}} です。`src` と [`srcset`](#srcset) のどちらか 1 つが `<img>` 要素に必要です。[`srcset`](#srcset) が指定された場合、`src` は 2 通りのどちらかで使われます。
    - `srcset` に対応していないブラウザー向けの代替手段として。
    - `srcset` で "x" 記述子が使用されている場合、`src` は密度記述子 `1x` を持つソースと同等となります。つまり、`src` で指定された画像は、低解像度の画面（一般的な 72 DPI や 96 DPI のディスプレイなど）で使用されます。

- `srcset`
  - : {{glossary("user agent", "ユーザーエージェント")}}が使用可能なソース画像のセットを示す、カンマ区切りで文字列を 1 個以上並べたリストです。
    各々の文字列の構成は以下のとおりです。
    1. 画像の {{glossary("URL")}}
    2. 任意で、ホワイトスペースの後に以下のいずれかを記述します。
       - 幅記述子（正の整数の直後に `w` を付加したもの）。参照される画像の内在的な幅と一致していなければなりません。幅記述子は実際の画素密度を計算するために、`sizes` 属性で与えられたソースサイズで割られます。例えば、レンダラーが幅 450 ピクセル程度の画像が必要である場合に使用する画像リソースを指定するには、幅記述子 `450w` を使用します。`srcset` に "w" 記述子が含まれている場合、ブラウザーはそれらの記述子と `sizes` 属性を組み合わせてリソースを選択します。
       - 画素密度記述子（正の浮動小数点数の直後に `x` を付加したもの）。これは、対応する画像リソースを画面の画素密度として使用する条件を指定します。例えば、画素密度が標準密度の2倍のときに使用する画像リソースを提供するには、画素密度記述子 `2x` または `2.0x` を使用します。

    記述子を指定しない場合は、ソースをデフォルトの記述子 `1x` に割り当てます。幅記述子と画素密度記述子を同一の `srcset` 属性に混在させると無効になります。重複した記述子（例えばひとつの `srcset` に 2 つのソースがあり、どちらも `2x` とする）も無効になります。

    空白文字は、URL とそれに対応する条件記述子を区切る空白を除き、無視されます。これには、先頭および末尾の空白、ならびに各カンマの前後にある空白が含まれます。ただし、画像候補文字列に記述子が含まれておらず、かつ URL の後に空白がない場合、その後に続く画像候補文字列（存在する場合）は、1 つ以上の空白で始まる必要があります。そうしないと、カンマが URL の一部であるとみなされてしまいます。

    `<img>` 要素の `srcset` 属性で `x` 記述子が使用されている場合、ブラウザーは `src` 属性に指定された URL（存在する場合）も候補として扱い、そこにデフォルトの記述子として `1x` を割り当てます。一方、`srcset` 属性で幅記述子が使用されている場合は、`src` 属性は考慮されず、代わりに `sizes` 属性が使用されます。
...
- `align` {{deprecated_inline}}
  - : 周囲のコンテキストに対する画像の配置を指定します。代わりに {{glossary("CSS")}} の {{cssxref('float')}} プロパティや {{cssxref('vertical-align')}} プロパティを使用してください。使用できる値は次の通りです。
    ...
    - `bottom`
      - : 値のデフォルト値はなく `vertical-align: unset;` または `vertical-align: initial;` と等価です。
    - `left`
      - : `float: left;` と等価です。
    - `right`
      - : `float: right;` と等価です。
