...
表の 2 行目の見出し行として、追加の行（{{HTMLElement("tr")}} 要素）を追加し、そこに 2 つの列見出し（`<th>` 要素）を追加しています。この方法で、「発音」列は 2 つの列に分割されます。1 つはIPA（国際音声記号）表記用、もう 1 つはスペル表記（元の発音列）用です。対応するデータセル（{{HTMLElement("td")}} 要素）が、その後のそれぞれの行に追加されます。
