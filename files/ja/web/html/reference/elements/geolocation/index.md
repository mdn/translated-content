...
[位置情報 API](/ja/docs/Web/API/Geolocation_API) は、位置データ処理のための古いもう一つの手段を提供しています。この API にはいくつか欠点があり、`<geolocation>` 要素はその解決を目指しています。特に顕著なのは、データリクエストのためのUIと基盤となるロジックを毎回ゼロから実装する必要がある点、およびその権限を処理する際に直感的でない場合がある点です。
