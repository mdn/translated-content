...
入力欄の種類によってはデフォルトの最小値があるものもあります。入力欄にデフォルトの最小値がなく、 `min` に有効な数値に変換できない値が指定された場合（あるいは最小値が設定されなかった場合）、その入力には最小値が設定されません。
...
{{htmlelement('meter')}} 要素における `min` 属性は、測定範囲の上限の数値を定義します。これは、指定されている場合、最大値 ([`max`](/ja/docs/Web/HTML/Reference/Attributes/max) 属性) よりも小さくなければなりません。どちらの場合も、省略された場合、値はデフォルトで 1 となります。
...
`min` と `step` の値は、 `step` 属性がない場合も含め、どの値が有効であるかを定義します。 `step` のデフォルト値は `0` です。
...
それから入力欄の最小値を 7.2 と定義し、 step 属性は省略しているので、デフォルト値が 1 になります。

```html
<input id="myNumber" name="myNumber" type="number" min="7.2" value="8" />
```

`step` のデフォルト値は 1 なので、有効な値は `7.2`, `8.2`, `9.2`, などになります。 8 の値は有効ではありません。無効な値が入っているので、対応しているブラウザーは値が無効であることを表示します。

{{EmbedLiveSample("step_への影響",200,55)}}

明示的に含まれていない限り、 `number`、 `range` では `step` はデフォルト値の 1 となり、 date/time 入力型では 1 単位 (秒, 週, 月, 日) となります。
