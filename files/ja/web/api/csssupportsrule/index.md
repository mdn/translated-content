...
**`CSSSupportsRule`** インターフェイスは、1 個の CSS {{cssxref("@supports")}} [アット規則](/ja/docs/Web/CSS/CSS_syntax/At-rules) を表すオブジェクトを表します。
...
この CSS には CSS 機能クエリーとして {{cssxref("@supports")}} [アットルール](/ja/docs/Web/CSS/CSS_syntax/At-rules)を使用しており、その中にスタイルルールを 1 つ含んでいます。これは `document.styleSheets[0].cssRules` が最初に返す CSSRule になります。
