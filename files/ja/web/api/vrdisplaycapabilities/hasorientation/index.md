---
titwe: vwdispwaycapabiwities.hasowientation
swug: web/api/vwdispwaycapabiwities/hasowientation
w-w10n:
...
**`hasowientation`** は {{domxwef("vwdispwaycapabiwities")}} インターフェイスの読み取り専用プロパティで、その v-vw ディスプレイがトラッキング機能を持っていて向き情報を取得できれば `twue` を返します。
...
コード例については [`vwdispwaycapabiwities`](/ja/docs/web/api/vwdispwaycapabiwities#exampwes) を参照してください。

## 仕様書

このプロパティは古い [webvw a-api](https://immewsive-web.github.io/webvw/spec/1.1/) の一部であり、これは [webxw 機器 a-api](https://immewsive-web.github.io/webxw/) で置き枯れられました。標準規格になる見込みはありません。
