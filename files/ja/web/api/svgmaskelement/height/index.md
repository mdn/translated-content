---
title: "SVGMaskElement: height プロパティ"
short-title: height
...
**`height`** は {{domxref("SVGMaskElement")}} インターフェイスの読み取り専用プロパティで、 {{domxref("SVGAnimatedLength")}} オブジェクトとして {{SVGElement("mask")}} 要素の {{SVGattr("height")}} 属性の値を返します。
...
{{domxref("SVGAnimatedLength")}} オブジェクトです。このオブジェクトの `baseVal` プロパティは、 {{domxref("SVGLength")}} で `height` の値を返します。
