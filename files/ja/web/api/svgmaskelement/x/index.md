...
**`x`** は {{domxref("SVGMaskElement")}} インターフェイスの読み取り専用プロパティで、 {{SVGElement("mask")}} の {{SVGattr("x")}} 属性の値を含む {{domxref("SVGAnimatedLength")}} オブジェクトを返します。これは、マスク領域の左上隅の X 軸座標を表します。
...
{{domxref("SVGMaskElement.maskUnits")}} で定義された座標系における {{domxref("SVGAnimatedLength")}} オブジェクトです。このオブジェクトの `baseVal` プロパティは、 {{domxref("SVGLength")}} で `x` の値を返します。
