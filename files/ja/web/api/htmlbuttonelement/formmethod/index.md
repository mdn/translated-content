...
**`formMethod`** は {{domxref("HTMLButtonElement")}} インターフェイスのプロパティで、この {{HTMLElement("button")}} 要素がこのフォーム ({{HtmlElement("form")}}) を送信するコントロールである場合に、フォームを送信するコントロールである場合、送信に使用される {{Glossary("HTTP")}} メソッドです。これは、この `<button>` の [`formmethod`](/ja/docs/Web/HTML/Element/input#formmethod) 属性の値を反映します。
