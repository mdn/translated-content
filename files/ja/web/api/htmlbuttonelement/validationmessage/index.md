...
**`validationMessage`** は {{domxref("HTMLButtonElement")}} インターフェイスの読み取り専用プロパティで、（もしあれば）この {{htmlelement("button")}} コントロールが満たさない検証制約を記述したローカライズされたメッセージを表す文字列を返します。このコントロールが制約検証の対象ではない場合（`<button>` の [`type`](/ja/docs/Web/HTML/Element/button#type) が `button` または `reset` である場合）、または制約を満たしている場合は空文字列となります。
