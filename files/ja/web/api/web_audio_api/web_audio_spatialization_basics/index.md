...
> **メモ:** 単純な左右のステレオパンニング効果を作成する、一般的な用途に対処するために設計された {{domxref("StereoPannerNode")}} もあります。
...
> **メモ:** ヘッドフォンを使用するか、コンピューターに接続するサラウンドサウンドシステムがあれば、はるかに良い経験ができます。
...
最初に、移動させたい要素の参照を取得し、次に実際に移動させるために [CSS 座標変換](/ja/docs/Web/CSS/CSS_transforms)を設定するときに変更する値の参照を格納します。
...
> **メモ:** さまざまなブラウザー間で、音声の空間化には若干の違いがあります。
