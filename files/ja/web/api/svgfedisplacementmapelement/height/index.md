---
title: "SVGFEDisplacementMapElement: height プロパティ"
short-title: height
...
**`height`** は {{domxref("SVGFEDisplacementMapElement")}} インターフェイスの読み取り専用プロパティで、SVG フィルタープリミティブの垂直サイズを {{domxref("SVGAnimatedLength")}} で記述します。

これは {{SVGElement("feDisplacementMap")}} 要素の {{SVGAttr("height")}} フィルタープリミティブ属性を反映します。この属性は [`<length>`](/ja/docs/Web/SVG/Guides/Content_type#length) またはこのフィルター領域の高さに対する [`<percentage>`](/ja/docs/Web/SVG/Guides/Content_type#percentage) です。デフォルト値は `100%` です。このプロパティの値は、ユーザー座標系単位における長さです。
