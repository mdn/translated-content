...
**`formAction`** は {{domxref("HTMLInputElement")}} インターフェイスのプロパティで、このコントロールを含むフォームが送信された際にサーバー上で実行されるプログラムの URL です。これは、`<input>` の [`formaction`](/ja/docs/Web/HTML/Element/input#formaction) 属性の値を反映します。

このプロパティは [`submit`](/ja/docs/Web/HTML/Element/input/submit) および [`image`](/ja/docs/Web/HTML/Element/input/image) 型の `<input>` 要素にのみ有効です。
