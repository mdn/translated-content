...
**`formEnctype`** は {{domxref("HTMLInputElement")}} インターフェイスのプロパティで、`<input>` タグのformEnctype属性がフォーム送信の方法として指定されている場合、サーバーに送信されるコンテンツの {{Glossary("MIME_type", "MIME タイプ")}}です。これは、`<input>` の [`formenctype`](/ja/docs/Web/HTML/Element/input#formenctype) 属性の値を反映します。
