...
**`formTarget`** は {{domxref("HTMLInputElement")}} インターフェイスのプロパティで、送信される {{HtmlElement("form")}} のレスポンスが表示されるタブ、ウィンドウ、iframe です。これは、この {{HTMLElement("input")}} 要素の [`formtarget`](/ja/docs/Web/HTML/Reference/Elements/input#formtarget) 属性の値を反映します。
