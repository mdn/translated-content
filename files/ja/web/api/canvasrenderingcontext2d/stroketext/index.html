...
<p>Canvas 2D API の <code><strong>CanvasRenderingContext2D.strokeText()</strong></code> メソッドは、指定した位置 <em>(x, y)</em> にテキストの輪郭線を描画します。省略可能な第 4 引数である最大幅を与えると、この幅に収まるようにテキストを縮小します。</p>

<p>テキストを塗りつぶして描画するための {{domxref("CanvasRenderingContext2D.fillText()")}} メソッドもご覧ください。</p>
...
<h3 id="Using_the_strokeText_method" name="Using_the_strokeText_method"><code>strokeText</code> メソッドの使用例</h3>

<p><code>strokeText</code> メソッドを使用する、シンプルなコードスニペットです。</p>
