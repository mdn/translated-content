...
> [!NOTE]
> このインターフェイスは[保護されたコンテキスト](/ja/docs/Web/Security/Defenses/Secure_Contexts)の外でも使用可能ですが、{{domxref("BaseAudioContext.audioWorklet")}} プロパティはそうではありません。そのため、独自の {{domxref("AudioWorkletProcessor")}} は保護されたコンテキストの外では使用できません。
