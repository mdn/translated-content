...
- `options` {{optional_inline}}
  - : オブジェクトは次のものを含みます。
    ...
    - `partitioned` {{Optional_Inline}}
      - : 論理値で、既定値は `false` です。`true` に設定すると、設定された Cookie はパーティション化された Cookie になります。詳しくは[個別区画化された状態を持つCookie  (CHIPS)](/ja/docs/Web/Privacy/Guides/Third-party_cookies/Partitioned_cookies) を参照してください。
