---
title: "DOMMatrix: fromMatrix() 静的メソッド"
...
**`fromMatrix()`** は {{domxref("DOMMatrix")}} インターフェイスの静的メソッドで、新しい {{domxref("DOMMatrix")}} オブジェクトを、与えられた既存の行列またはそのプロパティの値を提供するオブジェクトから作成します。
...
この例では、オブジェクト内で行列値を指定することで `DOMMatrix` を作成します。
...
この例では、新しい `DOMMatrix` を既存の `DOMMatrix` から作成します。
...
- {{domxref("DOMMatrix.DOMMatrix", "DOMMatrix()")}} コンストラクター
