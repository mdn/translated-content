...
<p><strong><code>HTMLFieldSetElement</code></strong> インターフェイスは、 ({{domxref("HTMLElement")}} から継承したものより詳細に) {{ HTMLElement("fieldset") }} 要素のレイアウトと表示を操作するための特別なプロパティやメソッドを提供します。</p>
...
<dl>
 ...
 <dd> {{jsxref("Boolean")}} で、 HTML の {{htmlattrxref("disabled", "fieldset")}} 属性を反映し、ユーザーがこのコントロールと対話できるかどうかを示します。</dd>
 <dt>{{domxref("HTMLFieldSetElement.elements")}}{{ReadOnlyInline}}</dt>
 <dd>このフィールドセットに所属する要素です。このプロパティの型は、ブラウザーが実装している仕様書のバージョンによって異なります。</dd>
 <dt>{{domxref("HTMLFieldSetElement.form")}}{{ReadOnlyInline}}</dt>
 <dd> {{domxref("HTMLFormControlsCollection")}} または {{domxref("HTMLCollection")}} で、この要素がフォーム内にあった場合、含んでいるフォーム要素を参照します。<br>
 このフィールドセットがフォーム要素の子孫でなかった場合は、この属性は関連付けられた同じ文書内の何れかのフォームになるか、一致するものがなければ <code>null</code> になります。</dd>
 <dt>{{domxref("HTMLFieldSetElement.name")}}</dt>
 <dd> {{domxref("DOMString")}} で、 HTML の {{htmlattrxref("name", "fieldset")}} 属性を反映し、フォームを送信するときに使われるフィールドセットの名前が入ります。</dd>
 <dt>{{domxref("HTMLFieldSetElement.type")}}{{ReadOnlyInline}}</dt>
 <dd> {{domxref("DOMString")}} で "<code>fieldset</code>" です。</dd>
 ...
 <dd> {{jsxref("Boolean")}} で <code>false</code> です。 {{HTMLElement("fieldset")}} オブジェクトは制約検証の対象になることがないからです。</dd>
</dl>
...
<dl>
 ...
 <dd>常に <code>true</code> を返します。 {{HTMLElement("fieldset")}} オブジェクトは制約検証の対象になることがないためです。</dd>
...
<table class="standard-table">
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><code>disabled</code>, <code>elements</code>, <code>name</code>, <code>type</code>, <code>valdiationMessage</code>, <code>validity</code>, and <code>willValidate</code> プロパティが追加された。<br>
    <code>checkValidity()</code>, <code>setCustomValidity()</code> メソッドが追加された。</td>
  </tr>
  <tr>
...
<ul>
 <li>このインターフェイスを実装している HTML 要素: {{ HTMLElement("fieldset") }}</li>
</ul>
