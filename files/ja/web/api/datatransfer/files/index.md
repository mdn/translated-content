...
> [!NOTE]
> [`DataTransfer`](/ja/docs/Web/API/DataTransfer) オブジェクトの `files` プロパティは、 `drop` イベントの中からのみアクセスできます。それ以外のどのイベントでも、 `files` プロパティは空になります。その基礎となるデータストアが[保護モード](https://html.spec.whatwg.org/multipage/dnd.html#the-drag-data-store)になるからです。
