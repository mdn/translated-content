...
**`HTMLCanvasElement.height`** プロパティは CSS ピクセル数として解釈される正の整数であり、この {{HTMLElement("canvas")}} 要素の [`height`](/ja/docs/Web/HTML/Reference/Elements/canvas#height) 属性を反映します。この属性が指定されなかった場合、または負の数など、無効な値が設定された場合、既定値の `150` が使用されます。
