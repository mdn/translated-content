...
<div class="hidden"><p>{{event("loadend")}}は英語版では {{domxref("XMLHttpRequest/loadend_event", "loadend")}} にリダイレクトされますが、日本語版ではリダイレクト元ページが存在しない（作成も不可）ため、日本語版ではリダイレクト先を直接記述しています。</p></div>
