...
リンクの既定の CSS プロパティがいくつもあることが分かりましたか？ JavaScript の 2 行目を更新して、 {{htmlelement("a")}} ではなく {{htmlelement("p")}} を選択するようにしてみてください。{{cssxref("margin-top")}} と {{cssxref("margin-bottom")}} の既定の計算値の違いに気が付くでしょう。
...
カスタムプロパティはスタイルシートに書かれている通りの値を保持しますが、計算されたスタイルは計算値として表示されることに注意してください。 {{cssxref('color')}} は [`rgb()`](/ja/docs/Web/CSS/Reference/Values/color_value) 値として表示され、 {{cssxref('font-weight')}} は[名前付き色](/ja/docs/Web/CSS/Reference/Values/named-color)を使用しても `bold` を使用しても `700` と返されます。
...
- {{cssxref("image")}} は {{domxref('CSSImageValue')}} を返します。
- {{cssxref("&lt;color&gt;")}} は {{domxref('CSSStyleValue')}} を返します。
...
```html-nolint hidden
<p>
  ここには何も表示されません。出力内容を確認するにはブラウザーのコンソールを開いてください！
</p>
```
...
次の例では、 `allComputedStyles` を参照しています。

### CSSUnparsedValue

{{domxref('CSSUnparsedValue')}} は[カスタムプロパティ](/ja/docs/Web/CSS/Guides/Cascading_variables/Using_custom_properties)を表しています。
...
[`<button>`](/ja/docs/Web/HTML/Reference/Elements/button) 要素は既定ではインライン要素ですが、サイズ調整を可能にするために [`display: inline-block;`](/ja/docs/Web/CSS/Guides/Display) を追加しています。 CSS では、`width: calc(30% + 20px);` を指定していますが、これは {{cssxref("calc()")}} という関数で幅を定義しています。
...
また、 [`display: inline-block;`](/ja/docs/Web/CSS/Guides/Display) により座標変換もできるようになります。私たちの CSS では、 `transform: scale(0.95);` が {{cssxref('transform')}} 関数です。
...
`'background-image'` を `get()` すると、 {{domxref('CSSImageValue')}} が返されます。 CSS の {{cssxref('background')}} 一括指定プロパティを使っていますが、継承された {{jsxref("Object/toString", "Object.prototype.toString()")}} メソッドは、画像 `'url("magicwand.png")'` のみを返していることが分かります。
...
最後に、これらすべてを1つのライブサンプルに用意します。ブラウザーのコンソールを使用して出力を確認することを忘れないでください。

{{EmbedLiveSample("CSSStyleValue", 120, 300)}}

### まとめ

これで、 CSS 型付きオブジェクトモデルを理解することができるはずです。さらに学ぶために[CSS 型付きオブジェクトモデル](/ja/docs/Web/API/CSS_Typed_OM_API)の全てのインターフェイスを見てみてください。
