...
アテステーションの目的は、新しく生成された鍵ペアが特定の端末から発行されたことを暗号学的に証明することです。これにより、新しく生成された鍵ペアに対する信頼の根拠が提供されると同時に、使用されている端末の属性（秘密鍵の保護方法、生体認証が使用されるかどうかとその種類、端末が認証済みかどうかなど）を特定することが可能になります。アテステーションは信頼の根拠を確立する機能を提供する一方で、その信頼の根拠自体を検証する必要性はあまりない点に留意すべきである。新規アカウントの認証器登録時には通常、まず使用時認証 (TOFU) モデルが適用され、既存アカウントへの認証器追加時にはユーザーは既に認証済みであり、安全なセッションが確立されています。
