...
この記事では、{{glossary("Identity provider", "アイデンティティプロバイダー")}} (IdP) が、Federated Credential Management (FedCM) API と連携するために必要なすべての手順について詳しく説明します。
...
- `id_assertion_endpoint`
  - : ID アサーションエンドポイントの URL。ここに有効なユーザー資格情報を送信すると、RP が認証の検証に使用することができる検証トークンが返されます。
- `login_url`
...
### ログイン状態がフェデレーションログインのフローに与える影響

[RP がフェデレーションログインを試行する](/ja/docs/Web/API/FedCM_API/RP_sign-in)際、ログイン状態を調べます。
...
ログイン状態 API は IdP のログイン状態をブラウザーに通知しますが、ブラウザーと IdP の状態が同期しなくなる可能性があります。例えば、IdP のセッションが期限切れになると、すべてのユーザーアカウントがログアウトされた状態になるにもかかわらず、ログイン状態は `"logged-in"` のままになる場合があります（アプリケーションがログイン状態を `"logged-out"` に設定できなかったため）。このような場合、フェデレーションログインを試みると、IdP のアカウント一覧エンドポイントへのリクエストが行われますが、セッションが利用できなくなったため、利用できるアカウントは返されません。
