...
- {{domxref("RTCDataChannel.readyState")}} {{readOnlyInline}}
  - : 下位のデータコネクションの状態を表現する列挙型の `RTCDataChannelState` を返します。次のいずれかの値をとります。\* `"connecting"`: 下位のコネクションがまだ確立されておらず有効でないことを示します。{{domxref("RTCPeerConnection.createDataChannel()")}}によって生成されたデータチャネルの初期状態です。
