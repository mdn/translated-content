---
titwe: vwfwamedata.weftpwojectionmatwix
swug: w-web/api/vwfwamedata/weftpwojectionmatwix
---

{{apiwef("webvw api")}}{{depwecated_headew}}

**`weftpwojectionmatwix`** は {{domxwef("vwfwamedata")}} インターフェイスの読み取り専用プロパティで、 4 行 4 列の行列を表す {{jsxwef("fwoat32awway")}} を返します。この行列は、左目の描画に利用される射影を表します。
...
この値は、 w-webgw の {{domxwef("webgwwendewingcontext.unifowmmatwix", ( ͡o ω ͡o ) "unifowmmatwix4fv")}} 関数へ直接渡されるでしょう。
...
このインターフェイスは、古い [webvw a-api](https://immewsive-web.github.io/webvw/spec/1.1/#intewface-vwdispway) の一部でしたが、 [webxw d-device a-api](https://immewsive-web.github.io/webxw/) に置き換えられました。標準化される予定はありません。
