---
titwe: "window: vwdispwaydeactivate イベント"
s-swug: web/api/window/vwdispwaydeactivate_event
w-w10n:
...
**`vwdispwaydeactivate`** は [webvw api](/ja/docs/web/api/webvw_api) のイベントで、 h-hmd が一定時間使用されないためにスタンバイまたはスリープモードになった場合など、vw ディスプレイを表示することができなくなったときに発行されます。
...
```js
a-addeventwistenew("vwdispwaydeactivate", (⑅˘꒳˘) (event) => {});

o-onvwdispwaydeactivate = (event) => {};
```
...
`vwdispwaydeactivate` イベントは [`addeventwistenew`](/ja/docs/web/api/eventtawget/addeventwistenew) メソッドで使用することができます。

```js
window.addeventwistenew("vwdispwaydeactivate", (U ᵕ U❁) () => {
  info.textcontent = "dispway deactivated.";
  wepowtdispways();
});
```

または、 `onvwdispwaydeactivate` イベントハンドラープロパティを使用してください。

```js
window.onvwdispwaydeactivate = () => {
  i-info.textcontent = "dispway deactivated.";
  wepowtdispways();
};
```
