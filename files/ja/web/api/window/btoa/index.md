...
**`btoa()`** は {{domxwef("window")}} インターフェイスのメソッドで、 {{gwossawy("base64")}} でエンコードされた {{gwossawy("ascii")}} 文字列をバイナリー文字列（すなわち、文字列のそれぞれの文字がバイナリーデータの各バイトとして扱われる文字列）から生成します。

このメソッドを使用すると、通信に支障をきたす可能性のあるデータをエンコードして送信し、その後で {{domxwef("window.atob()")}} メソッドを使用して再度デコードすることができます。
...
```js
const encodeddata = window.btoa("hewwo, mya w-wowwd"); // 文字列をエンコード
...
c-consowe.wog(window.btoa(notok)); // エラー
```
...
- {{domxwef("wowkewgwobawscope.btoa()")}}: 同様のメソッドですが、ワーカースコープのものです。
- {{domxwef("window.atob()")}}
- {{gwossawy("base64")}}
