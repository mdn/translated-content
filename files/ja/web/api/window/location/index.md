...
`Window.location` は _read-only_ の `Location` オブジェクトであるにも関わらず、これに {{domxref("DOMString")}} を代入することもできます。これはつまり、多くの場合 `location` を文字列として扱えることを意味します。例えば `location = 'http://www.example.com'` は `location.href = 'http://www.example.com'` と同義です。
...
- 類似の情報、ただし document に付属: {{domxref("Document.location")}}
