...
- {{domxref("DOMMatrixReadOnly.isIdentity")}} {{ReadOnlyInline}}
  - : 行列が[恒等行列](https://ja.wikipedia.org/wiki/恒等行列)である場合に `true` となる論理値です。
...
- {{domxref("DOMMatrixReadOnly.multiply()")}}
  - : 指定した行列と入力行列の積を計算して作成した、新しい {{domxref("DOMMatrix")}} を返します。元の行列は変更されません。
...
- {{domxref("DOMMatrixReadOnly.toFloat32Array()")}}
  - : 単精度浮動小数点数の新しい {{jsxref("Float32Array")}} に、行列を構成する全 16 要素を入れたものを返します。
- {{domxref("DOMMatrixReadOnly.toFloat64Array()")}}
  - : 倍精度浮動小数点数の新しい {{jsxref("Float64Array")}} に、行列を構成する全 16 要素を入れたものを返します。
...
- {{domxref("DOMMatrixReadOnly.fromFloat32Array_static", "fromFloat32Array()")}}
  - : 新しい `DOMMatrixReadOnly` オブジェクトを作成し、6 個または 16 個の単精度（32 ビット）浮動小数点値の {{jsxref("Float32Array")}} で指定された値で初期化します。
- {{domxref("DOMMatrixReadOnly.fromFloat64Array_static", "fromFloat64Array()")}}
  - : 新しい `DOMMatrixReadOnly` オブジェクトを作成し、6 個または 16 個の単精度（64 ビット）浮動小数点値の {{jsxref("Float64Array")}} で指定された値で初期化します。
