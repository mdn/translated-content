...
- options {{optional_inline}}
  - : `match` 操作のオプションを設定するオブジェクト。 使用可能なオプションは次のとおりです。\* `ignoreSearch`: URL のクエリ文字列を無視するかどうかを指定する {{jsxref("Boolean")}}。 例えば、`true` に設定すると、`http://foo.com/?value=bar` の `?value=bar` の部分は、照合の実行時に無視されます。 デフォルトは `false` です。
