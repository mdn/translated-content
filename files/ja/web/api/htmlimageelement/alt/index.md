...
おそらく `alt` プロパティを使用する最も重要な理由は、[アクセシビリティ](/ja/docs/Web/Accessibility)に対応することです。なぜなら `alt` テキストは画面リーダーや他の支援技術によって使用され、障碍のあるユーザーがコンテンツを十分に活用できるようサポートするからです。
...
{{Glossary("SVG")}} 形式で表示される概念図やグラフに含まれるテキストは、画面リーダーで読み取られる可能性があることに留意してください。
