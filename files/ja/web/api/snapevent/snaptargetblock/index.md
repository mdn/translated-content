...
`SnapEvent` で利用可能なプロパティ値は、スクロールコンテナ－に設定された {{cssxref("scroll-snap-type")}} プロパティの値に直接対応します。 `snapTargetBlock` は、スナップ軸が `block` （または、現在の書字方向で `block` に相当する物理軸値の場合）または `both` と指定されている場合、要素参照のみを返します。
