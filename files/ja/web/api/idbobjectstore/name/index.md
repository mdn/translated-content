---
title: "IDBObjectStore: name プロパティ"
...
{{domxref("IDBObjectStore")}} インターフェイスの **`name`** プロパティは、このオブジェクトストアの名前を表します。
...
以下のコード断片では、データベースの読み書きトランザクションを開き、`add()` を用いてデータをオブジェクトストアに追加します。オブジェクトストアの生成後、コンソールに `objectStore.name` を記録します。動く例全体は、[To-do Notifications](https://github.com/mdn/dom-examples/tree/main/to-do-notifications) アプリケーションを参照してください。([動く例を見る](https://mdn.github.io/dom-examples/to-do-notifications/))
