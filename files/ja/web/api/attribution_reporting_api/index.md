...
自分のサイトで帰属レポート API を使用するには、[プライバシーサンドボックス登録プロセス](/ja/docs/orphaned/Web/Privacy/Guides/Privacy_sandbox/Enrollment)でそれを指定する必要があります。これを行わないと、 API フローはレスポンス時点でブロックされ、レスポンスヘッダーが無視され、ソースとトリガーが登録されません。
