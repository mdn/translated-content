...
<p><a href="/ja/docs/Web/API/Intersection_Observer_API">Intersection Observer API</a> は、{{domxref("Document")}} 自体を含む共有祖先ノードまたは要素によって、対象の要素が多かれ少なかれ不明瞭になったときに、非同期的に通知されることを容易にします。 <span class="seoSummary">この記事では、ページのコンテンツ間に多数の広告が散在している模擬ブログを作成し、その後 Intersection Observer API を使用して、各広告がユーザに見える時間を追跡します。表示時間の1分を超える広告は、新しい広告と置き換えられます。</span></p>

<p>この例の多くの側面は実際の使用状況とは一致しません (特に記事はすべて同じテキストを持ち、データベースからは読み込まれません。また、配列から選択された単なるテキストのみの広告がほんの一握りです )、これは API の十分な理解を提供して、あなたのサイトに Intersection Observer API をどのように適用するかを素早く学ぶべきです。</p>

<p>この例で、広告の視認性を追跡するという概念が使用されているのには理由があります。Web 上での広告の Flash やその他のスクリプトの最も一般的な用途の1つは、課金と収入のために各広告が表示される期間を記録することです。Intersection Observer API がなければ、個々の広告の間隔やタイムアウト、またはページを遅くする傾向があるその他の手法を使用して完了します。この API を使用することで、ブラウザによる効率化が図られ、パフォーマンスへの影響が大幅に軽減されます。</p>
...
<h2 id="CSS_によるサイトのスタイル設定">CSS によるサイトのスタイル設定</h2>
...
<h2 id="JavaScript_と連携させる">JavaScript と連携させる</h2>
...
<h3 id="定期的なアクションの処理">定期的なアクションの処理</h3>

<p>Our interval handler, <code>handleRefreshInterval()</code>, is called about once per second courtesy of the call to {{domxref("WindowOrWorkerGlobalScope.setInterval", "setInterval()")}} made in the <code>startup()</code> function <a href="#setting_up">described above</a>. Its main job is to update the timers every second and schedule a redraw to update the timers we'll be drawing within each ad.</p>
...
<h3 id="広告の視認性タイマーを更新する">広告の視認性タイマーを更新する</h3>
...
<h3 id="広告のタイマーを描画する">広告のタイマーを描画する</h3>
...
<h3 id="ページコンテンツの構築">ページコンテンツの構築</h3>
...
<h3 id="既存の広告を置き換える">既存の広告を置き換える</h3>
...
<p>結果のページは次のようになります。スクロールして試してみて、視認性の変化が各広告のタイマーに与える影響を見てみましょう。 また、1分の視認性と、ドキュメントがバックグラウンドでタブ表示されている間にタイマーが一時停止すると、各広告が置き換えられます。</p>
...
<ul>
 ...
 <li><a href="/ja/docs/Web/Guide/User_experience/Using_the_Page_Visibility_API">Page Visibility API</a></li>
</ul>
