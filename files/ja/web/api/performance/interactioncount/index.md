...
これは {{Glossary("Interaction_to_next_paint", "Interaction to Next Paint (INP)")}} を算出する際に有用で、特に長時間表示され続けるページにおける外れ値を除外するために役立ちます。INP はページ上のインタラクションの 98 パーセンタイル値を採用するため、50回のインタラクションのうち 1 回を、ページ全体の応答性を反映しない「外れ値」として除外します。
...
多数のインタラクションがあるページでは、以下のパターンを使用して、 50 回ごとに 1 回の外れ値を除外した上でINPを再計算できます。
