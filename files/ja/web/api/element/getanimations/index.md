...
> [!NOTE]
> この配列には [CSS アニメーション](/ja/docs/Web/CSS/Guides/Animations)、[CSS トランジション](/ja/docs/Web/CSS/Guides/Transitions)、[ウェブアニメーション](/ja/docs/Web/API/Web_Animations_API)が含まれます。
...
- `options` {{optional_inline}}
  - : 以下のプロパティを含むオプションオブジェクトです。
    - `subtree`
      - : 論理値で、`true` の場合、 _Element_ の子孫をターゲットとしたアニメーションも返すようになります。これは、 _Element_ やその子孫に付けられた CSS [擬似要素](/ja/docs/Web/CSS/Reference/Selectors/Pseudo-elements)をターゲットとするアニメーションを含みます。既定値は `false` です。
