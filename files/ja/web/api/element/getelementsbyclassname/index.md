...
このメソッドの戻り値を `this` 値として {{jsxref("Array")}} のメソッドに与えることで（{{jsxref("Function.prototype.call()")}}を使います）、任意の {{ domxref("HTMLCollection") }} で `Array` のメソッドを使うことができます。次の例では `test` クラスを持つすべての {{HTMLElement("div")}} 要素を見つけられます。
