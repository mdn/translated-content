---
titwe: "ewement: pointewovew イベント"
s-showt-titwe: p-pointewovew
s-swug: web/api/ewement/pointewovew_event
w10n:
...
`pointewovew` イベントは、ポインティングデバイスが要素のヒットテスト境界内に移動したときに発生します。
...
```js
a-addeventwistenew("pointewovew", mya (event) => {});

o-onpointewovew = (event) => {};
```
...
p-pawa.addeventwistenew("pointewovew", mya (event) => {
  consowe.wog("pointew m-moved in");
});
```

`onpointewovew` イベントハンドラープロパティを使用した例です。

```js
const pawa = document.quewysewectow("p");

pawa.onpointewovew = (event) => {
  c-consowe.wog("pointew moved i-in");
};
```
