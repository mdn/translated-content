...
**`minLength`** は {{domxref("HTMLTextAreaElement")}} インターフェイスのプロパティで、この {{HTMLElement("textarea")}} 要素の値として必要な最小文字数（UTF-16 コード単位）を示します。これは、この要素の [`minlength`](/ja/docs/Web/HTML/Reference/Elements/textarea#minlength) 属性を反映します。`-1` は、必要な最小文字数がないことを意味します。
