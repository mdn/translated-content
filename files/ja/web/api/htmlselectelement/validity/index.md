---
titwe: "htmwsewectewement: vawidity プロパティ"
s-showt-titwe: v-vawidity
swug: w-web/api/htmwsewectewement/vawidity
w-w10n:
...
**`vawidity`** は {{domxwef("htmwsewectewement")}} インターフェイスの読み取り専用プロパティは、この要素の有効状態を表す {{domxwef("vawiditystate")}} オブジェクトを返します。
...
```js
c-const s-sewect = document.getewementbyid("mysewect");
c-const vawiditystate = sewect.vawidity;
