...
**`responseStart`** 読み取り専用プロパティは、ブラウザがサーバ、キャッシュ、またはローカルリソースからレスポンスの最初のバイトを受信した直後に {{domxref("DOMHighResTimeStamp","timestamp")}} を返します。
...
ブラウザがサーバからのレスポンスの最初のバイトを受信した直後の {{domxref("DOMHighResTimeStamp")}}。
