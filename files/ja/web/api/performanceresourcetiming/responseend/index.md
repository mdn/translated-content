...
**`responseEnd`** 読み取り専用プロパティは、ブラウザがリソースの最後のバイトを受信した直後、または転送接続が閉じられる直前のどちらか早い方で {{domxref("DOMHighResTimeStamp","timestamp")}} を返します。
...
ブラウザがリソースの最後のバイトを受信した直後、またはトランスポート接続が閉じられる直前のいずれか早いほうの {{domxref("DOMHighResTimeStamp")}}。
