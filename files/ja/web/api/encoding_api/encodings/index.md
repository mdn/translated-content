...
[エンコーディング API](/ja/docs/Web/API/Encoding_API) のインターフェイスである {{domxref("TextDecoder")}} と {{domxref("TextDecoderStream")}} のコンストラクターには、オプションとして `label` を渡し、用いるエンコーディングを示すことができます。
既定値は `UTF-8` です。
