...
<dl>
 ...
 <dd>描画するプリミティブの種類を指定する {{domxref("GLenum")}}。以下の値を取ることができます。
 <ul>
  <li><code>gl.POINTS</code>: 単一の点を描画します。</li>
  <li><code>gl.LINE_STRIP</code>: 次の線へと直線を描画します。</li>
  <li><code>gl.LINE_LOOP</code>: 次の線へと直線を描画し、最後の頂点は最初のものに接続します。</li>
  <li><code>gl.LINES</code>: 頂点 2 つごとに、その間に線を描画します。</li>
  ...
  <li><code>gl.TRIANGLES</code>: 頂点 3 つの集まりごとに、三角形を描画します。</li>
 </ul>
 </dd>
 <dt>first</dt>
 <dd>頂点ベクトルの配列の開始インデックスを指定する {{domxref("GLint")}}。</dd>
 <dt>count</dt>
 <dd>描画されるインデックスの数を指定する {{domxref("GLsizei")}}。</dd>
</dl>
...
<ul>
 <li><code>mode</code> が許容された値のどれでもない場合、<code>gl.INVALID_ENUM</code> エラーがスローされます。</li>
 <li><code>first</code> や <code>count</code> が負数の場合、<code>gl.INVALID_VALUE</code> エラーがスローされます。</li>
 <li><code>gl.CURRENT_PROGRAM</code> が {{jsxref("null")}} の場合、<code>gl.INVALID_OPERATION</code> エラーがスローされます。</li>
</ul>
