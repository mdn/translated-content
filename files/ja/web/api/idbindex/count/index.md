...
{{domxref("IDBIndex")}} インターフェイスの **`count()`** メソッドは、{{domxref("IDBRequest")}} オブジェクトを返し、別スレッドでキー範囲内のレコードの数を返します。
...
操作に成功した場合は、この要求の {{domxref("IDBRequest.result", "result")}} プロパティの値は与えられたキーもしくはキー範囲にマッチするレコードの数になります。
