...
- `aria-atomic` は設定されていません (デフォルトの `false` が入ります) ので、追加または削除されたユーザーのみが読み上げられ、全体の roster は都度読み上げられないはずです。
- `aria-relevant="additions removals"` により roaster に追加もしくは削除されたユーザーが読み上げられることを確かにします。
