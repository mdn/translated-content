...
<p><code>aria-describedby</code> 属性はフォーム要素にのみ使用されるものではありません。 静的テキストをウィジェット、要素のグループ、ペイン、見出しを持つ領域、定義等々に関連付けるためにも使用されます。 以下の<a href="#examples">例</a>のセクションでは、これらの場合に属性を使用する方法の詳細について説明します。</p>
