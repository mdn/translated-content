...
{{InteractiveExample("JavaScript デモ: 小なりイコール演算子 (<=)")}}
...
オペランドは、[小なり](/ja/docs/Web/JavaScript/Reference/Operators/Less_than)演算子と同じアルゴリズムを使用して、オペランドを入れ替えて比較され、結果が反転されます。 `x <= y` は通常 `!(y < x)` と同等ですが、 `x <= y` と `x > y` の両方が `false` となる 2 つの例外があります。
