...
`--` 演算子は、数値と[長整数](/ja/docs/Web/JavaScript/Reference/Global_Objects/BigInt)の 2 種類のオペランドに対してオーバーロードされています。最初に[オペランドを数値の値に変換](/ja/docs/Web/JavaScript/Guide/Data_structures#数値変換)し、その型を検査します。オペランドが長整数になった場合、長整数のデクリメントを実行し、そうでない場合は数値のデクリメントを実行します。
