...
大きさだけでなく、入力の_精度_も考慮することが重要です。例えば、数値がフォーム入力から収集され、入力値が `0.1` のステップでしか調整できない場合（すなわち [`<input type="number" step="0.1">`](/ja/docs/Web/HTML/Attributes/step)）、データは `0.1` の精度しかないので通常は `0.01` など、はるかに大きな許容差を認めることを意味します。
