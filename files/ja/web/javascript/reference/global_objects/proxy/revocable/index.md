...
`revoke` 関数は引数を取らず、`this` 値にも頼りません。作成された `proxy` オブジェクトは `revoke` 関数に[プライベートプロパティ](/ja/docs/Web/JavaScript/Reference/Classes/Private_elements)として結び付けられ、`revoke` 関数が呼び出されたときに自分自身でアクセスします（プライベートプロパティの存在は外からは観測できませんが、ガーベジコレクションが発生する方法については影響があります）。`proxy` オブジェクトは `revoke` 関数の[クロージャ](/ja/docs/Web/JavaScript/Guide/Closures)内には収容されません（`revoke` がまだ生きている場合、`proxy` のガベージコレクションは不可能になります）。
