...
**`Float16Array()`** コンストラクターは、新しい {{jsxref("Float16Array")}} オブジェクトを作成します。初期化データが明示的に指定されていない場合、内容は `0` に初期化されます。
...
> [!NOTE]
> `Float16Array()` は [`new`](/ja/docs/Web/JavaScript/Reference/Operators/new) 付きでのみ構築できます。`new` なしで呼び出そうとすると、{{jsxref("TypeError")}} が発生します。
