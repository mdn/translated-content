...
**`Atomics.compareExchange()`** は静的メソッドで、指定した期待値と配列の指定した位置にあった値が等しければ、配列の値を指定した置換値と交換します。返値は、交換が成功したかにかかわらず、配列に元々あった値です。これは不可分操作で、変更された値が書き戻されるまで、他の書き込みが行われないことが保証されます。
