...
このプロパティは、文字列内のコード単位の数を返します。 JavaScript で使用される文字列書式である {{interwiki("wikipedia", "UTF-16")}} は、ほとんどの一般の文字は単一の 16 ビットコードユニットで表しますが、あまり使われない文字に対しては 2 つのコードユニットを使用する必要があり、 `length` で返される値が文字列の実際の文字数と一致しなくなる可能性があります。
