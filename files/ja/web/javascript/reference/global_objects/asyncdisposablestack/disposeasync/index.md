...
{{jsxref("AsyncDisposableStack")}} インスタンスの **`disposeAsync()`** メソッドは、登録されているすべてのディスポーザーを登録順とは逆の順序で呼び出し、各ディスポーザーの完了を待ってから次のディスポーザーを呼び出すことで、このスタックを破棄します。スタックがすでに解放されている場合、このメソッドは何も行いません。
