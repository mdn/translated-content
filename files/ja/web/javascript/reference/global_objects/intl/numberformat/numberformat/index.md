...
丸める_way_は、[`roundingMode`](#roundingmode) プロパティによって決まります（詳細は[丸めモード](#丸めモード)の章で指定されています）。
...
値の小数点以下の桁数がコンストラクターのオプションで指定された数を超える場合、書式化された値は指定された小数点以下の桁数に丸められます。
値が丸められる_方法_は、[`roundingMode`](#roundingmode) プロパティによって決まります。
