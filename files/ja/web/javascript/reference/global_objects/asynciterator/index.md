...
> [!NOTE]
> `AsyncIterator.prototype` は [`[Symbol.iterator]()`](/ja/docs/Web/JavaScript/Reference/Global_Objects/Symbol/iterator) を実装していないので、非同期イテレーターは既定では[同期反復可能](/ja/docs/Web/JavaScript/Reference/Iteration_protocols#反復可能プロトコル)ではありません。
...
- [`AsyncIterator.prototype[Symbol.asyncDispose]()`](/ja/docs/Web/JavaScript/Reference/Global_Objects/AsyncIterator/Symbol.asyncDispose)
  - : `this` の `return()` メソッドが存在する場合、それを呼び出して待機します。これにより非同期破棄可能プロトコルが実装され、{{jsxref("Statements/await_using", "await using")}} と組み合わせて使用されている際に破棄することができるようになります。
