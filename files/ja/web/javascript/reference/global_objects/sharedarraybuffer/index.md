...
{{jsxref("SharedArrayBuffer")}} オブジェクトを使用して、クラスター内のあるエージェント（エージェントとは、ウェブページのメインプログラムまたはそのウェブワーカーのひとつ）から別のエージェントへメモリーを共有するためには、[`postMessage`](/ja/docs/Web/API/Worker/postMessage) と[構造化クローン](/ja/docs/Web/API/Web_Workers_API/Structured_clone_algorithm)を使用します。
