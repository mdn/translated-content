...
このコードは国際化対応していません。言語によっては、リストの区切り文字がカンマではない場合があります。ほとんどの言語（英語を含む）では、最後のアイテムの前に接続詞が必要です。しかし、単に手動で "and" を追加しただけでは、すべての英語話者にとって正しいとは限りません。なぜなら、英語には[オックスフォードカンマ](https://en.wikipedia.org/wiki/Serial_comma)に関する議論があるからです。"apple, banana, and cherry" と "apple, banana and cherry" のどちらが正しいかという問題です。
