...
> また、ファビコンでの利用は {{SectionOnPage("/ja/docs/Web/HTML/Element/link", "Providing icons for different usage contexts")}} で述べたように、 PNG ファイルと {{HTMLElement("link")}} 要素を使用することで抑制されています。
...
SVG ファイルはテキストファイルで、解釈されると希望する画像が描画されるソースコードを含んでいる。
...
{{interwiki("wikipedia", "Tagged Image File Format", "TIFF")}} はスキャンした写真を格納するために作成されたラスターファイル形式ですが、あらゆる種類の画像に使用使用することができます。
        ...
        <table class="standard-table">
          <tbody>
            ...
            <tr>
              ...
              <td>4 および 8</td>
              <td>
                各画素は <code>ColorMap</code> レコードのインデックスであり、画像に使用される色を定義するものである。カラーマップは、赤の値すべて、緑の値すべて、青の値すべての順に並びます（<code>rgb, rgb, rgb...</code> ではなく）。
              </td>
            </tr>
            <tr>
              <th scope="row">アルファ付きグレースケール</th>
