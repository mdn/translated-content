...
最近のウェブアプリは、ウェブアプリマニフェストに設定されたプロパティや、最近のスマートフォンブラウザーで利用できる[ウェブアプリのインストール](/ja/docs/Web/Progressive_web_apps/Guides/Installing)と呼ばれる機能によって、このようなネイティブアプリの感覚を持つことができます。
...
レスポンシブウェブアプリでは、[メディアクエリー](/ja/docs/Web/CSS/CSS_media_queries)や[ビューポート](/ja/docs/Glossary/Viewport)などの技術を用いて、デスクトップ、モバイル、タブレットなど、あらゆるフォームファクターに対応する UI を実現しています。
...
[ウェブアプリマニフェスト](/ja/docs/Web/Manifest)や[プッシュ通知](/ja/docs/Web/API/Push_API)、[お知らせ](/ja/docs/Web/API/Notifications_API)、[ホーム画面に追加](/ja/docs/Web/Progressive_web_apps/Guides/Making_PWAs_installable)などの他の機能も幅広く対応されています。現在、Safari では、ウェブアプリマニフェストとホーム画面に追加の対応が制限されており、ウェブプッシュ通知には対応していません。ただし、他の主要なブラウザーはこれらすべての機能に対応しています。
