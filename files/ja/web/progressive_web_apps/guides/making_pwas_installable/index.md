...
- [`prefer_related_applications`](/ja/docs/Web/Progressive_web_apps/Manifest/Reference/prefer_related_applications) が `false` または存在してはいけない

それぞれのメンバーの完全な説明は、[ウェブアプリマニフェスト](/ja/docs/Web/Progressive_web_apps/Manifest)のリファレンスドキュメントを参照してください。
...
- [PWA を Meta Quest ストアで公開する方法](https://developers.meta.com/horizon/resources/publish-submit/)
...
- Safari は Add to Dock (_File_ > _Add to Dock..._) に macOS Sonoma (Safari 17) 以降で対応しており、マニフェストファイルの有無にかかわらずあらゆるウェブアプリで対応しています。
- Firefox は、マニフェストファイルを使用した PWA のインストールに対応していません。
...
PWA は、既定ではブラウザーが提供する UI に頼っていますが、ユーザーがインストールプロンプトを開くためのページ内 UI を自分で提供することもできます。これにより、PWA はユーザーが PWA をインストールするためのコンテキストと理由を提供することができ、インストールユーザーフローを発見しやすくすることができます。

このテクニックは、 [`beforeinstallprompt`](/ja/docs/Web/API/Window/beforeinstallprompt_event) イベントを利用するものです。このイベントは、ブラウザーが PWA がインストール可能であると判断するとすぐに、グローバルな [`Window`](/ja/docs/Web/API/Window) オブジェクト上で発行されます。このイベントにはインストールプロンプトを表示させる [`prompt()`](/ja/docs/Web/API/BeforeInstallPromptEvent/prompt) メソッドがあります。つまり PWA は、
...
既定では、インストールプロンプトには PWA の名前とアイコンが表示されます。マニフェストメンバーに [`description`](/ja/docs/Web/Progressive_web_apps/Manifest/Reference/description) と [`screenshots`](/ja/docs/Web/Progressive_web_apps/Manifest/Reference/screenshots) の値が指定された場合、 Android のみこれらの値がインストールプロンプトに表示され、ユーザーに PWA をインストールする余分なコンテキストと動機を与えます。
