...
ブラウザーがアイコン画像を取得する能力は、マニフェストのオーナー文書内のコンテンツセキュリティポリシー ({{Glossary("CSP")}}) によって管理されており、特に [`img-src`](/ja/docs/Web/HTTP/Headers/Content-Security-Policy/img-src) ディレクティブによって管理されています。このセキュリティの側面は、 `src` プロパティに関連しています。
...
`type` プロパティを指定しなかった場合、ブラウザーはよりリソースに集中した方法、たとえばファイルのシグネチャを [MIME スニッフィング](/ja/docs/Web/HTTP/MIME_types#mime_sniffing)などを使用する必要があるかもしれません。
