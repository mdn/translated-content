...
![クライアントとサーバーのライフライン間の HTTP メッセージを説明するシーケンス図。](basic-auth.svg)
...
HTTP の "Basic" 認証方式は {{rfc(7617)}} で定義されており、資格情報を送信するのに、ユーザー ID とパスワードの組を base64 を使用してエンコードします。

### Basic 認証の安全性

ユーザー ID とパスワードは、ネットワークを介してクリアテキストとして渡されるため（base64 でエンコードされますが、 base64 は可逆エンコードです）、Basic 認証方式は**安全ではありません**。
資格情報の傍受を防ぐためには、HTTPS/TLS を使用する必要があります。

さらに、HTTP の Basic 認証が使用されているサイトは、特に[クロスサイトリクエストフォージェリ (CSRF)](/ja/docs/Glossary/CSRF) 攻撃に対して脆弱です。これは、ユーザー資格情報が送信元に関係なくすべてのリクエストで送信されるためです（これはクッキーベースの認証メカニズムとは異なります。クッキーはサイト間リクエストで一般的にブロックされるためです）。
データを変更する際は、ウェブサイトは常に POST リクエストを使用し、[CSRF トークン](/ja/docs/Web/Security/Attacks/CSRF)を付加する必要があります。

これらの追加のセキュリティ強化機能がない場合は、機密情報や重要な情報を保護するために Basic 認証を使用しないでください。
...
- [HTTP セキュリティのベストプラクティス](/ja/docs/Web/Security)
