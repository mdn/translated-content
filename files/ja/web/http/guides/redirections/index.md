...
**URL リダイレクト**は、「URL 転送」とも呼ばれ、ページ、フォーム、ウェブサイト全体、ウェブアプリケーションなどに 2 つ以上の URL のアドレスを与える技術です。 HTTP ではこの操作のために、特別な種類のレスポンスである ***HTTP リダイレクト***を提供しています。
...
HTTP では、リダイレクトはリクエストに対して、サーバーが特別な*リダイレクト*レスポンスを送信することによって、サーバーがリダイレクトを発生させます。リダイレクトレスポンスは[ステータスコード](/ja/docs/Web/HTTP/Reference/Status)が `3` で始まり、 {{ httpheader("Location") }} ヘッダーがリダイレクト先の URL を保持しています。
...
![最初のリクエストはクライアントからサーバーに送られます。サーバーはリダイレクト先のURLとともに 301:moved permanently で応答します。クライアントは新しい URL への GET リクエストを行い、サーバーは 200 OK レスポンスでこれを返します。](httpredirect.svg)
...
検索エンジンのロボットは、新たな一時的 URL を記録しません。一時リダイレクトは、リソースを作成、更新、削除しているときに一時的な進捗ページを提供するためにも利用されます。
...
2. 少し意外ですが、JavaScript リダイレクトは HTML リダイレクトよりも先に実行されます。これは、`<meta>` リダイレクトはページが完全に読み込まれた後、つまりすべてのスクリプトが実行された後に現れるためです。
3. HTML リダイレクト ({{HTMLElement("meta")}}) は、ページが読み込まれる前に実行された HTTP リダイレクトや JavaScript リダイレクトがない場合に実行されます。
4. ページが読み込まれた後に JavaScript リダイレクトが現れた場合（例えば、ボタンをクリックした場合など）、ページが前回のメソッドによってすでにリダイレクトされていない場合、そのリダイレクトは最後に実行されます。

可能であれば常に HTTP リダイレクトを使用し、{{HTMLElement("meta")}} は追加しないようにしましょう。誰かが HTTP リダイレクトを変更して HTML リダイレクトを変更し忘れると、リダイレクトが同一ではなくなったり、無限ループになったり、その他の問題が発生したりします。
...
一般的に、ユーザーは {{HTTPMethod("PUT")}}、{{HTTPMethod("POST")}}、{{HTTPMethod("DELETE")}} のリクエストを再送信することは望みません。リクエストの結果としてレスポンスを返すだけである場合は、再読み込みボタンを押すことで (おそらく確認メッセージの後に)、リクエストを再送信します。
...
ほとんどの場合はサーバー側の問題であり、サーバーで検出できる場合は {{HTTPStatus("500")}} `Internal Server Error` を返すでしょう。サーバーの設定を変更した直後にこのようなエラーが発生した場合は、リダイレクトループが発生しているかもしれません。

サーバーがそれを検出できない場合があります。リダイレクトループが複数のサーバーに広がり、それぞれのサーバーが全貌を把握していない場合です。この場合、ブラウザーがそれを検出し、エラーメッセージを表示します。Firefox では、次のように表示されます。
...
リダイレクトループは使い勝手を完全に損ないますので、避けることが重要です。

## 関連情報

- [3XX リダイレクト](/ja/docs/Web/HTTP/Reference/Status#リダイレクトメッセージ)レスポンスステータス
- {{HTTPHeader("Location")}} ヘッダー
- JavaScript を使用した {{domxref("window.location")}} リダイレクト
