...
- \<type>
  - : [認証方式](/ja/docs/Web/HTTP/Authentication#認証方式)。一般的には ["Basic"](/ja/docs/Web/HTTP/Authentication#basic_認証方式) です。それ以外に以下のような種類があります。\* [認証スキームの IANA レジストリ](http://www.iana.org/assignments/http-authschemes/http-authschemes.xhtml)
...
- \<credentials>
  - : "Basic" 認証方式を使用している場合、認証情報は次のように構築されます。\* コロンで結合したユーザー名とパスワード (`aladdin:opensesame`)。
    - 結果の文字列は [base64](/ja/docs/Glossary/Base64) でエンコードされます (`YWxhZGRpbjpvcGVuc2VzYW1l`)。> **Note:** **注**: Base64 エンコードは暗号化でもハッシュでもありません。この方法の安全性はクリアテキストで認証情報を送るのと同等です (Base64 は可逆エンコーディングです)。 Basic 認証は HTTPS との組み合わせで使用することをお勧めします。
