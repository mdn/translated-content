...
![HTTP はアプリケーション層のプロトコルであり、 TCP （トランスポート層）や IP （ネットワーク層）やプレゼンテーション層以下の上の階層に当たります。](http-layers.svg)
...
![複数のプロキシーを経由してクライアントからサーバーに転送された HTTP リクエストと、同じルートを辿ってクライアントに導かれるレスポンス。](client-server-chain.svg)
...
HTTP/2 ではこれらのメッセージがバイナリー構造の*フレーム*に埋め込まれており、ヘッダーの圧縮や多重化といった最適化が可能になりました。
...
![基本的な HTTP リクエスト](http-request.svg)
...
![GET リクエストに対する "200 OK" HTTP レスポンスの概要。レスポンスヘッダーが含まれます。](http-response.svg)
...
HTTP ベースの API で最も一般的に使用されているのは[フェッチ API](/ja/docs/Web/API/Fetch_API) で、JavaScript から HTTP リクエストを行うために使用することができます。フェッチ API は {{domxref("XMLHttpRequest")}} API を置き換えるものです。
...
セッションのフローは依然として基本的であり、フローの調査やシンプルな [HTTP ネットワークモニター](https://firefox-source-docs.mozilla.org/devtools-user/network_monitor/index.html)でデバッグすることができます。
