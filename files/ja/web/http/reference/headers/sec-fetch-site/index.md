...
同一オリジンからのリクエストは通常デフォルトで許可さ れますが、別のオリジンからのリクエストについては、要求され ているリソースや、別の{{Glossary("fetch metadata request header", "フェッチメタデータリクエストヘッダー")}}内の情報にさらに依存する可能性があります。デフォルトでは、受け入れられないリクエストは {{HTTPStatus("403")}} レスポンスコードで拒否されるべきです。
