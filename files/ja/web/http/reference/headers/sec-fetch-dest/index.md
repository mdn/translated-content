...
これにより、サーバーはそのリクエストを処理すべきかどうかを、そのリクエストが_想定された_使用方法に適しているかどうかに基づいて判断できます。例えば、`audio` の宛先を持つリクエストは、音声データを要求すべきであり、他の種類のリソース（例えば、機密性の高いユーザー情報を含む文書など）を要求すべきではありません。
