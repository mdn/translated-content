...
**`deferred-fetch`** は {{HTTPHeader("Permissions-Policy")}} のディレクティブで、 [`fetchLater()` API](/ja/docs/Web/API/Fetch_API) の一部です。
...
それ以外の例は [`fetchLater()` のクォータ](/ja/docs/Web/API/Fetch_API/Using_Deferred_Fetch) を参照してください。
