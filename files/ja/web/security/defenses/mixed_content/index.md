...
「混在ダウンロード」とは、保護されたコンテキストから開始されたリソースのダウンロードを指しますが、保護されていない接続で取得します。
...
アップグレード可能なコンテンツリクエストは、オリジンのスキームを `http` から `https` に変更することで、保護されていないリクエストが自動的に保護されたリクエストにアップグレードされます。
...
混在コンテンツのリクエストとは、[保護されたコンテキスト](/ja/docs/Web/Security/Defenses/Secure_Contexts)における保護されていないリソースのリクエストです。

次の例は、保護されたコンテンツ、保護されていないコンテンツ、および混在コンテンツのリクエストを説明しています。

- `http://insecure.com` が `http://also.insecure.com` を読み込む — どちらのオリジンも保護されていないため、混在コンテンツのリクエストではありません。
- `https://secure.com` が `http://insecure.com` を読み込む — これは混在コンテンツのリクエストです。保護されていないリソース `http://insecure.com` が保護されたコンテキスト `https://secure.com` に読み込まれるからです。
...
- `https://secure.com` が `data:` URL をフレーム化し、`http://insecure.com` を読み込む — これは混在コンテンツのリクエストです。 `https://secure.com` （したがって、`data:`）は安全に読み込まれ、 `http://insecure.com` は保護されていないからです。
...
次のスクリーンショットは、 Firefox で画像をアップグレードされる際にコンソールに表示される警告を示しています（Chrome にも同様の警告があります）。
