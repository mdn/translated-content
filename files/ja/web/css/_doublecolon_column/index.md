...
このデモでは、コンテンツのそれぞれの「ページ」を所定の位置にスナップする、レスポンシブなコンテナーを作成します。 {{cssxref("columns")}} プロパティと `::columns` 擬似要素を使用して、親{{glossary("scroll container", "スクロールコンテナー")}}の全幅にまたがるコンテンツの段を作成します。この列は水平方向にスクロールできます。それぞれの段には 1 つ以上のリスト項目が含まれ、その数はビューポートの幅によって異なります。
