...
---
<p><strong><code>grid-column</code></strong> は CSS のプロパティで、 {{cssxref("grid-column-start")}} および {{cssxref("grid-column-end")}} の<a href="/ja/docs/Web/CSS/Shorthand_properties">一括指定</a>プロパティであり、{{glossary("grid column", "グリッド列")}}の中におけるグリッドアイテムの寸法と位置を指定し、線、区間、なし (自動) をグリッド配置に適用されることで、{{Glossary("grid areas", "グリッド領域")}}の行の開始と終了の端を指定します。</p>
...
<p>このプロパティは一つまたは二つの <code>&lt;grid-line&gt;</code> の値で指定します。</p>

<p>二つの <code>&lt;grid-line&gt;</code> 値を指定する場合は、 "/" で区切ります。個別指定の <code>grid-column-start</code> はスラッシュの前に設定し、個別指定の <code>grid-column-end</code> はスラッシュの後に設定します。</p>

<p>それぞれの <code>&lt;grid-line&gt;</code> の値は以下の何れかを指定できます。</p>

<ul>
 <li><code>auto</code> キーワード</li>
 <li><code>&lt;custom-ident&gt;</code> 値</li>
 <li><code>&lt;integer&gt;</code> 値</li>
 <li><code>&lt;custom-ident&gt;</code> および <code>&lt;integer&gt;</code> を空白で区切ったもの</li>
 <li><code>span</code> キーワードと <code>&lt;custom-ident&gt;</code> または <code>&lt;integer&gt;</code> またはその両方。</li>
</ul>
...
<dl>
 ...
 <dd>プロパティをグリッドアイテムの配置に影響させず、自動的に配置し、間隔を自動的に取るか、既定の <code>1</code> とするためのキーワードです。</dd>
 <dt><code>&lt;custom-ident&gt;</code></dt>
 <dd>'&lt;custom-ident&gt;-start'/'&lt;custom-ident&gt;-end' という名前の付いた線がある場合、これはそのような線の最初がグリッドのアイテムの配置に関わります。
 <p class="note"><strong>メモ:</strong> 名前付きグリッド領域、自動的にこの形で暗黙の名前付き線を生成しますので、 <code>grid-column: foo;</code> と指定すると名前付きグリッド領域の先頭/末尾側の端を選択します (その前に <code>foo-start</code>/<code>foo-end</code> という名前の線が明示的に存在しない限り)。</p>

 <p>そうでなければ、これは <code>&lt;custom-ident&gt;</code> に沿って整数の <code>1</code> が指定されたものとして扱われます。</p>
 </dd>
 <dt><code>&lt;integer&gt; &amp;&amp; &lt;custom-ident&gt;?</code></dt>
 <dd>n 番目のグリッド線をグリッドアイテムの配置に使用します。負の整数が指定された場合は、逆方向にカウントし、明示的なグリッドの末尾の端から始めます。
 <p>名前が &lt;custom-ident&gt; として与えられた場合、その名前の付いた線のみがカウントされます。その名前がある線の数が十分にない場合は、この位置を探す目的においては、すべての暗黙のグリッド線がその名前を持つと仮定されます。</p>

 <p><code>0</code> の {{cssxref("integer")}} 値は無効です。</p>
 </dd>
 <dt><code>span &amp;&amp; [ &lt;integer&gt; || &lt;custom-ident&gt; ]</code></dt>
 <dd>グリッドアイテムのグリッド領域の列開始の端が終了の端から n 行になるように、グリッドアイテムの配置にグリッドスパンを設定します。
 <p>名前が &lt;custom-ident&gt; として与えられた場合、その名前の付いた線のみがカウントされます。その名前を持つ線の数が十分にない場合は、検索方向に対応する明示的グリッドの側にあるすべての暗黙的グリッド線が、この区間をカウントする目的でその名前を持つと仮定されます。</p>

 <p>the &lt;integer&gt; が省略された場合の既定値は <code>1</code> です。負の数や0は無効です。</p>
 </dd>
</dl>
...
<h2 id="Internet_Explorer_issues" name="Internet_Explorer_issues">Internet Explorer での問題</h2>

<p>IE11 はグリッドアイテムの自動配置に対応していません。すべてのアイテムは、明示的に <code>-ms-</code>{{cssxref("grid-column")}} および <code>-ms-</code>{{cssxref("grid-row")}} を記述しない限り、最初の行・列に配置されます。小さな JavaScript を用いて自動的な配置をすることもできます。 <a href="https://github.com/motine/css_grid_annotator">ソースリポジトリ</a></p>
...
<ul>
 <li>関連する CSS プロパティ: {{cssxref("grid-row")}}, {{cssxref("grid-row-start")}}, {{cssxref("grid-row-end")}}, {{cssxref("grid-column-start")}}, {{cssxref("grid-column-end")}}</li>
 <li>グリッドレイアウトガイド: <em><a href="/ja/docs/Web/CSS/CSS_Grid_Layout/Line-based_Placement_with_CSS_Grid">CSS グリッドでの行ベースの配置</a></em></li>
...
<ol>
 ...
 <li data-default-state="open"><a href="#"><strong>ガイド</strong></a>
  <ol>
   ...
   <li><a href="/ja/docs/Web/CSS/CSS_Grid_Layout/CSS_Grid_and_Progressive_Enhancement">CSS グリッドおよび進歩的な拡張</a></li>
