...
> **メモ:** セレクターが一致する参照点（またはスコープ）となる要素を表す {{CSSXref(":scope")}} 擬似クラスは、名前とは裏腹に、[セレクター](/ja/docs/Web/CSS/CSS_selectors)モジュールで定義されています。これは CSS スコープモジュールとは無関係で、シャドウ DOM のスコープ機構に関連するスコープに焦点を当てています。
