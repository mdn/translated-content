...
> **メモ:** {{cssxref("flex")}} 一括指定を使用する方が、個別に `flex-shrink`、{{cssxref("flex-grow")}}、{{cssxref("flex-basis")}} の宣言を使用するよりも推奨されます。この文書では、一括指定部分の一つである``flex-shrink``プロパティについて説明していますので、ここではこれらを別個のものとしています。
