...
入力される 2 つの色、`color-one` と `color-two` は外側に表示され、関数が返す混合色は中央に表示されます。外側のカラーチップをクリックして、混合する新しい色を選択します。スライダーを使用して、混合に含める各入力色の割合を変更します。ドロップダウンメニューを使用して、関数の出力の色空間を変更します。結果の色値は、ウィジェットの下にある表にさまざまな形式で表示され、コピーできます。また、色形式名をクリックすると、その詳細を確認できます。あ
