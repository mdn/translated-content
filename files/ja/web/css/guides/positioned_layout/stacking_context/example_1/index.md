...
基礎的な例から始めましょう。ルートの重ね合わせコンテキストに、2 つの 相対位置指定された `<div>` 要素 (DIV #1 と DIV #3) があり、これらには `z-index` プロパティはありません。 DIV #1 の中に絶対位置指定がされた DIV #2 があり、一方 DIV #3 の中には絶対位置指定がされた DIV #4 があります。どちらも `z-index` プロパティの指定はありません。
