...
連続メディアにおける{{glossary("viewport", "ビューポート")}}の初期の包含ブロックは、ビューポートと同じサイズをになります。ビューポートは一般的に画面よりも大きくないため、スマートフォンやタブレットなど画面の小さい端末では、デスクトップやノートパソコンなどの大型端末に比べて、通常、ビューポートのサイズが小さくなります。
