...
レイアウトが CSS の一部に含まれているため、コードベースの 1 か所だけで、さまざまなブレークポイントに応じた変更を行うことが可能です。これは、グリッドの再定義、グリッド上のアイテムの位置の再定義、またはその両方を一度に行うことができます。
...
ネット上で見かけるグリッドの例の多くは、ページ全体のレイアウトにグリッドを使用していることを想定しています。しかし、グリッドは小さなセクションのレイアウトにも同様に役立ちます。{{cssxref("grid-template-areas")}} を使用すると、コード内で要素がどのように表示されるかを視覚的に確認することができるので、特に便利です。
