...
- インラインブロック（{{cssxref("display", "display: inline-block")}} である要素）。これは {{htmlelement("button")}} と button 型の {{htmlelement("input")}} 要素のデフォルトです。
- 表のセル ({{cssxref("display", "display: table-cell")}} を持つ要素。これは HTML の表のセルのデフォルト値です)
- 表のキャプション ({{cssxref("display", "display: table-caption")}} を持つ要素。HTMLの、表のキャプションのデフォルト値です)
- {{cssxref("display", "display: table")}}, `table-row`, `table-row-group`, `table-header-group`, `table-footer-group` (つまりそれぞれ HTML の表、表の行、表の本体、表のヘッダー、表のフッターのデフォルト値), `inline-table` のついた要素によって暗黙的に生成された無名の表のセル。
- {{cssxref("display", "display: flow-root")}} である要素。
- フレックスアイテム（{{cssxref("display", "display: flex")}} または `inline-flex` である要素の直接の子要素）で、それ自身が[フレックス](/ja/docs/Glossary/Flex_Container)でも[グリッド](/ja/docs/Glossary/Grid_Container)でも[表](/ja/docs/Web/CSS/Guides/Table)でもないもの。
- グリッドアイテム（{{cssxref("display", "display: grid")}} または `inline-grid` である要素の直接の子要素）で、それ自身が[フレックス](/ja/docs/Glossary/Flex_Container)でも[グリッド](/ja/docs/Glossary/Grid_Container)でも[表](/ja/docs/Web/CSS/Guides/Table)でもないもの。
- {{ cssxref("overflow") }} の値が `visible` や `clip` 以外であるブロック要素。
- {{cssxref("contain", "contain: layout")}}, `content`, `paint` の付いた要素
- クエリーコンテナー（{{cssxref("container-type")}} が `normal` 以外の要素）。
...
.float {
  float: left;
  overflow: hidden; /* resize:both で必要 */
  resize: both;
  margin-right: 25px;
  width: 200px;
  height: 100px;
...
{{EmbedLiveSample("マージンの相殺の抑止", 120, 170)}}
