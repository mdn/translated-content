...
仕様書では、ブロック要素間のマージンは*相殺される*と説明されています。つまり、上マージンを持つ要素が下マージンを持つ要素の直後に来た場合、空間の合計はこれら 2 つのマージンの合計になるのではなく、マージンが相殺され、本質的には 2 つのマージンのうち大きい方のマージンと同じくらいの大きさになるということです。
...
このガイドでは、要素が通常フローにおいて、ブロック要素やインライン要素としてどのように表示されるかについて見てきました。CSS によるスタイル設定がまったく施されていない HTML 文書でも、このデフォルトの挙動のおかげで読みやすい形で表示されます。通常フローの仕組みを理解することは、CSS レイアウト全体を理解するための重要な出発点となります。
