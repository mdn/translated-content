...
アイテムを行と列に並べる二次元グリッドを作成したい場合は、CSSグリッドレイアウトを選択すべきです。フレックスボックスがフレックスコンテナーの直接の子要素に対して機能するのと同様に、グリッドレイアウトはグリッドコンテナーの直接の子要素に対して機能します。コンテナーに {{cssxref("display", "display: grid;")}}を設定するだけです。このコンテナーに設定されたプロパティ（{{cssxref("grid-template-columns")}} や {{cssxref("grid-template-rows")}} など）によって、アイテムが行と列に沿ってどのように配置されるかが定義されます。
