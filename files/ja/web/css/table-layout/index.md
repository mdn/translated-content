...
- `fixed`
  ...
  - : 固定表レイアウトアルゴリズムが使用されます。このキーワードを使用する場合、表の幅は [`width`](/ja/docs/Web/CSS/width) プロパティを使用して明示的に指定する必要があります。width プロパティの値が auto に設定されているか、指定されていない場合、ブラウザーは自動表レイアウトアルゴリズムを使用します。この場合、固定値は効果はありません。\
