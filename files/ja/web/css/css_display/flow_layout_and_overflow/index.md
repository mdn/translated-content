...
コンテンツがボックスに入ります。ボックスがいっぱいになると、ボックスからあふれ出し、ボックスの外にコンテンツが表示され、場合によっては、その下のコンテンツの下にも表示されます。オーバーフローの動作を制御するのは [`overflow`](/ja/docs/Web/CSS/overflow) プロパティであり、初期値は `visible` です。これが、あふれたコンテンツが見える理由です。
...
[`text-overflow`](/ja/docs/Web/CSS/text-overflow) プロパティは、インライン方向におけるテキストのオーバーフローを扱います。これは 2 つの値のうちの 1 つに `clip` があると、コンテンツがあふれたときにコンテンツが切り取られます。これが既定値であり、既定の動作です。また、 `ellipsis` という値もあり、これは省略記号を表示します。省略記号は、使用する言語や書字方向に適した別の文字に置き換えることができます。
