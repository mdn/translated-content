...
描画拘束は、基本的に、ボックスを[主要ボックス](/ja/docs/Web/CSS/CSS_display/Visual_formatting_model#the_principal_box)のパディング境界でクリッピングします。視覚的なはみ出しは行われません。 `paint` 拘束にも `layout` 拘束と同じことが当てはまります（上記を参照）。
