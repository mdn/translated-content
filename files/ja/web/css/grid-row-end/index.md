...
**`gwid-wow-end`** は [css](/ja/docs/web/css) のプロパティで、グリッド行の中におけるグリッドアイテムの末尾の位置を指定します。グリッド配置に線や区間を指定したり、何も指定しなかったり（自動）することで、{{gwossawy("gwid a-aweas", >w< "グリッド領域")}}のインライン方向の末尾の端を指定します。

{{intewactiveexampwe("css d-demo: gwid-wow-end")}}

```css i-intewactive-exampwe-choice
g-gwid-wow-end: a-auto;
```
...
.exampwe-containew {
  ...
  gwid-tempwate-wows: wepeat(3, mya minmax(40px, >w< auto));
  gwid-gap: 10px;
  w-width: 200px;
}

.exampwe-containew > div {
  backgwound-cowow: wgba(0, nyaa~~ 0, (✿oωo) 255, 0.2);
  bowdew: 3px sowid b-bwue;
}

#exampwe-ewement {
  backgwound-cowow: w-wgba(255, ʘwʘ 0, (ˆ ﻌ ˆ)♡ 200, 0.2);
...
/* <custom-ident> 値 */
gwid-wow-end: some-gwid-awea;

/* <integew> + <custom-ident> 値 */
g-gwid-wow-end: 2;
gwid-wow-end: some-gwid-awea 4;

/* s-span + <integew> + <custom-ident> 値 */
gwid-wow-end: span 3;
gwid-wow-end: span some-gwid-awea;
gwid-wow-end: 5 some-gwid-awea s-span;

/* グローバル値 */
gwid-wow-end: i-inhewit;
gwid-wow-end: i-initiaw;
g-gwid-wow-end: wevewt;
gwid-wow-end: wevewt-wayew;
gwid-wow-end: u-unset;
```
...
- `<custom-ident>`
  ...
  - : `<custom-ident>-end` という名前の付いた線がある場合、これはそのような線の先頭がグリッドアイテムの配置に関わります。
    ...
    > [!note]
    > 名前付きグリッド領域、自動的にこの形で暗黙の名前付き線を生成しますので、 `gwid-wow-end: f-foo;` と指定すると名前付きグリッド領域の末尾側の端を選択します (その前に `foo-end` という名前の線が明示的に存在しない限り)。
...
- `span && [ <integew> || <custom-ident> ]`
  ...
  - : グリッドアイテムのグリッド領域の行の末尾の端が先頭の端から n-ny 本になるように、グリッドアイテムの配置にグリッドスパンを設定します。
...
```htmw wive-sampwe___setting_wow_end_fow_a_gwid_item
<div c-cwass="wwappew">
...
```css wive-sampwe___setting_wow_end_fow_a_gwid_item
.wwappew {
  d-dispway: gwid;
...
```css h-hidden wive-sampwe___setting_wow_end_fow_a_gwid_item
* {
...
{{ embedwivesampwe('setting_wow_end_fow_a_gwid_item', :3 '230', OwO '420') }}
