...
**`rule-break`** は [CSS](/ja/docs/Web/CSS) [一括指定](/ja/docs/Web/CSS/Guides/Cascade/Shorthand_properties)プロパティで、列間罫や行間罫を行と列の溝が交差する場所で線分に分割する挙動を設定し、{{cssxref("column-rule-break")}} および {{cssxref("row-rule-break")}} を同じ値に設定します。
...
- `none`
  - : 行と列の線が交差する箇所には切れ目がなく、それぞれの隙間の端から端まで、単一の装飾が施されています。
- `normal`
  - : フレックスコンテナーおよびグリッドコンテナーでは、`none` と同じ挙動を示します。段組みでは、`column-rule-break` は `intersection` と同じ挙動を示し、`row-rule-break` は `none` と同じ挙動を示します。これがデフォルト値です。
- `intersection`
  - : 罫線は隙間と交差するたびに常に途切れます。罫線の線分は、コンテナーと隙間の端で始まり、そこで終わります。

## 解説

`rule-break` プロパティは、線が隙間を横切る際に、線を線分に分割する動作を設定するために使用されています。

隙間の装飾は、1つまたは複数の隙間装飾線分として隙間の内部に描画され、これらの線分は任意の 2 つの隣接するアイテムの間に生成されます。コンテナーの種類によっては、デフォルトで、これらの線分が隙間の端で終了する場合もあれば、列および行の罫線によってコンテナーの高さおよび幅全体に拡張される場合もあります。

罫線に区切りがある場合、デフォルトで、線分は区切りの端から始まり、その端で終わります。区切りのサイズが `0` の場合、区切りは表示されないことがあります。`rule-break` プロパティは、区切りが発生するかどうかを決定します。それぞれの線分の末端にある区切りのサイズは、`rule-inset` プロパティで制御できます。区切りがなく、線が連続している場合、`rule-inset` プロパティはコンテナーの先頭と末尾の罫線にのみ影響します。区切りがある場合、`rule-inset` プロパティは罫線の各線分の先頭と末尾に影響します。

罫線がデフォルトで単一の連続した線分で構成されるか、あるいは隙間と交差する際に区切られる複数の線分で構成されるかは、コンテナーの種類によって異なります。
...
チェックボックスをオンにすると、`rule-break` が `intersection` に設定され、連続した線がすべての「交差」箇所で途切れるようになります。
...
段組みコンテナーでは、デフォルトの `normal` 動作は、行間罫と列間罫で異なります。列間罫の線分は、行の隙間と交差した時点で始まったり終わったりし、`column-rule-break: intersection` として動作します。一方、行ルールは、列の隙間と交差しても途切れず、`row-rule-break: none` として動作します。
...
この例では、`rule-break` プロパティを使用して、グリッドコンテナー内のそれぞれの罫線の線分を分割し、罫線同士が交差しないようにしています。
...
順序なしリストを 8 列のコンテナーとして定義し、{{cssxref("grid-template-columns")}} プロパティを使用して列と行を作成し、{{cssxref("list-style-type")}} を `none` に設定して箇条書き記号を除去します。`20px` の幅を持つ実線の列と行のルールに提供する余地を確保するため、列と行の間に十分な余地を提供するよう、{{cssxref("gap")}} を `20px` に設定します。最後に、罫線の線分が交差するのではなく、区切られるように設定します。
