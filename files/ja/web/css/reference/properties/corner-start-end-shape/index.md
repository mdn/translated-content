...
**`corner-start-end-shape`** は [CSS](/ja/docs/Web/CSS) のプロパティで、ボックスのブロック先頭、インライン末尾の角の {{cssxref("border-radius")}} 領域における形状を指定します。
...
`corner-start-end-shape` プロパティは {{cssxref("&lt;corner-shape-value>")}} 値で指定します。
...
### 基本的な `corner-start-end-shape` の使い方
...
ボックスにいくつかの基本的なスタイルを設定していますが、簡略化のため省略します。また、{{cssxref("box-shadow")}} と、`border-radius` を `20px 40px 60px 80px` に、`corner-start-end-shape` を `notch` に適用しています。
