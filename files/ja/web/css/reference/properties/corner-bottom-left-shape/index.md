...
**`corner-bottom-left-shape`** は [CSS](/ja/docs/Web/CSS) のプロパティで、ボックスの左下の角の {{cssxref("border-radius")}} 領域における形状を指定します。
...
`corner-bottom-left-shape` プロパティは {{cssxref("&lt;corner-shape-value>")}} 値で指定します。
...
ボックスにいくつかの基本的なスタイルを設定していますが、簡略化のため省略します。また、{{cssxref("box-shadow")}} と、`border-radius` を `80px 40% 60px 20%` に、`corner-bottom-left-shape` を `squircle` に適用しています。
