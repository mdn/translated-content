...
すべての画像の幅と高さを同じにスタイル設定した後、それぞれのクラス（つまりそれぞれの画像）に異なる `object-view-box` 値を設定します。描画された手の画像の内在サイズは `298px`× `332px` です。{{cssxref("height")}} と {{cssxref("width")}} を設定し、外因的なサイズを `200px` × `200px` に設定します。
