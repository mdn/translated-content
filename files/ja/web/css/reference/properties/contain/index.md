...
抑制によって DOM のサブセクションを分離することができ、レイアウト、スタイル、描画、サイズ、または任意の組み合わせの計算をページ全体ではなく DOM サブツリーに制限することによって、性能上で有利になります。抑制は、CSS カウンターや引用のスコープにも使用することができます。
...
- `none`
  - : その要素が通常通り描画され、抑制を適用されません。
- `strict`
  - : すべての抑制ルールがその要素に適用されます。これは `contain: size layout paint style` と同等です。
- `content`
  - : `size` 以外の抑制ルールがその要素に適用されることを示します。これは `contain: layout paint style` と同等です。
- `size`
  - : サイズ抑制が、インライン方向でもブロック方向でも要素に適用されます。要素のサイズは、子要素を無視して単独で計算することができます。この値は `inline-size` と組み合わせることはできません。
- `inline-size`
  - : インラインサイズ抑制が要素に適用されます。要素のインラインサイズは、子要素を無視して単独で計算することができます。この値は `size` と組み合わせることはできません。
- `layout`
...
コンテナー内の抑制プロパティへの変更は、内包された要素の外側からページの残りの部分には伝搬されません。
抑制の主な利点は、ブラウザーが DOM やページレイアウトを頻繁に再レンダリングする必要がなくなることで、静的なページのレンダリング時にはわずかなパフォーマンス上の利点を、より動的なアプリケーションではより大きなパフォーマンス上の利点につながります。

`contain` プロパティを使用することで、要素の内部が外接ボックスの外で副作用が発生することを防ぐことができるため、独立した要素をたくさん持つページで有用です。

> [!NOTE]
> `paint`, `strict`, `content` のいずれかの値を使用すると、以下のものを生成します。
>
> 1. 新しい[包含ブロック](/ja/docs/Web/CSS/Guides/Display/Containing_block) ({{cssxref("position")}} プロパティが `absolute` または `fixed` である子孫を対象とする)。
> 2. 新しい[重ね合わせコンテキスト](/ja/docs/Web/CSS/Guides/Positioned_layout/Stacking_context)。
> 3. 新しい[ブロック整形コンテキスト](/ja/docs/Web/CSS/Guides/Display/Block_formatting_context)。
...
レイアウト抑制を適用した場合としない場合の要素の動作を示す下記の例を考えてみましょう。
...
最初のカードにはレイアウト抑制が適用され、レイアウトがページの他の部分から分離されています。
...
スタイルの抑制は[カウンター](/ja/docs/Web/CSS/Guides/Counter_styles/Using_counters)と[引用符](/ja/docs/Web/CSS/Reference/Properties/quotes)を抑制される要素にスコープします。
CSS カウンターの場合、{{cssxref("counter-increment")}} と {{cssxref("counter-set")}} プロパティは、要素が文書のルートにあるかのように要素にスコープされます。

#### 抑制とカウンター

下記の例では、スタイルの抑制が適用された場合のカウンターの動作を見ていきます。
...
抑制がなければ、カウンターはリストアイテムごとに 1 から 5 まで増加します。
スタイル抑制を行うと、{{cssxref("counter-increment")}} プロパティが要素のサブツリーにスコープされ、カウンターは再び 1 から始まります。

{{EmbedLiveSample('抑制とカウンター', '100%', 140)}}

#### 抑制と引用符

CSS の引用符も同様に、{{cssxref("content")}} の引用符関連の値が要素にスコープされます。
...
抑制のため、最初の閉じ引用符は内側のスパンを無視し、外側のスパンの閉じ引用符を代わりに使用します。

{{EmbedLiveSample('抑制と引用符', '100%', 40)}}
