...
**`corner-inline-end-shape`** は [CSS](/ja/docs/Web/CSS) のプロパティで、ボックスのインライン末尾にあるの両方の角の {{cssxref("border-radius")}} 領域における形状を指定します。
...
/* ブロック先頭/インライン末尾の角、ブロック末尾/インライン末尾の角 */
...
`corner-inline-end-shape` プロパティは 1 つまたは 2 つの {{cssxref("&lt;corner-shape-value>")}} 値で指定します。

- **1 つの値**が使用された場合、**インライン末尾の両方**の角の形状を指定します。
- **2 つの値**が使用された場合、1 つ目は**ブロック先頭/インライン末尾**の角、2 つ目は**ブロック末尾/インライン末尾**の角の形状を指定します。
...
### 基本的な `corner-inline-end-shape` の使い方
...
ボックスにいくつかの基本的なスタイルを設定していますが、簡略化のため省略します。また、{{cssxref("box-shadow")}} と、`border-radius` を `15% 30%` に、`corner-inline-end-shape` を `bevel squircle` に適用しています。
...
- {{cssxref("border-start-end-radius")}} および {{cssxref("border-end-end-radius")}}
