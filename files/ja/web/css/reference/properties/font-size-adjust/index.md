---
title: "`font-size-adjust` プロパティ (CSS)"
...
**`font-size-adjust`** は [CSS](/ja/docs/Web/CSS) のプロパティで、フォントサイズを調整し、小文字の高さなどの指定されたフォントメトリックを、{{cssxref("font-size")}} の指定された比率に一致するようにします。このプロパティは、代替フォントのメトリックを、第一候補のフォントが持つメトリックと類似したものにするために使用されます。
...
このプロパティは、キーワード `none`、 1 つ（`<number>` または `from-font`）、または 2 つ（`<font-metric>` および `<number>` と `from-font` のどちらか）の値で指定します。

- `none`
  - : フォントサイズの調整は行われません。
- `<font-metric>` {{optional_inline}}
  - : フォントサイズの調整に使用するフォントメトリックを定義します。デフォルトは `ex-height` であり、この引数は以下のキーワードのいずれかで指定します。
    - `ex-height`
      - : x-height（フォント内の小文字 "x" の高さ）とフォントサイズ（アスペクト値）の比を用いてフォントサイズを調整します。このキーワード値は、フォント間で小文字を正規化するために用いられます。
    - `cap-height`
      - : cap-height（大文字の高さ）とフォントサイズの比を用いてフォントサイズを調整します。このキーワード値は、フォント間で大文字を正規化するために用いられます。
    - `ch-width`
...
- {{cssxref("&lt;number&gt;")}}
  - : 指定された `<font-metric>` に基づいて、使用されているフォントサイズを調整する割合。
...
`font-size-adjust` プロパティは、小文字の高さなどの指定されたフォントメトリックを、{{cssxref("font-size")}} の指定された比率に合わせて調整します。この調整は、代替フォントだけでなく、テキストのレンダリングで使用されているすべてのフォントに対して適用されます。

`<number>` を設定すると、デフォルトまたは指定された `<font-metric>` に基づいてフォントサイズが調整されます。例えば、`ex-height` に設定した場合、`<number>` の値によって、選択されたフォントサイズが調整され、その `x` の文字の高さがフォントサイズの指定された倍数になります。この値は、通常、第一候補フォントのアスペクト値（x-height とフォントサイズの比率）と一致させる必要があります。これにより、利用できる場合、第一候補フォントはすべてのブラウザーで一貫して表示されます。別の値を選択しても第一候補フォントは変倍されますが、その場合は、代わりに {{cssxref("font-size")}} を変更することを推奨します。
...
> [!NOTE]
> x-height や cap height といったフォントのメトリクスは、同じ {{cssxref("font-family")}} 内のフォントフェイス（太字や斜体など）によって異なる場合があります。`font-size-adjust` で `from-font` または固定値が指定された場合、同じフォントファミリー内のフォントフェイス間の相対的な違いにかかわらず、それぞれのフォントフェイスのメトリクスは個別に調整されます。

調整されたフォントサイズは、式 `u  =  ( m / m′ ) s` を使用して計算されます。

- `m` は、`<number>` で指定される、あるいは `from-font` を使用して得られる、目的の比率です。
- `m′` は、選択されたメトリックと、調整するフォントのフォントサイズとの比率です。
- `s` は `font-size` プロパティの値です。
- `u` は、調整後の新しいフォントサイズです。
...
`C` では、 `font-size-adjust` プロパティに 1 つの値しか指定されていないので、デフォルトの `<font-metric>` の `ex-height` 値が使用されていることに注意してください。 `D` は、大文字の高さを調整した場合、 `A` と比べてフォントがどのように見えるかを示しています。
