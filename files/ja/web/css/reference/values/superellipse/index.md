...
> [!NOTE]
> 引数 `K` にどのような値が渡されても、`superellipse()` 関数の返値はその `K` 値に対して常に同じになります。この値を 2 つの要素に適用した場合、ボックスサイズや {{cssxref("border-radius")}} の値が異なると、角の形状の外観が異なる可能性があります。
