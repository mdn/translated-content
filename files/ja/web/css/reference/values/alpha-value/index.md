...
アニメーションの際、 CSS の `<alpha-value>` データ型の値は実数の浮動小数点値として{{Glossary("interpolation", "補間")}}されます。補間の速度はアニメーションと関連づけられた[イージング関数](/ja/docs/Web/CSS/Reference/Values/easing-function)によって決められます。

## 例

### テキスト色の透明度の設定

[`rgb()`](/ja/docs/Web/CSS/Reference/Values/color_value/rgb)関数は、アルファ値を指定する 4 番目のオプション値を受け付けます。
