...
- `ex`
  - : その要素の {{Cssxref("font")}} における[エックスハイト](https://ja.wikipedia.org/wiki/エックスハイト)を表します。 `x` の文字を持つフォントでは一般的に、フォントにおける小文字の高さに相当します。多くのフォントでは `1ex ≈ 0.5em` です。
- `ic`
  - : 描画に使用されるフォントにおける「水」 (CJK water ideograph, U+6C34) の文字の{{Glossary("advance measure", "送り幅")}}の使用値を表します。
- `lh`
...
- `rex`
  - : ルート要素の {{Cssxref("font")}} におけるエックスハイトを表します。
- `ric`
...
**ビューポートパーセント値の長さの単位**は、 4 つの異なるビューポートサイズ（小、大、動的、デフォルト）に基づきます。さまざまなビューポートサイズの許容範囲は、ブラウザーインターフェイスが動的に拡大・縮小したり、下にあるコンテンツを隠したり表示させたりすることに応答します。
...
- **デフォルトビューポート単位**
  - : デフォルトビューポートサイズはブラウザーによって定義されます。ビューポートパーセント値の単位の動作結果は、小ビューポートサイズ、大ビューポートサイズ、 2 つの間の中間サイズ、動的ビューポートサイズのいずれかに基づくビューポートパーセント値の単位と同等になる可能性があります。

    > [!NOTE]
    > 例えば、ブラウザーがデフォルトのビューポートパーセント値単位である高さ単位 (`vh`) を、大型ビューポートパーセント値高さ単位 (`lvh`) と同等として実装する場合があります。その場合、ブラウザーインターフェイスが展開されている間、全ページ表示でコンテンツが隠れてしまう可能性があります。今のところ、すべてのデフォルトビューポート単位（`vh`、`vw`など）は、それに対応する大型ビューポート単位（`lvh`、`lvw`など）と同じです。
...
- `vh`
  - : ビューポートの初期[包含ブロック](/ja/docs/Web/CSS/Guides/Display/Containing_block)の高さのパーセント値を表します。 `1vh` はビューポートの高さの 1% です。例えば、ビューポートの高さが `300px` の場合、プロパティで `70vh` の値を指定すると `210px` になります。
    ...
    小、大、動的ビューポートサイズに対応するビューポートパーセント値単位はそれぞれ `svh`、`lvh`、`dvh` です。`vh` は `lvh` と同等であり、大ビューポートサイズに基づくビューポートパーセント値の長さの単位を表します。
...
- `vw`
  - : ビューポートの初期[包含ブロック](/ja/docs/Web/CSS/Guides/Display/Containing_block)の幅のパーセント値を表します。 `1vw` はビューポートの幅の 1% です。例えば、ビューポートの幅が `800px` の場合、プロパティで `50vw` の値を指定すると `400px` になります。
    ...
    `vw` は `lvw` と同等であり、大ビューポートサイズに基づくビューポートパーセント値の長さ単位を表します。

- `vmax`
  - : `vw` と `vh` の大きい方を表します。

    小、大、動的ビューポートサイズのそれぞれのビューポートパーセント値の単位は `svmax`, `lvmax`, `dvmax` です。
    `vmax` は、ブラウザーのデフォルトのビューポートサイズに基づく、ビューポートパーセント値の長さの単位を表します。

- `vmin`
  - : `vw` と `vh` の小さい方を表します。

    小、大、動的ビューポートサイズのそれぞれのビューポートパーセント値の単位は `svmin`, `lvmin`, `dvmin` です。
    `vmin` は、ブラウザーのデフォルトのビューポートサイズに基づく、ビューポートのパーセント値の長さの単位を表します。

- `vb`
  - : 初期[包含ブロック](/ja/docs/Web/CSS/Guides/Display/Containing_block)における、ルート要素の[ブロック軸](/ja/docs/Web/CSS/Guides/Logical_properties_and_values)方向のパーセント値を表します。

    小、大、動的ビューポートサイズのそれぞれのビューポートパーセント値の単位は `svb`, `lvb`, `dvb` です。
    `vb` はブラウザーデフォルトビューポートサイズに基づくビューポートパーセント値の長さの単位を表します。

- `vi`
  - : 初期[包含ブロック](/ja/docs/Web/CSS/Guides/Display/Containing_block)における、ルート要素の[インライン軸](/ja/docs/Web/CSS/Guides/Logical_properties_and_values)方向のパーセント値を表します。

    小、大、動的ビューポートサイズのそれぞれのビューポートパーセント値の単位は `svi`, `lvi`, `dvi` です。
    `vi` はブラウザーデフォルトビューポートサイズに基づくビューポートパーセント値の長さの単位を表します。
...
クエリーに適合するコンテナーが利用できない場合、コンテナークエリーの長さの単位は、その軸に対して[小ビューポート単位](#小ビューポート単位)が (`sv*`) デフォルトで適用されます
...
**絶対的な長さの単位**は、出力メディアの物理的な特性が分かっている場合 (印刷レイアウトなど) に物理的な長さを表します。この方法は、単位の 1 つを**物理単位**または**視角単位**に固定し、それ以外にも単位をそれに相対的に定義することです。物理単位には `cm`、`in`、`mm`、`pc`、`pt`、`px`、`Q` などがあります。固定する単位は画面のような低解像度の端末か、あるいはプリンターのような高解像度の端末かによって異なります。
...
- `px`
  - : 1 ピクセルです。画面表示では、伝統的に 1 {{glossary("device pixel", "デバイスピクセル")}}（ドット）を表します。しかし、プリンターや高解像度の画面では、 CSS の 1 ピクセルが端末の複数ピクセルになることがあります。 `1px` = `1in / 96` です。
- `cm`
...
次の例では、指定された入力フィールドが提供されます。このフィールドに `<length>` 値（例：`300px`、`50%`、`30vw`）を入力すると、<kbd>Enter</kbd> キーまたは <kbd>Return</kbd> キーを押した際に、その下に現れる結果バーの幅が設定されます。
