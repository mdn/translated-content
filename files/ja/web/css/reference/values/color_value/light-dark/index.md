...
**`light-dark()`** は [CSS](/ja/docs/Web/CSS) の [`<color>` 関数](/ja/docs/Web/CSS/Reference/Values/Functions#color_functions)で、プロパティに 2 つの色を設定することができます。開発者が明色カラースキームまたは暗色カラースキームを設定したか、ユーザーがライト色またはダーク色のテーマをリクエストしたか検出することによって、 2 つの色の選択肢のいずれかを返します。テーマ色を [`prefers-color-scheme`](/ja/docs/Web/CSS/Reference/At-rules/@media/prefers-color-scheme) [メディア特性](/ja/docs/Web/CSS/Guides/Media_queries/Using#targeting_media_features)クエリーに入れる必要はありません。
