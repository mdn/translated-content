...
> [!NOTE]
> `rgba()` 関数記法は `rgb()` の別名です。両者はまったく同じです。`rgb()` を使用することが推奨されています。
...
> [!NOTE]
> 互換性の理由から、[Web API](/ja/docs/Web/API/Window/getComputedStyle) でシリアル化された色値は、アルファチャンネル値がちょうど 1 の場合は `rgb()` 色として、それ以外の場合は `rgba()` 色として表されます。いずれの場合も、区切り文字としてカンマを使用した古い構文が使用されます（例えば `rgb(255, 0, 0)`）。
