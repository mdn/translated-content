...
`luminance` マスクの場合、マスクされた要素の表示は、マスクの不透明度と不透明領域の色の明度の両方に依存します。白（輝度 100%）の不透明領域（アルファ = 1）はマスクされ、表示されます。黒（輝度 0%）の領域は透明（アルファ = 0）になり、クリップされます。白と黒の中間の色で、不透明度が部分的な領域は、マスクを構成する各色の輝度とアルファ透過率を反映して、部分的にマスクされます。
