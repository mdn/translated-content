...
---
<p><span class="seoSummary"><strong><code>grid-column-start</code></strong> は CSS のプロパティで、グリッドの配置に線または区間を使用したり、使用しなかったりして (自動)、グリッド列内のグリッドアイテムの開始位置を指定します。この開始位置は、{{Glossary("grid areas", "グリッド領域")}}の行の開始の端を指定します。</span></p>
...
<pre class="brush:css no-line-numbers notranslat notranslate">/* キーワード値 */
...
<p>このプロパティは単一の <code>&lt;grid-line&gt;</code> 値で指定します。 <code>&lt;grid-line&gt;</code> 値は次のように指定します。</p>
...
<dl>
 ...
 <dd><code>&lt;custom-ident&gt;-start</code> という名前の付いた線がある場合、これはそのような線の最初がグリッアイテムの配置に関わります。
 <p class="note"><strong>メモ:</strong> 名前付きグリッド領域、自動的にこの形で暗黙の名前付き線を生成しますので、 <code>grid-column-start: foo;</code> と指定すると名前付きグリッド領域の先頭側の端を選択します (その前に <code>foo-start</code> という名前の線が明示的に存在しない限り)。</p>
 ...
 <p>名前が <code>&lt;custom-ident&gt;</code> として与えられた場合、その名前の付いた線のみがカウントされます。その名前がある線の数が十分にない場合は、この位置を探す目的においては、すべての暗黙のグリッド線がその名前を持つと仮定されます。</p>
 ...
 <p>名前が <code>&lt;custom-ident&gt;</code> として与えられた場合、その名前の付いた線のみがカウントされます。その名前を持つ線の数が十分おにない場合は、検索方向に対応する明示的グリッドの側にあるすべての暗黙的グリッド線が、この区間をカウントする目的でその名前を持つと仮定されます。</p>

 <p>the &lt;integer&gt; が省略された場合の既定値は <code>1</code> です。負の数や <code>0</code> は無効です。</p>

 <p><code>&lt;custom-ident&gt;</code> は <code>span</code> の値を取ることができません。</p>
 </dd>
</dl>
...
<h3 id="Setting_column_start_for_a_grid_item" name="Setting_column_start_for_a_grid_item">グリッドアイテムの列の開始位置の設定</h3>
...
<ul>
 <li>関連する CSS プロパティ: {{cssxref("grid-column-end")}}, {{cssxref("grid-column")}}, {{cssxref("grid-row-start")}}, {{cssxref("grid-row-end")}}, {{cssxref("grid-row")}}</li>
