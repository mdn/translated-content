...
- {{cssxref("revert-layer")}}
  - : [カスケードレイヤー](/ja/docs/Web/CSS/@layer)のプロパティの値を、前回カスケードレイヤーの要素と一致する CSS ルール内のプロパティの値にロールバックします。このキーワードを使用したプロパティの値は、現在のカスケードレイヤーのターゲット要素にルールが指定されていない場合と同様に再計算されます。
