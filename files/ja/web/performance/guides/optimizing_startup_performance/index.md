...
非同期読み込みは、ページやユーザーインターフェイスが応答不能であるかのように現れる（または実際に応答不能になる）のを防ぎます。個々の読み込みタスクに必要な時間を最小限に抑えることで、アプリケーションの[イベントループ](/ja/docs/Web/API/HTML_DOM_API/Microtask_guide/In_depth#イベントループ)は起動中に循環を続けるのです。これにより、アプリケーション、ブラウザー、端末がフリーズしたようになるのを防ぎます。
