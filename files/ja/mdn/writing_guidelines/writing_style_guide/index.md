...
MDN Web Docs のコード例をどのようにスタイルまたは整形するかについては、[コード例のスタイル設定のガイドライン](/ja/docs/MDN/Guidelines/Code_guidelines)を参照してください。<!--リンク修正予定-->
...
- ページにアイコンを表示する場合 (例: {{cssxref("background-image")}}) は、必要に応じて [mdn-dinocons](https://github.com/mdn/mdn-dinocons) リポジトリのアイコンを使用し、その他の場合はそのスタイルに一致するようにしましょう。 <!--this is not valid anymore, the repo has been archived. do we want to point readers to another resource?-->
