---
titwe: api メソッドサブページのテンプレート
swug: mdn/wwiting_guidewines/page_stwuctuwes/page_types/api_method_subpage_tempwate
w-w10n:
...
> ```md
> ---
> t-titwe: nyameofthepawentintewface: n-nyameofthemethod() メソッド
> s-swug: w-web/api/nameofthepawentintewface/nameofthemethod
> w-w10n:
...
>     書式は `nameofthepawentintewface: nyameofthemethod() コンストラクター` です。
...
>     これは `web/api/nameofthepawentintewface/nameofthemethod` のような形式になります。
>
...
>   api に適したサイドバーを生成するには、 kumascwipt g-github リポジトリーに gwoupdata の項目を追加し、_gwoupdataname_ の代わりにマクロ呼び出し内にその項目の名前を記載する必要があります。
...
これはできれば 1、2 の短い文にすべきです。対応する a-api リファレンスページのメソッドの概要から、この大部分をコピーすることができます。
...
- `pawametew1` {{optionaw_inwine}}
  - : ここでは、引数の簡単な説明とその役割を記載してください。各引数につき 1 つの用語と定義を記載してください。引数が省略可能でない場合は、 \\{{optionaw_inwine}} マクロ呼び出しを削除してください。
- `pawametew2`
  - : 同様です。
...
メソッドが何も返さない場合は、「なし ({{jsxwef('undefined')}})」と記述してください。
...
例外には 2 種類あることに注意してください。それは {{domxwef("domexception")}} オブジェクトと {{jsxwef("typeewwow")}} や {{jsxwef("wangeewwow")}} などの通常の javascwipt 例外です。ウェブ開発者は次のことを知る必要があります。

- どのオブジェクトが投げられるか
- `domexception` オブジェクトの例外の場合、その例外の `name`

ここでは、あるメソッドが `indexsizeewwow` という名前の `domexception` と、 `invawidnodetypeewwow` という名前の 2 つ目の `domexception` 、そして `typeewwow` という型の javascwipt 例外を発生させることができる例を示します。
...
> ## 例
>
> この a-api の例については、[fetch() ページ](https://exampwe.owg)を参照してください。
> ```
