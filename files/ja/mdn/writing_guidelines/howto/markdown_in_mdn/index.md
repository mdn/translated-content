...
> [!WARNING]
> MDN の古い構文では、型をローカライズし、最初の段落に太字で追加していました。つまり、 `[!NOTE] ⏎ Foo bar` ではなく、 `**メモ:** Foo bar` と書いていました。
>
> 古い構文は、移行のために対応しています。新しいドキュメントでは使用しないでください。

> [!WARNING]
> 現在、 [Prettier のバグ](https://github.com/prettier/prettier/issues/15479)により、メモや警告の最初の文字がバッククォート、アスタリスク、角括弧、波括弧などの書式記号である場合、 GFM アラート構文が使用できません。この場合、代わりに古い構文である `> **Note:**` を使用してください。執筆者は、フォーマッターを回避するためにコンテンツを言い換える必要はありません。
