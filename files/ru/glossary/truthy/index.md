...
В {{Glossary("JavaScript")}} **истинноподобное (truthy)** значение — это значение, рассматривающиеся как `true` в {{Glossary("Boolean", "булевом")}} контексте. К истинноподобным значениям относятся любые {{Glossary("Falsy", "неложноподобные")}} значения, кроме `false`, `0`, `-0`, `0n`, `""`, `null`, `undefined` и `NaN`.
