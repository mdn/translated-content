...
Говоря о CSS, нельзя не упомянуть о селекторах, о некоторых типах которых мы уже говорили в руководстве [Начало работы с CSS](/ru/docs/Learn_web_development/Core/Styling_basics/Getting_started). Селектор — это то, как мы обозначаем что-либо в нашем HTML-документе, чтобы стилизовать его. Если стиль не применился, то это, скорее всего, потому, что селектор в таблицах стилей не совпал с тем, что в HTML-документе.
...
> [!NOTE]
> Вы узнаете больше о селекторах в руководстве [CSS-селекторы](/ru/docs/Web/CSS/CSS_%D0%A1%D0%B5%D0%BB%D0%B5%D0%BA%D1%82%D0%BE%D1%80%D1%8B) в следующем модуле.
...
Понимание каскадов, или правил, улучшается с практикой. В статье [Каскад и наследование](/ru/docs/Learn_web_development/Core/Styling_basics/Handling_conflicts) я хорошенько объясню, как определить уровень спецификации. А пока что запомните, что иногда CSS не применяется так, как вы того хотели бы, так как у чего-то в таблице стилей больший уровень спецификации.
...
> [!WARNING]
> Если свойство или значение не определено, то объявление считается _недействительным_ — и будет попросту проигнорировано.

> [!WARNING]
> В CSS (и прочих веб-стандартах) американское написание является стандартом. Например, `color` надо _всегда_ писать `color`; британский вариант `colour` не будет работать.
...
{{EmbedLiveSample('Функция calc()', '100%', 200)}}
...
Другим примером могут служить различные {{cssxref("transform", "функции трансформации")}}, такие как `rotate()`.
...
{{EmbedLiveSample('Функции трансформации', '100%', 200)}}
...
- **{{cssxref("transform")}}**
- **{{cssxref("background-image")}}, в том числе значения градиента**
- **{{cssxref("color")}}, в том числе значения rgb и hsl**

## Правила

До сих пор не сталкивались мы с правилами [`@rules`](/ru/docs/Web/CSS/CSS_syntax/At-rule) (произносится как _эт-рулс_, от английского "at-rules"). Это особые правила, дающие CSS инструкции, как вести себя. У некоторых правил `@rules` простые названия и значения. Чтобы, к примеру, импортировать ещё одну таблицу стилей в основной CSS-файл, нужно использовать `@import`:
...
> [!WARNING]
> Стенографии позволяют пропускать некоторые величины, и это может отразиться на результате недолжным образом.
