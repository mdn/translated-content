...
Перед двоеточием у нас есть свойство, а после двоеточия — значение. CSS-{{Glossary("property/CSS","свойства")}} имеют разные допустимые значения в зависимости от того, какое свойство указывается. В нашем примере мы имеем свойство `color`, которое может принимать различные [цветовые значения](/ru/docs/Learn_web_development/Core/Styling_basics/Values_and_units#color). У нас также есть свойство `font-size`. Это свойство может принимать различные [значения размера](/ru/docs/Learn_web_development/Core/Styling_basics/Values_and_units#numbers_lengths_and_percentages), как и свойства.
...
> [!NOTE]
> Вы можете найти ссылки на все страницы свойств CSS (вместе с другими функциями CSS), перечисленные в MDN [Руководстве по CSS](/ru/docs/Web/CSS/Reference). Кроме того, вы должны привыкнуть к поиску "MDN _css-feature-name_" в вашем браузере, когда вам нужно узнать больше информации о функции CSS. Например, попробуйте поискать «mdn color» и «mdn font-size»!
...
Для конкретного примера давайте вернёмся к модулю Свойства фона и границ — вы можете подумать, что это логично для свойств [`background-color`](/ru/docs/Web/CSS/Reference/Properties/background-color) и [`border-color`](/ru/docs/Web/CSS/border-color), которые будут определены в этом модуле. И Вы правы.
