...
<p>Для детальной информации, смотрите руководство по  <a href="/ru/docs/Web/HTTP/Cookies">HTTP cookies</a>.</p>
...
<dl>
 ...
 <dd><p>Путь, который должен существовать в запрошенном URL, иначе браузер не отправит заголовок Cookie.</p>
 <p>Пример: <code>/</code> - cookie будет отправляться со всеми запросами<br>
 Пример: <code>/docs/</code> - cookie будет отправляться с запросами к директории docs и её поддиректориям</p></dd>
 <dt><code>Secure</code> {{optional_inline}}</dt>
 <dd><p>Cookie будет отправлен на сервер только с запросами c использованием SSL и протокола HTTPS.</p>
 <p>Cookie не будет дополнительно шифроваться, поэтому в нем не стоит хранить конфиденциальную информацию.</p>
 ...
 <dd><p>Запрещает JavaScript доступ к cookie</p>
 <p>Полезно для защиты от XSS-атак.</p></dd>
...
<ul>
 <li><a href="/ru/docs/Web/HTTP/Cookies">HTTP cookies</a></li>
