...
"отмечено/не отмечено" (флажков). Элементы с `role="checkbox"` должны также включать атрибут
[`aria-checked`](/ru/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-checked),
чтобы передавать состояние флажков вспомогательным технологиям.
...
может находиться в двух состояниях: ("отмечено" или "не отмечено"), а также в
...
так как предыдущие фрагменты кода эквивалентны следующему в
