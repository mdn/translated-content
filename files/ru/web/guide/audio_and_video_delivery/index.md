...
- [Audio/Video Codec Compatibility Table](/ru/docs/Web/HTML/Supported_media_formats#Browser_compatibility)
...
> **Примечание:** The preload attribute may be ignored by some mobile browsers.

For further info see [Cross Browser Audio Basics (HTML5 Audio In Detail)](/en-US/Apps/Build/Manipulating_media/Cross-browser_audio_basics#HTML5_audio_in_detail)
...
For further info see [\<video> element](/ru/docs/Web/HTML/Element/video) and [Creating a cross-browser video player](/en-US/Apps/Build/Manipulating_media/cross_browser_video_player).
...
> **Примечание:** Play will be ignored by some mobile browsers unless issued by a user-initiated event.
...
See [MediaRecorder API](/ru/docs/Web/API/MediaRecorder_API) for more details.
...
> **Примечание:** Currently Safari does not support DASH although dash.js will work on newer versions of Safari scheduled for release with OSX Yosemite.
...
> **Примечание:** The playback range portion of the media element URI specification was added to Gecko 9.0. At this time, this is the only part of the [Media Fragments URI specification](http://www.w3.org/TR/media-frags/) implemented by Gecko, and it can only be used when specifying the source for media elements, and not in the address bar.
...
- [DASH Adaptive Streaming for HTML 5 Video](/ru/docs/Web/HTML/DASH_Adaptive_Streaming_for_HTML_5_Video)
...
> **Примечание:** Firefox OS versions 1.3 and above support the [RTSP](http://en.wikipedia.org/wiki/Real_Time_Streaming_Protocol) protocol for streaming video delivery. A fallback solution for older versions would be to use `<video>` along with a suitable format for Gecko (such as WebM) to serve fallback content. More information will be published on this in good time.

## References

- [The video element](/ru/docs/Web/HTML/Element/video)
- [Media events API](/ru/docs/Web/Guide/Events/Media_events)
- [HTMLVideoElement API](/ru/docs/Web/API/HTMLVideoElement)
- [MediaSource API](/ru/docs/Web/API/MediaSource)
- [Web Audio API](/ru/docs/Web/API/Web_Audio_API)
- [MediaRecorder API](/ru/docs/Web/API/MediaRecorder_API)
- [getUserMedia](/ru/docs/Web/API/MediaDevices/getUserMedia)
