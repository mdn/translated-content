...
| `complete` {{readonlyInline}}                         | {{jsxref("Boolean")}} | Возвращает true, если браузер закончил загрузку картинки с ошибкой или успешно. Также возвращает true, когда не установлено значение {{domxref("HTMLImageElement.src", "src")}}, во всех остальных случаях - false. |
...
| `isMap`                                               | {{jsxref("Boolean")}} | Reflects the [`ismap`](/ru/docs/Web/HTML/Reference/Elements/img#ismap) HTML attribute, indicating that the image is part of a server-side image map.                                                                |
