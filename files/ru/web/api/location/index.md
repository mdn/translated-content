...
- {{domxref("Location.reload()")}}
  - : Перезагружает ресурс по текущему URL. Единственный опциональный параметр {{jsxref("Boolean")}} при значении `true` указывает, что страница должна быть заново загружена с сервера, при значении `false` страница может быть загружена из кеша.
