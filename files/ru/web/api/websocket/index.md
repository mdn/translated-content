...
| `onclose`        | {{ domxref("EventListener") }} | Обработчик событий, вызываемый, когда `readyState` WebSocket соединения изменяется на `CLOSED`. Наблюдатель получает [`CloseEvent`](/en-US/docs/Web/API/CloseEvent) с именем "close".                                                                                                                                   |
