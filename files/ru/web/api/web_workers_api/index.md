...
<p class="summary"><strong>Web Workers</strong> это механизм, который позволяет скрипту выполняться в фоновом потоке, который отделен от основного потока веб-приложения. Преимущество заключается в том, что ресурсоёмкие вычисления могут выполняться в отдельном потоке, позволяя запустить основной (обычно пользовательский) поток без блокировки и замедления.</p>
...
<dl>
 ...
 <dd>Представляет общее описание для любого воркера ({{domxref("Window")}} делает то же самое для обычного веб-контента). Различные типы воркеров, наследующие этот интерфейс, могут расширять его функциональность.</dd>
 <dt>{{domxref("DedicatedWorkerGlobalScope")}}</dt>
 <dd>Представляет описание dedicated worker, наследуется от {{domxref("WorkerGlobalScope")}} и расширяет его функциональность.</dd>
 <dt>{{domxref("SharedWorkerGlobalScope")}}</dt>
 <dd>Пердставляет описание shared worker, наследуется от {{domxref("WorkerGlobalScope")}} и расширяет его функциональность.</dd>
