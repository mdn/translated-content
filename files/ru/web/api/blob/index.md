...
> [!NOTE]
> Метод `slice()` имеет изначально задаваемую длину как второй аргумент, что используется для указания числа байт копируемых в новый `Blob.` Если указать такие параметры `start + length`, которые превышают размер исходного `Blob`, то возвращаемый `Blob` будет содержать данные от начального индекса (start index) до конца исходного `Blob.`

> [!NOTE]
> Следует помнить ,что метод `slice()` имеет сторонние префиксы в некоторых браузерах: `blob.mozSlice()` для Firefox 12 и ранее, так же `blob.webkitSlice()` в Safari. Старая версия метода `slice()`, без сторонних приставок, имеет другой алгоритм и устарела. Поддержка `blob.mozSlice()` была убрана в Firefox 30.
...
- {{DOMxRef("Blob.text()")}}
  - : Возвращает promise, который исполняется с {{jsxref("String")}}, содержащей всё содержимое `Blob`, интерпретируемое как текст UTF-8.
