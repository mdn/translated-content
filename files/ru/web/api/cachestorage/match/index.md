...
- options {{optional_inline}}
  - : Объект, свойства которого определяют, как проверяется совпадение в операции сопоставления. Доступны следующие варианты:
    - `ignoreSearch`: {{jsxref("Boolean")}} свойство. Определяет, следует ли игнорировать параметры запроса в строке url или нет. Например, если установлено `true`, параметры `?value=bar` запроса `http://foo.com/?value=bar` будут проигнорированы во время сопоставления. Значением по умолчанию является `false`.
    - `ignoreMethod`: {{jsxref("Boolean")}} свойство. Когда установлено `true`, предотвращает проверку `http` метода запроса {{domxref("Request")}} (обычно разрешены только `GET` и `HEAD`.) По умолчанию установлено `false`.
    - `ignoreVary`: {{jsxref("Boolean")}} свойство, определяющее, следует ли выполнять проверку заголовка `VARY.` Если установлено `true`, совпадения будут найдены, независимо от того, имеет ли {{domxref("Response")}} заголовок `VARY` или нет. По умолчанию установлено `false`.
    - `cacheName`: Строка {{jsxref("String")}} - имя кеша для поиска.
