...
Доступное только для чтения свойство **`url`** интерфейса {{domxref("Response")}} содержит URL ответа. Значением свойства `url` будет конечный URL-адрес, полученный после любых перенаправлений.
