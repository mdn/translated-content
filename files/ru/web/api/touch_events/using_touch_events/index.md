...
- [`touchstart`](/ru/docs/Web/API/Element/touchstart_event) - вызывается в момент касания сенсорной поверхности
- [`touchmove`](/ru/docs/Web/API/Element/touchmove_event) - вызывается при перемещении по сенсорной поверхности
- [`touchend`](/ru/docs/Web/API/Element/touchend_event) - вызывается, если убрали палец
- [`touchcancel`](/ru/docs/Web/API/Element/touchcancel_event) - вызывается в момент прерывания события (например, создано слишком много точек касания).
...
Списки касаний, которые использует приложение, зависят от семантики его жестов. Например, если приложение поддерживает однократное касание (тап) одного элемента, будет использоваться список {{domxref("TouchEvent.targetTouches","targetTouches")}} в обработчике события [`touchstart`](/ru/docs/Web/API/Element/touchstart_event) для обработки точки касания в свойственной приложению манере. Если приложение поддерживает свайп двумя пальцами для любых двух точек касания, оно будет использовать список {{domxref("TouchEvent.changedTouches","changedTouches")}} в обработчике [`touchmove`](/ru/docs/Web/API/Element/touchmove_event) для того, чтобы определить, были ли перемещены эти две точки касания, а затем реализовать семантику этого жеста в свойственной приложению манере.
...
- Добавляйте обработчики событий [`touchmove`](/ru/docs/Web/API/Element/touchmove_event), [`touchend`](/ru/docs/Web/API/Element/touchend_event) и [`touchcancel`](/ru/docs/Web/API/Element/touchcancel_event) внутрь [`touchstart`](/ru/docs/Web/API/Element/touchstart_event).
...
Внедрение новых механизмов ввода усложняет обработку всех возможных событий, таких как события клавиатуры, мыши, пера, касаний. Чтобы помочь решить эту проблему, [стандарт "Pointer Events"](https://www.w3.org/TR/pointerevents/) определяет события и связанные с ними интерфейсы для обработки ввода с таких указывающих устройств, как мышь, перо, сенсорный экран и т.д. То есть, абстрактный _указатель_ создаёт унифицированную модель ввода, которая может представлять точку контакта пальца, пера/стилуса или мыши. Подробнее в MDN-статье [События указателя](/ru/docs/Web/API/Pointer_events).
...
Статус реализации событий указателя в браузерах [относительно высок](/ru/docs/Web), поскольку Chrome, Firefox, IE11 и Edge имеют полные реализации.
...
- [Добавление поддержки сенсорных экранов на сайта (простой способ)](https://www.codicode.com/art/easy_way_to_add_touch_support_to_your_website.aspx)
...
- [Стандарт "Pointer Events"](https://www.w3.org/TR/pointerevents/)
