...
Событие [`orientationchange`](/ru/docs/Web/Events/orientationchange) возникает каждый раз, когда устройство изменяет ориентацию экрана и самого себя, и может быть отслежено свойством {{domxref("Screen.orientation")}}.
...
> то переход из приложения A в приложение B (или наоборот) не вызовет событие изменения ориентации экрана [`orientationchange`](/ru/docs/Web/Events/orientationchange), т. к. оба приложения сохраняют заданную ориентацию.
>
> В то же время, событие [`orientationchange`](/ru/docs/Web/Events/orientationchange) может возникнуть в момент блокировки ориентации, если для удовлетворения заданному параметру блокировки изменяется положение экрана.
