...
> [!NOTE]
> В спецификации DOM4 (как реализовано в Gecko 13.0), `deep` необязательный аргумент. Если он опущен, метод действует как если бы значение `deep` было **`true`**, по умолчанию используется глубокое клонирование. Для создания поверхностного клонирования, `deep` должен быть установлен как `false`.
>
