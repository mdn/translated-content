...
- {{domxref("createImageBitmap", "Window.createImageBitmap()")}}
  - : Принимает множество различных изображений и возвращает {{jsxref("Promise")}}, который возвращает {{domxref("ImageBitmap")}}. Опционально ресурс может быть обрезан до прямоугольника, заданного в пикселях _(sx, sy)_ с шириной sw и высотой sh.
