...
| `absolute` {{readonlyInline}}   | {{jsxref("Boolean", "boolean")}} | This value is `true` if the orientation is provided as a difference between the device coordinate frame and the Earth coordinate frame; if the device can't detect the Earth coordinate frame, this value is `false`. |
