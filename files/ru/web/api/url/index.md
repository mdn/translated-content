...
- {{domxref("URL.hash", "hash")}}
  - : Строка типа {{jsxref("String")}}. Содержит идентификатор фрагмента и символ `'#'` в начале строки.
- {{domxref("URL.host", "host")}}
  - : Строка типа {{jsxref("String")}}. Содержит доменное имя (_hostname_) за которым следует (если был указан порт) символ `':'` и номер порта.
- {{domxref("URL.hostname", "hostname")}}
  - : Строка типа {{jsxref("String")}}. Содержит доменное имя.
- {{domxref("URL.href", "href")}}
  - : Строка типа {{jsxref("String")}}. Содержит полный URL-адрес.
- {{domxref("URL.origin", "origin")}} {{readonlyInline}}
  - : Строка типа {{jsxref("String")}}. Содержит схему (протокол), доменное имя и номера порта.
- {{domxref("URL.password", "password")}}
  - : Строка типа {{jsxref("String")}}. Содержит пароль, указанный перед доменным именем.
- {{domxref("URL.pathname", "pathname")}}
  - : Строка типа {{jsxref("String")}}. Содержит относительный пусть и символ `'/'` в начале строки. Параметры запроса и идентификатор фрагмента не входят в эту строку.
- {{domxref("URL.port", "port")}}
  - : Строка типа {{jsxref("String")}}. Содержит номер порта.
- {{domxref("URL.protocol", "protocol")}}
  - : Строка типа {{jsxref("String")}}. Содержит название протокола и символ `':'` на конце.
- {{domxref("URL.search", "search")}}
  - : Строка типа {{jsxref("String")}}. Содержит параметры запроса. Включает в себя все параметры, которые были указаны и начинается с символа `'?'`.
...
- {{domxref("URL.username","username")}}
  - : Строка типа {{jsxref("String")}}. Содержит имя пользователя указанное перед именем домена.

## Методы

- {{domxref("URL.toString", "toString()")}}
  - : Возвращает строку типа {{jsxref("String")}}, содержащую полный URL-адрес. Возвращаемое значение аналогично значению свойства {{domxref("URL.href")}}. Разница лишь в том, что при помощи этого метода нельзя изменить значение URL-адреса.
- {{domxref("URL.toJSON", "toJSON()")}}
  - : Возвращает строку типа {{jsxref("String")}}, содержащую полный URL-адрес. Возвращаемое значение аналогично значению свойства {{domxref("URL.href")}}.

## Статические методы

- {{domxref("URL.createObjectURL", "createObjectURL()")}}
  - : Возвращает строку типа {{jsxref("String")}}, содержащую уникальный URL-адрес `Blob` объекта. Этот адрес предоставляет из себя строку с `blob:` в качестве схемы, за которой идёт строка однозначно идентифицирующая `Blob` объект в браузере.
...
- [Что такое URL-адрес?](/ru/docs/Learn_web_development/Howto/Web_mechanics/What_is_a_URL)
