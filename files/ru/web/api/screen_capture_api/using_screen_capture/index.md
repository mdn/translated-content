...
> [!NOTE]
> Из соображений конфиденциальности и безопасности источники совместного использования экрана не перечисляются с использованием метода {{domxref("MediaDevices.enumerateDevices", "enumerateDevices()")}}. По той-же причине, событие [`devicechange`](/ru/docs/Web/Events/devicechange) никогда не вызывается, когда есть изменения в доступных источниках при выполнении `getDisplayMedia()`.
