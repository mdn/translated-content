...
Значение {{jsxref("Operators/null", "null")}} ведёт себя как 0 в числовом контексте и как `false` в логическом контексте:
...
- Семь типов данных, которые являются {{Glossary("Primitive", "примитивами")}}:
  1. {{Glossary("Boolean")}}. `true` и `false`.
  2. {{Glossary("null")}}. Специальное ключевое слово, обозначающее нулевое или «пустое» значение. Поскольку JavaScript чувствителен к регистру, `null` не то же самое, что `Null`, `NULL` или любой другой вариант.
  3. {{Glossary("undefined")}}. Свойство глобального объекта, не имеющее присвоенного значения.
  4. {{Glossary("Number")}}. Целое число или число с плавающей запятой. Например: `42` или `3.14159`.
  5. {{Glossary("BigInt")}}. Целое число с произвольной точностью. Например: `9007199254740992n`.
  6. {{Glossary("String")}}. Последовательность символов, представляющая текстовое значение. Например: `"Привет"`.
  7. [Symbol](/ru/docs/Web/JavaScript/Reference/Global_Objects/Symbol). Тип данных, экземпляры которого уникальны и неизменяемы.
