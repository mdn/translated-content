...
<p>JavaScript содержит набор встроенных объектов. Также вы можете создавать свои объекты. Начиная с JavaScript 1.2, вы можете создавать объект с помощью инициализатора объекта. Другой способ —  создать функцию-конструктор и сделать экземпляр объекта с помощью этой функции и оператора <code>new</code>.</p>
...
<p>До JavaScript 1.1 не было возможности пользоваться инициализаторами объекта. Единственный способ создавать объекты — это пользоваться функциями-конструкторами или функциями других объектов, предназначенных для этой цели. Смотрите <a href="#using_a_constructor_function">Using a constructor function</a>.</p>
...
<p><em>Метод</em> — это функция, ассоциированная с объектом или, проще говоря, метод — это свойство объекта, являющееся функцией. Методы определяются так же, как и обычные функции, за тем исключением, что они присваиваются свойству объекта. Например вот так:</p>
...
<p><em><a href="/ru/docs/Web/JavaScript/Reference/Functions/get">Геттер </a>(от англ. get - получить) </em>—<em> </em>это метод, который получает значение определённого свойства. <em><a href="/ru/docs/Web/JavaScript/Reference/Functions/set">Сеттер </a>(от англ. set </em>—<em> присвоить)</em> — это метод, который присваивает значение определённому свойству объекта. Вы можете определить геттеры и сеттеры для любых из встроенных или определённых вами объектов, которые поддерживают добавление новых свойств. Синтаксис определения геттеров и сеттеров использует литеральный синтаксис объектов.</p>
...
console.log(o.b); // Запускает геттер, который возвращает a + 1 (тоесть 6)</pre>
...
<p>Смотри <a href="Expressions_and_operators#delete"><code>delete</code></a> чтобы получить дополнительную информацию.</p>
...
<pre class="brush: js">// Две переменных ссылаются на два объекта с одинаковыми свойствами
var fruit = {name: 'apple'};
var fruitbear = {name: 'apple'};

fruit == fruitbear; // вернёт false
fruit === fruitbear; // вернёт false</pre>

<pre class="brush: js">// Две переменные ссылаются на один общий объект
var fruit = {name: 'apple'};
...
fruit === fruitbear; // вернёт true</pre>

<pre class="brush: js">fruit.name = 'grape';
console.log(fruitbear);    // вернёт { name: "grape" } вместо { name: "apple" }</pre>
