...
В JavaScript все числа являются [числами двойной точности в соответствии с 64-битным двоичным форматом IEEE 754](https://en.wikipedia.org/wiki/Double-precision_floating-point_format) (т.е. числами, принадлежащими диапазону между -(253 -1) и 253 -1). **Целые числа не рассматриваются как отдельный тип чисел**. В дополнение к числам с плавающей запятой, к числовому типу данных относятся также три символьные величины: `+`{{jsxref("Infinity", "бесконечность")}}, `-`{{jsxref("Infinity", "бесконечность")}}, и {{jsxref("NaN")}} (не-число). В разделе [типы и структуры данных в JavaScript](/ru/docs/Web/JavaScript/Guide/Data_structures) числовой тип данных описан в контексте с другими примитивными типами в JavaScript.
...
| Метод                                | Описание                                                                                                                                                                                    |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| {{jsxref("Number.parseFloat()")}}    | Принимает строку как аргумент, и возвращает числовое значение с плавающей точкой, которое удалось распознать. Тоже самое что и глобальная функция {{jsxref("parseFloat()")}}.               |
| {{jsxref("Number.parseInt()")}}      | Принимает строку как аргумент, и возвращает целочисленное значение в заданной системе исчисления, которое удалось распознать. Тоже самое что и глобальная функция {{jsxref("parseInt()")}}. |
...
| {{jsxref("Number.isNaN()")}}         | Определяет, является ли число, переданное в качестве аргумента, {{jsxref("NaN")}} (не числом). Аналогичный, но более надёжный метод чем глобальная функция {{jsxref("isNaN()")}}.           |
