...
<pre class="brush: js">function doSomethingOldStyle(successCallback, failureCallback) {
  console.log("Готово.");
...
<pre class="syntaxbox">new Promise((resolve, reject) =&gt; {
    console.log('Начало');
...
<pre>setTimeout(() =&gt; saySomething("10 seconds passed"), 10000);</pre>
...
<pre>const wait = ms =&gt; new Promise(resolve =&gt; setTimeout(resolve, ms));
...
<pre>[func1, func2].reduce((p, f) =&gt; p.then(f), Promise.resolve());</pre>
...
<pre>const applyAsync = (acc,val) =&gt; acc.then(val);
...
<pre>const transformData = composeAsync(func1, asyncFunc1, asyncFunc2, func2);
...
<pre>for (const f of [func1, func2]) {
  await f();
}</pre>
...
<pre>Promise.resolve().then(() =&gt; console.log(2));
