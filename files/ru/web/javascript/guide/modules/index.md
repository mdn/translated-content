...
В этих примерах мы создаём элемент [`<canvas>`](/ru/docs/Web/HTML/Reference/Elements/canvas)
...
- `canvas.js` — содержит функции, связанные с настройкой canvas:
  - `create()` — создаёт холст заданной ширины `width` и высоты `height` внутри [`<div>`](/ru/docs/Web/HTML/Reference/Elements/div)-обертки с указанным `id` и помещённой в родителя `parent`. Результатом выполнения функции будет объект, содержащий 2D-контекст холста и id обертки.
...
Это нормально, если вы уже используете такую среду или ещё нет, но знаете, что делать, и имеете нужные доступы (то есть вы можете настроить свой сервер, чтобы он устанавливал корректный [`Content-Type`](/ru/docs/Web/HTTP/Reference/Headers/Content-Type)-заголовок для `.mjs`-файлов).
...
Прежде всего, вам нужно добавить `type="module"` в [`<script>`](/ru/docs/Web/HTML/Reference/Elements/script)-элемент, чтобы объявить, что скрипт является модулем. Чтобы подключить модуль `main.js`, нужно написать следующее:
...
- Нет необходимости использовать атрибут `defer` (см. [атрибуты `<script>` элемента](/ru/docs/Web/HTML/Reference/Elements/script#attributes)) при загрузке модуля, модули являются deferred по умолчанию.
