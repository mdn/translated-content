...
Примечание: обратите внимание на значение в левом операнде. Если это пустая строка ({{Glossary("falsy", "ложноподобное")}} значение), то должно использовать оператор `||=`, в противном случае — оператор `??=` (если ожидается получение значения {{jsxref("Operators/null", "null")}} или {{jsxref("undefined")}}).
