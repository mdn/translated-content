...
Аргументы `target`, `start` и `end` приводятся к {{jsxref("Number")}} и обрезаются до целых значений.
...
Функция `copyWithin` намеренно является _обобщённой_, она не требует, чтобы значение `this` внутри неё было объектом {{jsxref("Array")}}, и кроме того, функция `copyWithin` является _изменяющим методом_, она изменит объект `this` и вернёт его, а не просто вернёт копию.
