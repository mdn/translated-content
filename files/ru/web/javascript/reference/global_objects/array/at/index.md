...
Метод `at()` считывает свойство `length` для значения `this` и вычисляет запрашиваемый индекс.
