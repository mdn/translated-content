...
Метод `Set` [`has`](/ru-RU/docs/Web/JavaScript/Reference/Global_Objects/Set/has) проверяет, находится ли значение в объекте `Set`, используя подход, который в среднем быстрее, чем тестирование большинства элементов, которые ранее были добавлен к объекту `Set`. В частности, это, в среднем, быстрее, чем метод [`Array.prototype.includes`](/ru-RU/docs/Web/JavaScript/Reference/Global_Objects/Array/includes), когда объект `Array` имеет `длину`, равную размеру объекта `Set`.
...
- {{jsxref("Set.@@species", "get Set[@@species]")}}
  - : Функция конструктора, которая используется для создания производных объектов.
...
- {{jsxref("Set.prototype.clear()")}}
  - : Удаляёт все значения из объекта `Set`.
...
- {{jsxref("Set.prototype.entries()")}}
  ...
  - : Возвращает новый объект итератора, который содержит **массив `[value, value]`** для каждого элемента в объекте `Set` в порядке вставки.
   ...
   Это похоже на объект {{jsxref("Map")}}, так что _key_ каждой записи совпадает с ее _value_ для `Set`.
...
// пересечение может быть смоделировано так
const intersection = new Set([...mySet1].filter((x) => mySet2.has(x)));

// разница может быть смоделирована так
