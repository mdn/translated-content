...
Результаты, предоставляемые функцией {{jsxref("Intl/Collator/compare()", "Collator.prototype.compare()")}} отличаются в зависимости от языка. Для получения порядка сортировки языка, используемого в пользовательском интерфейсе вашего приложения, убедитесь, что вы указали этот язык (и, возможно, несколько запасных языков) используя параметр `locales`:
...
Результат, предоставляемый функцией {{jsxref("Intl/Collator/compare()", "Collator.prototype.compare()")}}, может быть настроен с помощью параметра `options`:
