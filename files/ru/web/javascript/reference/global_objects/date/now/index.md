...
Метод `now()` возвращает количество миллисекунд, прошедших с 1 января 1970 года 00:00:00 по UTC по текущий момент времени в качестве {{jsxref("Number", "числа", "", 1)}}.

Поскольку метод `now()` является статическим методом объекта {{jsxref("Date")}}, вы всегда должны использовать его как `Date.now()`.
