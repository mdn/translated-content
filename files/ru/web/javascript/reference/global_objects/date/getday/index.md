...
Вторая инструкция в примере ниже присваивает значение 1 переменной `weekday` из значения объекта {{jsxref("Date")}} `date`. 25 декабря 1995 года было понедельником.
...
> [!NOTE]
> При необходимости полное название дня (например, "`Понедельник`") можно получить, используя {{jsxref("Intl/DateTimeFormat", "Intl.DateTimeFormat")}} с параметром `options` parameter. С помощью этого метода упрощается интернационализация:
>
> ```js
