...
Выбрасывает исключение {{jsxref("TypeError")}}, если параметр `proto` не является {{jsxref("Operators/null", "null")}} или объектом (исключение составляют объекты-обёртки примитивных типов).
...
Функция примешивания должна копировать функции из прототипа суперкласса в прототип подкласса, она должна предоставляться пользователем. Примером примеси может служить функция [jQuery.extend()](https://api.jquery.com/jQuery.extend/).
