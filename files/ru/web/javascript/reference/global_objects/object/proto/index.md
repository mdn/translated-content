...
Свойство `__proto__` объекта {{jsxref("Object")}} является свойством доступа (комбинацией геттера и сеттера), которое расширяет внутренний прототип `[[Prototype]]` объекта (являющийся объектом или {{jsxref("Operators/null", "null")}}), через который осуществлялся доступ.
...
Геттер свойства `__proto__` расширяет значение внутреннего прототипа `[[Prototype]]` объекта. Для объектов, созданных с использованием литеральной формы создания объекта, это значение равно {{jsxref("Object")}}. Для функций это значение равно {{jsxref("Function")}}. Для объектов, созданных с использованием формы `new fun`, где `fun` является одной из встроенных функций-конструкторов, предоставляемых JavaScript ({{jsxref("Array")}}, {{jsxref("Boolean")}}, {{jsxref("Date")}}, {{jsxref("Number")}}, {{jsxref("Object")}}, {{jsxref("String")}} и так далее — включая новые конструкторы, добавленные в процессе развития JavaScript), это значение равно `fun.prototype`. Для объектов, созданных с использованием формы `new fun`, где `fun` является функцией, определённой в скрипте, это значение равно значению `fun.prototype` во время вычисления `new fun`. Именно поэтому при присваивании `fun.prototype` нового значения, ранее созданные экземпляры `fun` продолжат использовать предыдущее значение в качестве своего прототипа `[[Prototype]]`, а последующие вызовы `new fun` будут использовать вновь присвоенное значение в качестве своего прототипа `[[Prototype]]`.

Геттер `__proto__` позволяет прототипу `[[Prototype]]` объекта быть изменяемым. Объект должен быть расширяемым в соответствии с {{jsxref("Object.isExtensible()")}}: если это не так, выкидывается исключение {{jsxref("TypeError")}}. Предоставляемое значение должно быть объектом или {{jsxref("Operators/null", "null")}}. Предоставление любого другого значения ничего не даст.
...
Свойство `__proto__` является простым свойством доступа на объекте {{jsxref("Object")}} — свойством, состоящим из геттера и сеттера. Свойство `__proto__` будет найдено, если, в конечном итоге, его поиск пройдёт через {{jsxref("Object")}}, но при доступе к нему не через {{jsxref("Object")}}, оно найдено не будет. Если перед просмотром {{jsxref("Object")}} буден найдено какое-нибудь другое свойство `__proto__`, оно скроет искомое свойство {{jsxref("Object")}}.
...
Обратите внимание, что свойство `__proto__` может быть переопределено даже у объекта `Object.prototype`, если новая цепочка заканчивается {{jsxref("Operators/null", "null")}}:
...
Если свойство `__proto__` объекта {{jsxref("Object")}} не установлено в {{jsxref("Operators/null", "null")}}, или в другой объект, чья цепочка прототипов, в конечном итоге, явно не заканчивается значением {{jsxref("Operators/null", "null")}}, будет выкинуто исключение {{jsxref("TypeError")}} «циклическое значение \_\_proto\_\_», поскольку цепочка должна заканчиваться {{jsxref("Operators/null", "null")}} (как это и происходит на {{jsxref("Object")}} при нормальных обстоятельствах).
