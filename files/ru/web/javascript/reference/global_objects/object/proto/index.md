...
Свойство `__proto__` объекта {{jsxref("Object")}} является свойством доступа (комбинацией геттера и сеттера), которое расширяет внутренний прототип `[[Prototype]]` объекта (являющийся объектом или {{jsxref("null")}}), через который осуществлялся доступ.
...
Обратите внимание, что свойство `__proto__` может быть переопределено даже у объекта `Object.prototype`, если новая цепочка заканчивается {{jsxref("null")}}:
...
Если свойство `__proto__` объекта {{jsxref("Object")}} не установлено в {{jsxref("null")}}, или в другой объект, чья цепочка прототипов, в конечном итоге, явно не заканчивается значением {{jsxref("null")}}, будет выкинуто исключение {{jsxref("TypeError")}} «циклическое значение \_\_proto\_\_», поскольку цепочка должна заканчиваться {{jsxref("null")}} (как это и происходит на {{jsxref("Object")}} при нормальных обстоятельствах).
