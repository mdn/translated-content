...
- `regexp`
  - : Объект регулярного выражения. Если будет передан объект `obj`, не являющийся регулярным выражением, он будет неявно преобразован в объект {{jsxref("RegExp")}} через вызов конструктора `new RegExp(obj)`.

### Возвращаемое значение

- `array`
  - : Объект {{jsxref("Array")}}, содержащий результаты сопоставления, или {{jsxref("Operators/null", "null")}}, если ничего не было сопоставлено.

## Описание

Если регулярное выражение не содержит флаг `g`, возвращаемый результат будет тем же самым, что и при вызове метода {{jsxref("RegExp.prototype.exec()", "RegExp.exec()")}}. Возвращаемый объект {{jsxref("Array", "массива", "", 1)}} имеет дополнительное свойство `input`, которое содержит оригинальную строку. Кроме того, он имеет свойство `index`, которое представляет индекс (нумерация с нуля) сопоставления в строке.

Если регулярное выражение содержит флаг `g`, метод вернёт {{jsxref("Array", "массив", "", 1)}}, содержащий все сопоставления. Если сопоставлений не было, метод вернёт значение {{jsxref("Operators/null", "null")}}.
