...
- {{jsxref("URIError")}}
  - : Создаёт экземпляр, представляющий ошибку, возникающую при передаче в функции {{jsxref("encodeURI()")}} или {{jsxref("decodeURI()")}} недопустимых параметров.
...
Для обработки только определённых типов ошибок можно делать проверку типа с помощью ключевого слова {{jsxref("instanceof")}}:
