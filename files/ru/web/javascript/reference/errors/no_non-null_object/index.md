...
Объект где-то ожидался и не был предоставлен. {{jsxref("Operators/null", "null")}} не является объектом и не будет работать. Вы должны обеспечить надлежащий объект в данной ситуации.

## Примеры

### Ожидается дескриптор свойства

Когда такие методы, как {{jsxref("Object.create()")}} или {{jsxref("Object.defineProperty()", "object.defineProperty()")}} и {{jsxref("Object.defineProperties()", "object.defineProperties()")}} используются, дополнительный параметр дескриптора ожидает объект дескриптора свойства. Если не указать объект (например, просто число), возникнет ошибка:
