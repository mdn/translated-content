...
<pre class="brush: js">var foo = 42; // сейчас foo типа Number
...
<p>Для получения самого маленького и самого большого доступного значения в диапазоне от <code>+0</code> до <code>+Infinity</code> можно использовать константы {{jsxref("Number.MIN_VALUE")}} и {{jsxref("Number.MAX_VALUE")}}. Для диапазона от <code>-Infinity</code> до <code>-0</code> эти же константы используются со знаком минус. А начиная с ECMAScript 2015, вы также можете проверить, находится ли число в безопасном для целых чисел диапазоне, используя метод {{jsxref("Number.isSafeInteger()")}}, либо константы {{jsxref("Number.MAX_SAFE_INTEGER")}} и {{jsxref("Number.MIN_SAFE_INTEGER")}}. За пределами этого диапазона операции с целыми числами будут небезопасными, и возвращать приближённые значения.</p>
