...
- `type`
  - : Задаёт тип нумерации:
    ...
    - `1` для цифр (по умолчанию)указанный тип используется для всего списка, если только не указан любой другой атрибут [`type`](/ru/docs/Web/HTML/Reference/Elements/li#type) в элементе {{HTMLElement("li")}}.
      > [!NOTE]
...
- CSS-свойства, которые могут быть полезны для стилизации `<ol>` элемента:
  ...
  - [CSS-счётчики](/ru/docs/Web/CSS/Guides/Counter_styles/Using_counters), для более сложных вложенных списков,
