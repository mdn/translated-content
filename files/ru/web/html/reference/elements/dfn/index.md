...
| ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Допустимый контент                                                | [Фразовый контент](/ru/docs/Web/HTML/Guides/Content_categories#phrasing_content), но ни один {{HTMLElement("dfn")}} элемент не должен быть потомком.                            |
...
В HTML5 [`title`](/ru/docs/Web/HTML/Reference/Global_attributes#title) атрибут имеет особое значение, как указано ниже.
...
1. Если `<dfn>` элемент имеет [`title`](/ru/docs/Web/HTML/Reference/Global_attributes#title) атрибут, значение атрибута `title` считается определяемым термином. Элемент должен по-прежнему иметь текст внутри него, но этот текст может быть аббревиатурой (возможно, с использованием{{HTMLElement("abbr")}}) или другой формой термина.
...
> [!NOTE]
> Если `<dfn>` элемент имеет атрибут `title`, он _должен_ содержать определяемый термин и никакой другой текст.

### Ссылки на `<dfn>` элементы

Если вы включаете атрибут [`id`](/ru/docs/Web/HTML/Reference/Global_attributes#id) в `<dfn>` элемент, вы можете ссылаться на него, используя {{HTMLElement("a")}} элементы. Они должны ссылаться на использование термина, чтобы читатель смог быстро перейти к определению термина, если он ещё не знает его, нажав на ссылку термина.
...
Здесь мы видим определение - теперь с атрибутом [`id`](/ru/docs/Web/HTML/Reference/Global_attributes#id) `«definition-dfn»`, который может использоваться в качестве цели для ссылки. Позднее создаётся ссылка с использованием `<a>` и с [`href`](/ru/docs/Web/HTML/Reference/Elements/a#href)атрибутом, установленным на `«#definition-dfn»`, чтобы установить ссылку обратно на определение.
