...
В этом примере, <kbd>s</kbd> это специфичная клавиша доступа (ты должен нажать <kbd>s</kbd> плюс конкретные клавиши-модификаторы для вашей комбинации браузера и операционной системы; смотри [accesskey](/ru/docs/Web/HTML/Global_attributes/accesskey) для получения их списка).
...
function updateButton() {
  if (button.value === 'Включить ПК') {
    button.value = 'Выключить пк';
    paragraph.textContent = 'ПК включен!';
  } else {
...
<canvas class="myCanvas">
  <p>Добавьте подходящий запасной вариант здесь.</p>
</canvas>
```
