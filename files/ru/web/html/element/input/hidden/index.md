...
2. Контент для редактирования присылается из базы данных и подгружается в HTML форму, чтобы пользователь мог внести изменения.
...
Это затруднило бы для злоумышленников создавать поддельные формы, выдавая себя за настоящие банки, чтобы обманом вынудить жертву перевести деньги не туда. Подобный вид атак называется подделкой межсайтовых запросов (CSRF); практически любой крупный серверный фреймворк использует токены безопасности для предотвращения атак этого типа.
...
- Подробнее о других типах `input` и о самом [input](/files/ru/web/html/element/input/).
