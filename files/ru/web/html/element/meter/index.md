...
- {{htmlattrdef("value")}}
  - : Текущее числовое значение. Он должен быть между минимальным и максимальным значением (`min` атрибут и `max` атрибут), если они указаны. Если он не указан или имеет неверное значение, значение равно 0. Если указан, но не в пределах диапазона, заданного атрибутами `min` и `max`, значение будет равно ближайшему концу диапазона.> **Примечание:** **Примечание по использованию:** Если атрибут `value` не находится в диапазоне от `0` до `1` (включительно), то атрибуты `min` и `max` должны определять диапазон, в котором будет находиться значение `value`.
- {{htmlattrdef("min")}}
