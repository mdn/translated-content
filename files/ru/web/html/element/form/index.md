...
- {{htmlattrdef("accept")}} {{HTMLVersionInline(4)}} {{obsolete_inline}}
  ...
  > **Примечание:** **Примечание об использовании:** Этот атрибут был удалён в HTML5 и его не следует больше использовать. Взамен, используйте {{htmlattrxref("accept", "input")}} атрибут заданного {{HTMLElement("input")}} элемента.
...
- {{htmlattrdef("enctype")}}
  - : Когда значение атрибута method равно `post`, атрибут - [MIME тип](http://en.wikipedia.org/wiki/Mime_type) содержимого, которое используется, чтобы передать форму на сервер. Возможные значения:\* `application/x-www-form-urlencoded`: Значение по умолчанию, если атрибут не задан.
