...
- Если задан атрибут [`itemprop`](/ru/docs/Web/HTML/Global_attributes/itemprop), то элемент `<meta>` предоставляет _определенные пользователем метаданные_.
...
- `http-equiv`
  ...
  - : Определяет директиву pragma. Название атрибута сокращено от `http-equivalent`, так как все допустимые значения являются именами конкретных HTTP-заголовков:
    ...
    - `content-security-policy`
      ...
      - : Позволяет авторам страниц определять [политику содержимого](/ru/docs/Web/HTTP/Headers/Content-Security-Policy) для текущей страницы. Политики содержимого в основном указывают разрешённые источники сервера и конечные точки скриптов, что помогает защититься от атак межсайтового скриптинга (XSS).
    ...
    - `refresh`
      ...
      - : Эта инструкция задаёт:
        ...
        >
        > - [MDN Understanding WCAG, Guideline 2.2 explanations](/ru/docs/Web/Accessibility/Understanding_WCAG/Operable#guideline_2.2_—_enough_time_provide_users_enough_time_to_read_and_use_content)
        > - [MDN Understanding WCAG, Guideline 3.2 explanations](/ru/docs/Web/Accessibility/Understanding_WCAG/Understandable#guideline_3.2_—_predictable_make_web_pages_appear_and_operate_in_predictable_ways)
...
- `name`
  ...
  - : Атрибуты `name` и `content` могут использоваться вместе для предоставления метаданных документа в виде пар имя-значение, где атрибут `name` задаёт имя метаданных, а атрибут `content` — их значение.
    ...
    Подробнее см. [стандартные имена метаданных](/ru/docs/Web/HTML/Element/meta/name) для информации о наборе стандартных имен метаданных, определённых в спецификации HTML.
...
- [Стандартные имена метаданных](/ru/docs/Web/HTML/Element/meta/name)
- [Изучение: `<meta>`](/ru/docs/Learn/HTML/Introduction_to_HTML/The_head_metadata_in_HTML#метаданные_элемент_meta)
- [Мета-тег viewport](/ru/docs/Web/HTML/Viewport_meta_tag)
