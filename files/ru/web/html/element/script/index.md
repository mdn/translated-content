...
- {{htmlattrdef("nomodule")}}
  - : Этот логический атрибут установлен, чтобы указать, что скрипт не должен выполняться в браузерах, поддерживающих [модули ES2015](https://hacks.mozilla.org/2015/08/es6-in-depth-modules) — по сути, это может быть использовано для предоставления резервных сценариев для старых браузеров, которые не поддерживают модульный код JavaScript.- {{htmlattrdef("nonce")}}
...
- {{htmlattrdef("src")}}
  - : Этот атрибут определяет URI внешнего скрипта; это может быть использовано в качестве альтернативы встраиванию скрипта непосредственно в документ.
...
- {{htmlattrdef("text")}}
  - : Как и атрибут `textContent`, этот атрибут задает текстовое содержимое элемента. Однако, в отличие от атрибута `textContent`, этот атрибут оценивается как исполняемый код после того, как узел вставлен в DOM. - {{htmlattrdef("type")}}
  - : Этот атрибут указывает тип представленного скрипта. Значение этого атрибута будет находиться в одной из следующих категорий:
    - **Omitted или MIME-тип JavaScript:** Для браузеров, совместимых с HTML5, это означает, что сценарий является JavaScript. Спецификация HTML5 настоятельно призывает авторов опустить атрибут, а не предоставлять избыточный MIME-тип. В более ранних браузерах это определяло язык сценариев встроенного или импортированного (через атрибут `src`) кода. Типы MIME JavaScript [перечислены в спецификации](/ru/docs/Web/HTTP/Basics_of_HTTP/MIME_types#JavaScript_types).
    - **`module`:** Для браузеров, совместимых с HTML5, код обрабатывается как модуль JavaScript. На обработку содержимого скрипта не влияют атрибуты `charset` и `{{htmlattrdef("defer")}}`. Для получения информации об использовании "модуля" см. [ES6 подробно: Модули](https://hacks.mozilla.org/2015/08/es6-in-depth-modules). Код может вести себя по-разному при использовании ключевого слова `module`.
    - **Любое другое значение:** Встроенный контент обрабатывается как блок данных, который не будет обрабатываться браузером. Разработчики должны использовать допустимый тип MIME, который не является типом MIME JavaScript, для обозначения блоков данных. Атрибут `src` будет проигнорирован.
      > **Примечание:** в Firefox вы можете указать версию JavaScript, содержащуюся в элементе `<script>`, включив нестандартный параметр `version` внутри атрибута `type` - например `type="text/javascript;version=1.8"`. Это было удалено в Firefox 59 (см. {{bug(1428745)}}).
...
- {{htmlattrdef("language")}} {{Deprecated_inline}}
  - : Как и атрибут `type`, этот атрибут определяет используемый язык сценариев. Однако, в отличие от атрибута `type`, возможные значения этого атрибута никогда не были стандартизированы. Вместо этого следует использовать атрибут `type`.
...
В старых браузерах, которые не поддерживают атрибут `async`, вставленные синтаксическим анализатором скрипты блокируют синтаксический анализатор; вставленные скрипты выполняются асинхронно в IE и WebKit, но синхронно в Opera и Firefox версии до 4. В Firefox 4 свойство DOM `async` по умолчанию имеет значение `true` для сценариев, созданных скриптом, поэтому поведение по умолчанию соответствует поведению IE и WebKit.

Чтобы запросить выполнение внешних скриптов, вставленных в скрипт, в порядке вставки в браузерах, где значение `document.createElement("script").async` равно `true` (например, Firefox 4), установите `async=false` для скриптов, порядок которых вы хотите поддерживать.

> **Предупреждение:** Никогда не вызывайте `document.write()` из асинхронного скрипта. В Firefox 3.6 вызов `document.write()` имеет непредсказуемый эффект. В Firefox 4 вызов `document.write()` из асинхронного скрипта не имеет никакого эффекта (кроме вывода предупреждения на консоль ошибок).
