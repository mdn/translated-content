...
> **Примечание:** Не используйте этот элемент. Стилизовать текст следует с помощью [CSS-свойств](/ru/docs/Web/CSS/CSS_fonts).
...
- `face` {{Deprecated_Inline}}
  - : Этот атрибут содержит разделённых запятыми список имён шрифтов. Текст документа отображается с использованием первого поддерживаемого браузером шрифта из этого списка. Если в системе нет указанных шрифтов, браузер обычно использует пропорциональный или фиксированный шрифт, который есть в системе.
