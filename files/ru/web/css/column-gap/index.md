...
Свойство [CSS](/ru/docs/Web/CSS) **`column-gap`** устанавливает промежуток ({{glossary("Gutters","расстояние")}}) между колонками.

{{EmbedInteractiveExample("pages/css/column-gap.html")}}

Изначально было определено в рамках [многоколоночной раскладки](/ru/docs/Web/CSS/CSS_Columns), назначение `column-gap` было расширено для применения в контексте других методов раскладки. Теперь это свойство определено спецификации [Box Alignment](/ru/docs/Web/CSS/CSS_Box_Alignment) и может использоваться в многоколоночной, флексбокс- и грид-раскладках.
...
- `normal`
  - : Между колонками используется стандартное расстояние, определённое браузером. В случае многоколоночной раскладки это будет `1em`. Для всех остальных типов раскладки это значение равно 0.
- {{CSSxRef("&lt;length&gt;")}}
