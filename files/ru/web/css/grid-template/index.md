...
- `none`
  - : Это ключевое задаёт значение `none` для всех трёх связанных свойств, что означает отсутствие явно заданной грид-раскладки. Соответственно, грид-области будет неименованными. Ряды и колонки будут создаваться неявным образом, а их размер определяться свойствами {{cssxref("grid-auto-rows")}} и {{cssxref("grid-auto-columns")}}.
...
- `[ <line-names>? <string> <track-size>? <line-names>? ]+ [ / <explicit-track-list> ]?`
  ...
  - : Перечисленные строки задаёт свойству {{cssxref("grid-template-areas")}}, после каждой из строки для свойства {{cssxref("grid-template-rows")}} устанавливает размеры грид-полос (либо `auto` в случае их отсутствия), объединяя всё в именованную грид-линию, далее после слеша свойству {{cssxref("grid-template-columns")}} определяет размеры колонок (либо `none` в случае их отсутствия).

**Note:** при перечислении грид-полос нельзя использовать функцию {{cssxref("repeat()")}}, поскольку предполагается, ряды и колонки будут расположены вместе с другом другом, как в стиле "ASCII-рисунков".

> **Note:** сокращённое свойство {{cssxref("grid")}} имеет тот же самый формат значений, но также сбрасывает неявные грид-свойства к их первоначальным значениям. Используйте `grid` (в отличие от `grid-template`), чтобы предотвратить каскадирование этих значений по отдельности.
