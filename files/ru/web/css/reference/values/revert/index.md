...
> [!NOTE]
> Ключевое слово `revert` отличается от {{cssxref("initial")}}, поэтому его не следует с ним путать. {{cssxref("initial")}} использует [начальное значение](/ru/docs/conflicting/Web/CSS/CSS_cascade/Value_processing), которое определено отдельно для каждого свойства CSS-спецификацией. Для сравнения, браузеры устанавливают значения по умолчанию в зависимости от CSS селектора.Например, [начальное значение](/ru/docs/conflicting/Web/CSS/CSS_cascade/Value_processing) для свойства {{cssxref("display")}} — `inline`, тогда как браузер устанавливает значение по умолчанию свойства {{cssxref("display")}} элемента {{HTMLElement("div")}} как `block`, а элемента {{HTMLElement("table")}} как `table`, и т.д
