...
[CSS](/ru/docs/Web/CSS) [Псевдо-класс](/ru/docs/Web/CSS/Reference/Selectors/Pseudo-classes) **`:focus-within`** соответствует элементу, который либо сам находится в фокусе, либо содержит элемент, который находится в фокусе. Другими словами, он представляет элементу, который соответствует псевдоклассу {{CSSxRef(":focus")}} либо имеет потомка, который соответствует `:focus`. (Включая потомков в [shadow trees](/ru/docs/Web/API/Web_components/Using_shadow_DOM).)
