...
Поля элемента (**padding area)** — это пустая область, окружающая контент. Она может быть залита каким-то цветом, покрыта фоновой картинкой, а её границы называются краями полей (_padding edge)_.
...
Область рамки (**border area)** окружает поля элемента, а её граница называется краем рамки (_border edge)._ Ширина рамки задаётся отдельным свойством {{ cssxref("border-width") }} или в составе свойства {{ cssxref("border") }}. Размеры элемента с учётом полей и рамки иногда называют внешней шириной/высотой элемента.
