...
CSS-свойство **`z-index`** определяет положение [позиционированного](/ru/docs/Web/CSS/position) элемента и его нижестоящих элементов или флекс-элементов по оси z. Перекрывающие элементы с большим значением z-index будут накладываться поверх элементов с меньшим z-index.
...
Значение для свойства `z-index` может быть ли `auto`, либо целым числом (`<integer>`).
...
- `<integer>`
  - : Порядок наложения блока в виде целого числа ({{cssxref("&lt;integer&gt;")}}) в текущем контексте наложения. При этом будет элемент создаст новый локальный контекст наложения. Таким образом значения z-index дочерних элементов не будут сравниваться со значениями z-index элементов за пределами этого блока.
