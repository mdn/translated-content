...
Способом исправить проблему [невалидного списка селекторов](#невалидный_список_селекторов) является использование псевдокласса {{CSSxRef(":is", ":is()")}} или {{CSSxRef(":where", ":where()")}} который принимает прощающий список селекторов. Каждый селектор щадящего списка анализируется индивидуально. Поэтому любые невалидные селекторы в списке игнорируются и используются оставшиеся валидные.
