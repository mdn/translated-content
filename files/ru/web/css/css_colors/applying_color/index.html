...
<p>Мы затронем большинство из того, что нужно знать при использовании цвета, включая <a href="#что_может_иметь_цвет">список элементов, которые могут иметь цвет, и необходимые для этого CSS-свойства</a>, <a href="#как_задать_цвет">как задать цвет</a>, и как  <a href="#использование_цвета">использовать его в таблицах стилей и в JS скриптах</a>. Мы также рассмотрим как <a href="#предоставляем_возможность_пользователю_выбрать_цвет">предоставить возможность пользователю выбрать цвет</a>. </p>

<p>Завершим мы статью размышлениями на тему как <a href="#using_color_wisely">использовать цвет с умом</a>: как выбрать подходящий цвет, учитывая потребности людей с различными визуальными способностями.</p>
...
<dl>
 ...
 <dd>См. раздел <a href="#borders">Borders</a> с перечнем свойств CSS, с помощью которых можно присвоить цвет границам блока.</dd>
 <dt>{{cssxref("background-color")}}</dt>
...
<p>Теперь, когда вы знаете какие существуют свойства CSS для присваивания цвета к элементам и какие есть форматы описания цвета, вы можете соединить это вместе, чтобы начать использовать цвет. Как вы уже видели в списке под разделом <a href="#что_может_иметь_цвет">Что может иметь цвет</a>, существует множество вещей, к которым можно применить цвет, используя CSS. Давайте взглянем на это с двух сторон: использовать цвет в таблицах стилей ({{Glossary("stylesheet")}}) и добавлять, изменять цвет, используя {{Glossary("JavaScript")}} код.</p>
...
<div>{{EmbedLiveSample("Цвет_в_таблицах_стилей_CSS", 650, 150)}}</div>
