...
> [!WARNING]
> **Важно**: Если вы не следили за остальным нашим курсом, [скачайте этот пример кода](https://github.com/mdn/beginner-html-site-styled/archive/gh-pages.zip) и используйте его в качестве стартовой точки.
...
> [!NOTE]
> Точка с запятой в конце строки указывает, где заканчивается оператор. Точку с запятой нужно обязательно использовать для разеделения операторов, располагающихся на одной строке. Однако некоторые люди считают, что хорошая практика — указывать её в конце каждого оператора. Существуют и другие правила, когда нужно ставить точку с запятой, а когда нет. Смотрите более подробно в статье [ваше руководство по точкам с запятой в JavaScript](http://news.codecademy.com/your-guide-to-semicolons-in-javascript/).

> [!NOTE]
> Вы можете назвать переменную практически как угодно, но есть некоторые ограничения для её имени (смотрите [в правилах именования переменных](http://www.codelifter.com/main/tips/tip_020.shtml)). Если вы не уверены, вы можете [проверить имя вашей переменной](https://mothereff.in/js-variables), чтобы увидеть корректно ли оно.
...
> [!NOTE]
> Для получения более подробной информации о разнице между var и let, смотрите: [Разница между var и let](/ru/docs/Web/JavaScript/Reference/Statements/let).
...
Обратите внимание, что переменные имеют разные [типы данных](/ru/docs/Web/JavaScript/Guide/Data_structures):
...
Существуют множество способов прикрепить событие к элементу. Здесь мы выбираем {{htmlelement("html")}} элемент и устанавливаем ему обработчик свойства [`onclick`](/ru/docs/Web/API/Element/click_event) анонимной функцией (т.е. безымянной) которая содержит код, который мы хотим запустить для события клика.
