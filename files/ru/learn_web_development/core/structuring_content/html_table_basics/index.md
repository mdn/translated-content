...
| Начальные условия: | Знание основ HTML (читайте [Введение в HTML - Introduction to HTML](/ru/docs/conflicting/Learn_web_development/Core/Structuring_content)). |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ |
...
Не питайте ложных иллюзий - чтобы эффективно представлять таблицы в веб, необходимо придать им хорошую структуру в HTML и применить к ним таблицы стилей ([CSS](/ru/docs/Learn_web_development/Core/Styling_basics)). В данном разделе мы сфокусируемся на HTML, чтобы узнать о том, что касается CSS, вам надо обратиться к статье [Стилизация таблиц](/ru/docs/Learn_web_development/Core/Styling_basics/Tables).
...
HTML-таблицы следует использовать для табличных данных — это то, для чего они предназначены. К сожалению, многие используют таблицы HTML для оформления веб-страниц, например, одна строка для заголовка, одна для содержимого, одна для сносок, и тому подобное. Подробнее об этом можно узнать в разделе [Вёрстка](/ru/docs/Learn_web_development/Core/Accessibility/HTML#%d0%92%d1%91%d1%80%d1%81%d1%82%d0%ba%d0%b0) на [Начальном обучающем модуле доступности](/ru/docs/Learn_web_development/Core/Accessibility). Это происходило из-за плохой поддержки CSS в разных браузерах; в наше время такое встречается гораздо реже, но иногда всё же попадается.

Короче говоря, использование таблиц в целях оформления вместо [методов CSS](/ru/docs/Learn_web_development/Core/CSS_layout) является плохой идеей по следующим причинам :
...
3. Самым маленьким контейнером в таблице является ячейка, она создаётся элементом **[`<td>`](/ru/docs/Web/HTML/Reference/Elements/td)** ('td' - сокращение от 'table data'). Введите внутри тегов table следующее:
...
И последняя возможность, о которой рассказывается в данной статье. HTML позволяет указать, какой стиль нужно применять к целому столбцу данных сразу — для этого применяют элементы **[`<col>`](/ru/docs/Web/HTML/Reference/Elements/col)** и **[`<colgroup>`](/ru/docs/Web/HTML/Element/colgroup)**. Их ввели, поскольку задавать стиль для каждой ячейки в отдельности или использовать сложный селектор вроде {{cssxref(":nth-child()")}} было бы слишком утомительно.
...
Он не идеален, поскольку нам пришлось повторить информацию о стилях для всех трёх ячеек в столбце (в реальном проекте лучше будет добавить CSS-правило и атрибут `class` для всех ячеек). Вместо этого мы можем указать информацию о стилях один раз — в элементе `<col>`. Он должен быть размещён внутри контейнера `<colgroup>` сразу же за открывающим тегом `<table>`:
