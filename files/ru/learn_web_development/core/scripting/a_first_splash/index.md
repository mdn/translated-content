...
Перейдите к [консоли JavaScript в инструментах разработчика](/ru/docs/Learn_web_development/Howto/Tools_and_setup/What_are_browser_developer_tools), и введите следующую строку:
...
Чтобы получить это значение, мы использовали метод {{domxref("document.querySelector", "querySelector()")}} объекта {{domxref("document")}}. `querySelector()` "берет" одну часть информации - [CSS selector](/ru/docs/Learn_web_development/Core/Styling_basics/Basic_selectors), который выбирает нужный элемент*.*
...
2. Далее, откройте [инструменты разработчика в вашем браузере](/ru/docs/Learn_web_development/Howto/Tools_and_setup/What_are_browser_developer_tools), и убедитесь, что вы перешли во вкладку с консолью JavaScript.
