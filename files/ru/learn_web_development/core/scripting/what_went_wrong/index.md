...
> [!NOTE]
> См. наш [TypeError: «x»](/ru/docs/Web/JavaScript/Reference/Errors/Not_a_function) не является справочной страницей функций для получения дополнительной информации об этой ошибке.

### Синтаксические ошибки: второй раунд

> [!NOTE]
> [`console.log()`](/ru/docs/Web/API/console/log_static) это часто используемая функция отладки, которая выводит значение в консоль. Поэтому она будет выводить значение `lowOrHi` в консоли, как только мы попытаемся установить его в строке 48.
...
3. На этот раз сообщается об ошибке: "TypeError: lowOrHi is null", в строке 78.
   ...
   > [!NOTE]
   > [`Null`](/ru/docs/Glossary/Null) — это специальное значение, которое означает "ничего" или "не значение". Поэтому `lowOrHi` был объявлен и инициализирован без значения — у него нет типа или значения.
