...
- Запускать код в соответствии с определёнными событиями происходящими на web странице. В нашем примере выше, мы использовали [`click`](/ru/docs/Web/API/Element/click_event) событие, для определения момента, когда кнопка была кликнута, в соответствии с этим запускался код, который обновлял текст.
- И многое другое!
...
> [!NOTE]
> Эти API являются продвинутыми, и мы не будем их рассматривать в нашем курсе, но ссылки, данные выше, предлагают полную документацию, если вы заинтересованы в более подробной информации.
...
Давайте составим краткий бриф, что же происходит когда мы загружаем страничку в браузере (первое упоминание в статье [Как работает CSS](/ru/docs/Learn_web_development/Core/Styling_basics/What_is_CSS#how_does_css_actually_work)). Когда вы загружаете страничку в браузере, вы запускаете ваш код (HTML, CSS и JavaScript) внутри исполняемой среды (внутри вкладки браузера). Это как будто фабрика берёт сырьё (некий код) и выдаёт продукцию (веб-страничку).
...
> [!NOTE]
> Есть способы отправлять код и данные между разными веб-сайтами/вкладками безопасным способом, но это продвинутые методы, которые мы не будем рассматривать в рамках этого курса.
...
С другой стороны, серверный код запускается на сервере, затем его результаты загружаются и отображаются в браузере. Примеры популярных серверных веб-языков включают PHP, Python, Ruby и ASP.NET. И JavaScript! JavaScript так же может использоваться, как серверный язык, например в популярной среде Node.js — вы можете больше узнать о серверном JavaScript в нашем разделе [Dynamic Websites – Server-side programming](/ru/docs/Learn_web_development/Extensions/Server-side).
...
> [!NOTE]
> Если ваш пример не работает, пройдите ещё раз все шаги и проверьте, сделали ли вы все правильно. Сохранили ли вы вашу локальную копию начального кода, как `.html` файл? Добавили ли ваш {{htmlelement("script")}} элемент после тэга `</body>`? Ввели ли вы JavaScript именно так, как показано? **JavaScript регистрозависимый, и очень привередливый. Поэтому вам нужно вводить синтаксис именно так, как показано, в противном случае оно может не работать.**

> [!NOTE]
> Вы можете увидеть эту версию на GitHub-е как [apply-javascript-internal.html](https://github.com/mdn/learning-area/blob/master/javascript/introduction-to-js-1/what-is-js/apply-javascript-internal.html) ([посмотреть вживую](https://mdn.github.io/learning-area/javascript/introduction-to-js-1/what-is-js/apply-javascript-internal.html)).
...
> [!NOTE]
> Вы можете увидеть эту версию на GitHub-е как [apply-javascript-external.html](https://github.com/mdn/learning-area/blob/master/javascript/introduction-to-js-1/what-is-js/apply-javascript-external.html) и [script.js](https://github.com/mdn/learning-area/blob/master/javascript/introduction-to-js-1/what-is-js/script.js) ([посмотреть вживую](https://mdn.github.io/learning-area/javascript/introduction-to-js-1/what-is-js/apply-javascript-external.html)).
...
> [!NOTE]
> Попробуйте отредактировать вашу версию `apply-javascript.html` и добавить ещё несколько кнопок в файл. После перезагрузки вы должны увидеть, что все кнопки создают параграф, если кликнуть на них. Классно, да?
