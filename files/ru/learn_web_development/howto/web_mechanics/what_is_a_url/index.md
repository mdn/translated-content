...
> [!NOTE]
> Есть и [другие составные части и правила](https://en.wikipedia.org/wiki/Uniform_Resource_Locator), касающиеся URL, но обычно они не используются ни пользователями, ни разработчика. Поэтому не стоит о них беспокоиться, вам не обязательно их знать, чтобы формировать работоспособные URL.
...
> [!NOTE]
> При указании URL-адресов для загрузки ресурсов как части страницы (например, при использовании `<script>`, `<audio>`, `<img>`, `<video>`, и т.д.), следует использовать только URL-адреса HTTP и HTTPS. Использование FTP, например, не особенно безопасно и больше не поддерживается многими браузерами.
