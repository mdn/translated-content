...
Веб-браузеры взаимодействуют с [веб-серверами](/ru/docs/Learn/Common_questions/Web_mechanics/What_is_a_web_server) при помощи гипертекстового транспортного протокола ({{glossary("HTTP")}}). Когда вы нажимаете на ссылку на веб-странице, заполняете форму или запускаете поиск, **HTTP-запрос** отправляется из вашего браузера на целевой сервер.
