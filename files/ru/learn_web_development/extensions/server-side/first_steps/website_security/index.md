...
> [!NOTE]
> Это вводная статья, призванная помочь вам задуматься о безопасности веб-сайта, но она не является исчерпывающей.
...
> [!NOTE]
> Этот раздел в значительной степени основан на информации из [Wikipedia](https://en.wikipedia.org/wiki/SQL_injection).
...
> [!NOTE]
> Уловка здесь в том, что Джону не нужен доступ к файлам cookie пользователя (или учётным данным). Браузер пользователя сохраняет эту информацию и автоматически включает её во все запросы к соответствующему серверу.
