...
2.  Если имя класса `"dark"`, изменяет класс `<button>` на `"light"` (с помощью [`setAttribute()`](https://developer.mozilla.org/en-US/docs/Web/API/Element/setAttribute)), его текстовое содержимое на "Светлее" и {{cssxref("background-color")}} наложения `<div>` на `"rgba (0,0,0,0.5)"`.
