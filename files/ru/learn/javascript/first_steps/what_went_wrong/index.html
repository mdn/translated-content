...
<ol>
 ...
 <li>Посмотрите на строку 78, и вы увидите следующий код:
  <pre class="brush: js">lowOrHi.textContent = «Последнее предположение было слишком высоко!»;</pre>
 </li>
...
<h3 id="SyntaxError_отсутствует_перед_постановкой">SyntaxError: отсутствует ; перед постановкой</h3>
...
<h3 id="SyntaxError_отсутствует_после_списка_аргументов">SyntaxError: отсутствует ) после списка аргументов</h3>
...
<p>Эти ошибки обычно означает, что вы пропустили открывающую или закрывающую кавычку для строковых значений. В первой ошибки выше,  <em>строка</em>  будет заменена на неожиданный персонаж (ей) , что браузер нашёл вместо кавычек в начале строки. Вторая ошибка означает , что строка не закончилась кавычки.</p>
...
<div class="note">
<p><strong>Примечание</strong> : Смотрите наш <a href="/en-US/docs/Web/JavaScript/Reference/Errors/Unexpected_token">SyntaxError: Неожиданный токен</a> и <a href="/en-US/docs/Web/JavaScript/Reference/Errors/Unterminated_string_literal">SyntaxError: незавершённая строка</a> эталонных страниц для получения более подробной информации об этих ошибках.</p>
</div>
...
<div>
<ul>
 <li>Есть много других типов ошибок, которые не перечислены здесь; мы составляем ссылку , которая объясняет , что они означают подробно - см. <a href="/en-US/docs/Web/JavaScript/Reference/Errors">ссылку ошибки JavaScript</a> .</li>
 <li>Если вы столкнётесь с любыми ошибками в коде, которые вы не знаете , как исправить после прочтения этой статьи, вы можете получить помощь! Спросите на <a class="external external-icon" href="https://discourse.mozilla-community.org/t/learning-web-development-marking-guides-and-questions/16294">нить обучения Область дискурсе</a> , или в <a href="irc://irc.mozilla.org/mdn">#mdn</a> IRC канал на <a class="external external-icon" href="https://wiki.mozilla.org/IRC">Mozilla IRC</a>. Расскажите нам, какая у вас ошибка, и мы постараемся вам помочь. Приложите пример своего кода для большей ясности проблемы.</li>
</ul>
</div>
