...
// shopping теперь возвратит [ "tahini", "milk", "cheese", "hummus", "noodles" ]</pre>
  ...
  <div class="note"><strong>Примечание</strong>: Мы уже упоминали это прежде, но просто как напоминание — компьютеры начинают считать с нуля!</div>
 </li>
...
<p>В будущих статьях вы узнаете о циклах, но вкратце этот код говорит:</p>

<ol>
 <li>Начать цикл с номера позиции 0 в массиве.</li>
 ...
 <li>Для каждого элемента вернуть его значение в консоли браузера с помощью <code><a href="/en-US/docs/Web/API/Console/log">console.log()</a></code>.</li>
</ol>

<h2 id="Некоторые_полезные_методы_массивов">Некоторые полезные методы массивов</h2>

<p>В этом разделе мы рассмотрим некоторые полезные методы, связанные с массивом, которые позволяют нам разбивать строки на элементы массива и наоборот, а также добавлять новые элементы в массивы.</p>

<h3 id="Преобразование_между_строками_и_массивами">Преобразование между строками и массивами</h3>
...
<div class="note">
<p><strong>Примечание</strong>: Хорошо, технически это строковый метод, не метод массива, но мы поместили его в массивы, так как он хорошо подходит для них.</p>
</div>
