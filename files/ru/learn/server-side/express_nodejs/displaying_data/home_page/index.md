...
> **Примечание:** Мы будем использовать Mongoose для получения информации из базы данных.
...
> **Примечание:** Мы не экранируем значения счётчиков (то есть мы используем синтаксис `!{}`) потому что эти значения рассчитываются системой. Если же информация поступает от пользователей, то мы должны экранировать переменные.
...
> **Примечание:** Вы пока не сможете использовать ссылки в боковой панели, потому что это ещё не реализовано. Если вы попробуете, то получите ошибки вида «NOT IMPLEMENTED: Book list».
