...
`ItemComponent` будет позволять пользователю возможность отмечать элементы как выполненные, редактировать или удалять их.
...
Angular использует `\{{item.description}}` чтобы получить описание текущего `item` из массива `items`.
...
Эта `*ngIf` означает, что если `editable` равен `false`, то `<div>` будет присутствовать в DOM. Если `editable` равен `true`, Angular удалит этот `<div>` из DOM.
...
`*ngIf` так же применена к следующему `<div>`.
...
Так что если `description` это `eat`, `description` уже в `<input>`.
Таким образом, когда пользователь редактирует элемент, значение `<input>` уже является `eat`.

Переменная шаблона, `#editedItem`, на `<input>` означает, что Angular хранит все, что пользователь печатает в этом `<input>` в переменной `editedItem`.
Событие `keyup` вызывает метод `saveItem()` и передает туда значение из `editedItem` если пользователь нажмет Enter вместо кнопки **Save**.
...
Когда `editable` является `false`, Angular вставляет `<div>` вместе с кнопками **Edit** и **Delete** обратно в DOM.
...
`editable` это тоже свойство, что и директиве `*ngIf`, `*ngIf="editable"`.
...
Используйте `@Input()` чтобы указать, что значение свойства может поступать из вне.
...
Если пользователь вводит текст и нажимает сохранить, `saveItem()` устанавливает `editable` в false, что заставляет `*ngIf` в шаблоне удалить форму редактирования отобразить кнопки **Edit** и **Delete** снова.

Хотя приложение уже должно компилироваться, вам нужно использовать `ItemComponent` в `AppComponent` чтобы вы могли увидеть новые функции в браузере.
