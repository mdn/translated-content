...
Как вы уже убедились в своей работе с HTML и CSS, текст внутри элемента
выкладывается в поле содержимого элемента. Он начинается в левом верхнем углу
области содержимого (или в правом верхнем углу, если используется RTL)
и течёт к концу строки. Как только он достигает конца, он переходит к следующей
строке, и так происходит, пока всё содержимое не будет помещено в элемент. Текстовое
...
> **Примечание:** Приведённый выше абзац вызывает у вас вопросы?
> Вернитесь и просмотрите нашу статью о
> [блочной модели](/ru/docs/Learn/CSS/Building_blocks/The_box_model), чтобы
...
- **Стили шрифта**: Свойства, влияющие на шрифт, применяемый к тексту,
  ...
  полужирным, курсивным, а также на другие подобные параметры.
...
> одна единая сущность. Вы не можете выбирать и стилизовать части текста,
...
Вы можете посмотреть [завершённый пример на GitHub](https://mdn.github.io/learning-area/css/styling-text/fundamentals/)
...
Браузер применит [шрифт по умолчанию](#шрифты_по_умолчанию), если на устройстве,
...
Актуальный список безопасных шрифтов будет изменяться с развитием операционных
...
шрифт. Уточним этот момент: абзацы по умолчанию используют шрифт с засечками
...
{{ EmbedLiveSample('Пример_добавления_font-family', '100%', 220) }}
...
- `em`: 1 `em` равен размеру шрифта родительского элемента (конкретнее, ширине
  ...
  как вы увидите ниже. К чему беспокоиться? Сложности в начале использования
...
1. Горизонтальное смещение тени относительно текста — принимает большинство
   [единиц измерения CSS](/en-US/docs/Learn/CSS/Building_blocks/Values_and_units#length_and_size),
   чаще всего используется `px`; положительные
...
- {{cssxref("direction")}}: Управляет направлением текста (зависит от языка,
...
В этом уроке нет конкретных упражнений, стилизируйте текст как хотите. Можете
...
{{ EmbedLiveSample('Активное_обучение_играем_со_стилями_текста', 700, 800) }}
