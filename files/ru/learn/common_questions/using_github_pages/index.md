...
1.  Направьте командную строку в вашу папку `test-site` (или папку с любым другим названием, содержащую ваш сайт). Для этого используйте команду `cd` (т.е. «\_c_hange \_d_irectory» — «поменяй директорию»). Вот что вы бы набрали, если бы разместили свой сайт в папке `test-site` на рабочем столе:
