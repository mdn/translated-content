...
Эта страница описывает то, как мы используем Markdown для написания докуметации MDN. Мы выбрали GitHub-Flavored Markdown (GFM) в качестве основы, и немного расширили его для поддержки некоторых вещей, необходимых MDN и которые не поддерживает GFM.

## Основа: GitHub-Flavored Markdown

Основа для MDN Markdown это GitHub-Flavored Markdown (GFM): [https://github.github.com/gfm/](https://github.github.com/gfm/). Это значит, что в некоторых случаях вы можете ссылаться на спецификацию GFM. GFM в свою очередь является надмножеством CommonMark ([https://spec.commonmark.org/](https://spec.commonmark.org/)).

## Примеры с блоками кода

В GFM и CommonMark, авторы могут использовать "изгороди с кодом" для разграничения `<pre>` блоков. Начало изгороди может иметь какой-то текст, который называется "info string". Из спецификации:
...
В этой строке может быть несколько слов:
...
На MDN, писатели используют разграничители для блоков кода. They must specify the language of the code sample using the first word of the info string, and this will be used to provide syntax highlighting for the block. The following words are supported:
...
- `svg`
- `xml`
- `wasm` (для WebAssembly text format)
...
````plain
```js example-good
const greeting = "Это хороший пример";
```

```js example-bad
const greeting = "Это плохой пример";
```
````
...
## Примечания, предупреждения и сноски
...
##### Предупреждение на других языках
