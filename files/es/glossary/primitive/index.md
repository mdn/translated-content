...
- Luego, se ejecutan las instrucciones de las funciones:
  ...
  - En la segunda función, se creó una variable `foo` local. ¡Esto incrementa su valor en 2, no el valor original (externo) de `foo`!, además, en esta situación, no se puede acceder directamente a la variable `foo` externa. Esto se debe al alcance léxico de JavaScript y al ensombrecimiento de variables resultante. El `foo` local oculta al `foo` externo. Para obtener más información, consulta [Cierres](/es/docs/Web/JavaScript/Guide/Closures). (Ten en cuenta que `window.foo` aún se podría usar para acceder a la variable `foo` externa).
...
El método {{JSxRef("Object.valueOf"," valueOf()")}} del contenedor devuelve el valor primitivo.
