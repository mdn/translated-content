...
El [atributo global](/es/docs/Web/HTML/Reference/Global_attributes) [`autocapitalize`](/es/docs/Web/HTML/Reference/Elements/input#autocapitalize) es un atributo enumerado que controla si la entrada de texto se escribe en mayúsculas automáticamente a medida que el usuario la introduce/edita. El atributo debe tomar uno de los siguientes valores:
...
El atributo `autocapitalize` nunca hace que se habilite la autocapitalización para un elemento {{HTMLElement("input")}} con un atributo [`type`](/es/docs/Web/HTML/Reference/Elements/input#type) cuyo valor es `url`, `email` o `password`.
