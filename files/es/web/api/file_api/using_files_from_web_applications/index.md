...
Cuando el usuario elige el fichero, la función `handleFiles()` es llamada con un objeto [`FileList`](/en-US/DOM/FileList)el cual a su vez contiene un objeto [`File`](/en-US/DOM/File) que representa el fichero elegido por el usuario.
...
En este caso, la lista de ficheros pasada a la función `handleFiles()` contiene un objeto [`File`](/en-US/DOM/File) por cada fichero seleccionado por el usuario.
...
Gecko 2.0 (Firefox 4 / Thunderbird 3.3 / SeaMonkey 2.1) introduce soporte para los métodos de DOM {{ domxref("window.createBlobURL()") }} y {{ domxref("window.revokeBlobURL()") }}. El cual te permite crear un simple string con la URL que referenciar cualquier dato que pueda referenciar usando un objeto [`File`](/en-US/DOM/File) DOM, esto incluye los ficheros locales del usuario del dispositivo.

Cuando tienes un objeto[`File`](/en-US/DOM/File) te gusta poder referenciarlo con una URL desde el HTML, la manera de crear tu propio blob URL para poder hacer esto es la siguiente:

```
var blobURL = window.createBlobURL(fileObj);
```

El blob URL es un string que identifica el objeto[`File`](/en-US/DOM/File). Cada vez que se llama a {{ domxref("window.createBlobURL()") }}, un único blob URL se crea. Cada vez que esto pasa debe de ser liberado. Cuando el documento se cierra, se libera automáticamente, pero si tu página lo usa dinámicamente deberás liberarlos explicitamente llamando a {{ domxref("window.revokeBlobURL()") }}:
...
El objeto [`FileList`](/en-US/DOM/FileList) es dado por el DOM y lista todos los ficheros selecionados por el usuario, cada uno es un objeto [`File`](/en-US/DOM/File). Para determinar el número de ficheros que han sido seleccionados se comprueba la longitud de la lista con el atributo `length`:

```
var numFiles = files.length;
```

Al objeto [`File`](/en-US/DOM/File) se accede de manera sencilla, accediendo a la lista como si fuera un array:
...
Cada objeto [`File`](/en-US/DOM/File) tiene tres atributos que contienen información util sobre el fichero.
...
La función `FileUpload()` mostrada arriba crea un throbber, el cual es usado para mostrar la información del progreso, a continuación se crea un [`XMLHttpRequest`](/en-US/XMLHttpRequest) para manejar la subida de los datos.
...
- [`File`](/en-US/DOM/File)
- [`FileList`](/en-US/DOM/FileList)
- [`FileReader`](/en-US/DOM/FileReader)
- [Usando XMLHttpRequest](/en-US/XMLHttpRequest/Using_XMLHttpRequest)
