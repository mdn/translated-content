...
- `func`
  - : Una función {{jsxref("Operators/function", "function")}} que se ejecuta cada cierto tiempo. El tiempo lo determina `delay`, estando éste en milisegundos. La primera ejecución ocurre tras el tiempo determinado por `delay`.
...
> [!NOTE]
> El argumento `delay` se convierte en un número entero de 32 bits con signo. Efectivamente, esto limita `delay` a 2.147,483,647 milisegundos, ya que es especificado en el IDL como un número entero con signo.
...
Véase también: [`clearInterval()`](/es/docs/Web/API/Window/clearInterval).
...
[Reasons for delays longer than specified](/es/docs/Web/API/Window/setTimeout#reasons_for_delays_longer_than_specified) para ver ejemplos.
