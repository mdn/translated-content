...
Los eventos de teclado solo son generados por `<input>`, `<textarea>`, `<summary>` y cualquier cosa con el atributo `contentEditable` o `tabindex`. Si no se detectan, [burbujean](/es/docs/Learn/JavaScript/Building_blocks/Events#event_bubbling) por el [árbol DOM](/docs/Web/API/Document_object_model/Using_the_Document_Object_Model#what_is_a_dom_tree) hasta llegar a {{domxref("Document")}}.
