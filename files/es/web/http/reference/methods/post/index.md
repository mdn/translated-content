...
Una solicitud `POST` es tipicamente enviada por un [formulario HTML](/es/docs/Learn_web_development/Extensions/Forms) y resulta en un cambio en el servidor. En este caso, el tipo de contenido es seleccionado poniendo la cadena de texto adecuada en el atributo [`enctype`](/es/docs/Web/HTML/Reference/Elements/form#enctype) del elemento {{HTMLElement("form")}} o el atributo [`formenctype`](/es/docs/Web/HTML/Reference/Elements/input#formenctype) de los elementos {{HTMLElement("input") }} o {{HTMLElement("button")}} :
