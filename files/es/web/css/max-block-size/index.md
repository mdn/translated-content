...
Cada vez que usaría normalmente `max-height` o `max-width`, debería usar `max-block-size` para establecer el máximo "alto" del contenido (incluso si esto no es un valor vertical) y `max-inline-size` para establecer el máximo "ancho" del contenido (aunque esto puede ser vertical en lugar de horizontal). Vea el {{SectionOnPage("/es/docs/Web/CSS/writing-mode", "Ejemplo")}}, que muestra los diferentes modos de escritura en acción.
...
- {{cssxref("&lt;percentage&gt;")}}
  - : Define el `max-block-size` como un porcentaje del tamaño del contenedor en el eje de bloque.
- `none`
