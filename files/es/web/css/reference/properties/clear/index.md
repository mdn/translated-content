...
La propiedad [CSS](/es/docs/Web/CSS) **`clear`** especifica si un elemento puede estar al lado de elementos [flotantes](/es/docs/Web/CSS/Reference/Properties/float) que lo preceden o si debe ser movido (cleared) debajo de ellos. La propiedad `clear` aplica a ambos elementos flotantes y no flotantes.

Cuando es aplicado a bloques no flotantes, mueve el [border edge](/es/docs/Web/CSS/Guides/Box_model/Introduction) del elemento hacia abajo hasta que este debajo del [margin edge](/es/docs/Web/CSS/Guides/Box_model/Introduction) de todos los floats relevantes. Este movimiento (cuando acontece) causa que [margin collapsing](/es/docs/Web/CSS/Guides/Box_model/Margin_collapsing) no ocurra.

Cuando se aplica a elementos flotantes, mueve el [margin edge](/es/docs/Web/CSS/Guides/Box_model/Introduction) del elemento debajo del [margin edge](/es/docs/Web/CSS/Guides/Box_model/Introduction) de todos los floats relevantes. Esto afecta la posición de floats posteriores, ya que estos no pueden ser posicionados más arriba que los primeros.

Los floats que son relevantes para ser limpiados (cleared) son los primeros floats dentro del mismo [contexto de formato de bloque](/es/docs/Web/CSS/Guides/Display/Block_formatting_context).

> [!NOTE]
> Si deseas que un elemento contenga todos los elementos flotantes dentro, puedes hacer dos cosas, o bien flotar el contenedor también o usar `clear` en un [pseudo-element](/es/docs/Web/CSS/Reference/Selectors/Pseudo-elements) {{cssxref("::after")}}.
>
> ```css
> #container::after {
>   content: "";
