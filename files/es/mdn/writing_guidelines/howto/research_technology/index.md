...
3. Los últimos navegadores web modernos: Estas deberían ser compilaciones experimentales/alfa como [Firefox Nightly](https://www.mozilla.org/es/firefox/channel/desktop/)/[Chrome Canary](https://www.google.com/intl/es/chrome/canary/) que tienen más probabilidades de admitir las funciones que está documentando. Esto es especialmente pertinente si está documentando una función que está en "camino" (upcoming).
4. Demostraciones/publicaciones de blog/otra información: busca tanta información como pueda. Si estás actualizando una tecnología porque ha cambiado, asegúrate de que los recursos que estás utilizando para aprender no estén desactualizados. Por eso los dos primeros puntos anteriores son importantes.
