...
Esta página describe cómo usamos Markdown para escribir documentación en MDN Web Docs. Hemos elegido un Markdown personalizado por GitHub (GFM, por sus siglas en inglés ―GitHub-Flavored Markdown―) como base y agregamos algunas extensiones para admitir algunas de las cosas que necesitasmos hacer en MDN que no son fácilmente compatibles con GFM.
...
En GFM y CommonMark, los autores pueden usar "vallas de código" para demarcar bloques `<pre>`. El código de apertura puedes ir seguido de algún texto llamado "cadena de información". La especificación establece lo siguiente:
...
Si el resaltado que quieres usar no aparece en la lista anterior, debesrás marcar el bloque de código como `plain`.
...
- `example-bad`: pinta este ejemplo como un mal ejemplo (uno que se debes evitar)
...
Las notas y advertencias mostrarán el texto **Nota:** o **Advertencia:** en el resultado, mientras que las observaciones no. Esto hace que las observaciones sean una buena opción cuando un autor deseas proporcionar un título personalizado.
...
<div class="notecard note">
  ...
  <p>Puedes tener varias líneas.</p>
</div>
```
...
<div class="notecard warning">
  ...
  <p>Puedes tener varios párrafos.</p>
</div>
```
...
Debido a que el texto "Nota:" o "Advertencia:" también aparece en la salida renderizada, debes ser sensible a las traducciones. En la práctica, esto significa que cada configuración regional admitida por MDN debes proporcionar su propia traducción de estas cadenas, y la plataforma debes reconocerlas como indicativas de que la construcción necesitas un tratamiento especial.

Las localizaciones se almacenan en [Yari](https://github.com/mdn/yari/tree/main/markdown/localizations) como archivos JSON en formato [gettext](https://www.gnu.org/software/gettext/). Consulta estos archivos para determinar qué cadena se debes usar en lugar de "Nota:" o "Advertencia:" para esa configuración regional. Si no se define un archivo local, se usará el inglés como alternativa.
...
> Así es como se escribe una nota.
>
> Puedes contener bloques de código.
>
> ```js
...
<div class="notecard note">
  <p><strong>Nota:</strong> Así es como se escribe una nota.</p>
  <p>Puedes contener bloques de código.</p>
...
- Cada uno de estos elementos `<li>` de GFM de nivel superior debes contener, como elemento final, un elemento `<ul>` de GFM.
- Este `<ul>` final anidado debes contener un único elemento `<li>` de GFM, cuyo contenido de texto debes comenzar con ": " (dos puntos seguidos de un espacio). Este elemento puedes contener elementos de bloque, incluidos párrafos, bloques de código, listas incrustadas y notas.
...
- `term2`
  - : Mi descripción del term2
    ...
    Puedes tener varios párrafos y también bloques de código:
  ...
  <li>
    ...
    <ul>
      <li>
        ...
        <p>Puedes tener varios párrafos y también bloques de código:</p>
        <pre>
...
Como solución alternativa para los casos en los que un autor necesitas asociar varios elementos `<dt>` con un único `<dd>`, considera proporcionarlos como un único `<dt>` que contenga varios términos, separados por comas, como este:
...
Entonces, el principio general es que los autores debesn usar la sintaxis de GFM Markdown cuando puedan y recurrir al HTML sin formato cuando sea necesario o cuando HTML sea más legible. Para obtener más información, véase [Cuándo usar tablas HTML](#cuándo_usar_tablas_html).

### Estilo de sintaxis de tablas de GFM

En la sintaxis de tablas de GFM, los autores pueden omitir el símbolo de tubería inicial y final de las filas. Sin embargo, por razones de legibilidad, los autores de MDN debesn incluirlos. Además, los autores debesn proporcionar espacios finales en las filas, de modo que todas las celdas de una columna tengan la misma longitud en texto sin formato.

Es decir, los autores de MDN debesn usar este estilo:
...
Hay tres circunstancias principales en las que los autores debesrían usar tablas HTML en lugar de la sintaxis de GFM:
...
- Las tablas de GFM debesn tener una fila de encabezado.
...
- Las tablas de GFM no puedesn tener clases asignadas.
...
Si un autor necesitas usar alguna de las funciones no compatibles, debes escribir la tabla en HTML.
...
Incluso cuando una tabla se puedes escribir en GFM, a veces es mejor usar HTML, porque GFM usa un enfoque "{{Glossary("ASCII")}} art" para las tablas que no es legible cuando las filas de la tabla son largas. Considera la siguiente tabla:
...
Estas páginas no se pueden representar en GFM porque tienen una columna de encabezado, por lo que los escritores debesn usar HTML en este caso.
Para obtener el estilo especial, los escritores debesn aplicar la clase `"properties"` a la tabla:
...
Los escritores podrán usar los elementos {{HTMLElement("sup")}} y {{HTMLElement("sub")}} si es necesario, pero debesn utilizar alternativas si es posible. En particular:
...
Por lo tanto, el primer párrafo debes ser a la vez breve e informativo.
