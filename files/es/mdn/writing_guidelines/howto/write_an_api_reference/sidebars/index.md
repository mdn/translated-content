...
Puedes incluir un menú lateral personalizado en las páginas de referencia de la API para que muestre enlaces a interfaces relacionadas, tutoriales y otros recursos relevantes solo para esa API.
...
1. Crea sus páginas de referencia de API.
...
El archivo `GroupData.json` contiene todos los datos relacionados con los enlaces que debesn aparecer en los menús laterales de referencia de la API.
...
2. Haz una copia del repositorio de contenido de MDN, cree una nueva rama para contener tus cambios y clone el repositorio localmente.
3. Revise su nueva rama antes de comenzar a trabajar y asegúrese de enviar tus cambios en ella después de terminar.
4. Crea una solicitud de incorporación para que el equipo de MDN pueda revisar su trabajo y solicitar cambios si es necesario.

El archivo `GroupData.json` se puedes encontrar dentro del directorio `files/jsondata/`.
...
Por ejemplo, consulta la página [API Fetch](/es/docs/Web/API/Fetch_API) en MDN.
...
3. `"methods"`: el valor es un arreglo que debes contener cualquier método que la especificación agregue a las interfaces asociadas con otras API, como los métodos de instanciación creados en {{domxref ("Navigator")}} o {{domxref("Window")}}.
...
4. `"properties"` — el valor es un arreglo que debes contener todas las propiedades asociadas con la API.
   Esto puedes incluir propiedades que son miembros de interfaces definidas en la especificación de la API y propiedades que la API define en otras interfaces.
...
5. `"events"`: el valor es una matriz que debes contener todos los eventos asociados con la API, definidos en la especificación de la API o en otro lugar.
...
7. `"diccionarios"`: una serie de cadenas que enumeran todos los diccionarios que forman parte de la API.
   En general, aquí solo se debesn enumerar los diccionarios utilizados por más de una propiedad o método, a menos que tengan un significado especial o sea probable que requieran referencias de varias páginas.
...
8. `"types"`: un arreglo de definiciones de tipo y tipos enumerados definidos por la API.
   Puedes optar por enumerar solo aquellos que son de especial importancia o a los que se hace referencia desde varias páginas, con el fin de mantener la lista corta.
   > [!NOTE]
...
9. `"callbacks"`: el valor es un arreglo que contiene una lista de todos los tipos de _callback_ definidos para la API.
   Puedes que le resulte innecesario utilizar este grupo, incluso en las API que incluyen tipos de _callbacks_, ya que a menudo no son útiles para documentar por separado.
...
Se añaden decoraciones para tubmiembros experimentales (etiqueta "Experimental"), no estándar (etiqueta "Non Standard" o "Non-standard") o en desuso (etiqueta "Deprecated").
