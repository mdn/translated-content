...
> Los valores debesn actualizarse adecuadamente para el constructor.
>
> ```md
> ---
...
>     Ten en cuenta que el nombre de la función del constructor en el slug omite los paréntesis (termina en `NombreDeLaInterfazPadre` no `NombreDeLaInterfazPadre()`).
> - **page-type**
...
>
>     Ten en cuenta que es posible que primero necesite crear/actualizar una entrada para el constructor de la API en nuestro [repositorio de datos de compatibilidad del navegador](https://github.com/mdn/browser-compat-data), y la entrada para la API debesrá incluir información de especificación.
>     Ve a nuestra [guía sobre cómo hacer esto](/es/docs/MDN/Writing_guidelines/Page_structures/Compatibility_tables).
>
> ---
>
...
>   Si la tecnología que está documentando no es experimental, debesría eliminar esto.
>   Si es experimental, y la tecnología está oculta detrás de una preferencia en Firefox, también debesría completar una entrada para ella en la página [Características experimentales en Firefox](/es/docs/Mozilla/Firefox/Experimental_features).
...
>   Si no lo es, entonces puedes eliminar la llamada al macro.
...
>   Si no lo es, entonces puedes eliminar la llamada al macro.
>   Si lo es, entonces también debesría completar una entrada para ella en la página [Funciones restringidas a contextos seguros](/es/docs/Web/Security/Secure_Contexts/features_restricted_to_secure_contexts).
...
>   Para generar el menú lateral correcto para tu API, debes agregar una entrada `GroupData` a nuestro repositorio de GitHub e incluir el nombre de la entrada dentro de la llamada a la macro en lugar de _GroupDataName_.
>   Ve a nuestra [guía de menús laterales de referencia de API](/es/docs/MDN/Writing_guidelines/Howto/Write_an_api_reference/Sidebars) para obtener información sobre cómo hacerlo. Recuerda eliminar el macro `\{{MDNSidebar}}` cuando copie esta página.
>
...
Idealmente, esto debesría ser una o dos frases cortas.
...
- `parámetro1` {{optional_inline}}
  - : Incluye una breve descripción del parámetro y lo que hace aquí. Incluye un término y definición para cada parámetro.
...
Incluye una descripción del valor de retorno del constructor, incluido el tipo de dato y lo que representa.
...
Incluye una lista de todas las excepciones que puede generar el constructor. Incluye un término y definición para cada excepción.
...
Cada ejemplo debes tener un encabezado H3 que nombre el ejemplo. El encabezado debes ser descriptivo de lo que hace el ejemplo. Por ejemplo, "Un ejemplo simple" no dice nada sobre el ejemplo y, por lo tanto, no es un buen encabezado. El encabezado debesría ser conciso. Para una descripción más larga, use el párrafo después del encabezado.
