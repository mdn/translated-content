...
- O volume do audio relativo a {{domxref("KeyboardEvent.key")}} foram renomeados. `"VolumeDown"` é agora `"AudioVolumeDown"`, `"VolumeUp"` é agora `"AudioVolumeUp"`, e `"VolumeMute"` e agora `"AudioVolumeMute".` Isso trás Firefox p*ara alinhamento com os últimos rascunhos de especificações de UI Events* ({{bug(1272578)}}). Veja {{SectionOnPage("/pt-BR/docs/Web/API/KeyboardEvent/code", "Code values")}} para uma lista completa de possibilidades de códigos.
