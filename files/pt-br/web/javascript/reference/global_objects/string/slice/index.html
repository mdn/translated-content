...
<dl>
 ...
 <dd><p>É o índice, de base zero, onde se inicia a extração. Se for um número negativo, será o mesmo que executar <em><code>str.length + startIndex</code></em> (por exemplo, se <em><code>startIndex</code></em> for <em><code>-3</code></em>, será o mesmo que executar <em><code>str.length - 3</code></em>).</p>
 <pre class="brush: js notranslate">const nome = 'Ricardo';
console.log(nome.slice(-3)); // retorna 'rdo'</pre>
 <p>Se <em><code>startIndex</code></em> for maior ou igual a <code><var>str</var><em>.length</em></code>, uma string vazia será retornada.</p>
 <pre class="brush: js notranslate">const nome = 'Ricardo';
console.log(nome.slice(7)); // retorna &lt;empty string&gt;'
</pre>
 </dd>
 <dt><code>endIndex</code></dt>
 <dd><p>Opcional. É o índice, de base zero, onde termina a extração. O caractere que possuir este índice não será incluso.</p>
 <p>Se omitido ou possuir valor de <em><code>undefined</code></em>, ou for maior que <code><var>str</var><em>.length</em></code>, <em><code>slice()</code></em> extrairá até o fim da string.</p>
 <pre class="brush: js notranslate">const nome = 'Ricardo';
...
console.log(nome.slice(0, 7)); // retorna 'Ricardo'</pre>
 <p>Se negativo, é o mesmo que executar <em><code>str.length + endIndex</code></em> onde <em><code>str.length</code></em> é o tamanho da string (por exemplo, se <em><code>endIndex</code></em> é <em><code>-3</code></em>, é como executar <code><em>str.length - 3</em></code>).</p>
 <pre class="brush: js notranslate">const nome = 'Ricardo';<strong>
</strong>console.log(nome.slice(0, -3)); // retorna 'Rica'
</pre>
 <p>Se for um valor diferente de indefinido e, ao mesmo tempo não for um número, uma string vazia será retornada</p>
 <pre class="brush: js notranslate">const nome = 'Ricardo';
