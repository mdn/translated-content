...
- `thisArg`
  - : O valor de `this` proveu a chamada para _`fun`_. Note que `this` pode não ser o valor atual visto pelo método: se esse método é uma função em {{jsxref("Strict_mode", "non-strict mode", "", 1)}} code, {{jsxref("Operators/null", "null")}} e {{jsxref("undefined")}} serão reescritos com o objeto global, e valores primitivos serão encaixados.
- `arg1, arg2, ...`
