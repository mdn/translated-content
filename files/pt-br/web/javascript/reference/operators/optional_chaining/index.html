...
<p>O operador <strong><code>?.</code></strong> funciona de maneira similar ao operador <code>.</code> de encadeamento, exceto que, ao invés de causar um erro se a referência é <a href="/pt_BR/docs/Glossary/nullish">nullish</a> ({{JSxRef("null")}} ou {{JSxRef("undefined")}}), a expressão sofre um "curto-circuito" e retorna com um valor de <code>undefined</code>. Quando utilizado com uma chamada de função, retorna <code>undefined</code> se a função executada não existir.</p>
...
<p>Usar encadeamento opcional com chamadas de função faz com que a expressão automaticamente retorne <code>undefined</code> ao invés de lançar uma exceção se o método não é encontrado:</p>
