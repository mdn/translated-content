...
- `declarações`
  - : A declaração é executada pelo menos uma vez e re-executada cada vez que a condição (`condition`) for avaliada como verdadeira (true). Para executar múltiplas declarações dentro do laço, use um {{jsxref("Statements/block", "block")}} declaração (`{ ... }`) ao grupo dessas declarações.
- `condição`
