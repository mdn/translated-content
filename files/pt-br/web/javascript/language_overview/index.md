...
Você pode converter uma string em um inteiro usando a função embutida [`parseInt()`](/en/JavaScript/Reference/Global_Objects/parseInt "en/Core_JavaScript_1.5_Reference/Global_Functions/parseInt"). Ela tem um segundo parâmetro opcional para a base da conversão, parâmetro esse que você deveria sempre prover:
...
Similarmente, você pode fazer a conversão de números de ponto flutuante usando a função embutida [`parseFloat()`](/en/JavaScript/Reference/Global_Objects/parseFloat "en/JavaScript/Reference/Global Objects/parseFloat") que usa a base 10 sempre, ao contrário de seu primo [`parseInt()`](/en/JavaScript/Reference/Global_Objects/parseInt "en/JavaScript/Reference/Global Objects/parseInt").
...
Um valor especial chamado [`NaN`](/en/JavaScript/Reference/Global_Objects/NaN "en/Core_JavaScript_1.5_Reference/Global_Properties/NaN") (sigla de "Not a Number ou Não é Número") é retornado se a string não é um valor numérico:
...
Você pode testar se é `NaN` usando a função embutida [`isNaN()`](/en/JavaScript/Reference/Global_Objects/isNaN "en/Core_JavaScript_1.5_Reference/Global_Functions/isNaN"):
...
JavaScript também tem os valores especiais [`Infinity`](/en/JavaScript/Reference/Global_Objects/Infinity "en/Core_JavaScript_1.5_Reference/Global_Properties/Infinity") e `-Infinity`:
...
Você pode testar se o valor é `Infinity`, `-Infinity` e `NaN` usando a função embutida [`isFinite()`](/en/JavaScript/Reference/Global_Objects/isFinite "en/Core_JavaScript_1.5_Reference/Global_Functions/isFinite"):
...
> **Nota:** As funções [`parseInt()`](/en/JavaScript/Reference/Global_Objects/parseInt "en/JavaScript/Reference/Global Objects/parseInt") e [`parseFloat()`](/en/JavaScript/Reference/Global_Objects/parseFloat "en/JavaScript/Reference/Global Objects/parseFloat") fazem a conversão da string até alcançarem um caracter que não é válido para o formato numérico especificado, então elas retornam o número convertido até aquele ponto. Contudo, o operador "+" simplesmente converte a string em `NaN` se tiver algum caracter inválido nela. Apenas tente por si mesmo converter a string "10.2abc" usando cada um desses métodos no console e entenderá melhor essas diferenças.

## Strings

Strings em JavaScript são sequências de caracteres. Para ser mais exato, elas são sequências de [Unicode characters](/en/JavaScript/Guide/Obsolete_Pages/Unicode "en/Core_JavaScript_1.5_Guide/Unicode"), em que cada um deles é representado por um número de 16-bits. Isso deveria ser uma notícia bem-vinda para aqueles que tiveram que lidar com internacionalização.

Se você quiser representar um único caractere, você só tem que usar uma string de tamanho 1.

Para obter o tamanho de uma string, acesse sua propriedade [`length`](/en/JavaScript/Reference/Global_Objects/String/length "en/Core_JavaScript_1.5_Reference/Global_Objects/String/length"):
...
Novas variáveis em JavaScript são declaradas usando a palavra-chave [`var`](/en/JavaScript/Reference/Statements/var "en/Core_JavaScript_1.5_Reference/Statements/var"):
...
O [operador](/en/JavaScript/Reference/Operators/String_Operators "en/Core_JavaScript_1.5_Reference/Operators/String_Operators") `+ `também faz concatenação de string:
...
[Comparações](/en/JavaScript/Reference/Operators/Comparison_Operators "en/Core_JavaScript_1.5_Reference/Operators/Comparison_Operators") em JavaScript podem ser feitas usando `<`, `>`, `<=` e `>=`. Isso funciona tanto para strings como para números. A igualdade é um pouco menos simples. O operador igual-duplo faz a coersão de tipo se você colocar tipos diferentes, algumas vezes com resultados interessantes:
...
JavaScript também tem [operações de bit-a-bit](/en/JavaScript/Reference/Operators/Bitwise_Operators "en/Core_JavaScript_1.5_Reference/Operators/Bitwise_Operators"). Se quiser usá-las, elas estarão lá.
...
Note que esse truque só deveria ser usado em vetores que você sabe não conter valores "falsy" (vetores de objeto ou nós [DOM](/en/DOM "en/DOM") por exemplo). Se você iterar sobre dados numéricos que possam ter o 0 ou sobre dados string que possam ter uma string vazia, você deveria usar a segunda forma como alternativa.

Uma outra forma de iterar é usar o laço [`for...in`](/en/JavaScript/Reference/Statements/for...in "en/Core_JavaScript_1.5_Reference/Statements/for...in"). Note que se alguém adicionou novas propriedades ao `Array.prototype`, elas também podem ser iteradas usando este laço:
...
Pode parecer um pouco bobo, mas no corpo da função você tem acesso a uma variável adicional chamada [`arguments`](/en/JavaScript/Reference/Functions_and_function_scope/arguments "En/Core_JavaScript_1.5_Reference/Functions_and_function_scope/arguments"), que é um objeto parecido com um vetor que contém todos os valores passados para a função. Vamos rescrever a função add para tomarmos tantos valores quanto quisermos:
...
Porém, seria legal se pudéssemos reusar a função que já tínhamos criado. Felizmente, JavaScript lhe permite chamar a função, e chamá-la com um conjunto arbitrário de argumentos, usando o método [`apply()`](/en/JavaScript/Reference/Global_Objects/Function/apply "en/Core_JavaScript_1.5_Reference/Global_Objects/Function/apply") presente em qualquer objeto função.
...
JavaScript lhe permite chamar funções recursivamente. Isso é particularmente útil quando estamos lidando com estruturas de árvore, como quando estavamos navegando no [DOM](/en/DOM "en/DOM").
...
> **Nota:** Para uma discursão mais detalhada de programação orientada a objetos em JavaScript, veja [Introdução a JavaScript Orientado a Objeto](/en/JavaScript/Introduction_to_Object-Oriented_JavaScript "https://developer.mozilla.org/en/Introduction_to_Object-Oriented_JavaScript").
...
Há algo aqui que não havíamos visto anteriormente: a palavra-chave '[`this`](/en/JavaScript/Reference/Operators/this "en/Core_JavaScript_1.5_Reference/Operators/Special_Operators/this_Operator")'. Usada dentro de uma função, '`this`' refere-se ao objeto corrente. O que aquilo de fato significa é especificado pelo modo em que você chamou aquela função. Se você chamou-a usando [notação ponto ou notação colchete](/en/JavaScript/Reference/Operators/Member_Operators "en/Core_JavaScript_1.5_Reference/Operators/Member_Operators") em um objeto, aquele objeto torna-se '`this`'. Se a notação ponto não foi usada pela chamada, '`this`' refere-se ao objeto global. Isso é uma frequente causa de erros. Por exemplo:
...
`apply()` tem uma função irmã de nome [`call`](/en/JavaScript/Reference/Global_Objects/Function/call "en/Core_JavaScript_1.5_Reference/Global_Objects/Function/call"), que novamente permite você configurar o '`this`' mas toma uma lista expandida de argumentos, ao invés de um array.
