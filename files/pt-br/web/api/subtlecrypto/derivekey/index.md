...
- `algorithm` é um objeto definindo o algoritmo de derivação a se usar. Os valores suportados são:
  ...
  - `{"name": "PBKDF2", salt, iterations, hash}` where _`salt`_ is an {{jsxref("ArrayBuffer")}} ou um {{jsxref("ArrayBufferView")}}, _`iterations`_ é o número de interações e _`hash`_ é um {{jsxref("String")}} identificando o algoritmo hashing para uso.
