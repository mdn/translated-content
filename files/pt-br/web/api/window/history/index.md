...
Consulte [Manipulação do histórico do navegador](/pt-BR/docs/Web/API/History_API) para obter exemplos e detalhes. Em particular, esse artigo explica os recursos de segurança dos métodos pushState () e replaceState () que você deve conhecer antes de usá-los.
...
Não há nenhuma maneira de limpar o histórico da sessão ou desabilitar a navegação para trás / para frente do código não privilegiado. A solução disponível mais próxima é o método [location.replace ()](/pt-BR/docs/Web/API/Window/location#replace), que substitui o item atual do histórico da sessão pelo URL fornecido.
