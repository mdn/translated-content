...
<p><span class="seoSummary">A interface <strong><code>RTCPeerConnection</code></strong> cria uma conexão WebRTC entre um computador local e um ponto remoto. Esta interface fornece métodos que possibilitam a conexão a um ponto remoto, mantendo e monitorando a conexão, e fechando-a quando não for mais necessária.</span></p>
...
<dl>
 ...
 <dd>Cria um novo {{domxref("RTCDTMFSender")}},associado ao específico {{domxref("MediaStreamTrack")}}, que irá permitir enviar {{Glossary("DTMF")}} sinal de telefone pela conexão</dd>
</dl>

<h2 id="Eventos">Eventos</h2>

<p>Monitore esses eventos utilizando {{domxref("EventTarget.addEventListener", "addEventListener()")}} ou atribuindo um ouvinte de evento à propriedade oneventname dessa interface.</p>



<dl>
 <dt>{{domxref("RTCPeerConnection.connectionstatechange_event", "connectionstatechange")}}</dt>
 <dd>Enviado ao objeto RTCPeerConnection quando o status de conectividade geral do RTCPeerConnection é alterado.<br>
 Também disponível através da propriedade do manipulador de eventos {{domxref ("RTCPeerConnection.onconnectionstatechange", "onconnectionstatechange")}}.</dd>
</dl>
...
<dl>
 ...
 <dd>Sent to the object when a new ICE candidate arrives during negotiation.<br>
 Also available using the {{domxref("RTCPeerConnection.onicecandidate", "onicecandidate")}} event handler property.</dd>
 ...
 <dd>Sent to the <code>RTCPeerConnection</code> when it's time to perform ICE negotiation. This can happen both when first opening a connection as well as any time it's necessary to adapt to changing network conditions.<br>
 Also available using the {{domxref("RTCPeerConnection.onnegotiationneeded", "onnegotiationneeded")}} event handler property.</dd>
 <dt>{{domxref("RTCPeerConnection.icecandidate_event", "icecandidate")}}</dt>
 ...
 <dt>{{domxref("RTCPeerConnection.negotiationneeded_event", "negotiationneeded")}}</dt>
 <dd>Sent to the <code>RTCPeerConnection</code> when negotiation or renegotiation of the ICE connection needs to be performed. The receiver should respond by creating an offer and sending it to the other peer.<br>
