...
> **Note:** **FYI:** Esse processo, aparentemente complicado existe para que seja óbvio para o cliente se o servidor suporta ou não o WebSockets. Isso é importante por causa de problemas com segurança que aparecem se o servidor aceita a conexão WebSocket mas interpreta que os dados são uma requisição HTTP.
...
> **Note:** O servidor pode enviar outros cabeçalhos como Set-Cookie, ou perguntar por autenticação ou redirecionar via outros códigos de status, antes enviando a resposta do handshake.
...
> **Note:** Você tem que mascarar as mensagens mesmo quando usando secure socket (SSL).
...
> **Note:** Se você receber mais de um ping antes de ter a chance de enviar um pong, você envia apenas um pong.
...
> **Note:** Códigos WebSocket, extensões, subprotocols, etc. são registrados na [IANA WebSocket Protocol Registry](http://www.iana.org/assignments/websocket/websocket.xml).
...
> **Note:** **Essa sessão precisa ser mais desenvolvida. Por favor edite se você tiver conhecimento sobre.**
...
> **Note:** Extensões são explicadas nas sessões 5.8, 9, 11.3.2 e 11.4 da especificação.
...
> **Note:** Subprotocolos são explicados nas sessões 1.9, 4.2, 11.3.4 e 11.5 da especificação.
...
> **Warning:** O servidor não pode enviar mais de um cabeçalho `Sec-Websocket-Protocol`.
