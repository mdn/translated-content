...
| ---------------- | ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `binaryType`     | {{jsxref("String")}}         | Uma string indica o tipo de dado binário que está sendo transmitido pela conexão. Este deve ser tanto "blob" se o objeto DOM {{domxref("Blob")}} estiver sendo usado ou "arraybuffer" se os objetos {{jsxref("ArrayBuffer")}} estiver sendo usada                                                                                               |
...
| `extensions`     | {{jsxref("String")}}         | As extensões selecionadas pelo servidor. Este é atualmente apenas a string vazia ou uma lista de extensões negociadas pela conexão.                                                                                                                                                                                                             |
...
| `protocol`       | {{jsxref("String")}}         | Uma sequência de caracteres que indica o nome do sub-protocolo selecionado pelo servidor, este será um dos parâmetros especificados no `protocolo` ao criar o objeto WebSocket.                                                                                                                                                                 |
...
| `url`            | {{jsxref("String")}}         | O URL resolvido pelo construtor. Este é sempre um URL absoluto. **Apenas leitura.**                                                                                                                                                                                                                                                             |
...
> [!NOTE]
> `Note: No Gecko, este método não suporta nenhum parâmetro antes do Gecko 8.0.`
