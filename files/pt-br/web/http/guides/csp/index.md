...
A política é uma string contendo as [diretivas de políticas](/pt-BR/docs/Web/HTTP/Reference/Headers/Content-Security-Policy) descrevendo seu Content Security Policy.

## Escrevendo uma política

Uma politica é descrita usando uma série de [diretivas de políticas](/pt-BR/docs/Web/HTTP/Reference/Headers/Content-Security-Policy), cada uma descrevendo uma política para certos tipos de recursos ou áreas de política. Sua política deve incluir uma diretiva de política padrão `default-src`, que cobre os outros tipos de recursos quando eles não possuem política própria. (Para uma lista completa, veja a descrição da diretiva default-src.) Uma política precisa incluir uma diretiva `default-src` ou script-src para evitar que scripts inline sejam executados, assim como bloquear o uso de `eval()`. Uma política precisa incluir uma diretiva `default-src` ou `style-src`para restringir estilos inline de serem aplicados a partir de um elemento `<style>` ou um `.style` atributo.

A sintaxe para uma política é uma string de diretivas separadas por ponto e vírgula `;`, cada seguindo a sintaxe descrita em [Diretivas de políticas suportadas](/pt-BR/docs/Web/HTTP/Reference/Headers/Content-Security-Policy#supported_policy_directives).
...
Note that this example doesn't specify a [`script-src`](/pt-BR/docs/Web/HTTP/Reference/Headers/Content-Security-Policy#script-src); with the example CSP, this site uses the setting specified by the [`default-src`](/pt-BR/docs/Web/HTTP/Reference/Headers/Content-Security-Policy#allow) directive, which means that scripts can be loaded only from the originating server.
...
- [Introducing Content Security Policy](/pt-BR/docs/Web/HTTP/Guides/CSP)
- [CSP policy directives](/pt-BR/docs/Web/HTTP/Reference/Headers/Content-Security-Policy)
- [Using CSP violation reports](/pt-BR/docs/Web/HTTP/Guides/CSP)
