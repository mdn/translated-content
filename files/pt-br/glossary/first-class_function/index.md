...
> **Note:** **Mesmo se sua função for nomeada,** você pode usar o nome da variável pra chamá-la. Nomeá-la será útil quando for debugar seu código. _Mas não afetará a maneira que à chamamos._
...
> **Note:** A função que passamos como um argumento pra outra função, chamou uma **[Função Callback](/pt-BR/docs/Glossary/Callback_function).** _`sayHello` é uma Função Callback._
...
> **Note:** Uma função que retorna uma função é chamada de **Higher-Order Function**
...
> **Note:** Você tem que usar outra variável. Se você fosse chamar `sayHello` diretamente, ela retornaria a função em si **sem chamar a função retornada**.
