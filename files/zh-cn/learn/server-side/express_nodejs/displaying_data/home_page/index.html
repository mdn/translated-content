...
<p>成功时，回调函数调用 <code><a href="https://expressjs.com/en/4x/api.html#res.render">res.render()</a></code>，指定名为 '<strong>index</strong>' 的视图（模板），以及一个对象包含了要插入其中的数据（这包括我们模型计数的结果对象）。数据以键值对的形式提供，可以使用键在模板中访问。</p>
