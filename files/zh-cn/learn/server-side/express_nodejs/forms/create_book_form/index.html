...
---
<p><a href="/en-US/docs/Learn/Server-side/Express_Nodejs/forms$edit#Create_book_form">Edit</a> 此子文档显示如何定义页面/表单以创建<code>Book</code>对象。这比相同的作者<code>Author</code>或种类<code>Genre</code>页面稍微复杂一点，因为我们需要在我们的书本表单中，获取并显示可用的作者和种类记录。</p>
...
<pre class="brush: js">sanitizeBody('*').trim().escape(),</pre>
...
<pre class="brush: js">sanitizeBody('genre.*').trim().escape(),</pre>
...
<p>运行应用程序，将浏览器打开到<a href="http://localhost:3000/">http://localhost:3000</a>，然后选择 Create new book 链接。如果一切设置正确，您的网站应该类似于以下屏幕截图。提交有效的图书后，应将其保存，然后您将进入图书详细信息页面。</p>
