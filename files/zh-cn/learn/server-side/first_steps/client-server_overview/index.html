...
<p>到目前为止的讨论中还没有真正的代码，因为我们还没有选择一个 web 框架来写我们的代码呢！然而这个讨论仍旧十分重要，因为我们描述的行为必须通过你的服务器端代码来实现，不管你选择什么编程语言和 web 框架。</p>

<h2 id="网络服务器和HTTP（入门）">网络服务器和 HTTP（入门）</h2>

<p>网络浏览器通过超文本标记语言传输协议（<a href="/en-US/docs/Web/HTTP">HTTP</a>）与网络服务器（<a href="/en-US/docs/Learn/Common_questions/What_is_a_web_server">web servers</a>）。 当你在网页上点击一个链接、提交一个表单、或者进行一次搜索的时候，浏览器发送一个 HTTP 请求给服务器。</p>
...
<ul>
 ...
 <li>一个定义了请求行为的方法（比如，获得一个文档或者上传某些数据）。不同的方法/动作以及与他们相关的行为罗列如下：
  <ul>
   <li>GET：获取一份指定（比如一个包含了一个产品或者一系列产品相关信息的 HTML 文档）。</li>
   <li>POST：创建一份新的资源（比如给 wiki 增加一片新的文章、给数据库增加一个新的节点）。</li>
   ...
   <li><code>TRACE</code>、<code>OPTIONS</code>、<code>CONNECT、PATCH</code>等动作是为一些不常见任务设计的，因此我们在这里的讲解不会涉及到他们。</li>
  </ul>
 </li>
 <li>额外的信息可以和请求一起被编码（比如 HTML 表单数据）。信息可以被编码成如下：
  <ul>
   <li>URL 参数：GET 请求通过在 URL 末尾增加的键值对，来编码包含在发送给服务器的 URL 中的数据——比如，<code>http://mysite.com<strong>?name=Fred&amp;age=11</strong></code>，你经常会用到问号（？）来将 URL 剩余的部分和 URL 参数分隔开来，一个赋值符号（=）将名称和与之相关的值分隔开来，然后一个“&amp;”符号分割不同的键值对。当他们被用户改变然后提交时，URL 参数具有与生俱来地“不安全性”。因此，一个 URL 参数或者 GET 请求是不会用来在服务器上更新数据的。</li>
...
<p>网络服务器等待来自客户的请求信息，当请求到达时处理它们，然后发给浏览器 HTTP 响应消息。回应包含一个 HTTP 响应状态码（<a href="/en-US/docs/Web/HTTP/Status">HTTP Response status code</a>）来暗示请求是否成功 (比如 "<code>200 OK</code>" 连接成功， "<code>404 Not Found</code>" 资源没有找到，"<code>403 Forbidden</code>" 用户没有被授权查看资源，等等).  一个成功的响应主体，会包含 GET 请求所请求的资源。</p>
...
<p>你可以通过点击一个链接或者在网站进行一次搜索（比如搜索引擎的首页）做出一次简单的 GET 请求。比如，当你在 MDN 上进行一次对“客户端概览”词条的搜索时，HTTP 请求就被发送出去了，你将会看到正如下面一样被展示出来的文本信息（展示出来的信息不一定是相同的，因为其中一部分信息还取决于你的浏览器）。</p>

<div class="note">
<p><strong>备注：</strong> HTTP 消息的格式是在“网络标准”（<a href="http://www.rfc-editor.org/rfc/rfc7230.txt">RFC7230</a>）中定义的。你不需要知道这个标准的细节，但是现在你至少得知道所有这些是来自哪儿的！</p>
</div>

<h4 id="请求">请求</h4>

<p>每一行请求都包含着相关信息。第一部分被称为<strong>header</strong>，并且包含着关于这个请求的有用信息，同样地一个<a href="/en-US/docs/Learn/HTML/Introduction_to_HTML/The_head_metadata_in_HTML">HTML head</a>包含着关于 HTML 文档的有用信息（但是却没有自身的实际内容，内容在主体里面）。</p>
...
<p>header 的剩余部分还包括一些回应的其他信息（比如回应在什么时候生成的），有关服务器的信息，还有它期望浏览器如何处理这个包（比如， <code>X-Frame-Options: DENY</code> 告诉浏览器不允许这个网页嵌入在其他网站的 HTML 元素{{htmlelement("iframe")}}上。</p>

<h3 id="POST_请求响应举例">POST 请求/响应举例</h3>

<p>当你提交一个表单，并且希望表单所包含的信息存储到服务器的时候，你就生成了一次 HTTP POST 请求。</p>
...
<p>当用户想要导航到页面时，浏览器会发送一个指定 HTML 页面的 URL 的 HTTP 的<code>GET</code>请求。<br>
 服务器从它的文件系统中检索所请求的文档，并返回包含文档和 HTTP 响应状态码“<code>200 OK</code>”(表示成功) 的 HTTP 响应。服务器可能会返回一个不同的状态码，例如，"<code>404 Not Found</code>"表明文件不在服务器上，或者"<code>301 Moved Permanently</code>"表明如果文件存在，则被重定向到另一个位置。</p>
...
<p>动态站点可以根据特定的请求 URL 和数据生成和返回内容 (而不是总是返回同一个 URL 的硬编码文件)。使用产品网页的示例，服务器将把产品“数据”存储在数据库中，而不是单独的 HTML 文件。当接收到一个产品的 HTTP <code>GET</code> 请求时，服务器将确定产品 ID，从数据库中获取数据，然后通过将数据插入到 HTML 模板中来构造响应的 HTML 页面。与静态站点相比，这有很大的优势</p>

<p>通过使用数据库，可以有效地将产品信息存储在易于扩展、可修改和可搜索的方式中。</p>

<p>使用 HTML 模板可以很容易地改变 HTML 结构，因为这只需要在一个模板中的某一处地方完成，而不需要跨越数千个静态页面。</p>
...
<p>在服务器中，更新数据库中的记录的操作将被类似地与上述过程一样处理，但是更新数据库的这一类的操作，应该指定来自浏览器的 HTTP 请求为<code>POST</code>请求。</p>
...
<h3 id="返回HTML以外的内容">返回 HTML 以外的内容</h3>

<p>服务器端网站代码并非只能在响应中返回 HTML 代码片段/文件。它可以动态地创建和返回其他类型的文件（text，PDF，CSV 等）甚至是数据（JSON，XML 等）。<br>
 <br>
...
<h2 id="web框架简化服务器端的web编程">web 框架简化服务器端的 web 编程</h2>

<p>服务器端 web 框架使得编写解决我们上面描述的操作的代码变得简单得多。</p>
...
<p>显然地 web 框架可以帮助你解决很多问题。我们在下一篇文章里将会大量讨论这些好处和一些流行的 web 框架。</p>

<h2 id="总结">总结</h2>

<p>到这里你应该对于服务器端代码不得不进行的操作有一个整体上的理解，并且知道一个服务器端 web 框架是从那些方面让这些变得更简单的。</p>

<p>在接下来的模块里面我们会帮助你选择对于你的第一个网站来说最适合的 web 框架。</p>
