...
<div class="warning">
<p><strong>警告：</strong> 切记，对于网站安全来说最重要一点就是<strong>“永远不要相信浏览器端提交的数据”</strong>。 这些数据包括使用<code>GET</code>方式请求时 URL 中的参数，<code>POST</code> 方式请求的数据，HTTP headers 和 cookies，以及用户上传的文件等等。请确保一定要检查和清洗这些提交的数据。对于网站安全来说，总是要做好最坏的打算。</p>
</div>
...
<ol>
 ...
 <li>输入姓名、生日等信息，随后在 Last Name 这个字段里面填入以下的内容：<br>
  ...
  <div class="note">
  <p><strong>备注：</strong> 这一段代码并没有任何杀伤力，在执行的时候只会在浏览器中弹出一个警告提示框。如果这个警告提示框出现，则表明本网站存在可被 XSS 攻击的漏洞。</p>
  </div>
 </li>
 <li>点击 <strong>Submit</strong> 按钮保存信息。</li>
 <li>保存后的作者信息将会显示为下图的样式。因为 XSS 防护措施的存在，注入代码中的<code>alert()</code>部分并没有执行，而只是用文本的方式直接显示了出来。<img alt="Author detail view XSS test" src="author_detail_alert_xss.png"></li>
</ol>
...
<div class="note">
<p><strong>备注：</strong> 这个示例里面的黑客没有考虑对钱下手。而现实生活中的黑客则极有可能会产生更加危险的操作（例如，把钱转入他们自己的账户中等等）。</p>
</div>
