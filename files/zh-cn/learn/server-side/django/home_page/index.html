...
<div class="note">
<p><strong>备注：</strong> Django 允许您以任何您喜欢的方式构建您的 URL - 您可以如上所示编码 URL 正文中的信息，或使用 URL <code>GET</code>参数（例如 <code>/book/?id=6</code>）。无论您使用哪种方法，URL 都应保持清洁，逻辑和可读性 (<a href="https://www.w3.org/Provider/Style/URI">check out the W3C advice here</a>).<br>
 <br>
...
<div class="note">
<p><strong>备注：</strong> 本节应该特别注意。一些”材料“在所有页面都通用。</p>
</div>
...
<div class="note">
<p><strong>备注：</strong> 在 <strong>/locallibrary/locallibrary/urls.py</strong> </p>
...
<div class="note">
<p><strong>备注：</strong> 我们当然可以硬编码上面的链接（如：<code>&lt;a href="<strong>/catalog/</strong>"&gt;Home&lt;/a&gt;</code>），但是如果我们改变了主页的模式，模版将不再正确链接，使用反向网址映射会更灵活和强大。</p>
</div>
...
<div class="note">
<p><strong>备注：</strong> Django 将根据你的项目的设置文件，来查看模版的许多位置 (在已安装的应用程序中进行搜索是默认设置). 你可以查阅更多关于 Django 如何找到模版以及它支持的模版格式在<a href="https://docs.djangoproject.com/en/1.10/topics/templates/">（Templates</a> ）。</p>
</div>
...
<div class="note">
<p><strong>备注：</strong> 模版标签就像你可以在模版中使用的函数循环列表，基于变量的值执行条件操作等。除了模版标签，模版语法允许你引用模版变量（通过从视图进入模版），并使用模版过滤器，其中重新格式化变量（例如，将字符串设置为小写）。</p>
</div>
...
<div class="note">
<p><strong>备注：</strong> 我们再介绍两个额外的模版标签： <code>url</code> 和 <code>load static </code>。下文中我们会详细介绍。</p>
</div>
...
<div class="note">
<p><strong>备注：</strong> 由于本网站就是通过 <strong>django</strong> 来运维，<strong><code>\{\{</code></strong> 的模版标签 在上面代码中会运行，只能通过增加 <code>\</code>来转义，而不能直接写出“双大括号”。</p>
</div>

<p>在动态内容部分，我们的占位符（模版变量），是给我们想要视图的信息声明。变量使用“双大括号“或者“句柄“语法进行标记。</p>

<div class="note">
<p><strong>备注：</strong> 你可以轻松地识别是否使用变量或模版标签（函数），因为变量具有双括号 (<code>\{{ num_books }}</code>) 而标记被包含在带有百分比符号 (<code>{% extends "base_generic.html" %}</code>) 的单个大括号中。</p>
</div>
...
<div class="note">
<p><strong>备注：</strong> 上面的更改指定文件所在的位置，但 Django 默认不提供它们。当我们<a href="/en-US/docs/Learn/Server-side/Django/skeleton_website">created the website skeleton</a>,我们在全局 URL 映射器 r (<strong>/locallibrary/locallibrary/urls.py</strong>) 中开发 Web 服务器提供服务，你仍然需要安排它们在生产中投放。我们接下来看一看</p>
</div>

<p>更多内容—<a href="https://docs.djangoproject.com/en/1.10/howto/static-files/">Managing static files</a> (Django docs).</p>

<h4 id="链接URL">链接 URL</h4>
...
<div class="note">
<p><strong>备注：</strong> 由于尚未定义这些网页的网址，视图和模板，因此您将无法使用“ <strong>所有图书</strong>和<strong>所有作者”</strong>链接（目前我们刚刚在<code>base_generic.html</code>模板中插入了这些链接的占位符）</p>
</div>
