...
<div class="note">
<p><strong>备注：</strong> 可以看到，JavaScript 中许多代码特性和其他编程语言是一致的（ 函数、循环，等等）。尽管代码语法不尽相同，但概念基本类似。</p>
</div>
...
<p>{{ EmbedLiveSample('示例——猜数字游戏', '100%', 320) }}</p>
...
<div class="note">
<p><strong>备注：</strong> 稍后将讲解更多关于变量/常量的信息。<a href="/en-US/docs/user:chrisdavidmills/variables">参见下节</a>。</p>
</div>
...
<div>
<div>函数是可复用的代码块，可以一次编写，反复运行，从而节省了大量的重复代码。它们真的很有用。定义函数的方法很多，但现在我们先集中考虑当前这个简单的方式。这里我们使用关键字 <code>function</code> 、一个函数名、一对小括号定义了一个函数。随后是一对花括号（<code>{}</code>）。花括号内部是调用函数时要运行的所有代码。</div>
</div>

<div>
要运行一个函数代码时，可以输入函数名加一对小括号。</div>

<div>让我们尝试一下。保存你的代码并刷新浏览器页面。然后进入<br>
...
<div class="note">
<p><strong>备注：</strong> <a href="/zh-CN/docs/learn/JavaScript/Building_blocks/Functions">后续课程 </a>将讲解更多有关函数的知识。</p>
</div>
...
<p>{{ EmbedLiveSample('运算符（Operator）', '100%', 300) }}</p>
