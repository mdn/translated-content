...
<p><img alt="A basic representation of a web site architecture" src="https://mdn.mozillademos.org/files/6475/web-site-architechture@2x.png"></p>
...
<div class="note">
<p><strong>备注：</strong> 在早期，这种通用技术被称为 Asynchronous JavaScript and XML<strong>（Ajax），</strong> 因为它倾向于使用{{domxref("XMLHttpRequest")}} 来请求 XML 数据。 但通常不是这种情况 (你更有可能使用 <code>XMLHttpRequest</code> 或 Fetch 来请求 JSON), 但结果仍然是一样的，术语“Ajax”仍然常用于描述这种技术。</p>
</div>

<p><img alt="A simple modern architecture for web sites" src="https://mdn.mozillademos.org/files/6477/moderne-web-site-architechture@2x.png"></p>
...
<p><img alt="A basic web app data flow architecture" src="https://mdn.mozillademos.org/files/6479/web-app-architecture@2x.png"></p>
...
<p>首先，我们调用了<code><a href="/zh-CN/docs/Web/API/WorkerOrWindowGlobalScope/fetch">fetch()</a></code>方法，将我们要获取的资源的 URL 传递给它。这相当于现代版的 XHR 中的<code>request.open()</code>,另外，您不需要任何等效的<code>send()</code>方法。</p>

<p>然后，你可以看到<code>.then()</code>方法连接到了<code>fetch()</code>末尾 - 这个方法是<a href="/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Promise"><code>Promises</code></a>的一部分，是一个用于执行异步操作的现代 JavaScript 特性。<code>fetch()</code>返回一个 promise，它将解析从服务器发回的响应。我们使用<code>then()</code>来运行一些后续代码，这是我们在其内部定义的函数。这相当于 XHR 版本中的<code>onload</code>事件处理程序。</p>
...
<p>在 promise 的函数内部，我们做的和在 XHR 版本中差不多— 设置 <a href="/en-US/docs/Web/HTML/Element/pre"> element represents preformatted text which is to be presented exactly as written in the HTML file."><code>&lt;pre&gt;</code></a> 元素的文本内容为 text 的值。</p>
...
<p>实际上，传递给 <code>then()</code> 是一段不会立即执行的代码 — 而是当返回响应时代码会被运行。注意，你还可以选择把你的 promise 保存到一个变量里，链接 <a href="/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/then"><code>.then()</code></a> 在相同的位置。下面的代码会做相同的事情。</p>
...
<p>response 对象有个 <a href="/en-US/docs/Web/API/Body/text"><code>text()</code></a>方法，获取响应主体中的原始数据 a 并把它转换成纯文本，那是我们想要的格式。它也返回一个 promise (解析结果文本字符串), 所以这里我们再使用 <a href="/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/then"><code>.then()</code></a>, 在里面我们再定义一个操作文本字符串的函数。我们设置诗歌的 <a href="/en-US/docs/Web/HTML/Element/pre"> element represents preformatted text which is to be presented exactly as written in the HTML file."><code>&lt;pre&gt;</code></a> 元素的 <code><a href="/en-US/docs/Web/API/Node/textContent">textContent</a></code> 属性和这个文本字符串相同，这样就非常简单地解决了。</p>
...
<div class="note">
<p><strong>备注：</strong> 如果您在这方面有问题，请到 Github 将您的代码和完成的版本进行对比 ( <a href="https://github.com/mdn/learning-area/blob/master/javascript/apis/fetching-data/can-store-xhr/can-script.js">see the source here</a>, see also <a href="https://mdn.github.io/learning-area/javascript/apis/fetching-data/can-store-xhr/">see it running live</a> )。</p>
</div>
...
<ul>
 <li><a href="/en-US/docs/Learn/JavaScript/Client-side_web_APIs/Introduction">介绍 web APIs</a></li>
 <li><a href="/en-US/docs/Learn/JavaScript/Client-side_web_APIs/Manipulating_documents">操作 DOM documents</a></li>
 <li><a href="/en-US/docs/Learn/JavaScript/Client-side_web_APIs/Fetching_data">从服务器获取数据</a></li>
 <li><a href="/en-US/docs/Learn/JavaScript/Client-side_web_APIs/Third_party_APIs">第三方 APIs</a></li>
 <li><a href="/en-US/docs/Learn/JavaScript/Client-side_web_APIs/Drawing_graphics">绘图</a></li>
 <li><a href="/en-US/docs/Learn/JavaScript/Client-side_web_APIs/Video_and_audio_APIs">视频音频 APIs</a></li>
