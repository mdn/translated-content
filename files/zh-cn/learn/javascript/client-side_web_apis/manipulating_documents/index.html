...
<p>在编写 web 页面或应用时，你最想做的事情之一就是以某种方式操作文档结构。这通常使用一套大量使用{{domxref("Document")}}对象来控制 HTML 和样式信息的文档对象模型（DOM）来实现，在本文中，我们可以更详细的看到怎样使用 DOM，连同一些其他有趣的 API 以有趣的方式改变你的环境</p>

<table class="learn-box standard-table">
 <tbody>
  <tr>
   <th scope="row">前提：</th>
   <td>基础的计算机常识，基本了解 HTML、CSS 和 JavaScript - 包括 JavaScript 对象。</td>
  </tr>
  <tr>
   <th scope="row">目标：</th>
...
<h2 id="web浏览器的重要部分">web 浏览器的重要部分</h2>

<p>web 浏览器的软件中有很多活动的程序片段，而许多片段 web 开发人员无法使用 JavaScript 来控制或操作，因此 Web 浏览器是一个很复杂的软件组合。你可能认为这样的限制是不是好事，但是浏览器被锁定是有充分理由的，主要集中在安全方面。如果一个网站可以访问您存储的密码或其他敏感信息，犹如你一样登录到网站，试想会发生什么？</p>
...
<ul>
 <li>window 是载入浏览器的标签，在 JavaScript 中用{{domxref("Window")}}对象来表示，使用这个对象的可用方法，你可以返回窗口的大小（参见{{domxref("Window.innerWidth")}}和{{domxref("Window.innerHeight")}}），操作载入窗口的文档，存储客户端上文档的特殊数据（例如使用本地数据库或其他存储设备），为当前窗口绑定<a href="/en-US/docs/Learn/JavaScript/Building_blocks/Events#A_series_of_fortunate_events">event handler</a>，等等。</li>
 <li>navigator 表示浏览器存在于 web 上的状态和标识（即用户代理）。在 JavaScript 中，用{{domxref("Navigator")}}来表示。你可以用这个对象获取一些信息，比如来自用户摄像头的地理信息、用户偏爱的语言、多媒体流等等。</li>
 <li>document（在浏览器中用 DOM 表示）是载入窗口的实际页面，在 JavaScript 中用{{domxref("Document")}} 对象表示，你可以用这个对象来返回和操作文档中 HTML 和 CSS 上的信息。例如获取 DOM 中一个元素的引用，修改其文本内容，并应用新的样式，创建新的元素并添加为当前元素的子元素，甚至把他们一起删除。</li>
</ul>
...
<div class="note">
<p><strong>备注：</strong> 这个 DOM 树状图是用 Ian Hickson 的<a href="https://software.hixie.ch/utilities/js/live-dom-viewer/">Live DOM viewer</a>生成的</p>
</div>
...
<p>在用 DOM 工作之前，熟悉这些术语是很有用的，因为你将会遇到大量代码术语的使用。你在研究 CSS 时也会遇到这些术语（例如后代选择器、子选择器）</p>

<h2 id="主动学习_基本的DOM_操作">主动学习：基本的 DOM 操作</h2>

<p>要开始学习 DOM 操作，我们先做一个实际的例子。</p>
...
<ol>
 ...
 <li>现在你有了一个存储在变量中的元素引用，你可以使用它的可用属性和方法来操作它（在{{htmlelement("a")}}元素的情况下定义为接口{{domxref("HTMLAnchorElement")}}，它更常常用的父接口是{{domxref("HTMLElement")}}和表示 DOM 中所有节点的{{domxref("Node")}}）。首先，更新 {{domxref("Node.textContent")}}属性的值来修改链接中的文字。在上面的代码后面加入一行代码：
...
<div>
<p>注意，和 JavaScript 中的许多事情一样，有很多方法可以选择一个元素，并在一个变量中存储一个引用。{{domxref("Document.querySelector()")}}是推荐的主流方法，它允许你使用 CSS 选择器选择元素，使用很方便。上面的<code>querySelector()</code>调用会匹配它在文档中遇到的第一个{{htmlelement("a")}}元素。如果想对多个元素进行匹配和操作，你可以使用{{domxref("Document.querySelectorAll()")}}，这个方法匹配文档中每个匹配选择器的元素，并把它们的引用存储在一个<a href="/en-US/docs/Learn/JavaScript/First_steps/Arrays">array</a>中。</p>

<p>对于获取元素引用，还有一些更旧的方法，如：</p>

<ul>
 <li>{{domxref("Document.getElementById()")}}，选择一个<code>id</code>属性值已知的元素，例如<code>&lt;p id="myId"&gt;My paragraph&lt;/p&gt;</code>。ID 作为参数传递给函数，即 <code>var elementRef = document.getElementById('myId')</code>。</li>
...
<ol>
 <li>回到当前的例子，我们先获取到{{htmlelement("section")}}元素的引用 — 在已有 script 中添加下列代码（其他代码也同样处理）：
...
<p>这是给 DOM 添加节点要做的大部分工作 — 在构建动态接口时，你将做大量使用这些方法（我们在后面可以看到一些例子）。</p>

<h3 id="移动和删除元素">移动和删除元素</h3>

<p>也许有时候你想移动或从 DOM 中删除节点，这是完全可能的。</p>

<p>如果你想把具有内部链接的段落移到 sectioin 的底部，简单的做法是：</p>

<pre class="brush: js">sect.appendChild(linkPara);</pre>

<p>这样可以把段落下移到 section 的底部。你可能想过要做第二个副本，但是情况并非如此 — <code>linkPara</code>是指向该段落唯一副本的引用。如果你想做一个副本并也把它添加进去，只能用{{domxref("Node.cloneNode()")}} 方法来替代。</p>
...
<p>通过 JavaScript 以不同的方式来操作 CSS 样式是可能的。</p>
...
<ol>
 ...
 <li>在 HTML 的{{htmlelement("head")}}中添加下列代码 :
  <pre>&lt;style&gt;
.highlight {
  color: white;
...
&lt;/style&gt;</pre>
 ...
 <li>现在我们改为使用 HTML 操作的常用方法 — {{domxref("Element.setAttribute()")}} — 这里有两个参数，你想在元素上设置的属性，你要为它设置的值。在这种情况下，我们在段落中设置类名为 highlight：
...
<div class="note">
<p><strong>备注：</strong> 你可以在 GitHub 上找到演示程序<a href="https://github.com/mdn/learning-area/blob/master/javascript/apis/document-manipulation/dom-example-manipulated.html">finished version of the dom-example.html</a> (<a href="http://mdn.github.io/learning-area/javascript/apis/document-manipulation/dom-example-manipulated.html">see it live also</a>).</p>
</div>

<h2 id="主动学习：从Window对象中获取有用的信息">主动学习：从 Window 对象中获取有用的信息</h2>

<p>到目前为止，我们只真正看到使用{{domxref("Node")}}和{{domxref("Document")}} 特性来操纵文档，但是没有理由不能从其他来源获取数据并在 UI 中使用它。想想我们最新文件中的演示例子 <a href="http://mdn.github.io/learning-area/javascript/apis/introduction/maps-example.html">maps-example.html</a> — 那里我们获取一些位置数据并用来显示你所在区域的地图。你只要确保你的数据格式正确；使用 JavaScript 比其他许多语言更容易，因为它是弱类型的——例如，当你想把它们打印到屏幕上时，数字会自动转换成字符串。</p>

<p>在这个例子中，我们解决了一个常见的问题 — 不管窗口的大小是多少，确保应用程序和它所在的窗口视图一样大。在玩游戏的情况下，想在游戏中尽可能多地使用屏幕区域，这种方法是很有用的。</p>

<p>一开始，要做一个<a href="https://github.com/mdn/learning-area/blob/master/javascript/apis/document-manipulation/window-resize-example.html">window-resize-example.html</a>和<a href="https://github.com/mdn/learning-area/blob/master/javascript/apis/document-manipulation/bgtile.png">bgtile.png</a>文件的本地拷贝。打开文件看一看 — 你可以看到我们用一个{{htmlelement("div")}}元素包裹屏幕的小部分，用来获得应用的 background tile。我们也用它来表示应用的 UI 区域。</p>

<ol>
 <li>首先，获取这个 div 的引用，然后获取视窗（显示文档的内部窗口）的宽度和高度，并存入变量中 — 这两个值包含在{{domxref("Window.innerWidth")}} 和 {{domxref("Window.innerHeight")}}属性中。在已存在的{{htmlelement("script")}}元素中加入下列代码：
...
div.style.height = HEIGHT + 'px';</pre>
 ...
 <li>在我们调整窗口时，我们怎样用事件来调整 div 的大小？ {{domxref("Window")}}对象有一个称为 resize 的可用事件。每次窗口调整大小时都会触发该事件 — 我们可以通过{{domxref("Window.onresize")}} 事件处理程序来访问它，并返回每次改变大小的代码。在代码底部添加下列程序：
...
<ol>
 <li>首先，下载<a href="https://github.com/mdn/learning-area/blob/master/javascript/apis/document-manipulation/shopping-list.html">shopping-list.html</a>文件，并存入本地。你会看到它有一些极小的 CSS，一个带有 label、input 和 button 的 list 和一个空的 list 以及{{htmlelement("script")}} 元素。要添加的所有程序都在 script 里面。</li>
 <li>创建三个变量来保存 list({{htmlelement("ul")}})、{{htmlelement("input")}}和{{htmlelement("button")}}元素的引用。</li>
 ...
 <li>创建三个新元素 — 一个 list 项（{{htmlelement('li')}}），{{htmlelement('span')}}和 {{htmlelement('button')}}，并把它们存入变量之中。</li>
...
<div class="note">
<p><strong>备注：</strong> 如果你卡住了，请查看<a href="https://github.com/mdn/learning-area/blob/master/javascript/apis/document-manipulation/shopping-list-finished.html">finished shopping list</a> (<a href="http://mdn.github.io/learning-area/javascript/apis/document-manipulation/shopping-list-finished.html">see it running live also</a>.)</p>
</div>

<h2 id="总结">总结</h2>

<p>我们已经结束了对文档和 DOM 操作的研究。在这一点上，你应该了解 Web 浏览器在控制文档和用户 Web 体验的其他方面方面的重要部分。更重要的是，你应该了解什么是文档对象模型，怎样操作它来创建有用的功能。</p>
...
<p>(MDN 上有完整的 Web API 列表，参见<a href="/zh-CN/docs/Web/API">Web API 接口参考</a> !)</p>
