...
<p>我们在之前的文章里大多学的是理论，这篇文章将提供一个练习的机会——您将练习构建一些您自己风格的函数。在练习过程中，我们也会解释一些针对函数的更深层的实用细节。</p>

<table class="learn-box standard-table">
 <tbody>
  <tr>
   <th scope="row">先修知识：</th>
   <td>基本的电脑常识，对于 HTML 和 CSS 的基本了解， <a href="zh-CN/docs/Learn/JavaScript/First_steps">JavaScript 第一步</a>， <a href="zh-CN/docs/Learn/JavaScript/Building_blocks/Functions">函数 - 可复用代码块</a>。</td>
  </tr>
  <tr>
   <th scope="row">目标：</th>
...
<p>我们将构建的传统函数将被命名为 <code>displayMessage()</code>，它向用户展示一个传统的消息盒子于 web 页面的顶部。它充当浏览器内建的 <a href="/zh-CN/docs/Web/API/Window/alert">alert()</a> 函数更有用的替代品。你已经看过了这个，但是我们回复一下我们的记忆——在你的浏览器的 JavaScript 控制台中，在任意一个页面里尝试以下代码</p>
...
<div class="note">
<p><strong>备注：</strong> 这个例子能够在现代浏览器上很好的工作，但是这个风格在老的浏览器上并没那么有趣。我们建议你实现这个例子时在现代浏览器上，例如 Firefox，Opera 或者 Chrome 浏览器。</p>
</div>
...
<div class="note">
<p><strong>备注：</strong> 对于函数命名约定，应遵循与<a href="/en-US/Learn/JavaScript/First_steps/Variables#An_aside_on_variable_naming_rules">变量命名约定</a>相同的规则。这很好，尽你所能理解它们之间的区别 - 函数名称后带有括号，而变量则没有。</p>
</div>

<ol>
 <li>我们希望您首先访问<a href="https://github.com/mdn/learning-area/blob/master/javascript/building-blocks/functions/function-start.html">function-start.html</a>文件并创建一个本地拷贝。您将会看到这个 HTML 很简单 — 我们的 body 块仅包含一个按钮。我们还提供了一些基本的 CSS 来装饰自定义消息框，以及一个用于放置 JavaScript 代码的{{htmlelement("script")}}元素。</li>
...
<p>第一行代码使用了一个 DOM（文档对象模型）的内置方法 {{domxref("document.querySelector()")}} 来选择{{htmlelement("html")}} 元素并且把它存放在一个叫 <code>html</code>的常量中，这样方便我们接下来使用这个元素：</p>

<pre class="brush: js notranslate">const html = document.querySelector('html');</pre>

<p>下段代码使用了另一个名字叫做 {{domxref("Document.createElement()")}} 的 DOM 方法，用来创建 {{htmlelement("div")}} 元素并且把该新建元素的引用（实际上是新建对象的地址）放在一个叫做 <code>panel</code>的常量中。 这个元素将成为我们的消息框的外部容器。</p>

<p>然后我们又使用了一个叫做 {{domxref("Element.setAttribute()")}} 的 DOM 方法给 panel 元素添加了一个值为<code>msgBox</code> 的<code>class</code> 类属性。 这样做方便我们来给这个元素添加样式 — 查看 CSS 代码你就知道我们使用<code>.msgBox</code> 类选择器来给消息框和消息内容设置样式。</p>

<p>最后，我们还使用了一个叫做 {{domxref("Node.appendChild()")}} 的 DOM 方法，给 <code>html</code> 常量（我们之前定义好的）追加了我们设置好样式的 panel 元素 。该方法追加了元素的同时也把 panel<code>&lt;div&gt;</code>元素指定为<code>&lt;html&gt;</code>的子元素 。这样做是因为我们创建了一个元素之后这个元素并不会莫名其妙的出现在我们的页面上（浏览器只知道我们创建了一个元素，但是不知道把这个元素怎么呈现出来） — 因此，我们给这个元素了一个定位，就是显示在 html 里面！</p>

<pre class="brush: js notranslate">const panel = document.createElement('div');
panel.setAttribute('class', 'msgBox');
html.appendChild(panel);</pre>

<p>下面这两段使用了我们之前使用过的方法<code>createElement()</code>和<code>appendChild()</code>  — 创建了一个 {{htmlelement("p")}}  元素和一个{{htmlelement("button")}}元素 —  并且把他们追加到了 panel<code>&lt;div&gt;</code>之下。我们使用元素的 {{domxref("Node.textContent")}}（Node 泛指一个元素并不是说是某个元素是叫 Node）属性— 表示一个元素的文本属性 — 给一个 p 元素赋值，同样按钮也有这个属性，该属性就是按钮显示的‘X’。这个按钮的功能就是关闭消息提示框。</p>
...
<p>简单来说，这个 <code>onclick</code> 句柄是一个按钮的属性 (事实上，页面上的任何元素) 当按钮被点击的时候能够执行一些代码。 你可以在之后的介绍事件的章节了解详情。我们给 <code>onclick</code> 句柄绑定了一个匿名函数，函数中代码在元素被点击的时候运行。函数里面的这行代码使用了 {{domxref("Node.removeChild()")}} DOM 方法指定了我们想要移除的 HTML 的子元素 — 在这里指 panel<code>&lt;div&gt;</code>.</p>

<p>PS：我来解释下是什么意思，panel 是消息框，panel.parentNode 就是指 panel 的上一级，就是整个 DOM，然后再来用这个父亲来干掉这个儿子，儿子不能自己干掉自己，所以要这么做。</p>

<pre class="brush: js notranslate">closeBtn.onclick = function() {
  panel.parentNode.removeChild(panel);
}</pre>

<p>大体上，这一整块的代码我就不解释了就是一个 div，一个段落，一个按钮，把这个加在页面上：</p>
...
<ol>
 <li>把下面这行代码加在写好的函数下面来调用函数（当然，不一定要放在函数下面来调用，在 C 语言中确实是还要先定义后使用，但是我们现在用的是 JavaScript，这玩意很强大，不管你是先定义后调用还是先调用后定义都行，但是别忘了定义）:
 ...
 <li>
  <p>现在打开浏览器开发工具，找到 JavaScript 控制台把上面这一句再输入一遍然后回车，你会看到又弹出了一次！有点意思... — 现在我们有了一个能够重复调用的函数，只要你高兴可以随时调用它。</p>
...
<ol>
 <li>第一步，从 Github 上下载我们需要的图标 (<a href="https://raw.githubusercontent.com/mdn/learning-area/master/javascript/building-blocks/functions/icons/warning.png">警告图标</a> 和 <a href="https://raw.githubusercontent.com/mdn/learning-area/master/javascript/building-blocks/functions/icons/chat.png">聊天图标</a>) 。把图标保存在一个叫做<code>icons</code> 的文件夹下，和你的 HTML 文件在同一个目录下。

  <div class="note"><p><strong>备注：</strong> 警告和聊天图标是在这个网站 iconfinder.com 上找到的，设计者是 <a href="https://www.iconfinder.com/nazarr">Nazarrudin Ansyari</a>。感谢他！</p></div>
 </li>
 <li>下一步，找到页面的 CSS 文件。我们要修改下以便我们使用图标。首先，修改 <code>.msgBox</code> 的宽度：
...
background-repeat: no-repeat;</pre>
 ...
 <li>CSS 改完了以后我们就要来修改函数 <code>displayMessage()</code> 让它能够显示图标。在你的函数结束符之前<code>}</code>添加下面这几行代码：
...
<div class="note">
<p><strong>备注：</strong> 如果你写这个例子遇到了困难，在这里查看免费的代码 <a href="https://github.com/mdn/learning-area/blob/master/javascript/building-blocks/functions/function-stage-4.html">完整版本的代码</a> (或者<a href="http://mdn.github.io/learning-area/javascript/building-blocks/functions/function-stage-4.html">在线运行的完整代码</a>), 也可以向我们寻求帮助。</p>
</div>
