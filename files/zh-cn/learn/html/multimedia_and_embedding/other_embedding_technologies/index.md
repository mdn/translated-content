...
在网络的早期，非常流行使用**框架（frame）**来创建网站——网站的各个小部分被存储在各自的 HTML 页面中。这些页面被嵌入到一个名为**框架集**的主文档中，该文档允许你指定每个框架在屏幕上所占的区域，这有点像是设定表格的列和行的大小。在 90 年代中到后期，这种做法被认为非常先进，并且有证据显示，这样将网页分割成更小的部分确实可以提高下载速度——这在当时缓慢的网络连接下尤其明显。然而，随着网络速度的提升，框架的许多问题开始凸显，这些问题远远超过了它的优点，因此现在已经很少看到有人使用框架了。
