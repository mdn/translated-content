...
<div class="note">
<p><strong>备注：</strong> HTML 标签不区分大小写。也就是说，输入标签时既可以使用大写字母也可以使用小写字母。例如，标签 {{htmlelement("title")}} 写作<code>&lt;title&gt;、</code><code>&lt;TITLE&gt;</code>、<code>&lt;Title&gt;</code>、<code>&lt;TiTlE&gt;</code>，等等都可以正常工作。不过，从一致性、可读性来说，最好仅使用小写字母。</p>
</div>
...
<p><img src="grumpy-cat-small.png"></p>
...
<p>{{ EmbedLiveSample('主动学习：创建第一个 HTML 元素', 700, 400) }}</p>

<h3 id="嵌套元素">嵌套元素</h3>

<p>你也可以把元素放到其它元素之中——这被称作嵌套。如果我们想要表明我们的小猫脾气很暴躁，可以将<strong>“very”</strong>嵌套在{{htmlelement("strong")}} 中，意味着这个单词被着重强调：</p>
...
<p>在 HTML 中有两种你需要知道的重要元素类别，块级元素和内联元素。</p>
...
<div class="note">
<p><strong>备注：</strong> HTML5 重新定义了元素的类别：见 <a href="https://html.spec.whatwg.org/multipage/indices.html#element-content-categories">元素内容分类</a>(<a href="/zh-CN/docs/Web/Guide/HTML/Content_categories">译文</a>)。尽管这些新的定义更精确，但却比上述的“块级元素”和“内联元素”更难理解，因此在之后的讨论中仍使用旧的定义。</p>
</div>

<div class="note">
<p><strong>备注：</strong> 在这篇文章中提到的“块”和“内联”，不应该与 <a href="/en-US/docs/Learn/CSS/Introduction_to_CSS/Box_model#Types_of_CSS_boxes">the types of CSS boxes</a> 中的同名术语相混淆。尽管他们默认是相关的，但改变 CSS 显示类型并不会改变元素的分类，也不会影响它可以包含和被包含于哪些元素。防止这种混淆也是 HTML5 摒弃这些术语的原因之一。</p>
</div>

<div class="note">
<p><strong>备注：</strong> 你可以查阅包含了块级元素和内联元素列表的参考页面—see <a href="/en-US/docs/Web/HTML/Block-level_elements">Block-level elements</a> and <a href="/en-US/docs/Web/HTML/Inline_elements">Inline elements</a>.</p>
</div>
...
<div class="note">
<p><strong>备注：</strong> 空元素（Empty elements）有时也被叫作 <em>void elements</em>.</p>
</div>
...
<p><img alt='&amp;amp;amp;lt;p class="editor-note">我的猫咪脾气爆&amp;amp;amp;lt;/p>' src="grumpy-cat-attribute-small.png"></p>

<p>属性包含元素的额外信息，这些信息不会出现在实际的内容中。在上述例子中，这个 class 属性给元素赋了一个识别的名字（id），这个名字此后可以被用来识别此元素的样式信息和其他信息。</p>
...
<ul>
 <li><code>href</code>: 这个属性声明超链接的 web 地址，当这个链接被点击浏览器会跳转至 href 声明的 web 地址。例如：<code>href="https://www.mozilla.org/"</code>。</li>
...
<p>编辑下面的文本框中的内容，使之变成指向任一个你喜欢的 web 地址的链接。首先，添加&lt;a&gt;元素，然后为它添加 href 属性和 title 属性。你可以即时的在输出区域看到你修改的内容。你应该可以看到一个连接，当鼠标移上此链接时会显示 title 属性值，当点击此链接时会跳转到 href 指定的 web 地址。记住：在元素名和属性名之间以及两个属性之间要有一个空格。</p>
...
<p>{{ EmbedLiveSample('学习实践：为一个元素添加属性', 700, 350) }}</p>

<div class="blockIndicator note">
<p><strong>备注：</strong> 可到 Github 在线使用这个“<a href="https://roy-tian.github.io/learning-area/extras/tools/playable-code">代码操场</a>”。</p>
</div>
...
<pre class="brush: html">&lt;!-- 使用 disabled 属性来防止终端用户输入文本到输入框中 --&gt;
...
<p>上面两段 HTML 代码产生的效果如下：</p>

<p>{{ EmbedLiveSample('布尔属性', 700, 100, "", "", "hide-codepen-jsfiddle") }}</p>

<h3 id="省略包围属性值的引号">省略包围属性值的引号</h3>

<p>当你浏览那些粗糙的 web 网站，你将会看见各种各样奇怪的标记风格，其中就有不给属性值添加引号。在某些情况下它是被允许的，但是其他情况下会破坏你的标记。例如，我们可以写一个只拥有一个 href 属性的链接，如下：</p>

<pre class="example-bad brush: html">&lt;a href=<code>https://www.mozilla.org/</code>&gt;收藏页面&lt;/a&gt;</pre>

<p>然而，当我们再添加一个 title 属性时就会出错，如下：</p>

<pre class="example-bad brush: html">&lt;a href=<code>https://www.mozilla.org/</code> title=The Mozilla homepage&gt;收藏页面&lt;/a&gt;</pre>

<p>此时浏览器会误解你的标记，它会把 title 属性理解为三个属性——title 的属性值为"The“，另外还有两个布尔属性“<code>Mozilla</code>”和“<code>homepage</code>”。看下面的例子，它明显不是我们所期望的，并且在这个编码里面它会报错或者出现异常行为。试一试把鼠标移动到链接上，看会显示什么 title 属性值！</p>
...
<p>在目前为止，本章内容所有的属性都是由双引号来包裹。也许在一些 HTML 中，你以前也见过单引号。这只是风格的问题，你可以从中选择一个你喜欢的。以下两种情况都可以：</p>
...
<p>在一个 HTML 中已使用一种引号，你可以在此引号中嵌套另外一种引号：</p>

<pre class="brush: html">&lt;a href="http://www.example.com" title="你觉得'好玩吗'？"&gt;示例站点链接&lt;/a&gt;</pre>

<p>如果你想将引号当作文本显示在 html 中，你就必须使用<a href="#实体引用：_在HTML中包含特殊字符">实体引用</a>。</p>

<h2 id="剖析HTML文档">剖析 HTML 文档</h2>

<p>学习了一些 HTML 元素的基础知识，这些元素单独一个是没有意义的。现在我们来学习这些特定元素是怎么被结合起来，从而形成一个完整的 HTML 页面的：</p>
...
<ol>
 <li><code>&lt;!DOCTYPE html&gt;</code>: 声明文档类型。很久以前，早期的 HTML(大约 1991 年 2 月)，文档类型声明类似于链接，规定了 HTML 页面必须遵从的良好规则，能自动检测错误和其他有用的东西。使用如下：
...
"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"&gt;</pre>
 ...
 <li><code>&lt;head&gt;&lt;/head&gt;</code>: <code>&lt;head&gt;</code>元素。这个元素是一个容器，它包含了所有你想包含在 HTML 页面中但不想在 HTML 页面中显示的内容。这些内容包括你想在搜索结果中出现的关键字和页面描述，CSS 样式，字符集声明等等。以后的章节能学到更多关于&lt;head&gt;元素的内容。</li>
 <li><code>&lt;meta charset="utf-8"&gt;</code>: 这个元素设置文档使用 utf-8 字符集编码，utf-8 字符集包含了人类大部分的文字。基本上他能识别你放上去的所有文本内容。毫无疑问要使用它，并且它能在以后避免很多其他问题。</li>
...
<h3 id="学习实践：为HTML文档添加一些特征">学习实践：为 HTML 文档添加一些特征</h3>

<p>如果你想在你的本地练习写一些 HTML 页面，你可以这样做：</p>
...
<div class="note">
<p><strong>备注：</strong> 可在 <a href="https://github.com/roy-tian/learning-area/blob/master/html/introduction-to-html/getting-started/index.html">学习区代码仓库</a> 上查看该示例。</p>
</div>

<p>你可以打开浏览器看看这段代码的效果是什么样的，然后改变代码刷新浏览器看看新的结果。最开始的代码是这样的效果：</p>

<p><img alt="A simple HTML page that says This is my page" src="template-screenshot.png">所以在这段练习中，你可以用你的电脑在本地编写运行代码，如上所述，你也可以在下面的简单可编辑窗口编辑它 (此时这个简单的可编辑窗口仅显示&lt;body&gt;标签内的内容.) 我们希望你能够实践以下步骤：</p>
...
<ul>
 ...
 <li>在你的文档中添加一个超文本链接，<a href="/zh-CN/docs/Learn/HTML/Introduction_to_HTML/Getting_started#%E5%AD%A6%E4%B9%A0%E5%AE%9E%E8%B7%B5%EF%BC%9A%E4%B8%BA%E4%B8%80%E4%B8%AA%E5%85%83%E7%B4%A0%E6%B7%BB%E5%8A%A0%E5%B1%9E%E6%80%A7">如前所述</a>。</li>
 <li>在段落下方向你的文档添加一张图片，<a href="/zh-CN/docs/learn/HTML/Introduction_to_HTML/Getting_started#%E7%A9%BA%E5%85%83%E7%B4%A0">如前所述</a>。如果你尝试对不同的图片 (在你的本地电脑或是在 Web 的其他位置上) 添加链接，那你就更棒了。</li>
</ul>
...
<p>{{ EmbedLiveSample('学习实践：为 HTML 文档添加一些特征', 700, 600)}}</p>

<h3 id="HTML中的空白">HTML 中的空白</h3>
...
<p>无论你在 HTML 元素的内容中使用多少空格 (包括空白字符，包括换行)，当渲染这些代码的时候，HTML 解释器会将连续出现的空白字符减少为一个单独的空格符。</p>

<p>那么为什么我们会在 HTML 元素的嵌套中使用那么多的空白呢？答案就是为了可读性 —— 如果你的代码被很好地进行格式化，那么就很容易理解你的代码是怎么回事，反之就只有聚做一团的混乱.。在我们的 HTML 代码中，我们让每一个嵌套的元素以两个空格缩进。你使用什么风格来格式化你的代码取决于你 (比如所对于每层缩进使用多少个空格)，但是你应该坚持使用某种风格。</p>

<h2 id="实体引用：_在HTML中包含特殊字符">实体引用：在 HTML 中包含特殊字符</h2>

<p>在 HTML 中，字符 <code>&lt;</code>, <code>&gt;</code>,<code>"</code>,<code>'</code> 和 <code>&amp;</code> 是特殊字符。它们是 HTML 语法自身的一部分，那么你如何将这些字符包含进你的文本中呢，比如说如果你真的想要在文本中使用符号&amp;或者小于号，而不想让它们被浏览器视为代码并被解释？</p>
...
<p>在下面的例子中你可以看到两个段落，它们在谈论 web 技术：</p>
...
<p>{{ EmbedLiveSample('实体引用：在 HTML 中包含特殊字符', 700, 200) }}</p>

<div class="note">
<p><strong>备注：</strong> 维基百科上有一个包含所有可用 HTML 字符实体引用的列表：<a href="http://en.wikipedia.org/wiki/List_of_XML_and_HTML_character_entity_references">XML 和 HTML 字符实体引用列表</a>。</p>
</div>

<h2 id="HTML注释">HTML 注释</h2>

<p>如同大部分的编程语言一样，在 HTML 中有一种可用的机制来在代码中书写注释 —— 注释是被浏览器忽略的，而且是对用户不可见的，它们的目的是允许你描述你的代码是如何工作的和不同部分的代码做了什么等等。如果你在半年后重新返回你的代码库，而且不能记起你所做的事情 —— 或者当你处理别人的代码的时候，那么注释是很有用的。</p>

<p>为了将一段 HTML 中的内容置为注释，你需要将其用特殊的记号&lt;!--和--&gt;包括起来，比如：</p>
...
<p>{{ EmbedLiveSample('HTML 注释', 700, 100) }}</p>

<h2 id="总结">总结</h2>

<p>你已经来到了这篇文章的结尾 —— 希望你享受你的基础的 HTML 学习的旅程。在这里你应该可以理解 HTML 语言的全貌，它在基础的级别是如何工作，而且可以使用一些元素和属性。在这个模块的后续文章中，我们会深入一些你已经见过的东西的细节，并且介绍一些新的 HTML 的特性。未完待续！</p>

<div class="note">
<p><strong>备注：</strong> 现在，你将开始学习更多关于 HTML 的知识，你可能也想了解一些层叠样式列表（<a href="/zh-CN/docs/Learn/CSS">CSS</a>）的基础知识。CSS 是一种用来设计网页样式的语言（比如，用它改变字体、颜色或页面布局等）。你很快就会发现，HTML 和 CSS 能很好地协调配合。</p>
</div>
