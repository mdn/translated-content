...
尤其要注意 `Last-Modified` 标头。它可以实现高效的页面缓存；通过这个标头，可以向用户代理传达它想要加载的文件的相关信息，例如文件的最后修改时间。大多数 web 服务器会根据存储在文件系统中的最后修改日期，自动将 `Last-Modified` 标头附加到静态页面（如“.html”、“.css”）上。对于动态页面（如 `.php`、`.aspx`），当然无法做到这一点，因此不会发送头信息。
