...
<p><a href="http://www.seleniumhq.org/">Selenium</a>是最流行的浏览器自动化测试工具。最易使用的方法是使用基于 Selenium 的 WebDriver API，它通过调用浏览器接口实现自动化，执行诸如“打开网页”、“移动网页上的元素”、“点击链接”、“查看链接是否打开 URL”等。对于运行自动化测试来说是一个十分理想的方法。</p>

<p>安装和使用 WebDriver 的方式取决于你的编程环境。常见的环境都提供有安装 WebDriver 的包或框架，并且支持与 WebDriver 通信的多语言绑定，如 Java, C#, Ruby, Python, JavaScript (Node) 等。查看<a href="http://www.seleniumhq.org/docs/03_webdriver.jsp#setting-up-a-selenium-webdriver-project">建立一个 Selenium-WebDriver 的工程</a>了解 Selenium 在不同语言下建立的更多细节。</p>

<p>不同的浏览器需要使用不同的驱动，来使 WebDriver 能够与浏览器交互并控制浏览器。查看<a href="http://www.seleniumhq.org/about/platforms.jsp">支持 Selenium 的平台</a>来了解获取浏览器驱动的信息。</p>

<p>我们将使用 Node.js 来编写和运行 Selenium 测试用例。Node.js 是一个前端开发者都很熟悉，并且容易上手的开发环境。</p>

<div class="note">
<p><strong>备注：</strong> 如果你需要了解在其他服务器端环境下使用 WebDriver 的方式，也可以点击<a href="http://www.seleniumhq.org/about/platforms.jsp">支持 Selenium 的平台</a>来获取更多有用的链接。</p>
</div>

<h3 id="在Node下建立Selenium">在 Node 下建立 Selenium</h3>

<ol>
 <li>参考上一个章节<a href="/en-US/docs/Learn/Tools_and_testing/Cross_browser_testing/Automated_testing#Setting_up_Node_and_npm">创建 Node 和 npm</a>，创建一个新的 npm 工程，取一个不同的名字，如<code>selenium-test</code>.</li>
 <li>接下来，我们需要安装一个框架来允许我们从 Node 中运行 Selenium。我们选择 selenium 官方提供的 <a href="https://www.npmjs.com/package/selenium-webdriver">selenium-webdriver</a>, 它的文档更新得很好，维护得也很好。如果你想要其它的选择，<a href="http://webdriver.io/">webdriver.io</a> 和 <a href="http://nightwatchjs.org/">nightwatch.js</a>也都不错。要安装 selenium-webdriver，在你的工程目录下运行如下命令：
...
<p>接下来，你需要下载相应的驱动，使 WebDriver 能控制你需要测试的浏览器。在<a href="https://www.npmjs.com/package/selenium-webdriver">selenium-webdriver</a> (第一部分的表格) 页面查看如何下载。 显然，有的浏览器是 OS 定制的，我们将使用 Firefox 和 Chrome, 它们在主流的 OS 下都支持。</p>
...
<p>在 Windows 下设置 <code>PATH</code> 变量，参考 <a href="http://windowsitpro.com/systems-management/how-can-i-add-new-folder-my-system-path">如何添加一个新的文件目录到系统路径？</a></p>
...
<p>你会看到 Firefox 自动打开了一个窗口！Google 自动加载到 tab 中，"webdriver" 被输入到搜索框，然后搜索按钮被点击。WebDriver 等待 2 秒; 然后获取文本标题，如果标题是"webdriver - Google Search", 将返回测试成功的消息。然后 WebDriver 关闭 Firefox 窗口并结束。</p>
...
<h3 id="获取测试的document">获取测试的 document</h3>
...
<div class="note">
<p><strong>备注：</strong> 可以查看 <a href="http://seleniumhq.github.io/selenium/docs/api/javascript/module/selenium-webdriver/lib/webdriver_exports_WebDriver.html">WebDriver class reference</a> 了解这一部分提到内容的详情。</p>
</div>
...
<h3 id="同document交互">同 document 交互</h3>

<p>得到测试的 document 后，我们就需要同它进行交互操作了，比如获取特定的元素对它进行某种测试。WebDriver 提供了<a href="http://www.seleniumhq.org/docs/03_webdriver.jsp#locating-ui-elements-webelements">通过多种方法选择 UI 元素</a>，比如通过 ID, class, element name 等等。具体的选择通过 <code>findElement()</code> 方法完成，只需要将选择的元素作为参数传给它即可。举例来说，通过 ID 选择一个元素：</p>
...
<h3 id="测试element">测试 element</h3>

<p>同 web documents 和 elements 交互的方式很多。在 WebDriver 的文档中有很多常用的例子，你可以从<a href="http://www.seleniumhq.org/docs/03_webdriver.jsp#getting-text-values">获取文本值</a>开始。</p>
...
<p>有时候，在进行下一步处理之前我们需要 WebDriver 等待一些操作完成。比如装载一个新的页面，在与页面元素交互之前，需要等待 DOM 完成加载，否则测试有可能会失败。</p>
...
<p>除此之外，我们还要提一下测试结果/测试报告 — 在前面的例子中，我们只是简单的把测试结果通过 <code>console.log()</code> 语句打印出来，这个完全只在 JavaScript 中完成，你可以使用任何你想要的测试运行和报告系统，如<a href="https://mochajs.org/">Mocha</a>/<a href="http://chaijs.com/">Chai</a>/或者其它的组合。</p>

<ol>
 <li>举个例子，将 <code><a href="https://github.com/mdn/learning-area/blob/master/tools-testing/cross-browser-testing/selenium/mocha_test.js">mocha_test.js</a></code> 拷贝到你的工程目录下。把它放到一个叫做 <code>test</code>的子文件夹内，这个例子使用了一长串的 promise 来执行测试的所有步骤  — WebDriver 使用这些基于 promise 的方法保证正常工作。</li>
...
<div class="note">
<p><strong>备注：</strong> <a href="https://github.com/saucelabs-sample-test-frameworks">saucelabs-sample-test-frameworks</a> 中包含了一些关于如何设置不同测试/断言工具组合的有用示例。</p>
</div>
...
<p>在 BrowserStack 进行 Selenium 远程测试很简单，参考下面的代码示例：</p>
...
driver.quit();</pre>
 ...
 <li>在 <a href="https://www.browserstack.com/automate">BrowserStack automation dashboard</a>, 获取你的用户名和 Key (查看 <em>Username 和 Access Keys</em>)，替换代码中 <code>YOUR-USER-NAME</code> 和<code>YOUR-ACCESS-KEY</code> 的对应值。(确保它们依然安全).</li>
 <li>执行下面命令：
 ...
 <li>现在回到 <a href="https://www.browserstack.com/automate">BrowserStack automation dashboard</a> 页面，你会看到测试列出来的结果：<br>
  <img src="bstack_automated_results.png"></li>
</ol>

<p>点击测试链接，会打开一个屏幕播放记录了测试的视频，和在测试过程中相关的 log 详情。</p>

<div class="note">
<p><strong>备注：</strong> Browserstack 自动化仪表盘的 Resources 菜单选项上 提供了许多运行自动化测试的有用信息。查看 <a href="https://www.browserstack.com/automate/node">关于 Node JS 编写自动化测试的文档</a> 获取相关信息。研究一下使用 BrowserStack 可以帮你做到哪些事情。</p>
</div>

<div class="note">
<p><strong>备注：</strong> 如果你不想自己写测试用例，可以使用 docs 中嵌入的生成器，查看 <a href="https://www.browserstack.com/automate/node#run-tests-on-mobile">在手机浏览器上运行测试</a>和<a href="https://www.browserstack.com/automate/node#setting-os-and-browser">在桌面浏览器上运行测试</a>.</p>
</div>
...
<p><img src="bstack_custom_results.png"></p>

<h3 id="Sauce_Labs">Sauce Labs</h3>

<p>在 Sauce Labs 远程运行 Selenium 测试与在 BrowserStack 一样简单，尽管它们有一些语法的差异。代码示例如下：</p>
...
driver.quit();</pre>
 ...
 <li>从你的 <a href="https://saucelabs.com/beta/user-settings">Sauce Labs 用户设置</a>, 获取你的用户名和 key. 并替换代码中对于 <code>YOUR-USER-NAME</code> 和<code>YOUR-ACCESS-KEY</code> 的值 (确保它们依然安全).</li>
 <li>执行如下命令运行测试：
 ...
 <li>现在访问你的 <a href="https://saucelabs.com/beta/dashboard/tests">Sauce Labs 自动化测试仪表盘</a> 页面，会看到列出的测试; 同样你也可以看到视频、截屏和其他类似的数据。<br>
  <img src="sauce_labs_automated_test.png"></li>
</ol>

<div class="note">
<p><strong>备注：</strong> Sauce Labs' <a href="https://wiki.saucelabs.com/display/DOCS/Platform+Configurator/#/">Platform Configurator</a> is a useful tool for generating capability objects to feed to your driver instances, based on what browser/OS you want to test on.</p>
</div>

<div class="note">
<p><strong>备注：</strong> 你可以查看 <a href="https://wiki.saucelabs.com/display/DOCS/Getting+Started+with+Selenium+for+Automated+Website+Testing">使用 Selenium 进行 Web 自动化测试</a>, 和 <a href="https://wiki.saucelabs.com/display/DOCS/Instant+Selenium+Node.js+Tests">实时 Selenium Node.js 测试</a>获取更多关于 Sauce Labs 和 Selenium 测试的有用信息。</p>
</div>
...
<ol>
 ...
 <li>Require saucelabs — 在 <code>sauce_google_test.js</code> 文件顶端，就在之前的变量声明下面添加如下代码：
  <pre class="brush: js">var SauceLabs = require('saucelabs');</pre>
 </li>
 <li>接着添加如下代码，创建一个新的 SauceLabs 实例：
  ...
  同样，用真实的用户名和 key 来替换<code>YOUR-USER-NAME</code> 和 <code>YOUR-ACCESS-KEY</code> 的值 (注意  saucelabs 的 npm 包使用的是 <code>password</code>, 而不是<code>accessKey</code>). Since you are using these twice now, you may want to create a couple of helper variables to store them in.</li>
...
<p><img src="sauce_labs_updated_job_info.png"></p>

<h3 id="您自己的远程服务器">您自己的远程服务器</h3>

<p>如果你不想使用 Sauce Labs 或 BrowserStack 之类的服务，你也可以设置自己的远程测试服务器。具体操作如下：</p>

<ol>
 <li>Selenium 的远端服务器要求运行 Java，从<a href="http://www.oracle.com/technetwork/java/javase/downloads/index.html"> Java SE 下载页面</a>下载适合你平台的最新 JDK 并安装。</li>
 <li>接着，下载最新的 <a href="http://selenium-release.storage.googleapis.com/index.html">Selenium 单机服务器</a> — 它作为你的脚本和浏览器驱动之间的一个代理。选择最新的稳定版本 (比如不要选一个 beta 版本), 从列表中选择以 "selenium-server-standalone"开头的文件。下载完成后，放到一个靠谱的地方，比如 home 目录下。如果你还没有将位置添加到 <code>PATH</code>, 现在就需要添加了 (查看 "在 Node 中创建 Selenium"章节).</li>
...
<p>是不是很酷。我们是在本地测试的，但你可以任何一台服务器上使用相关的浏览器驱动进行测试，使用相应的 URL 将你的脚本和服务器连接起来就可以了。</p>

<h2 id="将selenium和CI工具集成">将 selenium 和 CI 工具集成</h2>

<p>另外，将 Selenium 同 Sauce Labs 之类的其它工具集成到持续集成（CI）工具中是很有用的，这意味着你可以通过 CI 工具来运行测试，只有测试通过才允许提交代码的修改。</p>
