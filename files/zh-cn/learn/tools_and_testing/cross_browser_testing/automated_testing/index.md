...
> [!NOTE]
> 以上两类并不相互排斥。可以设置一个任务运行器，通过 API 访问 Sauce Labs 或 LambdaTest 等服务，运行跨浏览器测试，并返回结果。我们在下面也会看一下这个。
...
> [!NOTE]
> 与其他基于云的跨浏览器测试服务提供商不同，LambdaTest 提供了一个免费账户，你可以终身使用这个平台。高级计划和免费计划之间的唯一区别在于消费的数量。对于通过其 Selenium 网格进行的自动化测试，LambdaTest 提供每月 60 分钟的免费测试。
