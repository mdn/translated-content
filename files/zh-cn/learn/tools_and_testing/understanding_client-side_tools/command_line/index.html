...
<p>终端是一个文本界面，用于执行基于文本的程序。如果您正在运行任何用于 web 开发的工具，那么几乎可以保证您必须打开命令行并运行一些命令来使用您所选择的工具 (您经常会看到这样的工具被称为<strong>CLI 工具</strong>命令行接口工具)。</p>
...
<p>终端机起源于 20 世纪五六十年代，它最初的形式与我们今天使用的并不相似 (这是我们应该感谢的)。你可以在维基百科的词条中读到一些历史 <a href="https://en.wikipedia.org/wiki/Computer_terminal">Computer Terminal</a>。</p>
...
<p>现在许多开发人员都在使用基于 unix 的工具 (例如，终端，以及你可以通过它访问的工具)。目前 web 上存在的许多教程和工具都支持 (可悲的是假定) 基于 unix 的系统，但不用担心它们在大多数系统上都可用。在本节中，我们将了解如何访问所选系统上的终端。</p>
...
<p>更好的程序可以在 Windows 上提供终端体验，比如 Powershell (<a href="https://github.com/PowerShell/PowerShell">see here to find installers</a>), 和 Gitbash (作为一部分 <a href="https://gitforwindows.org/">git for Windows</a> 工具箱)</p>

<p>然而，在现代，Windows 的最佳选择是 Windows Linux 子系统 (WSL)，它是一个兼容层，用于从 Windows 10 中直接运行 Linux 操作系统，允许您直接在 Windows 上运行真正的终端，而不需要虚拟机。</p>

<p>这可以直接从 Windows 商店免费安装。在目录中可以找到所需的所有文档 <a href="https://docs.microsoft.com/en-us/windows/wsl">Windows Subsystem for Linux Documentation</a>.</p>
...
<p>通常你会发现这两个术语可以互换使用。从技术上讲，终端是启动并连接到 shell 的软件。shell 是您的会话和会话环境 (提示符和快捷方式等内容可以在其中定制)。命令行是您输入命令并且光标闪烁的文字行。</p>
...
<div class="blockIndicator note">
<p><strong>备注：</strong> 在 windows 中，你使用反斜杠而不是正斜杠。<code>cd Desktop\project\src</code> — 他的可能看起来很奇怪，但是如果你感兴趣的话，<a href="https://www.youtube.com/watch?v=5T3IJfBfBmI">watch this YouTube clip</a> 微软的一位主要工程师对此进行了解释。</p>
</div>

<h3 id="列出目录内容">列出目录内容</h3>

<p>另一个内置的 Unix 命令是 ls (list 的缩写)，它列出当前所在目录的内容。注意，这不会工作，如果你使用默认的 Windows 命令提示符 (cmd)，相当于 dir。</p>
...
<div class="blockIndicator note">
<p><strong>备注：</strong> 要确切地了解每个命令有哪些可用选项，您可以查看它<a href="https://en.wikipedia.org/wiki/Man_page">man page</a>. 通过键入 <code>man</code> 命令，后跟要查找的命令的名称，例如<code>man ls</code>. 这将在终端的默认文本文件查看器中打开手册页 (例如，<code><a href="https://en.wikipedia.org/wiki/Less_(Unix)">less</a></code>(在我的终端中)，然后您应该能够使用箭头键或其他类似的机制在页面中滚动。手册页详细地列出了所有选项，一开始可能有点吓人，但至少在需要时您知道它就在那里。一旦您完成了对手册页的查看，您需要使用文本查看器的 quit 命令退出它 ("q" 在 <code>less</code>;如果不明显，你可能需要在网上搜索才能找到).</p>
</div>
...
<ul>
 <li><code>mkdir</code> —这将在您所在的当前目录中创建一个新目录，名称是您在命令名之后提供的。例如<code>mkdir my-awesome-website将创建一个新目录叫</code><code>my-awesome-website</code>.</li>
 <li><code>rmdir</code> —删除指定目录，但仅当它为空时。例如<code>rmdir my-awesome-website</code>
  <p>将删除我们在上面创建的目录。如果您希望删除一个非空的目录 (并删除其中包含的所有内容)，则可以使用<code>-r</code> 选项（递归），但这很危险。确保以后在目录中不需要任何内容​​，因为它将永远消失。</p>
 </li>
 <li><code>touch</code> —在当前目录中创建一个新的空文件。例如<code>touch mdn-example.md</code> 创建一个新的空文件叫做 <code>mdn-example.md</code>.</li>
 <li><code>mv</code> —
  <p>例如，将文件从第一个指定的文件位置移动到第二个指定的文件位置<code>mdn-example.md mdn-example.txt</code>(这些位置被写成文件路径)。此命令移动一个名为<code>mdn-example.md</code>在当前目录中调用一个文件<code>mdn-example.txt</code> in the current directory.从技术上讲，文件正在被移动，但是从实际的角度来看，这个命令实际上是在重命名文件。</p>
 </li>
...
<p>我们之前提到过这一点，但为了明确起见，您需要小心使用终端。简单的命令不会带来太多危险，但是当您开始将更复杂的命令组合在一起时，您需要仔细考虑该命令将执行什么操作，并在最终在指定的目录中运行它们之前先尝试测试它们。假设您在一个目录中有 1000 个文本文件，您想遍历它们，并且只删除文件名中有特定子字符串的文件。如果不小心，可能会删除一些重要的内容，在此过程中丢失大量工作。</p>
...
<p>快速浏览特定终端命令的一个很好的资源是 <a href="https://tldr.sh/">tldr.sh</a>. 这是一个社区驱动的文档服务，类似于 MDN，但特定于终端命令。</p>

<p>在下一节中，让我们更进一步 (实际上是几个层次)，看看如何在命令行上将工具连接在一起，从而真正了解终端如何优于常规的桌面用户界面。</p>
...
<p><code>因为ls</code> 在自己的行上打印每个文件或目录，这有效地为我们提供了目录和文件计数。</p>

<p>这是怎么回事？(unix) 命令行工具的一般原理是，它们将文本打印到终端 (也称为“打印到标准输出”或<code>STDOUT</code>). 很多命令也可以从流输入 (称为“标准输入”o) 中读取内容 <code>STDIN</code>).</p>

<p>管道操作符可以将这些输入和输出连接在一起，允许我们构建越来越复杂的操作，以满足我们的需要。一个命令的输出可以成为下一个命令的输入。在这种情况下，<code>ls</code> 通常会将其输出到<code>STDOUT</code>, 但是相反 <code>ls</code>输出被制成<code>wc</code>, 它将该输出作为输入，计算它包含的行数，然后将该计数输出到 STDOUT。</p>

<h2 id="一个稍微复杂一点的例子">一个稍微复杂一点的例子</h2>

<p>让我们看一些更复杂的东西。我们将首先尝试获取 MDN 的“获取”页面的内容 <code>curl</code> 命令 (可用于从 url 请求内容)<a href="/en-US/docs/Web/API/fetch">https://developer.mozilla.org/en-US/docs/Web/API/fetch</a>.</p>

<p>但是，这个 URL 是页面的旧位置。如果您在一个新的浏览器标签中输入它，您将 (最终) 被重定向到<a href="/en-US/docs/Web/API/fetch">https://developer.mozilla.org/enUS/docs/Web/API/fetch</a>.</p>

<p>因此，如果您使用 curl 请求 https://developer.mozilla.org/docs/Web/API/fetch，则不会得到输出。现在就试试：</p>
...
<p>通过组合这些命令，我们定制了输出以显示完整的 url，当我们请求时，Mozilla 服务器将通过这些 url 重定向<code>/docs/Web/API/fetch</code> URL.了解您的系统将在未来几年证明是有用的，您将了解这些单一服务工具是如何工作的，以及它们如何成为您解决小众问题的工具库的一部分。</p>

<h2 id="添加工具">添加工具</h2>

<p>现在我们已经了解了系统自带的一些内置命令，让我们看看如何安装和使用第三方 CLI 工具。</p>

<p>目前，用于前端 web 开发的可安装工具的巨大生态系统主要存在于内部 <a href="https://www.npmjs.com">npm</a>, 与 Node.js 紧密合作的私有的包托管服务。随着时间的推移，您可以期望看到更多的包提供者。</p>

<p><a href="https://nodejs.org/en/">Installing Node.js</a> 还要安装 npm 命令行工具 (以及一个以 npm 为中心的补充工具 npx)，它提供了安装其他命令行工具的网关。js 和 npm 在所有系统上都能工作:macOS、Windows 和 Linux。</p>
...
<p>尽管我们将在下一篇文章中讨论许多不同的工具，但我们将继续深入研究 <a href="https://prettier.io/">Prettier</a>. Prettier 是一种固执己见的代码格式化程序，它只有“很少的选择”。更少的选择往往意味着更简单。考虑到工具在复杂性方面有时会失控，“很少的选项”可能非常有吸引力。</p>

<h3 id="在哪里下载我们的CLI工具？">在哪里下载我们的 CLI 工具？</h3>

<p>在开始安装 Prettier 之前，有一个问题需要回答：“我们应该安装到哪里？”</p>
...
<table class="standard-table">
 ...
 <tbody>
  ...
  <tr>
   <td>只下载一次</td>
   <td>
    <p>您团队中的其他开发人员无法使用这些工具，例如，如果您通过 git 这样的工具共享代码仓库。</p>
   </td>
  </tr>
  <tr>
   <td>
   ...
   <td>
    <p>与前一点相关的是，它使得项目代码更难复制 (如果您在本地安装工具，可以将它们设置为依赖项并使用 npm 进行安装 (<code>npm install</code>).</p>
   </td>
  </tr>
  <tr>
   <td>
...
<p>让我们快速演示一下 Prettier，这样您就可以看到它是如何工作的。</p>
...
<ul>
 <li>
  <p>在将代码提交到 git 存储库之前，使用<a href="https://github.com/typicode/husky">Husky</a>.</p>
 </li>
 <li>
  <p>当你在代码编辑器中点击“保存”的时候，无论是<a href="https://marketplace.visualstudio.com/items?itemName=esbenp.prettier-vscode">VS Code</a>, <a href="https://atom.io/packages/prettier-atom">Atom</a>, 或<a href="https://packagecontrol.io/packages/JsPrettier">Sublime Text</a>.</p>
 </li>
 <li>
  <p>作为持续集成检查的一部分，可以使用以下工具<a href="https://github.com/features/actions">Github Actions</a>.</p>
 </li>
</ul>

<p>我们个人的偏好是第二个当使用 say VS 代码时，Prettier 会启动并清理每次我们点击保存时需要做的格式化。关于以不同方式使用 Prettier，您可以在 <a href="https://prettier.io/docs/en/">Prettier docs</a>.</p>
...
<ul>
 ...
 <li><code><a href="http://denilson.sa.nom.br/prettyping/">prettyping</a></code> — <code>ping</code>在命令行上，但是是可视化的 (ping 是检查服务器是否有响应的有用工具)。</li>
 <li><code><a href="http://hisham.hm/htop/">htop</a></code> —进程查看器，当某些东西使您的 CPU 风扇的行为像一个喷气发动机，并且您想要识别出错的程序时，它非常有用。</li>
 <li><code><a href="https://tldr.sh/#installation">tldr</a></code> —在本章前面提到的，但是可以作为命令行工具使用。</li>
</ul>

<p>注意，上面的一些建议可能需要使用 npm 进行安装，就像我们使用 Prettier 所做的那样。</p>
