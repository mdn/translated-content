 ...
 <tbody>
  <tr>
   ...
   <td>
    ...
    <p>React 使用称为 JSX（JavaScript 和 XML）的 HTML-in-JavaScript 语法。熟悉 HTML 和 JavaScript 可以帮助您学习 JSX，并更好地确定应用程序中的错误是与 JavaScript 还是与 React 的更特定领域相关。</p>
   </td>
  </tr>
  <tr>
   <th scope="row">目的：</th>
...
<h2 id="你好_React">你好 React</h2>

<p>如其官方口号所示，<a href="https://reactjs.org/">React</a> 是一个用于构建用户界面的库。React 不是一个框架 —— 它的应用甚至不局限于 Web 开发，它可以与其他库一起使用以渲染到特定环境。例如，<a href="https://reactnative.dev/">React Native</a> 可用于构建移动应用程序；<a href="https://facebook.github.io/react-360/">React 360</a> 可用于构建虚拟现实应用程序……</p>

<p>为了构建 Web 应用，开发人员将 React 与 <a href="https://reactjs.org/docs/react-dom.html">ReactDOM 结合使用</a>。React 和 ReactDOM 通常被与其他真正的 Web 开发框架相提并论，并用于解决相同的问题。当我们将 React 称为“框架”时，就是在进行口语化的理解。</p>

<p>React 的主要目标是最大程度地减少开发人员构建 UI 时发生的错误。它通过使用组件——描述部分用户界面的、自包含的逻辑代码段——来实现此目的。这些组件可以组合在一起以创建完整的 UI，React 将许多渲染工作进行抽象化，使您可以专注于 UI 设计 (译者注：显而易见，此设计不等于视觉稿的设计)。</p>

<h2 id="用例">用例</h2>

<p>与本模块中涵盖的其他框架不同，React 不会对代码约定或文件组织实施严格的规则。这使团队可以设置最适合自己的约定，并以他们希望的任何方式采用 React。React 可以处理一个按钮，一个界面的几个部分或应用程序的整个用户界面。</p>

<p>尽管 React 可以用于<a href="https://reactjs.org/docs/add-react-to-a-website.html">界面的小片段</a>中，但要和 jQuery 这样的库，甚至是像 Vue 这样的框架那样“引入”应用程序并不容易 —— 当你使用 React 构建整个应用程序时更容易上手。</p>
...
<p><img alt="Firefox MacOS的屏幕截图，打开到localhost：3000，显示了默认的create-react-app应用程序" src="default-create-react-app.png"></p>
...
<p>目录 <strong><code>public</code></strong> 包含了开发应用时浏览器会读取的文件，其中最重要的就是 <code>index.html</code>。React 将目录 <strong><code>src</code></strong> 中的代码嵌入这个文件，从而浏览器才能运行此文件。 <code>index.html</code>中的有些内容关乎 create-react-app 的运作，因此除非你知道自己在做什么样的修改，否则不建议编辑这个文件。当然，你可以修改<code>index.html</code>中的 <code><a href="/en-US/docs/Web/HTML/Element/title">&lt;title&gt;</a></code> 元素的内容来表现此应用程序通俗易懂的名称。</p>
...
<p>文件 <code>App.js</code> 主要由三部分组成： 顶部的 <code><a href="/en-US/docs/Web/JavaScript/Reference/Statements/import">import</a></code> 语句，中间的 <code>App</code> 组件，以及底部的 <code><a href="/en-US/docs/Web/JavaScript/Reference/Statements/export">export</a></code> 语句。大多数 React 组件都遵循这个模式。</p>
...
<div class="blockIndicator note">
<p><strong>备注：</strong> 译者补充：webpack 可用于打包 JS 和非 JS 的内容 (当然，非 JS 的内容需要一些插件或加载器来处理)，但是 JavaScript 标准只有关于 JS 的内容，所以 webpack 社区使用这种特殊的 `import` 语句来声明对非 JS 内容的引用。</p>
...
<p>表达式中的元素就像以前写的 HTML 一样，都拥有属性，并且遵循 <code>attribute="value"</code> 的模式。在第三行，开始标签 <code><a href="/en-US/docs/Web/HTML/Element/div">&lt;div&gt;</a></code> 有着 <code>className</code> 属性。这个属性与在 HTML 中的 <code><a href="/en-US/docs/Web/HTML/Global_attributes/class">class</a></code> 属性相同，但是由于 JSX 就是 JavaScript, 我们不能使用 <code>class</code> 属性 - 这个是关键字，意味着 JavaScript 已经用它执行其它任务，使用 <code>class</code> 属性将会在我们的代码中产生冲突。由于同样的原因，一些其它的 HTML 属性在 JSX 中也有着不同的书写方式，当我们碰到它们时，我们将会详述。</p>

<p>把第 6 行的 <code><a href="/en-US/docs/Web/HTML/Element/p">&lt;p&gt;</a></code> 标签内容改为 "Hello, world!" 并保存文件。你会发现这个改变也会立即在浏览器的<code>http://localhost:3000</code> 中同步渲染。现在删掉 <code><a href="/en-US/docs/Web/HTML/Element/a">&lt;a&gt;</a></code> 标签并保存，"Learn React"链接也会同样被删除。</p>
...
<p><a href="/en-US/docs/Web/API/Service_Worker_API/Using_Service_Workers">Service workers</a> 能让我们的 App 离线运行，但它不在本篇文章的范围中，您可以删除第 5 行和第 9 到 12 行。</p>
...
<p>接下来，我们将使用一些 JavaScript 的技巧来让我们在 React 中编辑组件以及处理数据更加顺手。我们将讨论如何在 JSX 中 使用 variables，并且介绍 props，props 是我们用来往组件里传入数据的一种方法，传入之后我们可以用 variable 访问传入的变量。</p>

<h3 id="Variables_in_JSX">Variables in JSX</h3>

<p>回到 <code>App.js</code>, 让我们看一下第 9 行：</p>
...
<ul>
 ...
 <li>Ember
  <ul>
   ...
   <li><a href="/en-US/docs/Learn/Tools_and_testing/Client-side_JavaScript_frameworks/Ember_routing">在 Ember 中路由</a></li>
   <li><a href="/en-US/docs/Learn/Tools_and_testing/Client-side_JavaScript_frameworks/Ember_resources">Ember 资源和故障排除</a></li>
  </ul>
 </li>
 <li>Vue
  <ul>
   <li><a href="/en-US/docs/Learn/Tools_and_testing/Client-side_JavaScript_frameworks/Vue_getting_started">Vue 入门</a></li>
   <li><a href="/en-US/docs/Learn/Tools_and_testing/Client-side_JavaScript_frameworks/Vue_first_component">创建我们的第一个 Vue 组件</a></li>
   <li><a href="/en-US/docs/Learn/Tools_and_testing/Client-side_JavaScript_frameworks/Vue_rendering_lists">渲染 Vue 组件列表</a></li>
   <li><a href="/en-US/docs/Learn/Tools_and_testing/Client-side_JavaScript_frameworks/Vue_methods_events_models">添加新的待办事项表单：Vue 事件，方法和模型</a></li>
   <li><a href="/en-US/docs/Learn/Tools_and_testing/Client-side_JavaScript_frameworks/Vue_styling">使用 CSS 样式化 Vue 组件</a></li>
   <li><a href="/en-US/docs/Learn/Tools_and_testing/Client-side_JavaScript_frameworks/Vue_computed_properties">使用 Vue 计算的属性</a></li>
   <li><a href="/en-US/docs/Learn/Tools_and_testing/Client-side_JavaScript_frameworks/Vue_conditional_rendering">Vue 条件渲染：编辑现有待办事项</a></li>
   <li><a href="/en-US/docs/Learn/Tools_and_testing/Client-side_JavaScript_frameworks/Vue_refs_focus_management">使用 Vue 裁判进行焦点管理</a></li>
