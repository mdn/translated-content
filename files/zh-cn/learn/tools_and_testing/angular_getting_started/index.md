  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <a href="/zh-CN/docs/Learn/JavaScript">JavaScript</a> 语言的核心，
        <a
...
当你用 Angular 搭建一个应用，您正在利用一个可以从单开发人员项目扩展到企业级应用程序的平台。Angular 被设计用来使版本迭代尽可能简单。因此，您可以毫不费力地利用最新成果（developments）。最为重要的是，Angular 的生态系统由超过 170 万的开发人员、库（library）作者和内容创作者组成的多元化群体组成。
...
你将会发现 Angular 的命令行界面是一个用于构建应用程序的宝贵工具。
...
- **Node.js**
  ...
  Angular 需要一个 Node.js 的[目前可用（active）的长期支持版本（LTS）或者一个维护（maintenance）的长期支持版本](https://nodejs.org/about/releases)。有关特定版本要求的信息，参见在 [package.json](https://unpkg.com/@angular/cli/package.json) 文件中的 `engines` 键值。
...
Angular 命令行界面命令均以 `ng` 开头, 随后是您希望命令行界面执行的操作。
...
`ng new` 命令会在桌面上创建一个最小的入门 Angular 应用程序。
附加属性 `--routing` 和 `--style` 规定了如何处理应用程序中的导航和样式。
本教程稍后将会更详细地介绍这些功能。
...
如果您在任何时候想要停止提供应用程序，请在终端中按 `Ctrl + c`。
