...
和 HTML 类似，CSS 不是一门编程语言。它也不是标记语言。**CSS 是一门样式表语言**。CSS 用于选择性地为 HTML 元素添加样式。例如，下列的 CSS 代码选择了所有的段落文本，并将它们设置为红色。
...
- 选择器（Selector）
  - : HTML 元素的名称位于规则集开始。它选择了一个或多个需要添加样式的元素（本例中就是 {{HTMLElement("p")}} 元素）。要给不同的元素添加样式，只需要更改选择器。
- 声明（Declaration）
  - : 一条单独的规则，如 `color: red;` 用来指定要添加样式的元素**属性**。
- 属性（Properties）
  - : 为 HTML 元素添加样式的方式（本例中 `color` 就是 {{htmlelement("p")}} 元素的属性）。在 CSS 中，选择你想在规则中操纵的属性。
- 属性值（Property value）
...
也可以选择多个元素并为它们添加单个规则集。将不同的选择器用逗号分开。例如：
...
之后，我们将标题文本的背景颜色设置为和 HTML 一样的背景颜色。
...
> [!NOTE]
> 上面的做法假定所选图片小于 body的 宽度（600 像素）。超过 600 像素的图片会溢出 body 并占据页面的其他位置。要解决这个问题，可以：1）使用[图片编辑器](https://en.wikipedia.org/wiki/Raster_graphics_editor) 减小图片宽度；2）用 CSS 限制图片大小，将 `<img>` 元素 {{cssxref("width")}} 属性的值设置为小于 600 像素的值。
...
（你可以[在这里](https://mdn.github.io/beginner-html-site-styled/)查看我们的版本。）如果你遇到困难，你可以将 Github 上的[完整示例代码](https://github.com/mdn/beginner-html-site-styled/blob/gh-pages/styles/style.css)与你的文件进行比较。
