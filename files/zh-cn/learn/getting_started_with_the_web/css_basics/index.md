...
- `margin: 0 auto;`——为 `margin` 或 `padding` 等属性设置两个值时，第一个值代表元素的上方*和*下方（在这个例子中设置为 `0`），而第二个值代表左边*和*右边（在这里，`auto` 是一个特殊的值，意思是将可用的水平空间平均分配给左右两侧）。你也可以使用一个、两个、三个或四个值，具体请参考 [margin 语法](/zh-CN/docs/Web/CSS/margin#语法)文档。
