...
> [!NOTE]
> 上面的做法假定所选图片小于 body 的 宽度（600 像素）。超过 600 像素的图片会溢出 body 并占据页面的其他位置。要解决这个问题，可以：1）使用[图片编辑器](https://en.wikipedia.org/wiki/Raster_graphics_editor) 减小图片宽度；2）用 CSS 限制图片大小，将 `<img>` 元素 {{cssxref("width")}} 属性的值设置为小于 600 像素的值。
