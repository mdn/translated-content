...
<table class="learn-box standard-table">
 <tbody>
  <tr>
   <th scope="row">前提：</th>
   <td>你需要知道<a href="/zh-CN/docs/learn/How_the_Internet_works">互联网是怎么工作的</a>，以及<a href="/zh-CN/docs/Learn/Common_questions/What_is_a_web_server">什么是网络服务器</a>。</td>
  </tr>
  <tr>
   <th scope="row">目标：</th>
...
<p>为了解决异步请求的问题，我们需要通过在本地 Web 服务器上运行这些示例来测试这些示例。为我们的目的，最简单的方法之一就是使用 Python 的<code>SimpleHTTPServer</code>模块。</p>

<p>我们需要：</p>

<ol>
 <li>
  <p>安装 Python。如果您正在使用 Linux 或 Mac OS X，则应该已经在您的系统上可用。如果您是 Windows 用户，则可以从 Python 主页获取安装程序，并按照说明进行安装：</p>
 ...
 <li>
  <p>打开你的命令提示符（Windows）/终端（OS X / Linux）。要检查 Python 是否安装，请输入以下命令：</p>
...
cd ..</pre>
 ...
 <li>
  ...
  <pre>＃如果上面返回的 Python 版本是 3.X 
python -m http.server 
＃如果上面返回的 Python 版本是 2.X 
python -m <code>SimpleHTTPServer</code></pre>
 </li>
 <li>
  <p>默认情况下，这将在本地 Web 服务器上的端口 8000 上运行目录的内容。您可以通过转到<code>localhost:8000</code>Web 浏览器中的 URL 来访问此服务器。在这里你会看到列出的目录的内容 - 点击你想运行的 HTML 文件。</p>
 </li>
</ol>
...
<p>Python 的<code>SimpleHTTPServer</code>模块是有用的，但它不知道如何运行用 PHP 或 Python 等语言编写的代码。为了处理这个问题，你需要更多的东西 - 正是你需要的东西取决于你正在运行的服务器端语言。这里有几个例子：</p>

<ul>
 <li>要运行 Python 服务器端代码，您需要使用 Python 网络框架。您可以通过阅读<a href="/en-US/docs/Learn/Server-side/Django">Django Web Framework（Python）</a>来了解如何使用 Django 框架。<a href="http://flask.pocoo.org/">Flask</a>也是一个不错的选择（稍微轻量一点）。要运行 Flask，你需要先<a href="/en-US/docs/Learn/Server-side/Django/development_environment#Installing_Python_3">安装 Python / PIP</a>，然后使用<code>pip3 install flask </code>来安装 Flask。此时，您应该能够运行 Python Flask 示例<code>python3 python-example.py</code>，然后在您的浏览器中打开 <code>localhost:5000</code> 查看。</li>
 ...
 <li>要运行 PHP 服务器端代码，您需要一个可以解释 PHP 的服务器设置。本地 PHP 测试的好选择是<a href="https://www.mamp.info/en/downloads/">MAMP</a>（Mac 和 Windows），  <a href="http://ampps.com/download">AMPPS</a>（Mac，Windows，Linux）和<a href="https://www.linux.com/learn/easy-lamp-server-installation">LAMP</a>（Linux，Apache，MySQL 和 PHP / Python / Perl）。这些是完整的包，创建本地设置，允许您运行 Apache 服务器，PHP 和 MySQL 数据库。</li>
</ul>
