...
<p>多列布局声明提供了一种多列组织内容的方式，正如你在一些报纸中看到的那样。 这篇文章介绍怎么使用这一特性。</p>
...
<table class="learn-box standard-table">
 <tbody>
  ...
  <tr>
   <th scope="row">目标：</th>
   <td>掌握在 web 页面中创建多列布局的方法？，正如你在一些报纸中看到的那样。 </td>
  </tr>
 </tbody>
</table>
...
<p>我们将学习怎么使用多列布局，通常也简写为 <em>multicol</em>。你可以从这里开始 <a href="https://github.com/mdn/learning-area/blob/master/css/css-layout/multicol/0-starting-point.html">downloading the multicol starting point file</a> 然后在合适的地方加入 CSS。在这一小节的结尾，你可以看到最终代码的效果。</p>

<h3>三列布局</h3>

<p>我们从一些很简单的 HTML 开始； 用带有类 <code>container</code> 的简单包装，里面是标题和一些段落。</p>

<p>带有 <code>.container</code> 的 {{htmlelement("div")}} 将成为我们 multicol 的容器。通过这两个属性开启 multicol {{cssxref("column-count")}} 或者 {{cssxref("column-width")}}。  <code>column-count</code> 将创建指定数量的列，所以如果你把下面的 CSS 加到样式表里让后重载入页面，你将得到 3 列：</p>
...
<p>{{ EmbedLiveSample('三列布局', '100%', 400) }}</p>

<h3>设置列宽</h3>

<p>像下面这样使用 <code>column-width</code> 更改 CSS：</p>
...
<p>{{ EmbedLiveSample('设置列宽', '100%', 400) }}</p>
...
<p>{{ EmbedLiveSample('给多列增加样式', '100%', 400) }}</p>
...
<p>{{ EmbedLiveSample('列与内容折断', '100%', 600) }}</p>
...
<p>{{ EmbedLiveSample('设置 break-inside', '100%', 600) }}</p>
