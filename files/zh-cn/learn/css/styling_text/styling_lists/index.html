...
<p><a href="/zh-CN/Learn/HTML/Introduction_to_HTML/HTML_text_fundamentals#Lists">List 列表</a> 大体上和其他文本一样，但是仍有一些你需要知道的特殊 CSS 属性，和一些可供参考的最佳实践，这篇文章将阐述这一切。</p>
...
<ul>
 <li>{{htmlelement("ul")}}  和  {{htmlelement("ol")}}  元素设置{{cssxref("margin")}}的顶部和底部：16px(1em) 0;和 padding-left: 40px(2.5em); （在这里注意的是浏览器默认字体大小为 16px）。</li>
 <li>{{htmlelement("li")}}  默认是没有设置间距的。</li>
 <li>{{htmlelement("dl")}}  元素设置 margin 的顶部和底部：16px(1em) ，无内边距设定。</li>
 <li>{{htmlelement("dd")}} 元素设置为： {{cssxref("margin-left")}}  <code>40px</code> (<code>2.5em</code>)。</li>
 <li>在参考中提到的 {{htmlelement("p")}}  元素设置 margin 的顶部和底部：16px(1em)，和其他的列表类型相同。</li>
</ul>
...
<ul>
 ...
 <li>规则集 4 在段落和列表项目上设置相同的 {{cssxref("line-height")}} ，因此段落和每个单独的列表项目将在行之间具有相同的间距。这也将有助于保持垂直间距一致。</li>
 <li>规则集 5-7 适用于描述列表 - 我们在描述列表的术语和其描述上设置与段落和列表项相同的行高，以及 {{cssxref("margin-bottom")}} 为 1.5 rem（与段落（p）和列表项目（li））相同。再次强调一遍，这里很好地实现了一致性！我们还使描述术语具有粗体字体，因此它们在视觉上脱颖而出。 </li>
</ul>
...
<ul>
 ...
 <li>将 {{cssxref("list-style-type")}} 设置为 none，以便默认情况下不会显示项目符号。我们将使用 {{cssxref("background")}} 属性来代替项目符号。</li>
 <li>为每个无序列表项插入项目符号，其相应的属性如下：
  <ul>
   ...
   <li>{{cssxref("background-size")}}: 设置背景图片的大小。理想条件下，我们想要项目符号与列表项的大小相同（比列表项稍大或稍小亦可）。我们使用的尺寸为 1.6rem（16px），它非常吻合我们为项目符号设置的 20px  的填充， 16px 加上 4px 的空格间距，可以使项目符号和列表项文本效果更好。</li>
...
<p><img alt="an unordered list with the bullet points set as little star images" src="https://mdn.mozillademos.org/files/12956/image-bullets.png"></p>
...
<p>有时，您可能想在有序列表上进行不同的计数方式。例如：从 1 以外的数字开始，或向后倒数，或者按步或多于 1 计数。HTML 和 CSS 有一些工具可以帮助您</p>

<h3 id="start">start</h3>

<p>{{htmlattrxref("start","ol")}} 属性允许你从 1 以外的数字开始计数。示例如下：</p>
...
<div class="note">
<p><strong>备注：</strong> 纵然你使用非数字的 {{cssxref("list-style-type")}}, 你仍需要使用与数值同等意义的值作为 value 的属性。</p>
</div>
...
<p>{{ EmbedLiveSample('主动学习：为嵌套式列表添加样式', 700, 800) }}</p>

<h2 id="另请参阅">另请参阅</h2>

<p>CSS 计数器提供用于自定义列表计数和样式的高级工具，但它们相当复杂。如果你想更深入了解，请查看如下资源：</p>
