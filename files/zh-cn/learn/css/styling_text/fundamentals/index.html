...
<p>在这篇文章中，我们将带你开始掌握 {{glossary("CSS")}} 的文字样式的旅程。这里我们将详细介绍文本/字体样式的所有基本原理，包括设置文字的粗细，字体和样式，文字的属性简写，文字的对齐，和其他效果，以及行和字母间距。</p>
...
<h2 id="CSS中的文字样式涉及什么？">CSS 中的文字样式涉及什么？</h2>
...
<div class="note">
<p><strong>备注：</strong> 如果上面的段落让你感到困惑，没关系，在继续之前，可以重新看看我们的 <a href="/zh-CN/docs/Learn/CSS/Introduction_to_CSS/Box_model">Box model</a> 文件，复习盒模型的理论。</p>
</div>
...
<p>{{ EmbedLiveSample('颜色', '100%', 230) }}</p>

<h3 id="字体种类">字体种类</h3>

<p>要在你的文本上设置一个不同的字体，你可以使用 {{cssxref("font-family")}}  属性，这个允许你为浏览器指定一个字体 (或者一个字体的列表)，然后浏览器可以将这种字体应用到选中的元素上。浏览器只会把在当前机器上可用的字体应用到当前正在访问的网站上；如果字体不可用，那么就会用浏览器默认的字体代替 <a href="#默认字体">default font</a>. 下面是一个简单的例子：</p>
...
<p>实际的 Web 安全字体列表将随着操作系统的发展而改变，但是可以认为下面的字体是网页安全的，至少对于现在来说 (它们中的许多都非常流行，这要感谢微软在 90 年代末和 21 世纪初期的倡议<em><a href="https://en.wikipedia.org/wiki/Core_fonts_for_the_Web">Core fonts for the Web</a></em> )：</p>
...
<div class="note">
<p><strong>备注：</strong> 在各种资源中，<a href="http://www.cssfontstack.com/">cssfontstack.com</a> 网站维护了一个可用在 Windows 和 Mac 操作系统上使用的网页安全字体的列表，这可以帮助决策网站的安全性。</p>
</div>
...
<p>{{ EmbedLiveSample('一个使用 font-family 的例子', '100%', 220) }}</p>
...
<p>当调整你的文本大小时，将文档 (document) 的基础  <code>font-size</code> 设置为 10px 往往是个不错的主意，这样之后的计算会变得简单，所需要的 (r)em 值就是想得到的像素的值除以 10，而不是 16。做完这个之后，你可以简单地调整在你的 HTML 中你想调整的不同类型文本的字体大小。在样式表的指定区域列出所有<code>font-size</code>的规则集是一个好主意，这样它们就可以很容易被找到。</p>
...
<p>{{ EmbedLiveSample('一个简单的 size 示例', '100%', 260) }}</p>
...
<ul>
 ...
 <li>{{cssxref("text-decoration")}}: 设置/取消字体上的文本装饰 (你将主要使用此方法在设置链接时取消设置链接上的默认下划线。) 可用值为：
  ...
  你应该注意到 {{cssxref("text-decoration")}} 可以一次接受多个值，如果你想要同时添加多个装饰值， 比如 <code>text-decoration: underline overline</code>.。同时注意 {{cssxref("text-decoration")}} 是一个缩写形式，它由 {{cssxref("text-decoration-line")}}, {{cssxref("text-decoration-style")}} 和 {{cssxref("text-decoration-color")}} 构成。你可以使用这些属性值的组合来创建有趣的效果，比如 <code>text-decoration: line-through red wavy</code>.</li>
</ul>
...
<p>{{ EmbedLiveSample('字体样式、字体粗细、文本转换和文本装饰', '100%', 260) }}</p>
...
<ol>
 ...
 <li>模糊半径 - 更高的值意味着阴影分散得更广泛。如果不包含此值，则默认为 0，这意味着没有模糊。可以使用大多数的 CSS 单位 <a href="/zh-CN/Learn/CSS/Introduction_to_CSS/Values_and_units#Length_and_size">length and size units</a>.</li>
...
<pre class="brush: css"><code class="language-css">text-shadow: -1px -1px 1px #aaa,
...
<p>{{ EmbedLiveSample('多种阴影', '100%', 260) }}</p>

<div class="note">
<p><strong>备注：</strong> 你可以看到更多有趣的关于 <code>text-shadow</code> 使用的示例在 <a href="http://www.sitepoint.com/moonlighting-css-text-shadow/">Moonlighting with CSS text-shadow</a>.</p>
</div>
...
<p>{{ EmbedLiveSample('文本对齐', '100%', 260) }}</p>
...
<p>{{ EmbedLiveSample('行高', '100%', 300) }}</p>
...
<p>{{ EmbedLiveSample('字母和单词间距', '100%', 330) }}</p>
...
<ul>
 ...
 <li>{{cssxref("direction")}}: 定义文本的方向 (这取决于语言，并且通常最好让 HTML 来处理这部分，因为它是和文本内容相关联的。)</li>
...
<p>在这个动手练习中，我们没有任何具体的练习来做：我们只是希望你和一些字体/文本布局属性相处地愉快，看看你可以制作什么！你可以使用离线 HTML / CSS 文件进行此操作，也可以将代码输入到下面的实时可编辑示例中。</p>
...
<p>{{ EmbedLiveSample('动手练习：使用样式文本', 700, 800) }}</p>
