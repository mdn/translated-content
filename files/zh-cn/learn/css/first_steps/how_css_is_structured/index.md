...
有几种情况下，内联样式是比较常见的。如果你的工作环境有很大的限制，你可能不得不使用内联样式。例如，也许你的 CMS 只允许你编辑 HTML 主体。你也可能在 HTML 电子邮件中看到大量的内联样式，以实现与尽可能多的电子邮件客户端的兼容。
...
选择器以 HTML 为目标，对内容应用样式。我们已经在 [CSS 入门](/zh-CN/docs/Learn/CSS/First_steps/Getting_started)的教程中探索了各种选择器。如果 CSS 没有像预期的那样应用于内容，你的选择器可能与你认为应该匹配的方式不一致。
...
然而，在我们前面的例子中，类选择器和元素选择器之间存在冲突，类选择器占了上风，使段落文本变成了红色。即使冲突的样式在样式表的后面出现，这怎么可能发生呢？一个类被认为是更具体的，因为它比元素选择器**优先级**更强，所以它取消了其他冲突的样式声明。
...
现在，请记住优先级的存在。有时，CSS 可能不会像你预期的那样应用，因为样式表中的其他东西可能有更强的优先级。认识到不止一个规则可以适用于一个元素，是解决这类问题的第一步。
...
CSS 属性和值是不区分大小写的。一个属性——值对中的属性和值用冒号（`:`）分开。
...
留白是指实际的空格、制表符和新行。就像浏览器忽略了HTML中的空白一样，浏览器也忽略了CSS中的空白。留白的价值在于它可以提高可读性。
...
下一个例子显示了压缩格式的等效 CSS。虽然这两个例子的工作原理是一样的，但下面这个例子更难读。
