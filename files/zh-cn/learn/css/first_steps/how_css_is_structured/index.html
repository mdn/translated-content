...
<h2 id="在你的HTML里面应用CSS">在你的 HTML 里面应用 CSS</h2>

<p>我们需要了解的第一件事情就是在文档中应用 CSS 的三种方法</p>

<h3 id="外部样式表">外部样式表</h3>

<p>在<a href="/en-US/docs/Learn/CSS/First_steps/Getting_started">Getting started with CSS</a> 中我们将外部样式表链接到页面。这是将 css 附加到文档中的最常见和最有用的方法，因为您可以将 css 链接到多个页面，从而允许您使用相同的样式表设置所有页面的样式。在大多数情况下，一个站点的不同页面看起来几乎都是一样的，因此您可以使用相同的规则集来获得基本的外观。</p>
...
<p>{{htmlelement("link")}} 元素的 <code>href</code> 属性需要引用你的文件系统中的一个文件。</p>
...
<p>内部样式表是指不使用外部 CSS 文件，而是将 CSS 放在 HTML 文件{{htmlelement("head")}}标签里的{{htmlelement("style")}}标签之中。</p>
...
<p>内联样式表存在于 HTML 元素的 style 属性之中。其特点是每个 CSS 表只影响一个元素：</p>
...
<h2 id="在文本中使用CSS">在文本中使用 CSS</h2>

<p>有的时候我们会在文本中使用大量的 CSS。为了做到这些，我们推荐你在你的计算机上创建一个新的文件夹，然后在文件夹中创建下面两个文件的副本。</p>
...
<p>如果你没有可以轻松创建文件的系统，则可以使用下面交互式编译器进行实验。</p>
...
<p>讲到 CSS 就不得不说到选择器，并且在之前的辅导教程中我们已经列举了一些不同的选择器。为了样式化某些元素，我们会通过选择器来选中 HTML 文档中的这些元素。如果你的样式没有生效，那很可能是你的选择器没有像你想象的那样选中你想要的元素。</p>
...
<p>注意：在下一个模块中，您将在我们的<strong>CSS 选择器</strong>教程中了解更多关于选择器的内容。</p>
</div>

<h3 id="专一性">专一性</h3>

<p>通常情况下，两个选择器可以选择相同的 HTML 元素。考虑下面的样式表，其中我有一个规则，其中有一个将段落设置为蓝色的 p 选择器，还有一个将选定元素设置为红色的类。</p>
...
<p><strong>自己试试上面的实验--将 HTML 添加到您的实验中，然后将两个 p{.}规则添加到样式表中。接下来，将第一个 p 选择器更改为.Special，以查看它如何更改样式。</strong></p>
...
<p>在最基本的层面上，CSS 由两个组成部分组成：</p>

<ul>
 <li><strong>属性：</strong>人类可读的标识符，指示您想要更改的样式特征 (例如<code><a href="/en-US/docs/Web/CSS/font-size">font-size</a></code>, <code><a href="/en-US/docs/Web/CSS/width">width</a></code>, <code><a href="/en-US/docs/Web/CSS/background-color">background-color</a></code>) 你想改变。</li>
...
<p>最后，CSS 声明块与选择器配对，以生成 CSS 规则集 (或 CSS 规则)。我们的图像包含两个规则，一个用于 H1 选择器，另一个用于 p 选择器。h1 的规则被突出显示。</p>

<p><img alt="The rule for h1 highlighted" src="rules.png"></p>

<p>将 CSS 属性设置为特定值是 CSS 语言的核心功能。CSS 引擎计算哪些声明应用于页面的每个元素，以便适当地布局和样式它。重要的是要记住，在 css 中，属性和值都是区分大小写的。每对中的属性和值由冒号 (：) 分隔。</p>

<p><strong>尝试查找以下属性的不同值，并编写将它们应用于不同 HTML 元素的 CSS 规则：</strong><strong> </strong></p>
...
<div class="warning">
<p><strong>重要事项：</strong>如果属性未知或某个值对给定属性无效，则声明被视为无效，并被浏览器的 CSS 引擎完全忽略。</p>
</div>

<div class="warning">
<p><strong>重要：</strong>在 CSS(和其他网络标准) 中，当语言表达存在不确定性时，美国的拼写被视作公认的标准。例如，颜色应该始终拼写为 color。colour 是不起作用的。</p>
</div>

<h3 id="函数">函数</h3>

<p>虽然大多数值是相对简单的关键字或数值，但也有一些可能的值以函数的形式出现。一个例子是 calc() 函数。这个函数允许您在 CSS 中进行简单的计算，例如：</p>
...
<p>一个函数由函数名和一些括号组成，其中放置了该函数的允许值。在上面的 calc() 示例中，我要求此框的宽度为包含块宽度的 90%，减去 30 像素。这不是我可以提前计算的东西，只是在 CSS 中输入值，因为我不知道 90% 会是什么。与所有值一样，MDN 上的相关页面将有使用示例，这样您就可以看到函数是如何工作的。</p>

<p>另一个例子是{{cssxref("&lt;transform&gt;")}}, 例如 <code>rotate()</code>.</p>
...
<p>到目前为止，我们还没有遇到 <code><a href="/en-US/docs/Web/CSS/At-rule">@rules</a></code> (pronounced "at-rules"). 这是一些特殊的规则，为 CSS 提供了一些关于如何表现的指导。有些<code>@rules</code> 规则很简单，有规则名和值。例如，要将额外的样式表导入主 CSS 样式表，可以使用<code>@import</code>:</p>

<pre class="brush: css notranslate">@import 'styles2.css';</pre>

<p>您将遇到的最常见的 <code>@rules</code> 之一是 @media，它允许您使用 <a href="/en-US/docs/Web/CSS/Media_Queries">媒体查询 </a>来应用 CSS，仅当某些条件成立 (例如，当屏幕分辨率高于某一数量，或屏幕宽度大于某一宽度时)。</p>
...
<p>在这些教程中，你将遇到一些其他的规则 <code>@rules</code> </p>

<p><strong>查看是否可以将媒体查询添加到 CSS 中，该查询将根据视口宽度更改样式。更改浏览器窗口的宽度以查看结果。</strong></p>

<h2 id="速记属性">速记属性</h2>

<p>一些属性，如 {{cssxref("font")}}, {{cssxref("background")}}, {{cssxref("padding")}}, {{cssxref("border")}}, and {{cssxref("margin")}} 等属性称为速记属性--这是因为它们允许您在一行中设置多个属性值，从而节省时间并使代码更整洁。</p>
...
<p>与任何的代码工作一样，在编写 CSS 过程中，最好的练习方式就是添加注释。这样做可以帮助您在过了几个月后回来修改或优化代码时了解它们是如何工作的，同时也便于其他人理解您的代码。</p>

<p>CSS中的注释以/*开头，以*/结尾。在下面的代码块中，注释标记了不同代码节的开始。当代码库变得更大时，这对于帮助您导航代码库非常有用--在代码编辑器中搜索注释可以高效地定位代码节。</p>
...
<p>添加一些注释到您的 CSS，以适应使用他们。</p>

<h2 id="空白">空白</h2>

<p>空白是指实际空格、制表符和新行。以与 HTML 相同的方式，浏览器往往忽略 CSS 中的大部分空白；许多空白只是为了提高可读性。</p>
...
<p>试着在 CSS 中使用空格，看看什么情况下破坏了东西，什么时候没有破坏<strong>。</strong></p>

<h2 id="接下来是什么？">接下来是什么？</h2>

<p>了解一下浏览器如何将 HTML 和 CSS 转换成网页是很有用的，所以在下一篇文章 — <a href="/zh-CN/docs/Learn/CSS/First_steps/CSS如何运行">CSS 是如何工作的</a> — 我们将看看这个过程。</p>

<p>{{PreviousMenuNext("Learn/CSS/First_steps/开始", "Learn/CSS/First_steps/CSS 如何运行", "Learn/CSS/First_steps")}}</p>

<h2 id="在这个模块中">在这个模块中</h2>

<ol>
 <li><a href="/zh-CN/docs/Learn/CSS/First_steps/What_is_CSS">什么是 CSS?</a></li>
 <li><a href="/zh-CN/docs/Learn/CSS/First_steps/开始">开始使用 CSS </a></li>
 <li><a href="/zh-CN/docs/Learn/CSS/First_steps/How_CSS_is_structured">如何构建 CSS </a><a href="/en-US/docs/Learn/CSS/First_steps/How_CSS_is_structured"> </a></li>
 <li><a href="/zh-CN/docs/Learn/CSS/First_steps/CSS如何运行">CSS 是如何工作的</a></li>
 <li><a href="/zh-CN/docs/Learn/CSS/First_steps/Using_your_new_knowledge">使用你的新知识</a></li>
</ol>
