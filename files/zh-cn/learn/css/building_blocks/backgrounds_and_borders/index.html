...
<p>在这节课中，我们来看看，使用 CSS 背景和边框来做一些，具有一些创造性的事情。渐变、背景图像和圆角，背景和边框的巧妙运用是 CSS 中许多样式问题的答案。</p>

<table class="learn-box standard-table">
 <tbody>
  <tr>
   <th scope="row">预备知识：</th>
   <td>基本的计算机知识，<a href="/zh-CN/docs/Learn/Getting_started_with_the_web/Installing_basic_software">安装基本的软件</a>，<a href="/zh-CN/docs/Learn/Getting_started_with_the_web/Dealing_with_files">文件处理</a>基本知识，HTML 基础知识  (如果不了解 HTML，请移步 <a href="/zh-CN/docs/Learn/HTML/Introduction_to_HTML">学习 HTML 入门</a>)，以及 CSS 如何工作的基本常识 (如果不了解 CSS，请移步 <a href="/zh-CN/docs/Learn/CSS/First_steps">学习 CSS 第一步</a>.)</td>
  </tr>
  <tr>
   <th scope="row">目标：</th>
...
<h2 id="CSS的背景样式">CSS 的背景样式</h2>

<p>CSS {{cssxref("background")}} 属性是我们将在本课中学习的许多普通背景属性的简写。如果您在样式表中发现了一个复杂的背景属性，可能会觉得难以理解，因为可以同时传入这么多值。</p>
...
<p>{{cssxref("background-color")}} 属性定义了 CSS 中任何元素的背景颜色。属性接受任何有效的<code>&lt;color&gt;值</code>。背景色扩展到元素的内容和内边距的下面。</p>

<p>在下面的示例中，我们使用了各种颜色值来为元素盒子添加背景颜色：heading 和{{htmlelement("span")}}元素。</p>
...
<p>{{cssxref("background-image")}} 属性允许在元素的背景中显示图像。在下面的例子中，我们有两个方框——一个是比方框大的背景图像，另一个是星星的小图像。</p>
...
<p>{{cssxref("background-repeat")}} 属性用于控制图像的平铺行为。可用的值是：</p>
...
<p><strong>在下面的示例中尝试这些值。我们已经将值设置为 no-repeat，因此您将只能看到一个星星。尝试不同的值—repeat-x 和 repeat-y—看看它们的效果如何。</strong></p>

<p>{{EmbedGHLiveSample("css-examples/learn/backgrounds-borders/repeat.html", '100%', 800)}}</p>

<h4 id="调整背景图像的大小">调整背景图像的大小</h4>

<p>在上面的例子中，我们有一个很大的图像，由于它比作为背景的元素大，所以最后被裁剪掉了。在这种情况下，我们可以使用 {{cssxref("background-size")}}属性，它可以设置长度或百分比值，来调整图像的大小以适应背景。</p>
...
<p>{{cssxref("background-position")}} 属性允许您选择背景图像显示在其应用到的盒子中的位置。它使用的坐标系中，框的左上角是 (0,0)，框沿着水平 (x) 和垂直 (y) 轴定位。</p>
...
<p>你可以使用像<code>top</code>和<code>right</code>这样的关键字 (在 {{cssxref("background-image")}} 页面上查找其他的关键字):</p>
...
<p>或者使用 <a href="/zh-CN/docs/Web/CSS/length">长度值</a> 和 <a href="/zh-CN/docs/Web/CSS/percentage">百分比</a>：</p>
...
<div class="blockIndicator note">
<p><strong>备注：</strong> <code>background-position</code> 是 {{cssxref("background-position-x")}} 和 {{cssxref("background-position-y")}} 的简写，它们允许您分别设置不同的坐标轴的值。</p>
</div>

<h3 id="渐变背景">渐变背景</h3>

<p>当渐变用于背景时，也可以使用像图像一样的 {{cssxref("background-image")}} 属性设置。</p>

<p>您可以在 MDN 的 <a href="/zh-CN/docs/Web/CSS/gradient"><code>&lt;gradient&gt;</code></a> 数据类型页面上，了解更多关于渐变的不同类型，以及使用它们可以做的事情。使用渐变的一个有趣方法是，使用 web 上可用的许多 CSS 渐变生成器之一，比如<a href="https://cssgradient.io/">这个</a>。您可以创建一个渐变，然后复制并粘贴生成它的源代码。</p>
...
<pre class="brush: css notranslate">background-image: url(image1.png), url(image2.png), url(image3.png), url(image4.png);
...
<p>另一个可供选择的背景是指定他们如何滚动时，内容滚动。这是由 {{cssxref("background-attachment")}} 属性控制的，它可以接受以下值：</p>
...
<p>{{cssxref("background-attachment")}} 属性只有在有内容要滚动时才会有效果，所以我们做了一个示例来演示这三个值之间的区别——看看 <a href="http://mdn.github.io/learning-area/css/styling-boxes/backgrounds/background-attachment.html">background-attachment.html</a> (或者看看这儿的 <a href="https://github.com/mdn/learning-area/tree/master/css/styling-boxes/backgrounds">源代码</a>))。</p>

<h3 id="使用background_的简写">使用 background 的简写</h3>

<p>正如我在本课开始时提到的，您将经常看到使用 {{cssxref("background")}} 属性指定的背景。这种简写允许您一次设置所有不同的属性。</p>
...
<p>查看 {{cssxref("background")}} 的 MDN 页面，以查看所有的注意事项。</p>
...
<p>在学习盒子模型时，我们发现了边框如何影响盒子的大小。在这节课中，我们将看看如何创造性地使用边界。通常，当我们使用 CSS 向元素添加边框时，我们使用一个简写属性在一行 CSS 中设置边框的颜色、宽度和样式。我们可以使用 {{cssxref("border")}} 为一个框的所有四个边设置边框。</p>
...
<div class="blockIndicator note">
<p><strong>备注：</strong> 这些顶部、右侧、底部和左侧边框属性还具有与文档写入模式相关的映射逻辑属性 (例如，从左到右或从右到左的文本，或从上到下)。在下一课中，我们将探讨这些问题，这包括处理不同的文本指示 <a href="/zh-CN/docs/Learn/CSS/Building_blocks/Handling_different_text_directions">详情</a>。</p>
</div>
...
<p>通过使用 {{cssxref("border-radius")}} 属性和与方框的每个角相关的长边来实现方框的圆角。可以使用两个长度或百分比作为值，第一个值定义水平半径，第二个值定义垂直半径。在很多情况下，您将只传递一个值，这两个值都将使用。</p>
...
<p>我们在下面的示例中设置了所有四个角，然后更改右上角的值使之不同。您可以使用这些值来更改圆角样式。查看 {{cssxref("border-radius")}} 的属性页，查看可用的语法选项。</p>

<p>{{EmbedGHLiveSample("css-examples/learn/backgrounds-borders/corners.html", '100%', 800)}}</p>

<h2 id="玩转背景和边框">玩转背景和边框</h2>

<p>为了测试你的新知识，尝试使用背景和边框创建以下内容，使用下面的例子作为起点：</p>

<ol>
 <li>给方框一个 5px 的黑色实心边框，圆角为 10px。</li>
 <li>添加一个背景图像 (使用 URL balloons.jpg)，并调整它的大小，使它能够覆盖盒子。</li>
 <li>给&lt;h2&gt;一个半透明的黑色背景颜色，并使文本为白色。</li>
</ol>

<p>{{EmbedGHLiveSample("css-examples/learn/backgrounds-borders/task.html", '100%', 800)}}</p>

<div class="blockIndicator note">
<p><strong>备注：</strong> 你可以点击 <a href="https://github.com/mdn/css-examples/blob/master/learn/solutions.md">这里</a>查看解决方案——但请先尝试一下自己能否实现。</p>
</div>

<h2 id="总结">总结</h2>

<p>我们在这里已经介绍了很多，您可以看到有很多要添加背景或边框到盒子中。如果您想了解更多关于我们讨论过的特性的信息，请浏览不同的属性页面。MDN 上的每个页面都有更多的用法示例，供您玩转并增强您的知识。</p>
