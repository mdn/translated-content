...
<p>在 CSS 中，所有的元素都被一个个的“盒子（box）”包围着，理解这些“盒子”的基本原理，是我们使用 CSS 实现准确布局、处理元素排列的关键。</p>
...
<table class="learn-box standard-table">
 <tbody>
  <tr>
   ...
   <td>
    <p>基本的计算机知识，<a href="/zh-CN/Learn/Getting_started_with_the_web/Installing_basic_software">安装基本的软件</a>，<a href="/zh-CN/Learn/Getting_started_with_the_web/Dealing_with_files">文件处理</a>基本知识，HTML 基础知识  (如果不了解 HTML，请移步 <a href="/zh-CN/docs/Learn/HTML/Introduction_to_HTML">学习 HTML 入门</a>)，以及 CSS 如何工作的基本常识 (如果不了解 CSS，请移步 <a href="/zh-CN/docs/Learn/CSS/First_steps">学习 CSS 第一步</a>.)</p>
   </td>
  </tr>
  <tr>
...
<p>同样盒模型还有内部显示类型，它决定了盒子内部元素是如何布局的。默认情况下是按照 <strong><a href="/zh-CN/docs/Learn/CSS/CSS_layout/Normal_Flow">正常文档流</a> </strong>布局，也意味着它们和其他块元素以及内联元素一样 (如上所述).</p>

<p>但是，我们可以通过使用类似 <code>flex</code> 的 <code>display</code> 属性值来更改内部显示类型。如果设置 <code>display: flex</code>，在一个元素上，外部显示类型是 <code>block</code>，但是内部显示类型修改为 <code>flex</code>。该盒子的所有直接子元素都会成为 flex 元素，会根据<a href="/zh-CN/docs/Learn/CSS/CSS_layout/Flexbox">弹性盒子（Flexbox）</a>规则进行布局，稍后您将了解这些规则。</p>

<div class="blockIndicator note">
<p><strong>备注：</strong> 想要了解更多有关显示值以及盒子在块和内联布局中的工作原理，请参阅<a href="/zh-CN/docs/Web/CSS/CSS_Flow_Layout/Block_and_Inline_Layout_in_Normal_Flow">常规流中的块和内联布局</a>。</p>
</div>

<p>当你进一步了解 css 布局的更多细节的时候，你会了解到 <code>flex</code>，和其他内部显示类型会用到的值，例如 <a href="/zh-CN/docs/Learn/CSS/CSS_layout/Grids"><code>grid</code></a> 。</p>

<p>块级和内联布局是 web 上默认的行为 —— 正如上面所述， 它有时候被称为 <em>正常文档流</em>， 因为如果没有其他说明，我们的盒子布局默认是块级或者内联。</p>

<h2 id="不同显示类型的例子">不同显示类型的例子</h2>

<p>让我们继续看看别的例子。下面三个 html 元素，都有一个外部显示类型 <code>block</code>。第一个是一个段落，在 CSS 中加了边框。浏览器把它渲染成一个块级盒子，所以段落从新的一行开始，而且宽度占满一行。</p>
...
<h2 id="什么是CSS_盒模型">什么是 CSS 盒模型？</h2>
...
<div class="blockIndicator note">
<p><strong>备注：</strong> <a href="https://github.com/mdn/css-examples/blob/master/learn/solutions.md#the-box-model">单击此处查看答案</a>。</p>
</div>

<h3 id="使用调试工具来查看盒模型">使用调试工具来查看盒模型</h3>

<p><a href="/zh-CN/docs/Learn/Common_questions/What_are_browser_developer_tools">浏览器开发者工具</a>可以使你更容易地理解 box 模型。如果你在 Firefox 的 DevTools 中查看一个元素，你可以看到元素的大小以及它的外边距、内边距和边框。这是一个很好的检查元素大小的方式，可以便捷的判断你的盒子大小是否符合预期 !</p>
...
<p>有许多规则规定了什么时候外边距会折叠，什么时候不会折叠。相关更多信息，请参阅<a href="/zh-CN/docs/Web/CSS/CSS_Box_Model/Mastering_margin_collapsing">外边距重叠</a>。现在首先要记住的事情是，外边距会折叠这个事情。如果你用外边距创建空间而没有得到你想要的效果，那这可能就是这个原因。</p>
...
<h2 id="使用display_inline-block">使用 display: inline-block</h2>
...
<p>在下一节课中，我们将看看如何使用<a href="/zh-CN/docs/Learn/CSS/Building_blocks/Backgrounds_and_borders">背景和边框</a>来使你的普通盒子看起来更有趣。</p>
