...
<h3 id="安装Rust">安装 Rust</h3>

<p>前往 <a href="https://www.rust-lang.org/install.html">Install Rust</a> 页面并跟随指示安装 Rust。这里会安装一个名为“rustup”的工具，这个工具能让你管理多个不同版本的 Rust。默认情况下，它会安装用于惯常 Rust 开发的 stable 版本 Rust Release。Rustup 会安装 Rust 的编译器 <code>rustc</code>、Rust 的包管理工具 <code>cargo</code>、Rust 的标准库 <code>rust-std</code> 以及一些有用的文档 <code>rust-docs</code>。</p>
...
<h4 id="编写能够在JavaScript中调用的_Rust_函数">编写能够在 JavaScript 中调用的 Rust 函数</h4>
...
<p>让我们建立一个使用我们包的网站！人们通过各种打包工具使用 npm 包，在本教程中，我们将使用 <code>webpack</code>。它比其他某些打包工具稍微复杂一点，但展示了更实际的用法。<br>
 <br>
...
<p>请注意，您需要在依赖项部分的 <code>@</code> 之后填写自己的用户名。<br>
 <br>
 接下来，我们需要配置 Webpack。创建 <code>webpack.config.js</code> 并输入：</p>
...
<p>请注意，您需要再次填写您的 npm 用户名。<br>
 <br>
 这将从<code>node_modules</code>文件夹导入我们的模块。这不是最佳做法，但这里只做一个演示，因此暂时就这样用。加载后，它将从该模块调用<code>greet</code>函数，并传入字符串“WebAssembly”参数。注意这里看上去没有什么特别的，但是我们正在调用 Rust 代码！就 JavaScript 代码所知，这只是一个普通模块。<br>
 <br>
...
<p>这将启动一个 Web 服务器。访问 <a href="http://localhost:8080">http://localhost:8080 </a>，您应该会在屏幕上看到一个警告框，其中包含 <code>Hello, WebAssembly!</code> ！我们已经成功地从 JavaScript 调用了 Rust，并从 Rust 调用了 JavaScript。</p>
