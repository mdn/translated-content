...
<p>当你在用 C/C++ 之类的语言编写模块时，你可以使用<a href="/zh-CN/docs/Mozilla/Projects/Emscripten">Emscripten</a>来将它编译到 WebAssembly。让我们来看看它是如何工作的。</p>
...
<ul>
 <li><a href="https://git-scm.com/">Git</a> — Linux 和 macOS 的机器一般已经预装了，在 Windows 下您可以从这里下载 <a href="https://git-scm.com/download/win">Git for Windows installer</a>。</li>
 <li>CMake — 在 Linux 或者 macOS 上，使用类似 apt-get 或 <a href="http://brew.sh/">brew</a> 这样的包管理器来安装它，请确保依赖以及路径是否正确。在 Windows 上，使用 <a href="https://cmake.org/download/">CMake installer</a>。</li>
 <li>主系统编译器 — 在 Linux 下，<a href="http://askubuntu.com/questions/154402/install-gcc-on-ubuntu-12-04-lts">安装 GCC</a>。在 macOS 下，<a href="https://itunes.apple.com/us/app/xcode/id497799835">安装 Xcode</a>。在 Windows 下，安装 <a href="https://www.microsoft.com/zh-CN/download/details.aspx?id=48146">Visual Studio Community 2015 with Update 3 or newer</a>。</li>
 <li>Python 2.7.x — On Linux and macOS, this is most likely provided out of the box. 从 <a href="https://wiki.python.org/moin/BeginnersGuide/Downloadhere">初学者指南</a> 获取帮助。在 Windows 上，从 <a href="https://www.python.org/downloads/">Python 主页</a>获取安装包。</li>
</ul>

<div class="note">
<p><strong>备注：</strong> 在 Windows 下您可能需要<a href="https://sourceforge.net/projects/pywin32/files/pywin32/">pywin32</a>，为了降低安装 pywin32 可能遇到的错误，请使用管理员权限在 cmd 内运行安装程序。</p>
</div>

<h3 id="编译Emscripten">编译 Emscripten</h3>

<p>接下来，您需要通过源码自己编译一个 Emscripten。运行下列命令来自动化地使用 Emscripten SDK。(在你想保存 Emscripten 的文件夹下运行)。</p>
...
<pre class="brush: bash notranslate"># on Linux or Mac macOS
source ./emsdk_env.sh
...
<p>现在环境配置完毕，让我们看看如何使用它把 C 代码编译到 Emscripten。当使用 Emscripten 来编译的时候有很多种不同的选择，我们介绍其中主要的 2 种：</p>
...
<p>我们先来看一个最简单的例子，通过这个，你可以使用 Emscripten 来将任何代码生成到 WebAssembly，然后在浏览器上运行。</p>
...
<ul>
 <li><code>-s WASM=1</code> — 指定我们想要的 wasm 输出形式。如果我们不指定这个选项，Emscripten 默认将只会生成<a href="http://asmjs.org/">asm.js</a>。</li>
...
<div class="note">
<p><strong>备注：</strong> Firefox 52+ 和 Chrome 57+ 和最新版本的 Opera 已经默认启用，你也可以在 Firefox 47+ 中通过在 <em>about:config</em> 页面启用 <code>javascript.options.wasm</code> 字段获得支持，Chrome 51+ 和 Opera 38+ 可以在 <em>chrome://flags 页面启用 Experimental WebAssembly 选项以支持 </em>WebAssembly<em>。</em></p>
</div>
...
<h2 id="使用自定义HTML模板">使用自定义 HTML 模板</h2>
...
cp ~/emsdk/emscripten/1.38.15/src/shell_minimal.html html_template</pre>
 ...
 <li>
  <p>现在使用你的 Emscripten 编译器环境的终端窗口进入你的新目录，然后运行下面的命令：</p>
 ...
 <li>
  <p>下面让我们来运行这个例子。上面的命令已经生成了 hello2.html，内容和我们使用的模板非常相像，只不过多加了一些 js 胶水和加载 wasm 文件的代码。 在浏览器中打开它，你会看到与上一个例子相同的输出。</p>
 </li>
</ol>
...
<h2 id="调用一个定义在C中的自定义方法">调用一个定义在 C 中的自定义方法</h2>
...
int EMSCRIPTEN_KEEPALIVE myFunction(int argc, char ** argv) {
 ...
 <li>
  <p>运行以下命令编译：(注意由于使用 ccall 函数，需要添加指定参数)</p>
