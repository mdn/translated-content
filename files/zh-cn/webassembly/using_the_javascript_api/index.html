...
<p>如果您已经使用 Emscripten 等工具编译了另一种语言的模块，或者自己加载并运行代码，那么下一步是了解如何使用 WebAssembly JavaScript API 的其他功能。这篇文章告诉你你需要知道什么。</p>
...
<p>让我们通过一步一步的例子来了解如何在 WebAssembly 中使用 Javascript API，和如何在网页中加载一个 wasm 模块。</p>

<div class="note">
<p><strong>备注：</strong> 你可以发现同样的代码在 <a href="https://github.com/mdn/webassembly-examples">webassembly-examples</a> GitHub 仓库。</p>
</div>
...
<ol>
 ...
 <li>然后，创建一个简单的 HTML 文件命名为 index.html 和并且你的本机的 wasm 文件处于同一目录下 ( 如果你没有模板可以使用我们提供的 <a href="https://github.com/mdn/webassembly-examples/blob/master/template/template.html">simple template</a> ).</li>
 <li>现在，为了帮助我们理解发生了什么，让我们来看看这个 wasm 模块的文本表示 (也可以在<a href="/zh-CN/docs/WebAssembly/Text_format_to_wasm#初识文本格式">将 WebAssembly 文本格式转换为 wasm</a>见到):
...
<h3 id="在开发者工具查看wasm">在开发者工具查看 wasm </h3>
...
<p>从 Firfox 开始，除了将 WebAssembly 视为文本，开发者可以使用文本格式调试 (打断点，检查调用堆栈，单步调试等等.) WebAssembly 代码。通过这个视频<a href="https://www.youtube.com/watch?v=R1WtBkMeGds">使用 Firefox 开发者工具调试 WebAssembly</a>预览。</p>

<h2 id="内存">内存</h2>

<p>在 WebAssembly 的底层内存模型中，内存被表示为称为 <a href="http://webassembly.org/docs/semantics/#linear-memory">线性内存</a> 的无类型字节的连续范围，通过模块中的<a href="http://webassembly.org/docs/semantics/#linear-memory-accesses">加载和存储指令</a>读取和写入。  在这个内存模型中， 任何加载或存储都可以访问整个线性存储器中的任何字节，这是忠实地表示 C / C ++概念（如指针）所必需的。</p>

<p>然后，和原生 C/C++ 程序不同的是可用内存范围跨越整个进程，特定 WebAssembly 实例可访问的内存被限制在由 WebAssembly Memory 对象包含的一个特定的 —— 可能非常小的范围内。</p>

<p>在 JavaScript 中，内存实例可以被认为是可调整大小的 ArrayBuffer，就像 ArrayBuffers 一样，一个 Web 应用程序可以创建许多独立的内存对象。您可以使用 {{jsxref("WebAssembly.Memory()")}} 构造函数创建一个，它将参数作为初始大小和（可选）最大大小）。</p>
...
<ol>
 ...
 <li>
  ...
  <p>初始和最大的单位是 WebAssembly pages ——这些页面的大小固定为 64KB。这意味着上述内存实例的初始大小为 640KB，最大大小为 6.4MB。</p>

  <p>WebAssembly 内存通过简单地提供一个返回 ArrayBuffer 的缓冲区 getter / setter 来显示它的字节。例如，要直接将 42 写入线性内存的第一个单词，你可以这样做：</p>
 ...
 <li>
  <p>现在尝试这个演示 —— 保存目前为止添加的内容，将其加载到浏览器中，然后尝试在 JavaScript 控制台中输入上述两行。</p>
 </li>
</ol>
...
<p>Note: 由于 {{domxref("ArrayBuffer")}} 的 byteLength 是不可变的，所以在成功 {{jsxref("Memory.prototype.grow()")}} 操作之后，缓冲区 getter 将返回一个新的 ArrayBuffer 对象 新的 byteLength）和任何先前的 ArrayBuffer 对象变成“分离”，或者与先前指向的底层内存断开连接。</p>

<p>和函数一样，线性内存可以在模块内部进行定义或者导入。类似地，模块还可以可选地导出其内存。这这意味着 JavaScript 可以通过创建<code>new WebAssembly.Memory</code> 并将其作为导入或通过接收内存导出传递给 WebAssembly 实例的内存来访问 (通过 <code><a href="/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/WebAssembly/Instance/exports">Instance.prototype.exports</a></code>).</p>

<h3 id="更复杂的内存示例">更复杂的内存示例</h3>

<p>让我们通过看一个更复杂的内存示例——一个对整数数组进行求和的 WebAssembly 模块——来明确上面的概念。你可以在这里<a href="https://github.com/mdn/webassembly-examples/raw/master/js-api-examples/memory.wasm">memory.wasm</a>找到示例。</p>

<ol>
 <li>
  <p>像前面那样在相同的目录下复制一份 memory.wasm。</p>

  <div class="note">
  <p><strong>备注：</strong> 你可以在这里<a href="https://github.com/mdn/webassembly-examples/blob/master/js-api-examples/memory.wat">memory.wat</a>找到模块的文本表示形式。</p>
  </div>
 </li>
 <li>
  <p>回到你的示例文件 memory.html，像前面那样获取、编译和实例化你的 wasm 模块——在你的脚本代码底部加入下面的代码：</p>
 ...
 <li>
  <p>因为该模块导出了它的内存，给定该模块的一个实例，我们可以使用一个导出函数 accumulate() 在该模块实例的线性内存（mem）中创建和填入一个输入数组。在前面指明的地方加入如下代码：</p>
...
<p>WebAssembly 表格是一个可变大小的带类型的引用数组，其中的引用可以被 JavaScript 和 WebAssembly 代码存取。然而，内存提供的是一个可变大小的带类型的原始字节数组。所以，把引用存储在内存中是不安全。由于安全、可移植性和稳定性等原因，作为引擎信任的引用值是千万不能被直接读写的。</p>
...
<p>表格可以通过<a href="/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/WebAssembly/Table/set">Table.prototype.set()</a>和 <a href="/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/WebAssembly/Table/grow">Table.prototype.grow()</a>进行更改，它们会更新表格中的一个值和增加可以存储在表格的大小。这允许间接可调用函数集合可以随着时间而改变，其对于<a href="http://webassembly.org/docs/dynamic-linking/">动态链接技术</a>来说是必要的。这些更改对于 JavaScript 和 wasm 模块来说是立即生效的。同时，在 JavaScript 可以通过<a href="/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/WebAssembly/Table/get">Table.prototype.get()</a>得到最新值。</p>

<h3 id="表格示例">表格示例</h3>

<p>让我们看一个简单的表格示例——一个 WebAssembly 模块，该模块创建并导出了一个带有两个元素的表格：元素 0 返回 13，元素 1 返回 42。你可以在<a href="https://github.com/mdn/webassembly-examples/raw/master/js-api-examples/table.wasm">table.wasm</a>中找到该示例。</p>

<ol>
 <li>
  <p>在一个新的目录中复制一份 table.wasm。</p>

  <div class="note">
  <p><strong>备注：</strong> 你可以在<a href="https://github.com/mdn/webassembly-examples/blob/master/js-api-examples/table.wat">table.wat</a>中查看模块的文本表示。</p>
  </div>
 </li>
 <li>
 ...
 <li>
  <p>如前所示，获取、编译并且实例化你的 wasm 模块——将下面的代码放入到 HTML body 底部的<a href="/zh-CN/docs/Web/HTML/Element/script">&lt;script&gt;</a>节点里面：</p>
...
<p>这段代码获取获取了存储在表格中的每一个函数引用，然后实例化它们从而将它们拥有的值打印到控制台——注意每一个函数引用是如何使用<a href="/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/WebAssembly/Table/get">Table.prototype.get()</a>函数获取的以及在其后面增加一对小括号从而真正的调用该函数。</p>
...
<p>现在，我们已经展示了 WebAssembly 的主要组成模块的使用，这里是提到多样性概念的好地方。这为 WebAssembly 提供了大量的关于架构效率的优势：</p>

<ul>
 <li>一个模块可以有 N 个实例，这与一个函数可以产生 N 个闭包值一样。</li>
 <li>一个模块实例可以使用 0-1 个内存实例，它为这个实例提供了“地址空间”。将来的 WebAssembly 版本可能允许每个模块实例拥有 0-N 个内存实例（参考<a href="http://webassembly.org/docs/future-features/#multiple-tables-and-memories">多表格与内存</a>）。</li>
 <li>一个模块实例可以使用 0-1 个表格实例——这是该实例的“函数地址空间”，可以用来实现 C 函数指针。将来的 WebAssembly 版本可能允许每个模块实例拥有 0-N 个表格实例。</li>
 <li>一个内存或表格实例能够被 0-N 个模块实例使用——这些实例全部共享相同的地址空间，这使得<a href="http://webassembly.org/docs/dynamic-linking">动态链接</a>成为可能。</li>
</ul>
...
<p>本文带你了解了使用 WebAssembly 的 JavaScript API 的基本知识，包括在 JavaScript 上下文中导入一个 WebAssembly 模块、使用该模块的函数以及在 JavaScript 中使用 WebAssembly 的内存和表格。同时，我们也介绍了多样性的概念。</p>
