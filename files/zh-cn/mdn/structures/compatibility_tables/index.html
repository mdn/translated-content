...
<p><strong>警告：</strong> <em>数据的生成方式已经发生了变更</em>。过去，我们的表格直接嵌入在页面中，而且数据是手动填写的。这样效率很低，难以维护，而且使得数据不够灵活，不便更新。所以我们正在把我们的兼容性表格迁移到一个数据 repo 中（<a href="https://github.com/mdn/browser-compat-data">https://github.com/mdn/browser-compat-data</a>）并且改为使用程序化的方式生成它。<br>
 <br>
...
<div class="note">
<p><strong>备注：</strong> 如果您需要本指南任何步骤的帮助，欢迎您在<a href="https://discourse.mozilla-community.org/c/mdn">MDN 论坛</a>上与我们联系。</p>
</div>
...
<div class="note">
<p><strong>备注：</strong> 您会留意到，该仓库还包含了<a href="/en-US/Add-ons/WebExtensions">浏览器拓展</a>和<a href="/en-US/docs/Web/HTTP">HTTP</a><strong>的数据。</strong>These data sets are basically finished as they stand, but more features may need to be added in the future.</p>
</div>
...
<div class="note">
<p><strong>备注：</strong> You should put the most current or important support point first in the array — this makes the data easier to read for people who just want to scan it for the latest info.</p>
</div>
...
<div class="note">
<p><strong>备注：</strong> The alternative name might not be an exact alias — it might have differing behaviour to the standard version.</p>
</div>
...
<div class="note">
<p><strong>备注：</strong> 文件名通常与给予 JSON 结构内的接口的标签相匹配，但事实并非总是如此。当宏调用生成表时，他们遍历所有文件，直到找到相关的 JSON 使用，所以文件名不是关键。说到这一点，你应该始终尽可能直观地命名它们。</p>
</div>
