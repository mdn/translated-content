...
在报告议题或参与议题讨论时，确保你的内容有助于项目的整体发展。考虑你所创建的议题和你在议题中的评论是否具有建设性、是否与主题相关，而不仅仅是添加噪音。
...
如果你认为你发现了 MDN Web 文档内容或网站外观和感官的错误，请搜索[相关的仓库](/zh-CN/docs/MDN/Community/Contributing/Our_repositories)中当前未解决的议题，确保没有其他人报告该议题。

### 报告议题

- 根据你发现的问题类型，在以下仓库中的一个创建议题：
...
1. **查找议题**：如果你想要做出贡献，请搜索带有 [`good first issue`、`help wanted`](#设置其他标签) 或 [`p3`](#设置优先级标签) 标签的议题。大多数仓库都有带有这些标签的议题。你可以浏览并选择适合你的技能的议题。另一个寻找议题的有用方法是 [MDN 贡献者任务板](https://github.com/orgs/mdn/projects/25)。该项目的视图列出了多个仓库中的未解决的议题。你可以根据你感兴趣的主题（`label` 列）来筛选列表。请参阅[议题分类过程](#议题分类过程)中应用的一些[标签](#设置其他标签)的描述。
...
- 如果议题设计的活动并不是 en-US 区域的，请设置适当的标签（例如，`l10n-fr`、`l10n-zh` 或 `l10n-ja`）。这些区域的团队将会接手这些议题并对其进行分类。
...
- 评估修复的影响是较小的还是整个网站的。
- 评估议题的修复是否需要讨论，如果是，请让作者先发起[讨论](https://github.com/orgs/mdn/discussions)。
...
- 主要议题：此类议题可能会严重影响页面的实用性。例如，大量过时的信息、不起作用的复杂而重要的代码示例、糟糕且难以理解的大量行文，或大量损坏的链接。
