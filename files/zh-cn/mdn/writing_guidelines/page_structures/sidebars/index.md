...
页面的侧边栏是通过创建一个 ID 为“Quick_links”的 {{HTMLElement("section")}} 块来提供的。然后，将要放入侧边栏框中的内容放在 section 内。这些链接需要格式化为 {{HTMLElement("ol")}} 有序列表（可以嵌套）。你可以通过使用编辑器工具栏中的编号列表按钮来实现这一点。例如，你的侧边栏 HTML 可能如下所示：
...
你的宏可以是简单或复杂的（若有必要）；它需要输出类似于上面的[侧边栏语法](#侧边栏语法)中所示的 HTML。
