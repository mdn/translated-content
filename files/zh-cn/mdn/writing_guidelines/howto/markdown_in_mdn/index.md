...
有时，即使可以使用 GFM 编写表格，也应该使用 HTML。因为 GFM 使用“{{Glossary("ASCII")}} 艺术”来实现表格，当表格的一行变得过长时，将变得难以阅读。例如，考虑以下表格：
...
- 对于幂指数，请使用脱字符：`2^53`。
- 对于像 1<sup>st</sup> 这样的序数表达式，推荐使用“第一”等词语。
- 对于脚注，请勿标记脚注引用，如 `<sup>[1]</sup>`。
