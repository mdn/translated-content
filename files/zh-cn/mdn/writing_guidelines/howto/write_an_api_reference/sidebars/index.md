...
5. `"events"`——该值是一个数组，其中应包含与 API 相关的所有事件，这些事件在 API 规范或其他地方定义。如果有大量的事件，你可能需要考虑只列出最受欢迎的事件，或者将它们放在列表的前面。“animationstart”会创建如下链接：[https://developer.mozilla.org/zh-CN/docs/Web/Events/animationstart](/zh-CN/docs/Web/Events/animationstart)。
6. `"guides"`——该值是一个数组，其中应包含一个或多个对象，这些对象定义了指南链接，用于解释如何使用 API。每个对象包含两个子成员——“url”，其中包含指向指南文章的部分 URL；以及“title”，它定义了链接文本。例如，以下对象：
   ...
   创建一个标题为“Detecting device orientation”的链接，该链接指向 [https://developer.mozilla.org/zh-CN/docs/Web/API/Detecting_device_orientation](/zh-CN/docs/Web/API/Detecting_device_orientation)。
