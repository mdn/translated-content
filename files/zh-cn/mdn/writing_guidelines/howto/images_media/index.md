...
要在文档中添加图像，请将图像文件添加到该文档的文件夹中，然后在文档的 `index.md` 文件中使用 `<img>` 元素或[等效的 Markdown 语法](https://github.github.com/gfm/#images)引用该图像。
...
3. 运行 `filecheck` 命令检查每个图像，如果有问题可能会报错。请参阅[压缩图像](#压缩图像)部分了解详细信息。
...
4. 在文档中使用 `<img>` 元素和 `alt` 属性或等效的 Markdown 语法引用图像，例如 `files/en-us/web/css/index.md`：
...
5. 暂存并提交所有已删除、已创建和已修改的文件，然后将你的分支推送到你复刻的仓库：
...
对于每一张图像（`![]` 和 `<img>`），都必须包含替代（`alt`）文本。替代文本应该简短，并提供图像传达的所有相关信息。在编写图像描述时，请考虑图像有价值的信息以及如何向那些可以阅读页面内容但无法加载图像的人传达这些信息。

请确保图像的替代文本基于其上下文。如果狗狗 Fluffy 的照片用作了 Yuckymeat 狗粮的评价旁边的头像，则 `alt="狗狗 Fluffy"` 是合适的。如果相同的照片是 Fluffy 的动物救援领养页面的一部分，则图像传达的信息对于潜在的狗狗认养者来说是相关的，例如 `alt="狗狗 Fluffy，三色条纹，毛发非常短，嘴里叼着一个网球。"`。周围的文本可能有 Fluffy 的大小和品种，因此包含这些信息将是多余的。请勿过多地描述图像：潜在的认养者不需要知道狗是在室内还是室外，也不需要知道它是否有红色的项圈和蓝色的牵引绳。

对于屏幕截图，请写出你从图像中了解到的内容，而不是详细描述屏幕截图的内容，并省略读者不需要或已经知道的信息。例如，如果你在一个关于在必应更改设置的页面上，且你有一个必应搜索结果的屏幕截图，请不要包含搜索词或结果数量等信息，因为这些不是图像的重点。将 `alt` 限制为目前的主题：如何在必应中更改设置。替代文本可能是 `alt="设置图标位于搜索字段下方的导航栏中。"`。不要包含“屏幕截图”或“必应”，用户不需要知道它是屏幕截图，并且已经知道这是必应，因为他们在一个解释如何更改必应设置的页面上。
...
在描述某种难以用语言简介描述的指令序列或多步骤工作流时，我们倾向于使用视频：_“这样做，然后那样做，然后这样就会发生”_。当试图描述跨越多个应用程序或窗口的过程并且包括可能难以描述的 GUI 交互时，它尤其有用：_“现在点击左上角附近的看起来有点像鸭子的按钮”_。
...
- **短的**：尽量将视频时长控制在 30 秒以内，最好是在 20 秒以内。足够短的内容不会要求读者保持注意力的高度集中。
- **简单的**：尽量使工作流程简单，2-4 个不同的步骤。这使得它们更容易跟随。
...
> **备注：** 如果视频脱离了页面的上下文而没有意义（如果是一个短视频，那么它可能没有意义），请将其标记为“未列出”（unlisted）。
...
宏调用所接受的单个属性是视频 URL 末尾的字符字符串，而不是整个 URL。例如，如果视频 URL 是 `https://www.youtube.com/watch?v=ELS2OOUvxIw`，则所需的宏调用将是：
