...
MDN Web Docs 的文档内容是通用的，并不偏向某一个浏览器。这使得 Web 开发人员可以编写与任何浏览器都兼容的代码。本文介绍了在 MDN Web Docs 上应该包含哪些主题或类型的内容的有关信息。

## 编辑准则

本节描述了 Mozilla MDN 员工为管理 MDN Web Docs 上的内容而设置的准则。MDN Web Docs 的所有贡献者都应遵守这些政策。

### 关联性

MDN Web Docs 上的所有内容都必须与其出现的技术相关。网站绝不会接受垃圾信息（商业广告）和其他不相关的内容。继续尝试提交垃圾信息的贡献者可能会被 Mozilla MDN 工作人员酌情禁止访问 MDN。

与主题相关的商业网站的站外链接将根据具体情况进行判断。他们在帮助 Web 开发人员方面的价值必须超过其商业利益价值。

### 中立性

MDN Web Docs 上的文章必须保持[中立的观点](https://zh.wikipedia.org/wiki/Wikipedia:中立的观点)，在没有编辑偏见的情况下报告浏览器的变化。不接受对任何浏览器或用户代理的贬损评论。

### 开放性

MDN Web Docs 上记录的 Web 技术应该朝着标准方向发展，并且必须至少有一个渲染引擎实现了它。浏览器支持的变化会记录在文章的浏览器兼容性部分。

## 内容建议

如果你想为 MDN Web Docs 提供内容建议，请务必在提交之前阅读此页面以确保你的建议是合适的。
...
换句话说，要在 MDN Web Docs 上记录的 Web 技术应该满足以下条件：
...
MDN Web Docs 上的所有内容都必须与其出现的技术部分相关。贡献者应遵循这些 MDN 的编写风格、代码示例以及一些其他主题的[撰写指南](/zh-CN/docs/MDN/Writing_guidelines)。

有关技术是否可以记录在 MDN Web Docs 上的标准的更多详细信息，请参阅[包含页面的标准](/zh-CN/docs/MDN/Writing_guidelines/What_we_write/Criteria_for_inclusion)。

### 当我们记录一项新技术时

在 MDN Web Docs 上，我们一直在努力寻找合适的文档记录新的 Web 标准技术。我们力求在尽早发布文档以便开发人员可以及时了解新功能，同时也需要在足够晚的时间发布文档以取得平衡，使技术成熟稳定，从而文档不需要频繁更新或快速地被移除。
...
- 至少一个浏览器中已经始终如一地实现了该规范，并且其他浏览器开发者也表现出了兴趣（例如，已经存在一个有效的 ticket 或“意图实现”流程正在进行中）。
...
- MDN Web Docs 的这部分内容并不聚焦于开放式 Web 技术，并且是一种维护负担。
...
一般情况下，MDN 是*产品*文档，而不是*项目*或记录过程的文档。因此，如果文档是关于“如何使用一项技术”或“一项技术是如何工作的”（这项技术在上面提到的一个主题类别中）那么它可以被放在 MDN 上。
