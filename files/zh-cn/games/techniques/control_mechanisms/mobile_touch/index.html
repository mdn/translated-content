...
<p><strong>说明</strong>：游戏 <a href="http://rogers2.enclavegames.com/demo/">Captain Rogers: Battle at Andromeda</a> 是基于<a href="http://phaser.io/">Phaser</a> 和 Phaser-based 管理控制，但它也可以用纯 JavaScript 实现。使用 Phaser 的好处它提供了辅助变量和方法可以直接调用，有助于快速的开发游戏，这需要根据项目实际情况选择。</p>

<h2 id="纯_JavaScript_方式实现">纯 JavaScript 方式实现</h2>

<p>我们可以实现自己的触摸事件 — 给 document 添加事件监听，并传入自定义功能的方法，非常简单：</p>
...
<div class="note">
<p><strong>备注：</strong> 这篇 <a href="/en-US/docs/Web/API/Touch_events">touch events</a> 参考文章提供了更多的实例和介绍。</p>
</div>

<h3 id="纯JavaScript示例">纯 JavaScript 示例</h3>

<p>这个实现了移动端触摸的<a href="https://github.com/end3r/JavaScript-Game-Controls/">little demo</a>代码已经放到了 GibHub 上，我们下载这个示例就可以实现在移动端屏幕上移动飞船。</p>
