...
正如你所见，我们再一次使用了 {{domxref("CanvasRenderingContext2D.beginPath()","beginPath()")}} 方法 和 {{domxref("CanvasRenderingContext2D.closePath()","closePath()")}} 方法；在这两个方法中间，最重要的部分是 {{domxref("CanvasRenderingContext2D.arc()","arc()")}} 方法。它需要六个参数：
