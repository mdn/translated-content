...
<div>
<p>这是<a href="/en-US/docs/Games/Workflows/2D_Breakout_game_Phaser">Gamedev Phaser 教程</a> 16 的<strong>第 14 步</strong>。您可以在<a href="https://github.com/end3r/Gamedev-Phaser-Content-Kit/blob/gh-pages/demos/lesson14.html">Gamedev-Phaser-Content-Kit / demos / lesson14.html</a>完成本课程后找到源代码。</p>
</div>

<p>为了使游戏看起来更加多汁和活泼，我们可以使用动画和补间。这将导致更好，更有趣的体验。让我们来探讨如何在游戏中实现 Phaser 动画和补间。</p>

<h2 id="动画">动画</h2>

<p>在 Phaser，动画中，涉及从外部来源获取 spritesheet 并依次显示 sprites。作为一个例子，当碰到一些东西时，我们会让球摇摆。</p>

<p>首先，<a href="https://github.com/end3r/Gamedev-Phaser-Content-Kit/blob/gh-pages/demos/img/wobble.png">从 Github 抓取 spritesheet</a>并将其保存在您的<code>/img</code>目录中。</p>
...
<p>而不是加载单个图像的球，我们可以加载整个 spritesheet - 不同图像的集合。我们将按顺序显示精灵，创造动画的幻觉。该<code>spritesheet()</code>方法的两个额外的表格确定给定 spritesheet 文件中每个单个框架的宽度和高度，指示程序如何切割以获取单个框架。</p>

<h2 id="加载动画">加载动画</h2>

<p>接下来，进入你的 create() 函数，找到加载球精灵的行，下面的调用<code>animations.add()</code>如下所示：</p>
...
<p>这个补间将使用弹性宽松在半秒内将砖的比例翻倍，将自动启动，延迟 100 毫秒。</p>
...
<p>动画和 tweens 看起来很不错，但我们可以添加更多的我们的游戏 - 在下一节我们将看看处理<a href="/en-US/docs/Games/Workflows/2D_Breakout_game_Phaser/Buttons">按钮</a>输入。</p>
