...
<div>
<p>这是<a href="/en-US/docs/Games/Workflows/2D_Breakout_game_Phaser">Gamedev Phaser 教程</a> 16 的<strong>第 7 步</strong>。您可以在<a href="https://github.com/end3r/Gamedev-Phaser-Content-Kit/blob/gh-pages/demos/lesson07.html">Gamedev-Phaser-Content-Kit / demos / lesson07.html</a>完成本课程后找到源代码。</p>
</div>

<p>我们有球从墙上移动并弹跳，但它很快变得无聊 - 没有互动！我们需要一种介绍游戏的方法，所以在这篇文章中，我们将创建一个桨来移动并击中球。</p>
...
<p>首先，添加<code>paddle</code>我们将在我们的游戏中使用的<code>ball</code>变量，就在变量之后：</p>
...
<h3 id="添加桨图形">添加桨图形</h3>

<p>所以我们不要忘记，在这一点上，你应该从 Github 抓住这个<a href="https://github.com/end3r/Gamedev-Phaser-Content-Kit/blob/gh-pages/demos/img/paddle.png">图形</a>，并保存在你的<code>/img</code>文件夹中。</p>

<h3 id="渲染桨用物理引擎">渲染桨用物理引擎</h3>

<p>接下来，我们将通过<code>add.sprite()</code>在<code>create()</code>函数中添加以下调用来初始化我们的桨，将其添加到底部：</p>
...
<p>接下来的问题是我们不能移动桨。要做到这一点，我们可以使用系统的默认输入（鼠标或触摸，取决于平台），并将桨位置设置到位置的<code>input</code>位置。将以下新行添加到<code>update()</code>函数中，如下所示：</p>
...
<p>如果您还没有这样做，请重新加载<code>index.html</code>并尝试！</p>

<h2 id="定位球">定位球</h2>

<p>我们有桨按预期工作，所以我们把球放在上面。它非常类似于定位桨 - 我们需要将其放置在屏幕中间水平和垂直的底部，与底部有一点偏移。要按照我们想要的方式放置它，我们将把锚定位到球的正中间。找到现有的行，并将其替换为以下两行：<code>ball = game.add.</code><code>sprite( ... )</code></p>
...
<p>我们可以移动桨，并将球反弹，但是如果球从屏幕的底部边缘反弹，那又有什么意义？我们来介绍丢失的可能性 - 也称为<a href="/en-US/docs/Games/Workflows/2D_Breakout_game_Phaser/Game_over">游戏</a>逻辑。</p>
