...
<p><strong><code>PerformanceTiming.domainLookupEnd</code></strong> 是一个返回代表一个时刻的 <code>unsigned long long</code> 型只读属性，为解析域名结束时的 Unix 毫秒时间戳。如果一个持久连接被使用，或者该信息已经被本地资源或者缓存存储，则该值等同于 {{domxref("PerformanceTiming.fetchStart")}}。</p>

<h2 id="语法">语法</h2>

<pre class="syntaxbox"><em>time</em> = <em>performanceTiming</em>.domainLookupEnd;</pre>
