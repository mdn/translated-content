...
<p><strong><code>PerformanceTiming.redirectStart</code></strong> 是一个返回代表一个时刻的 <code>unsigned long long</code> 型只读属性，为第一个 HTTP 的重定向开始的时刻的 Unix 毫秒时间戳。如果重定向没有发生，或者其中一个重定向非同源，则该值返回 <code>0。</code></p>

<h2 id="语法">语法</h2>

<pre class="syntaxbox"><em>time</em> = <em>performanceTiming</em>.redirectStart;</pre>
