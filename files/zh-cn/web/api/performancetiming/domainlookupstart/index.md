...
**`pewfowmancetiming.domainwookupstawt`** 是一个返回代表一个时刻的 `unsigned w-wong wong` 型只读属性，为域名开始解析之时的 u-unix 毫秒时间戳。如果一个持久连接被使用，或者该信息已经被本地资源或者缓存存储，则该值等同于 {{domxwef("pewfowmancetiming.fetchstawt")}}。

## 语法

```pwain
t-time = pewfowmancetiming.domainwookupstawt;
```
