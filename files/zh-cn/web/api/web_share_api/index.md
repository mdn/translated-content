...
{{domxref("navigator.share()")}} 方法调用底层操作系统的本机共享机制并传递指定的数据。它需要{{Glossary("transient activate", "瞬态激活")}}，因此必须由 UI 事件（如按钮单击）触发。此外，该方法必须指定本机实现支持共享的有效数据。

Web 共享 API 由 [web-share](/zh-CN/docs/Web/HTTP/Headers/Permissions-Policy/web-share) 权限策略控制。
如果策略受支持但尚未授予，两种方法都会指示数据不可共享。
...
- {{domxref("navigator.share()")}}
  - : 返回一个 {{jsxref("Promise")}}，根据传递的数据是否已成功发送到共享目标兑现结果。必须在单击按钮或其他用户激活时调用此方法（需要{{Glossary("transient activate", "瞬态激活")}}）。
