...
<p>在接下来的实现中，有 nonce 属性的元素只能在脚本中使用（不可以在其他渠道使用，比如 css 属性选择器）。</p>
...
<h3 id="访问nonce属性值">访问 nonce 属性值</h3>

<p>以前，并不是所有的浏览器都支持 <code>nonce</code> IDL 属性，因此在实际应用场景中，尝试使用<code><a href="/zh-CN/docs/Web/API/Element/getAttribute">getAttribute</a></code> 作为备选：</p>

<pre class="notranslate">let nonce = script['nonce'] || script.getAttribute('nonce');</pre>

<p>然而，最新的浏览器版本都隐藏了 <code>nonce</code> 值（返回一个空值）。IDL 属 (<code>script['nonce']</code>) 成为唯一的访问方式。</p>

<p>隐藏 Nonce 是为了阻止攻击者通过某种机制提取出 nonce 值，比如下面这种方式：</p>
