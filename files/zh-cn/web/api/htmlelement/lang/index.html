...
<p>该属性返回的语言代码（language code）被定义在 <a href="http://tools.ietf.org/html/rfc1766">RFC 1766</a>。通常，"en" 表示英语（English）、"ja" 表示（Japanese）、"zh-cn" 表示简体中文等等。该属性的默认值未知（<code>unknown</code>）。尽管该属性可以应用在单独的元素上，但是通常在文档的根元素（html）上指定。</p>
<p>该属性只对 <code>lang</code> 属性（attribute）有效，不适用于 <code>xml:lang</code>。</p>
<h2 id="Syntax">语法</h2>
<pre class="syntaxbox">var <var>languageUsed</var> = elementNodeReference.lang; // 获取 lang 值
elementNodeReference.lang = <var>NewLanguage</var>; // 为 lang 设置新值
</pre>
