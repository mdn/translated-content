...
当 IFrame 加载完成后，我们将为 {{domxref("MessageChannel.port1")}} 注册一个 {{domxref("MessagePort/message_event", "onmessage")}} 处理事件，并使用 {{domxref("window.postMessage")}} 方法将 {{domxref("MessageChannel.port2")}} 和一条信息传输到 IFrame。
