...
- {{domxref("HTMLOListElement.reversed")}}
  - : 一个布尔值，反映 [`reversed`](/zh-CN/docs/Web/HTML/Element/ol#reversed) 并定义是否序号是递减（即它的值是 `true`），还是递增（`false`）。
- {{domxref("HTMLOListElement.start")}}
  - : 一个 `long` 值，反映 [`start`](/zh-CN/docs/Web/HTML/Element/ol#start) 并定义列表第一个元素的开始序号的值。
- {{domxref("HTMLOListElement.type")}}

  - : 一个字符串，反映 [`type`](/zh-CN/docs/Web/HTML/Element/ol#type) 并定义要用于显示的标记种类，它可以具有以下的值：
...
- {{domxref("HTMLOListElement.compact")}} {{deprecated_inline}}
  - : 一个布尔值，指示应减小列表项之间的间距，此属性仅反映 [`compact`](/zh-CN/docs/Web/HTML/Element/ol#compact) 属性，不考虑现代页面中用于该行为的 {{cssxref("line-height")}} CSS 属性。
