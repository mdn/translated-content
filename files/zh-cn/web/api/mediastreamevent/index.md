...
- {{domxref("MediaStreamEvent.MediaStreamEvent()", "MediaStreamEvent()")}}
  - : 返回一个新的 `MediaStreamEvent`. 它需要两个参数，第一个为 {{jsxref("String")}} 代表事件的类型；第二个是一个 {{domxref("MediaStream")}} 集合。

## Methods

_一个 {{domxref("MediaStreamEvent")}} 作为一个 {{domxref("Event")}}, 这个事件也实现了这些属性_. _没有特定的 *{{domxref("MediaStreamEvent")}}* 方法。_
