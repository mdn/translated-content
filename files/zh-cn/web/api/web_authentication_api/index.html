...
<ol>
 <li><strong>服务器</strong> - WebAuthn API 旨在在服务器（也称为服务或<a href="https://en.wikipedia.org/wiki/Relying_party">依赖方</a>）上注册新的凭据，以供稍后在同一服务器上使用相同的凭据对用户进行身份验证。</li>
...
<ol>
 ...
 <li><strong>服务器验证数据并完成注册 </strong>- 最后，服务器需要执行一系列检查以确保注册完成且数据未被篡改。步骤包括：
  ...
  验证步骤的完整列表<a href="https://w3c.github.io/webauthn/#registering-a-new-credential">可以在 WebAuthn 规范中找到</a>。一旦验证成功，服务器将会把新的公钥与用户帐户相关联以供将来用户希望使用公钥进行身份验证时使用。</li>
</ol>
