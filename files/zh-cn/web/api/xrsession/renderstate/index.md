...
{{DOMxRef("XRSession")}} 对象的 **`renderState`** 只读属性返回描述了应如何渲染用户的环境的 {{DOMxRef("XRRenderState")}} 对象。所提供的信息包括渲染对象的最小和最大距离、在 `inline` 会话模式下渲染时应使用的垂直视场角，以及用于内联合成的 {{domxref("XRWebGLLayer")}}。
