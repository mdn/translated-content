...
- {{ domxref("XMLHttpRequestEventTarget.onloadstart") }}
  - : 当一个 HTTP 请求开始加载数据时调用，接受 [`loadstart`](/zh-CN/docs/Web/API/XMLHttpRequestEventTarget/loadstart_event) 对象作为参数。
- {{ domxref("XMLHttpRequestEventTarget.onprogress") }}
  - : 间歇调用该方法用来获取请求过程中的信息，接受 [`progress`](/zh-CN/docs/Web/API/XMLHttpRequestEventTarget/progress_event) 对象作为参数。
...
- {{ domxref("XMLHttpRequestEventTarget.onloadend") }}
  - : 当内容加载完成，不管失败与否，都会调用该方法，接受 [`loadend`](/zh-CN/docs/Web/API/XMLHttpRequestEventTarget/loadend_event) 对象作为参数。
