...
<div><code><strong>ANGLE_instanced_arrays</strong>是</code>属于 <a href="/zh-CN/docs/Web/API/WebGL_API">WebGL API </a><code>的一个扩展 API，</code>它允许多次绘制相同的对象或相似对象组，前提是它们共享相同的顶点数据、基本图形的个数和类型。</div>



<div>WebGL 的扩展都能使用{{domxref("WebGLRenderingContext.getExtension()")}} 这个方法。更多详细信息，请参考 <a href="/zh-CN/docs/Web/API/WebGL_API/Tutorial">WebGL tutorial</a><em>（WebGL 使用教程）里的</em><a href="/zh-CN/docs/Web/API/WebGL_API/Using_Extensions">Using Extensions</a> <em>（使用扩展）</em>。</div>



<div class="note">
<p><strong>实用性：</strong> 这个扩展仅仅能使用在 {{domxref("WebGLRenderingContext", "WebGL1", "", 1)}} 上下文中。在 {{domxref("WebGL2RenderingContext", "WebGL2", "", 1)}}，默认情况下这个扩展的在 WebGL2 的上下文中起作用，它的常量以及方法使用过程中没有“ANGEL”后缀。</p>

<p>尽管名字叫“ANGLE”，只要硬件支持，它可以运行在任意设备上，而不仅是在 Windows 上起作用。 "ANGLE"只是表明了这个扩展是被 ANGLE 这个库的作者书写的，并没有更多的含义。</p>
</div>
...
<dl>
 ...
 <dd>当这个常量在{{domxref("WebGLRenderingContext.getVertexAttrib()", "gl.getVertexAttrib()")}}中作为 pname 参数使用时，将返回一个用于实例渲染的频率因子{{domxref("GLint")}}。</dd>
</dl>
