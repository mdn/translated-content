...
<p>屏幕捕获 API 对现有的媒体捕获和流 API 进行了补充，让用户选择一个屏幕或屏幕的一部分（如一个窗口）作为媒体流进行捕获。然后，该流可以被记录或通过网络与他人共享。</p>

<h2 id="屏幕捕捉API的概念和用法">屏幕捕捉 API 的概念和用法</h2>

<p>屏幕捕捉 API 使用起来相对简单。其唯一的方法是{{domxref("MediaDevices.getDisplayMedia()")}}，它的任务是以捕获的形式要求用户选择一个屏幕或屏幕的一部分的{{domxref("MediaStream")}}。</p>
...
<p>屏幕捕捉 API 没有自己的任何接口，而是在现有的 {{domxref("MediaDevices")}} 接口上添加了一个方法。</p>
