...
[Notifications API](/zh-CN/docs/Web/API/Notifications_API) 的接口 `Notification` 用于想用户配置和显示桌面通知。
...
> **备注：** 在上面的示例中，我们生成了响应用户手势的的通知（点击按钮）。这是最好的实践——你不应该向用户发送它们不同意的通知——而且未来浏览器将明确的禁止未响应用户手势发出的通知。例如， Firefox 已经从版本 72 开始就这么做了。
