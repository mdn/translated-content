...
`address` 字段的值是从传递给 {{domxref("RTCIceCandidate.RTCIceCandidate", "RTCIceCandidate()")}} 构造函数的 `candidateInfo` 选项中设置的。你不能直接在对象中配置 `address` 的值，但如果`a-line`(属性行) 的格式正确，则其值会自动从对象的候选者 `a-line`(属性行) 中提取。
...
这里需要注意的是，虽然 WebRTC 不要求 {{domxref("RTCPeerConnection")}} 上的两个对等端知道彼此的真实 IP 地址，但 `RTCIceCandidate` 上的 `address` 属性*可能*会向用户暴露有关远程对等端来源的更多信息。IP 地址可以用于推断有关远程设备位置、网络拓扑等的信息。它也可以用于 [fingerprinting](/zh-CN/docs/Glossary/Fingerprinting) (浏览器指纹) 识别。
...
通过将 `iceTransportPolicy` 设置为 `"relay"`，任何主机候选者（IP 地址为对等端的 IP 地址的候选者）都将被排除在候选者池之外，任何其他不是中继候选者的候选者也将被排除在外。

## 使用说明

假设描述 ICE 候选者的 {{Glossary("SDP")}} `a-line`（属性行）：
