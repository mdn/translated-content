...
{{domxref("RTCIceCandidate")}} 方法 **`toJSON()`** 将其调用的 `RTCIceCandidate` 转换为 JSON 格式。通过在返回的对象上调用 {{jsxref("JSON.stringify", "stringify()")}} 来获取对象的字符串版本。
...
> **备注：** 返回的 JSON 对象的格式/属性，与传递给 {{domxref("RTCIceCandidate.RTCIceCandidate()","RTCIceCandidate(candidateInfo)")}} 造函数的配置候选者的 `candidateInfo` 对象相同。
