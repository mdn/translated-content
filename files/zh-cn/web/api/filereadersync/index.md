...
该方法可以读取指定的 [`Blob`](Blob) 或者 [`File`](File)对象中的内容。当读取完毕后，返回一个 [`ArrayBuffer`](/zh-CN/docs/JavaScript_typed_arrays/ArrayBuffer) 对象，里面包含了被读取文件的内容数据。如果在读取过程中发生了错误，则会抛出相关的异常。
...
一个 [`ArrayBuffer`](/zh-CN/docs/JavaScript_typed_arrays/ArrayBuffer) 对象，包含了被读取文件的内容。
