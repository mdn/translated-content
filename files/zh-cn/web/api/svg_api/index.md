...
1. [元素接口](#svg_element_interfaces) 提供对 SVG 元素属性的访问以及用于操作它们的方法。
2. [静态数据类型](#svg_data_type_interfaces) 接口提供对元素属性值的访问以及用于操作这些值的方法。
3. 对于可动画的属性，[动画数据类型接口](#svg_data_type_interfaces) 提供对当前动画属性的值的只读访问。
4. [同步多媒体集成语言（SMIL）接口](#smil-related_interfaces)
5. [其他](#other_svg_interfaces)
