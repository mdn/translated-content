...
<p><strong><code>MediaStreamTrack</code></strong> 接口在 User Agent 中表示一段媒体源，比如音轨或视频。</p>

<h2 id="属性">属性</h2>

<dl>
 <dt>{{domxref("MediaStreamTrack.enabled")}}</dt>
 <dd>布尔值，为 true 时表示轨道有效，并且可以被渲染。为 false 时表示轨道失效，只能被渲染为静音或黑屏。如果该轨道连接中断，该值还是可以被改变但不会有任何效果了。</dd>
 <dt>{{domxref("MediaStreamTrack.id")}} {{readonlyInline}}</dt>
 <dd>返回一个由浏览器产生的{{domxref("DOMString")}}类型的 GUID 值，作为这个轨道的唯一标识值。</dd>
 ...
 <dd>返回一个布尔类型的值，为 true 时表示轨道是静音，其它为 false。</dd>
 <dt>{{domxref("MediaStreamTrack.readonly")}} {{readonlyInline}}</dt>
 <dd>返回一个布尔类型的值，为 true 时表示该轨道是只读的，比如视频文件源或一个被设置为不能修改的摄像头源，或则为 false。</dd>
 <dt>{{domxref("MediaStreamTrack.readyState")}} {{readonlyInline}}</dt>
 <dd>返回枚举类型的值，表示轨道的当前状态。该枚举值为以下中的一个：
 <ul>
  <li>"live"表示当前输入已经连接并且在尽力提供实时数据。在这种情况下，输出数据可以通过操作 MediaStreamTrack.enabled 属性进行开关。</li>
  <li>“ended”表示这个输出连接没有更多的数据了，而且也不会提供更多的数据了。</li>
 </ul>
 </dd>
 <dt>{{domxref("MediaStreamTrack.remote")}} {{readonlyInline}}</dt>
 <dd>返回布尔值类型，当为 true 时表示数据是通过{{domxref("RTCPeerConnection")}}提供的，否则为 false。</dd>
</dl>
...
<dl>
 ...
 <dd>这是{{event("started")}}事件在这个对象上被触发时调用的事件处理器{{event("Event_handlers", "event handler")}}，这时一个新的{{domxref("MediaStreamTrack")}}对象被添加到轨道源上。</dd>
 <dt>{{domxref("MediaStreamTrack.onmute")}}</dt>
 <dd>这是{{event("mute")}}事件在这个对象被触发时调用的事件处理器{{event("Event_handlers", "event handler")}}，这时这个流被中断。</dd>
 <dt>{{domxref("MediaStreamTrack.onunmute")}}</dt>
 <dd>这是{{event("unmute")}}事件在这个对象上被触发时调用的事件处理器{{event("Event_handlers", "event handler")}}，未实现。</dd>
 ...
 <dd>这是{{event("ended_(MediaStream)", "ended")}}事件在这个对象被触发时调用的事件处理器{{event("Event_handlers", "event handler")}}，未实现。</dd>
</dl>
...
<ul>
 <li><a href="/en-US/docs/WebRTC/MediaStream_API">Using the MediaStream API</a></li>
</ul>
