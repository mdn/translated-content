...
<p>Service workers 本质上充当 Web 应用程序、浏览器与网络（可用时）之间的代理服务器。这个 API 旨在创建有效的离线体验，它会拦截网络请求并根据网络是否可用来采取适当的动作、更新来自服务器的的资源。它还提供入口以推送通知和访问后台同步 API。</p>

<h2 id="Service_worker_的概念和用法">Service worker 的概念和用法</h2>

<p>Service worker 是一个注册在指定源和路径下的事件驱动<a href="/zh-CN/docs/Web/API/Worker">worker</a>。它采用 JavaScript 控制关联的页面或者网站，拦截并修改访问和资源请求，细粒度地缓存资源。你可以完全控制应用在特定情形（最常见的情形是网络不可用）下的表现。</p>

<p>Service worker 运行在 worker 上下文，因此它不能访问 DOM。相对于驱动应用的主 JavaScript 线程，它运行在其他线程中，所以不会造成阻塞。它设计为完全异步，同步 API（如<a href="/zh-CN/docs/Web/API/XMLHttpRequest">XHR</a>和<a href="/zh-CN/docs/Web/Guide/API/DOM/Storage">localStorage</a>）不能在 service worker 中使用。</p>

<p>出于安全考量，Service workers 只能由 HTTPS 承载，毕竟修改网络请求的能力暴露给中间人攻击会非常危险。在 Firefox 浏览器的<a href="https://support.mozilla.org/zh-CN/kb/%E9%9A%90%E7%A7%81%E6%B5%8F%E8%A7%88">用户隐私模式</a>，Service Worker 不可用。</p>

<div class="note">
<p><strong>注意：</strong>Service workers 之所以优于以前同类尝试（如<a href="http://alistapart.com/article/application-cache-is-a-douchebag">AppCache</a>），是因为它们无法支持当操作出错时终止操作。Service workers 可以更细致地控制每一件事情。</p>
</div>

<div class="note">
<p><strong>注意：</strong>Service workers 大量使用<a href="/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Promise">Promise</a>，因为通常它们会等待响应后继续，并根据响应返回一个成功或者失败的操作。Promise 非常适合这种场景。</p>
</div>

<h3 id="注册">注册</h3>

<p>使用 {{domxref("ServiceWorkerContainer.register()")}} 方法首次注册 service worker。如果注册成功，service worker 就会被下载到客户端并尝试安装或激活（见下文），这将作用于整个域内用户可访问的 URL，或者其特定子集。</p>
...
<p>你可以监听{{domxref("InstallEvent")}}，事件触发时的标准行为是准备 service worker 用于使用，例如使用内建的 storage API 来创建缓存，并且放置应用离线时所需资源。</p>

<p>还有一个 activate 事件，触发时可以清理旧缓存和旧的 service worker 关联的东西。</p>

<p>Servcie worker 可以通过 {{domxref("FetchEvent")}} 事件去响应请求。通过使用 {{domxref("FetchEvent.respondWith") }} 方法，你可以任意修改对于这些请求的响应。</p>
...
<p>Service workers 也可以用来做这些事情：</p>
...
<ul>
 <li><a href="https://github.com/slightlyoff/BackgroundSync">后台同步</a>：启动一个 service worker 即使没有用户访问特定站点，也可以更新缓存</li>
...
<dl>
 ...
 <dd>表示 service worker client 的作用域。一个 service worker client 可以是浏览器上下文的一个文档，也可以是一个由活动 worker 控制的{{domxref("SharedWorker")}}。</dd>
 <dt>{{domxref("Clients") }} {{Experimental_Inline}}</dt>
 <dd>表示一个{{domxref("Client")}}对象容器，是访问当前源的活动 service worker clients 的主要途径。</dd>
 <dt>{{domxref("ExtendableEvent") }} {{Experimental_Inline}}</dt>
 <dd>扩展被分发到{{domxref("ServiceWorkerGlobalScope")}}的<code>install 和 activate</code>事件时序，作为 service worker 生命周期的一部分。这会确保任何功能型事件（如{{domxref("FetchEvent")}}）不被分发到{{domxref("ServiceWorker")}}，直到它更新了数据库架构、删除过期缓存项等等以后。</dd>
 ...
 <dd>传递给{{domxref("ServiceWorkerGlobalScope.onfetch")}}处理函数的参数，FetchEvent 代表一个在{{domxref("ServiceWorker")}}的{{domxref("ServiceWorkerGlobalScope")}}中分发的请求动作。它包含关于请求和响应的结果信息，并且提供{{domxref("FetchEvent.respondWith", "FetchEvent.respondWith()")}}方法，这个方法允许我们提供任意的响应返回到控制页面。</dd>
 <dt>{{domxref("InstallEvent") }}{{Experimental_Inline}}</dt>
 <dd>传递给 {{domxref("ServiceWorkerGlobalScope.oninstall", "oninstall")}} 处理函数的参数，<code>InstallEvent</code> 接口代表一个在 {{domxref("ServiceWorker")}} 的 {{domxref("ServiceWorkerGlobalScope")}} 中分发的安装动作，作为 {{domxref("ExtendableEvent")}} 的子事件，它保证诸如 {{domxref("FetchEvent")}} 的功能性事件在安装过程中不会被分发。</dd>
...
<dl>
 ...
 <dd>表示一个 service worker。多个浏览的上下文 (例如 pages、workers 等等) 都能通过相同的 <code>ServiceWorker</code> 对象相关联。</dd>
 <dt>{{domxref("ServiceWorkerContainer") }} {{Experimental_Inline}}</dt>
 <dd>提供一个在网络生态中把 service worker 作为一个整体的对象，包括辅助注册，反注册以及更新服务工作者，并且访问 service worker 的状态以及他们的注册信息。</dd>
 <dt>{{domxref("ServiceWorkerGlobalScope") }}</dt>
 <dd>表示 service worker 的全局执行上下文。</dd>
 ...
 <dd>表示 service worker 的注册。</dd>
 <dt>{{DOMxRef("ServiceWorkerState")}} {{Experimental_Inline}}</dt>
 <dd>Associated with its <a href="https://developer.mozilla.org/en-US/docs/Web/API/ServiceWorker"><code>ServiceWorker</code></a>'s state.</dd>
 <dt>{{domxref("SyncEvent")}} {{non-standard_inline}}</dt>
 <dd>传递给同步函数的参数，SyncEvent 接口代表在 ServiceWorker 里{{domxref("ServiceWorkerGlobalScope")}}分发的同步动作。</dd>
 ...
 <dd>表示在浏览器上下文中记录的 service worker 客户端的作用域，被活动的工作者控制。是{{domxref("Client")}}对象的特殊类型，包含一些附加的方法和可用的属性。</dd>
</dl>
