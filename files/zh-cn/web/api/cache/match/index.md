...
- options {{optional_inline}}
  - : 一个为 `match` 操作设置选项的对象。有效的选项如下：
    - `ignoreSearch`: 一个 {{jsxref("Boolean")}} 值用来设置是否忽略 url 中的 query 部分。例如，如果该参数设置为 `true` ，那么 `http://foo.com/?value=bar` 中的 `?value=bar` 部分就会在匹配中被忽略。该选项默认为 `false`。
    ...
    - `cacheName`: 一个 {{jsxref("String")}} ，代表一个具体的要被搜索的缓存。注意该选项被 `Cache.match()` 方法忽略。
