...
<p>一个域可以有多个命名 Cache 对象。你需要在你的脚本 (例如，在 {{domxref("ServiceWorker")}} 中) 中处理缓存更新的方式。除非明确地更新缓存，否则缓存将不会被更新；除非删除，否则缓存数据不会过期。使用 {{domxref("CacheStorage.open", "CacheStorage.open(cacheName)")}} 打开一个 Cache 对象，再使用 Cache 对象的方法去处理缓存。</p>
...
<div class="note">
<p><strong>Note:</strong> 自 Chrome 46 版本起，Cache API 只保存安全来源的请求，即那些通过 HTTPS 服务的请求。</p>
</div>
...
<div class="note">
<p><strong>Note:</strong> Cache API 不支持 HTTP 缓存头。</p>
</div>
...
<dl>
 ...
 <dd>返回一个 {{jsxref("Promise")}}对象，resolve 的结果是跟 {{domxref("Cache")}} 对象匹配的第一个已经缓存的请求。</dd>
 <dt>{{domxref("Cache.matchAll", "Cache.matchAll(request, options)")}}</dt>
 <dd>返回一个{{jsxref("Promise")}} 对象，resolve 的结果是跟{{domxref("Cache")}}对象匹配的所有请求组成的数组。</dd>
 <dt>{{domxref("Cache.add", "Cache.add(request)")}}</dt>
 <dd>抓取这个 URL，检索并把返回的 response 对象添加到给定的 Cache 对象。这在功能上等同于调用 fetch(), 然后使用 Cache.put() 将 response 添加到 cache 中。</dd>
 <dt>{{domxref("Cache.addAll", "Cache.addAll(requests)")}}</dt>
 <dd>抓取一个 URL 数组，检索并把返回的 response 对象添加到给定的 Cache 对象。</dd>
 <dt>{{domxref("Cache.put", "Cache.put(request, response)")}}</dt>
 <dd>同时抓取一个请求及其响应，并将其添加到给定的 cache。</dd>
 <dt>{{domxref("Cache.delete", "Cache.delete(request, options)")}}</dt>
 <dd>搜索 key 值为 request 的{{domxref("Cache")}} 条目。如果找到，则删除该{{domxref("Cache")}} 条目，并且返回一个 resolve 为 true 的{{jsxref("Promise")}}对象；如果未找到，则返回一个 resolve 为 false 的{{jsxref("Promise")}}对象。</dd>
 <dt>{{domxref("Cache.keys", "Cache.keys(request, options)")}}</dt>
 <dd>返回一个{{jsxref("Promise")}}对象，resolve 的结果是{{domxref("Cache")}}对象 key 值组成的数组。</dd>
</dl>

<h2 id="示例">示例</h2>

<p>此代码段来自 <a href="https://github.com/GoogleChrome/samples/blob/gh-pages/service-worker/selective-caching/service-worker.js">service worker selective caching sample</a>. (请参阅 <a href="https://googlechrome.github.io/samples/service-worker/selective-caching/">selective caching live</a>) 。该代码使用{{domxref("CacheStorage.open", "CacheStorage.open(cacheName)")}} 打开任何具有以<code>font/开始的</code>Content-Type 头的{{domxref("Cache")}}对象。</p>

<p>代码然后使用{{domxref("Cache.match", "Cache.match(request, options)")}}查看缓存中是否已经有一个匹配的 font，如果是，则返回它。 如果没有匹配的字体，代码将通过网络获取字体，并使用 {{domxref("Cache.put","Cache.put(request, response)")}}来缓存获取的资源。</p>

<p>代码处理从{{domxref("Globalfetch.fetch","fetch()")}} 操作抛出的异常。 请注意，HTTP 错误响应（例如 404）不会触发异常。 它将返回一个具有相应错误代码集的正常响应对象。</p>

<p>该代码片段还展示了服务工作线程使用的缓存版本控制的最佳实践。 虽然在这个例子中只有一个缓存，但同样的方法可用于多个缓存。 它将缓存的速记标识符映射到特定的版本化缓存名称。 代码还会删除命名不在 CURRENT_CACHES 中的所有缓存。</p>

<div class="note"><strong>注意：</strong> 在 Chrome 中，请访问 chrome://inspect/#service-workers ，然后单击注册的服务工作线程下面的“inspect”链接，查看 <a href="https://github.com/GoogleChrome/samples/blob/gh-pages/service-worker/selective-caching/service-worker.js">service-worker.js</a> 脚本正在执行的各种操作的日志记录。</div>
