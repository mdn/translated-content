...
- `end` {{ optional_inline() }}
  - : 这个参数代表 {{domxref("Blob")}} 里的下标，表示第一个 \*不会\*被拷贝进新的 {{domxref("Blob")}} 的字节的位置（换句话说，这个位置的字节不会被拷贝）。如果你传入了一个负数，那么这个偏移量将会从数据的末尾从后到前开始计算。举例来说， -10 将会是 {{domxref("Blob")}} 的倒数第十个字节。它的默认值就是它的原始长度 `size`.
