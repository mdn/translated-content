...
<p>Blob 表示的不一定是 JavaScript 原生格式的数据。{{domxref("File")}} 接口基于<code>Blob</code>，继承了 blob 的功能并将其扩展使其支持用户系统上的文件。</p>

<p>要从其他非 blob 对象和数据构造一个 <code>Blob</code>，请使用 {{domxref("Blob.Blob", "Blob()")}} 构造函数。要创建一个 blob 数据的子集 blob，请使用 {{domxref("Blob.slice()", "slice()")}} 方法。要获取用户文件系统上的文件对应的 <code>Blob</code> 对象，请参阅 {{domxref("File", "File")}} 文档。</p>

<p>接受 <code>Blob</code> 对象的 API 也被列在 {{domxref("File", "File")}} 文档中。</p>
...
<dl>
 ...
 <dd>返回一个能读取 blob 内容的 {{domxref("ReadableStream")}}。</dd>
 <dt>{{domxref("Blob.text()", "Blob.text()")}}</dt>
 <dd>返回一个 promise 且包含 blob 所有内容的 UTF-8 格式的 {{domxref("USVString")}}。</dd>
 <dt>{{domxref("Blob.arrayBuffer()", "Blob.arrayBuffer()")}}</dt>
 <dd>返回一个 promise 且包含 blob 所有内容的二进制格式的 {{domxref("ArrayBuffer")}} </dd>
</dl>
...
<h3 id="示例：使用_Blob_创建一个指向类型化数组的URL">示例：使用 Blob 创建一个指向类型化数组的 URL</h3>
...
<p>一种从 Blob 中读取内容的方法是使用 {{domxref("FileReader")}}。以下代码将 Blob 的内容作为类型数组读取：</p>
...
<p>通过使用 {{domxref("FileReader")}} 的其它方法可以把 Blob 读取为字符串或者数据 URL。</p>
...
<p>[3] 在 Gecko 12.0 {{ geckoRelease("12.0") }} 之前，有个 bug 会影响 <code>slice()</code> 的行为，就是参数 <code>start</code> 和 <code>end</code> 的值不能超出 64 位有符号数字范围，现已修复。</p>
