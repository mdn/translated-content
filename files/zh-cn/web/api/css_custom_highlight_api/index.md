...
CSS 自定义高亮 API 通过提供一种创建和设置任意 {{domxref('Range')}} 对象样式的方法（而不是局限于浏览器定义的范围），扩展了其他高亮伪元素的概念，例如 {{cssxref('::selection')}}、{{cssxref('::spelling-error')}}、{{cssxref('::grammar-error')}} 和 {{cssxref('::target-text')}}。

使用 CSS 自定义高亮 API，你可以通过编程方式创建文本范围并突出显示它们，而不会影响页面中的 DOM 结构。
