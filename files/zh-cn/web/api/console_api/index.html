...
<p>Console API 提供了允许开发人员执行调试任务的功能，例如在代码中的某个位置记录消息或变量值，或者计算任务完成所需的时间。</p>

<h2 id="概念和用法">概念和用法</h2>

<p>Console API 最初是一个专有的 API，不同的浏览器以自己的实现方式来实现它。<a href="https://console.spec.whatwg.org/">Console API </a> 规范统一了这个 API 的行为，并且所有现代浏览器最终都决定实现这种行为 — 尽管一些实现仍然有自己的附加专有功能。了解这些请查看：</p>
...
<p>用法非常简单 — {{domxref("console")}} 对象 — 可以通过{{domxref("window.console")}}获取到，在 workers 里面使用{{domxref("WorkerGlobalScope.console")}}获取，<code>console</code> — 包含许多方法，您可以调用它们来执行基本的调试任务，通常专注于将各种值记录到浏览器中 <a href="/en-US/docs/Tools/Web_Console">WEB 控制台</a>.</p>

<p>到目前为止，最常用的方法是 console.log，它用于记录特定变量中包含的当前值。</p>
