...
<p>The <strong><code>WebGLRenderingContext.vertexAttribPointer()</code></strong> method of the <a href="/en-US/docs/Web/API/WebGL_API">WebGL API</a> binds the buffer currently bound to <code>gl.ARRAY_BUFFER</code> to a generic vertex attribute of the current vertex buffer object and specifies its layout.<br>
 告诉显卡从当前绑定的缓冲区（bindBuffer() 指定的缓冲区）中读取顶点数据。</p>

<p>WebGL API 的<code><strong>WebGLRenderingContext.vertexAttribPointer()</strong></code>方法绑定当前缓冲区范围到<code>gl.ARRAY_BUFFER</code>,成为当前顶点缓冲区对象的通用顶点属性并指定它的布局 (缓冲区对象中的偏移量)。</p>
...
<dl>
 ...
 指定每个顶点属性的组成数量，必须是 1，2，3 或 4。</dd>
 ...
 <ul>
  <li>For types <code>gl.BYTE</code> and <code>gl.SHORT</code>, normalizes the values to [-1, 1] if true.<br>
   对于类型<code>gl.BYTE</code>和<code>gl.SHORT</code>，如果是 true 则将值归一化为 [-1, 1]</li>
  <li>For types <code>gl.UNSIGNED_BYTE</code> and <code>gl.UNSIGNED_SHORT</code>, normalizes the values to [0, 1] if true.<br>
   对于类型<code>gl.UNSIGNED_BYTE</code>和<code>gl.UNSIGNED_SHORT</code>，如果是 true 则将值归一化为 [0, 1]</li>
  <li>For types <code>gl.FLOAT</code> and <code>gl.HALF_FLOAT</code>, this parameter has no effect.<br>
   对于类型<code>gl.FLOAT</code>和<code>gl.HALF_FLOAT</code>，此参数无效</li>
 </ul>
 </dd>
 <dt><code>stride</code></dt>
 <dd>一个 {{domxref("GLsizei")}}，以字节为单位指定连续顶点属性开始之间的偏移量 (即数组中一行长度)。不能大于 255。如果 stride 为 0，则假定该属性是紧密打包的，即不交错属性，每个属性在一个单独的块中，下一个顶点的属性紧跟当前顶点之后。</dd>
...
<ul>
 ...
 <li>如果<code>stride</code>和 offset 不是数据类型大小的倍数，则抛出<code>gl.INVALID_OPERATION</code>错误。</li>
 <li>A <code>gl.INVALID_OPERATION</code> error is thrown if no WebGLBuffer is bound to the ARRAY_BUFFER target.</li>
 <li>如果没有将 WebGLBuffer 绑定到<code>ARRAY_BUFFER</code>目标，则抛出<code>gl.INVALID_OPERATION</code>错误。</li>
