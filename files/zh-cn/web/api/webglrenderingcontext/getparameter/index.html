...
<p><a href="/en-US/docs/Web/API/WebGL_API">WebGL API</a> 的 <strong><code>WebGLRenderingContext.getParameter()</code></strong> 方法为传入的参数名称返回一个值。</p>

<h2 id="语法">语法</h2>

<pre class="syntaxbox">any <var>gl</var>.getParameter(<var>pname</var>);
</pre>
...
<dl>
 ...
 <dd>一个指定要返回哪个参数值的 {{domxref("GLenum")}}。请参阅下文的可能值。</dd>
</dl>
...
<p>使用 {{domxref("WebGLRenderingContext")}} 时，您可以查询以下 <code>pname</code> 参数。</p>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>返回压缩的纹理格式。<br>
    ...
    <p>当使用 {{domxref("WEBGL_compressed_texture_s3tc_srgb")}} 扩展时：</p>
...
<p>使用 {{domxref("WebGL2RenderingContext")}} 时，您可以查询以下 <code>pname</code> 参数。</p>
...
<table class="standard-table">
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>{{domxref("WebGLTransformFeedback")}}</td>
   <td> </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>{{domxref("WebGLVertexArrayObject")}}</td>
   <td> </td>
  </tr>
 </tbody>
</table>

<h3 id="WebGL_扩展">WebGL 扩展</h3>

<p>当使用 <a href="/en-US/docs/Web/API/WebGL_API/Using_Extensions">WebGL 扩展</a> 时，您可以查询以下 <code>pname</code> 参数：</p>
...
<table class="standard-table">
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>GLSL 内置函数的导数计算精度：<code>dFdx</code>、 <code>dFdy</code> 和 <code>fwidth</code>。</td>
  </tr>
  <tr>
   <td><code>ext.MAX_COLOR_ATTACHMENTS_WEBGL</code></td>
   <td>{{domxref("GLint")}}</td>
   <td>{{domxref("WEBGL_draw_buffers")}}</td>
   <td>帧缓冲区颜色附着点的最大数量。</td>
  </tr>
  <tr>
