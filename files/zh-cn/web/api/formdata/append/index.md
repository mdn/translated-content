...
- `name`
  - : `value` 中包含的数据对应的表单名称。
- `value`
  - : `表单的值。`可以是{{jsxref("String")}} 或 {{domxref("Blob")}} (包括子类型，如 {{domxref("File")}})。
- `filename` {{optional_inline}}
  - : 传给服务器的文件名称 (一个 {{jsxref("String")}}), 当一个 {{domxref("Blob")}} 或 {{domxref("File")}} 被作为第二个参数的时候， {{domxref("Blob")}} 对象的默认文件名是 "blob"。 {{domxref("File")}} 对象的默认文件名是该文件的名称。
