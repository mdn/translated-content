...
<div class="note">
<p><strong>提示：</strong> <code>RTCPeerConnection</code> 和<a href="https://developer.mozilla.org/en-US/docs/Web/API/RTCSessionDescription"><code>RTCSessionDescription</code></a> 是很多浏览器中使用的名称。强烈建议使用补充库，例如强大并且被广泛支持的<a href="https://github.com/webrtcHacks/adapter">Adapter.js</a>，以确保您网站或 Web 应用程序的兼容性。值得注意的是<a href="https://github.com/webrtcHacks/adapter">Adapter.js</a>不仅仅提供这些，它还做了一些其他补充以增强 WebRTC 在浏览器中的兼容性。参考：</p>
...
<p>由于 RTCPeerConnection 实现了 {{domxref("EventTarget")}} 接口，故其可以接收处理事件。</p>

<h3 id="构造函数"><strong>构造函数</strong></h3>

<dl>
 <dt>{{domxref("RTCPeerConnection.RTCPeerConnection()")}}</dt>
 <dd>构造函数;创建一个新的 RTCPeerConnection 对象。</dd>
</dl>
...
<dl>
 ...
 <dd><code>如果远端支持 UDP 打洞或支持通过中继服务器连接，则该属性值为 true。否则，为 false。该属性的值依赖于远端设置且仅在本地的</code> {{domxref("RTCPeerConnection.setRemoteDescription()")}}方法被调用时有效，如果该方法没被调用，则其值为 null.</dd>
 <dt>{{domxref("RTCPeerConnection.connectionState")}} {{ReadOnlyInline}}</dt>
 <dd>只读 connectionState 属性通过返回由枚举 RTCPeerConnectionState 指定的字符串值之一来指示对等连接的当前状态。</dd>
 <dt>{{domxref("RTCPeerConnection.currentLocalDescription")}} {{ReadOnlyInline}}</dt>
 <dd>只读属性 RTCPeerConnection.currentLocalDescription 返回一个描述连接本地端的 RTCSessionDescription 对象，因为自上次 RTCPeerConnection 完成协商并连接到远程对等体之后，它最近成功协商。 还包括可能已经由 ICE 代理生成的任何 ICE 候选者的列表，因为首先被描述的描述所表示的要约或答案。</dd>
 <dt>{{domxref("RTCPeerConnection.currentRemoteDescription")}} {{ReadOnlyInline}}</dt>
 <dd>只读属性 RTCPeerConnection.currentRemoteDescription 返回一个 RTCSessionDescription 对象，描述连接的远程端，因为最近一次 RTCPeerConnection 完成协商并连接到远程对等体后最近成功协商。 还包括可能已经由 ICE 代理生成的任何 ICE 候选者的列表，因为首先被描述的描述所表示的要约或答案。</dd>
 <dt>{{domxref("RTCPeerConnection.defaultIceServers")}} {{ReadOnlyInline}}</dt>
 <dd>只读属性 RTCPeerConnection.defaultIceServers 根据 RTCIceServer 字典返回一个对象数组，该字典指示如果在 RTCConfiguration 中没有提供给 RTCPeerConnection 的默认情况下，浏览器将使用 ICE 服务器。 然而，浏览器根本不需要提供任何默认的 ICE 服务器。</dd>
 <dt>{{domxref("RTCPeerConnection.iceConnectionState")}} {{ReadOnlyInline}}</dt>
 <dd>只读属性 RTCPeerConnection.iceConnectionState 返回与 RTCPeerConnection 关联的 ICE 代理的状态类型为 RTCIceConnectionState 的枚举。</dd>
 <dt>{{domxref("RTCPeerConnection.iceGatheringState")}} {{ReadOnlyInline}}</dt>
 <dd>只读属性，返回一个 RTCIceGatheringState 类型的结构体，它描述了连接的 ICE 收集状态</dd>
 ...
 <dd>只读属性，返回一个 {{domxref("RTCSessionDescription")}} ，它描述了这条连接的本地端的会话控制（用户会话所需的属性以及配置信息）。如果本地的会话控制还没有被设置，它的值就会是 null。</dd>
 <dt>{{domxref("RTCPeerConnection.peerIdentity")}} {{ReadOnlyInline}}</dt>
 <dd>只读属性，返回一个<code>RTCIdentityAssertion，它由一组信息构成，包括一个域名（idp）以及一个名称（name），它们代表了这条连接的远端机器的身份识别信息。如果远端机器还没有被设置以及校验，这个属性会返回一个 null 值。一旦被设置，它不能被一般方法改变。</code></dd>
 <dt>{{domxref("RTCPeerConnection.pendingLocalDescription")}} {{ReadOnlyInline}}</dt>
 <dd>blah</dd>
 ...
 <dd>返回一个 RTC 通信状态的结构体，这个结构体描述了本地连接的通信状态。这个 状态描述了一个定义连接配置的 SDP offer。它包含了下列信息，与{{domxref("MediaStream")}} 类型本地相关的对象的描述，媒体流编码方式或 RTP 和  RTCP 协议的选项 ，以及被 ICE 服务器收集到的 candidates(连接候选者)。当{{domxref("RTCPeerConnection.signalingState")}}的值改变时，对象上的{{event("signalingstatechange")}}事件会被触发。</dd>
</dl>

<h3 id="基本用法"><strong>基本用法</strong></h3>

<p>一个基本的 RTCPeerConnection 使用需要协调本地机器以及远端机器的连接，它可以通过在两台机器间生成 Session Description 的数据交换协议来实现。呼叫方发送一个 offer(请求)，被呼叫方发出一个 answer（应答）来回答请求。双方 - 呼叫方以及被呼叫方，最开始的时候都要建立他们各自的 RTCPeerConnection 对象。</p>
...
<dl>
 <dt>{{domxref("RTCPeerConnection.iceConnectionState")}} {{ReadOnlyInline}}</dt>
 <dd>返回一个 RTCIceConnectionState 类型的结构体，这个结构体描述了连接的 ICE 连接状态。当这个状态的值改变时，这个对象会触发一个{{event("iceconnectionstatechange")}} 事件。状态可能存在的值如下：
 <ul>
 ...
 <dt>{{domxref("RTCPeerConnection.iceGatheringState")}} {{ReadOnlyInline}}</dt>
 <dd>返回一个 iceGatheringState 类型的结构体，它描述了这条连接的 ICE 收集状态。该状态可能取以下的值：
 <ul>
 ...
 <dt>{{domxref("RTCPeerConnection.localDescription")}} {{ReadOnlyInline}}</dt>
 <dd>返回一个 {{domxref("RTCSessionDescription")}} ，它描述了这条连接的本地端的会话控制（用户会话所需的属性以及配置信息）。如果本地的会话控制还没有被设置，它的值就会是 null。</dd>
 <dt>{{domxref("RTCPeerConnection.peerIdentity")}} {{ReadOnlyInline}}</dt>
 <dd>返回一个<code>RTCIdentityAssertion，它由一组信息构成，包括一个域名（idp）以及一个名称（name），它们代表了这条连接的远端机器的身份识别信息。如果远端机器还没有被设置以及校验，这个属性会返回一个 null 值。一旦被设置，它不能被一般方法改变。</code></dd>
 <dt>{{domxref("RTCPeerConnection.remoteDescription")}} {{ReadOnlyInline}}</dt>
 <dd>返回一个 {{domxref("RTCSessionDescription")}} 它描述了这条连接的远端机器的会话控制，如果远端机器还未被设置，它的值会是 null。</dd>
 <dt>{{domxref("RTCPeerConnection.signalingState")}} {{ReadOnlyInline}}</dt>
 <dd>返回一个 RTC 通信状态的结构体，这个结构体描述了本地连接的通信状态。这个 状态描述了一个定义连接配置的 SDP offer。它包含了下列信息，与{{domxref("MediaStream")}} 类型本地相关的对象的描述，媒体流编码方式或 RTP 和  RTCP 协议的选项 ，以及被 ICE 服务器收集到的 candidates(连接候选者)。当{{domxref("RTCPeerConnection.signalingState")}}的值改变时，对象上的{{event("signalingstatechange")}}事件会被触发。 它可能取下列的值：
 <ul>
...
<dl>
 ...
 <dd>是收到{{event("addstream")}} 事件时调用的事件处理器。 Such an event is 当{{domxref("MediaStream")}} 被远端机器添加到这条连接时，该事件会被触发。 当调用{{domxref("RTCPeerConnection.setRemoteDescription()")}}方法时，这个事件就会被立即触发，它不会等待 SDP 协商的结果。</dd>
 ...
 <dd>是收到 {{event("identityresult")}}事件时调用的事件处理器。 当通过{{domxref("RTCPeerConnection.getIdentityAssertion()", "getIdentityAssertion()")}}生成身份断言，或在生成一个 answer 或一个 offer 的过程中，这个事件被触发。</dd>
 ...
 <dd>是收到{{event("peeridentity")}} 事件时调用的事件处理器，当一条连接的 peer identify 被设置以及校验后，该事件被触发</dd>
...
<dl>
 ...
 <dd>RTCPeerConnection 的初始化函数，通过 new RTCPeerConnection() 初始化一个 RTCPeerConnection 实例。</dd>
 <dt>{{domxref("RTCPeerConnection.createOffer()")}}</dt>
 <dd>生成一个 offer，它是一个带有特定的配置信息寻找远端匹配机器（peer）的请求。这个方法的前两个参数分别是方法调用成功以及失败的回调函数，可选的第三个参数是用户对视频流以及音频流的定制选项（一个对象）。</dd>
 <dt>{{domxref("RTCPeerConnection.createAnswer()")}}</dt>
 <dd>在协调一条连接中的两端 offer/answers 时，根据从远端发来的 offer 生成一个 answer。这个方法的前两个参数分别是方法调用成功以及失败时的回调函数，可选的第三个参数是生成的 answer 的可供选项。</dd>
 ...
 <dd>更新 ICE 服务器时调用的方法。</dd>
 <dt>{{domxref("RTCPeerConnection.addIceCandidate()")}}</dt>
 <dd>添加 iceCandidate 时调用的方法。</dd>
 ...
 <dd>返回连接中与所给 id 匹配的媒体流 {{domxref("MediaStream")}}，如果没有匹配项，返回 null。</dd>
 ...
 <dd>关闭一个 RTCPeerConnection 实例所调用的方法。</dd>
 ...
 <dd>根据所给的三个参数设置身份提供者（IdP)，这三个参数是它的名称，通信所使用的协议（可选），以及一个可选的用户名。只有当一个断言被需要时，这个 IdP 才会被使用。</dd>
...
<div class="note">
<p><strong>注意：</strong> PeerConnection 需要传递一个 RTCConfiguration 对象作为参数，如果你没有传递参数的话，火狐浏览器会自动提供一个参数。</p>
</div>
...
<pre class="brush: js">pc.addIceCandidate(new RTCIceCandidate(candidate));
</pre>
