...
- {{DOMxRef("RTCPeerConnection.currentRemoteDescription", "currentRemoteDescription")}} {{ReadOnlyInline}}
  - : 返回一个 {{DOMxRef("RTCSessionDescription")}} 对象，该对象描述自最后完成协商并连接的远程连接描述。
...
- {{DOMxRef("RTCPeerConnection.peerIdentity", "peerIdentity")}} {{ReadOnlyInline}}
  - : 返回一个解析标识远程对等端 {{DOMxRef("RTCIdentityAssertion")}} 的 {{jsxref("Promise")}}。一旦这个 `Promise` 成功解析，得到的身份信息就是目标对等端的身份信息，并且在连接的持续时间内不会改变。
...
- {{DOMxRef("RTCPeerConnection.signalingState", "signalingState")}} {{ReadOnlyInline}}
  - : 返回一个字符串，该描述连接到另一个对等体时，在连接或重新连接过程中本地端口的信令过程状态。它是以下值之一： `stable`（稳定）、
    `have-local-offer`（有本地提议）、`have-remote-offer`（有远程提议）、`have-local-pranswer`（有本地预答）、`have-remote-pranswer`（有远程预答）、`closed`（关闭）。

## 静态方法

- {{DOMxRef("RTCPeerConnection.generateCertificate_static", "RTCPeerConnection.generateCertificate()")}}
  - : 创建一个 X.509 证书及其对应的私钥，返回一个 {{jsxref("Promise")}} 对象，证书生成后该 {{jsxref("Promise")}} 将会 resolve 一个新的 {{DOMxRef("RTCCertificate")}} 。
...
- {{DOMxRef("RTCPeerConnection.getIdentityAssertion", "getIdentityAssertion()")}}
  - : 该方法返回一个 {{jsxref("Promise")}} ，该 `Promise` 解析为编码为字符串的身份断言。
    仅在 {{DOMxRef("RTCPeerConnection.signalingState", "signalingState")}} 的值不为 `closed` 时解析有效。
