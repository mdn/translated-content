...
结果是一组轨道被发送到远程对等端，而没有流与其关联。远程对等端上的 {{DOMxRef("RTCPeerConnection/track_event", "track")}} 事件处理程序将负责确定将每个轨道添加到哪个流中，这意味着也可以将它们全部添加到同一个流中。{{domxref("RTCPeerConnection.track_event", "ontrack")}} 处理程序可能如下所示：
...
在这里，事件中如果指定了特定的流，则 `track` 事件处理程序会将轨道添加到由事件对象所指定的第一个流中。否则，当 `ontrack` 第一次被调用时，会创建一个新的流并将其附加到视频元素上，然后将轨道添加到新流中。此后，新的轨道会被添加到该流中。你也可以为每个接收到的轨道创建一个新的流：
