...
<p>开发一个使用服务器发送的事件的 Web 应用程序是很容易的。你需要在服务器上的一些代码将事件流传输到 Web 应用程序，但 Web 应用程序端的事情几乎完全相同，处理任何其他类型的事件。</p>

<p>在 Web 应用程序中使用服务器发送事件很简单。在服务器端，只需要按照一定的格式返回事件流，在客户端中，只需要为一些事件类型绑定监听函数，和处理其他普通的事件没多大区别。</p>

<h2 id="从服务器接受事件">从服务器接受事件</h2>

<p>服务器发送事件 API 也就是<a href="/zh-CN/Server-sent_events/EventSource"><code>EventSource</code></a>接口，在你创建一个新的<a href="/zh-CN/Server-sent_events/EventSource"><code>EventSource</code></a>对象的同时，你可以指定一个接受事件的 URI.例如：</p>

<pre class="notranslate">const evtSource = new EventSource("ssedemo.php");
</pre>

<div class="note"><strong>注：</strong>从 Firefox 11 开始，<code>EventSource</code>开始支持<a href="/zh-CN/HTTP_access_control">CORS</a>.虽然该特性目前并不是标准，但很快会成为标准。</div>
...
<p>当<strong>不通过 HTTP / 2 使用时</strong>，SSE（server-sent events）会受到最大连接数的限制，这在打开各种选项卡时特别麻烦，因为该限制是针对每个浏览器的，并且被设置为一个非常低的数字（6）。该问题在 <a href="https://bugs.chromium.org/p/chromium/issues/detail?id=275955">Chrome</a> 和 <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=906896">Firefox</a>中被标记为“无法解决”。此限制是针对每个浏览器 + 域的，因此这意味着您可以跨所有选项卡打开 6 个 SSE 连接到 www.example1.com，并打开 6 个 SSE 连接到 www.example2.com。（来自 <a href="https://stackoverflow.com/a/5326159/1905229">Stackoverflow</a>）。使用 HTTP / 2 时，HTTP 同一时间内的最大连接数由服务器和客户端之间协商（默认为 100）。</p>
</div>

<h2 id="服务器端如何发送事件流">服务器端如何发送事件流</h2>

<p>服务器端发送的响应内容应该使用值为<code>text/event-stream</code>的 MIME 类型。每个通知以文本块形式发送，并以一对换行符结尾。有关事件流的格式的详细信息，请参见<a href="#事件流格式">事件流格式</a>。</p>
...
<p><strong>注</strong>: 您可以在 github 上找到以上代码的完整示例—参见<a href="https://github.com/mdn/dom-examples/tree/master/server-sent-events">Simple SSE demo using PHP.</a></p>
</div>

<h2 id="错误处理">错误处理</h2>

<p>当发生错误 (例如请求超时或与<a href="/zh-CN/docs/Web/HTTP/Access_control_CORS">HTTP 访问控制（CORS）</a>有关的问题), 会生成一个错误事件。您可以通过在<code>EventSource</code>对象上使用<code>onerror</code>回调来对此采取措施：</p>
...
<dl>
 ...
 <dd>事件类型。如果指定了该字段，则在客户端接收到该条消息时，会在当前的<code>EventSource</code>对象上触发一个事件，事件类型就是该字段的字段值，你可以使用<code>addEventListener() 方法在当前 EventSource</code>对象上监听任意类型的命名事件，如果该条消息没有<code>event</code>字段，则会触发<code>onmessage 属性上的事件处理函数</code>.</dd>
 <dt><code>data</code></dt>
 <dd>消息的数据字段。如果该条消息包含多个<code>data</code>字段，则客户端会用换行符把它们连接成一个字符串来作为字段值。</dd>
 <dt><code>id</code></dt>
 <dd>事件 ID，会成为当前<code>EventSource</code>对象的内部属性"最后一个事件 ID"的属性值。</dd>
 <dt><code>retry</code></dt>
 <dd>一个整数值，指定了重新连接的时间 (单位为毫秒),如果该字段值不是整数，则会被忽略。</dd>
</dl>
