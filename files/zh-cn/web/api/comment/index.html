...
<p><code><strong>Comment</strong></code> 接口代表标签（markup）之间的文本符号（textual notations）。尽管它通常不会显示出来，但是在查看源码时可以看到它们。在 HTML 和 XML 里，注释（Comments）为 '<code>&lt;!--</code>' 和 '<code>--&gt;</code>' 之间的内容。在 XML 里，注释中不能出现字符序列 '<code>--</code>'。</p>
