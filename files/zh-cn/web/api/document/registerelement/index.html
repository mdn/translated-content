...
<div class="note">
<p><strong>注意：</strong>这是一项正在试验的技术。你可以在支持 Web Components 的浏览器中使用。参考 <a href="/en-US/docs/Web/Web_Components#Enabling_Web_Components_in_Firefox">在火狐浏览器启用 Web Components</a> 。</p>
</div>
...
<p>我们把新标签注册到浏览器。 <code>Mytag</code> 变量拥有一个构造函数，你可以像如下所示一样使用它创造一个新的 my-tag 元素。</p>

<pre class="brush: js">document.body.appendChild(new Mytag());</pre>

<p>这样我们就插入了一个空的 my-tag 元素，你可以通过浏览器提供的开发者工具来找到它。如果你使用浏览器来查看源代码它是不可见的。由于现在元素并没有内容，所以我们在页面中是看不到它的。我们可以给它添加一些内容，以便在页面中看到。 下面是一种往新标签里面添加内容的方式。</p>
