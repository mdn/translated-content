...
<p>对于 URI http://developer.mozilla.org/en-US/docs/Web，此示例将 currentDomain 设置为字符串“ developer.mozilla.org”。</p>

<pre class="brush:js">var currentDomain = document.domain;</pre>

<h3 id="关闭窗口">关闭窗口</h3>

<p>如果文档（例如 www.example.xxx/good.html）的域为“ www.example.xxx”，则本示例将尝试关闭窗口。</p>
...
<p>Mozilla 会区分 <code>document.domain</code> 属性 <strong>从没有被设定过值 </strong>和 <strong>被显示的设定为跟该文档的 URL 的 domain 一致的值</strong>，尽管这两种状况下，该属性会返回同样的值。两个文档，只有在 <code>document.domain</code> 都被设定为同一个值，表明他们打算协作；或者都没有设定 <code>document.domain</code> 属性并且 URL 的域是一致的 (<a href="https://mxr.mozilla.org/mozilla-central/source/caps/src/nsScriptSecurityManager.cpp#1003">如何判断一致</a>)，这两种条件下，一个文档才可以去访问另一个文档。如果没有这个特殊的策略，每一个站点都会成为他的子域的 XSS 攻击的对象（例如，<a href="https://bugzilla.mozilla.org">https://bugzilla.mozilla.org</a> 可以被来自 <a href="https://bug*.bugzilla.mozilla.org">https://bug*.bugzilla.mozilla.org</a> 站点的 bug 附件攻击）。</p>
