...
<div class="note">
<p><strong>注意：</strong> 在 DOM4 规范中 (实现于 Gecko 10.0 {{ geckoRelease("10.0") }}) ,<code>deep</code> 是个可选的参数。如果省略不写，则使用默认值 <code>true</code>, 表示深度克隆。如果你想使用浅克隆，则传入 false 参数。</p>

<p>在没有实现 DOM4 的浏览器中，这个参数不可以省略。</p>
</div>
...
<p>将外部文档的节点插入当前文档之前，你必须使用 <a href="/zh-CN/docs/Web/API/Document/importNode"><code>document.importNode()</code></a> 从外部文档导入源节点，或者使用 <a href="/zh-CN/docs/Web/API/Document/adoptNode"><code>document.adoptNode()</code></a>导入源节点，想要了解更多的 <a href="/zh-CN/docs/Web/API/Node/ownerDocument"><code>Node.ownerDocument</code></a> 问题，请参考 <a href="http://www.w3.org/DOM/faq.html#ownerdoc">W3C DOM FAQ</a>.</p>

<p>即使你不执行导入动作，就执行插入外部文档中的节点.Firefox 目前也不会报错 (如果严格按标准执行，很多已有的网站都无法正常运行). 我们鼓励开发者严格按标准修改自己已有的不符合上述标准的代码。</p>
