...
<p><strong><code>Document</code></strong> 接口表示任何在浏览器中载入的网页，并作为网页内容的入口，也就是<a href="/en-US/docs/Using_the_W3C_DOM_Level_1_Core">DOM 树</a>。DOM 树包含了像 {{HTMLElement("body")}} 、{{HTMLElement("table")}} 这样的元素，以及<a href="/zh-CN/docs/Web/HTML/Element">大量其他元素</a>。它向网页文档本身提供了全局操作功能，能解决如何获取页面的 URL ，如何在文档中创建一个新的元素这样的问题。</p>

<p>{{inheritanceDiagram}}</p>

<p><code>Document</code> 接口描述了任何类型的文档的通用属性与方法。根据不同的文档类型（例如<a href="/en-US/docs/HTML">HTML</a>、<a href="/en-US/docs/XML">XML</a>、<a href="/zh-CN/docs/Web/SVG">SVG</a>，...），还能使用更多 API：使用 <code>"text/html"</code> 作为内容类型（content type）的 HTML 文档，还实现了 {{DOMxRef("HTMLDocument")}} 接口，而 XML 和 SVG 文档则（额外）实现了 {{DOMxRef("XMLDocument")}} 接口。</p>
