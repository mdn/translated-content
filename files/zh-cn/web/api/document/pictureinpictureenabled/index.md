...
一个布尔值，如果视频可以通过 {{domxref("HTMLVideoElement.requestPictureInPicture()")}} 调用进入画中画模式并在浮动窗口中播放，则为 `true`。如果画中画模式不可用，则值是 `false`。

## 示例

在此示例中，{{htmlElement("video")}} 元素试图进入画中画模式之前，会检查 `pictureInPictureEnabled` 的值，为避免在该功能不可用时调用。
