...
<p>创建一个新的 <code>&lt;div&gt;</code> 并且插入到 ID 为“<code>div1</code>”的元素前。</p>
...
<ul>
 ...
 <li>当在一个被标记为 HTML 文档的文档对象上执行时，createElement() 优先将参数转换为小写。</li>
 <li>当创建一个带限制条件的元素时，请使用{{ domxref("document.createElementNS()") }}。</li>
 <li>Gecko 2.0{{ geckoRelease("2.0") }}之前，quirks 模式下 tagName 可以包含尖括号 (&lt;和&gt;)；从 Gecko2.0 开始，该方法在 quirks 模式和标准模式下表现一致。</li>
 <li>从 Gecko19.0{{ geckoRelease("19.0") }}开始， <code>createElement(null)</code> 和 <code>createElement("null")</code> 相同。Opera 也会将 null 字符串化，但是 Chrome 和 IE 都会抛出错误。</li>
 <li>从 Gecko22.0{{geckoRelease("22.0")}}开始，当参数为"bgsounds", "multicol", 或"image"时， <code>createElement()</code> 不再使用 {{domxref("HTMLSpanElement")}} 接口， 参数为 "bgsound" 和 "multicol" 时，使用 <code>HTMLUnknownElement</code> ，为“image”时使用{{domxref("HTMLElement")}} <code>HTMLElement</code>。</li>
 <li><code>createElement</code> 的 Gecko 实现不遵循 XUL 和 XHTML 的 DOM 说明文档：创建元素的<code>localName</code>和<code>namespaceURI 不会被设置为 null</code>. 更多细节详见 {{Bug(280692)}}。</li>
</ul>
