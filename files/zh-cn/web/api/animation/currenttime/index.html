...
<p>Animation.currentTime 属性返回或设置动画的当前时间值（以毫秒为单位），无论动画正在运行还是已暂停。</p>

<p>如果动画缺少{{domxref("AnimationTimeline", "timeline")}}，处于非活动状态或尚未播放，则当前时间返回值为 null。</p>
...
<p>表示当前时间的数字（以毫秒为单位），或为 null。</p>

<h2 id="例子">例子</h2>

<p>在<a href="http://codepen.io/rachelnabors/pen/PNYGZQ?editors=0010">Drink Me/Eat Me game</a>中，爱丽丝的高度是可变动的，所以它可以从小到大或从大到小。 在游戏开始时，通过将她的动画的 currentTime 设置为她的 keyframeEffect 的持续时间的一半让她的高度设置在两个极端之间：</p>

<pre class="brush: js">aliceChange<strong>.currentTime</strong> = aliceChange.effect.timing.duration / 2;</pre>

<p>寻求动画的 50％标记的更通用的方法：</p>
