...
> **备注：** 在 `init-buffers.js` 文件中添加如下方法：
...
> **备注：** 在 `initBuffers()` 函数中用下面代码替换旧的返回片段：
...
> **备注：** 在 `main()`函数中更新 `vsSource` 值，定义如下：
...
> **备注：** 在 `main()`函数中更新 `fsSource`值，定义如下：
...
> **备注：** 在 `main()`函数中更新 `programInfo`值，定义如下：
...
```
> **备注：** 在 `drawScene()`函数中 `gl.useProgram()` 调用 `setColorAttribute()` 
