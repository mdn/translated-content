...
> [!NOTE]
> 在“draw-scene.js”文件 `setPositionAttribute()` 函数中，将 `numComponents` 从 `2` 改为 `3`:
...
> [!NOTE]
> 在“init-buffers.js”文件 `initBuffers()`函数中，添加下面的代码替换之前的 `return` 代码片段：
...
> [!NOTE]
> 在 `drawScene()` 函数中，`gl.useProgram` 代码前添加如下代码：
...
> [!NOTE]
> 在“draw-scene.js”文件 `drawScene()` 函数中，用下面这段代码替换之前 `gl.drawArrays()`：
...
> [!NOTE]
> 在 `drawScene()` 函数声明中，将变量 `squareRotation` 替换成 `cubeRotation`：
