...
<p><a href="http://www.khronos.org/webgl/">WebGL</a> 使得在支持 HTML 的 <a href="/zh-CN/HTML/Canvas"><code>canvas</code></a> 标签的浏览器中，不需要安装任何插件，便可以使用基于 <a href="http://www.khronos.org/opengles/">OpenGL ES</a> 2.0 的 API 在 canvas 中进行 2D 和 3D 渲染。WebGL 程序包括用 JavaScript 写的控制代码，以及在图形处理单元（GPU, Graphics Processing Unit）中执行的着色代码（GLSL，注：GLSL 为 OpenGL 着色语言）。WebGL 元素可以和其他 HTML 元素混合使用，并且可以和网页其他部分或者网页背景结合起来。</p>
...
<p>我们的 JavaScript 代码中的 <code>main()</code> 函数将会在文档加载完成之后被调用。它的任务是设置 WebGL 上下文并开始渲染内容。</p>
