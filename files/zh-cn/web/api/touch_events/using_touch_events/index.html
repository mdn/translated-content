...
<p>今天，大多数 Web 内容是为键盘和鼠标输入而设计的。然而，具有触摸屏（特别是便携式设备）的设备是主流的，Web 应用程序可以使用 {{domxref("Touch_events","Touch Events")}} 直接处理基于触摸的输入，或者应用程序可以使用可解释的鼠标事件以处理应用程序的输入。使用鼠标事件的缺点是它们不支持并发用户输入，而触摸事件支持多个同时输入（可能在触摸面上的不同位置），从而增强用户体验。</p>
...
<p>触摸事件有三个接口 ({{domxref("Touch")}}, {{domxref("TouchEvent")}} 和 {{domxref("TouchList")}}) 和以下事件类型：</p>

<ul>
 <li>{{event("touchstart")}} - 当触摸点放置在触摸面上时触发。</li>
 <li>{{event("touchmove")}} - 当触摸点沿触摸表面移动时触发。</li>
 <li>{{event("touchend")}} - 当触摸点从触摸表面移除时触发。</li>
 <li>{{event("touchcancel")}} - 当触摸点以实现特定的方式中断（例如，创建的触摸点太多）时触发。</li>
</ul>

<p>{{domxref("Touch")}} 接口表示触敏设备上的单个接触点。接触点通常被称为触摸点或仅仅是触摸点。触摸通常由触摸屏，笔或触控板上的手指或触控笔产生。触摸点的属性包括唯一标识符，触摸点的目标元素以及触摸点相对于视口，页面和屏幕的位置的 X 和 Y 坐标。</p>

<p>{{domxref("TouchList")}} 接口表示触摸表面上的触点的列表。因此，如果用户用一根手指触摸触控表面，则该列表将包含一个触点，并且如果用户用三个手指触摸该表面，则列表长度将为三个。</p>

<p>{{domxref("TouchEvent")}} 接口表示当触控屏上触点的状态发生改变时会发送的事件。当与触控屏开始接触时状态开始改变，移动触摸点且手指在触控屏上，释放触点然后退出触摸事件。这个接口的属性包括几个修饰键的状态（例如 <kbd>shift</kbd>键）和下面的触摸列表：</p>

<ul>
 <li>{{domxref("TouchEvent.touches","touches")}} - 屏幕上当前所有触摸点的列表。</li>
 <li>{{domxref("TouchEvent.targetTouches","targetTouches")}} - 目标 DOM 元素上的触摸点列表。</li>
 <li>{{domxref("TouchEvent.changedTouches","changedTouches")}} - 依赖于关联的触摸事件类型的触摸点列表：
  <ul>
   <li>对于 {{event("touchstart")}} 事件，它是当前事件变为活动的触摸点的列表。</li>
   <li>对于 {{event("touchmove")}} 事件，它是自上次事件以来更改的触摸点列表。</li>
   <li>对于 {{event("touchend")}} 事件，它是从表面移除的触摸点的列表（即，与手指不再接触表面的触摸点集合）。</li>
  </ul>
 </li>
</ul>
...
<p>应用程序使用的触摸列表取决于应用程序手势的语义。例如，如果应用程序在一个元素上支持单一触摸（点击），则它将使用 {{event("touchstart")}} 中的 {{domxref("TouchEvent.targetTouches","targetTouches")}} 列表事件处理程序以特定应用程序处理触摸点。如果应用程序支持任意两个触摸点的双指滑动，它将使用 {{event("touchmove")}} 事件处理程序中的 {{domxref("TouchEvent.changedTouches","changedTouches")}} 列表确定两个触摸点是否已移动，然后以应用程序特定的方式实现该手势的语义。</p>

<p>当只有一个活动的触摸点时，浏览器通常会分派仿真的鼠标和点击事件。涉及两个或多个活动触摸点的多点触控交互通常只会产生触摸事件。为了防止模拟的鼠标事件被发送，请在触摸事件处理程序中使用 {{domxref("Event.preventDefault()","preventDefault()")}} 方法。有关鼠标和触摸事件之间的交互的详细信息，请参阅  {{domxref("Touch_events.Supporting_both_TouchEvent_and_MouseEvent", "Supporting both TouchEvent and MouseEvent")}}。</p>

<h2 id="Basic_steps">Basic steps</h2>

<p>本节包含使用上述接口的基本用法。有关更详细的示例，请参阅 {{domxref("Touch_events","Touch Events Overview")}} 。</p>
...
<ul>
 ...
 <li>在 {{event("touchstart")}} 中添加 {{event("touchend")}}， {{event("touchcancel")}} 和 {{event("touchmove")}} 事件处理程序。</li>
...
<p>关于触摸点的 <a href="https://developer.mozilla.org/en-US/docs/Web/API/Touch_events/Web/API/Touch#Touch_area">touch area</a> 的一些新功能 - 用户和触摸表面之间的接触面积正在被标准化。新功能包括最接近触摸点与触摸面的接触区域的椭圆的 X 和 Y 半径。接触点的旋转角度 - 应用于所描述的椭圆以与接触面积对准的旋转角度的数量也被标准化，以及触摸点上对屏幕的力量。</p>
...
<p>The implementation status of pointer events in browsers is <a href="http://caniuse.com/#search=pointer">relatively low</a> with IE11 and Edge having complete implementations. Firefox's implementation has been withdrawn because of {{bug("1166347")}}.</p>
