...
<p>在触摸平面上移动一根或者几根手指会触发 {{event("touchmove")}} 事件，从而将调用<code>handleMove()</code> 函数。本例中这个函数用于更新触摸点信息，并为每个触摸点从之前位置到当前位置之间绘制直线。</p>
...
<p>这个函数不仅对所有已改变的触摸点进行了遍历，还从缓存的触摸信息数组中得到了每个触摸点要绘制的新线段的起点。这是通过读取每个触摸点的 {{domxref("Touch.identifier")}} 属性实现的。对每个触摸点而言，该属性是个唯一的整数，且手指接触表面的整个过程中，这个属性保持不变。</p>

<p>这样我们就可以取得每个触摸点之前位置的坐标，并且使用恰当的上下文方法绘制片段，从而将新旧两个位置连接起来。</p>

<p>当片段绘制完毕后，我们调用 <code><a href="/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Array/splice">Array.splice()</a></code> 将 <code>ongoingTouches</code> 数组中触摸点之前的信息替换为当前信息。</p>
...
<p>这个函数与上一个很相像，唯一的实质性区别就是在调用 <code><a href="/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Array/splice">Array.splice()</a></code> 时， 我们把用过的触摸实体从 <code>ongoingTouches</code> 数组中直接移除了，不再添加更新信息。对这个触摸点的跟踪随之终止。</p>
...
<p>这个函数返回一个表示颜色的字符串，可以在调用 {{ HTMLElement("canvas") }} 的函数时使用。比如，若 {{ domxref("Touch.identifier") }} 的值为 10，则返回的字符串为 "#aaa"。</p>

<div class="blockIndicator warning">
<p><strong>译注：</strong>这里的 <code>colorForTouch()</code> 不是一个好主意。<code>Touch.identifier</code> 是一个实验性属性，存在兼容性问题，不同浏览器之间实现方法不同，因此会得到不同的结果。</p>
</div>
...
<p>为了阻止页面产生类似 pinchZoom 的行为，可以通过“在系列触摸点的第二个调用 <code>preventDefault()</code>”技术来实现。但是该技术的行为并没有在触摸事件的标准中做出完整定义，并且在不同浏览器中会产生不同行为（比如，iOS 将阻止缩放，但允许二指平移；Android 允许缩放但阻止平移；Opera 和 Firefox 目前会阻止所有缩放和平移操作）。目前，对于此类情况最好依靠 <code>meta viewport</code> 来阻止缩放，而不要依赖于上述行为。</p>
