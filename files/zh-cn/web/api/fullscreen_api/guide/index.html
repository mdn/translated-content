...
<p>本文主要说明如何使用全屏 API 将给定元素设置为全屏模式，以及如何检测浏览器何时进入或退出全屏模式。</p>
...
<p>值得留意的是，目前 Gecko 和 WebKit 的实现之间的关键差异：Gecko 自动为元素添加了 CSS 规则，使其拉伸以填满屏幕： "<code>width: 100%; height: 100%</code>"。WebKit 没有这样做，相反地，它将全屏元素居中，不改变大小，而屏幕的其他部分为黑色。为了在 Webkit 中获得相同的全屏行为，你需要自行为元素添加 CSS "<code>width: 100%; height: 100%;</code>"：</p>
...
<h2 id="Things_your_users_want_to_know">你的用户想了解的信息</h2>
...
<div class="note">
<p>现在，只有 Firefox 64 和 Chrome 71 支持无前缀。</p>
</div>

<p>目前并不是所有的浏览器都实现了 API 的无前缀版本（你可以使用 <a href="https://github.com/rafrex/fscreen">Fscreen</a> 获取跨浏览器全屏 API），这里有一份表格总结了前缀和它们之间的命名区别：</p>
