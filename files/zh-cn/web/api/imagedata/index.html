...
<dl>
 ...
 <dd>三个参数，第一个 是{{jsxref("Uint8ClampedArray")}}的实例，第二个和第三个表示的是 width 和 height，必须保证 Uint8ClampedArray 的 length = 4*width*height 才不会报错，如果第一个参数 Uint8ClampedArray 没有的话，自动按照 width 和 height 的大小，以 0 填充整个像素矩阵。<br>
 使用给定的{{jsxref("Uint8ClampedArray")}}创建一个 <code>ImageData</code> 对象，并包含图像的大小。如果不给定数组，会创建一个“完全透明”(因为透明度值为 0) 的黑色矩形图像。注意，这是最常见的方式去创建这样一个对象，在 {{domxref("CanvasRenderingContext2D.createImageData", "createImageData()")}} 不可用时。</dd>
</dl>
...
<dl>
 ...
 <dd>无符号长整型（<code>unsigned</code> <code>long</code>），使用像素描述 <strong>ImageData</strong> 的实际高度。</dd>
 <dt>{{domxref("ImageData.width")}} {{readonlyInline}}</dt>
 <dd>无符号长整型（<code style="font-style: normal;">unsigned</code> <code style="font-style: normal;">long</code>），使用像素描述 <strong>ImageData</strong> 的实际宽度。</dd>
</dl>
