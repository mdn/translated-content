...
**`TransformStream()`** 构造函数创建一个新的 {{domxref("TransformStream")}} 对象，该对象表示一对流：一个 {{domxref("WritableStream")}} 表示可写端，并且一个 {{domxref("ReadableStream")}} 表示可读端。
...
- `transformer`{{Optional_Inline}}
  ...
  - : 一个表示 `transformer` 的对象。如果未提供，则生成的流将是一个 **identity transform stream**，它将所有写入可写端的分块转发到可读端，不会有任何改变。
...
> **备注：**你可以定义你自己的自定义 `readableStrategy` 或 `writableStrategy`，或者使用 {{domxref("ByteLengthQueuingStrategy")}} 或 {{domxref("CountQueuingStrategy")}} 的实例作为对象的值。

## 示例

### 转换文本到大写

以下示例将文本逐块转换为大写。这个示例来自 [Streams—The Definitive Guide](https://web.dev/streams/)，它有一个不同类型流的一些示例。
