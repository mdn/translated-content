...
<h3 id="Blocking_default_click_handling">阻止默认的点击事件执行</h3>
...
<h3 id="Stopping_keystrokes_from_reaching_an_edit_field">在编辑域中阻止按键</h3>

<p>下面的这个例子说明了如何使用<code>preventDefault()</code>在文本编辑域中阻止有效的文本输入。如今，你通常可以使用<a href="/zh-CN/docs/Learn/HTML/Forms/Data_form_validation">原生的 HTML 表单验证</a>来代替。</p>
...
<p>当用户按下一个有效按键的时候，我们就给这个 warning box 加上一些样式：</p>
...
<p>这里是相关的 JavaScript 代码。首先，监听{{event("keypress")}}事件：</p>
...
<p>你可以使用 {{domxref("Event.cancelable")}} 来检查该事件是否支持取消。为一个不支持 cancelable 的事件调用<code>preventDefault()</code>将没有效果。</p>
