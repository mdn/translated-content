...
<p><strong><code>Event</code></strong> 接口表示在 DOM 中出现的事件。</p>
...
<p>很多 DOM 元素可以被设计接收 (或者监听) 这些事件，并且执行代码去响应（或者处理）它们。通过<code><a href="/en-US/docs/Web/API/EventTarget/addEventListener">EventTarget.addEventListener()</a></code>方法可以将事件处理函数绑定到不同的<a href="/en-US/docs/Web/HTML/Element">HTML elements</a>上 (比如<code>&lt;button&gt;</code>, <code>&lt;div&gt;</code>, <code>&lt;span&gt;</code>等等) 。这种绑定事件处理函数的方式基本替换了老版本中使用 HTML <a href="/en-US/docs/HTML/Global_attributes">event handler attributes</a>来绑定事件处理函数的方式。除此之外，通过正确使用<a href="/en-US/docs/Web/API/EventTarget/removeEventListener"><code>removeEventListener()</code></a>方法，这些事件处理函数也能被移除。</p>
...
<p>当有很多嵌套的元素，并且每一个元素都有着自己的事件处理函数，事件处理过程会变得非常复杂。尤其当一个父元素和子元素绑定有相同类型的事件处理函数的时候。因为结构上的重叠，事件处理函数可能会依次被触发，触发的顺序取决于<a href="/en-US/docs/Learn/JavaScript/Building_blocks/Events#Event_bubbling_and_capture">事件冒泡和事件捕获</a>在每一个元素上的设置情况。</p>
