...
![图中有三个网格坐标系，网格的每一格为屏幕上的像素点。每一个网格左上角标有 (0,0)。在第一个网格中，一个从 (2,1) 到 (5,5) 的矩形区域被填充为浅红色。在第二个网格中，从 (3,1) 到 (3,5) 绘制了一条深蓝色线条，框度为 1 像素。深色线条以网格线为中心，扩展到了 x 轴的 2.5 到 3.5，占到了线条两侧像素的一半，两侧剩下区域从 x 轴的 2 到 4 被浅蓝色填充。为了避免网格二中的浅蓝色线条扩展，第三个网格中展示了从 (3.5,1) 到 (3.5,5) 的线条，这个宽度为 1 像素的线段包含在像素块完整区域内。](canvas-grid.png)
