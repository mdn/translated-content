...
<p>{{HTMLElement("canvas")}} 元素创造了一个固定大小的画布，它公开了一个或多个<strong>渲染上下文</strong>，其可以用来绘制和处理要展示的内容。我们将会将注意力放在 2D 渲染上下文中。其他种类的上下文也许提供了不同种类的渲染方式；比如， <a href="/en-US/docs/Web/WebGL">WebGL</a> 使用了基于<a href="http://www.khronos.org/opengles/">OpenGL ES</a>的 3D 上下文 ("experimental-webgl") 。</p>
...
<p>替换内容是用于在不支持 {{HTMLElement("canvas")}} 标签的浏览器中展示的。通过简单的测试 <code>getContext()</code> 方法的存在，脚本可以检查编程支持性。上面的代码片段现在变成了这个样子：</p>
