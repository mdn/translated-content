...
<p><strong>EventTarget.addEventListener() </strong>方法将指定的监听器注册到 {{domxref("EventTarget")}} 上，当该对象触发指定的事件时，指定的回调函数就会被执行。 事件目标可以是一个文档上的元素 {{domxref("Element")}},{{domxref("Document")}}和{{domxref("Window")}}或者任何其他支持事件的对象 (比如 <code><a href="/zh-cn/DOM/XMLHttpRequest">XMLHttpRequest</a></code>)<code>。</code></p>
...
<dl>
 ...
 <dd>{{jsxref("Boolean")}}，在 DOM 树中，注册了 listener 的元素， 是否要先于它下面的 EventTarget，调用该 listener。 当 useCapture(设为 true) 时，沿着 DOM 树向上冒泡的事件，不会触发 listener。当一个元素嵌套了另一个元素，并且两个元素都对同一事件注册了一个处理函数时，所发生的事件冒泡和事件捕获是两种不同的事件传播方式。事件传播模式决定了元素以哪个顺序接收事件。进一步的解释可以查看 <a href="http://www.w3.org/TR/DOM-Level-3-Events/#event-flow">事件流</a> 及 <a href="http://www.quirksmode.org/js/events_order.html#link4">JavaScript Event order</a> 文档。 如果没有指定， <code>useCapture</code> 默认为 false 。 </dd>
</dl>

<div class="note"><strong>备注：</strong> 对于事件目标上的事件监听器来说，事件会处于“目标阶段”，而不是冒泡阶段或者捕获阶段。捕获阶段的事件监听器在任何非捕获阶段的事件监听器之前被调用。</div>

<dl>
 <dt><code>wantsUntrusted</code> {{optional_inline}} {{Non-standard_inline}}</dt>
...
<p>下面这个例子用来展示如何使用 <code>addEventListener()</code> 监听鼠标点击一个元素。</p>
...
<h3 id="Why_use_addEventListener.3F">为什么要使用 <code>addEventListener</code>?</h3>
...
<h3 id="Adding_a_listener_during_event_dispatch">在事件分派时添加事件处理器</h3>
...
<div class="note"><strong>注意：</strong> JavaScript 1.8.5 引入了<code><a href="/zh-cn/JavaScript/Reference/Global_Objects/Function/bind">Function.prototype.bind()</a></code> 方法，允许制定函数调用时的 <code>this</code> 的值。这使得想要绕开由于调用情况不同，this 取值不同的问题变得十分容易 。然而请注意，你应该保留一个 <code>listener 的</code>引用，以便在未来需要的时候能够比较好地移除。</div>
...
<h3 id="Compatibility">传统的 Internet Explorer 及其 attachEvent 方法</h3>

<p>对于 Internet Explorer 来说，在 IE 9 之前，你必须使用 <code><a href="http://msdn.microsoft.com/en-us/library/ms536343(VS.85).aspx">attachEvent</a></code> 而不是使用标准方<code>法 addEventListener</code>。为了支持 IE，前面的例子需要改成这样：</p>
...
<h3 id="Older_way_to_register_event_listeners">注册 <code>listener </code>的旧方法</h3>

<p><code>addEventListener()</code> 在 DOM 2 <a href="http://www.w3.org/TR/DOM-Level-2-Events">Events</a> 规范中引入。在这之前，事件监听器应该用以下的方法注册：</p>
...
<p>在第一种情况下，每个循环中都会创建一个新的（匿名）函数。在第二种情况下，会使用先前声明的相同的函数作为事件处理器。这样的结果是占用的存储空间更小。而且，在第一种情况中，由于没有保持到匿名函数的引用，它不可能被调用 <code><a href="/zh-cn/DOM/element.removeEventListener">element.removeEventListener</a></code>，这是因为我们没有一个可参考的处理器，而在第二种情况，它可以被 <code>myElement.removeEventListener("click", processEvent, false)</code>。</p>
