---
title: Touch.radiusY
slug: Web/API/Touch/radiusY
translation_of: Web/API/Touch/radiusY
---
<p>{{ ApiRef() }}</p>
<h3 id="Summary">概述</h3>
<p>能够包围用户和触摸平面的接触面的最小椭圆的垂直轴 (Y 轴) 半径。这个值的单位和 <code>screenY</code> 相同。</p>
<p>这个属性和 {{ domxref("Touch.radiusX") }}、{{ domxref("Touch.rotationAngle") }} 一起，描述了用户和触摸平面的接触面。这在面向非精确触摸设备 (由手指直接操作的触摸屏) 开发时非常有用。这些值描述了一个尽可能接近实际接触面 (例如用户的指尖) 的椭圆。</p>
<h3 id="Syntax">语法</h3>
<pre class="eval">var <em>yRadius</em> = <em>touchItem</em>.radiusY;
</pre>
<h3 id="Return_Value">返回值</h3>
<dl>
 <dt>
  <code>yRadius</code></dt>
 <dd>
  能够包围用户和触摸平面的接触面的最小椭圆的垂直轴 (Y 轴) 半径。</dd>
</dl>
<h3 id="Specification">标准定义</h3>
<p><a href="http://www.w3.org/TR/touch-events/">Touch Events Specification</a></p>
<h3 id="相关链接">相关链接</h3>
<ul>
 <li>{{ domxref("Touch.radiusX") }}</li>
 <li>{{ domxref("Touch.rotationAngle") }}</li>
 <li>{{ domxref("Touch.force") }}</li>
</ul>
