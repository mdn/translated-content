...
在以下简单代码片段中，我们假设用户在 id 为`source` 的元素上发起多个触摸点接触，然后与表面释放接触。当触发 {{domxref("Element/touchstart_event", "touchstart")}} 事件处理器时，可以访问每个触摸点的 `Touch.screenX` 和 {{domxref("Touch.screenY")}} 坐标。
