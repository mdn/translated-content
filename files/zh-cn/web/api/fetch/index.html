...
<p>全局的 <strong><code>fetch()</code></strong> 方法用于发起获取资源的请求。它返回一个 promise，这个 promise 会在请求响应后被 resolve，并传回 {{domxref("Response")}} 对象。</p>
...
<p>当遇到网络错误时，{{domxref("fetch()")}} 返回的 promise 会被 reject，并传回 {{jsxref("TypeError")}}，虽然这也可能因为权限或其它问题导致。成功的 fetch() 检查不仅要包括 promise 被 resolve，还要包括 {{domxref("Response.ok")}} 属性为 true。HTTP 404 状态并不被认为是网络错误。</p>
...
<div class="note">
<p><strong>备注：</strong>{{domxref("fetch()")}} 方法的参数与 {{domxref("Request.Request","Request()")}} 构造器是一样的。</p>
</div>
...
<dl>
 ...
 <ul>
  ...
  <li><code>credentials</code>: 请求的 credentials，如 <code>omit</code>、<code>same-origin</code> 或者 <code>include</code>。为了在当前域名内自动发送 cookie，必须提供这个选项，从 Chrome 50 开始，这个属性也可以接受 {{domxref("FederatedCredential")}} 实例或是一个 {{domxref("PasswordCredential")}} 实例。</li>
  ...
  <li><code>referrer</code>: 一个 {{domxref("USVString")}} 可以是 <code>no-referrer</code>、<code>client</code> 或一个 URL。默认是 <code>client</code>。</li>
  ...
  <li><code>integrity</code>: 包括请求的 <a href="/en-US/docs/Web/Security/Subresource_Integrity">subresource integrity</a> 值（例如： <code>sha256-BpfBw7ivV8q2jLiT13fxDYAe2tJllusRSZ273h2nFSE=</code>）。</li>
 </ul>
 </dd>
</dl>
...
<table class="standard-table">
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>从<a href="/en-US/docs/Mozilla/Firefox/Releases/43">Firefox 43</a>开始，如果<code>fetch()</code>接收到含有用户名和密码的 URL（例如<code>http://user:password@example.com</code>），它将会抛出一个<code>TypeError</code> 。</td>
  </tr>
 </tbody>
</table>
...
var myRequest = new Request('flowers.jpg', myInit);</pre>
