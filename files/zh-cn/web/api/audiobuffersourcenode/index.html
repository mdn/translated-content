...
<p><strong><code>AudioBufferSourceNode</code></strong> 接口继承自 {{domxref("AudioScheduledSourceNode")}}，表现为一个音频源，它包含了一些写在内存中的音频数据，通常储存在一个 ArrayBuffer 对象中。在处理有严格的时间精确度要求的回放的情形下它尤其有用。比如播放那些需要满足一个指定节奏的声音或者那些储存在内存而不是硬盘或者来自网络的声音。为了播放那些有时间精确度需求但来自网络的流文件或者来自硬盘，则使用 {{domxref("AudioWorkletNode")}} 来实现回放。</p>

<p>{{InheritanceDiagram}}</p>

<p><code>AudioBufferSourceNode</code> 没有输入却有一个输出，其通道数与其 {{domxref("AudioBufferSourceNode.buffer", "buffer")}} 属性所指定的 <code>AudioBuffer</code> 相同。如果没有设置 buffer，也就是说 <code>buffer</code> 属性是 <code>null</code> 的话，输出将包含一个无声的单通道（每个采样点均为 0）。</p>
...
<dl>
 ...
 <dd>是一个 {{domxref("AudioBuffer")}} 它定义了要播放的音频，当设置它的值为 0 时，它会定义一个静默的单通道。</dd>
...
<p>在这个例子中，我们将会创建一个 2 秒的缓冲器，并用白噪音填充它，然后通过{{domxref("AudioBufferSourceNode")}}来播放它.  注释里说明了它的功能。</p>

<div class="note">
<p><strong>注意</strong>: 你可以 <a href="http://mdn.github.io/audio-buffer/">查看在线演示</a> 或 <a href="https://github.com/mdn/audio-buffer">查看源代码</a>.</p>
</div>
