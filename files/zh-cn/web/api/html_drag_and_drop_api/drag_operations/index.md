...
- 将想要拖拽的元素的 [`draggable`](/zh-CN/docs/Web/HTML/Reference/Global_attributes#draggable) 属性设置成 `"true"`。
...
属性 [`draggable`](/zh-CN/docs/Web/HTML/Reference/Global_attributes#draggable) 设置为 `"true"`，所以这个元素变成可拖拽的。如果该属性被省略或被设置为 `"false"`，则该元素将不可拖拽，此时拖拽只会选中文本。

[`draggable`](/zh-CN/docs/Web/HTML/Reference/Global_attributes#draggable) 属性可在任意元素上设置，包括图像和链接。然而，对于后两者，该属性的默认值是 `true`，所以你只会在禁用这二者的拖拽时使用到 [`draggable`](/zh-CN/docs/Web/HTML/Reference/Global_attributes#draggable) 属性，将其设置为 `false`。
...
在这个例子中，数据值是“Text to drag”，数据类型是 [`text/plain`](/zh-CN/docs/Web/API/HTML_Drag_and_Drop_API/Drag_data_store#text) 格式。
...
注意，在本例中，[`text/uri-list`](/zh-CN/docs/Web/API/HTML_Drag_and_Drop_API/Drag_data_store#link) 和 [`text/plain`](/zh-CN/docs/Web/API/HTML_Drag_and_Drop_API/Drag_data_store#text) 包含相同的数据。这通常是正确的，但不一定要这么做。
...
在本例中，我们使用 `includes` 方法来检查 [`text/uri-list`](/zh-CN/docs/Web/API/HTML_Drag_and_Drop_API/Drag_data_store#link) 是否出现在类型列表中。如果出现了，我们将取消这个事件以允许放置。如果拖拽数据不包含链接，则不取消事件，此位置也不允许放置。
...
你可以取回其他类型的数据。如果数据是一个链接，其类型应为 [`text/uri-list`](/zh-CN/docs/Web/API/HTML_Drag_and_Drop_API/Drag_data_store#link)。你可以将链接插入到内容中。
...
这个例子使用拖拽数据插入链接。顾名思义，[`text/uri-list`](/zh-CN/docs/Web/API/HTML_Drag_and_Drop_API/Drag_data_store#link) 类型可包含一个 URL 列表，每行一个 URL。在上述代码中，我们使用 [split](/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/String/split) 方法将字符串按行分割，然后迭代列表的每一行，将每一个链接都插入到文档中。注意到我们跳过了井号（#）开头的链接，因为那些只是注释。
...
- [HTML5 动态标准：拖放](https://html.spec.whatwg.org/multipage/interaction.html#dnd)
