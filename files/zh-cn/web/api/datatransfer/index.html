...
<p><code>DataTransfer</code> 对象用于保存拖动并放下（drag and drop）过程中的数据。它可以保存一项或多项数据，这些数据项可以是一种或者多种数据类型。关于拖放的更多信息，请参见 <a href="/En/DragDrop/Drag_and_Drop">Drag and Drop</a>.</p>

<p>这个对象可以从所有拖动事件 {{domxref("DragEvent","drag events")}} 的 {{domxref("DragEvent.dataTransfer","dataTransfer")}} 属性上获取。</p>
...
<dl>
 ...
 <dd>提供一个包含所有拖动数据列表的 {{domxref("DataTransferItemList")}} 对象。</dd>
 <dt>{{domxref("DataTransfer.types")}} {{readonlyInline}}</dt>
 <dd>一个提供 {{event("dragstart")}} 事件中设置的格式的 {{domxref("DOMString","strings")}} 数组。</dd>
</dl>
...
<dl>
 ...
 <dd>按下鼠标按钮按下启动拖动操作时鼠标光标所在的 {{ domxref("Node") }} 。对于外部拖动或调用方无法访问节点，此值为  <code>null</code> 。</dd>
