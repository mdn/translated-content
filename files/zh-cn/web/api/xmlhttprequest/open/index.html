...
<p><strong>XMLHttpRequest.open()</strong> 方法初始化一个请求。该方法要从 JavaScript 代码使用；从原生代码初始化一个请求，使用 <code><a href="/en-US/docs/Mozilla/Tech/XPCOM/Reference/Interface/nsIXMLHttpRequest#openRequest()">openRequest()</a></code> 替代。</p>
...
<dl>
 <dt><code>method</code></dt>
  <dd>要使用的 HTTP 方法，比如 <code>GET</code>、<code>POST</code>、<code>PUT</code>、<code>DELETE</code>、等。对于非 HTTP(S) URL 被忽略。</dd>
 <dt><code>url</code></dt>
 <dd>一个 {{domxref("DOMString")}} 表示要向其发送请求的 URL。</dd>
 ...
 <div class="note"><strong>注意：</strong>主线程上的同步请求很容易破坏用户体验，应该避免；实际上，许多浏览器已完全弃用主线程上的同步 XHR 支持。在 {{domxref("Worker")}} 中允许同步请求</div>
 </dd>
