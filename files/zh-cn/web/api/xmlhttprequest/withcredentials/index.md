...
此外，这个标志还用于指示是否需要忽略响应中的 cookie。默认值是 `false`。如果在发送来自其它域的 `XMLHttpRequest` 请求之前，未设置`withCredentials` 为 `true`，那么就不能为它自己的域设置 cookie 值。而通过设置 `withCredentials` 为 true 获得的第三方 cookie，将依旧遵循同源策略，因此不能被发起请求的脚本通过 [document.cookie](/zh-CN/docs/Web/API/Document/cookie) 或者响应标头访问。
