...
- {{domxref("DocumentType.internalSubset")}} {{readonlyInline}} {{Deprecated_Inline}}
  - : 一个表示内部子集的{{jsxref("String")}}，如果没有的话则为`null` ，例：`"<!ELEMENT foo (bar)>"`
- {{domxref("DocumentType.name")}} {{readonlyInline}}
  - : {{jsxref("String")}}，文档类型的名称，例：`<!DOCTYPE HTML>`中的“`html`”
...
- {{domxref("DocumentType.publicId")}} {{readonlyInline}}
  - : 一个{{jsxref("String")}}，例：HTML5 中的空字符串——`"-//W3C//DTD HTML 4.01//EN"`。
- {{domxref("DocumentType.systemId")}} {{readonlyInline}}
  - : 一个{{jsxref("String")}}，例：HTML5 中的空字符串——`"http://www.w3.org/TR/html4/strict.dtd"`。
