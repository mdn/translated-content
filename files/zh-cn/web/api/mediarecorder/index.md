...
<dl>
 ...
 <dd>创建一个新的<code>MediaRecorder</code>对象，对指定的{{domxref("MediaStream")}} 对象进行录制，支持的配置项包括设置容器的 MIME 类型 (例如<code>"video/webm"</code> 或者 <code>"video/mp4"</code>) 和音频及视频的码率或者二者同用一个码率</dd>
</dl>
...
<dl>
 ...
 <dd>返回录制器对象 <code>MediaRecorder </code>创建时构造函数传入的 stream 对象</dd>
 <dt>{{domxref("MediaRecorder.ignoreMutedMedia")}}</dt>
 <dd>用以指定 <code>MediaRecorder</code>是否录制无声的输入源。如果这个属性是 false. 录制器对象<code>MediaRecorder</code>  会录制无声的音频或者黑屏的视频，默认值是 false</dd>
...
<dl>
 ...
 <dd>返回一个{{domxref("Boolean")}} 值，来表示设置的 MIME type 是否被当前用户的设备支持。</dd>
...
<dl>
 ...
 <dd>静态方法，判断给定的 MIME 类型是否支持。返回{{domxref("Boolean")}}类型的值。</dd>
</dl>
