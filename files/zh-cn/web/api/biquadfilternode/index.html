...
<div>
<p><code>BiquadFilterNode接口表示一个简单低阶滤波器(双二阶滤波器)</code>, 通过 {{ domxref("AudioContext.createBiquadFilter()") }} 方法创建。它是一个能表示不同类型的过滤器，声调控制设备，图形均衡器的{{domxref("AudioNode")}} . 一个<code>BiquadFilterNode(</code>双二阶滤波器<code>)</code> 总是恰好有一个输入和一个输出。</p>
</div>
...
<dl>
 ...
 <dd>Is an <a href="/en-US/docs/Web/API/AudioParam#a-rate">a-rate</a> {{domxref("AudioParam")}} representing detuning of the frequency in <a href="http://en.wikipedia.org/wiki/Cent_%28music%29">cents</a>.</dd>
 ...
 <table>
  ...
  <tbody>
   ...
   <tr>
    ...
    <td>Standard <a href="http://en.wikipedia.org/wiki/Band-stop_filter">notch</a> filter, also called a <em>band-stop</em> or <em>band-rejection</em> filter. It is the opposite of a bandpass filter: frequencies outside the give range of frequencies pass through; frequencies inside it are attenuated.</td>
    <td>The center of the range of frequencies.</td>
    <td>Controls the width of the frequency band. The greater the <code>Q</code> value, the smaller the frequency band.</td>
    <td><em>Not used</em></td>
   </tr>
   <tr>
    ...
    <td>Standard second-order <a href="http://en.wikipedia.org/wiki/All-pass_filter#Digital_Implementation">allpass</a> filter. It lets all frequencies through, but changes the phase-relationship between the various frequencies.</td>
    <td>The frequency with the maximal <a href="http://en.wikipedia.org/wiki/Group_delay_and_phase_delay">group delay</a>, that is, the frequency where the center of the phase transition occurs.</td>
