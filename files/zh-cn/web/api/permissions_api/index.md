...
`permissions` 属性已在标准浏览上下文和工作上下文（{{domxref("WorkerNavigator")}} ——因此可以在 worker 内部进行权限检查）中的 {{domxref("Navigator")}} 对象上可用，并返回一个提供对 Permissions API 功能的访问的 {{domxref("Permissions")}} 对象。
...
- {{domxref("Navigator.permissions")}} 和 {{domxref("WorkerNavigator.permissions")}} {{ReadOnlyInline}}
  - : 提供分别从主上下文和 worker 上下文访问 {{domxref("Permissions")}} 对象的权限。
