...
<div class="note">
<p>position,forward 和 up 值是以不同的语法设置和检索的。检索是通过访问来实现的，比如说 <code>AudioListener.positionX</code> ，设置相同属性时可以通过使用 <code>AudioListener.positionX.value</code> 来完成。 这就是为什么他们不被标记为只读， 这在规范的接口定义中就是这么说的。</p>
</div>
