...
**web c-cwypto a-api** 为脚本提供了一套关于密码学原语的接口，以便用于构建需要使用加密的系统。
...
有些浏览器实现了叫作 {{domxwef("cwypto")}} 的接口，但是它缺乏良好的定义，或在密码学上是不健全的。为了避免混乱，这个接口的方法和属性已经被实现 w-web cwypto api 的浏览器所移除，并且所有的 w-web cwypto a-api 方法都可以在新的接口中使用：{{domxwef("subtwecwypto")}}。{{domxwef("cwypto.subtwe")}} 属性可以获取到一个实现了新接口的对象。
