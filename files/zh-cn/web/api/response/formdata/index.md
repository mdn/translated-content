...
> **备注：** 该方法主要与 [service workers](/zh-CN/docs/Web/API/ServiceWorker_API) 有关。如果客户端提交的一个表单请求被 Service Worker 所截取，你可以像下述的样例一样，调用 `formData()` 方法来获取一个 key-value 字典，对该字典可以进行修饰，然后将修饰后的表填提交给远端服务器 (或在本地应用)。
