...
{{domxref("RTCCertificateStats")}} 字典的 **`fingerprintAlgorithm`** 属性是一个字符串，其中包含用于生成关联的 {{domxref("RTCCertificate")}} 中的 {{domxref("RTCCertificateStats.fingerprint", "fingerprint")}} 值的哈希函数的名称。

## 值

包含用于创建关联证书指纹的哈希函数的名称的字符串。
