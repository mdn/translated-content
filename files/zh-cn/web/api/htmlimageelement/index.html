...
<p><strong><code>HTMLImageElement</code></strong> 接口提供了特别的属性和方法 (在常规的 {{domxref("HTMLElement")}}之外，它也能通过继承使用) 来操纵 {{HTMLElement("img")}} 元素的布局和图像。</p>
...
<dl>
 ...
 <dd>返回一个 {{domxref("Boolean")}} 如果浏览器已经下载完毕，并且图像是<a href="/en-US/docs/HTML/Element/Img#Image_Format">受支持的图片类型</a>、解码的过程中没有发生错误，则返回 <code>true</code>。That means this value is also <code>true</code> if the image has no {{domxref("HTMLImageElement.src", "src")}} value indicating an image to load.</dd>
 <dt>{{domxref("HTMLImageElement.crossOrigin")}}</dt>
 <dd>一个 {{domxref("DOMString")}} 表示这个 img 元素的 CORS 设置。参考 <a href="/en-US/docs/HTML/CORS_settings_attributes">CORS settings attributes</a>。This may be <code>null</code> if CORS is not used.</dd>
 <dt>{{domxref("HTMLImageElement.currentSrc")}} {{readonlyInline}}</dt>
 <dd>返回一个 {{domxref("DOMString")}}  表示加载当前显示的图像的 URL。<br>
 ...
 <dd>一个 {{domxref("DOMString")}} 表示 HTML 属性 {{htmlattrxref("srcset", "img")}}，包含了候选图像列表，用逗号分隔（<code>',', U+002C COMMA</code>）。一个候选的图像是一个 URL 跟着一个 <code>'w'</code> 表示图像的宽度，或者一个 <code>'x'</code> 表示像素密度。</dd>
...
<dl>
 ...
 <dt>{{domxref("HTMLImageElement.lowSrc")}} {{Deprecated_Inline}}</dt>
...
var img2 = document.createElement('img'); // 使用 DOM <a href="http://mxr.mozilla.org/mozilla-central/source/dom/interfaces/html/nsIDOMHTMLImageElement.idl">HTMLImageElement</a>
img2.src = 'image2.jpg';
img2.alt = 'alt text';
