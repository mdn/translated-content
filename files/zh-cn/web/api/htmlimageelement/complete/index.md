...
请思考一个图库应用程序，它具有将图片打开为灯箱模式的功能，以便更好地查看和编辑图像。这些照片可能非常大，所以你不想等待它们加载，所以你在代码中使用 `async`/`await` 在后台加载图像。

但是想象一下，你有其他代码仅仅需要在图像完成加载后运行，例如对灯箱中的图像执行红眼消除的命令。理想情况下，如果图像尚未完全加载，则不会执行此命令，但为了提高可靠性，你需要检查以确保是这种情况。
