...
<li>要指定在某个宽度下使用特定图像候选代表的图像资源， 请提供 <strong>宽度描述符</strong>，包含表示该宽度的数字（以像素为单位）+ 小写字母“w”。例如，渲染一个 450 像素宽的图像对应的描述符字符串： <code>450w</code>。指定的宽度必须是正数、非零整数，并且<em>必须</em>与引用图像的固有宽度相匹配。</li>
