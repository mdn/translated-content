...
> [!NOTE]
> 大多数时候自然高度是服务器发送的图像的实际高度。不过，浏览器可以在将图像推送到渲染器之前对其进行修改。例如，Chrome 会[降低低端设备上图像的分辨率](https://bugs.chromium.org/p/chromium/issues/detail?id=1187043#c7)。在这种情况下，`naturalHeight` 会将浏览器干预修改的图像高度视为自然高度，并返回该值。
