 ...
 **`console.log()`** 方法向 Web 控制台输出一条信息。
这条信息可能是单个字符串（包括可选的替代字符串)，也可能是一个或多个对象。
...
- `obj1` ... `objN`
  - : 一个用于输出的 JavaScript 对象列表。其中每个对象会以字符串的形式按照顺序依次输出到控制台。请注意，如果你在最新版本的 Chrome 和 Firefox 中输出对象，你在控制台中得到的是 _对该对象的引用_ ，这不一定是你调用 `console.log()` 时该对象的"值"，但它一定是该对象在你打开控制台时的值。
- `msg`
