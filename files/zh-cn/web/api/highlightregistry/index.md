...
`HighlightRegistry` 实例是一个 [类 `Map` 对象](/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Map#map-like_browser_apis)，其中每个键都是自定义高亮的名称字符串，对应的值是关联的 {{domxref("Highlight")}} 对象。
...
- {{domxref("HighlightRegistry.has()")}}
  - : 返回一个布尔值来指示对应的 `Highlight` 对象是否存在于该注册表中。
