...
{{domxref("NavigationActivation")}} 接口的 **`entry`** 只读属性包含一个 {{domxref("NavigationHistoryEntry")}} 对象，该对象表示导航中传入（“目标”）的文档历史记录条目。这相当于激活传入文档时的 {{domxref("Navigation.currentEntry")}} 属性。
