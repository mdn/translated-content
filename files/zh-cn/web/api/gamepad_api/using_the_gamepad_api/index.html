...
<p><a href="/en-US/docs/Web/API/Gamepad_API">Gamepad API</a> 引入新的事件在 {{ domxref("Window") }} 对象中，来读取手柄和控制器（以下称“控制器”）的状态。除此之外，API 还添加了一个 {{ domxref("Gamepad") }} 对象，你可以用它来查询已连接控制器的状态；还有一个 {{ domxref("navigator.getGamepads()") }} 方法，你可以使用它来获取页面已知的控制器列表。</p>

<h2 id="conntecting">连接控制器</h2>
...
<p>每个控制器都有一个与之关联的唯一 ID，其在事件的 {{domxref("GamepadEvent.gamepad", "gamepad")}} 属性上可用。</p>

<h2 id="disconnecting">断开控制器连接</h2>
...
<h2 id="querying">查询 Gamepad 对象</h2>
...
<ul>
 ...
 <li><code>timestamp</code>: 它将返回一个 {{ domxref("DOMHighResTimeStamp") }} ，该值表示上次更新此控制器数据的时间，以便开发者确定 <code>axes</code> 和 <code>button</code> 数据是否已从硬件更新。该值必须相对于 {{ domxref("PerformanceTiming") }} 接口的 <code>navigationStart</code> 对象。值是单调递增的，这意味着可以通过对比大小来确定数据更新的先后顺序，因为新的值始终比旧的值大。请注意 Firefox 当前不支持该属性。</li>
</ul>
...
<p>现在看主要的游戏循环。在每次我们所需的四个按钮被按下的时候进行处理。如果被按下了我就会适当地更新动量变量  <code>a</code> 和 <code>b</code> 的值，然后分别用 <code>a</code> 和 <code>b</code> 的值更新球的 {{ cssxref("left") }} 和 {{ cssxref("top") }} 属性。这样就可以在屏幕上移动数的位置了。在当前版本的 Chrome 中 (版本 34) button 的值是存储为数组的两个值，而不是 {{ domxref("GamepadButton") }} 对象。此问题已于开发者版本修复了。</p>
...
<p><strong>待讨论 (除了一个用 axes[i] 一个用 button[i].value ，其他基本一样，Firefox 与 Chrome 均是。)</strong></p>

<h2 id="完整的例子：显示控制器状态">完整的例子：显示控制器状态</h2>

<p>这个例子展示了怎样使用 {{ domxref("Gamepad") }} 对象，还有 {{ event("gamepadconnected") }} 和 {{ event("gamepaddisconnected") }} 事件显示所有已连接到系统的控制器的状态。你可以查看<a href="http://luser.github.io/gamepadtest/">在线演示</a>并且可在 Github 上看到<a href="https://github.com/luser/gamepadtest">完整的源代码</a>。</p>
