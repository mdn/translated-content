...
{{domxref("ServiceWorkerRegistration")}} 接口的 **`cookies`** 只读属性返回 {{domxref("CookieStoreManager")}} 接口的引用，它允许 Web APP 在 [Service Worker](/zh-CN/docs/Web/API/Service_Worker_API) 订阅和取消订阅 cookie 变更事件。是 [Cookie Store API](/zh-CN/docs/Web/API/Cookie_Store_API) 的入口点。
