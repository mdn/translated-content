...
HKDF 是一种*密钥派生函数*。它被用于从一些熵值相对较高的输入（如 ECDH 密钥协商操作的输出）派生密钥材料（key material）。
...
PBKDF2 也是一种*密钥派生函数*。它被用于从一些熵值相对较低的输入（例如密码）派生密钥材料。它通过将例如 HMAC 等函数以及加盐（salt）操作等一起应用到输入密码上，并多次重复此过程来派生密钥材料。这个过程重复的次数越多，密钥推到计算的成本就越高：这使得攻击者难以使用字典攻击这类暴力破解的方法来找出密钥。
