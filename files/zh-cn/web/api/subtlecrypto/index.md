...
- [`sign()`](https://developer.mozilla.org/zh-CN/docs/Web/API/SubtleCrypto/sign) 、 [`verify()`](https://developer.mozilla.org/zh-CN/docs/Web/API/SubtleCrypto/verify): 创建和验证数字签名。
- [`encrypt()`](https://developer.mozilla.org/zh-CN/docs/Web/API/SubtleCrypto/encrypt) 和 [`decrypt()`](https://developer.mozilla.org/zh-CN/docs/Web/API/SubtleCrypto/decrypt): 加密和解密数据。
- [`digest()`](https://developer.mozilla.org/zh-CN/docs/Web/API/SubtleCrypto/digest)：为数据生成一个定长的，防碰撞的消息摘要。
...
[`generateKey()`](https://developer.mozilla.org/zh-CN/docs/Web/API/SubtleCrypto/generateKey) 和 [`deriveKey()`](https://developer.mozilla.org/zh-CN/docs/Web/API/SubtleCrypto/deriveKey) 函数都可以创建一个新的 [`CryptoKey`](https://developer.mozilla.org/zh-CN/docs/Web/API/CryptoKey) 对象。
...
[`importKey()`](https://developer.mozilla.org/zh-CN/docs/Web/API/SubtleCrypto/importKey)与 `exportKey()` 刚好相反。你可以从其他系统导入密钥，并且支持像 [PKCS #8](https://tools.ietf.org/html/rfc5208) 和 [JSON Web Key](https://tools.ietf.org/html/rfc7517) 这样可以帮助你执行此操作的标准格式。`exportKey()` 函数以非标准格式导出密钥。

如果密钥是敏感的，你需要使用 [`wrapKey()`](https://developer.mozilla.org/zh-CN/docs/Web/API/SubtleCrypto/wrapKey), 该函数导出密钥并且使用另外一个密钥加密它。

` unwrapKey()与``wrapKey() `相反，该函数解密密钥后导入解密的密钥

#### 存储密钥

`CryptoKey`对象可以通过 [`structured clone algorithm`](https://developer.mozilla.org/zh-CN/docs/Web/API/Web_Workers_API/Structured_clone_algorithm)来存储，这意味着你可以通过 web storage APIs 来存储和获取他们。更为规范的方式是通过使用[`IndexedDB API`](https://developer.mozilla.org/zh-CN/docs/Web/API/IndexedDB_API) 来存储`CryptoKey`对象。
