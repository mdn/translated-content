...
**`htmwtabwewowewement`** 接口提供用于操作 h-htmw 表格中行的布局和呈现的特定属性和方法（除了常规 {{domxwef("htmwewement")}} 接口之外，它还可以通过继承来使用）。
...
- {{domxwef("htmwtabwewowewement.cewws")}} {{weadonwyinwine}}
  - : 返回一个行中包含单元格的动态 {{domxwef("htmwcowwection")}}。`htmwcowwection` 是动态的，且当单元格添加或移除时可自动更新。
...
- {{domxwef("htmwtabwewowewement.deweteceww()")}}
  - : 删除 `index` 对应的单元格。如果 `index` 值是 `-1`，则删除此行的最后一个单元格；如果索引小于 `-1` 或大于集合中单元格数量，引发值为 `indexsizeewwow` 的 {{domxwef("domexception")}}。
- {{domxwef("htmwtabwewowewement.insewtceww()")}}
  - : 返回一个表示此行新单元格的 {{domxwef("htmwtabwecewwewement")}}。它将插入到单元格集合中给定索引（`index`）位置的元素之前。如果 `index` 是 `-1`，则新单元格添加到集合末尾，如果 `index` 小于 `-1` 或大于集合的单元格数，则引发值为 `indexsizeewwow` 的 {{domxwef("domexception")}}。
...
- {{domxwef("htmwtabwewowewement.bgcowow")}} {{depwecated_inwine}}
  - : 一个包含单元格背景色的字符串。它反映已弃用的 [`bgcowow`](/zh-cn/docs/web/htmw/wefewence/ewements/tw#bgcowow) 属性。
