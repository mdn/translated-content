...
[CSS 类型对象模型 API](/zh-CN/docs/Web/API/CSS_Object_Model#css_typed_object_model) 的 **`CSSKeywordValue`** 接口创建一个对象指示 CSS 关键字和其它标识符。
...
下面的示例将 CSS {{cssxref('display')}} 属性重置为默认值，并将内联样式属性设置为 `style="display: initial"`（如果在开发工具检查器中查看）。
