...
<p><strong>WebVR API 能为虚拟现实设备的渲染提供支持 — 例如像 Oculus Rift 或者</strong>HTC Vive <strong>这样的头戴式设备与 Web apps 的连接。它能让开发者将位置和动作信息转换成 3D 场景中的运动。基于这项技术能产生很多有趣的应用，比如虚拟的产品展示，可交互的培训课程，以及超强沉浸感的第一人称游戏。</strong></p>
...
<p><strong>连接到电脑的所有 VR 设备都将由 {{domxref("Navigator.getVRDevices()")}} 方法返回。 这个方法将返回一个包含了所有已连接设备的对象数组，每个设备对应一个对象， 该对象继承自 {{domxref("VRDevice")}}  — 通常一个头显将包含两个设备 — 头显自身由 {{domxref("HMDVRDevice")}} 表示， 和一个跟踪头部位置的位置捕捉传感器，由 {{domxref("PositionSensorVRDevice")}} 表示。</strong></p>
...
<p><strong>{{domxref("HMDVRDevice.getEyeParameters()")}} 方法返回一个 {{domxref("VREyeParameters")}} 对象，可用于获取显示区域的信息 — 头显可以看到多少画面。 {{domxref("VREyeParameters.currentFieldOfView")}} 返回一个 {{domxref("VRFieldOfView")}} 对象 ，该对象包含了 4 个角度信息来描述当前的显示区域。你可以用 {{domxref("HMDVRDevice.setFieldOfView()")}} 来改变当前的显示区域。</strong></p>
...
<p><u><strong>注释：</strong>: 要了解更多关于如何在你的应用程序中使用这些接口，请阅读文章<a href="/en-US/docs/Web/API/WebVR_API/Using_the_WebVR_API">Using the WebVR API</a>. 要学习更多关于 VR 技术背后的基础概念，请阅读文章 <a href="/en-US/docs/Web/API/WebVR_API/WebVR_concepts">WebVR concepts</a>.</u></p>
</div>

<h3 id="Using_controllers_Combining_WebVR_with_the_Gamepad_API">Using controllers: Combining WebVR with the Gamepad API </h3>

<h3 id="使用控制器：将WebVR与Gamepad_API相结合">使用控制器：将 WebVR 与 Gamepad API 相结合</h3>
...
<dl>
 ...
 <strong>返回一个 Promise 对象，并通过 resolve 方式返回参数，参数为链接到电脑的 VR 设备数组。</strong></dd>
 ...
 <strong>返回一个包括了 VR 设备 IDs，描述等信息的类。HMDVRDevice 和 PositionSensorVRDevice 继承了 VRDevice。</strong></dd>
 ...
 <strong>头戴设备。提供设备双眼、当前 FOV（field of view）信息。</strong></dd>
 ...
 <strong>VR 设备的位置传感器。获取位置、方向信息。</strong></dd>
 ...
 <strong>给双眼提供正确渲染场景的所有信息，包括 FOV。</strong></dd>
 ...
 <strong>返回以视窗的中心点为基点的，表示 FOV 的 4 个角度值（downDegrees, leftDegrees, rightDegrees, upDegrees）。</strong></dd>
 ...
 <strong>定义一个 FOV 必须的角度属性。VRFieldOfView 继承了 VRFieldOfViewReadOnly。</strong></dd>
</dl>
...
<p><strong>你可以在 Github 的协议中找到一系列的案例：</strong></p>

<ul>
 <li><a href="https://github.com/mdn/webvr-tests">mdn/webvr-tests</a>: Simple demos built to illiustrate basic feature usage.</li>
 <li><strong><a href="https://github.com/mdn/webvr-tests">mdn/webvr-tests</a>: 简单构建的 demos 用于阐明基本的使用方法。</strong></li>
 <li> </li>
 <li><a href="https://github.com/MozVR/">MozVR team</a>: More advanced demos, the WebVR spec source, and more!</li>
 <li><strong><a href="https://github.com/MozVR/">MozVR team</a>: 更多复杂的 demos，关于 WebVR 特别的资源，以及更多的内容！</strong></li>
...
<p>在所有浏览器都实现新的 <a href="/zh-CN/docs/Web/API/WebXR_Device_API/Fundamentals">WebXR API</a> 之前，建议使用框架（如：<a href="https://aframe.io/">A-Frame</a>、<a href="https://www.babylonjs.com/">Babylon.js</a> 或 <a href="https://threejs.org/">Three.js</a>）或 <a href="https://github.com/immersive-web/webxr-polyfill">polyfill</a> 来开发适用于所有浏览器的 WebXR 应用程序。</p>
...
<ul>
 ...
 <li><strong>建立 WEBVR 的运行环境。</strong></li>
 ...
 <li><strong>怎样使用 WEBVR API。</strong></li>
 <li><a href="http://mozvr.com/">MozVr.com</a> — demos, downloads, and other resources from the Mozilla VR team.</li>
 <li><strong><a href="http://mozvr.com/">MozVr.com</a> — demos，下载，以及其他的来自 MOZILLA VR 团队的资源。</strong></li>
 ...
 <li><strong><a href="http://dsmu.me/ConsoleGameOnWeb/">Console Game on Web</a> — 一系列有趣的概念游戏 DEMO 的收集，其中有些包括了 WEBVR。</strong></li>
 ...
 <li><strong><a href="https://github.com/MozVR/vr-web-examples/tree/master/threejs-vr-boilerplate">threejs-vr-boilerplate</a> — 一个当你编写 WEBVR APP 时，非常有用的用于开始编程的模板。</strong></li>
