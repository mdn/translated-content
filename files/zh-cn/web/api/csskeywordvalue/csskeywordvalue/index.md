...
**`CSSKeywordValue()`** 构造函数创建一个新的 {{domxref("CSSKeywordValue")}} 对象指示 CSS 关键字和其它标识符。
...
- {{jsxref("TypeError")}}
  - : 如果参数 `value` 未设置或不是 {{jsxref('String')}} 类型，则抛出。

## 示例

下面的示例将 CSS {{cssxref('display')}} 属性重置为默认值，并将内联样式属性设置为 `style="display: initial"`（如果在[开发工具检查器](https://firefox-source-docs.mozilla.org/devtools-user/page_inspector/how_to/select_an_element/index.html)中查看）。
