...
**文档对象模型（DOM）** 通过将文档的结构（例如表示网页的 HTML）以对象的形式存储在内存中，将网页与脚本或编程语言连接起来。尽管将 HTML、SVG 或 XML 文档建模为对象并不是 JavaScript 核心语言的一部分，但它通常是指 JavaScript。
...
文档对象模型已被高度简化。为了实现这一目标，已在不同 DOM 第 3 级或更早规范中移除了以下接口。它们不再对 Web 开发者可用。
...
包含 HTML 的文档通过 {{DOMxRef("Document")}} 接口进行描述，接口由 HTML 规范扩展，包含了各种 HTML 特有的特性。特别是 {{domxref("Element")}} 接口被增强为 {{domxref("HTMLElement")}} 和各种子类，每个子类代表一个（或一系列密切相关的）元素。
