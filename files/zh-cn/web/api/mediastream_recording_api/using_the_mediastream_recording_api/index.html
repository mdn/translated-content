...
<p><a href="/zh-CN/docs/Web/API/MediaStream_Recording_API">媒体流 (音/视频) 录制 API</a>让记录音频流或视频流信息更加容易。当使用<a href="/zh-CN/docs/Web/API/Navigator/mediaDevices/getUserMedia">navigator.mediaDevices.getUserMedia()"</a>时，它提供了一种简单的方式从用户的输入设备中记录信息，并且可以马上在 web apps 中查看记录的信息。音/视频信息都可以被录制，可以分开也可以一块儿。本文针对于提供一个基础引导去让大家了解提供了这个 API 的 MediaRecorder 的界面。</p>

<h2 id="示例应用_Web录音机">示例应用: Web 录音机</h2>
...
<p>你可以看到<a href="https://mdn.github.io/web-dictaphone/">实例演示</a>或是 Github 上的<a href="https://github.com/mdn/web-dictaphone">源码</a>（也可以点此<a href="https://github.com/mdn/web-dictaphone/archive/master.zip">直接下载</a>）。</p>

<h2 id="CSS_goodies">CSS goodies</h2>

<p>在这个 app 应用中的网页是相当简单的，所以我们不会在这里大费周章；但有几个有点意思的 CSS 样式还是有必要提一下，所以接下来我们会讨论一下。如果你对 CSS 没有半毛钱兴趣并且想对 JavaSdcript 单刀直入，请跳转到下面的<a href="https://developer.mozilla.org/en-US/docs/Web/API/MediaStream_Recording_API/Using_the_MediaStream_Recording_API#Basic_app_setup">应用基础设置</a>章节。</p>

<h3 id="保持主界面对显示区域的约束，用calc()来忽略设备的尺寸">保持主界面对显示区域的约束，用 calc() 来忽略设备的尺寸</h3>

<p><a href="https://developer.mozilla.org/en-US/docs/Web/CSS/calc">calc() 函数</a>是 CSS3 中出现的非常实用的功能之一，虽然现在的用处和这个名称看上去关系不大，但是你很快就会觉得“WC，这个功能为什么我们之前没有？为什么之前 CSS2 的布局会这么蛋疼？”它允许你计算一个 CSS 单元的计算值，在这个过程中混合不同的单元。</p>
...
<div class="note">
<p>注意：现在的浏览器对<a href="https://developer.mozilla.org/en-US/docs/Web/CSS/calc">calc()</a>有着良好的支持，即使是像 IE9 那样的浏览器也可以。</p>
</div>

<h3 id="用于显示隐藏的复选框"><strong>用于显示/隐藏的复选框</strong></h3>

<p>虽然目前已经做的不错了，但是我们认为我们会提到一个复选框 hack 做法，它滥用了一个事实，你可以点击复选框的<a href="https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/label">label 标签</a>来切换选中/未选中。在 web 录音机中，通过点击屏幕右上角的问号图标来显示/隐藏信息屏幕。首先，在得到<a href="https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/label">&lt;label&gt;标签</a>之前我们得先设计它的样式，通过设置足够的 Z-index 堆叠次序来确保它总是坐落于其他元素之上，所以它应该是可点击的：</p>
...
<p>接下来，我们将设计信息显示区域（包括在<a href="https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/aside">&lt;aside&gt;元素</a>中），给它固定的位置，使它不出现在布局流程中去影响主要的 UI 三个户，将它转换为默认的位置，并使它平滑显示/隐藏：</p>
...
<p>我们使用 getUserMedia() 来捕获我们想要的媒体流。我们使用 MediaRecorder API 来记录信息流，并将每个记录的片段输出到生成的<a href="https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/audio">&lt;audio&gt;元素</a>的源中，以便可以回放。</p>
...
<p>一旦<code>getUserMedia</code>成功创建了媒体流，您可以使用 MediaRecorder() 构造函数创建一个新的媒体记录器实例，并直接传递该媒体流流。这是使用 MediaRecorder API 的入口点。现在，可以使用浏览器的默认编码格式将流捕获到<a href="https://developer.mozilla.org/zh-CN/docs/Web/API/Blob">Blob</a>。</p>

<pre class="brush: js">var mediaRecorder = new MediaRecorder(stream);</pre>

<p>为了能够方便的控制音频的录制，{{domxref("MediaRecorder")}}的实例提供了一系列有用的方法和事件，在 Web Dictaphone 这个简单的项目中我们只需使用其中的 2 个方法和一些事件。首先，为了能在点击 Record 按钮的时候开始录音，需要调用{{domxref("MediaRecorder.start()")}}：</p>
...
<p>最后在点击 Stop 按钮时我们调用{{domxref("MediaRecorder.stop()")}}方法结束录制，录制所产生的{{domxref("Blob")}}数据会在后面使用。</p>
...
<h2 id="抓取并使用blob数据">抓取并使用 blob 数据</h2>

<p>在停止录制后，实例的 state 属性会返回"inactive"，stop 事件也被触发。我们需要监听这个事件去处理我们收到的所有录制数据：</p>
...
<p>接下来，我们创建一个如下所示的 HTML 结构，将其插入到我的剪辑容器中，这是一个{{htmlelement("article")}}元素。</p>
...
<p>之后，我们从录制的音频块中创建组合{{domxref("Blob")}}，并使用 window.URL.createObjectURL(blob) 创建指向它的对象 URL。然后我们将 {{HTMLElement("audio")}}元素的{{htmlattrxref("src", "audio")}}属性的值设置为对象 URL，以便在音频播放器上按下播放按钮时，它会播放音频。</p>

<p>最后，我们监听删除按钮的 onclick 事件，以便能够删除整个剪辑 HTML 结构。</p>
