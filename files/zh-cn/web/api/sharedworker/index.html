...
<div><code><strong>SharedWorker</strong></code> 接口代表一种特定类型的 worker，可以从几个浏览上下文中<em>访问</em>，例如几个窗口、iframe 或其他 worker。它们实现一个不同于普通 worker 的接口，具有不同的全局作用域，{{domxref("SharedWorkerGlobalScope")}} 。</div>



<div class="note">
<p><strong>注意：如果要使 </strong>SharedWorker <strong>连接到多个不同的页面，这些页面必须是同源的（相同的协议、host 以及端口）。</strong></p>
</div>
...
<p>在这个 <a href="https://github.com/mdn/simple-shared-worker">shared worker </a>例子中 (<a href="http://mdn.github.io/simple-shared-worker/"> 运行 shared worker</a>)， 我们有两个 HTML 页面， 每个页面中使用一些 JavaScript 来执行简单的计算。 这些脚本使用相同的 shared worker 来执行计算 — 都可以访问这个 worker，即使脚本在不同窗口的两个页面内运行。</p>
...
<p>然后两个脚本都通过 {{domxref("MessagePort")}} 对象来访问 worker，这个对象用{{domxref("SharedWorker.port")}} 属性获得。如果已经用 addEventListener 监听了 onmessage 事件，则可以使用 <code>start()</code> 方法手动启动端口：</p>
...
<ul>
 ...
 <li><a href="/zh-CN/docs/Web/Guide/Performance/Using_web_workers">使用 web worker</a></li>
</ul>
