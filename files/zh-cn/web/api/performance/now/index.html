...
<p>和 JavaScript 中其他可用的时间类函数（比如<code><a href="/zh-CN/docs/JavaScript/Reference/Global_Objects/Date/now">Date.now</a></code>）不同的是，<code>window.performance.now()</code>返回的时间戳没有被限制在一毫秒的精确度内，相反，它们以浮点数的形式表示时间，精度最高可达微秒级。</p>
