...
- queueingStrategy {{optional_inline}}
  ...
  - : 一个可选择定义流的队列策略的对象。这需要两个参数：
    ...
    > **备注：** 你可以自定义 `queueingStrategy`，或者使用 {{domxref("ByteLengthQueueingStrategy")}} 或 {{domxref("CountQueueingStrategy")}} 的示例作为对象的值。如果 没有提供`queueingStrategy`，则默认值与 highWaterMark 为 1 的 `CountQueuingStrategy` 相同。
