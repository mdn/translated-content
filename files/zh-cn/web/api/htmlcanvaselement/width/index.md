...
**`HTMLCanvasElement.width`** 属性是一个正整数（`integer`），表示以 CSS 像素为单位解析的 {{HTMLElement("canvas")}} 元素的 [`width`](/zh-CN/docs/Web/HTML/Element/canvas#width) HTML 属性。当未指定该属性，或者设置为无效值（例如负数）时，将使用默认值 `300`。
