...
<p>在同一个 canvas 上以相同的 <code>contextType</code> 多次调用此方法只会返回同一个上下文。</p>
...
<dl>
 ...
 <ul>
  ...
  <li><code>"webgl"</code> (或<code>"experimental-webgl"</code>) 这将创建一个 {{domxref("WebGLRenderingContext")}} 三维渲染上下文对象。只在实现<a href="https://developer.mozilla.org/en-US/docs/Web/WebGL">WebGL</a> 版本 1(OpenGL ES 2.0) 的浏览器上可用。</li>
  <li>"<code>webgl2</code>" (或 "<code>experimental-webgl2</code>") 这将创建一个 {{domxref("WebGL2RenderingContext")}} 三维渲染上下文对象。只在实现 <a href="https://developer.mozilla.org/en-US/docs/Web/WebGL">WebGL</a> 版本 2 (OpenGL ES 3.0) 的浏览器上可用。{{experimental_inline}}</li>
  <li><code>"bitmaprenderer"</code> 这将创建一个只提供将 canvas 内容替换为指定{{domxref("ImageBitmap")}}功能的{{domxref("ImageBitmapRenderingContext")}}  。</li>
 </ul>
 ...
 <ul>
  ...
  <li>
   <p><strong><code>depth</code></strong>: {{jsxref("boolean")}}值表明绘制缓冲区包含一个深度至少为 16 位的缓冲区。</p>
  </li>
  <li>
   <p><code><strong>failIfMajorPerformanceCaveat</strong></code>: 表明在一个系统性能低的环境是否创建该上下文的{{jsxref("boolean")}}值。</p>
  </li>
  <li>
   <p><code><strong>powerPreference</strong></code>: 指示浏览器在运行 WebGL 上下文时使用相应的 GPU 电源配置。 可能值如下：</p>
  ...
  <li>
   <p><strong><code>premultipliedAlpha</code></strong>: 表明排版引擎将假设绘制缓冲区包含预混合 alpha 通道的{{jsxref("boolean")}}值。</p>
  </li>
  <li>
   <p><strong><code>preserveDrawingBuffer</code></strong>: 如果这个值为{{jsxref("true")}}缓冲区将不会被清除，会保存下来，直到被清除或被使用者覆盖。</p>
  </li>
  <li>
   <p><strong><code>stencil</code></strong>: 表明绘制缓冲区包含一个深度至少为 8 位的模版缓冲区{{jsxref("boolean")}}值。</p>
  </li>
 </ul>
 </dd>
</dl>
...
<p>现在你已经获取到了 2D 画布的渲染上下文 ({{domxref("CanvasRenderingContext2D")}})，可以使用它画你想画的了。</p>
