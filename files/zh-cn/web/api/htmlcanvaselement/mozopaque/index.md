...
非标准的 **`HTMLCanvasElement.mozOpaque`** 属性是一个布尔值，反映了 {{HTMLElement("canvas")}} 元素的 [`moz-opaque`](/zh-CN/docs/Web/HTML/Element/canvas#moz-opaque) HTML 属性。它让画布确定是否需要考虑半透明效果。如果画布确定没有半透明效果，则可以优化绘制性能。
...
- [`moz-opaque`](/zh-CN/docs/Web/HTML/Element/canvas#moz-opaque)：{{HTMLElement("canvas")}} 元素的 HTML 属性
