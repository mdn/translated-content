...
Web 浏览器和搜索引擎可能会使用此信息来更好地理解链接内容的语言，但它们并不一定要遵循此信息。为 `hreflang` 属性提供的值应遵循 {{RFC(5646, "Tags for Identifying Languages (also known as BCP 47)")}} 中定义的格式。如果不符合该格式，则会被忽略。
