...
{{domxref("HTMLVideoElement")}} 接口提供只读的 **`videoHeight`** 属性，指示视频的 [固有高度](#关于固定的宽高) ，使用 CSS 的像素值表示。
简单来说，这是媒体的初始高度。

## 属性

整数值，以 CSS 的像素值指定视频的固有高度。
如果元素的 {{domxref("HTMLMediaElement.readyState", "readyState")}} 是 `HTMLMediaElement.HAVE_NOTHING`，则此属性的值为 0，因为视频和开头图片的大小并不能知道
