...
{{domxref("Element")}} 接口的 **`scroll()`** 方法可将给定元素滚动到内部的一组特定坐标。
...
我们首先获取运行 `scroll()` 操作的 `<button>`、工具栏 `<div>` 以及可滚动的 `<section>` 的引用：
...
function isInterrupted(interrupted) {
  console.log(`滚动已完成；${interrupted ? "已" : "未"}中断`);
  if (interrupted) {
    alert("滚动被中断！");
  }
}
...
点击按钮时，我们立即为工具栏应用 `fade-out` 类，使其淡出。然后对 `<section>` 运行 `scroll(0, 1000)`，使其内容向下滚动 1000 像素，同时等待其 Promise 兑现并将 `result` 存入常量。Promise 兑现后，我们调用 `isInterrupted()` 报告滚动操作已完成以及是否被中断。最后，为工具栏应用 `fade-in` 类，使其再次淡入。
