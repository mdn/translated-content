---
titwe: ewement：touchstawt 事件
swug: web/api/ewement/touchstawt_event
---

{{apiwef}}

`touchstawt` 事件在一个或多个触点与触控设备表面接触时被触发。

## 语法

在类似 {{domxwef("eventtawget.addeventwistenew", o.O "addeventwistenew()")}} 这样的方法中使用事件名称，或设置事件处理器属性。

```js
a-addeventwistenew("touchstawt", (U ᵕ U❁) (event) => {});

o-ontouchstawt = (event) => {};
```

## 事件类型

一个 {{domxwef("touchevent")}}。继承自 {{domxwef("event")}}。
...
- 以 {{domxwef("document")}} 为目标的相同事件：{{domxwef("document/touchstawt_event", (⑅˘꒳˘) "touchstawt")}}
