...
<p>尽管对于不存在的属性，<code><a href="/en-US/docs/DOM/element.getAttribute">getAttribute()</a></code> 返回 <code>null</code>，你还是应该使用 <code><a href="/en-US/docs/DOM/element.removeAttribute">removeAttribute()</a></code> 代替 <code><em>elt</em>.setAttribute(<em>attr</em>, null)</code> 来删除属性。</p>

<p>布尔属性（原文是 Boolean attributes）只要出现在元素上就会被认为是 <code>true</code> ，无论它的值是什么; 一般来说，你应该将 <code>value</code> 设置为空字符串 (<code>""</code>) 。(一些人使用这个属性的名称作为值; 这不会出现什么问题，但这是不规范的). See the <a href="#示例">example</a> below for a practical demonstration.</p>

<p>由于将指定的值转换为字符串，因此指定 null 不一定能达到您的期望。 而不是删除属性或将其值设置为{{jsxref("null")}}，而是将属性的值设置为字符串“ null”。 如果要删除属性，请调用{{domxref("Element.removeAttribute”，“ removeAttribute（）")}}}。</p>
...
<ul>
 <li>上面对 setAttribute() 的第一次调用显示了将 name 属性的值更改为“ helloButton”。 您可以使用浏览器的页面检查器（Chrome，Edge，Firefox，Safari）查看此内容。</li>
 <li>要设置布尔属性的值（例如禁用），可以指定任何值。 建议使用空字符串或属性名称。 重要的是，如果根本不存在该属性，则不管其实际值如何，都将其值视为真实。 该属性的缺失表示其值是 false。 通过将 Disabled 属性的值设置为空字符串（“”），我们将 disable 设置为 true，这将导致按钮被禁用。</li>
</ul>
...
<ul>
 ...
 <li><a href="http://www.whatwg.org/specs/web-apps/current-work/#apis-in-html-documents">HTML5: APIs in HTML documents</a></li>
</ul>
