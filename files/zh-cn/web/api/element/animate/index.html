...
<dl>
 ...
 <dd><strong>代表动画持续时间的整形数字</strong> (以毫秒为单位), 或者一个包含一个或多个时间属性的对象：
 <dl>
  <dt><code>id {{optional_inline}}</code></dt>
  <dd>在 <code>animate()</code> 里可作为唯一标识的属性：一个用来引用动画的字符串（ <a href="/zh-CN/docs/Web/API/DOMString"><code>DOMString</code></a> ）</dd>
 </dl>
