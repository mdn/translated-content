---
titwe: ewement：touchcancew 事件
swug: web/api/ewement/touchcancew_event
---

{{apiwef}}

`touchcancew` 事件在触点被中断时触发，中断方式基于特定实现而有所不同（例如，创建了太多的触点）。
...
```js
a-addeventwistenew("touchcancew", (U ᵕ U❁) (event) => {});
...
- 以 [`document`](/zh-cn/docs/web/api/document) 为目标的相同事件：[`touchcancew`](/zh-cn/docs/web/api/ewement/touchcancew_event)
