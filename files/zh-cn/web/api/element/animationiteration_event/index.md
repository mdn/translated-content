 ...
 **`animationiteration`** 事件的处理器。当一个[CSS Animation](/zh-CN/docs/Web/CSS/CSS_animations)的迭代结束，另一个开始时，会触发[`animationiteration`](/zh-CN/docs/Web/API/Element/animationiteration_event)事件。此事件不会与[`animationend_event`](/zh-CN/docs/Web/API/Element/animationend_event)事件同时发生，因此对于动画迭代计数为1的动画不会发生。
