...
<p><strong><code>Element.innerHTML</code></strong> 属性设置或获取 HTML 语法表示的元素的后代。</p>
...
<p>{{domxref("DOMString")}} 包含元素后代的 HTML 序列。设置元素的 <code>innerHTML</code> 将会删除所有该元素的后代并以上面给出的 htmlString 替代。</p>
...
<ol>
 ...
 <li>如果元素内容被替换成 {{HTMLElement("template")}}  元素，<code>&lt;template&gt;</code> 元素的 {{domxref("HTMLTemplateElement.content", "content")}} 属性会被替换为步骤 1 中创建的新的 <code>DocumentFragment</code>。</li>
...
<p>尽管这看上去像 <a href="https://zh.wikipedia.org/wiki/cross-site_scripting">cross-site scripting</a> 攻击，结果并不会导致什么。HTML 5 中指定不执行由 <code>innerHTML</code> 插入的 {{HTMLElement("script")}} 标签。</p>
...
<div class="warning">
<p><strong>警告</strong>：如果你的项目将要经过各种形式的安全检查的话，使用 <code>innerHTML</code> 可能导致代码被拒绝。例如，如果你在<a href="/en-US/docs/Mozilla/Add-ons/WebExtensions">浏览器扩展</a>中<a href="https://wiki.mozilla.org/Add-ons/Reviewers/Guide/Reviewing#Step_2:_Automatic_validation">使用</a><a href="https://wiki.mozilla.org/Add-ons/Reviewers/Guide/Reviewing#Step_2:_Automatic_validation"> <code>innerHTML</code></a> 并将扩展提交到 <a href="https://addons.mozilla.org/">addons.mozilla.org</a> 的话，它将会在自动审核过程中被拒绝。</p>
</div>
