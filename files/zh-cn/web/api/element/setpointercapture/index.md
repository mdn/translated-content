...
{{domxref("Element")}}接口的 **`setPointerCapture()`** 方法用于将特定元素指定为未来指针事件的捕获目标。指针的后续事件将针对捕获元素，直到捕获被释放（通过 {{domxref("Element.releasePointerCapture()")}} 或触发 {{domxref("Element/pointerup_event", "pointerup")}} 事件）。
