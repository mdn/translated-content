...
<p>返回一个 {{domxref("Selection")}} 对象，表示用户选择的文本范围或光标的当前位置。</p>
...
<p>在  JavaScript 中，当一个对象被传递给期望字符串作为参数的函数中时（如 {{ Domxref("window.alert") }} 或 {{ Domxref("document.write") }}），对象的{{jsxref("Object.toString","toString()")}}方法会被调用，然后将返回值传给该函数。</p>

<p>在上例中，<code>selObj.toString()</code> 会在传递到 {{domxref("window.alert()")}}时自动调用。然而，当你试图在 {{domxref("Selection")}} 对象上使用一个 JavaScript 的{{jsxref("Global_Objects/String", "String")}} 对象上的属性或者方法时（如 {{jsxref("String.prototype.length")}} 或者 {{jsxref("String.prototype.substr()")}}），会导致错误（如果没有相应的属性或方法时）或返回不是期望的结果（如果存在相应的属性或方法）。如果想要将 <code>Selection</code> 对象作为一个字符串使用，请直接调用 <code>toString()</code> 方法：</p>
...
<p>值得注意的是，目前在 Firefox, Edge (非 Chromium 版本) 及 Internet Explorer 中，<code>getSelection()</code> 对 {{htmlelement("textarea")}} 及 {{htmlelement("input")}} 元素不起作用。 {{domxref("HTMLInputElement.setSelectionRange()")}} 或 <code>selectionStart</code> 及 <code>selectionEnd</code> 属性可用于解决此问题。</p>
...
<ul>
 <li><a href="https://dvcs.w3.org/hg/editing/raw-file/tip/editing.html#selections">HTML Editing API</a></li>
