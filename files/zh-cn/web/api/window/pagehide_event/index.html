...
<p>当浏览器在显示与会话历史记录不同的页面的过程中隐藏当前页面时，<strong><code>pagehide</code></strong>(页面隐藏) 事件会被发送到一个{{domxref("Window")}} 。例如，当用户单击浏览器的“后退”按钮时，当前页面在显示上一页之前会收到一个<code>pagehide</code>(页面隐藏) 事件。</p>
