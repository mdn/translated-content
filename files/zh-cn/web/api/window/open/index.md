...
- `windowFeatures` {{optional_inline}}
  ...
  - : 一个字符串，包含以逗号分隔的窗口特性列表，形式为 `name=value`，布尔特性则仅为 `name`。这些特性包括窗口的默认大小和位置、是否打开最小弹出窗口等选项。支持以下选项：
    ...
    - `popup`
      ...
      - : 如果启用此功能，则要求使用最小弹出窗口。弹出窗口中包含的用户界面功能将由浏览器自动决定，一般只包括地址栏。
...
另外，下面的示例演示了如何使用 `popup` 功能打开弹出窗口。
...
在某些情况下，JavaScript 被禁用或不可用，`window.open()` 将无法工作。我们可以提供替代解决方案，使网站或应用程序仍能正常运行，而不是完全依赖于该功能的存在。
