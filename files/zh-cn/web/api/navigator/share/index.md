...
- `data` {{optional_inline}}
  - : 包含要共享的数据的对象。用户代理未知的属性将被忽略；共享数据仅根据用户代理理解的属性进行评估。所有属性都是可选的，但必须至少指定一个已知的数据属性。可能的值为：
    ...
    - `files` {{optional_inline}}
      - : 表示要共享的文件的 {{domxref("File")}} 对象数组。有关可共享的文件类型，请参阅[下文](#shareable_file_types)。

### 返回值

一个兑现为 `undefined` 的 {{jsxref("Promise")}}，或者被下面给出的[异常](#exceptions)之一拒绝。

### 异常

{{jsxref("Promise")}} 可能会被拒绝并带有以下 `DOMException` 值之一：
...
此方法要求当前文档具有 [web-share](/zh-CN/docs/Web/HTTP/Headers/Permissions-Policy/web-share) 权限策略和{{Glossary("transient activation", "瞬态激活")}}。（它必须由 UI 事件触发，例如单击按钮，并且不能由脚本在任意点启动。）此外，该方法必须指定本机实现的支持共享的有效数据。
...
- <https://wpt.live/web-share/>（网络平台测试）
