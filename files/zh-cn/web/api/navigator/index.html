...
<p>可以使用只读的 {{domxref("window.navigator")}} 属性检索 navigator 对象。</p>
...
<dl>
 ...
 <dd>返回 {{domxref("Boolean")}} 表明浏览器是否支持 Java。</dd>
 ...
 <dd>返回 {{domxref("DOMString")}} 表示用户的首先语言，通常是浏览器用户界面的语言。当未知的时，返回 null。</dd>
 ...
 <dd>返回 {{domxref("MimeTypeArray")}} 数组用于列举浏览器所支持的 MIME 类型。</dd>
 ...
 <dd>返回一个 {{domxref("Permissions")}} 对象，该对象可用于查询和更新 <a href="/en-US/docs/Web/API/Permissions_API">Permissions API</a> 涵盖的 API 的权限状态。</dd>
...
<h3 id="Methods">非标准方法</h3>

<dl>
 <dt>{{domxref("window.navigator.buildID", "navigator.buildID")}} {{non-standard_inline}}</dt>
 <dd>返回浏览器识别码。这一方法返回时间戳，例如：在 Firefox 64 发行版中返回 <code>20181001000000</code>。</dd>
 <dt>{{domxref("Navigator.cookieEnabled")}} {{non-standard_inline}}</dt>
 <dd>返回布尔值以表明 Cookies 是否能再浏览器中启用</dd>
 <dt>{{domxref("navigator.doNotTrack")}} {{non-standard_inline}}</dt>
 <dd>报告用户的不追踪参数值，当值为 yes，你的网址或应用将不追踪用户</dd>
 <dt>{{domxref("navigator.id")}} {{non-standard_inline}}</dt>
 <dd>返回 {{domxref("window.navigator.id", "id")}} 对象，你能用 <a href="/en-US/docs/BrowserID">BrowserID</a> 添加支持到你的网址。</dd>
 <dt>{{domxref("window.navigator.mozApps", "navigator.mozApps")}} {{non-standard_inline}}</dt>
 <dd>Returns an {{domxref("window.navigator.mozApps", "Apps")}} object you can use to install, manage, and control <a href="/Open_Web_Apps">Open Web apps</a>.</dd>
 ...
 <dt><a href="/en-US/docs/API/Pointer_Lock_API"><code>navigator.webkitPointer</code></a> {{non-standard_inline}}</dt>
 <dd>Returns a PointerLock object for the <a href="/en-US/docs/API/Pointer_Lock_API">Mouse Lock API</a>.</dd>
</dl>
...
<dl>
 ...
 <dt>{{domxref("NavigatorID.taintEnabled()")}} {{deprecated_inline("1.7.8")}} {{experimental_inline}}</dt>
