...
{{domxref("Navigator")}} 接口的 **`vibrate()`** 方法会使得设备上的振动硬件发出振动（如果存在此类硬件）。如果设备不支持振动，则此方法无效。如果调用此方法时振动模式已经在进行中，则前一个模式将停止，新的模式将开始。
...
需要[粘性用户激活](/zh-CN/docs/Web/Security/User_activation)。用户必须与页面或 UI 元素进行交互才能使此功能发挥作用。
