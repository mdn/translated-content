...
<p>举个例子，此 API 允许 Web 邮件站点打开 <code>mailto:</code> URL，或让  <abbr>VoIP</abbr>  站点打开 <code>tel:</code> URL。</p>
...
<dl>
 ...
 <dt><code>url</code></dt>
 <dd>处理器的 URL，string 类型。这个字符串应该包含一个"%s"的占位符，其会被将要受理的文档的 <a href="/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/encodeURIComponent">escaped</a> 链接所替换。这个链接（译者按：指将要受理的文档的 escaped 链接，也就是替换占位符的字符串）可能是一个真实的 URL，或者是一个电话号码，邮件地址之类的。
 ...
 <dd>一个用户可理解的处理器标题。标题会展示给用户，例如弹出对话框“允许这个站点处理 [scheme] 链接吗？”或者在浏览器设置中列出注册的处理器时。
 <div class="blockIndicator note">
