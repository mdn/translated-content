...
<p>Web Audio API 的一个强大之处在于，它没有任何严格的声音呼叫控制。比如说，在同一时间它没有呼叫 32 或 64 的声音的限制。如果你的处理器性能好的话，同一时间播放 1000 多的声音不卡顿也是有可能的。这充分显示真正的进步，要知道几年前中高频的声卡仅能处理小部分的负载。</p>
</div>
...
<p>查看最终 demo 代码<a href="https://codepen.io/Rumyra/pen/qyMzqN/"> here on Codepen</a>，或者在 <a href="https://github.com/mdn/webaudio-examples/tree/master/audio-basics">GitHub 查看源代码 on GitHub</a>。</p>

<h2 id="浏览器支持">浏览器支持</h2>

<p>现代浏览器的 Web Audio API 对的大多数功能都有很好的支持。API 有很多的功能，因此要获得更准确的信息，你必须检查每个参考页面底部的浏览器兼容表。</p>
...
<p>为了能通过 Web Audio API 执行任何操作，我们需要创建音频上下文实例。这能让我们访问 API 所有的特性和功能。</p>
...
<p>现在，需要通过我们创建的音频上下文播放一些声音。Web Audio API 中有几种方法可以实现这一点。让我们通过一个简单的方法开始 — 因为我们有一个音箱，我们可能想播放一首完整的歌曲。 此外，为了便于访问，我们可以在在 DOM 中暴露该音轨。我们将使用 {{htmlelement("audio")}} 元素在页面上暴露这首歌曲。</p>
...
<p>为了使用 Web Audio API 的优秀特性，我们需要从该元素中获取源并将其传入我们创建的上下文中。幸运的是，有一个方法可以让我们做到这一点 — {{domxref("AudioContext.createMediaElementSource")}}:</p>
...
<div class="note">
<p><strong>注意</strong>：上面的 <code>&lt;audio&gt;</code> 元素在 DOM 中代表了一个{{domxref("HTMLMediaElement")}} 类型的对象，拥有其自身的一组功能。这一切都将保持不变。我们只是让 Web Audio API 能够访问到声音。</p>
</div>
...
<p>这些特殊的要求基本上是因为意外的声音可能会打扰到用户，令人厌烦，并且可能导致可访问性问题。你可以在文章 <a href="/en-US/docs/Web/Media/Autoplay_guide">媒体与 Web 音频 API 自动播放指南</a> 了解更多相关信息。</p>
...
<p>我们已经通过把音频元素传入 API 生成一个输入节点。在大多数情况下，你不需要生成一个输出节点，你只需要将其他节点连接到可以为你处理这种情况的 {{domxref("BaseAudioContext.destination")}}：</p>
...
<h2 id="关于Web_Audio编辑器">关于 Web Audio 编辑器</h2>

<p>Firefox 有一个名为 <a href="/en-US/docs/Tools/Web_Audio_Editor">Web Audio editor</a> 的工具。在其上运行音频图的任何页面上，你可以打开开发者工具，使用 Web Audio 选项卡查看音频图，可查看每个节点的可用属性，并可以修改这些属性来查看会有什么效果。</p>
...
<div class="note">
<p><strong>注意</strong>：Web Audio 编辑器默认不是开启的，你需要打开 Firefox developer tools 设置，选中 Default Developer Tools 部分中的 Web Audio 复选框来显示它。</p>
</div>

<h2 id="修改声音">修改声音</h2>

<p>让我们深入研究一些基本的修改节点以改变我们的声音。这就是 Web Audio API 真正开始派上用场的地方。首先，让我们改变音量。这可以通过  {{domxref("GainNode")}} 实现，它表示我们的声波有多大。</p>
...
<div class="note">
<p><strong>注意</strong>： <code>StereoPannerNode</code> 用于你只想从左到右进行立体声平移的简单情况。还有一个  {{domxref("PannerNode")}}， 它允许对 3D 空间或声音空间化进行大量控制以创建更复杂的效果。这在游戏和 3D 应用程序中生成小鸟飞过头顶或者来自用户身后的声音。</p>
</div>
...
<p>剩下要做的就是试试这个应用程序：<a href="https://codepen.io/Rumyra/pen/qyMzqN/">查看 Codepen 上的最终演示</a>。</p>

<h2 id="摘要">摘要</h2>

<p>好的，我们拥有一个音箱播放我们的“磁带”，我们可以调整音量和立体声声像，给我们提供了一个相当基本的工作音频图表。</p>

<p>这构成了开始向你的网站或 Web 应用添加音频所需的很少的几个基础知识。Web Audio API 还有很多功能，但一旦你掌握了节点的概念及将音频节点图联系在一起，我们可以继续研究更加复杂的功能。</p>

<h2 id="更多例子">更多例子</h2>

<p>还有其他示例可以了解有关 Web Audio API 的更多信息。</p>

<p> <a href="https://github.com/mdn/voice-change-o-matic">Voice-change-O-matic</a> 是一个有趣的语音操纵器和音频可视化 web 应用程序，允许你选择不同的效果和可视化。该应用程序相当初级，但它演示了同时使用多个 Web Audio API 特性（<a href="https://mdn.github.io/voice-change-o-matic/">运行 Voice-change-O-matic live</a>）。</p>
...
<p>另一个专门用于演示 Web Audio API 的例子是 <a href="http://mdn.github.io/violent-theremin/">Violent Theremin</a>， 一个允许你通过移动鼠标来改变它的音调音量的简单的应用程序。它还提供了一个迷幻的灯光秀（<a href="https://github.com/mdn/violent-theremin">查看 Violent Theremin 源代码</a>）</p>

<p><img alt="A page full of rainbow colours, with two buttons labeled Clear screen and mute. " src="violent-theremin.png"></p>
...
<p>1. 构建音频上下文 AudioContext 对象；</p>

<p>2. 在 AudioContext 对象内，构建音源，比如&lt;audio&gt;，oscillator，stream</p>

<p>3. 构建效果节点 effectNode，比如混响，双二阶滤波器，声相，压限器</p>

<p>4. 选择最终的音频目的地，比如说你的系统扬声器</p>

<p>5. 连接源到效果，效果到输出终端</p>

<h3 id="构建AudioContext对象">构建 AudioContext 对象</h3>

<p>首先，你需要构建一个 AudioContext 实例，来创建一个音频图。最简单的方法就像这样：</p>

<pre class="brush: js">var audioCtx = new AudioContext();
</pre>

<div class="note">
<p><strong>注释</strong>: 同样一个文档是可以存在多个 audioContext 对象的，但是比较浪费。</p>
</div>
...
<div class="note">
<p><strong>注释</strong>:当创建一个新的 conText 对象时，如果你不提示 window 对象，Safari 会无效。</p>
</div>

<h3 id="创建AudioSource">创建 AudioSource</h3>

<p>现在我们有了 AudioContext，可以用这个来做很多事。第一件我们需要做的事是玩音乐。音频可以来自于多样的地方：</p>

<ul>
 <li>通过 JavaScript 直接生成一个音频节点比如 oscillator. 一个 {{ domxref("OscillatorNode") }}是利用{{ domxref("AudioContext.createOscillator") }} 方法来构建。</li>
 <li>从原 PCM 数据构建: AudioContext 有解密被支持的音频格式的多种方法。 看 {{ domxref("AudioContext.createBuffer()") }}, {{ domxref("AudioContext.createBufferSource()") }}, 以及 {{ domxref("AudioContext.decodeAudioData()") }}.</li>
 <li>来自于 HTML 音频元素例如 {{HTMLElement("video")}} 或者{{HTMLElement("audio")}}: 可以看 {{ domxref("AudioContext.createMediaElementSource()") }}.</li>
 <li>直接来自于 <a href="https://developer.mozilla.org/en-US/docs/WebRTC">WebRTC</a>，{{domxref("MediaStream")}} 例如来自于摄像头或麦克风。可以看{{ domxref("AudioContext.createMediaStreamSource()") }}.</li>
</ul>
...
<div class="note">
<p><strong>注释</strong>: 为了直接播放一个音乐文件，你通常通过 XHR 来加载文件，通过 Buffer 来解码，创建 BufferSource. 看这个 <a href="https://github.com/mdn/voice-change-o-matic/blob/gh-pages/scripts/app.js#L48-L68">例子来自于 Voice-change-O-matic</a>.</p>
</div>

<div class="note">
<p><strong>注释：</strong> Scott Michaud 已经写了一个有用的库来加载和解码一个或多个音频实例，被称为 <a href="https://github.com/ScottMichaud/AudioSampleLoader">AudioSampleLoader</a>. 这个可以帮助简化 XHR/buffering 的处理操作。</p>
</div>
...
<p>你的设备的默认输出结构（通常是你的设备扬声器），通过{{ domxref("AudioContext.destination") }}来允许进入。为了连接 oscillator，gain node 以及输出端，如以下运用：</p>
...
<p><img alt="" src="https://mdn.mozillademos.org/files/7949/voice-change-o-matic-graph.png" style="display: block; height: 563px; margin: 0px auto; width: 232px;">你也可以链接多个节点到一个节点，比如说你想要混合多个音频源在一起，就让它们都通过一个效果节点，比如 gain node。</p>

<div class="note">
<p><strong>注释</strong>:Firefox32 以上版本已有完整的 firefox 开发者工具包括 <a href="/en-US/docs/Tools/Web_Audio_Editor">Web Audio Editor</a>, 一个对测试 web audio 表的 bug 非常有用的东西。</p>
</div>

<h3 id="播放音乐及设置音调">播放音乐及设置音调</h3>

<p>现在 audio 节点图已经建立，我们可以设置属性值及调用音频节点的方法来调节想要的音效。在这个简单的例子，我们可以设置特殊的音调，以赫兹为单位，设置为特殊类型，以及指示音乐播放：</p>
...
<h3 id="简单的canvas可视化">简单的 canvas 可视化</h3>

<p>每次鼠标的移动，canvasDraw() 方法会被调用，鼠标停留的位置会画出一个多圆圈组成的小簇，它的大小以及颜色会基于 frequency/gain 的值。</p>
...
<h3 id="theremin的静音">theremin 的静音</h3>

<p>当静音按钮点击，以下方法会被调用，disconnect 方法，将切断 gain node 与 destination 节点的链接，有效阻止了节点图的链接，所以没有声音会被产生。再次点击效果相反。</p>
...
<p>这里有许多通过 Web Audio API 来构建的节点，一个好消息就是，总体来说，正如我们所见，他们用同一种方法工作：构建节点，连接到图表的另一个节点，然后处理节点属性以及方法来作用于你想要的音源。</p>
...
<p>这个对象一定会数学化的定义 wave shape，一个被应用于基础声音波来创造扭曲的效果。这些波并不好被计算，最好的开始方法是搜索 web 算法。比如，我们可以从 <a href="http://stackoverflow.com/questions/22312841/waveshaper-node-in-webaudio-how-to-emulate-distortion">Stack Overflow</a> 找到：</p>
...
<p>我们在这里详述了过滤器的类型，频率值，增幅值。在 lowshelf 过滤器情况，所有的指定频率拥有 25 分贝的增幅值。</p>

<h2 id="Web_Audio_API的其他"> Web Audio API 的其他</h2>

<p>Web Audio API 可以做不仅仅音频可视化及专业化（如 panning）的事情。我们将会在之后的文章涉及其他的更多内容。</p>
