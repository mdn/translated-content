...
在 `<script type="speculationrules">` 元素或 `Speculation-Rules` 标头中包含 `prefetch` 规则，支持它的浏览器就会下载引用页面的响应体，但不会下载被页面所引用的子资源。当导航到预取的页面时，它的加载速度会比没有预取时快很多。
...
当进行预取时，浏览器通过单个 GET 请求下载引用页面的响应体，用户可能会在将来某个时间导航到该页面。当请求的 URL 执行服务器段发起的副作用，而你不希望它在导航到该 URL 之前发生时，尤其容易出现问题。
...
- URL 是[不安全预取](#不安全的预取)的。如果你还没有阅读前一部分，请先阅读并理解这些条件同样适用于不安全的预渲染。
