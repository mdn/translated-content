...
**`MediaSource.isTypeSupported()`** 静态方法返回一个布尔值，如果给定的 MIME 类型和（可选）编解码器*可能*被当前的 {{Glossary("user agent")}} 支持，则返回 `true`。
...
如果浏览器*或许*可以播放给定的媒体类型，则返回 `true`。这不能得到保证，并且你必须为你的代码可能为媒体无法正常播放做好准备。
