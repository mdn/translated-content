...
**`DataTransferItem`** 对象表示一个拖放项。在一个*拖动操作*中，每一个 {{domxref("DragEvent")}} 都有包含存有拖放数据的{{domxref("DataTransferItemList","列表", "", 1)}} 的 {{domxref("DragEvent.dataTransfer","dataTransfer")}} 属性，其中每一项都是一个 `DataTransferItem` 。
...
- {{domxref("DataTransferItem.getAsFile()")}}
  - : 返回一个关联拖动项的 {{domxref("File")}} 对象（当拖动项不是一个文件时返回 null）。
- {{domxref("DataTransferItem.getAsString()")}} {{Experimental_Inline}}
