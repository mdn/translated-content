...
**`PageTransitionEvent()`** 构造函数创建一个新 {{domxref("PageTransitionEvent")}} 对象，用于 {{domxref("Window/pageshow_event", "pageshow")}} 或 {{domxref("Window/pagehide_event", "pagehide")}} 事件，当页面加载或卸载时在 {{domxref("window")}} 对象上触发。
...
- `type`
  - : 一个表示事件名称的字符串。
    该字符串大小写敏感，浏览器通常设置为 `pageshow` 或 `pagehide`。
- `options` {{optional_inline}}
  - : 一个对象，除在 {{domxref("Event/Event", "Event()")}} 定义的属性外，还具有以下属性：
