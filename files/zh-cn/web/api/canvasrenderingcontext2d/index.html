...
<p><code><strong>CanvasRenderingContext2D</strong></code>接口是 Canvas API 的一部分，可为{{HTMLElement("canvas")}}元素的绘图表面提供 2D 渲染上下文。它用于绘制形状，文本，图像和其他对象。</p>

<p>请参阅侧边栏和下方的界面属性和方法。 <a href="/zh-CN/docs/Web/API/Canvas_API/Tutorial">Canvas 教程</a>提供了更多的信息，例子和资源。</p>

<h2 id="基础示例">基础示例</h2>

<p>要获得<code>CanvasRenderingContext2D</code> 实例，您必须首先具有 HTML <code>&lt;canvas&gt;</code>元素才能使用：</p>

<pre class="brush: html notranslate">&lt;canvas id="my-house" width="300" height="300"&gt;&lt;/canvas&gt;
</pre>

<p>要获取画布的 2D 渲染上下文，请在<code>&lt;canvas&gt;</code>元素上调用{{domxref("HTMLCanvasElement.getContext()", "getContext()")}}，并提供<code>'2d'</code>作为参数：</p>
...
<dl>
 ...
 <dd>阴影的颜色。 默认 fully-transparent black.</dd>
...
<dl>
 ...
 <dd>使用直线连接子路径的最后的点到 x,y 坐标。</dd>
 <dt>{{domxref("CanvasRenderingContext2D.bezierCurveTo()")}}</dt>
 <dd>添加一个 3 次贝赛尔曲线路径。该方法需要三个点。 第一、第二个点是控制点，第三个点是结束点。起始点是当前路径的最后一个点，绘制贝赛尔曲线前，可以通过调用 <code style="font-style: normal;">moveTo()</code> 进行修改。</dd>
 <dt>{{domxref("CanvasRenderingContext2D.quadraticCurveTo()")}}</dt>
 <dd>添加一个 2 次贝赛尔曲线路径。</dd>
 ...
 <dd>根据控制点和半径绘制圆弧路径，使用当前的描点 (前一个 moveTo 或 lineTo 等函数的止点)。根据当前描点与给定的控制点 1 连接的直线，和控制点 1 与控制点 2 连接的直线，作为使用指定半径的圆的<strong>切线</strong>，画出两条切线之间的弧线路径。</dd>
...
<dl>
 ...
 <dd>从当前路径创建一个剪切路径。在  <strong>clip() </strong>调用之后，绘制的所有信息只会出现在剪切路径内部。例如：参见 Canvas 教程中的 <a href="/en-US/docs/Web/API/Canvas_API/Tutorial/Compositing">剪切路径</a> 。</dd>
...
<p>在 <strong>CanvasRenderingContext2D </strong>渲染背景中的对象会有一个当前的变换矩阵，一些方法可以对其进行控制。当创建当前的默认路径，绘制文本、图形和{{domxref("Path2D")}}对象的时候，会应用此变换矩阵。下面列出的方法保持历史和兼容性的原因，是为了{{domxref("SVGMatrix")}}对象现在能够应用于大部分 API ，将来会被替换。</p>
...
<dl>
 ...
 <dd>根据 x 水平方向和 y 垂直方向，为 canvas 单位添加缩放变换。</dd>
 ...
 <dd>重新设置当前的变换为单位矩阵，并使用同样的变量调用 <strong>transform() </strong>方法。</dd>
...
<dl>
 ...
 <dd>返回一个 {{domxref("ImageData")}} 对象，用来描述 canvas 区域隐含的像素数据，这个区域通过矩形表示，起始点为<em>(sx, sy)、</em>宽为<em>sw、</em>高为<em>sh</em>。</dd>
...
<h2 id="canvas_状态">canvas 状态</h2>

<p><strong>C</strong><strong>anvasRenderingContext2D</strong>渲染环境包含了多种绘图的样式状态（属性有线的样式、填充样式、阴影样式、文本样式）。下面的方法会帮助你使用这些状态：</p>

<dl>
 <dt>{{domxref("CanvasRenderingContext2D.save()")}}</dt>
 <dd>使用栈保存当前的绘画样式状态，你可以使用 <strong>restore()</strong> 恢复任何改变。</dd>
 <dt>{{domxref("CanvasRenderingContext2D.restore()")}}</dt>
 <dd>恢复到最近的绘制样式状态，此状态是通过 <strong>save()</strong> 保存到”状态栈“中最新的元素。</dd>
...
<h2 id="不标准的_APIs">不标准的 APIs</h2>

<h3 id="临时的和_WebKit_内核">临时的和 WebKit 内核</h3>
...
<h3 id="WebKit_特有的">WebKit 特有的</h3>
...
<h3 id="Gecko_特有的">Gecko 特有的</h3>

<dl>
 <dt>{{non-standard_inline}} {{domxref("CanvasRenderingContext2D.filter")}}</dt>
 <dd>CSS 和 SVG 滤镜 作为 Canvas APIs。 可能在新版本的规范中会标准化。</dd>
</dl>

<h4 id="有前缀的_APIs">有前缀的 APIs</h4>
...
<dl>
 ...
 <dd>应用的 <a href="http://cairographics.org/manual/cairo-cairo-t.html#cairo-fill-rule-t">填充规则</a> 。必须是 <code>evenodd</code> 或者 <code>nonzero</code> (默认)。</dd>
