...
Canvas 2D API 的 **`CanvasRenderingContext2D.scale()`** 方法用于根据 x 水平方向和 y 垂直方向，为 canvas 单位添加缩放变换。

默认情况中，在 canvas 中一个单位实际上就是一个像素。例如，如果我们将 0.5 作为缩放因子，最终的单位会变成 0.5 像素，并且形状的尺寸会变成原来的一半。相似的方式，我们将 2.0 作为缩放因子，将会增大单位尺寸变成两个像素。形状的尺寸将会变成原来的两倍。
