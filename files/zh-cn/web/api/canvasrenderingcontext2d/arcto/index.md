...
> **备注：** 当使用相对较大的半径时，可能会得到意外的结果：连接圆弧的直线将以必要的方向走向指定的半径。
...
假设在调用 `arcTo()` 方法时，<em>P<sub>0</sub></em> 是路径上的点，<em>P<sub>1</sub></em> = (`x1`, `y1`) 和 <em>P<sub>2</sub></em> = (`x2`, `y2`) 分别是第一个和第二个控制点，_r_ 是调用中指定的 `radius`：
...
理解 `arcTo()` 方法的一种方式是想象两条直线段：一条从起始点到第一个控制点，另一条从第一个控制点到第二个控制点。如果没有 `arcTo()` 方法，这两条线段会形成一个尖角：`arcTo()` 方法在这个角落创建一个圆弧，并使其平滑连接。换句话说，这个圆弧与两条线段都是切线的。
...
在这个例子中，`arcTo()` 创建的路径是**粗黑色**的。切线是灰色的，控制点是红色的，起始点是蓝色的。

{{ EmbedLiveSample('`arcTo()` 方法的工作原理', 315, 165) }}

### 创建圆角

这个例子使用 `arcTo()` 方法创建了一个圆角。这是该方法最常见的用法之一。
...
圆弧从由 `moveTo()` 指定的点开始：(230, 20)。它的形状适应了控制点 (90, 130) 和 (20, 20)，半径为 50。`lineTo()` 方法将圆弧与点 (20, 20) 用直线连接起来。请注意，圆弧的第二个控制点和 `lineTo()` 指定的点是相同的，这样可以得到一个完全平滑的角落。
...
如果使用相对较大的半径，圆弧可能出现在意料之外的位置。
在这个例子中，圆弧的连接线会在指定的 `moveTo()` 坐标上方而不是下方。这是因为半径太大，圆弧无法完全适应在起始点下方。
...
这个演示展示了使用 `arcTo()` 方法构建路径的过程。在这个演示中，有半无限直线和以中心点 _C_ 为圆心、在 <em>T<sub>1</sub></em> 和 <em>T<sub>2</sub></em> 处与直线相切的圆，用来确定 `arcTo()` 方法生成的路径。
...
除了可以通过滑块设置弧度半径外，初始点 <em>P<sub>0</sub></em> 和控制点 <em>P<sub>1</sub></em>、<em>P<sub>2</sub></em> 可以通过按住鼠标左键拖动来移动。数值也可以直接编辑，使用箭头键可以改变处于焦点下划线标记的元素。
...
在这个例子中，你可以通过调整弧度半径来观察路径的变化。路径是从起始点 _p0_ 开始使用 `arcTo()` 方法绘制的，控制点为 _p1_ 和 _p2_，弧度半径从 0 变化到滑块选定的最大半径。然后通过 `lineTo()` 方法将路径连接至 _p2_ 完成绘制。
