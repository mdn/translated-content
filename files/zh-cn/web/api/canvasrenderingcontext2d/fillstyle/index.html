...
<p><strong><code>CanvasRenderingContext2D.fillStyle</code></strong> 是 Canvas 2D API 使用内部方式描述颜色和样式的属性。默认值是 <code>#000</code> （黑色）。</p>
...
<h3 id="Using_the_fillStyle_property">使用 <code>fillStyle</code> 属性设置不同的颜色</h3>
...
<p>在这个例子中， 我们使用两个 <code>for</code> 循环绘制一个矩形表格，每个单元格都有不同的颜色。 最终的结果图像看起来像屏幕截图，其实没有令人惊叹的事情发生。我们使用两个变量 i 和 j 为每一个单元格生成唯一的 RGB 颜色，并且只改变红色和绿色的数值。蓝色通道的值是固定不变的。通过修改这些通道，你能生成各种各样的调色板。通过逐步地增加，你能实现类似 Photoshop 的调色板。</p>
...
<p>{{EmbedLiveSample("fillStyle_使用_for_循环的例子", 160, 160, "https://mdn.mozillademos.org/files/5417/Canvas_fillstyle.png")}}</p>
