...
<p>一个 <strong>微任务（microtask）</strong>就是一个简短的函数，当创建该函数的函数执行之后，<em>并且</em> 只有当 Javascript 调用栈为空，而控制权尚未返还给被 {{Glossary("user agent")}} 用来驱动脚本执行环境的事件循环之前，该微任务才会被执行。 事件循环既可能是浏览器的主事件循环也可能是被一个 <a href="/en-US/docs/Web/API/Web_Workers_API">web worker</a> 所驱动的事件循环。这使得给定的函数在没有其他脚本执行干扰的情况下运行，也保证了微任务能在用户代理有机会对该微任务带来的行为做出反应之前运行。</p>

<p>JavaScript 中的 <a href="/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise">promises</a> 和 <a href="/en-US/docs/Web/API/Mutation_Observer_API">Mutation Observer API</a> 都使用微任务队列去运行它们的回调函数，但当能够推迟工作直到当前事件循环过程完结时，也是可以执行微任务的时机。为了允许第三方库、框架、polyfills 能使用微任务，{{domxref("Window")}} 暴露了 {{domxref("queueMicrotask()")}} 方法，而 {{domxref("Worker")}} 接口则通过 <code>WindowOrWorkerGlobalScope</code> mixin 提供了同名的 queueMicrotask() 方法。</p>
...
<ul>
 ...
 <li>执行到一个由 {{domxref("setTimeout()")}} 或 {{domxref("setInterval()")}} 创建的 timeout 或 interval，以致相应的回调函数被添加到任务队列时。</li>
</ul>
...
<p>其次，如果一个微任务通过调用  {{domxref("queueMicrotask()")}}, 向队列中加入了更多的微任务，则那些新加入的微任务 <em>会早于下一个任务运行</em> 。这是因为事件循环会持续调用微任务直至队列中没有留存的，即使是在有更多微任务持续被加入的情况下。</p>
...
<p>就其本身而言，应该使用微任务的典型情况，要么只有在没有其他办法的时候，要么是当创建框架或库时需要使用微任务达成其功能。虽然在过去要使得入列微任务成为可能有可用的技巧（比如创建一个立即 resolve 的 promise），但新加入的 {{domxref("queueMicrotask()")}} 方法增加了一种标准的方式，可以安全的引入微任务而避免使用额外的技巧。</p>
...
<p>通过在两种情况下各自都通过一个微任务（<code>if</code> 中用的是 <code>queueMicrotask()</code> 而 <code>else</code> 子句中通过 {{domxref("fetch()")}} 使用了 promise）处理了设置 <code>data</code> 和触发 <code>load</code> 事件，平衡了两个子句。</p>
...
<p>当微任务运行之时，等待它处理的可能是一个有若干条消息的数组。微任务函数先是通过 {{jsxref("JSON.stringify()")}} 方法将消息数组编码为 JSON。其后，数组中的内容就不再需要了，所以清空 <code>messageQueue</code> 数组。最后，使用 {{domxref("fetch()")}} 方法将编码后的 JSON 发往服务器。</p>
...
<p>在下面的代码中，我们看到对 {{domxref("queueMicrotask()")}} 的一次调用被用来调度一个微任务以使其运行。这次调用包含了 <code>log()</code>，一个简单的向屏幕输出文字的自定义函数。</p>
