...
{{domxref("AuthenticatorAssertionResponse")}} 接口的只读属性 **`userHandle`** 返回一个 {{jsxref("ArrayBuffer")}} 对象，能为给定用户提供不透明的标识符。依赖方服务器可以使用这类标识符将用户账户与其对应的凭据和其他数据相关联。

此值在传递给原始的 {{domxref("CredentialsContainer.create()","navigator.credentials.create()")}} 调用的选项中指定为 `user.id`。

## 值

表示当前用户的标识符的 {{jsxref("ArrayBuffer")}} 对象。该标识符不应被人阅读。依赖方应确保传递到原始 `create()` 调用中的 `user.id` **不包含任何个人身份信息**（例如用户名、电子邮件或电话号码）。
