...
---
<p>HTML5 引入了{{DOMxRef("History.pushState","pushState()")}}和{{DOMxRef("History.replaceState","replaceState()")}}方法，分别用于添加和修改历史记录。这些方法与{{domxref("Window.onpopstate","onpopstate")}} 事件一起工作。</p>
...
<p>假设 <code>http://mozilla.org/foo.html</code> 执行下面的 JavaScript:</p>
...
<p>Suppose <code>http://mozilla.org/foo.html</code> executes the following JavaScript:</p>
