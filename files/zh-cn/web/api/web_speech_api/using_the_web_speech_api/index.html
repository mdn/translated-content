...
---
<p>Web Speech API 提供了两类不同方向的函数——语音识别和语音合成 (也被称为文本转为语音，英语简写是 tts)——开启了有趣的新可用性和控制机制。这篇文章提供了这两个方向的简单介绍，并且都带有例子。</p>
...
<p>Web Speech API 有一个主要的控制接口——<a href="https://developer.mozilla.org/en-US/docs/Web/API/SpeechRecognition"><code>SpeechRecognition</code></a>， 外加一些如表示语法、表示结果等等亲密相关的接口。通常，设备都有可使用的默认语音识别系统，大部分现代操作系统使用这个语音识别系统来处理语音命令，比如 Mac OS X 上的 Dictation，iOS 上的 Siri，Win10 上的 Cortana，Android Speech 等等。</p>

<p>Demo</p>

<p>为了简单展示 Web speech recognition 的作用，我们写了一个 demo——<a href="https://github.com/mdn/web-speech-api/tree/master/speech-color-changer">Speech color changer</a>。点击屏幕之后，说出 HTML 颜色关键字 (网页里罗列的单词就是)，接下来应用的背景颜色就会变成你说的颜色。</p>

<p><img alt="The UI of an app titled Speech Color changer. It invites the user to tap the screen and say a color, and then it turns the background of the app that colour. In this case it has turned the background red." src="speech-color-changer.png"></p>

<p>为了跑这个 demo，可以 clone Github 仓库 (上面甩出的就是，或者<a href="https://github.com/mdn/web-speech-api/archive/master.zip">directly download</a>)，可以在支持的移动端浏览器 (比如 Chrome) 导航到 <a href="http://mdn.github.io/web-speech-api/speech-color-changer/">live demo URL</a> 直接观看 (亲测 desktop browser 也是可以的，不过只能是 Chrome)，也可以通过 <a href="https://developer.mozilla.org/en-US/docs/Tools/WebIDE">WebIDE</a> 作为一个 app 加载到 Firefox OS(Firefox OS 使用 API 的权限问题见下文)。</p>
...
<ul>
 <li>
  <p>Firefox 桌面端和移动端在 Gecko 44+ 中都支持，并且是没有前缀的，它可以在<code>about:config</code> 中把 <code>media.webspeech.recognition.enable</code> 设置为 <code>true</code> 打开。权限设置/UI 还没有整理出来，所以权限还不能被用户使用，也就是不能用。不过很快会修复吧~</p>
 </li>
 <li>
  <p>Firefox OS 2.5+ 也支持，但作为一个特权 API(privileged API) 需要权限，因此你需要在<a href="https://developer.mozilla.org/en-US/docs/Web/Apps/Build/Manifest">manifest.webapp</a> (也可以通过 WebIDE 下载， 或者使应用得到验证后在 <a href="https://marketplace.firefox.com/">Firefox Marketplace</a> 可使用) 如下设置：</p>
 ...
 <li>
  <p>Chrome 桌面端和 Android 端自 version 33 以来均支持，但是带有前缀，所以你需要使用带有前缀的版本，比如：<code>webkitSpeechRecognition</code></p>
 </li>
</ul>

<h3 id="HTML_和_CSS">HTML 和 CSS</h3>

<p>对于这个应用来说，HTML 和 CSS 部分是无足轻重的。仅仅只有一个标题，一个介绍段落和一个 div 用来输出 check 的结果。</p>
...
<p>CSS 也只是提供了简单的响应式样式，跨设备看上去也是 ok 的。</p>

<h3 id="JavaScript">JavaScript</h3>

<p>JavaScript 部分会介绍更多细节。</p>
...
<ul>
 ...
 <li>
  <p>你可以按照上面的结构，在多行中，想定义多少就定义多少<code>terms</code> ，也可以包括相当复杂的语法定义。对于我们这个简单的 demo，就把语法定义的简单些</p>
 </li>
</ul>

<h4 id="将grammer插入speech_recognition">将 grammer 插入 speech recognition</h4>

<p>接下来是使用 <a href="https://developer.mozilla.org/en-US/docs/Web/API/SpeechRecognition/SpeechRecognition"><code>SpeechRecognition()</code></a> 构造函数，定义一个 speech recognition 实例，控制对于这个应用的识别。还需要使用 <a href="https://developer.mozilla.org/en-US/docs/Web/API/SpeechGrammarList/SpeechGrammarList"><code>SpeechGrammarList()</code></a> 构造函数，创立一个 speech grammer list 对象，包含我们的语法。</p>
...
<ul>
 ...
 <li>
  <p><a href="https://developer.mozilla.org/en-US/docs/Web/API/SpeechRecognition/interimResults"><code>SpeechRecognition.interimResults</code></a> ：定义 speech recognition 系统要不要返回临时结果 (interim results)，还是只返回最终结果。对于这个简单 demo，只返回最终结果就够了。</p>
 </li>
 <li>
  <p><a href="https://developer.mozilla.org/en-US/docs/Web/API/SpeechRecognition/maxAlternatives"><code>SpeechRecognition.maxAlternatives</code></a> ：定义每次结果返回的可能匹配值的数量。这有时有用，比如要的结果不明确，你想要用一个列表展示所有可能值，让用户自己从中选择一个正确的。但这里这个简单 demo 就不用了，因此我们设置为 1(1 也就是默认值)。</p>
 </li>
</ul>
...
<div class="note">
<p><strong>Note</strong>: <a href="https://developer.mozilla.org/en-US/docs/Web/API/SpeechRecognition/continuous"><code>SpeechRecognition.continuous</code></a> 控制的是每一次允许多个结果被捕捉 (比如在这个 demo 中连着说两个颜色关键字，都可以被捕捉)，或者一次只能识别一个结果。代码中它被注释掉的原因是，在 Gecko 中它还不被支持，所以如果把它加进去会破坏这个应用。你可以在收到第一个结果后简单地停止识别，从而得到类似的结果，稍后将会看到。</p>
</div>

<h4 id="开始语音识别">开始语音识别</h4>

<p>在获取输出的<code>&lt;div&gt;</code> 和 html 元素引用之后 (这些我们可以用来待会输出语音识别诊断的结果，更新应用的背景颜色)，我们添加了一个<code>onclick</code> 事件处理，作用是当屏幕被点击后，语音识别服务将开启——这通过调用 <a href="https://developer.mozilla.org/en-US/docs/Web/API/SpeechRecognition/start"><code>SpeechRecognition.start()</code></a> 实现。<code>forEach()</code> 方法内部的工作是，为每个颜色关键字添加一个这个颜色的背景色，这样就直观知道了这个颜色关键字指向什么颜色。</p>
...
<p>Web Speech API 对此有一个主要控制接口——<a href="https://developer.mozilla.org/en-US/docs/Web/API/SpeechSynthesis"><code>SpeechSynthesis</code></a> ，外加一些处理如何表示要被合成的文本 (也被称为 utterances)，用什么声音来播出 utterances 等工作的相关接口。同样的，许多操作系统都有自己的某种语音合成系统，在这个任务中我们调用可用的 API 来使用语音合成系统。</p>

<h3 id="Demo">Demo</h3>

<p>为了展示 Web 语音合成的简单使用，我们提供了一个例子—— <a href="https://github.com/mdn/web-speech-api/tree/gh-pages/speak-easy-synthesis">Speak easy synthesis</a> 。例子是一套表单控件，包括输入需要被合成的文本，设置音调、语速和发出文本时需要的语音。在输入文本之后，按下<code>Enter</code>/<code>Return</code>键使它播放。</p>
...
<p>想跑这个例子，你可以 git clone Github 仓库中的部分 (或者<a href="https://github.com/mdn/web-speech-api/archive/master.zip">直接下载</a>)，在桌面版支持的浏览器打开 index.html 文件，或者在移动端浏览器直接导向 <a href="http://mdn.github.io/web-speech-api/speak-easy-synthesis/">live demo URL</a> ，像 Chrome 和 Firefox OS。</p>
...
<ul>
 ...
 <li>
  <p>Chrome 桌面版和安卓版自 33 版以来都支持，但是没有前缀</p>
 </li>
</ul>

<h3 id="HTML_和_CSS_2">HTML 和 CSS</h3>

<p>HTML 和 CSS 还是无足轻重，只是简单包含一个标题，一段介绍文字，以及一个表格带有一些简单控制功能。<code>&lt;select&gt;</code> 元素初始时空的，之后会用 js 使用<code>&lt;option&gt;</code> 填充。</p>
...
<p>让我们看看 js 在这个 app 中的强大表现。</p>

<h4 id="设置变量">设置变量</h4>

<p>首先我们获得 UI 中涉及的 DOM 元素的引用，但更有趣的是，我们获得了<a href="https://developer.mozilla.org/en-US/docs/Web/API/Window/speechSynthesis"><code>Window.speechSynthesis</code></a> 的引用。这是 API 的入口点——它返回了<a href="https://developer.mozilla.org/en-US/docs/Web/API/SpeechSynthesis"><code>SpeechSynthesis</code></a> 的一个实例，对于 web 语音合成的控制接口。</p>
...
<p>最后，我们在文本输入框添加了 <a href="https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/blur">blur()</a> 方法。这主要是在 Firefox 操作系统上隐藏键盘</p>
