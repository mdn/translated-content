...
---
<p>该篇文档描述了如何在扩展和网站内部通过 JavaScript 调用 <a href="/zh-CN/XPath">XPath</a> 接口。 Mozilla 实现了相当多的 <a href="http://www.w3.org/TR/DOM-Level-3-XPath/xpath.html">DOM 3 XPath</a>，意味着 Xpath 表达式已经可以在 HTML 和 XML 文档中使用。</p>

<p>使用 XPath 的主要接口是 <a href="/zh-CN/DOM/document">document</a> 对象的 <a href="/zh-CN/DOM/document.evaluate">evaluate</a> 方法。</p>

<h2 id="document.evaluate">document.evaluate</h2>

<p>此方法针对基于 <a href="/en-US/docs/Glossary/XML">XML</a> 的文档（包括 HTML 文档）评估 XPath 表达式，并返回 <a href="/en-US/docs/XPathResult">XPathResult</a> 对象，该对象可以是单个节点或一组节点。这个方法的现有文档位于 <a href="/en-US/docs/Web/API/Document.evaluate">document.evaluate</a>，但是对于我们现在的需求来说它相当稀疏；下面将给出更全面的研究。</p>
...
<p><a href="/en-US/docs/Web/API/Document.evaluate">evaluate</a> 函数共有五个参数：</p>
...
<ul>
 ...
 <li>
  ...
  <ul>
   <li>
    <p>使用 <a href="/en-US/docs/Using_XPath#Node-specific_evaluator_function"><code>XPathEvaluator</code></a> 对象的 <a href="/en-US/docs/Web/API/Document.createNSResolver"><code>createNSResolver</code></a> 方法<a href="/en-US/docs/Introduction_to_using_XPath_in_JavaScript#Implementing_a_Default_Namespace_Resolver">创建</a>。</p>
   </li>
   <li>
   ...
   <li>
    <p>用户定义的函数。有关详细信息，请参阅附录中的 <a href="/en-US/docs/Introduction_to_using_XPath_in_JavaScript#Implementing_a_User_Defined_Namespace_Resolver">使用一个用户定义的命名空间解析器</a> 部分。</p>
   </li>
  </ul>
 </li>
 <li>
  <p><code>resultType</code>：指定作为评估结果返回的所需结果类型的<a href="/en-US/docs/Introduction_to_using_XPath_in_JavaScript#XPathResult_Defined_Constants">常数</a>。最常传递的常量是 <code>XPathResult.ANY_TYPE</code>，它将返回 XPath 表达式的结果作为最自然的类型。附录中有一个部分，其中包含<a href="/en-US/docs/Introduction_to_using_XPath_in_JavaScript#XPathResult_Defined_Constants">可用常数</a>的完整列表。它们在下面“<a href="/en-US/docs/Introduction_to_using_XPath_in_JavaScript#Specifying_the_Return_Type">指定返回类型</a>”部分中进行解释。</p>
 </li>
 <li>
...
<p>注意：XPath 定义不带前缀的 QNames，以仅匹配 null 命名空间中的元素。XPath 没有办法选择应用于常规元素引用的默认命名空间（例如，<code>p[@id='_myid']</code> 对应于 <code>xmlns='http://www.w3.org/1999/xhtml'</code>）。要匹配非命名空间中的默认元素，您必须使用如 <code>[namespace-uri()='http://www.w3.org/1999/xhtml' and name()='p' and @id='_id']</code>（<a href="/en-US/docs/Introduction_to_using_XPath_in_JavaScript#Using_XPath_functions_to_reference_elements_with_a_default_namespace">这种方法</a>适用于命名空间未知的动态 XPath），或者使用前缀名测试，并创建一个命名空间解析器将前缀映射到命名空间。如果你想采取后一种方法，阅读更多关于<a href="/en-US/docs/Introduction_to_using_XPath_in_JavaScript#Implementing_a_User_Defined_Namespace_Resolver">如何创建一个用户定义的命名空间解析器</a>。</p>
...
<h3 id="Specifying_the_Return_Type">指定返回类型</h3>

<p><code>document.evaluate</code> 返回的变量 <code>xpathResult</code> 可以由单个节点（<a href="/en-US/docs/Introduction_to_using_XPath_in_JavaScript#Simple_Types">简单类型</a>）或节点集合（<a href="/en-US/docs/Introduction_to_using_XPath_in_JavaScript#Node-Set_Types">节点集类型</a>）组成。</p>

<h4 id="Simple_Types">简单类型</h4>
...
<p>以下使用 XPath 表达式 <a href="/en-US/docs/XPath/Functions/count"><code>count(//p)</code></a> 来获取 HTML 文档中的 <code>&lt;p&gt;</code> 元素数：</p>
...
<p>要在评估后确定类型，我们使用 <code>XPathResult</code> 对象的 <code>resultType</code> 属性。此属性的<a href="/en-US/docs/Introduction_to_using_XPath_in_JavaScript#XPathResult_Defined_Constants">常量</a>值在附录中定义。 None Yet =====Any_Type Example===== &lt;pre&gt; &lt;/pre&gt;</p>

<h2 id="Examples">示例</h2>

<h3 id="Within_an_HTML_Document">在 HTML 文档中</h3>
...
<p>为了使 XML 文档的内容在扩展中可用，我们创建一个 <a href="/en-US/docs/Web/API/XMLHttpRequest"><code>XMLHttpRequest</code></a> 对象以同步加载文档，变量 <code>xmlDoc</code> 将包含该文档作为 <a href="/en-US/docs/Web/API/XMLDocument"><code>XMLDocument</code></a> 对象，我们可以使用 <code>evaluate</code> 方法。</p>
...
<p>将选择作为 (X)HTML 表数据单元元素的子项的所有 <a href="/en-US/docs/Web/API/MathML">MathML</a> 表达式。</p>
...
<p>如前面<a href="/en-US/docs/Introduction_to_using_XPath_in_JavaScript#Implementing_a_Default_Namespace_Resolver">实现默认命名空间解析器</a>中所述，默认解析器不处理 XML 文档的默认命名空间。 例如使用本文档：</p>
...
<h4 id="使用XPath函数引用具有默认命名空间的元素">使用 XPath 函数引用具有默认命名空间的元素</h4>
...
<p>虽然可以修改上述部分中的方法来测试命名空间元素，而不管选择的前缀（使用 <a href="/en-US/docs/XPath/Functions/local-name"><code>local-name()</code></a> 结合 <a href="/en-US/docs/XPath/Functions/namespace-uri"><code>namespace-uri()</code></a> 而不是 <a href="/en-US/docs/XPath/Functions/name"><code>name()</code></a>），但是会发生更具挑战性的情况，如果希望在谓词中获取具有特定命名空间属性的元素（假设在 XPath 1.0 中没有与实现无关的变量）。</p>
...
<ul>
 <li><a href="/zh-CN/Using_XPath">Using XPath</a></li>
</ul>
