...
<dl>
 ...
 <dd>除了 HTML，CSS，JavaScript 和媒体文件这些前端模块之外，还有其他影响 Web 性能的因素，它们可能会导致应用程序变慢，或在主观和客观上使应用程序变快。有许多与 Web 性能相关的 API、开发人员工具、最佳实践和不当做法。我们将在基础层面上介绍这些影响因素，并提供进阶优化其中每一项性能的链接。</dd>
 <dt><a href="/zh-CN/docs/Learn/Performance/HTML">HTML 性能特性</a></dt>
 <dd>标签的某些属性和顺序可能会影响网站性能。 通过最大程度地减少 DOM 节点的数量，确保使用最佳顺序和属性，包括样式、脚本、媒体和第三方脚本等内容，可以大大改善用户体验。 该文详细介绍了如何使用 HTML 来确保最佳性能。</dd>
 ...
 <dd>如果正确使用 JavaScript，则可以提供交互式和身临其境的 Web 体验——否则可能会严重损害下载时间、渲染时间、应用内性能、电池寿命和用户体验。 该文概述了一些值得思考的 JavaScript 最佳实践，以确保即使复杂的内容也尽可能地具有高性能。</dd>
 <dt><a href="/zh-CN/docs/Learn/Performance/Mobile">移动端性能</a></dt>
 <dd>随着移动设备上的 Web 访问普及，并且所有移动平台都具有功能完善的 Web 浏览器，但由于受限于带宽、CPU、电池续航等因素，因此考虑这些平台上 Web 内容的性能非常重要。 本文着眼于特定于移动设备的性能注意事项。</dd>
</dl>

<h2 id="Using_Performance_APIs">使用 Performance API</h2>

<dl>
 <dt><a href="/zh-CN/docs/Web/API/Performance_API/Using_the_Performance_API">Performance API</a></dt>
 <dd>该指南介绍了如何使用 <a href="https://w3c.github.io/hr-time/">High-Resolution Time</a> 标准中定义的 <code><a href="/zh-CN/docs/Web/API/Performance">Performance</a></code> 接口。</dd>
 <dt><a href="/zh-CN/docs/Web/API/Resource_Timing_API/Using_the_Resource_Timing_API">Resource Timing API</a></dt>
 <dd><a href="/zh-CN/docs/Web/API/Resource_Timing_API">资源加载和定时加载</a> 这些资源，包括管理资源缓冲区和处理 CORS</dd>
 <dt><a href="/zh-CN/docs/Web/API/Performance_Timeline/Using_Performance_Timeline">性能时间线</a></dt>
 <dd><a href="/zh-CN/docs/Web/API/Performance_Timeline">Performance Timeline</a> 标准定义了对 <a href="/zh-CN/docs/Web/API/Performance"><code>Performance</code></a> 接口的扩展，以支持应用程序中的客户端延迟测量。这些接口一起可以用来帮助识别应用程序的性能瓶颈。</dd>
...
<h2 id="Other_documentation">其他文档</h2>
...
<dl>
 ...
 <dd>了解如何使用 Firefox 的内置分析器来分析应用程序性能。</dd>
</dl>
...
<dl>
 <dt><a href="/zh-CN/docs/Learn/Performance/JavaScript">JavaScript 性能最佳实践</a></dt>
 <dd>JavaScript，如果使用得当，可以提供交互式和身临其境的 web 体验...或者它会严重影响下载时间、渲染时间、应用内性能、电池寿命和用户体验。本文概述了一些 JavaScript 最佳实践，这些实践可以确保即使是复杂内容的性能也是最高的。</dd>
 <dt><a href="/zh-CN/docs/Learn/Performance/Mobile">移动端性能</a></dt>
 <dd>由于移动设备上的 web 访问非常流行，而且所有移动平台都有成熟的 web 浏览器，但带宽、CPU 和电池寿命可能有限，因此考虑这些平台上 web 内容的性能非常重要。本文还讨论了移动设备特定的性能考虑因素。</dd>
 <dt>Web 字体性能</dt>
 <dd>性能方面一个经常被忽视的方面是 web 字体。Web 字体在 Web 设计中比以往任何时候都更加突出，然而许多开发人员只是简单地将它们从第三方服务中嵌入而不考虑它。在本文中，我们将介绍如何使用高效的文件格式和子设置使字体文件尽可能小。从那以后，我们将继续讨论浏览器如何文本，以及如何使用 CSS 和 JavaScript 功能，以确保您的字体快速呈现，并将对用户体验的干扰降到最低。</dd>
 <dt>性能瓶颈</dt>
 <dt>理解带宽</dt>
 <dd>带宽是以兆位（Mb）或千位（Kb）为单位的每秒可发送的数据量。本文将解释带宽在富媒体 internet 应用程序中的作用，如何测量带宽，以及如何优化应用程序以最大限度地利用可用带宽。
 </dd>
 <dt>TLS 在性能中的作用</dt>
 <dd>
 <p>TLS 或 HTTPS（我们通常称之为 TLS）对于创建安全和安全的用户体验至关重要。虽然硬件降低了 TLS 对服务器性能的负面影响，但它仍然代表了我们等待浏览器连接到服务器所花费的大量时间。本文解释了 TLS 握手过程，并提供了一些减少时间的技巧，例如 OCSP 装订、HSTS 预加载头，以及资源提示在为第三方屏蔽 TLS 延迟方面的潜在作用。</p>
 </dd>
 <dt>阅读性能图表</dt>
 ...
 <dd>毫无疑问，JavaScript 是现代 web 开发的重要组成部分。虽然您应该一直努力减少在应用程序中使用的 JavaScript 数量，但是很难知道从哪里开始。在本指南中，我们将向您展示如何分析应用程序的脚本捆绑包，以便您知道您在使用什么，以及如何检测应用程序在捆绑包之间是否包含重复的脚本。</dd>
 <dt><a href="/zh-CN/docs/Web/Performance/Lazy_loading">延迟加载</a></dt>
 <dd>在初始页面加载时并不总是需要加载所有 web 应用程序资产。延迟加载是将页面上的资产（如脚本、图像等）的加载推迟到稍后的时间点，即实际需要这些资产的时候。</dd>
 <dt>使用动态 import 延迟加载 JavaScript</dt>
 <dd>当开发人员听到术语“延迟加载”时，他们会立即想到当滚动到视口时加载的折叠图像下方。但是你知道你也可以延迟加载 JavaScript 吗？在本指南中，我们将讨论 dynamic import（）语句，这是现代浏览器中按需加载 JavaScript 模块的功能。当然，由于这个特性不是到处都可用的，我们还将向您展示如何配置您的工具以广泛兼容的方式使用此特性。</dd>
 <dt><a href="/zh-CN/docs/Web/Performance/Controlling_resource_delivery_with_resource_hints">使用资源提示（resource hints）控制资源传输</a></dt>
 <dd>浏览器通常比我们更了解资源的优先级和交付，但是他们离克莱里奥万特还很远。本机浏览器特性使我们能够在浏览器应该连接到另一个服务器时提示浏览器，或者在浏览器知道它需要资源之前预先加载资源。如果使用得当，这可以使快速体验看起来更快。在本文中，我们将介绍诸如 rel=preconnect、rel=dns prefetch、rel=prefetch 和 rel=preload 等本机浏览器功能，以及如何使用它们来发挥您的优势。</dd>
 <dt><a href="/zh-CN/docs/Web/Performance/Performance_budgets">性能预算</a></dt>
 <dd>营销、设计和销售需求，以及开发人员的经验，通常是广告膨胀、第三方脚本和其他会降低 web 性能的功能。为了帮助设置优先级，设置一个性能预算是很有帮助的：一组在开发阶段不能超过的限制。在本文中，我们将讨论创建和坚持绩效预算。</dd>
...
<ul>
 ...
 <li><a href="/zh-CN/docs/Web/API/Background_Tasks_API">Cooperative Scheduling of Background Tasks API</a>
  <ul>
   <li><a href="/zh-CN/docs/Web/API/Window/requestIdleCallback">requestIdleCallback() </a></li>
  </ul>
 </li>
