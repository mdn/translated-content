...
如果你测量过解析 CSS 的时间，你将会被浏览器实际的“快”所震惊。更具体的规则更耗时，因为这些规则必须遍历更多的 DOM 树节点，但这所带来的额外的消耗通常很小。你可以先测量一下。然后按需优化。特定化或许不是最适合你的。在 CSS 中选择器的性能优化，提升仅仅是毫秒级的。还有其他一些方式来[优化 CSS](/zh-CN/docs/Learn_web_development/Extensions/Performance/CSS)，例如极简化和使用媒体查询，从而异步处理 CSS 为非阻塞的请求。
