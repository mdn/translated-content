...
相反，你应该这样写你的代码，让你的应用在后台线程创建一个 [Web worker](/zh-CN/docs/Web/API/Web_Workers_API/Using_web_workers) ，做尽可能多的工作（比如，获取和处理数据）。然后，所有必须在主线程中完成的事情（比如用户事件和渲染用户界面）应该被分成小的片段，这样，当应用启动时，应用的事件循环就可以持续地运行下去。这可以避免应用、浏览器以及/或者设备出现锁死。
...
虽然你可以使用 [Web workers](/zh-CN/docs/Web/API/Web_Workers_API) 异步运行体积巨大，持续时间长的 [JavaScript](/zh-CN/docs/Web/JavaScript) 代码块，但还是要给出一个重大警告：workers 不具备访问 [WebGL](/zh-CN/docs/Web/API/WebGL_API) 或音频的能力，亦不能向主线程发送同步消息，所以你甚至不能将这些 API 代理到主线程中。所有的这一切意味着，除非你够轻松地抽取启动过程中的“纯计算”代码块，加入到 workers 中，否则你最后还是得在主线程上运行大部分或全部的启动代码。
