...
- [生成页面：浏览器的工作原理](/zh-CN/docs/Web/Performance/Guides/How_browsers_work)
  - : 用户希望获得内容加载速度快、交互流畅的网络体验。因此，开发人员应努力实现这两个目标。要了解如何提高性能和感知性能，了解浏览器的工作原理很有帮助
...
- [导航和资源时间](/zh-CN/docs/Web/Performance/Guides/Navigation_and_resource_timings)
  - : **导航计时**是衡量浏览器文档导航事件的指标。**资源计时**是关于应用程序资源加载的详细网络定时测量。两者提供相同的只读属性，但导航计时测量的是主文档的计时，而资源计时提供的是主文档调用的所有资产或资源以及资源请求的时间。
...
- [关键渲染路径](/zh-CN/docs/Web/Performance/Guides/Critical_rendering_path)
  - : 关键渲染路径是浏览器将 HTML、CSS 和 JavaScript 转换为屏幕上像素的一系列步骤。优化关键呈现路径可提高呈现性能。关键呈现路径包括[文档对象模型](/zh-CN/docs/Web/API/Document_Object_Model)（DOM）、[CSS 对象模型](/zh-CN/docs/Web/API/CSS_Object_Model)（CSSOM）、呈现树和布局。
- [懒加载](/zh-CN/docs/Web/Performance/Guides/Lazy_loading)
  - : **懒加载**是一种将资源识别为非阻塞（非关键）并仅在需要时加载的策略。这是一种缩短[关键渲染路径](/zh-CN/docs/Web/Performance/Guides/Critical_rendering_path)长度的方法，可缩短页面加载时间。
...
- [性能预算](/zh-CN/docs/Web/Performance/Guides/Performance_budgets)
  - : 性能预算是防止性能下降的限制。它可以适用于一个文件、一种文件类型、页面上加载的所有文件、一个特定指标（例如[下次交互时间](/zh-CN/docs/Glossary/Time_to_interactive)）、一个自定义指标（例如 Time to Hero Element）或一段时间内的阈值。
...
- [CSS 和 JavaScript 动画性能](/zh-CN/docs/Web/Performance/Guides/CSS_JavaScript_animation_performance)
  - : 在许多应用程序中，动画对于愉悦的用户体验至关重要。实现网页动画的方法有很多，例如 CSS {{cssxref("transition", "过渡")}}/{{cssxref("animation", "动画")}}或基于 JavaScript 的动画（使用 {{domxref("Window.requestAnimationFrame", "requestAnimationFrame()")}}）。本文将分析基于 CSS 的动画和基于 JavaScript 的动画之间的性能差异。
...
- [后台任务 API](/zh-CN/docs/Web/API/Background_Tasks_API)
  - : **合作调度后台任务 API**（也称为后台任务 API 或 [`requestIdleCallback()`](/zh-CN/docs/Web/API/Window/requestIdleCallback) API）提供了排队任务的功能，以便用户代理在确定有空闲时间时自动执行任务。
...
- [FetchEvent.preloadResponse](/zh-CN/docs/Web/API/FetchEvent/preloadResponse)
  - : {{domxref("FetchEvent")}} 接口的 **`preloadResponse`** 只读属性返回一个 {{jsxref("Promise")}}，当触发了[导航预载](/zh-CN/docs/Web/API/NavigationPreloadManager)时，兑现为导航预载 {{domxref("Response")}}，否则兑现为 `undefined`。
...
- [Web Workers API](/zh-CN/docs/Web/API/Web_Workers_API)，包括[使用 Service Workers](/zh-CN/docs/Web/API/Service_Worker_API/Using_Service_Workers) 和[使用 Web Workers](/zh-CN/docs/Web/API/Web_Workers_API/Using_web_workers)
