...
从请求中获取到的第一个资源通常是 HTML 文件，然后该文件会调用额外的资源。正如在[关键渲染路径](/zh-CN/docs/Web/Performance/Critical_rendering_path)中描述的，浏览器在接收到 HTML 后，会立即开始处理，并在接收到内容时进行渲染，而不是等待额外资源的加载。
