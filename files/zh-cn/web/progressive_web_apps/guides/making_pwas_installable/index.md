...
PWA 定义的方面之一就是它可以有浏览器提示以将其安装在设备上。安装后，PWA 对用户而言就是特定于平台的应用程序，是用户设备的一个永久功能，用户可以像启动其他任何应用程序一样直接从操作系统启动它。
...
当支持的浏览器确定 Web 应用程序满足前述的可安装性标准时，它将提示用户安装当前应用。这意味着你可以将你的 PWA 作为一个网站进行分发，使其可以通过 Web 搜索找到，也可以在应用商店中分发，以便用户可以在那里找到它。
...
桌面版和移动版 Safari 以及桌面版 Edge 也支持将任何网站安装为应用程序。但是，这并不是 PWA 所特有的，因为网站不需要满足本指南中描述的可安装性标准，且浏览器不会主动提示安装网站。
