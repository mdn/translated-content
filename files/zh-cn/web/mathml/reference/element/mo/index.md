...
除了[全局 MathML 属性](/zh-CN/docs/Web/MathML/Reference/Global_attributes)外，该元素接受以下属性（[其默认值取决于运算符的形式和内容](https://w3c.github.io/mathml-core/#algorithm-for-determining-the-properties-of-an-embellished-operator)）：

- `accent` {{Non-standard_Inline}}
  - : 一个 [`<boolean>`](/zh-CN/docs/Web/MathML/Reference/Values#mathml_特定类型)，表示运算符在被用作[下标](/zh-CN/docs/Web/MathML/Reference/Element/munder)或[上标](/zh-CN/docs/Web/MathML/Reference/Element/mover)时是否应该被着重标识（即被放大并靠近基础表达式）。
- `fence`
...
- `separator`
  - : 一个 [`<boolean>`](/zh-CN/docs/Web/MathML/Reference/Values#mathml_特定类型)，表示运算符是否是一个分隔符（例如逗号）。该属性没有视觉效果。
- `stretchy`
  - : 一个 [`<boolean>`](/zh-CN/docs/Web/MathML/Reference/Values#mathml_特定类型)，表示运算符是否拉伸到相邻元素的大小。
- `symmetric`
  - : 一个 [`<boolean>`](/zh-CN/docs/Web/MathML/Reference/Values#mathml_特定类型)，表示拉伸运算符是否应该与虚拟的数学轴（中心分数线）垂直对称。

> [!NOTE]
> 对于 `lspace`、`maxsize`、`minsize` 和 `rspace` 属性，一些浏览器可能还接受[旧版 MathML 长度](/zh-CN/docs/Web/MathML/Reference/Values#旧版_mathml_长度)。
