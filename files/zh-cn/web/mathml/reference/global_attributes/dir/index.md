...
> - `dir` 属性用于设置数学公式的方向性，在阿拉伯语世界通常是从右到左的。然而，从右到左书写的语言通常嵌入从左到右书写的数学内容。因此，默认情况下，HTML `dir` 属性中的 `auto` 关键字不被识别，并且[用户代理样式表](/zh-CN/docs/Web/CSS/CSS_cascade/Cascade#用户代理样式表)会重置 [`math`](/zh-CN/docs/Web/MathML/Reference/Element/math) 元素上的方向属性。
