...
然而，你可能需要确保你的系统知道这些字体。添加一个 fontconfig 配置 `/etc/fonts/conf.avail/09-texlive-fonts.conf`，指向 TeXLive 的 `opentype` 目录，例如：
...
> **备注：** Noto Sans Math 提供了良好的数学符号 Unicode 覆盖范围，并且 [Google 计划为数学布局功能添加支持](https://github.com/notofonts/math/issues/14#issuecomment-1161414446)。
...
在接下来的章节中，你将找到安装和配置 MathML 字体的其他有用提示。
