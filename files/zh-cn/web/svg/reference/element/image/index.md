...
使用 `<image>` 显示的 SVG 文件[被视为图像](/zh-CN/docs/Web/SVG/Guides/SVG_as_an_image)：外部资源不会被加载、{{cssxref(":visited")}} 样式[不会被应用](/zh-CN/docs/Web/CSS/Privacy_and_the_:visited_selector)，且它们不能有交互。要包含动态 SVG 元素，可以尝试用 {{SVGElement("use")}} 引入外部 URL。要包含 SVG 文件并在其中运行脚本，请尝试在 {{SVGElement("foreignObject")}} 中嵌入 {{HTMLElement("object")}}。
