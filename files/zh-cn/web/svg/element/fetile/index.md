...
**`<feTile>`** [SVG](/zh-CN/docs/Web/SVG) 滤镜原图允许使用输入图像的重复、平铺模式来填充目标矩形。此效果类似于 {{SVGElement("pattern")}} 元素的效果。
...
<svg
  ...
  <defs>
    <!-- 定义过滤器的作用区域为 MDN 徽标（被过滤图像）的边框。这些参数将创建一个与图像相同覆盖区域的输出。 -->
    <filter id="tile" x="0" y="0" width="100%" height="100%">
      ...
      <!-- 如果不指定区域，默认情况下，feTile 会使用滤镜的整个区域。如果不指定“in”参数，那么默认值就是前一个滤镜原图的结果。因此，这第二个 feTile 会将吉祥物头部的图案在整个滤镜区域内进行平铺。 -->
      <feTile />
    </filter>
  </defs>
