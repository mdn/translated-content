...
<p>{{ EmbedLiveSample('创建剪切','240','240') }}</p>
...
<p>{{ EmbedLiveSample('遮罩','240','240') }}</p>

<h3 id="用opacity定义透明度">用 <code>opacity</code> 定义透明度</h3>
...
<p>{{ EmbedLiveSample('用 opacity 定义透明度','240','240') }}</p>
