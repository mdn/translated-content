...
要插入形状，需在文档中创建元素。不同元素对应不同形状，并通过不同参数描述形状的大小和位置。其中部分元素存在轻微冗余——它们可由其他形状派生创建，但所有元素的存在都是为了方便开发者，并使SVG文档尽可能简洁易读。所有基本形状如图所示。
...
{{SVGElement("ellipse")}} 是 {{SVGElement("circle")}} 元素更通用的形式，你可以分别调整圆的 x 半径和 y 半径（通常数学家称之为半长轴和半短轴）。
