...
- 路径 1 矩形与包含区块的左边缘和上边缘的偏移量为 `20px`。该路径矩形的尺寸与包含区块的尺寸相同，即宽度为包含区块宽度的 `100%`，高度为包含区块高度的 `100%`。注意盒子内的箭头如何沿着矩形路径边角上（由 `round 10%` 定义的）`10%` 曲线移动的。
- 由于 `xywh()` 中宽度和高度的上限都是无穷大，因此在路径 2 矩形中将高度设置为 `200%` 会使生成的矩形是包含的区块的两倍。请注意当未指定 `round <'border-radius'>` 时，盒子内的箭头在边角处的表现。
