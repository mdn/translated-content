...
**CSS 值函数**是调用特殊数据处理或计算的语句，以返回一个 CSS 属性的 [CSS](/zh-CN/docs/Web/CSS) [值](/zh-CN/docs/Web/CSS/Guides/Values_and_units)。CSS 值函数代表更复杂的[数据类型](/zh-CN/docs/Web/CSS/Reference/Values/Data_types)，它们可能需要一些输入参数来计算返回值。
...
> [!NOTE]
> CSS 值函数是作为属性值使用的，不应该与伪类相混淆。[函数式伪类](/zh-CN/docs/Web/CSS/Reference/Selectors/Pseudo-classes#函数式伪类)、[语言伪类](/zh-CN/docs/Web/CSS/Reference/Selectors/Pseudo-classes#语言的伪类)和几种[树状结构伪类](/zh-CN/docs/Web/CSS/Reference/Selectors/Pseudo-classes#树结构的伪类)都需要参数值，但它们不是值函数。[条件 @ 规则](/zh-CN/docs/Web/CSS/Guides/Syntax/At-rules#条件规则组)也不是值函数；括号是用来分组的。

## 转换函数

{{CSSxRef("&lt;transform-function&gt;")}} CSS [数据类型](/zh-CN/docs/Web/CSS/Reference/Values/Data_types)代表外观变换。它被用作 {{CSSxRef("transform")}} 属性的一个值。
...
{{CSSxRef("&lt;filter-function&gt;")}} CSS [数据类型](/zh-CN/docs/Web/CSS/Reference/Values/Data_types)表示可以改变输入图像的外观的图形效果。它被用于 {{CSSxRef("filter")}} 和 {{CSSxRef("backdrop-filter")}} 属性。
...
{{CSSxRef("color_value","&lt;color&gt;")}} CSS [数据类型](/zh-CN/docs/Web/CSS/Reference/Values/Data_types)指定了不同的颜色表示方法。
...
{{CSSxRef("&lt;image&gt;")}} CSS [数据类型](/zh-CN/docs/Web/CSS/Reference/Values/Data_types)提供了图像或渐变的图形化表示。
...
{{CSSxRef("&lt;basic-shape&gt;")}} CSS [数据类型](/zh-CN/docs/Web/CSS/Reference/Values/Data_types)表示一个图形形状。它被用于 {{CSSxRef("clip-path")}}、{{CSSxRef("offset-path")}} 和 {{CSSxRef("shape-outside")}} 属性中。
...
以下函数用于定义一个 [CSS 网格](/zh-CN/docs/Web/CSS/Guides/Grid_layout)。
