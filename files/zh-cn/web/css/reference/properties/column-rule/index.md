...
[CSS](/zh-CN/docs/Web/CSS) `column-rule` [简写属性](/zh-CN/docs/Web/CSS/Guides/Cascade/Shorthand_properties)可以在多列布局中设定分割线的宽度、样式和颜色。
...
该[简写属性](/zh-CN/docs/Web/CSS/Guides/Cascade/Shorthand_properties)使用简单方便的声明形式将各个 `column-rule-*` 属性放在了一起：{{Cssxref("column-rule-width")}}、{{Cssxref("column-rule-style")}} 和 {{Cssxref("column-rule-color")}}。
