...
> [!NOTE]
> 当你在 `animation-*` 属性上指定多个逗号分隔的值时，它们将按照 {{cssxref("animation-name")}} 出现的顺序应用于动画。对于动画数量和 `animation-*` 属性值不匹配的情况，请参见[设置多个动画属性值](/zh-CN/docs/Web/CSS/Guides/Animations/Using#设置多个动画属性值)。
...
参见 [CSS 动画](/zh-CN/docs/Web/CSS/Guides/Animations/Using)示例。
