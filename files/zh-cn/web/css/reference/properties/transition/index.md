...
**`transition`** [CSS](/zh-CN/docs/Web/CSS) 属性是 {{ cssxref("transition-property") }}、{{ cssxref("transition-duration") }}、{{ cssxref("transition-timing-function") }} 和 {{ cssxref("transition-delay") }} 的一个[简写属性](/zh-CN/docs/Web/CSS/Guides/Cascade/Shorthand_properties)。
...
请查看“[如何处理](/zh-CN/docs/Web/CSS/Guides/Transitions/Using#when_property_value_lists_are_of_different_lengths)”以了解当`transition`属性的值个数超过可以接收的值的个数时该如何处理。简而言之，当`transition`属性的值个数超过可以接收的值的个数时，多余的值都会被忽略掉，不再进行解析。
...
“[使用 CSS 过渡](/zh-CN/docs/Web/CSS/Guides/Transitions/Using)”一文中还包含了几个 CSS 过渡示例。
