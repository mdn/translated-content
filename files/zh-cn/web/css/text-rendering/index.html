...
<p>一个视觉上很明显的效果是，<code>optimizeLegibility 属性值会在某些字体（比如，微软的 Calibri，</code>Candara，Constantia 和 Corbel 或者 DejaVu 系列字体<code>）小于 20px 时把有些相邻字符连接起来</code>（ligatures，比如，ff、fi、fl 等，效果见下面的 LiveExample) 。</p>
...
<dl>
 ...
 <dd>浏览器依照某些根据去推测在绘制文本时，何时该优化速度，易读性或者几何精度。对于该值在不同浏览器中解释的差异，请看兼容性表。</dd>
 ...
 <dd>
 <p>浏览器在绘制文本时将着重考虑几何精度， 而不是渲染速度和易读性。字体的某些方面—比如字间距—不再线性缩放，所以该值可以使使用某些字体的文本看起来不错。</p>
