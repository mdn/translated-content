...
<ul>
 ...
 <li>当<code>position</code>设置为<code>sticky</code>时，如果元素在 viewport 里面，<code>top</code>属性的效果和 position 为<code>relative</code>等同；如果元素在 viewport 外面，<code>top</code>属性的效果和 position 为<code>fixed</code>等同。</li>
...
<dl>
 ...
 <dd>这个关键字表示：
 <ul>
  <li>对于绝对定位元素，元素将忽略此属性已{{ Cssxref("bottom") }}属性为准，如果此时设置<code>height: auto</code>，将基于内容需要的高度设置高度；如果<code>bottom</code>也为<code>auto</code>的话，元素的垂直位置就是它假如作为静态 (即 static) 元素时该在的位置。</li>
