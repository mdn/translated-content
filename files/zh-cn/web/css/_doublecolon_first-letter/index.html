...
<p><a href="/en/CSS">CSS</a> <a href="/en/CSS/Pseudo-elements">伪元素</a> <code>::first-letter</code>会选中某 <a href="https://developer.mozilla.org/en-US/docs/Web/CSS/Visual_formatting_model#Block-level_elements_and_block_boxes">block-level element</a>（块级元素）第一行的第一个字母，并且文字所处的行之前没有其他内容（如图片和内联的表格）。</p>
...
<ul>
 <li>首字符之前或紧跟首字符的标点符号在伪元素匹配范围内。标点符号（Punctuation）包括在开（Ps），闭（Pe），初始引用（Pi），结束引用（Pf）当中定义的任何 Unicode 字符以及其他种类的标点符号（Po）。</li>
 <li>另外还会有一些以大写双字母组合命名的语言，比如荷兰的 <code>IJ </code> . 在这种罕见情况下，两个大写字母都理应被 <code>::first-letter</code> 伪元素匹配到. (但是浏览器对此支持得很糟糕，可以用 <a href="/En/CSS/::first-letter#Browser_compatibility">browser compatibility table</a>进行核实).</li>
 <li>{{ cssxref("::before") }} 伪元素 和 {{ cssxref("content") }} 属性结合起来有可能会在元素前面注入一些文本。如此，<code>::first-letter</code> 将会匹配到 content 文本的首字母。</li>
</ul>

<p>首行只在 <a href="/en/CSS/Visual_formatting_model#Block-level_elements_and_block_boxes">block-container box</a>内部才有意义，因此 <code>::first-letter</code> 伪元素 只在{{ cssxref("display") }}属性值为 block, <code>inline-block</code>, <code>table-cell</code>, <code>list-item</code> 或者 <code>table-caption</code>的元素上才起作用。其他情况下，<code>::first-letter</code> 毫无意义。</p>
...
<p>只有一小部分 CSS 可以在包含使用了<code>::first-letter</code> 伪元素选择器的 CSS 规则集声明块内运用：</p>
