...
<p><strong><code>text-orientation</code></strong> <a href="/en-US/docs/Web/CSS">CSS</a> 属性设定行中字符的方向。但它仅影响纵向模式（当 {{cssxref("writing-mode")}} 的值不是<code>horizontal-tb</code>）下的文本。此属性在控制使用竖排文字的语言的显示上很有作用，也可以用来构建垂直的表格头。</p>
...
<dl>
 ...
 <dd>默认值。顺时针旋转水平书写的字符 90°，将垂直书写的文字自然布局。</dd>
 ...
 <dd>所有字符被布局为与水平方式一样，但是整行文本被顺时针旋转 90°。</dd>
 <dt><code>sideways-right</code></dt>
 <dd>处于兼容目的，<code>sideways</code> 的别名。</dd>
 <dt><code>use-glyph-orientation</code></dt>
 <dd>对于 SVG 元素，这个关键字导致使用已弃用的 SVG 属性。 <code>glyph-orientation-vertical</code> 和 <code>glyph-orientation-horizontal</code>。</dd>
</dl>
