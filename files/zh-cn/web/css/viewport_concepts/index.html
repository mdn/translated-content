...
<p>这篇文章阐述了<strong>视口 (viewport) </strong>的概念，对 视觉视口 (visual viewport) 和 布局视口 (layout viewport) 进行区分，并让你了解 viewport 的意义是什么，以及它对 CSS, SVG 和移动设备方面的影响。</p>
...
<p>在实测中，<code>innerWidth</code> 和 <code>outerWidth</code> 是相同的，但是 <code>outerHeight</code> 比 <code>innerHeight</code> 高 100px。这是因为 <code>outerHeight</code> 的测量包括浏览器框架在内，包括了地址栏和书签栏总共 100px 的高度，而浏览器没有左右边框。</p>
...
<p>视觉视口是屏幕的可视部分，不包括屏幕键盘，缩放外的区域。视觉视口要么跟布局视口相同，要么更小</p>
...
<p>我们通常认为宽度和高度的媒体查询是相对于浏览器窗口的宽度和高度而言的。它们实际上是相对于视口的，即主文档（document）中的窗口（window 对象），但它也是嵌套浏览上下文（如对象，iframe 和 SVG）中元素父级的本身的大小。在 CSS 中，我们也有基于视口大小的长度单位。1vh 单位是 1% 布局视口的高度，vw 单位与此类似。</p>
...
<p>在这个示例中，这个 SVG 的视口长宽比被设置为了 3:4，默认情况下宽为 400 像素，高为 300 像素。</p>
...
<p>如果把上面的 CSS 媒体查询样式添加到 SVG 文件中去，当 SVG 容器的宽度在 400 - 500 px 时，将应用该 CSS 样式。</p>
...
<p>这里 <code>width</code> 属性指的是视口宽度，现在视口的宽度被设置为了设备的屏幕宽度，即文档视口宽度大小与设备宽度大小 100% 对应（转换为 CSS 像素值相同）。除了 <code>width</code> 属性外，还有<code>initial-scale、</code><code>maximum-scale、minimum-scale</code>，以及 <code>user-scalable</code> 属性可供设置。这些属性分别设置了文档的初始缩放比例、最大缩放比例、最小缩放比例以及是否允许用户进行缩放操作。但这些属性的默认值已经很好了，开发时可以忽略掉这些属性。</p>
