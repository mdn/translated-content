...
- {{cssxref("&lt;basic-shape&gt;")}}
  - : 基于 [`inset()`](</zh-CN/docs/Web/CSS/basic-shape#inset()>)， [`circle()`](</zh-CN/docs/Web/CSS/basic-shape#circle()>)， [`ellipse()`](</zh-CN/docs/Web/CSS/basic-shape#ellipse()>)，或者 [`polygon()`](</zh-CN/docs/Web/CSS/basic-shape#polygon()>)其中一个创造出来的形状计算出浮动区域。如果同时存在 `<shape-box>` ，那么会为 `<basic-shape>` 方法定义一个参考盒，这个参考盒默认为 `margin-box` 。
- {{cssxref("&lt;image&gt;")}}
