...
<div>你可能经常需要创建一个包含几个栏目的布局，css 提供了几个方法来实现它。使用 Grid, Flexbox 或者 Multi-column layout 都取决于你想要实现什么目标，在这篇 recipe 中我们会探讨这些内容</div>



<div><img alt="three different styles of layouts which have two columns in the container." src="cookbook-multiple-columns.png"></div>
...
<p>Flexbox 通过设置 row 的{{cssxref("flex-direction")}} ，可以用于将内容分成列， 然而，flexbox 的目标是 flex 容器内的元素，并将把每个直接的子元素放到一个新列中，这是与 multicol 不同的地方。</p>
...
<p>目前没有办法可以在 flex 的 item 中添加规则，而且浏览器对{{cssxref("column-gap")}} 和 {{cssxref("row-gap")}}的支持是有限的，因此使用 margin 属性来创建 item 之间的间距</p>
...
<p>Flexbox 还可以被用来创建 flex items 自动换行的布局，通过给 flex container 设置  {{cssxref("flex-wrap")}} 属性为 wrap. 这些新的 flex 行只会沿该行分配空间——新行中的项不会与上面行中的项对齐，你可以在下面的例子中看到。这就是为什么 flexbox 被描述为一维。 他是为了将布局控制为行或列，但不是同时控制行和列。</p>
...
<ul>
 <li>For single rows or columns of items. 设置一行或者一列中的 items</li>
 <li>When you want to do alignment on the cross axis after laying out your items. 当你想在横轴上对齐您的 items</li>
...
<p>如果你想要 items 在行和列进行布局，你应该选择 Grid Layout. Grid Layout 作用于 grid container 的 直接子元素类似于 flexbox。 但是你可以在行和列两方面来控制他（flex 只能在行或者列）。</p>
