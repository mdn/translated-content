...
<p>如果您愿意为 Layout Cookbook 贡献示例文件，那么本页将通过阐释步骤指导您发布示例。</p>

<h2 id="好的_recipe_是怎样的">好的 recipe 是怎样的？</h2>

<p><strong>任何有用的 web 设计模式的最简版本。</strong>您 CSS 文件中的每一行代码都应该有助于演示该模式， 省去无用的部分就是为了能让示例显得完美。这种想法就是，某个人既能将这种模式应用在网站中，又可以保持自己的风格。</p>

<h2 id="发布_recipe_的步骤">发布 recipe 的步骤</h2>

<p>一份 recipe 包括以下内容：</p>

<ol>
 <li>一个动态的示例，存储在 <a href="https://github.com/mdn/css-examples">CSS Examples GitHub repo</a> 之上。</li>
 <li>这份示例的可供下载版本，同样也存储在 CSS Examples repo 之上。</li>
 <li>该网站的某个页面在 <a href="/en-US/docs/Web/CSS/Layout_cookbook">CSS layout cookbook</a> 上应详细说明：
  <ol>
   <li>要求</li>
   <li>recipe</li>
   <li>做出的选择</li>
   <li>任何有用的反馈或供选择的方法</li>
   <li>任何无障碍访问问题</li>
   <li>浏览器兼容性</li>
   <li>其他资源</li>
  </ol>
...
<h3 id="1._建立自己的模式">1. 建立自己的模式</h3>

<p>在将示例转换为 recipe 之前，先将您的模式创建成为一个简单的 HTML 页面。想想看您要展示的是什么，并使这个想法尽可能简单。避免使用特殊的 CSS 约定（例如 BEM），因为并非每个人都熟悉这些约定，反而会使他们将事情做得看起来要比预期中的更复杂。</p>

<p>确保 HTML 和 CSS 有效，并将其在一种以上的浏览器上进行测试。如果您使用的 CSS 属性并非广受支持，那也没关系！您可以在创建页面时包含浏览器支持信息。在某些情况下，创建示例的第二个版本可能会有帮助，其中包括回退支持。</p>

<h3 id="2._创建一个动态示例">2. 创建一个动态示例</h3>
...
<p>A simple worked example is <a href="https://github.com/mdn/css-examples/blob/master/css-cookbook/center.html">center.html</a>, which is embedded on the page <a href="/en-US/docs/user:chrisdavidmills/Layout_Cookbook_Home/Center_an_element">Center an element</a>. You can use this to see how to add the various bits correctly.</p>
...
<h3 id="3._创建一份可供下载的版本">3. 创建一份可供下载的版本</h3>
...
<h3 id="4._用您的示例发起_Pull_请求">4. 用您的示例发起 Pull 请求</h3>
...
<h3 id="5._创建您的页面">5. 创建您的页面</h3>

<p>Once your example is merged you can create your page. You will need an MDN account to do so with <a href="https://developer.mozilla.org/en-US/docs/MDN/Contribute/Howto/Create_and_edit_pages#Getting_page_creation_permissions">page creation permissions</a>, and you can then create the page as a subpage of the CSS Cookbook page. <a href="/en-US/docs/user:chrisdavidmills/Layout_Cookbook_Home/Contribute_a_recipe/Cookbook_template">There is a template for Cookbook pages here</a>.</p>
...
<ul>
 <li><a href="/en-US/docs/user:chrisdavidmills/Layout_Cookbook_Home/Contribute_a_recipe/Cookbook_template">Cookbook page template</a></li>
