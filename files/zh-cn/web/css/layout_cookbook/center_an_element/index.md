...
要将一个盒子居中放置在另一个之中，我们需要让作为容器的盒子变成伸缩容器。再将 {{cssxref("align-items")}} 设置为 center 来实现区块轴（block axis）上的居中，并把 {{cssxref("justify-content")}} 设置为 center 来实现行内轴（inline axis）上的居中。
