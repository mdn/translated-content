...
<p><strong><code>::first-line</code></strong> <a href="https://developer.mozilla.org/zh-CN/docs/Web/CSS">CSS</a> <a href="https://developer.mozilla.org/zh-CN/docs/Web/CSS/Pseudo-elements">pseudo-element</a> （CSS 伪元素）在某 <a href="https://developer.mozilla.org/zh-CN/docs/Web/CSS/Visual_formatting_model#Block-level_elements_and_block_boxes">block-level element</a> （块级元素）的第一行应用样式。第一行的长度取决于很多因素，包括元素宽度，文档宽度和文本的文字大小。</p>

<p>和其他所有的 伪元素一样，::first-line 不能匹配任何真实存在的 html 元素。</p>

<p> ::first-line 伪元素只能在块容器中，所以，<code>::first-line</code>伪元素只能在一个 display 值为 block, <code>inline-block</code>, <code>table-cell</code> 或者 <code>table-caption</code>中有用.。在其他的类型中，<code>::first-line</code> 是不起作用的。</p>
...
<p>在一个使用了 <code>::first-line</code> 伪元素的选择器中，只有很小的一部分 css 属性能被使用：</p>
...
<div class="note">
<p>在 CSS 2 中，伪元素是以 : 开头的。由于伪类也遵循同一规则，使得他们之间难以区分。为了解决这个问题，在 CSS 2.1 中，伪元素支持以 :: 开头。现在，使用伪元素时更推荐以 :: 开头，而使用伪类时使用 : 开头。</p>

<p>因为过去的浏览器都实现过 CSS 2 的规则，所以现在那些支持 :: 的浏览器通常同时也支持 : 的形式。</p>
