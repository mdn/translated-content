...
<ul>
 <li>在突厥语系中 - 比如土耳其语（<a href="https://zh.wikipedia.org/wiki/ISO_639-1">ISO 639-1</a>代码‘tr’）、阿塞拜疆语（<a href="https://zh.wikipedia.org/wiki/ISO_639-1">ISO 639-1</a>代码‘az’）、克里米亚鞑靼语（<a href="https://zh.wikipedia.org/wiki/ISO_639-3">ISO 639-3</a>代码‘crh’）、鞑靼语（<a href="https://zh.wikipedia.org/wiki/ISO_639-1">ISO 639-1</a>代码‘tt’）和巴什基尔语（<a href="https://zh.wikipedia.org/wiki/ISO_639-1">ISO 639-1</a>代码‘ba’），有两种发音近似的“i”字母（区别在于一种有顶部的“.”，另一种没有），而它们的大小写写法也有区别：一种是<code>i</code>/<code>İ</code>，另一种是 <code>ı</code>/<code>I</code>。</li>
 <li>在德语（<a href="https://zh.wikipedia.org/wiki/ISO_639-1">ISO 639-1</a>代码‘de’）中，<code>ß</code> 的大写可能会被写作<code>ẞ</code>（收录于通用字符集 U+1E9E）。</li>
 <li>在希腊语（<a href="https://zh.wikipedia.org/wiki/ISO_639-1">ISO 639-1</a>代码‘el’）中，如果整个单词是大写的（<code>ά</code>/<code>Α</code>），元音将失去重音。（分离字母 eta（<code>ή</code>/<code>Ή</code>）除外。）除此之外，重音在第一个元音的双元音将失去重音，并被在第二个元音上加上一个变音符（<code>άι</code>/<code>ΑΪ</code>）.</li>
</ul>
...
<p><code>font-variant-caps</code> 属性使用以下列表中的单一关键词确定。在任一使用场景中，如果字体库不支持 OpenType 值，则会被尺寸加工。</p>
...
<dl>
 ...
 <dd>允许小型大写字母的使用（OpenType 特性：<code>smcp</code>）。小型大写字母指使用大写形式，但尺寸与对应小写字母相同的字母。</dd>
 ...
 <dd>允许特小型大写字母的使用（OpenType 特性：<code>pcap</code>）。</dd>
