...
<p>Flexible Box 模型，通常被称为 flexbox，是一种一维的布局模型。它给 flexbox 的子元素之间提供了强大的空间分布和对齐能力。本文给出了 flexbox 的主要特性，更多的细节将在别的文档中探索。</p>
...
<p>另外一个需要理解的重点是 flexbox 不会对文档的书写模式提供假设。过去，CSS 的书写模式主要被认为是水平的，从左到右的。现代的布局方式涵盖了书写模式的范围，所以我们不再假设一行文字是从文档的左上角开始向右书写，新的行也不是必须出现在另一行的下面。</p>
...
<p>文档中采用了 flexbox 的区域就叫做 flex 容器。为了创建 flex 容器，我们把一个容器的 {{cssxref("display")}} 属性值改为 <code>flex</code> 或者 <code>inline-flex</code>。 完成这一步之后，容器中的直系子元素就会变为 <strong>flex 元素</strong>。所有 CSS 属性都会有一个初始值，所以 flex 容器中的所有 flex 元素都会有下列行为：</p>
...
<h3 id="更改flex方向_flex-direction">更改 flex 方向 flex-direction</h3>
...
<h2 id="用flex-wrap实现多行Flex容器">用 flex-wrap 实现多行 Flex 容器</h2>
...
<p> <code>flex-basis</code> 定义了该元素的<strong>空间大小（</strong>the size of that item in terms of the space<strong>）</strong>，flex 容器里除了元素所占的空间以外的富余空间就是<strong>可用空间</strong> available space。 该属性的默认值是 <code>auto</code> 。此时，浏览器会检测这个元素是否具有确定的尺寸。 在上面的例子中，所有元素都设定了宽度（width）为 100px，所以 <code>flex-basis</code> 的值为 100px。</p>
...
<p>如果我们给上例中的所有元素设定 <code>flex-grow</code> 值为 1，容器中的可用空间会被这些元素平分。它们会延展以填满容器主轴方向上的空间。</p>

<p>flex-grow 属性可以按比例分配空间。如果第一个元素 <code>flex-grow</code> 值为 2，其他元素值为 1，则第一个元素将占有 2/4（上例中，即为 200px 中的 100px）, 另外两个元素各占有1/4（各50px）。</p>

<h3 id="Flex_元素属性：_flex-shrink">Flex 元素属性： <code>flex-shrink</code></h3>

<p><code>flex-grow</code>属性是处理 flex 元素在主轴上增加空间的问题，相反<code>flex-shrink</code>属性是处理 flex 元素收缩的问题。如果我们的容器中没有足够排列 flex 元素的空间，那么可以把 flex 元素<code>flex-shrink</code>属性设置为正整数来缩小它所占空间到<code>flex-basis</code>以下。与<code>flex-grow</code>属性一样，可以赋予不同的值来控制 flex 元素收缩的程度 —— 给<code>flex-shrink</code>属性赋予更大的数值可以比赋予小数值的同级元素收缩程度更大。</p>

<p>在计算 flex 元素收缩的大小时，它的最小尺寸也会被考虑进去，就是说实际上 flex-shrink 属性可能会和 flex-grow 属性表现的不一致。因此，我们可以在文章《<a href="/zh-CN/docs/Web/CSS/CSS_Flexible_Box_Layout/Controlling_Ratios_of_Flex_Items_Along_the_Main_Ax">控制 Flex 子元素在主轴上的比例</a>》中更详细地看一下这个算法的原理。</p>

<div class="note">
<p>在给 <code>flex-grow</code> 和 <code>flex-shrink</code> 赋值时要注意比例。如果我们给所有 flex 元素的 flex 属性赋值为 <code>1 1 200px</code> ，并且希望其中一个元素可以增加到 2 倍，我们可以给该元素的 flex 属性赋值为<code>2 1 200px</code>。当然，你也可以选择赋值为 flex: <code>10 1 200px</code> 和 flex: <code>20 1 200px</code> 。</p>
</div>

<h3 id="Flex属性的简写">Flex 属性的简写</h3>
...
<p>你可以在下面的实例中尝试把 flex 简写形式中的数值更改为不同数值，但要记得第一个数值是 <code>flex-grow</code>。赋值为正数的话是让元素增加所占空间。第二个数值是<code>flex-shrink</code> — 正数可以让它缩小所占空间，但是只有在 flex 元素总和超出主轴才会生效。最后一个数值是 <code>flex-basis</code>；flex 元素是在这个基准值的基础上缩放的。</p>
...
<p><code>flex: initial</code> 是把 flex 元素重置为 Flexbox 的初始值，它相当于 <code>flex: 0 1 auto</code>。在这里 <code>flex-grow</code> 的值为 0，所以 flex 元素不会超过它们 <code>flex-basis</code> 的尺寸。<code>flex-shrink</code> 的值为 1, 所以可以缩小 flex 元素来防止它们溢出。<code>flex-basis</code> 的值为 <code>auto</code>. Flex 元素尺寸可以是在主维度上设置的，也可以是根据内容自动得到的。</p>

<p><code>flex: auto</code> 等同于 <code>flex: 1 1 auto</code>；和上面的 <code>flex:initial</code> 基本相同，但是这种情况下，flex 元素在需要的时候既可以拉伸也可以收缩。</p>

<p><code>flex: none</code> 可以把 flex 元素设置为不可伸缩。它和设置为 <code>flex: 0 0 auto</code> 是一样的。元素既不能拉伸或者收缩，但是元素会按具有 <code>flex-basis: auto</code> 属性的 flexbox 进行布局。</p>

<p>你在教程中常看到的 <code>flex: 1</code> 或者 <code>flex: 2</code> 等等。它相当于<code>flex: 1 1 0</code>。元素可以在<code>flex-basis</code>为 0 的基础上伸缩。</p>
...
<p>Flexbox 的一个关键特性是能够设置 flex 元素沿主轴方向和交叉轴方向的对齐方式，以及它们之间的空间分配。</p>

<h3 id="align-items"><code>align-items</code></h3>

<p>{{cssxref("align-items")}} 属性可以使元素在交叉轴方向对齐。</p>

<p>这个属性的初始值为<code>stretch</code>，这就是为什么 flex 元素会默认被拉伸到最高元素的高度。实际上，它们被拉伸来填满 flex 容器 —— 最高的元素定义了容器的高度。</p>

<p>你也可以设置<code>align-items</code>的值为<code>flex-start</code>，使 flex 元素按 flex 容器的顶部对齐，<code>flex-end</code> 使它们按 flex 容器的下部对齐，或者<code>center</code>使它们居中对齐。在实例中尝试——我给出了 flex 容器的高度，以便你可以看到元素在容器中移动。看看如果更改 align-items 的值为下列值会发生什么：</p>
...
<p>在读完这篇文章之后，你应该掌握了 flexbox 的基本特性。在下一篇文章中，我们将会学习如何<a href="/zh-CN/docs/Web/CSS/CSS_Flexible_Box_Layout/Relationship_of_Flexbox_to_Other_Layout_Methods">与其他 CSS 一起使用</a>。</p>
