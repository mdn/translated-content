...
/* </code>从下到上，从蓝色开始渐变、到高度 40% 位置是绿色渐变开始、最后以红色结束<code> */
linear-gradient(0deg, blue, green 40%, red);</code></pre>

<div>{{EmbedInteractiveExample("pages/css/function-linear-gradient.html")}}  </div>

<p>如同其他 gradient 函数一般，<strong><code>linear-gradient()</code></strong> 函数<a href="/zh-CN/docs/Web/CSS/image#no_intrinsic">没有内在尺寸</a>；即，它不具备固有的或首选的尺寸，也不具备首选的比率。该函数的具体尺寸将与其适用的元素尺寸匹配。</p>
...
<p><img alt="linear-gradient.png" src="linear-gradient.png">渐变线由包含渐变图形的容器的中心点和一个角度来定义的。渐变线上的颜色值是由不同的点来定义，包括起始点，终点，以及两者之间的可选的中间点（中间点可以有多个）。</p>
...
<p>不仅仅只有起点和终点的颜色值可以指定。通过提供额外的颜色中间点，Web 开发者可以构建在起始颜色值和终点颜色值之间的自定义更强的过渡效果，另外还可以提供多种颜色值的渐变线。</p>
...
linear-gradient(red 0%, orange 25%, yellow 50%, green 75%, blue 100%);</code>
</pre>

<p>默认情况下，从一个颜色的终止点平滑的过渡到另一个颜色的终止点，颜色之间的中点是两个颜色转换的中点。你可以将中点移动到这两个颜色之间的任意位置，方法是在两个颜色之间添加未标记的 %，以指示颜色的中转位置。下面的示例是从起始点到 10% 的位置标记红色，从 90% 到结束标记蓝色。在 10% 到 90% 之间，颜色从红色过渡到蓝色，然而过渡的中点是在 30% 的标记上，而不是在没有 30% 中转点的情况下会默认为 50%。</p>
...
<dl>
 ...
 <dd>描述渐变线的起始点位置。它包含 to 和两个关键词：第一个指出水平位置 left or right，第二个指出垂直位置 top or bottom。关键词的先后顺序无影响，且都是可选的。<br>
 to top, to bottom, to left 和 to right 这些值会被转换成角度 0 度、180 度、270 度和 90 度。其余值会被转换为一个以向顶部中央方向为起点顺时针旋转的角度。渐变线的结束点与其起点中心对称。</dd>
 <dt><code>&lt;angle&gt;</code></dt>
 <dd>用角度值指定渐变的方向（或角度）。角度顺时针增加。 </dd>
 <dt><code>&lt;linear-color-stop&gt;</code></dt>
 <dd>由一个{{cssxref("&lt;color&gt;")}}值组成，并且跟随着一个可选的终点位置（可以是一个百分比值或者是沿着渐变轴的{{cssxref("&lt;length&gt;")}}）。CSS 渐变的颜色渲染采取了与 SVG 相同的规则。</dd>
 <dt><code>&lt;color-hint&gt;</code></dt>
 <dd>颜色中转点是一个插值提示，它定义了在相邻颜色之间渐变如何进行。长度定义了在两种颜色之间的哪个点停止渐变颜色应该达到颜色过渡的中点。如果省略，颜色转换的中点是两个颜色停止之间的中点。</dd>
</dl>
...
<p><code>linear-gradient的语法</code>由 2008 年实施的<a href="http://www.webkit.org/blog/175/introducing-css-gradients/">first Apple proposal</a>发展而来。</p>

<pre class="notranslate"><code>-webkit-gradient(&lt;type&gt;, &lt;point&gt; [, &lt;radius&gt;]?, &lt;point&gt; [, &lt;radius&gt;]? [, &lt;stop&gt;]*)</code>
</pre>

<p>在最初语法中，使用同样的语法实现线性渐变和径向渐变。但这两种渐变所需要的参数有所不同，导致了需要增加第一个参数来区分两种渐变。如果再增加渐变类型，这样的处理方式会变得更加复杂。比如锥形渐变，需要用到函数和不规范的 CSS 值。W3C 并未收到相关草案。</p>

<p>一个替代语法在 2009 年由 Mozilla 提出并实现。这个语法需要两个 CSS 函数，一个用来做线性渐变，另一个用于径向渐变。然而，这个语法并没有被发布产品实现。有人提出了第三种语法，它将线性渐变的语法简化为：</p>

<pre class="notranslate">-moz-linear-gradient(<code>[ [ [top | bottom] || [left | right] ],]? &lt;color-stop&gt;[, &lt;color-stop&gt;]+);</code>
</pre>

<p>新的语法不需要<code>to()、</code><code>from()和</code><code>color-stop()函数，所以这些函数被丢弃。而</code><code>top</code>/<code>bottom</code>与<code>left</code>/<code>right的顺序也被标记为不重要，所以</code>Mozilla 移除了必需首先定义<code>top</code>/<code>bottom</code>的限制。</p>
...
<ul>
 ...
 <li>定义 magic corner 算法，允许使用简便的方式定义端点的颜色，从而简化了开发者的工作</li>
</ul>

<p>在预乘颜色空间里定义过渡色，从而可以防止在使用不同透明度颜色的情况下出现违和的灰色。在未舍弃原生语法的情况下，带前缀的版本被 Webkit 和 Trident（IE）实现。</p>
...
<p>在带前缀的变体和不带前缀的提案之间仍然存在一项语义的分歧。最终 Apple 的提案显示，带前缀的语法都使用极坐标定义{{ cssxref("&lt;angle&gt;") }}参数，导致了<code>0deg指向东方。为了与CSS的其他部分保持一致，标准将0deg指向北方。为了防止使用前缀版本属性的站点不至于崩溃，它们保持原始的角度定义（0deg指向东方）。在使用不带前缀版本的时候将会切换到正确的规格。在这种不兼容的情况下，</code>Gecko 给所有语法都加上前缀，不带前缀且没有 to 关键词的语法会被丢弃。</p>

<h2 id="范例">范例</h2>

<h3 id="例_45度渐变">例 45 度渐变</h3>

<p>可以为渐变轴某个位置指定某个颜色，这些位置可被称为“颜色中间点（color-stops）”。颜色会从一个颜色中间点平滑过渡到下一个。在渐变范围内，每个颜色都表现为一条垂直于渐变轴的直线。在下图中，渐变轴从左上角出发并且呈 45 度。渐变轴上定义了红色和蓝色两个颜色中间点。</p>
...
<p>{{EmbedLiveSample("45 度渐变",120,120)}}</p>

<h3 id="从60％的梯度线开始的渐变">从 60％的梯度线开始的渐变</h3>
...
<p>{{EmbedLiveSample("从 60％的梯度线开始的渐变")}}</p>

<h3 id="具有多个颜色停止的渐变">具有多个颜色停止的渐变</h3>

<p>如果第一个颜色中间点没有 <code>&lt;length&gt;</code> 或 <code>&lt;percentage&gt;</code>属性，那么它默认为 0%。如果最后一个颜色中间点没有 <code>&lt;length&gt;</code> 或者 <code>&lt;percentage&gt;</code>属性，则默认为 100%。如果一个既不是起始也不是终止的颜色中间点，没有被明确声明位置，那么这个颜色就会从前后两个颜色的中间位置开始。</p>
...
<p>如果第一个颜色中间点没有定义长度或百分比，它将包含缺省值 0。如果最后一个颜色中间点没有定义长度或百分比，它将包含缺省值 100%。如果中间的颜色中间点没定义长度或百分比，那么它将被设定为前后两站的平均值。</p>
...
<p>如果所有点和长度都使用固定单位（而不是相对于 background-size 的值指定的百分比或关键字），则渐变背景不受 {{Cssxref("background-size")}} 的影响。</p>
...
<div class="note">
<p>Notes：如果将&lt;body&gt;标签的 background-image 属性设置为线性渐变<code>linear-gradient</code>，除非您还设置文档根标签（例如&lt;html&gt;标签）的<a href="/zh-CN/docs/Web/CSS/min-height">min-height</a>属性，否则渐变不会填充浏览器屏幕到 100％。</p>
</div>
