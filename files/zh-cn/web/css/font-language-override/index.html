...
<p><strong>font-language-override</strong> CSS 属性控制在指定的语言中使用特定的字形。</p>
...
<p>默认情况下，HTML 的 <code>lang</code> 属性告诉浏览器使用语言的特定字形。例如，很多字体都有针对合成符 <code>fi</code> 的字形（i 上的点会被合并进 f 里）。而当使用土耳其语时，字体就不会使用合成字形。因为土耳其语有两个版本的 i: 一个带点 (<code>i</code>) 一个不带点 (<code>ı</code>)。这种情况下如果使用连写会把带点的 <code>i</code> 不正确地渲染成不带点的 <code>ı</code>。</p>
