...
<p>一个元素的尺寸和位置经常受其<strong>包含块 (containing block)</strong>的影响。大多数情况下，包含块就是这个元素最近的祖先<a href="/zh-CN/docs/Web/HTML/Block-level_elements">块元素</a>的<a href="/zh-CN/docs/Web/CSS/CSS_Box_Model/Introduction_to_the_CSS_box_model#content-area">内容区</a>，但也不是总是这样。在本文中，我们来过一遍确定包含块的所有因素。</p>
...
<p>许多开发者认为一个元素的包含块就是他的父元素的内容区。但事实并非如此。接下来让我们来看看，确定元素包含块的因素都有哪些。</p>

<h2 id="包含块的影响">包含块的影响</h2>

<p>在学习如何确定元素包含块之前，先了解一下它的重要性。</p>

<p>元素的尺寸及位置，常常会受它的包含块所影响。对于一些属性，例如 {{cssxref("width")}}, {{cssxref("height")}}, {{cssxref("padding")}}, {{cssxref("margin")}}，绝对定位元素的偏移值（比如 {{cssxref("position")}} 被设置为 <code>absolute</code> 或 <code>fixed</code>），当我们对其赋予百分比值时，这些值的计算值，就是通过元素的包含块计算得来。</p>

<h2 id="确定包含块">确定包含块</h2>

<p>确定一个元素的包含块的过程完全依赖于这个元素的 {{cssxref("position")}} 属性：</p>

<ol>
 <li>如果 position 属性为 <code><strong>static</strong></code>、<code><strong>relative</strong></code><strong> 或 <code>sticky</code></strong>，包含块可能由它的最近的祖先<strong>块元素</strong>（比如说 inline-block, block 或 list-item 元素）的内容区的边缘组成，也可能会建立格式化上下文 (比如说 a table container, flex container, grid container, 或者是 the block container 自身)。</li>
 ...
 <li>如果 position 属性是 <code><strong>absolute</strong></code> 或 <code><strong>fixed</strong></code>，包含块也可能是由满足以下条件的最近父级元素的内边距区的边缘组成的：
  <ol>
   <li>{{cssxref("transform")}} 或 {{cssxref("perspective")}} 的值不是 <code>none</code></li>
   <li>{{cssxref("will-change")}} 的值是 <code>transform</code> 或 <code>perspective</code></li>
   <li>{{cssxref("filter")}} 的值不是 <code>none</code> 或 <code>will-change</code> 的值是 <code>filter</code>(只在 Firefox 下生效).</li>
...
<div class="note">
<p><strong>注意：</strong>根元素 (&lt;html&gt;) 所在的包含块是一个被称为<strong>初始包含块</strong>的矩形。他的尺寸是视口 viewport (for continuous media) 或分页媒体 page media (for paged media).</p>
</div>

<h2 id="根据包含块计算百分值">根据包含块计算百分值</h2>

<p>如上所述，如果某些属性被赋予一个百分值的话，它的计算值是由这个元素的包含块计算而来的。这些属性包括盒模型属性和偏移属性：</p>

<ol>
 <li>要计算 {{cssxref("height")}} {{cssxref("top")}} 及 {{cssxref("bottom")}} 中的百分值，是通过包含块的 <code>height</code> 的值。如果包含块的 <code>height</code> 值会根据它的内容变化，而且包含块的 <code>position</code> 属性的值被赋予 <code>relative</code> 或 <code>static</code> ，那么，这些值的计算值为 auto。</li>
 <li>要计算 {{cssxref("width")}}, {{cssxref("left")}}, {{cssxref("right")}}, {{cssxref("padding")}}, {{cssxref("margin")}} 这些属性由包含块的 <code>width</code> 属性的值来计算它的百分值。</li>
</ol>

<h2 id="示例">示例</h2>

<p>接下来的示例，都使用如下 HTML 代码：</p>
