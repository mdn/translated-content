...
由 `:link` 和 [`:visited`](/zh-CN/docs/Web/CSS/:visited) 伪类定义的样式可以被任何具有相等特殊性的后续用户动作伪类（例如 {{cssxref(":hover")}} 或 {{cssxref(":active")}}）覆盖。为了适当地为链接设置样式，请按照 _LVHA 顺序_ 将 `:link` 规则放置在所有其他链接相关规则之前，顺序为 `:link` — `:visited` — `:hover` — `:active`。`:visited` 伪类和 `:link` 伪类是互斥的。
