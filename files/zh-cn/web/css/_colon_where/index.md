...
在这个优点人为的例子中，我们有两篇文章，每篇文章包含一个段落、一个侧边栏和一个页脚。它们由于使用标记子元素的不同而不同。
...
这个红色的链接不起作用，因为 `:is()` 中的选择器会计入整体选择器的优先级，并且类选择器的优先级高于元素元素器。

然而， `:where()` 中的选择器的优先级时 0，所以橘色的页脚链接将会覆盖将被我们的简单选择器覆盖。
