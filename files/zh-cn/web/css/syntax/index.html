...
<p>层叠样式表（Cascading Stylesheet，简称<a href="/en/CSS">CSS</a> ），其基本目标是让浏览器以指定的特性去绘制页面元素，比如颜色，定位，装饰。CSS 的语法反映了这个目标，由下面两个部分构建：</p>
...
<p>CSS 的核心功能是将 CSS 属性设定为特定的值。一个属性与值的键值对被称为”声明“（declaration） 。CSS 引擎会计算页面上每个元素都有哪些声明，并且会根据结果绘制元素，排布样式。<br>
 <br>
...
<p>目前 CSS 中有 <a href="/en/CSS/CSS_Reference">100 多个属性</a> ，并且几乎有无限多个不同的值。并非所有的属性与值的配对都是被允许的，每个属性都定义了它的合法值。如果值对给定的属性而言非法时，声明就会被视为无效的，整个声明就会被 CSS 引擎忽略。</p>
...
<div class="note">声明块的内容—开始与结束大括号之间的声明，可以放在 HTML <a href="/en/HTML/Global_attributes#attr-style"><code>style</code></a> 特性里。</div>

<h2 id="CSS_规则">CSS 规则</h2>

<p>如果样式表只能为每个页面元素添加一个声明，那就没有真正发挥出它的价值。其真正的目标是为文档不同部分添加不同的声明。<br>
 <br>
 为此，CSS 可以在声明块前面放置选择器（selector)，选择器用来选择页面多个元素的条件。一对选择器与声明块称为规则集（ruleset)，常简称为规则（rule)。</p>
...
<p>一个元素可能被多个选择器选中，因此会有多个规则，有可能以不同的值去设置同一属性。CSS 标准会规定哪个优先级最高并生效，称之为 <a href="/en/CSS/Getting_Started/Cascading_and_inheritance">层叠（cascade)</a> 算法。</p>
...
<ul>
 <li>规则。如上，将一组 CSS 声明与用选择器定义的条件相关联。</li>
 <li>at 规则（<a href="https://developer.mozilla.org/en/CSS/At-rule">at-rules</a>）。以 @ (U+0040 COMMERCIAL AT) 开始，随后是标识符，一直到以分号或右大括号结束。每个 at 规则由其标识符定义，可能有它自己的语法。at 规则涵盖了 meta 信息（比如 @charset  @import)，条件信息（比如 @media  @document), 描述信息（比如 @font-face)。</li>
</ul>
