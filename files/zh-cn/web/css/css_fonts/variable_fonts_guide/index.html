...
---
<p><strong>可变字体（Variable fonts）</strong>是 OpenType 字体规范上的演进，它允许将同一字体的多个变体统合进单独的字体文件中。从而无需再将不同字宽、字重或不同样式的字体分割成不同的字体文件。你只需通过 CSS 与一行{{cssxref("@font-face")}}引用，即可获取包含在这个单一文件中的各种字体变体。本文将介绍如何在 Web 中使用可变字体。</p>
...
<p>在这样的情景下，为了在一个单纯显示进行正文展示的页面中使用一个字体，你至少需要四个字体文件：常规、斜体、加粗、斜体加粗。如果你想添加更多的字重，比如让题注更轻或让额外强调的地方更重，意味着你需要更多文件。这就导致了更多大量 HTTP 请求的产生，同时也需要下载更多的数据（通常每个文件至少有 20k 或更多）。</p>
...
<p>您可能还会发现一些可变字体分为两个文件：一个用于正体及其所有变体，另一个包含斜体变体。有时这样做是为了在不需要或不使用斜体的情况下减小整体文件大小。在所有情况下，仍然可以通过通用的  {{cssxref("font-family")}}  名称链接它们，以便您可以使用相同的 font-family 和相应的  {{cssxref("font-style")}} 来调用它们。</p>

<h2 id="介绍“可变轴”_variation_axis">介绍“可变轴” (variation axis)</h2>

<p>可变字体新格式的核心是<strong>可变轴</strong>的概念，其描述了字体设计中某一特性的允许变化范围。例如‘字重轴’描述了字体的粗细；“宽度轴”描述了字体的宽窄；“斜体轴”描述是否使用斜体字形并且可相应地开关；等。请注意，轴既可以是范围选择又可以是开关选择。字重可能在 1-999 之间，而斜体可能只是简单的 0 或 1（关闭或打开）。</p>

<p>如规范中所定义，存在两种变形轴，<strong>注册轴</strong>和<strong>自定义轴</strong>：</p>

<ul>
 <li>
  <p>注册轴最为常见，常见到制定规范的作者认为有必要进行标准化。目前注册的五个轴是字重，宽度，倾斜度，斜体和光学尺寸。W3C 已经将它们映射到现有的 CSS 属性，并在一个案例中引入了一个新的属性，您将在下面看到。</p>
 </li>
 <li>
  <p>自定义轴实际上是无限的：字体设计师可以定义和界定他们喜欢的任何轴，并且只需要给它一个四个字母的标签以在字体文件格式本身中识别它。您可以在 CSS 中使用这些四个字母的标签来指定沿该变化轴的点，您也将在下面的代码示例中看到。</p>
 </li>
</ul>

<h3 id="注册轴和现有的CSS属性">注册轴和现有的 CSS 属性</h3>

<p>在本节中，我们将通过示例和相应的 CSS 来演示已定义的五个注册轴。在可能的情况下，包括标准语法和低级语法。较低级语法 ({{cssxref("font-variation-settings")}}) 是为了测试对可变字体支持的早期实现而实现的第一个机制，并且需要使用五个注册轴之外的新轴或自定义轴。但是，W3C 的意图是在其他属性可用时不使用此语法。因此，应尽可能使用相应的属性，<code>font-variation-settings</code>的低级语法仅用于设置值不可用的值或轴。</p>
...
<ol>
 ...
 <li>
  <p>如果您使用 <code>font-variation-settings</code> 设置了值并想要更改其中一个值，则必须重新声明所有值。您可以通过对各个值使用<a href="https://developer.mozilla.org/zh-CN/docs/Web/CSS/Using_CSS_custom_properties">CSS 自定义属性</a>（CSS 变量）来解决此限制，并只需修改单个自定义属性的值。该指南末尾有示例代码。</p>
 </li>
</ol>

<h3 id="字重">字重</h3>

<p>字重（由 <code>wght</code> 标签表示）是定义了字形的笔画粗细范围（在印刷术语中称为轻重）的设计轴。很长一段时间，CSS 都能够通过 {{cssxref("font-weight")}}) 这个采用 100 到 900 之间的以 100 为增量的数字值以及类似于 <code>normal</code> 或 <code>bold</code> 等这样有相应数字值的别名（在这种情况下为 400 和 700）的关键字的属性来实现。在处理非可变或可变字体时仍然会用到这些数值或关键字，但对于可变字体，1 到 1000 之间的任何数字都是有效的。</p>
...
<p>字宽（由 <code>wdth</code> 标签表示）定义字形可以是多窄或多宽（印刷术语中称为压缩或扩展）的设计轴。这通常在 CSS 中通过使用 {{cssxref("font-stretch")}}  属性设置，该属性值表示为高于或低于“正常”的百分比（100％），任何大于 0 的数字在技术上都是有效的 - 尽管它的范围更有可能是接近 100％的，例如 75％-125％。如果提供的数字值超出了字体编码的范围，浏览器应该以允许的最接近值渲染字体。</p>
...
<p>可以通过编辑下面实例的 CSS 来使用字宽值。</p>

<div>{{EmbedGHLiveSample("css-examples/variable-fonts/width.html", '100%', 520)}}</div>

<h3 id="斜体">斜体</h3>

<p>Italic（<code>ital</code>）轴的工作方式不同，因为它仅可以打开或关闭; 而没有中间值。斜体设计通常包括与其正体截然不同的字形，因此在从正体到斜体的过渡中，通常会发生许多字形（或字符）替换。Italic 和 oblique 通常可以互换使用，但事实上实际上是完全不同的。在这种情况下，Oblique 定义为术语 <code>slant</code> （参见下面的部分），并且字体通常具有一个或另一个斜体，但不是两者都有。</p>

<p>在 CSS 中，使用 {{cssxref("font-style")}} 属性对文本应用 italic 和 oblique。</p>

<pre class="brush: css notranslate">font-style: italic;

font-variation-settings: 'ital' 1;</pre>

<p>可以通过编辑下面实例的 CSS 来使用斜体值。</p>

<div>{{EmbedGHLiveSample("css-examples/variable-fonts/italic.html", '100%', 520)}}</div>

<h3 id="倾斜">倾斜</h3>

<p>Slant（由 <code>slnt</code> 标签表示），或者它经常被称为“oblique” - 与真正的斜体不同之处在于它改变了字体的角度但不执行任何类型的字符替换。它也是可变的，因为它表示为数值范围。这允许字体沿该轴的任何位置变化。允许的范围通常为 0（直立）到 20 度 — 可以使用该范围内的任何数值，因此字体可以倾斜一点点。但是，-90-90 度的任何值都是有效的。</p>
...
<p>这是数字字体和 CSS 的新功能，但其实对应于设计加工金属活字中的一项古老技艺。视觉尺寸是指基于物理尺寸改变字形的整体笔画厚度的做法。如果尺寸非常小（例如等于 10 或 12px），则字符将具有整体较粗的笔画，可能还会进行其他小的修改，以确保以较小的物理尺寸复制和可读性。相反，当使用更大尺寸（如 48 或 60px）时，笔画的粗细重量可能会有更大的变化，外观会更接近原始设计意图。</p>
...
<p>创建了一个在 CSS 中中支持可变字体的新属性 {{cssxref("font-optical-sizing")}} 。使用 <code>font-optical-sizing</code> 时，唯一允许的值是 <code>auto</code> 或 <code>none</code> - 因此该属性仅允许打开或关闭光学尺寸。但是，当使用 <code>font-variation-settings: 'opsz' &lt;num&gt;</code>时，您可以使用数值。在大多数情况下，您可能希望将 <code>font-size</code>（正在渲染的类型的物理大小）与 <code>opsz</code> 值（使用 <code>auto</code>时应用光学尺寸的方式）进行匹配。提供了特定值，以便在必要时（为了易认性，美观或其他原因）可以应用特定值来覆盖默认值。</p>
...
<p>可以使用 CSS 特性查询检查是否支持可变字体（请参阅 <a href="https://developer.mozilla.org/zh-CN/docs/Web/CSS/@supports">{{cssxref("@supports")}}</a>），因此在生产中，可以在特性查询块内使用可变字体。</p>
...
<p>以下示例页面展示了构造 CSS 的两种不同方法。第一种方法尽可能使用标准属性。第二个示例使用 CSS 自定义属性设置 <code>font-variation-settings</code> 字符串的值，并展示如何通过覆盖单个变量而不是重写整个字符串来更轻松地更新单个变量值。请注意 <code>h2</code>上的悬停效果，它只会改变等级轴自定义属性值。</p>
