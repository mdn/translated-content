...
<div class="warning">
<p><strong>注意：</strong>CSS 定义了很多的属性用以改变字体特性，然而很多没有完全实现。这里给出了所有属性的定义，但是大部分都只能用底层属性 {{cssxref("font-feature-settings")}} 来控制。可以同时使用两种方式定义 CSS，但是可能太笨重了。而只使用 <code>font-feature-settings</code> 控制一切的问题是，每次你要变动一个特性，就必须处理整个字符串（类似于用 {{cssxref("font-variation-settings")}} 操纵变量字体）。</p>
</div>
