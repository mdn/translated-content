...
[`<p>`](/zh-CN/docs/Web/HTML/Reference/Elements/p) 元素的 [`font-size`](/zh-CN/docs/Web/CSS/Reference/Properties/font-size) 设置为 `max(1.2rem, 1.2vw)`。其含义是 `font-size` 是 `1.2rem`，当计算出 `1.2vw` 比 `1.2rem` 大时，则 `font-size` 为 `1.2vw` 。 `max()` 允许你设置一个下限值。
