...
- _url_
  - : 是一个表示要引入资源位置的 {{cssxref("&lt;string&gt;")}} 或者 {{cssxref("&lt;uri&gt;")}} 。 这个 URL 可以是绝对路径或者相对路径。 要注意的是这个 URL 不需要指明一个文件； 可以只指明包名，然后合适的文件会被自动选择 (e.g. **chrome://communicator/skin/**). [See here](/en-US/docs/Mozilla/Tech/XUL/Tutorial/The_Chrome_URL) 了解更多。
- _list-of-media-queries_
  - : 是一个逗号分隔的 [媒体查询](/en-US/docs/Web/CSS/Media_Queries/Using_media_queries) 条件列表，决定通过 URL 引入的 CSS 规则 在什么条件下应用。如果浏览器不支持列表中的任何一条媒体查询条件，就不会引入 URL 指明的 CSS 文件。
