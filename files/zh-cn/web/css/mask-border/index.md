...
该属性为以下属性的[简写](/zh-CN/docs/Web/CSS/Shorthand_properties)：{{cssxref("mask-border-source")}}、{{cssxref("mask-border-slice")}}、{{cssxref("mask-border-width")}}、{{cssxref("mask-border-outset")}}、{{cssxref("mask-border-repeat")}} 和 {{cssxref("mask-border-mode")}}。与其他简写的属性一样，任何一个漏写的子属性，将会被设置为他们的[初始值](/zh-CN/docs/Web/CSS/CSS_cascade/initial_value)。
