...
<ul>
 ...
 <li>当<strong>2 点</strong>被指定的位置，所述第一值创建一个从测量的层<strong>的顶部和底部</strong>，所述第二创建从所测量的片<strong>左右</strong>。</li>
...
<ul>
 <li>1-4 区是<a id="corner-regions" name="corner-regions">拐角区域</a>。每个图像都使用一次以形成最终边框图像的角。</li>
 <li>5-8 区是<a id="edge-regions" name="edge-regions">边缘区域</a>。在最终的边框图像中对这些元素进行<a href="/en-US/docs/Web/CSS/mask-border-repeat">重复，缩放或其他方式的修改</a>，以匹配元素的尺寸。</li>
 <li>9 区是<a id="middle-region" name="middle-region">中间区域</a>。默认情况下将其丢弃，但如果<code>fill</code>设置了关键字，则它会用作背景图像。</li>
</ul>
...
<p>基于 Chromium 的浏览器支持此属性的过时版本<code>mask-box-image-slice</code>-带前缀：</p>

<pre class="brush: css notranslate">-webkit-mask-box-image-slice: 30 fill;</pre>

<div class="blockIndicator note">
<p><strong>注意</strong>：该<code><a href="/en-US/docs/Web/CSS/mask-border">mask-border</a></code>页面提供了一个有效的示例（使用 Chromium 支持的过期前缀边框蒙版属性），因此您可以大致了解效果。</p>
</div>

<div class="blockIndicator note">
<p><strong>注意</strong>：如果希望元素的内容可见，则必须包含 fill 关键字。</p>
</div>
...
<ul>
 <li><a href="/en-US/docs/Web/CSS/Shorthand_properties#Tricky_edge_cases">1 到 4 值语法的图解说明</a></li>
</ul>
