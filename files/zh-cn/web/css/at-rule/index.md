...
- {{cssxref("@font-feature-values")}} (加上 `@swash`, `@ornaments`, `@annotation`, `@stylistic`, `@styleset` and `@character-variant`) — 在 {{cssxref("font-variant-alternates")}} 中定义通用名称，以便在 OpenType 中以不同方式激活功能。 _(在候选推荐阶段，仅在Gecko中实现)_
- {{cssxref("@property")}} {{experimental_inline}} — 描述自定义属性和变量。_（目前处于工作草案阶段）_
- {{cssxref("@layer")}} – 声明一个级联层，并在有多个级联层时定义优先顺序。

## 条件规则组

就像属性值那样，每条 @规则都有不同的语法。不过一些 @规则可以归为一个特殊的分类：**条件规则组**。这些语句使用相同的语法。它们都可以包括 _嵌套语句_——*规则集*或者是*嵌套 @规则*。它们都表达：它们所指的条件 (类型不同) 总等效于 **true** 或者 **false**，如果为 **true** 那么它们里面的语句生效。
