...
要使网格容器成为一个包含块，需要为容器增加 `position` 属性，并把它的值设置为 `relative`，就像为任何其它绝对定位的元素创建一个包含块一样。接下来，如果再把一个网格元素设置为 `position: absolute`，那么网格容器就成为包含块，或者如果该元素具有网格定位的属性，那么被放置的网格就成为包含块。
...
如果具有绝对定位的子元素的父级是网格容器，即使网格容器没有被设置为新的定位上下文，该子元素仍会脱离文档流。定位是基于定位上下文的，和这其它布局方式没有区别。在我们的示例中，如果把容器的 `position: relative` 属性移除掉，定位上下文就变成了当前视口，像下图中显示的那样。

![包含父级的网格容器的图像](2_abspos_example.png)

此外，该元素不再被视为网格布局的一部分，当网格中其它元素调整尺寸或自动布局时，都与该元素无关。
...
正如本文中所述，CSS 网格布局只是你的工具包的一部分。不要害怕在布局时将它与其它布局方法混用，大胆用它获得你想要的效果吧。

## 参见

- [弹性盒子指南](/zh-CN/docs/Learn/CSS/CSS_layout/Flexbox)
- [多列布局指南](/zh-CN/docs/Web/CSS/CSS_Columns)
