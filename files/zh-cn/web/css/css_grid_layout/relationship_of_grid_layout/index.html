...
---
<p>CSS 网格布局作为实现布局的一个完整系统，其设计使之和 css 其它部分能协同合作。在这个指南中，我会向你解释网格布局是如何与你正在使用的其它技巧协同合作的。</p>
...
<p>这个例子中我用弹性盒布置了一些盒子。容器中有 5 个元素，它们被设置了伸缩属性，在 200px 基准上伸缩。</p>
...
<h3 id="用CSS网格达成同样布局">用 CSS 网格达成同样布局</h3>

<p>下例中我用网格创建同样的布局。这次我们有 3 个 1fr 的列轨道。我们并不需要给这些子元素设置任何属性，它们会自动按顺序填充到网格的单元格中。你可以看到它们按网格规整的排列，行与行、列与列对齐。当有 5 个子元素时，第二行的尾部会留出一个空隙。</p>
...
<p>{{ EmbedLiveSample('用 CSS 网格达成同样布局', '300', '170') }}</p>
...
<p>网格则从布局入手。当使用 CSS 网格时，你先创建网格，然后再把元素放入网格中，或者让自动放置规则根据把元素按照网格排列。我们能创建根据内容改变大小的网格轨道，但整个轨道的大小都会随之改变。</p>

<p>当你使用弹性盒，并发现自己禁用了一些弹性特性，那你可能需要的是 CSS 网格布局。例如，你给一个弹性元素设置百分比宽度来使它和上一行的元素对齐。这种情况下，网格很可能是一个更好的选择。</p>
...
<p>在这第一个例子中，我使用了弹性盒，一个容器内有三个元素。wrapper 设定了 {{cssxref("min-height")}}，确定了弹性容器的高度。我还在弹性容器中定义了{{cssxref("align-items")}}: <code>flex-end, 因此子元素会从弹性容器尾部开始排列。我还在box1上定义了</code>{{cssxref("align-self")}}<code>属性，这会覆盖默认值，使它延长至容器的高度；box2上也定义了</code>{{cssxref("align-self")}}，所以它会与弹性容器的起点对齐。</p>
...
<p>我们已经看到了 fr 单元如何在网格容器中分配一定比例的可用空间到我们的网格轨道。当与{{cssxref("minmax", "minmax()")}} 函数结合使用时，fr 单元可以给我们与 flexbox 中的 flex 属性非常相似的行为 - 同时仍然可以创建二维布局。</p>

<p>之前我们通过例子验证了一维布局和二维布局的差异，可以发现这两种布局方式对响应式的处理存在着不同。对于弹性布局，当我们拖拽以改变浏览器大小时，弹性盒会根据可用空间适当的调整行中项目的个数。当有足够的空间时，全部的 5 个项目全部在一行展示，当容器变的过窄时，每行则只能展示一个项目。</p>

<p>相较而言，网格布局则始终保持 3 列轨道。轨道会自动拉伸，但始终会保持我们定义网格时的轨道数量，也就是 3 个。</p>
...
<p>在接下来的例子中，在<code>repeat</code>方法中使用<code>auto-fill</code>属性替换整数值，并且设置轨道的宽度为 200px。这意味着网格布局将会根据容器的宽度创建最多的宽度为 200px 的列轨道。</p>
...
<p>到这为止就和弹性盒布局不太一样了。在弹性盒布局例子中，子元素在被包裹起来之前大于基准的 200px。我们可以用使用网格布局中的<code>auto-fill</code>和{{cssxref("minmax", "minmax()")}}函数实现同样的功能。在这个例子中，我创建了一个用<code>minmax</code>自动填充的轨道。我想要这个轨道的元素最小有 200px，最大<code>1fr</code>的宽度。 一旦浏览器计算出有多少个 200px 会填充到这个容器里面，并算好网格间距，那么，浏览器就会把剩余的空间等分成以最大<code>1fr</code>单位计算的区域给其他元素。</p>
...
<p>可以看到该项目占据的区域是纵向的第 2 条线到第 4 条线，横向起于第 1 条线。然后使用 <code>top</code> 和 <code>left</code> 属性在该区域中进行偏移。就像通常绝对定位的元素会脱离标准文档流一样，该项目也脱离了文档流，被自动定位规则定位在包含块中（译注：它会叠加在其他元素之上）。因为该项目脱离了文档流，所以不会再创建一个跨越纵向第 3 条线的轨道。</p>
...
<ol>
 ...
 <li><a href="#"><strong>指南</strong></a>
  <ol>
   ...
   <li><a href="/zh-CN/docs/Web/CSS/CSS_Grid_Layout/CSS_Grid_Layout_and_Accessibility">CSS 网格布局和可访问性</a></li>
