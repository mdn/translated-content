...
[浮动](/zh-CN/docs/Learn_web_development/Core/CSS_layout/Floats)曾经用于创建多列布局。如果你在旧代码库使用了浮动布局，则不会发生冲突。网格项目会忽略浮动属性；事实上，_网格项目优先级更高_。在下面的示例中，我们有一个简单的 media 对象。如果 {{cssxref("float")}} 没有从遗留的 CSS 中删除，且容器是网格容器，也不会有问题。我们可以使用 CSS 网格中实现的对齐属性。
