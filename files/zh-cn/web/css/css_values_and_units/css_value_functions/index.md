...
> [!NOTE]
> CSS 值函数是作为属性值使用的，不应该与伪类相混淆。[函数式伪类](/zh-CN/docs/Web/CSS/Reference/Selectors/Pseudo-classes#函数式伪类)、[语言伪类](/zh-CN/docs/Web/CSS/Reference/Selectors/Pseudo-classes#语言的伪类)和几种[树状结构伪类](/zh-CN/docs/Web/CSS/Reference/Selectors/Pseudo-classes#树结构的伪类)都需要参数值，但它们不是值函数。[条件 @ 规则](/zh-CN/docs/Web/CSS/CSS_syntax/At-rule#条件规则组)也不是值函数；括号是用来分组的。
