...
在标准化之前，这个属性允许将简单的元素显示为小部件，例如按钮或复选框。这被认为是一个缺陷，现在鼓励开发者仅使用标准关键字。

> **备注：** 如果您想在网站上使用此属性，应非常谨慎地进行测试。虽然它在大多数现代浏览器中得到支持，但其实现方式不同。在旧的浏览器中，即使是关键字 `none` 对于不同浏览器中的所有表单元素也没有相同的效果，有些浏览器甚至不支持它。在最新的浏览器中差异较小。
...
| `checkbox-container`                   | Firefox                    | 该元素的外观类似于复选框的容器，可能包括在某些平台下的预光背景效果。通常它会包含一个标签和一个复选框。 |
