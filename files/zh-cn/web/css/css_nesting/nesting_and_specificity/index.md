...
在这个示例中，ID 选择器（`#a`）的优先级是 [`1-0-0`](/zh-CN/docs/Web/CSS/CSS_cascade/Specificity#选择器类型)，而类型选择器（`b`）的优先级是 `0-0-1`。[`&` 嵌套选择器](/zh-CN/docs/Web/CSS/Reference/Selectors/Nesting_selector)和 `:is()` 伪类均有 `1-0-0` 的优先级，尽管 ID 选择器 `#a` 从未使用。
