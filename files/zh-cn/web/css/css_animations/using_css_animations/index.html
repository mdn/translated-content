...
<dl>
 <dt><strong>{{ cssxref("animation-delay") }}</strong></dt>
 <dd>设置延时，即从元素加载完成之后到动画序列开始执行的这段时间。</dd>
 <dt><strong>{{ cssxref("animation-direction") }}</strong></dt>
 <dd>设置动画在每次运行完后是反向运行还是重新回到开始位置重复运行。</dd>
 <dt><strong>{{ cssxref("animation-duration") }}</strong></dt>
 <dd>设置动画一个周期的时长。</dd>
 <dt><strong>{{ cssxref("animation-iteration-count") }}</strong></dt>
 <dd>设置动画重复次数，可以指定 infinite 无限次重复动画</dd>
 <dt><strong>{{ cssxref("animation-name") }}</strong></dt>
 <dd>指定由{{ cssxref("@keyframes") }}描述的关键帧名称。</dd>
 <dt><strong>{{ cssxref("animation-play-state") }}</strong></dt>
 <dd>允许暂停和恢复动画。</dd>
 <dt><strong>{{ cssxref("animation-timing-function") }}</strong></dt>
 <dd>设置动画速度，即通过建立加速度曲线，设置动画在关键帧之间是如何变化。</dd>
 <dt><strong>{{ cssxref("animation-fill-mode") }}</strong></dt>
