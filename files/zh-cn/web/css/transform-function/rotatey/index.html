...
<p><strong><code>rotateY()</code></strong>  函数定义了一个转换，它可以让一个元素围绕纵坐标 (垂直轴) 旋转，而不会对其进行变形。它的结果是一个{{cssxref("&lt;transform-function&gt;")}} 数据类型。</p>
...
<div class="note">
<p><strong>注意：</strong> <code>rotateY(a)</code> 相当于 <code><a href="/en-US/docs/Web/CSS/transform-function/rotate3d">rotate3d</a>(0, 1, 0, a)</code>.</p>
</div>

<div class="note"><strong>注意：</strong> 与二维平面上的旋转不同，三维旋转的组合顺序通常是不可交换的。换句话说，三维旋转的应用顺序，将会影响最终结果。</div>

<h2 id="语法">语法</h2>

<p><strong><code>rotateY() </code></strong>引起的旋转量由{{cssxref("&lt;angle&gt;")}}指定。如果为正，则顺时针方向移动；如果为负，则逆时针方向移动。</p>
