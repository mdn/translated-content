...
<p>多列布局（通常称为 multicol）是一种规范，用于将内容布置到一组列框中，就像报纸中的列一样。本指南解释了规范如何与一些常见的用例示例一起使用。</p>

<h2 id="关键概念和术语">关键概念和术语</h2>

<p> </p>

<p>Multicol 与 CSS 中的任何其他布局方法不同，它将内容（包括所有后代元素）分段为列。这与在使用 CSS Paged Media 时将内容分段为页面的方式相同，例如通过创建打印样式表。</p>
...
<p>在以下示例中，我们使用 column-count 属性在 <code>.container</code> 元素上创建三列。 <code>.container</code> 元素的内容包括其子元素都会分裂成三列。</p>
...
<p><code>column-width</code> 属性用于给每列设置一个最佳宽度。如果你声明 <code>column-width</code>,浏览器将算出 该宽度在 multicol 容器能分多少列，并且把额外的的空间填充到这些列当中。因此，应将列宽视为最小宽度，因为由于额外的空间，列可能而更宽。</p>

<p>当单个列的可用宽度小于 column-width 的值的情况下，列的宽度会缩小为小于所声明的列宽。</p>

<p>在以下示例中，我们使用 column-width 属性值为 200 px。但最终为了适配容器，列的宽度却大于 200 像素，额外的空间被平均分配了。</p>
...
<p>此 CSS 结果与示例 1 相同，column-count 设置为 3。</p>

<pre>.container {
  columns: 3;
}</pre>

<p>此 CSS 结果与示例 2 相同，column-width 为 200px。</p>

<pre>.container {
  columns: 200px;
}</pre>

<p>此 CSS 结果与示例 3 相同，同时设置 column-count 和 column-width。</p>
