...
具名颜色由标准颜色、关键字 [`transparent`](#transparent) 和 {{CSSXref("&lt;color&gt;", "currentcolor", "#currentcolor_keyword")}} 所构成。
...
所有名字均在 [sRGB 颜色空间](https://en.wikipedia.org/wiki/SRGB)中指定了一种颜色。虽然这些名字或多或少描述了其各自的颜色，但是这些名字基本上为生造的，用词背后并无严谨的道理。
