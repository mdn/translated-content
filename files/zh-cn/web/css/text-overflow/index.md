...
**`text-overflow`** <a href="https://developer.mozilla.org/en-US/docs/Web/CSS">CSS</a> 属性确定如何向用户发出未显示的溢出内容信号。它可以被剪切，显示一个省略号（'...'）或显示一个自定义字符串。
...
- 函数 `fade()`： 传入长度 {{cssxref("&lt;length&gt;")}} 或百分比 {{cssxref("&lt;percentage&gt;")}} 来控制淡出距离
- 一个字符串 `<string>`.
...
- `fade( <length> | <percentage> )` {{experimental_inline}}
  - : 这个函数将会截断行内溢出文本并在完全透明的行边缘添加一个淡出特效。
    ...
    参数决定淡出特效的距离。百分比 {{cssxref("&lt;percentage&gt;")}} 以行宽而定。小于 `0` 的值视为0。大于行宽的值视为行宽。
