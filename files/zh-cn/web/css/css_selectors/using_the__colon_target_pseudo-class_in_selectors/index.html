...
<p>为了辅助标识那些指向文档特定部分链接的目标，<a href="http://www.w3.org/TR/css3-selectors/#target-pseudo">CSS3 选择器</a> 引入了<code> {{ Cssxref(":target") }} </code><a href="/en/CSS/Pseudo-classes">伪类</a>. Netscape 7.1 已经在 Netscape 系列中加入了这个伪类的支持，这一新的举措让页面作者能够辅助用户在较大的页面中定位。 </p>

<h3 id="Picking_a_Target">选择一个目标</h3>

<p><code>{{ Cssxref(":target") }} 伪类用来指定那些包含片段标识符的 URI 的目标元素样式。</code> 例如，<code>http://developer.mozilla.org/en/docs/Using_the_:target_selector#Example</code> 这个 URI 包含了 <code>#Example 片段标识符。</code> 在 HTML 中，标识符是元素的 id 或者 name 属性，。由于这两者位于相同的命名空间，因此，这个示例 URI 指向的是文档顶层的 "Example" 。</p>
...
<h3 id="Targeting_the_Root_Element">定位所有元素</h3>
...
<p>在以下示例中，5 个链接指向了同一文档中的元素。例如，选择 "First" 链接会导致<code> &lt;h1 id="one"&gt; </code>成为目标元素。注意，由于目标元素有可能会被放置到浏览器窗口的顶层，因此文档可能会跳到新的滚动位置。</p>
