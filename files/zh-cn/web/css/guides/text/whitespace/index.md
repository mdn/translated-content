...
请注意，此列表不包含不可分割空格（U+00A0，HTML 中的 `&nbsp;`）。因此这些字符不会触发任何[压缩](#压缩与转换)，这也是它们常被用于在 HTML 中创建更长空格的原因。
...
- `preserve` 和 `break-spaces`：跳过整个算法，不发生任何空白字符压缩或转换。
- `preserve-breaks`：跳过第二步和第三步，保留换行符。
- `preserve-spaces`：跳过整个算法，算法变为仅将制表符和换行符转换为空格。

简而言之，不同类型的空白字符将按以下方式进行压缩和转换：
...
1. 首先，空白字符会像上一节所示那样被[压缩](#压缩与转换)，将以下内容：
...
- `preserve` 和 `break-spaces`：跳过整个算法（除了第三步），不发生任何空白字符压缩或转换。
...
并非所有 DOM API 都会保留空白字符；某些 API 设计上处理的是*渲染后的文本*。例如，{{domxref("HTMLElement.innerText")}} 返回的文本完全与渲染效果一致，所有空白字符均被压缩并修剪。{{domxref("Selection.toString()")}} 返回的是粘贴时的文本形式，通常意味着空白字符会被压缩。然而在 Firefox 中（如上文[压缩与转换](#压缩和转换)章节所述，该浏览器会压缩汉字间的空白字符），压缩后的空白字符在 `toString()` 返回的字符串和粘贴文本中均得以保留。
