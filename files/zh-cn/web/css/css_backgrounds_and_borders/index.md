...
你可以添加不同类型的边框样式，包括由任何类型图像（从光栅图像到 CSS 渐变）组成的边框。边框可以是方形或圆形，每个角都可以设置不同的半径。

无论元素是否具有可见边框，都可以对其进行圆角处理。

box-shadow 包括内阴影和外阴影、单个或多个阴影，以及实心或允许渐变为透明的阴影。外部 box-shadow 会投射出一种阴影，就好像元素的 border-box 是不透明的。内部 box-shadow 会投射出一种阴影，就好像填充边缘之外的所有内容都是不透明的。阴影可以是实心的，也可以包含一个扩散距离，阴影颜色过渡到透明。
