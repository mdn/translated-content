...
> **备注：** oklab() 函数**不**像其他某些颜色函数一样支持以逗号分隔其值，且可选的阿尔法值在指定时需置于正斜杠（`/`）之后。
...
- 函数记号：`oklab(L a b [/ A])`
  ...
  - `A`（阿尔法）为介于 `0` 和 `1` 之间的 {{CSSXref("&lt;number&gt;")}} 或 {{CSSXref("&lt;percentage&gt;")}}，此处数值 `1` 对应 `100%`（完全不透明）。`A` 表示颜色的透明度（即阿尔法通道）。
...
下列示例展示了改变 `oklab()` 函数的 `A`（阿尔法）值的效果。为演示不透明度的效果， `red` 和 `red-alpha` 元素与 `#background-div` 元素重叠。给予 `red-alpha` 元素 `0.4` 的不透明度使其显得比 `red` 元素更透明。
