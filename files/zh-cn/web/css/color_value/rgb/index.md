...
- `R`、`G`、`B`
  - : 每个都是个 `0` 到 `255` 之间的数字 {{CSSXref("&lt;number&gt;")}}，`0%` 到 `100%` 之间的 {{CSSXref("&lt;percentage&gt;")}}，或者关键词 `none`，均分别代表红色、绿色和蓝色的通道。
- `A` {{optional_inline}}
  - : {{CSSXref("&lt;alpha-value&gt;")}} 或者关键词 `none`，数字 `1` 对应 `100%`（完全不透明）。
