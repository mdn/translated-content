...
<p>全局的 CSS 文件会在全局范围内设置网站的外观，而直接在各个元素上定义的内联样式可能会覆盖您的全局 CSS 文件。内联样式和 !important 都被认为是非常不好的做法，但是有时你可以在 CSS 文件里用 !important 去覆盖内联样式。</p>
...
<p>许多 JavaScript 框架和库都添加了内联样式。有时候可以用<code>!important</code>与优先级高的选择器一起使用，以重写覆盖这些内联样式。</p>
...
<p>A) 很简单，只需再添加一条 带 <code>!important</code> 的 CSS 规则，再给这个给选择器更高的优先级（添加一个标签，ID 或类）；或是添加一样选择器，把它的位置放在原有声明的后面（总之，最后定义一条规则比胜）。</p>
...
<p>将 id 作为属性选择器的一部分而不是 id 选择器，将使其具有与类相同的特异性。上面的两个选择器现在具有相同的权重。在优先级相同情况下，后面定义的 CSS 样式会被应用。</p>
...
<h3 id="The_where_exception"><code>:where()</code> 例外规则 {{Experimental_Inline}}</h3>
...
<p>优先级是基于选择器的形式进行计算的。在下面的例子中，尽管选择器*[id="foo"] 选择了一个 ID，但是它还是作为一个<strong>属性选择器</strong>来计算自身的优先级。</p>
...
<h3 id="无视DOM树中的距离">无视 DOM 树中的距离</h3>
...
<p>{{EmbedLiveSample("无视 DOM 树中的距离")}}</p>
