...
<p><code><strong>text-shadow</strong></code>为文字添加阴影。可以为文字与  {{ cssxref("text-decoration","text-decorations") }}  添加多个阴影，阴影值之间用逗号隔开。每个阴影值由元素在 X 和 Y 方向的偏移量、模糊半径和颜色值组成。</p>
...
<p>每个阴影都有两到三个<code>&lt;length&gt;</code>参数，以及一个与阴影颜色相关的<code>&lt;color&gt;</code>参数。前两个<code>&lt;length&gt;</code>参数，是以“文字中心”为原点的坐标轴，分别为 x 轴 <code>&lt;offset-x&gt;</code> 和 y 轴 <code>&lt;offset-y&gt;</code> 的值；如果有第三个<code>&lt;length&gt;</code>参数，则第三个数值为形成阴影效果的半径的数值 <code>&lt;blur-radius&gt;</code> 。</p>
...
<dl>
 <dt>{{cssxref("&lt;color&gt;")}}</dt>
 <dd>可选。可以在偏移量之前或之后指定。如果没有指定颜色，则使用 UA（用户代理）自行选择的颜色。
  <div class="note">
   <p><strong>备注：</strong>如果想确保跨浏览器的一致性，请明确地指定一种颜色</p>
  </div>
 </dd>
 <dt><code>&lt;offset-x&gt; &lt;offset-y&gt;</code></dt>
 <dd>必选。这些长度值指定阴影相对文字的偏移量。<code>&lt;offset-x&gt;</code> 指定水平偏移量，若是负值则阴影位于文字左边。 <code>&lt;offset-y&gt;</code> 指定垂直偏移量，若是负值则阴影位于文字上面。如果两者均为 0，则阴影位于文字正后方（如果设置了<code>&lt;blur-radius&gt; </code>则会产生模糊效果)。<br>
 可用单位请查看 {{ cssxref("&lt;length&gt;") }}。</dd>
 <dt><code>&lt;blur-radius&gt;</code></dt>
 <dd>可选。这是 {{ cssxref("&lt;length&gt;") }} 值。如果没有指定，则默认为 0。值越大，模糊半径越大，阴影也就越大越淡（wider and lighter）。</dd>
</dl>
...
<p>{{EmbedLiveSample('简单的阴影', '689px', '90px')}}</p>
...
<p>{{EmbedLiveSample('多重阴影', '689px', '180px')}}</p>
