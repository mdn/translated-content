...
<p>通过 {{cssxref("border-image-source")}} 引用边框图片后，<strong><code>border-image-slice</code></strong>属性会将图片分割为 9 个区域：四个角，四个边（edges）以及中心区域。四条切片线，从它们各自的侧面设置给定距离，控制区域的大小。</p>

<p><img alt="The nine regions defined by the border-image or border-image-slice properties" src="border-image-slice.png"></p>
...
<ul>
 ...
 <li>区域 5-8 边区域（edge region）。在最终的边框图像中重复，缩放或修改它们以匹配元素的尺寸。（These are <a href="/zh-CN/docs/Web/CSS/border-image-repeat">repeated, scaled, or otherwise modified</a> in the final border image to match the dimensions of the element.）</li>
