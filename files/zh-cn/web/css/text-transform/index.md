...
- `math-auto`
  - : 用于在合适的时候自动使用数学斜体渲染文本。仅当在只有一个字符的文本节点上使用的时候，它将把拉丁字母、希腊字母和其他一些数学相关的符号转换到 [数学斜体符号](https://www.unicode.org/charts/PDF/U1D400.pdf)。举例来说，"x" 将会转换为 "𝑥"(U+1D465)，但 "exp" 将会保持是 "exp"。这个特性主要用来指定使用 MathML 语言的 {{MathMLElement("mi")}} 元素的行为。通常你应当使用 MathML 标记，它会自动应用正确的样式。
