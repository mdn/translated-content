...
<p><strong>CSS transitions </strong>提供了一种在更改 CSS 属性时控制动画速度的方法。其可以让属性变化成为一个持续一段时间的过程，而不是立即生效的。比如，将一个元素的颜色从白色改为黑色，通常这个改变是立即生效的，使用 CSS transitions 后该元素的颜色将逐渐从白色变为黑色，按照一定的曲线速率变化。这个过程可以自定义。</p>
...
<div class="note"><strong>Note:</strong> transition 可以不用厂商前缀，不过鉴于标准刚刚稳定，对于基于 Webkit 的浏览器仍然需要厂商前缀。如果想兼容旧版本的浏览器那么也需要厂商前缀（例如 Firefox 15 及之前版本，Opera 12 及之前版本)。详情见本页底部的兼容性表格。</div>

<h2 id="哪些_CSS_属性可以动画">哪些 CSS 属性可以动画？</h2>

<p>网页制作者可以定义哪个属性以哪种方式动画，这样能制作出复杂的过渡。因为有些属性动画无意义，所以 <a href="/zh-CN/docs/CSS/CSS_animated_properties">可动画属性列表</a> 是一个有限集合。</p>

<div class="note">Note: 可动画的属性集将有变动，开发者应关注。</div>

<p>auto 值常常较复杂，规范指出不要在它上动画。一些用户代理，比如基于 Gecko 的，遵循这点；一些，比如基于 WebKit 的，没有这么严格限制。在 <code>auto</code> 上动画结果可能不可预期，这取决于浏览器及其版本，应当避免使用。</p>

<p>同时应当留意这种情形，在插入元素（如 <code>.appendChild()</code>）或改变属性 <code>display: none</code> 后立即使用过渡，元素将视为没有开始状态，始终处于结束状态。简单的解决办法，改变属性前用 <code>window.setTimeout()</code> 延迟几毫秒。</p>
...
<p>CSS 过渡由简写属性 {{cssxref("transition")}} 定义是最好的方式，可以避免属性值列表长度不一，节省调试时间。</p>

<p>也可以用下面子属性来控制过渡的各部分：</p>

<p><strong>(注意下面示例中过渡无限循环是为了说明例子，过渡只是从开始到结束变化，如果需要循环，查看 <a href="/en-US/docs/CSS/animation"><code>animation</code></a> 。)</strong></p>
...
<dl>
 ...
 <dd>指定一个函数，定义属性值怎么变化。缓动函数 <em>Timing functions</em> 定义属性如何计算。多数 <a href="/en-US/docs/CSS/timing-function">timing functions</a> 由四点定义一个 bezier 曲线。也可以从 <a href="http://easings.net/">Easing Functions Cheat Sheet</a> 选择缓动效果。</dd>
...
<p>演示： <a href="http://jsfiddle.net/RwtHn/5/">http://jsfiddle.net/RwtHn/5/</a></p>
...
<ul>
 <li><a href="http://techstream.org/Web-Design/Dock-Menu-with-CSS3">CSS3 Dock Menu</a> using CSS transitions</li>
 <li>The {{ domxref("TransitionEvent") }} interface and the <a href="/en-US/docs/Mozilla_event_reference/transitionend"><code>transitionend</code></a> event.</li>
</ul>
