...
<p><strong><code>:nth-last-child()</code></strong> 这个<a href="/zh-CN/docs/Web/CSS">CSS</a> <a href="/zh-CN/docs/Web/CSS/Pseudo-classes">伪类</a> 从兄弟节点中从后往前匹配处于某些位置的元素</p>
...
<dl>
 <dt><code>odd</code></dt>
 <dd>代表一些元素，它们在所在的兄弟节点中，从后往前计算的数字位置是奇数，比如: 1, 3, 5 等。</dd>
 <dt><code>even</code></dt>
 <dd>代表一些元素，它们在所在的兄弟节点中，从后往前计算的数字位置是偶数，比如: 2, 4, 6 等。</dd>
</dl>
...
<dl>
 ...
 <dd>代表一些元素，它们在所在兄弟节点中的数字位置满足模式 <code>An+B</code>, <code>n</code>是 0 或者任意的正整数。从结尾开始计算的第一个元素的索引值是<code>1</code>. <code>A</code> 和 <code>B</code> 必须都是 {{cssxref("&lt;integer&gt;")}}.</dd>
</dl>
...
<dl>
 ...
 <dd>表示一组兄弟节点中的每个<code>&lt;p&gt;</code>元素。这与一个简单的<code>p</code>选择器相同。(由于<code>n</code>从 0 开始，而最后一个元素从 1 开始，<code>n</code>和<code>n+1</code>都会选择相同的元素。)</dd>
