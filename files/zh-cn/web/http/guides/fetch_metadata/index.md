...
Fetch 元数据可以让服务器了解以下信息（除此之外还有更多）：
...
>
> {{glossary("origin", "来源")}}的约束比{{glossary("site", "站点")}}更加严格。尤其是，一个站点包含域名下的全部子域名，而一个来源则不包含：因此 `https://example.org` 和 `https://login.example.org` 属于同一站点，但属于不同来源。
>
