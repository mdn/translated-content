...
HTTP 重定向是创建重定向的最佳方式，但是有时候你并不能控制服务器。针对这些特定的应用情景，可以尝试在页面的 {{HTMLElement("head")}} 中添加一个 {{HTMLElement("meta")}} 元素，并将其 [`http-equiv`](/zh-CN/docs/Web/HTML/Reference/Elements/meta#http-equiv) 属性的值设置为 `refresh`。当显示页面的时候，浏览器会检测该元素，然后跳转到指定的页面。
...
[`content`](/zh-CN/docs/Web/HTML/Reference/Global_attributes#content) 属性的值开头是一个数字，指示浏览器在等待该数字表示的秒数之后再进行跳转。建议始终将其设置为 `0` 来获取更好的无障碍体验。
