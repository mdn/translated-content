...
> **备注：** 注意：HTTP 内容协商的一些不好的地方在这篇文章中有介绍[a wiki page from WHATWG](https://wiki.whatwg.org/wiki/Why_not_conneg)，HTML5 提供其他的选择来进行内容协商，如[`<source>` element](/zh-CN/docs/Web/HTML/Element/source)。
...
- 客户端设置特定的 [HTTP 首部](/zh-CN/docs/Web/HTTP/Headers) （又称为服务端驱动型内容协商机制或者主动协商机制）；这是进行内容协商的标准方式；
- 服务器返回 {{HTTPStatus("300")}} (Multiple Choices) 或者 {{HTTPStatus("406")}} (Not Acceptable) [HTTP 状态码](/zh-CN/docs/Web/HTTP/Status) （又称为代理驱动型协商机制或者响应式协商机制）；这种方式一般用作备选方案。
