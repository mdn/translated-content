...
要解决此问题，请确保在发出涉及 CORS 的请求时使用 HTTPS URL，例如 {{domxref("XMLHttpRequest")}}、[Fetch](/zh-CN/docs/Web/API/Fetch_API) API、Web Fonts（`@font-face`）、[WebGL 纹理](/zh-CN/docs/Web/API/WebGL_API/Tutorial/Using_textures_in_WebGL)以及 XSL 样式表。

### 加载本地文件

来自相同的目录或者子目录的本地文件在历史上被视为[同源](/zh-CN/docs/Web/Security/Same-origin_policy)的。这意味着在测试期间可以从本地目录护着子目录加载文件以及它的所有子资源，并不会触发 CORS 错误。

不幸地是，这有安全性地隐患，正如此公告所述：[CVE-2019-11730](https://www.mozilla.org/zh-CN/security/advisories/mfsa2019-21/#CVE-2019-11730)。很多浏览器，包括 Firfox 和 Chrome，现在将所有本地文件视为*不透明*来源（默认）。因此，加载包含本地资源的本地文件现在会导致 CORS错误。
