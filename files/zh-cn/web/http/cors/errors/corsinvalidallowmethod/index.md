...
服务器发送 `Access-Control-Allow-Methods` 头信息，让客户端知道它对 CORS 请求支持哪些 [HTTP 请求方法](/zh-CN/docs/Glossary/HTTP)。`Access-Control-Allow-Methods` 的值是以逗号分隔的 HTTP 方法，例如 {{HTTPMethod("GET")}}、{{HTTPMethod("POST")}} 或 {{HTTPMethod("HEAD")}}。如果客户端[用户代理](/zh-CN/docs/Glossary/User_agent)无法识别返回的值，则会发生此错误。

这很有可能只能在服务端解决，方法是修改服务器的配置以不再发送带有 `Access-Control-Allow-Methods` 头的无效或未知方法名称。检查你的用户代理或者你使用的浏览器中的 HTTP 库是最新的。
