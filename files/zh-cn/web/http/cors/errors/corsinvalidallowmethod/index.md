...
服务器发送 `Access-Control-Allow-Methods` 头信息，让客户端知道它对 CORS 请求支持哪些 [HTTP 请求方法](/zh-CN/docs/Glossary/HTTP)。`Access-Control-Allow-Methods` 的值是以逗号分隔的 HTTP 方法，例如 {{HTTPMethod("GET")}}、{{HTTPMethod("POST")}} 或 {{HTTPMethod("HEAD")}}。如果客户端{{Glossary("user agent","用户代理")}}无法识别返回的值，则会发生此错误。
