...
例如，**`CONNECT`** 可以用来访问采用了 {{Glossary("SSL")}}（{{Glossary("HTTPS")}}）协议的站点。客户端要求 HTTP {{Glossary("Proxy
server","代理服务器")}} 将 [TCP](/zh-CN/docs/Glossary/TCP) 连接作为通往目的主机的隧道。之后该服务器会代替客户端与目的主机建立连接。连接建立好之后，{{Glossary("Proxy server","代理服务器")}}会面向客户端发送或接收 TCP 消息流。
