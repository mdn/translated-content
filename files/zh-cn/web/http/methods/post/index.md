...
- `application/x-www-form-urlencoded`：数据被编码成以 `'&'` 分隔的键——值对，同时以 `'='` 分隔键和值。键和值中非字母或数字的字符会被 [URL 编码](https://zh.wikipedia.org/wiki/百分号编码)，这也是这种类型不支持二进制数据的原因（应使用 `multipart/form-data` 代替）。
