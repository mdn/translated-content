...
<p>在 HTTP 协议中，请求方法 <strong>PATCH </strong> 用于对资源进行部分修改。</p>

<p>在 HTTP 协议中， {{HTTPMethod("PUT")}} 方法已经被用来表示对资源进行整体覆盖， 而 {{HTTPMethod("POST")}} 方法则没有对标准的补丁格式的提供支持。不同于  <code>PUT </code>方法，而与 <code>POST </code>方法类似，<code>PATCH</code>  方法是非幂等的，这就意味着连续多个的相同请求会产生不同的效果。</p>
