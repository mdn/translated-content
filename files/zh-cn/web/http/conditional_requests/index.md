...
- 对于安全（{{glossary("safe")}}）方法来说，例如 {{HTTPMethod("GET")}}，通常用来获取文件，条件请求可以被用来限定仅在满足条件的情况下返回文件。这样可以节省带宽。
- 对于非安全（{{glossary("safe", "unsafe")}}）方法来说，例如 {{HTTPMethod("PUT")}} 方法，通常用来上传文件，条件请求可以被用来限定仅在满足文件的初始版本与服务器上的版本相同的条件下才会将其上传。
