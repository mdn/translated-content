...
<p>浏览器会查找关系类型 (rel) 为 next 或 prefetch 的 HTML{{ HTMLElement("link") }}  或 <a href="/en-US/docs/Web/HTTP/Headers">HTTP <code>Link:</code> header</a>。下面是一个使用 link 标签的例子：</p>
...
<p><code>Link:</code> header 的格式在 <a href="http://tools.ietf.org/html/rfc5988">RFC 5988</a> section 5 中有所描述。</p>
...
<p>是的，您可以设置一个隐藏的首选项来禁用链接预取。 将此行添加到位于配置文件目录中的 prefs.js 文件中（或通过<a href="/about:config">about:config</a>进行适当的更改）：</p>
...
<p>除了上面已经提到的引用和 URL 跟随含义外，预取通常会导致访问预取站点的 cookie。（例如，如果您使用 google amazon，则 google 结果页面将预取 www.amazon.com，从而导致 amazon cookie 来回发送。您可以在 Firefox 中阻止第三方 cookie，请参参阅 <a href="http://support.mozilla.com/en-US/kb/Disabling%20third%20party%20cookies">Disabling third party cookies</a>。）</p>
