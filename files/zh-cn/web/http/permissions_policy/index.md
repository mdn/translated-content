...
权限策略与{{Glossary("CSP","内容安全策略")}}相似，但是它控制的是特征而非安全行为。
...
网络提供的功能和 API，如果被滥用，可能会有隐私或安全风险。在这种情况下，你可能希望严格限制功能在网站上的使用方式。在每一种情况下，都应该有一种直观的或非破坏性的方法，让网络开发者检测和处理禁用某项功能的情况。
...
对于一个 `<iframe>` 来说，其允许的原也必须在父页面的允许列表中。由于这种[继承行为](#嵌入式内容的继承策略)，最好在 HTTP 头中指定最广泛的可接受的功能支持，然后在每个 `<iframe>` 中指定你需要的支持子集。
