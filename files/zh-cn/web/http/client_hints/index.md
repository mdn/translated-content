...
**客户端提示**是一组 [HTTP 请求标头](/zh-CN/docs/Web/HTTP/Headers)字段，服务器可以主动地用途来获取关于设备、网络、用户以及用户代理指定的首选项的信息。然后，服务器可以根据客户端选择提供的信息来确定发送哪些资源。
...
你可能更喜欢省略特定的 {{HTTPHeader("Vary")}} 或者对值变化很大的客户端提示标头使用其它的策略，因为这有效地使资源不可缓存。（为每个唯一值创建新的缓存条目。）这尤其适用于网络客户端提示，像 {{HTTPHeader("Downlink")}} 和 {{HTTPHeader("RTT")}}。更多信息请参见 [HTTP 缓存 > Vary 响应](/zh-CN/docs/Web/HTTP/Caching#vary_响应).
