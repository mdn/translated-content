...
具体来说，如果定义的策略不允许使用此功能，则由 {{domxref('Navigator.bluetooth')}} 返回的 {{domxref('Bluetooth')}} 对象的方法将会阻止访问：
...
SecureCorp Inc. 希望在所有浏览上下文中禁用 Web Bluetooth API，除了其自身的来源和 `https://example.com` 的来源。
它可以通过发送以下 HTTP 响应头来定义权限策略：
...
FastCorp Inc. 希望为所有跨来源的子框架禁用 `bluetooth`，除了特定的 `<iframe>`。
它可以通过发送以下 HTTP 响应头来定义权限策略：
...
`<iframe>` 属性可以有选择地在某些框架中启用功能，而在其他框架中禁用这些功能，即使这些框架包含来自同一来源的文档。
