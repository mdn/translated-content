...
**`Sec-CH-UA`** 头在以逗号分隔的列表中提供与浏览器关联的每个品牌的品牌和重要版本。品牌是用户代理的商业名称，例如：Chromium、Opera、Google Chrome、Microsoft Edge、Firefox 和 Safari。
...
`Sec-CH-UA` 是一个[低熵提示](/zh-CN/docs/Web/HTTP/Client_hints#low_entropy_hints)。除非被用户代理策略明确阻止，否则它将在所有请求中发送（无需服务器通过发送{{HTTPHeader("Accept-CH")}}来选择加入）。
