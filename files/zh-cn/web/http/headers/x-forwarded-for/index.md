...
当在客户端和服务器之间没有受信任的反向代理服务器时（比如一个负载均衡服务器），`X-Forwarded-For` 首部是不可信的。如果客户端和所有的代理服务器都没有恶意且行为良好，那么此首部中的 IP 地址列表具有[指令](#指令)部分所描述的意义。但是如果客户端或任意代理服务器存有恶意或配置错误，那么此首部的任意部分（或整体）可能是仿冒的（并且有可能不是一个列表或者甚至不包含 IP 地址）。
...
对 `X-Forwarded-For` 首部的不当解析可能导致仿冒的值被用于安全相关的目地，造成本文之前提及过的负面后果。
