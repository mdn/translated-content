...
- `username`
  - : 带引号的字符串，其中包含指定 `realm` 的用户名，可以是纯文本，也可以是十六进制表示的哈希编码。如果名称包含字段中不允许的字符，则可以使用 `username*`（而不是“as well”）。
- `username*`
...
> **警告：** {{Glossary("Base64")}}-编码很容易被反转，以得到原始的名称和密码，所以 Basic 身份验证是完全不安全的。当时用身份验证时，{{Glossary("HTTPS")}} 总是被推荐，但当使用 `Basic` 身份验证时，更是如此。
