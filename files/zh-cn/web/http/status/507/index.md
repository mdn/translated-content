...
此状态码最初用于 Web 分布式创作与版本控制（{{Glossary("WebDAV")}}）的上下文中，但后来被推广到其他用例，用于描述服务器资源耗尽的情况。此错误的常见原因可能包括服务器目录可用空间不足、操作所需的可用内存（RAM）不足，或达到内部限制（例如应用程序特定的内存限制等）。导致此错误的请求不一定需要包含内容，因为该请求可能是在成功时会在服务器上创建资源的请求。

此问题被认为是暂时的，与 {{HTTPStatus("413", "413 Content Too Large")}} 不同，后者表示无论服务器资源限制如何，客户端请求都过大，服务器无法处理。
