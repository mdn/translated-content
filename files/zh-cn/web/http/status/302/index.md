...
HTTP **`302 Found`** 重定向状态码表明请求的资源被暂时的移动到了由该 HTTP 响应的响应头 {{HTTPHeader("Location")}} 指定的 URL 上。浏览器会重定向到这个 URL，但是搜索引擎不会对该资源的链接进行更新 (用 SEO 的行话来说，意思是“链接汁”（link juice）不会被传递到新的 URL)。
