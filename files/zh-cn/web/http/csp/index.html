...
<p>内容安全策略   ({{Glossary("CSP")}}) 是一个额外的安全层，用于检测并削弱某些特定类型的攻击，包括跨站脚本 ({{Glossary("XSS")}}) 和数据注入攻击等。无论是数据盗取、网站内容污染还是散发恶意软件，这些攻击都是主要的手段。</p>

<p>CSP 被设计成完全向后兼容（除 CSP2 在向后兼容有明确提及的不一致;  更多细节查看<a href="https://www.w3.org/TR/CSP2">这里</a> 章节 1.1）。不支持 CSP 的浏览器也能与实现了 CSP 的服务器正常合作，反之亦然：不支持 CSP 的浏览器只会忽略它，如常运行，默认为网页内容使用标准的同源策略。如果网站不提供 CSP 头部，浏览器也使用标准的<a href="https://developer.mozilla.org/en-US/docs/Web/Security/Same-origin_policy">同源策略</a>。</p>

<p>为使 CSP 可用，你需要配置你的网络服务器返回  {{HTTPHeader("Content-Security-Policy")}}  HTTP 头部 ( 有时你会看到一些关于<code>X-Content-Security-Policy</code>头部的提法，那是旧版本，你无须再如此指定它)。</p>
...
<p>除限制可以加载内容的域，服务器还可指明哪种协议允许使用；比如 (从理想化的安全角度来说)，服务器可指定所有内容必须通过 HTTPS 加载。一个完整的数据安全传输策略不仅强制使用 HTTPS 进行数据传输，也为所有的 cookie 标记安全标识 <a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Cookies">cookies with the secure flag</a>，并且提供自动的重定向使得 HTTP 页面导向 HTTPS 版本。网站也可以使用  {{HTTPHeader("Strict-Transport-Security")}}  HTTP 头部确保连接它的浏览器只使用加密通道<strong>。</strong></p>

<h2 id="使用_CSP">使用 CSP</h2>

<p>配置内容安全策略涉及到添加 {{HTTPHeader("Content-Security-Policy")}}  HTTP 头部到一个页面，并配置相应的值，以控制用户代理（浏览器等）可以为该页面获取哪些资源。比如一个可以上传文件和显示图片页面，应该允许图片来自任何地方，但限制表单的 action 属性只可以赋值为指定的端点。一个经过恰当设计的内容安全策略应该可以有效的保护页面免受跨站脚本攻击。本文阐述如何恰当的构造这样的头部，并提供了一些例子。</p>

<h3 id="制定策略">制定策略</h3>

<p>你可以使用  {{HTTPHeader("Content-Security-Policy")}} HTTP 头部 来指定你的策略，像这样：</p>

<pre>Content-Security-Policy: <em>policy</em></pre>

<p>policy 参数是一个包含了各种描述你的 CSP 策略指令的字符串。</p>

<h3 id="描述策略">描述策略</h3>

<p>一个策略由一系列策略指令所组成，每个策略指令都描述了一个针对某个特定类型资源以及生效范围的策略。你的策略应当包含一个{{CSP("default-src")}}策略指令，在其他资源类型没有符合自己的策略时应用该策略 (有关完整列表查看{{CSP("default-src")}} )。一个策略可以包含 {{CSP("default-src")}}  或者 {{CSP("script-src")}} 指令来防止内联脚本运行，并杜绝<code>eval()</code>的使用。 一个策略也可包含一个 {{CSP("default-src")}} 或  {{CSP("style-src")}} 指令去限制来自一个 {{HTMLElement("style")}} 元素或者 style 属性的內联样式。</p>
...
<p> 一个在线邮箱的管理者想要允许在邮件里包含 HTML，同样图片允许从任何地方加载，但不允许 JavaScript 或者其他潜在的危险内容 (从任意位置加载)。</p>

<pre>Content-Security-Policy: default-src 'self' *.mailsite.com; img-src *</pre>

<p> 注意这个示例并未指定{{CSP("script-src")}}。在此 CSP 示例中，站点通过 {{CSP("default-src")}} 指令的对其进行配置，这也同样意味着脚本文件仅允许从原始服务器获取。</p>

<h2 id="对策略进行测试">对策略进行测试</h2>

<p>为降低部署成本，CSP 可以部署为<em>报告 (report-only)</em>模式。在此模式下，CSP 策略不是强制性的，但是任何违规行为将会报告给一个指定的 URI 地址。此外，一个报告模式的头部可以用来测试一个修订后的未来将应用的策略而不用实际部署它。</p>
...
<p>支持 CSP 的浏览器将始终对于每个企图违反你所建立的策略都发送违规报告，如果策略里包含一个有效的{{CSP("report-uri")}} 指令。</p>

<h2 id="启用违例报告">启用违例报告</h2>

<p>默认情况下，违规报告并不会发送。为启用发送违规报告，你需要指定 {{CSP("report-uri")}} 策略指令，并提供至少一个 URI 地址去递交报告：</p>
...
<dl>
 ...
 <dd>被 CSP 阻止的资源 URI。如果被阻止的 URI 来自不同的源而非文档 URI，那么被阻止的资源 URI 会被删减，仅保留协议，主机和端口号。</dd>
...
<p>我们假设页面位于 <code>http://example.com/signup.html</code>。它使用如下策略，该策略禁止任何资源的加载，除了来自<code>cdn.example.com 的样式表。</code></p>

<pre>Content-Security-Policy: default-src 'none'; style-src cdn.example.com; report-uri /_/csp-reports</pre>

<p> <code>signup.html</code> 的 HTML 像这样：</p>
...
<p>你能看出其中错误吗？样式表仅允许加载自<code>cdn.example.com，</code>然而该页面企图从自己的源 (<code>http://example.com</code>) 加载。当该文档被访问时，一个兼容 CSP 的浏览器将以 POST 请求的形式发送违规报告到 <code>http://example.com/_/csp-reports</code>，内容如下：</p>
...
<p>如你所见，该报告在 <code>blocked-uri 字段</code> 中包含了违规资源的完整路径，但情况并非总是如此。比如，当 signup.html 试图从 <a href="http://anothercdn.example.com/stylesheet.css"><code>http://anothercdn.example.com/stylesheet.css</code></a> 加载 CSS 时，浏览器将不会包含完整路径，而只会保留源路径 (<code>http://anothercdn.example.com</code>)。CSP 技术规范小组对此古怪行为给出了<a href="https://www.w3.org/TR/CSP/#violation-reports">解释</a>。大体上说，这样是为了防止泄露跨域资源的敏感信息。</p>
