...
<p>在超文本标记语言中，在两种文档之间可以指定{{HTMLElement("a")}}, {{HTMLElement("area")}}或{{HTMLElement("link")}} 元素使一个页面链接到另一个页面。</p>

<table class="standard-table">
 <caption>下面是链接类型及其在 HTML 中的意义</caption>
 <thead>
  <tr>
 ...
 <tbody>
  <tr>
   ...
   <td>
    ...
    <p>（译注：link 的 alternate 定义了一个可替换的样式表，它和第一个 link 元素同时使用，第一个定义了首选样式，而 alternate 则让用户可选择替换的样式。但这个替换操作需要 浏览器支持，但很多浏览器比如 IE 都是不支持的。</p>

    <p>所以使用到替换样式的网页，一般都用一些样式表切换的 JS，让用户可以自由切换界面样式。这个应该大家都见过，一些网站会给网页定义多种配色。 稍高阶的一些，还可以利用 JS 弄成随时间变化样式的，比如白天的时候显示成明色，晚上的时候显示成暗色。）</p>
   </td>
  ...
  <tr>
   ...
   <td>定义一个超链接到一个描述作者信息的页面或者提供一个方法联系作者。<br>
    <br>
    <strong>Note:</strong> 这个方法可能是 mailto:邮箱链接，但不推荐这样，这样可能导致机器人发送大量的垃圾邮件到这个邮箱。这种情况下，最好带一个包含联系方法的页面.。<br>
    <br>
  ...
  <tr>
   ...
   <td>
    ...
    <p>在树型序列中，由于这些属性只是提示，并且这这些资源在进一步检查时可能是不适合的，浏览器可能选择另一个适合的。<br>
     ...
     <br>
     <code>在之前，经常可以看到 shortcut</code>,但他不是标准的，<strong>应该不再使用</strong>。</p>
   </td>
   <td>{{HTMLElement("link")}}</td>
   <td>{{HTMLElement("a")}}, {{HTMLElement("area")}}</td>
  </tr>
  <tr>
   ...
   <td><a href="https://developer.mozilla.org/en-US/docs/Web/Web_Components/HTML_Imports">HTML Imports</a></td>
   <td></td>
   <td></td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>表示链接到的文件是 <a href="https://developer.mozilla.org/en-US/docs/Web/Manifest">Web App Manifest</a>.</td>
   <td>{{HTMLElement("link")}}</td>
   <td>{{HTMLElement("a")}}, {{HTMLElement("area")}}</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>
    <p>告诉浏览器下载资源，因为在当前导航期间稍后将需要该资源。有关详细信息，请参见 <a href="https://developer.mozilla.org/en-US/docs/Web/HTML/Preloading_content">Preloading content with rel="preload"</a></p>
   </td>
   <td>{{HTMLElement("link")}}   </td>
   <td>{{HTMLElement("a")}}, {{HTMLElement("area")}},<br>
    {{HTMLElement("form")}}</td>
  ...
  <tr>
   ...
   <td>
    <p><a href="https://code.google.com/archive/p/shortlink/wikis/Specification.wiki"><code>shortlink</code> 规范</a><br>
     来自维基百科: <a href="https://en.wikipedia.org/wiki/URL_shortening">URL shortening</a><br>
  ...
  <tr>
   ...
   <td>
    <p>定义要用作样式表的外部资源。如果没有设置 <code>type</code>，在进一步检查之前，浏览器应该假定它是 <code>text/css</code> 样式表。<br>
     <br>
     如果与 <code>alternate</code> 关键字结合使用，则定义一个 <a href="/en-US/docs/Alternative_style_sheets">可选样式表</a> ;在这种情况下，{{htmlattrxref("title", "link")}} 属性必须出现，而不能是空字符串。</p>
   </td>
   <td>{{HTMLElement("link")}}</td>
   <td>{{HTMLElement("a")}}, {{HTMLElement("area")}}</td>
  </tr>
