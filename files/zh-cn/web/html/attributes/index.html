 ...
 <tbody>
  <tr>
   ...
   <td>
    ...
    <p>Note:这是遗留属性，请使用 CSS {{ Cssxref("border") }} 属性代替。</p>
   </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>From the <a href="https://w3c.github.io/html-media-capture/#the-capture-attribute">HTML Media Capture<br>
  ...
  <tr>
   ...
   <td><a href="https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input"><code>&lt;input&gt;</code></a>, <a href="https://developer.mozilla.org/en-US/docs/Web/HTML/Element/button"><code>&lt;button&gt;</code></a></td>
   <td>Indicates the action of the element, overriding the action defined in the <a href="https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form"><code>&lt;form&gt;</code></a>.</td>
  </tr>
  <tr>
   <td>headers</td>
   <td>{{ HTMLElement("td") }}, {{ HTMLElement("th") }}</td>
...
<p>大多数时候，IDL 属性会返回元素确实在用的值。例如，{{HTMLElement("input")}} 的默认 <code>type</code> 是 "text"，所以如果你设置 <code>input.type="foobar"</code>，<code>&lt;input&gt;</code> 元素仍然会是文本类型（在外观上和表现上），但 "type" 内容属性的值是 "foobar"。然而，<code>type</code> IDL 属性依旧会返回字符串 "text"。</p>

<p>IDL attributes 并不总是字符串；例如 <code>input.maxlength</code> 是一个数字（一个有符号的 long 类型）。使用 IDL 属性时，你读取或设置值的类型都是要求的类型。所以 <code>input.maxlength</code> 总会返回一个数字，而如果你要设置 <code>input.maxlength</code>，也需要使用数字。如果你传入了别的类型，则会根据标准 JavaScript 的类型转换规则被转换为数字。</p>

<p>IDL 属性可以 <a href="https://www.whatwg.org/specs/web-apps/current-work/multipage/urls.html#reflecting-content-attributes-in-idl-attributes">反映其他类型</a>，如 unsigned long, URLs, booleans 等。不幸的是，并没有清晰的规则规定一个属性的 IDL 属性与相关的内容属性如何关联。大多数时候，这会遵守 <a href="https://www.whatwg.org/specs/web-apps/current-work/multipage/urls.html#reflecting-content-attributes-in-idl-attributes">规范中列出的规则</a>，但有时并不遵守。HTML 规范尝试将这件事尽可能变得对开发者友好，但由于很多原因（大多是历史原因），一些属性表现得很奇怪（例如 <code>select.size</code>），你就需要阅读规范以理解它们实际上到底如何表现。</p>

<h2 id="布尔值属性">布尔值属性</h2>

<p>一些内容属性（例如 <code>required</code>, <code>readonly</code>, <code>disabled</code>）是<a href="https://www.w3.org/TR/html52/infrastructure.html#sec-boolean-attributes">布尔值属性</a>。如果一个布尔值属性存在，则其值是 <strong>true</strong>，如果不存在，其值是  <strong>false</strong>。</p>

<p>HTML5 定义了布尔值属性允许的取值：如果属性存在，其值必须是一个空字符串（即该属性的值未分配），或者是一个大小写无关的 ASCII 字符串，该字符串与属性名严格相同，前后都没有空格。下述例子是为一个布尔值属性取值的几个有效方式。</p>
...
<p>再明确一点，布尔值属性不能取值为 "<code>true</code>" 和 "<code>false</code>"。如果需要表示 false 值，布尔值属性需要整个忽略不写。这个限制澄清了一些常见误会：比如在元素中设置 <code>checked="false"</code> ，元素的 <code>checked</code> 属性会被解读为 <strong>true</strong>，因为这个属性出现了。</p>
