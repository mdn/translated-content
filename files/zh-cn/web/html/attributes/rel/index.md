...
关系的类型是由 `rel` 属性的值给出的，如果存在的话，它的值必须是一组无序的、唯一的、用空格隔开的关键字。与不表达语义的 `class` 名称不同，`rel` 属性必须使用对机器和人类都有语义的标记。目前关于 `rel` 属性的可能值的注册表是 [IANA 链接关系注册表](https://www.iana.org/assignments/link-relations/link-relations.xhtml)、[HTML 现行标准](https://html.spec.whatwg.org/multipage/links.html#linkTypes)和 microformats wiki 中可自由编辑的 [existing-rel-values 页面](https://microformats.org/wiki/existing-rel-values)（根据[现行标准](https://html.spec.whatwg.org/multipage/links.html#other-link-types)的建议）。如果使用一个不存在于上述三个来源之一的 `rel` 属性，一些 HTML 验证器（如 [W3C Markup Validation Service](https://validator.w3.org/)）会产生一个警告。

下表列出了一些最重要的现有关键词。在一个以空格分隔的值内的每个关键词在该值内都应该是唯一的。

| `rel` 值                                                          | 描述                                                                                                                                                                                                                                                                                  | `{{htmlelement('link')}}` | `{{htmlelement('a')}}` 和 `{{htmlelement('area')}}` | `{{htmlelement('form')}}` |
| -------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | ---------------------------------------------------- | ------------------------- |
| [`alternate`](#attr-alternate)                                       | 当前文件的替代描述。                                                                                                                                                                                                                                           | 链接                      | 链接                                                 | 不允许               |
...
| [`external`](#attr-external)                                         | 引用的文件与当前的文件不属于同一个站点。                                                                                                                                                                                                                | 不允许               | 注解                                           | 注解                |
...
| [`next`](#attr-next)                                                 | 表示当前文档是一个系列的一部分，该系列中的下一个文件是被引用的文件。                                                                                                                                                               | 链接                      | 链接                                                 | 链接                      |
...
| [`prev`](#attr-prev)                                                 | 表示当前文档是一个系列的一部分，该系列中的上一个文件是被引用的文件。                                                                                                                                                           | 链接                      | 链接                                                 | 链接                      |
...
- {{htmlattrdef("alternate")}}
  ...
  - : 表示当前文档的另一种方式。对 {{htmlelement('link')}}、{{htmlelement('a')}} 和 {{htmlelement('area')}} 有效，其含义取决于其他属性的值。
    ...
    - 与文档语言不同的 [`hreflang`](/zh-CN/docs/Web/HTML/Element/link#attr-hreflang) 属性表示一个翻译。
...
- {{htmlattrdef("author")}}
  ...
  - : 表示被引用的文档提供了关于当前文档或文章作者的进一步信息。与 {{htmlelement('link')}}、{{htmlelement('a')}} 和 {{htmlelement('area')}} 元素有关。
    ...
    > **备注：** 由于历史原因，废弃的属性值 `rev="made"` 视为 `rel="author"`。
...
- {{htmlattrdef("license")}}
  ...
  - : 在 {{HTMLElement("a")}}、{{HTMLElement("area")}}、{{HTMLElement("form")}}、 {{HTMLElement("link")}} 元素上有效，`license` 值表示该超链接指向描述许可信息的文件；当前文件的主要内容被引用文件描述的版权许可所覆盖。如果不在{ {HTMLElement("head")}} 元素内，标准并不区分适用于文档特定部分的超链接还是适用于整个文档的超链接。只有页面上的数据可以表明这一点。
    ...
    > **备注：** 虽然同义词 `copyright` 也是认可的，但它是不正确的，必须避免使用。
...
- {{htmlattrdef("nofollow")}}
  - : 与 {{htmlelement('form')}}、{{htmlelement('a')}} 和 {{htmlelement('area')}} 元素相关， `nofollow` 关键字告诉搜索引擎蜘蛛忽略链接关系。nofollow 关系可能表明当前文档的所有者并不认可被引用的文档。它经常被搜索引擎优化者包括在内，假装他们的链接养殖场不是垃圾页面。
