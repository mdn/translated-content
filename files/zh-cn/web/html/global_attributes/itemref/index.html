...
<p><strong><code>itemref</code></strong> <a href="/en-US/docs/Web/HTML/Global_attributes">全局属性</a> 不具有 <code>itemscope</code> 属性的元素的后代，才可以与具有 <code><strong>itemref</strong></code> 的元素关联。<code>itemref</code> 提供了元素 id（并不是 <code>itemid</code>）的列表，并具有文档其它地方的额外属性。</p>

<p><code>itemref</code> 属性只能在指定了 <code>itemscope</code> 的元素上指定。</p>

<p><strong>注：</strong><code>itemref</code> 属性并不是 microdata 数据模型的一部分。它仅仅是个语义结构，用于帮助作者向页面添加注解，其中被注解的数据不遵循便利的树结构。例如，它允许作者标记表格中的数据，以便每一列定义一个单独的条目，同时在表格中保留属性。</p>
