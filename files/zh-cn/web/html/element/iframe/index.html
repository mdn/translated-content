...
<p><strong>HTML 内联框架元素 (<code>&lt;iframe&gt;</code>)</strong> 表示嵌套的{{Glossary("browsing context")}}。它能够将另一个 HTML 页面嵌入到当前页面中。</p>

<div>{{EmbedInteractiveExample("pages/tabbed/iframe.html", "tabbed-standard")}}</div>

<p>每个嵌入的浏览上下文（embedded browsing context）都有自己的<a href="/zh-CN/docs/Web/API/History">会话历史记录 (session history)</a>和<a href="/zh-CN/docs/Web/API/Document">DOM 树</a>。包含嵌入内容的浏览上下文称为<em><dfn>父级</dfn>浏览上下文</em>。顶级浏览上下文（没有父级）通常是由 {{domxref("Window")}} 对象表示的浏览器窗口。</p>
...
<dl>
 ...
 <dd>设置为<code>true</code>时，可以通过调用 <code>&lt;iframe&gt;</code> 的 {{domxref("Element.requestFullscreen", "requestFullscreen()")}} 方法激活全屏模式。
 <div class="note">这是一个历史遗留属性，已经被重新定义为 <code>allow="fullscreen"</code>。</div>
 </dd>
 <dt>{{htmlattrdef("allowpaymentrequest")}}</dt>
 <dd>设置为<code>true</code>时，跨域的 <code>&lt;iframe&gt;</code> 就可以调用 <a href="https://developer.mozilla.org/en-US/docs/Web/API/Payment_Request_API">Payment Request API</a>。
 ...
 <dd>以 CSS 像素格式，或像素格式，或百分比格式指定 frame 的高度。默认值为<code>150</code>。</dd>
 ...
 <dd>用于定位嵌入的浏览上下文的名称。该名称可以用作 {{HTMLElement("a")}} 标签与 {{HTMLElement("form")}} 标签的 <code>target</code> 属性值，也可以用作 {{HTMLElement("input")}} 标签和 {{HTMLElement("button")}} 标签的 <code>formtarget</code> 属性值，还可以用作 {{domxref("Window.open()","window.open()")}} 方法的 <code>windowName</code> 参数值。</dd>
 ...
 <dd>被嵌套的页面的 URL 地址。使用 <code>about:blank</code> 值可以嵌入一个遵从<a href="/zh-CN/docs/Web/Security/Same-origin_policy">同源策略</a>的空白页。在 Firefox（version 65 及更高版本）、基于 Chromium 的浏览器、Safari/iOS 中使用代码移除 <code>iframe</code> 的 <code>src</code> 属性（例如通过 {{domxref("Element.removeAttribute()")}} ）会导致 <code>about:blank</code> 被载入 frame。</dd>
 ...
 <dd>以 CSS 像素格式，或以像素格式，或以百分比格式指定的 frame 的宽度。默认值是<code>300</code>。</dd>
</dl>

<h3 id="不赞成使用的属性">不赞成使用的属性</h3>
...
<h3 id="非标准属性_non-standard_inline">非标准属性 {{non-standard_inline}}</h3>

<dl>
 <dt><strong>{{htmlattrdef("mozbrowser")}} {{non-standard_inline}} </strong></dt>
 <dd>这个属性可以让 <code>&lt;iframe&gt;</code> 变得像顶级浏览器窗口，详情请参看 <a href="/zh-CN/docs/Web/API/Using_the_Browser_API">Browser API</a>。这个属性<strong>只能</strong>在 <a href="/zh-CN/docs/Mozilla/Add-ons/WebExtensions">WebExtensions</a> 中使用。</dd>
</dl>
...
<p>脚本访问框架内容必须遵守<a href="/zh-CN/docs/Web/Security/Same-origin_policy">同源策略</a>，并且无法访问非同源的 <code>window</code> 对象的几乎所有属性。同源策略同样适用于子窗体访问父窗体的 <code>window</code> 对象。跨域通信可以通过 <a href="/zh-CN/docs/Web/API/Window/postMessage">window.postMessage</a> 来实现。</p>
...
<h3 id="Example1">一个简单的 <code>&lt;iframe&gt;</code></h3>
...
<h3 id="Example2">在新标签页中打开 <code>&lt;iframe&gt;</code> 里面的链接</h3>
...
<pre class="brush: html line-numbers language-html"><code class="language-html">&lt;iframe title="Wikipedia page for Avocados" src="https://en.wikipedia.org/wiki/Avocado"&gt;&lt;/iframe&gt;</code></pre>
