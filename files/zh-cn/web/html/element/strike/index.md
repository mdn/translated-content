...
> **警告：** 这个元素在 HTML 4 和 XHTML 1 中废除了，并且在[现存的 HTML 标准](https://html.spec.whatwg.org/#strike)被废弃。如果语义上合适的话（也就是如果表示*删除*的内容），使用 {{HTMLElement("del")}} 来代替。在所有其它的情况中，使用 {{HTMLElement("s")}}。
