...
---
<p><strong>HTML 元素<code>&lt;tfoot&gt;</code> </strong> 定义了一组表格中各列的汇总行。</p>
...
<dl>
 ...
 <ul>
  <li>为达到与<code>left</code>, <code>center</code>, <code>right</code>或<code>justify</code>相同的效果，请使用 CSS {{cssxref("text-align")}}属性。</li>
  <li>为达到与 char 值相同的效果，在 CSS3 中可将{{htmlattrxref("char", "tfoot")}}的值用作{{cssxref("text-align")}}的属性值{{unimplemented_inline}}。</li>
 </ul>
 </div>
 </dd>
</dl>
...
<dl>
 ...
 <dd>此属性定义了列内单元格的背景色。定义此属性使用'#'作为前缀，其后是定义于<a href="https://www.w3.org/Graphics/Color/sRGB">sRGB</a>的 6 位十六进制码。
 ...
 <div class="note"><strong>Usage Note: </strong>请勿使用此属性，因为这并非标准，且只有某些特定版本的 Microsoft Internet Explorer（IE 浏览器）支持：{{HTMLElement("tfoot")}}元素应使用<a href="/en-US/docs/Web/CSS">CSS</a>设计。若想得到与<strong>bgcolor</strong>属性相似的效果，可在相关的 {{HTMLElement("td")}}或{{HTMLElement("th")}}元素中使用<a href="/en-US/docs/Web/CSS">CSS</a> {{cssxref("background-color")}}属性。</div>
 </dd>
</dl>
...
<dl>
 ...
 <div class="note"><strong>Note: </strong>请勿使用此属性，因为在最新标准中此属性被废弃（且不受支持）。想要达到与{{htmlattrxref("char", "tbtfootody")}}相同的效果，在 CSS3 中，可将{{cssxref("text-align")}}属性设置为{{htmlattrxref("char", "tfoot")}}的属性值{{unimplemented_inline}}。</div>
 </dd>
</dl>
...
<dl>
 ...
 <dd>此属性指定表头（译者注：英文原文为 table header，疑似错误）中每一行内的文本的垂直对齐方式。此属性的可选值为：
 <ul>
  <li><code>baseline</code>，会使文本尽可能接近单元格底部，但并不向底部对齐，而将它们以字符<a href="https://zh.wikipedia.org/wiki/%E5%9F%BA%E7%B7%9A">基线</a>对齐。如果所有字符的大小相同，此值与<code>bottom</code>的效果相同；</li>
  <li><code>bottom</code>，设置文本向下对齐；</li>
  <li><code>middle</code>，设置文本居中对齐；</li>
  <li>and <code>top</code>，设置文本向上对齐。</li>
 </ul>

 <div class="note"><strong>Note: </strong> 请勿使用此属性，因为在最新标准中此属性被废弃（且不受支持）：使用 CSS 属性{{cssxref("vertical-align")}}作为替代。</div>
 </dd>
</dl>
...
<ul>
 <li>其他 table 相关的 HTML 元素：{{HTMLElement("caption")}}, {{HTMLElement("col")}}, {{HTMLElement("colgroup")}}, {{HTMLElement("table")}}, {{HTMLElement("tbody")}}, {{HTMLElement("td")}}, {{HTMLElement("tbody")}}, {{HTMLElement("th")}}, {{HTMLElement("thead")}}, {{HTMLElement("tr")}};</li>
 <li>在设计<code>&lt;tfoot&gt;</code>时可能会有特殊效果的 CSS 属性和伪类：
  <ul>
