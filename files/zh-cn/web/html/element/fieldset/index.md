...
它的 {{cssxref("display")}} 值默认为 `block`，因此建立了一个[块级格式化上下文](/zh-CN/docs/Web/CSS/CSS_display/Block_formatting_context)。如果将 `<fieldset>` 的 `display` 值设置为行内级别，则会表现为 `inline-block`，否则会表现为 `block`。默认情况下 `<fieldset>` 会有 `2px` `groove` 的边界围绕着内容，还有一个默认的小的内边距，还有 {{cssxref("min-inline-size", "min-inline-size: min-content")}} 。
