...
`<textarea>` 是 [可被替换的元素](/zh-CN/docs/CSS/Replaced_element)——其有自己的固有尺寸，像。默认情况下，其 {{cssxref("display")}} 值为 `inline-block`。与其他表单元素相比，这个元素相对比较容易设置样式，使用常规的 CSS 就可以轻松设置它的盒模型、字体、颜色方案等。
