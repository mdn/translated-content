...
<p>上述例子展示了 <code>&lt;audio&gt;</code> 元素的基本用法。与 {{htmlelement("img")}} 元素类似，我们在 <code>src</code> 属性中添加了嵌入媒体的路径；也可以使用其他属性来规定一些功能，例如是否自动播放、是否循环播放、是否显示浏览器的默认音频控件等等。</p>
...
<table>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>
    <p>媒体置空。举个例子，当一个媒体已经加载（或部分加载）的情况下话调用 <code><a href="/zh-CN/docs/XPCOM_Interface_Reference/NsIDOMHTMLMediaElement">load()</a> </code>方法，这个事件就将被触发。</p>
   </td>
  </tr>
  <tr>
