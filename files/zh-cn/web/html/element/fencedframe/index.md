...
`<fencedframe>` 元素是一种内置更多原生隐私功能的 `<iframe>`。它解决 `<iframe>` 的一些缺陷，比如对第三方 cookies 的依赖以及其他隐私风险。参见[围栏框架 API](/zh-CN/docs/Web/API/Fenced_frame_API) 获取更多信息。
...
- `allow` {{experimental_inline}}
  ...
  - : 为 `<fencedframe>` 指定[权限策略](/zh-CN/docs/Web/HTTP/Permissions_Policy)，该策略根据请求的来源定义 `<fencedframe>` 可用的功能。请参见[围栏框架可用的权限策略](#围栏框架可用的权限策略)了解在围栏框架上设置的策略来控制哪些功能的详细信息。
...
围栏框架内策略可以启用的唯一特性是设计用于围栏框架内的特定功能：
...
当前这些功能在围栏框架内总是启用的。未来将能够通过 `<fencedframe>` 的 `allow` 属性来控制哪些功能被启用。以这种方式阻止隐私沙箱功能，也将阻止围栏框架的加载——即完全不存在通信渠道。
...
> [!NOTE] > {{cssxref("object-fit")}} 属性对 `<fencedframe>` 元素无效。
...
<table class="properties">
  <tbody>
    <tr>
      ...
      <td>
        <a href="/zh-CN/docs/Web/HTML/Content_categories#flow_content">流式内容</a>、
        <a href="/zh-CN/docs/Web/HTML/Content_categories#phrasing_content">短语内容</a
        >、嵌入内容、交互内容、可感知内容
      </td>
    </tr>
    <tr>
