...
---
<p><code>&lt;canvas&gt;</code>元素可被用来通过 JavaScript（<a href="/zh-CN/docs/Web/API/Canvas_API">Canvas</a> API 或 <a href="https://developer.mozilla.org/zh-CN/docs/Web/API/WebGL_API">WebGL</a> API）绘制图形及图形动画。</p>
...
<dl>
 ...
 <dd>通过设置这个属性，来控制 canvas 元素是否半透明。如果你不想 canvas 元素被设置为半透明，使用这个元素将可以优化浏览器绘图性能。</dd>
...
<p>直接在 html 标签中设置 width 和 height 属性或者使用 JavaScript 来指定画布尺寸，这将改变一个画布的水平像素和垂直像素数，就像定义了一张图片的大小一样。</p>

<p>可以使用 CSS 的 width 和 height 以在渲染期间缩放图像以适应样式大小，就像&lt;img&gt;元素一样。如果您发现&lt;canvas&gt;元素中展示的内容变形，您可以通过&lt;canvas&gt;自带的 height 和 width 属性进行相关设置，而不要使用 CSS。</p>
...
<p><code>&lt;canvas&gt;</code> 元素本身只是一个位图，不提供任何绘制对象的信息。画布内容不像 HTML 那样具有语义并能暴露给可访问性工具。以下指南可以帮助您更方便地访问它。</p>
