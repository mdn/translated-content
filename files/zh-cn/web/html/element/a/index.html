...
<p><strong>HTML <code>&lt;a&gt;</code> 元素</strong>（或称锚元素）可以通过<a href="#href">它的 <code>href</code> 属性</a>创建通向其他网页、文件、同一页面内的位置、电子邮件地址或任何其他 URL 的超链接。<code>&lt;a&gt;</code> 中的内容<strong>应该</strong>应该指明链接的意图。如果存在<a href="#href"> <code>href</code> 属性</a>，当 <code>&lt;a&gt;</code> 元素聚焦时按下回车键就会激活它。</p>
...
<table class="properties">
 <tbody>
  <tr>
   <th scope="row"><dfn><a href="/en-US/docs/HTML/Content_categories">内容分类</a></dfn></th>
...
<dl>
 ...
 <dd>包含超链接指向的 URL 或 URL 片段。URL 片段是哈希标记（#）前面的名称，哈希标记指定当前文档中的内部目标位置（HTML 元素的 ID）。URL 不限于基于 Web（HTTP）的文档，也可以使用浏览器支持的任何协议。例如，在大多数浏览器中正常工作的<a href="https://en.wikipedia.org/wiki/File_URI_scheme"><code>file:</code></a>、ftp:和 mailto：。
 <div class="note">
 <p>可以使用 <code>href="#top"</code> 或者 <code>href="#"</code> 链接返回到页面顶部。<a href="http://www.whatwg.org/specs/web-apps/current-work/multipage/history.html#scroll-to-fragid">这种行为是 HTML5 的特性</a>。</p>
 </div>
 </dd>
 <dt>{{htmlattrdef("hreflang")}}</dt>
 <dd>该属性用于指定链接文档的人类语言。其仅提供建议，并没有内置的功能。hreflang 允许的值取决于 HTML5 <a href="http://www.ietf.org/rfc/bcp/bcp47.txt">BCP47</a>。</dd>
 <dt>{{htmlattrdef("ping")}}</dt>
 <dd>包含一个以空格分隔的 url 列表，当跟随超链接时，将由浏览器 (在后台) 发送带有正文 PING 的 POST 请求。通常用于跟踪。</dd>
 <dt>{{htmlattrdef("referrerpolicy")}} {{experimental_inline}}</dt>
 <dd>表明在获取 URL 时发送哪个提交者（referrer）:
 <ul>
...
<dl>
 ...
 <div class="note">
 <p>在 <code>&lt;a&gt;</code> 元素上使用 <code>target="_blank"</code> 隐式提供了与使用 <code><a href="/en-US/docs/Web/HTML/Link_types/noopener">rel="noopener"</a></code> 相同的 <code>rel</code> 行为，即不会设置 <code>window.opener</code>。
 </div>
 </dd>
...
<dl>
 ...
 <dd>此属性定义链接资源的字符编码。该值是一个空格- 和/或逗号分隔的<a href="http://tools.ietf.org/html/rfc2045">RFC 2045</a>中定义的字符集列表。默认值是 ISO-8859-1。
 <div class="note">
 <p>在 HTML5 中该属性已作废，不应使用。 为了实现其效果，使用 HTTP content - type header 的链接资源。</p>
 </div>
 </dd>
 ...
 <dd>该属性在页面中定义锚点的目标位置时是必须的。 <strong>name</strong> 的值类似于 ID 核心属性值，该属性值在文档中是唯一的且由数字字母标示符所组成的。在 HTML 4.01 规范中，<strong>id</strong> 和 <strong>name</strong> 都可以使用 &lt;a&gt; 元素，只要他们有相同的值。
 <div class="note">
 <p>该属性在 HTML5 中是过时的，使用 <a href="/zh-CN/docs/HTML/Global_attributes#attr-id">全局属性 <strong>id</strong></a> 来代替。</p>
 </div>
 </dd>
...
<dl>
 ...
 <dd>此属性用于定义一个可选的超链接相关的一个数字来创建图像映射区域，该属性的值是圆，默认情况下，多边形，矩形。 所述的<strong>coords</strong> 属性的格式取决于形状的值。对于圆，该值的 x，y，r 其中 x 和 y 为圆心像素坐标，r 是像素值半径。对于矩形，该<strong>coords</strong> 属性应该是 x，y，w，h。x，y 值定义的矩形的左上角的位置，而 w 和 h 分别定义宽度和高度。多边形形状的值需要 x1，y1，x2，y2，......值来定义 coords。每对 x，y 定义多边形的一个点，连续点由直线接合和，最后一个点连接到第一个点。默认为形状的值需要将整个封闭区域，典型的图像，可以使用。
 <div class="note">建议使用 <a href="/zh-CN/docs/Web/HTML/Element/img#attr-usemap">使用 map 属性</a>在 {{HTMLElement("img")}} 元素和相关的{{HTMLElement("map")}}元素中来定义热点而不是用形状属性。</div>
 </dd>
</dl>
...
<p><a href="http://www.mozilla.com/">外部链接</a></p>
...
<p>这个小例子使用图像链接到 MDN 主页。主页将在新的浏览环境，这是一个新的页面或新标签中打开研究。</p>
...
<h3 id="创建一个email链接">创建一个 email 链接</h3>

<p>这是常见的创建按钮或链接，将用户的电子邮件程序打开，让他们发送新邮件。这是通过使用一个 mailto 链接完成的。这里有一个简单的例子：</p>
...
<p>有关 mailto URL 方案的更多细节，比如如何包含主题，正文，或其他预定内容，参考 <a href="/zh-CN/docs/Web/Guide/HTML/Email_links">Email links</a> or {{RFC(6068)}}.</p>
...
<p>如果你想允许用户下载一个 HTML 画布，你可以创建一个下载属性和画布数据作为文件 URL 链接图像：</p>
