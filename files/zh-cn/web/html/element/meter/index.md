...
- `low`
  - : 测量范围低端的上限数值。该值必须大于最小值（`min` 属性），如果指定了高值和最大值（分别为 `high` 属性和 `max` 属性），也必须小于高值和最大值。如果未指定或小于最小值，`low` 值等于最小值。
- `high`
  - : 测量范围高端的下限数值。该值必须小于最大值（`max` 属性），如果指定了低值和最小值（分别为 `low` 属性和 `min` 属性），也必须大于这两个值。如果未指定或大于最大值，`high` 值等于最大值。
- `optimum`
  - : 该属性表示最佳数值。必须在 `min` 属性和 `max` 属性定义的范围内。当与 `low` 属性和 `high` 属性一起使用时，它表示范围内哪个位置更合适。例如，如果值介于 `min` 属性和 `low` 属性之间，则认为较低范围是首选。浏览器可能会根据值是否小于或等于最佳值来对 meter 元素的条形进行不同的颜色显示。
- `form`
  - : 此可选属性用于显式设置 `<meter>` 元素的 {{HTMLElement("form")}} 所有者。如果省略，则 `<meter>` 元素将与其 `<form>` 父元素关联，或者与另一个父元素（例如 {{HTMLElement("fieldset")}}）上的 `form` 属性所设置的表单关联相关联。如果包含，则该值必须是同一文档树中 `<form>` 元素的 [`id`](/zh-CN/docs/Web/HTML/Global_attributes/id)。
