...
<p>表格标签支持<a href="https://developer.mozilla.org/en-US/docs/HTML/Global_attributes">全局属性</a>。下面列出的属性现在已经弃用（请使用 CSS 代替）。</p>

<dl>
 <dt>{{htmlattrdef("align")}} {{Deprecated_inline}}</dt>
 <dd> 这个枚举属性指定了包含在文档中的表格必须如何对齐。有如下值：
 <ul>
  <li>left，表格将在文档左侧显示；</li>
  <li>center， 表格将在文档中央显示；</li>
  <li>right， 表格将在文档右侧显示；</li>
 </ul>

 <div class="note"><strong>小贴士：</strong>

 <ul>
  <li><strong>不要使用这个属性</strong>，它已经不赞成被使用。  {{HTMLElement("table")}} 元素应该使用<a href="/en-US/docs/CSS">CSS</a> 制定样式。设置 {{cssxref("margin-left")}} 、{{cssxref("margin-right")}} 为 <code>auto</code> ( 或者 {{cssxref("margin")}} <code>设置为 0 auto</code>) 来实现类似于 align 属性的效果。</li>
...
<dl>
 ...
 <dd>这个属性定义了表格的背景颜色。 它是一个 6 位的十六进制编码，定义于 <a href="http://www.w3.org/Graphics/Color/sRGB">sRGB</a>，需要加上'#'作为前缀。 当然也可以使用下面已经被预先定义的字符串，来表示常用的 16 种颜色：
 <table>
  <tbody>
   <tr>
 ...
 <div class="note"><strong>小贴士：</strong>不要使用这个属性，该属性已废弃，{{HTMLElement("table")}} 元素应当使用 <a href="/en-US/docs/CSS">CSS</a> 定制样式。 <a href="/en-US/docs/CSS">CSS</a> 中的 {{cssxref("background-color")}} 属性能够和 bgcolor 属性达到一样的效果。</div>
 </dd>
</dl>

<dl>
 <dt>{{htmlattrdef("border")}} {{Deprecated_inline}}</dt>
 <dd>这个整型元素使用像素，定义了表格边框的大小。 如果设置为 0，这意味着{{htmlattrxref("frame", "table")}} 属性被设置为空。如果设置为 1，表示表格具有 1px 大小的边框。
 <div class="note"><strong>小贴士：</strong>不要使用这个属性，该属性已废弃，{{HTMLElement("table")}} 元素应当使用 <a href="/en-US/docs/CSS">CSS</a> 定制样式。 <a href="/en-US/docs/CSS">CSS</a> 属性 {{cssxref("border")}}, {{cssxref("border-color")}}, {{cssxref("border-width")}} 和 {{cssxref("border-style")}} 可实现相同效果。</div>
 </dd>
</dl>
...
<dl>
 ...
 <div class="note"><strong>小贴士：不要使用这个属性</strong>，因为它已经被废弃。 {{HTMLElement("table")}} 元素应该使用<a href="/en-US/docs/CSS"> CSS</a> 定制样式。 在{{HTMLElement("table")}} 元素上使用 <a href="/en-US/docs/CSS">CSS</a> 属性值为 collapse 的 {{cssxref("border-collapse")}} 属性，在 {{HTMLElement("td")}} 元素上使用属性 {{cssxref("padding")}}，以达到类似于 cellpadding 的效果。</div>
 </dd>
</dl>
...
<dl>
 ...
 <div class="note"><strong>小贴士：不要使用这个属性，因为它已经被废弃。</strong> {{HTMLElement("table")}} 元素应该使用 <a href="/en-US/docs/CSS">CSS</a> 定制样式。在 {{HTMLElement("table")}} 元素上使用 CSS 的属性 {{cssxref("border-spacing")}} ，以达到类似于 cellspacing 的效果。</div>
 </dd>
</dl>
...
<dl>
 ...
 <div class="note"><strong>小贴士：不要使用这个属性，因为它已经被废弃。</strong> {{HTMLElement("table")}} 元素应该使用 <a href="/en-US/docs/CSS">CSS</a> 定制样式。在 {{HTMLElement("table")}} 元素上使用 CSS 的属性 {{cssxref(" border-style ")}} 和  {{cssxref("border-width")}} ，以达到类似于 frame 的效果。</div>
 </dd>
</dl>
...
<dl>
 ...
 <ul>
  <li>none，这表明没有规格将被显示；这是默认的值；</li>
  <li>groups，这将使规则只显示在行组（row groups，通过 {{HTMLElement("thead")}} ， {{HTMLElement("tbody")}} 和 {{HTMLElement("tfoot")}} 元素定义）和列组（ column groups，通过{{HTMLElement("col")}} 和 {{HTMLElement("colgroup")}} 元素定义）之间</li>
  <li>rows，这将使规则在行之间显示；</li>
  <li>columns，这将使规则在列之间显示；</li>
  <li>
   <p>all，这将使规则在列和行之间显示；</p>
  </li>
 </ul>
 ...
 <ul>
  ...
  <li><strong>不要使用这个属性，因为它已经被废弃。</strong>规则应该使用 <a href="/en-US/docs/CSS">CSS</a> 定义和设计样式。使用 {{cssxref("border")}} 属性，在适当的 {{HTMLElement("thead")}}, {{HTMLElement("tbody")}}, {{HTMLElement("tfoot")}}, {{HTMLElement("col")}} 或 {{HTMLElement("colgroup")}} 元素上。</li>
 </ul>
 </div>
 </dd>
 <dt>{{htmlattrdef("summary")}} {{Deprecated_inline}}</dt>
 <dd>该属性定义了一个替代文本，当表格无法在用户代理中显示的时候用来描述表格。通常，它被用来为残障人士提供可用性，比如，盲人使用盲文屏幕（Braille screen）浏览网页，从中获取信息。如果要想对于非视力受限的人来说也可以提供作用，考虑使用 {{HTMLElement("caption")}} 代替。summary 属性不是强制性的，当一个 {{HTMLElement("caption")}} 发挥它应有的作用时，可以省略 summary 属性。
 ...
 <div class="note"><strong>小贴士：</strong> <strong>不要使用这个属性，因为它已经被废弃。</strong>规则应该使用 <a href="/en-US/docs/CSS">CSS</a> 定义和设计样式。使用 {{cssxref("width")}} 属性代替。</div>
 </dd>
</dl>
...
<ul>
 <li>其它表格相关的 HTML 元素: {{HTMLElement("caption")}}, {{HTMLElement("col")}}, {{HTMLElement("colgroup")}}, {{HTMLElement("tbody")}}, {{HTMLElement("td")}}, {{HTMLElement("tfoot")}}, {{HTMLElement("th")}}, {{HTMLElement("thead")}}, {{HTMLElement("tr")}};</li>
