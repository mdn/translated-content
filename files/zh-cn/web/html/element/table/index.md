...
- {{htmlattrdef("rules")}} {{Deprecated_inline}}
  ...
  - : 这个枚举属性定义了在一个表格中分隔线的显示位置。它可以有以下值：
    ...
    - `groups`，这将使得分隔线只显示在行组（row groups，通过 {{HTMLElement("thead")}}、{{HTMLElement("tbody")}} 和 {{HTMLElement("tfoot")}} 元素定义）和列组（column groups，通过 {{HTMLElement("col")}} 和 {{HTMLElement("colgroup")}} 元素定义）之间
    - `rows`，这将使得分隔线在行之间显示；
    - `cols`，这将使得分隔线在列之间显示；
    - `all`，这将使得分隔线在列和行之间显示；
