...
默认情况下，当关闭时，该小组件的高度只足以显示展开三角形和摘要。当打开时，它就会膨胀，显示其中包含的细节。
...
除了 HTML 元素支持的常规事件外，`<details>` 元素还支持 {{domxref("HTMLDetailsElement/toggle_event", "toggle")}} 事件，当 `<details>` 元素的状态在打开和关闭之间变化时，该事件会被派发到该元素。该事件是在状态发生变化*后*发送的，不过如果状态在浏览器派发该事件之前发生了多次变化，那么这些事件就会聚集起来，从而只发送一个。
