...
<dl>
 ...
 <dd>该属性仅在<code>&lt;link&gt;</code>元素设置了 <code>rel="preload"</code> 或者 <code>rel="prefetch"</code> 时才能使用。它规定了<code>&lt;link&gt;元素</code>加载的内容的类型，对于内容的优先级、请求匹配、正确的<a href="/zh-CN/docs/Web/HTTP/CSP">内容安全策略</a>的选择以及正确的 {{httpheader("Accept")}}请求头的设置，这个属性是必需的。<br />
  <table>
  <thead>
   <tr>
    <th scope="col">值</th>
 ...
 <dd>此枚举属性指定在加载相关资源时是否必须使用 CORS. <a href="/en-US/docs/Web/HTML/CORS_Enabled_Image">启用 CORS 的图片</a> 可以在 {{HTMLElement("canvas")}} 元素中重复使用，并避免其被<em>污染</em>. 可取的值如下：<br />
 <dl>
 ...
 <dd>这个属性规定了外部资源适用的媒体类型。它的值必须是"<a href="https://developer.mozilla.org/zh-CN/docs/Web/Guide/CSS/Media_queries">媒体查询</a>"。这个属性使得用户代理能选择最适合设备运行的媒体类型。<br />
 <ul>
 ...
 <dd>一个字符串，指示在获取资源时使用哪个引荐来源网址：<br />
 <ul>
...
<dl>
 ...
 <dd>此属性的值显示了 {{HTMLAttrxRef("href", "link")}} 属性所定义的当前文档与链接文档的关系。 因此，该属性定义了与 rel 属性的值相比的反向关系。 该属性的<a href="/zh-CN/docs/Web/HTML/Link_types">Link type value</a>类似于{{HTMLAttrxRef("rel", "link")}}的可能值。<br />
 <div class="blockIndicator note">
