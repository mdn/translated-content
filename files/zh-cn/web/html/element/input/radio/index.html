...
<p><img alt="Shows what radio buttons looked like in the olden days." src="old-radio.jpg"></p>

<div class="note">
<p><strong>注意：</strong><a href="/en-US/docs/Web/HTML/Element/input/checkbox">复选框</a>类似于单选按钮，但是有个重要的区别 -- 单选按钮为选择其中一项而设计，然而复选框允许你开启或关闭单个值。每个独立的单选按钮大致上是 {{domxref("Boolean")}} 控件 --  是或不是。</p>
</div>
...
<p>你可以在这里看到三个单选按钮，每个按钮的 <code>name</code> 属性都设置为 <code>contact</code> 并且都含有在单选按钮组中唯一的 <code>value</code> 属性。 每一个单选按钮也有一个给{{HTMLElement("label")}} 元素的 {{htmlattrxref("for", "label")}}属性使用的{{domxref("Element.id", "id")}}，将 label 元素和点选按钮关联。</p>

<p><em>译者注：你可以用 label 元素把 <code>input</code> 元素包裹起来，以减少 <code>id</code> 的使用。</em></p>
