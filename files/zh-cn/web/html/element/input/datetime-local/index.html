...
<p><code>datetime-local</code> 控件并没有地方可以设置日期/时间的时区和/或区域属性。在 <code><a href="/en-US/docs/Web/HTML/Element/input/datetime">datetime</a></code> 输入类型上提供过此功能，但这个类型现在已被废弃，也已从标准中移除。这项被移除的主要原因是缺少浏览器的支持，以及出于用户交互/用户体验方法的考量。相比之下，仅使用一个（或多个）控件来设置日期/时间，然后单独在另一控件处理时区，这样更容易一些。</p>
