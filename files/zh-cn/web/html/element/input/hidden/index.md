...
`{{HTMLElement("input")}}` 元素的 `{{htmlattrxref("value", "input")}}` 属性包含要提交到服务器的隐藏数据。尽管可以通过浏览器开发人员工具编辑该值，但明确要求用户不能通过用户界面看到或编辑这个值。

> **警告：** 尽管该值并未作为页面内容中显示给用户，但仍然可以使用任何浏览器的开发人员工具或“查看源代码”功能来查看并进行编辑。请不要将表单的安全性依赖于使用 `hidden` 输入上。
...
隐藏的 `{{HTMLElement("input")}}` 不参与约束验证；它们没有可受到约束的属性值。
