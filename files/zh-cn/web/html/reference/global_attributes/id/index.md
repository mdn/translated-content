...
> [!NOTE]
> 从技术上讲，`id` 属性的值可以包含除了{{glossary("whitespace","空白")}}字符的任何字符。然而，为了避免无意中的错误，只能使用 {{glossary("ASCII")}} 字母、数字、`'_'` 和 `'-'`，并且`id` 属性的值应该以字母开头。例如，`.` 在 CSS 中具有特殊的意义（它作为一个[类选择器](/zh-CN/docs/Web/CSS/Class_selectors)）。除非你注意在 CSS 中转义它，否则它不会被识别为 `id` 属性值的一部分。很容易忘记这样做，导致你的代码中出现难以检测的错误。
