...
- _[内容类别](/zh-CN/docs/Web/HTML/Guides/Content_categories)_ 无
...
- `selected`
  - : 这个布尔属性存在时表明这个选项是否一开始就被选中。如果 `<option>` 元素是 {{HTMLElement("select")}} 元素的子元素，并且 {{HTMLElement("select")}} 元素的 [`multiple`](/zh-CN/docs/Web/HTML/Reference/Elements/select#multiple) 属性没有设置，则 {{HTMLElement("select")}} 元素中只有一个 `<option>` 元素可以拥有 `selected` 属性。
- `value`
