...
该控件的用户界面通常因浏览器而异。该控件旨在表示*本地日期和时间*，而非必然代表*用户的本地日期和时间*。换言之，该输入允许任何有效的年、月、日、时、分组合——即使该组合在用户本地时区内无效（例如夏令时春季调整时段内的一小时过渡期）。
...
<form>
  <div>
    <label for="party">
      请选择偏好的派对日期和时间（必填项，6 月 1 日上午 8:30 至 6 月 30 日下午
      4:30）：
    </label>
    <input
      id="party"
