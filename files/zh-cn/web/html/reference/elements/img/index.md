...
- `sizes`
  - : 一个或多个用逗号分隔的值，可以是资源尺寸或 `auto` 关键字。
    ...
    媒体条件描述的是*视口*的属性，而非*图像*本身。例如，`(height <= 500px) 1000px` 表示当*视口*高度小于等于 500 像素时，应使用宽度为 1000 像素的图像源。由于源尺寸描述符指定了布局过程中图像应采用的宽度，媒体条件通常（但非必然）基于[宽度](/zh-CN/docs/Web/CSS/@media/width)属性。
