...
<p>如果一条语句独占一行的话，那么分号是可以省略的。（译者注：并不建议这么做。）但如果一行中有多条语句，那么这些语句必须以分号分开。</p> 

<div class="note">
<P><strong>备注：</strong>ECMAScript 规定了在语句的末尾自动插入分号（<a href="/zh-CN/docs/Web/JavaScript/Reference/Lexical_grammar#自动分号补全">ASI</a>）。（如果想要了解更多信息，请参阅 JavaScript <a href="/zh-CN/docs/Web/JavaScript/Reference/Lexical_grammar">词法语法</a> 。）</P>
</div>

<p>虽然不是必需的，但是在一条语句的末尾加上分号是一个很好的习惯。这个习惯可以大大减少代码中产生 bug 的可能性。</p>
...
<div class="note">
  <p><strong>备注：</strong>你可能会在一些 JavaScript 脚本中见到像这样<code>#!/usr/bin/env node</code>的第三种注释</p>
  <p>这种注释我们称为<strong>hashbang 注释 (hashbang comment)</strong>，这种注释被用来指定执行 JaveScript 脚本的引擎的路径，查看这篇文章来了解更详细的信息：<a href="/zh-CN/docs/Web/JavaScript/Reference/Lexical_grammar#hashbang_注释">Hashbang 注释</a></p>
</div>
...
<p>
  你可以使用<a href="/zh-CN/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment">解构赋值</a>将<a href="#对象字面量_object_literals">对象字面量</a>的属性绑定到变量。就像这样<code>let { bar } = foo</code>。这会创建一个名为<code>bar</code>的变量，并且将<code>foo</code>对像中属性名与之相同的属性的值赋给它。
</p>

<p>
  你也可以直接给属性赋值。像这样<code>x = 42</code>。这种声明方式将会创建一个<a href="/zh-CN/docs/Web/JavaScript/Reference/Statements/var#%E6%8F%8F%E8%BF%B0">未声明全局变量</a>。这样做还会产生 JavaScript 警告。因为未声明的全局变量常常导致预期之外的行为，所以不建议使用。
</p>

<h3 id="变量求值">变量求值</h3>

<p>用 <code>var</code> 或 <code>let</code> 语句声明的变量，如果没有赋初始值，则其值为 <code><a href="/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/undefined">undefined</a></code>。</p>

<p>如果访问一个未声明的变量会导致抛出一个<code><a href="zh-CN/docs/Web/JavaScript/Reference/Global_Objects/ReferenceError">引用错误</a></code>（ReferenceError）异常：</p>
...
<p>你可以使用 <code>undefined</code> 来判断一个变量是否已赋值。在以下的代码中，变量<code>input</code>未被赋值，因此 <code><a href="https://developer.mozilla.org/zh-CN/docs/JavaScript/Reference/Statements/if...else">if</a></code> 条件语句的求值结果是 <code>true</code> 。</p>
...
<p><code>undefined</code> 值在布尔类型环境中会被当作 <code>false</code> 。例如，下面的代码将会执行函数 <code>myFunction</code>，因为数组 <code>myArray</code> 中的元素未被赋值：</p>
...
<p>当你对一个 <code><a href="/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/null">null</a></code> 变量求值时，空值 <code>null</code> 在数值类型环境中会被当作 0 来对待，而布尔类型环境中会被当作 <code>false</code>。例如：</p>
...
<p>实际上，全局变量是<em>全局对象</em>的属性。在网页中，（译注：缺省的）全局对象是 <code><a href="/zh-CN/docs/Web/API/Window">window</a></code> ，所以你可以用形如 <code>window.</code><em><code>variable</code> </em>的语法来设置和访问全局变量。</p>
...
<p>虽然这些数据类型相对来说比较少，但是通过他们你可以在程序中开发有用的功能。对象（<a href="https://developer.mozilla.org/zh-CN/docs/JavaScript/Reference/Global_Objects/Object">Objects</a>）和函数（<a href="https://developer.mozilla.org/zh-CN/docs/JavaScript/Reference/Global_Objects/Function">functions</a>）是这门语言的另外两个基本元素。你可以把对象当作存放值的一个命名容器，然后将函数当作你的程序能够执行的步骤。</p>

<h3 id="数据类型的转换">数据类型的转换</h3>

<p>JavaScript 是一种动态类型语言 (dynamically typed language)。这意味着你在声明变量时可以不必指定数据类型，而数据类型会在代码执行时会根据需要自动转换。因此，你可以按照如下方式来定义变量：</p>
...
<p>在涉及其它运算符（译注：如下面的减号'-'）时，JavaScript 语言不会把数字变为字符串。例如（译注：第一例是数学运算，第二例是字符串运算）：</p>
...
<ul>
  <li><code><a href="https://developer.mozilla.org/zh-CN/docs/JavaScript/Reference/Global_Objects/parseInt">parseInt()</a></code></li>
  <li><code><a href="https://developer.mozilla.org/zh-CN/docs/JavaScript/Reference/Global_Objects/parseFloat">parseFloat()</a></code></li>
</ul>
...
<p>在 JavaScript 中，你可以使用各种字面量。这些字面量是脚本中按字面意思给出的固定的值，而不是变量。（译注：字面量是常量，其值是固定的，而且在程序脚本运行中不可更改，比如<em>false</em>，3.1415，thisIsStringOfHelloworld<em> </em>，invokedFunction: myFunction("myArgument")。本节将介绍以下类型的字面量：</p>

<ul>
 <li><a href="#数组字面量_array_literals">数组字面量 (Array literals)</a></li>
 <li><a href="#布尔字面量_boolean_literals">布尔字面量 (Boolean literals)</a></li>
 <li><a href="#数字字面量_Numeric_literals">数字字面量 (Numeric_literals)</a></li>
...
<div class="note">
<p><strong>备注：</strong><strong> </strong>这里的数组字面值也是一种对象初始化器。参考<a href="https://developer.mozilla.org/zh-CN/docs/JavaScript/Guide/Working_with_Objects#Using_Object_Initializers">对象初始化器的使用</a>。</p>
</div>

<p>若在顶层（全局）脚本里用字面值创建数组，JavaScript 语言将会在每次对包含该数组字面值的表达式求值时解释该数组。另一方面，在函数中使用的数组，将在每次调用函数时都会被创建一次。</p>
...
<p>在这个数组中，有两个已被赋值的元素，和一个空元素（fish[0] 是"Lion"，fish[1] 是 undefined，而 fish[2] 是"Angel"；译注：此时数组的长度属性 fish.length 是 3)。</p>

<p>如果你在元素列表的尾部添加了一个逗号，它将会被忽略。在下面的例子中，数组的长度是 3，并不存在 myList[3] 这个元素（译注：这是指数组的第 4 个元素噢，作者是在帮大家复习数组元素的排序命名方法）。元素列表中其它所有的逗号都表示一个新元素（的开始）。</p>
...
<p>在下面的例子中，数组的长度是 4，元素<code>myList[0]</code>和<code>myList[2]</code>缺失（译注：没被赋值，因而是 undefined）。</p>

<pre class="brush: js">var myList = [ , 'home', , 'school'];
</pre>

<p>再看一个例子。在这里，该数组的长度是 4，元素<code>myList[1]</code>和<code>myList[3]</code>被漏掉了。（但是）只有最后的那个逗号被忽略。</p>

<pre class="brush: js">var myList = ['home', , 'school', , ];
</pre>

<p>理解多余的逗号（在脚本运行时会被如何处理）的含义，对于从语言层面理解 JavaScript 是十分重要的。但是，在你自己写代码时：<strong>显式地将缺失的元素声明为<code>undefined</code>，将大大提高你的代码的清晰度和可维护性</strong>。</p>
...
<p>不要混淆作为布尔对象的真和假与布尔类型的原始值 true 和 false。布尔对象是原始布尔数据类型的一个包装器。参见<a href="https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Boolean"> 布尔对象</a>。</p>

<h3 id="数字字面量_Numeric_literals">数字字面量</h3>

<p>JavaScript 数字字面量包括多种基数的整数字面量和以 10 为基数的浮点数字面量</p>

<p>值得一提的是，语言标准要求数字字面量必须是无符号的。但是像<code>-123.4</code>这样的代码片段还是没有问题的，会被解释为一元操作符<code>-</code>应用于数字字面量<code>123.4</code></p>

<h4 id="整数字面量_Integers_literals">整数字面量</h3>

<p>整数可以用十进制（基数为 10）、十六进制（基数为 16）、八进制（基数为 8）以及二进制（基数为 2）表示。</p>

<ul>
 <li>十进制整数字面量由一串数字序列组成，且没有前缀 0。</li>
 <li>八进制的整数以 0（或 0O、0o）开头，只能包括数字 0-7。</li>
 <li>十六进制整数以 0x（或 0X）开头，可以包含数字（0-9）和字母 a~f 或 A~F。</li>
 <li>二进制整数以 0b（或 0B）开头，只能包含数字 0 和 1。</li>
</ul>

<p>严格模式下，八进制整数字面量必须以 0o 或 0O 开头，而不能以 0 开头。</p>
...
0b11, 0b0011 and -0b11 (二进制，基数为 2)</code></pre>

<h4 id="浮点数字面量_Floating-point_literals">浮点数字面量</h3>
...
<div class="warning">
<p><strong>警告：</strong>你不能在一条语句的开头就使用对象字面值，这将导致错误或产生超出预料的行为， 因为此时左花括号（{）会被认为是一个语句块的起始符号。（译者：这 里需要对语句 statement、块 block 等基本名词的解释）</p>
</div>

<p>以下是一个对象字面值的例子。对象<code>car</code>的第一个元素（译注：即一个属性/值对）定义了属性<code>myCar</code>；第二个元素，属性<code>getCar</code>，引用了一个函数调用（即 CarTypes("Honda")）；第三个元素，属性<code>special</code>，使用了一个已有的变量（即 Sales）。</p>
...
<p>你可以在字符串字面值上使用字符串对象的所有方法——JavaScript 会自动将字符串字面值转换为一个临时字符串对象，调用该方法，然后废弃掉那个临时的字符串对象。你也能用对字符串字面值使用类似 String.length 的属性：</p>

<pre class="brush: js"><code>console.log("John's cat".length)
// 将打印字符串中的字符个数（包括空格）
// 结果为：10</code></pre>

<p>在 ES2015 中，还提供了一种<a href="/zh-CN/docs/Web/JavaScript/Reference/Template_literals">模板字面量</a>，模板字符串提供了一些语法糖来帮你构造字符串。这与 Perl、Python 还有其他语言中的字符串插值（string interpolation）的特性非常相似。除此之外，你可以在通过模板字符串前添加一个 tag 来自定义模板字符串的解析过程，这可以用来防止注入攻击，或者用来建立基于字符串的高级数据抽象。</p>
...
<p>除非有特别需要使用字符串对象，否则，你应当始终使用字符串字面值。要查看字符串对象的有关细节，请参见<a href="https://developer.mozilla.org/zh-CN/docs/JavaScript/Guide/Predefined_Core_Objects#String_Object">字符串对象</a>。</p>
...
<p>以下表格列举了你能在 JavaScript 的字符串中使用的特殊字符。</p>
...
<table class="standard-table">
 ...
 <tbody>
  ...
  <tr>
   <td>\<em>XXX</em></td>
   <td>由从 0 到 377 最多三位八进制数<em>XXX</em>表示的 Latin-1 字符。例如，\251 是版权符号的八进制序列。</td>
  </tr>
  <tr>
   <td>\x<em>XX</em></td>
   <td>由从 00 和 FF 的两位十六进制数字 XX 表示的 Latin-1 字符。例如，\ xA9 是版权符号的十六进制序列。</td>
  </tr>
  <tr>
   <td><em>\uXXXX</em></td>
   <td>由四位十六进制数字 XXXX 表示的 Unicode 字符。例如，\ u00A9 是版权符号的 Unicode 序列。见<a href="/en-US/docs/Web/JavaScript/Reference/Lexical_grammar#String_literals">Unicode escape sequences</a> (Unicode 转义字符).</td>
  </tr>
  <tr>
   <td>\u<em>{XXXXX}</em></td>
   <td>Unicode 代码点 (code point) 转义字符。例如，\u{2F804} 相当于 Unicode 转义字符 \uD87E\uDC04 的简写。</td>
  </tr>
 </tbody>
</table>
...
<p>对于那些未出现在表 2.1 中的字符，其所带的前导反斜线'\'将被忽略。但是，这一用法已被废弃，应当避免使用。</p>
...
<p>Javascript 没有“heredoc”语法，但可以用行末的换行符转义和转义的换行来近似实现 </p>
...
<p>ECMAScript 2015 增加了一种新的字面量，叫做模板字面量 <strong><a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/template_strings">template literals</a>。</strong>它包含一些新特征，包括了多行字符串！</p>
...
<p>本章重点包括声明和类型的基本语法。学习更多关于的 JavaScript 语言，可参见本站以下章节：</p>
