...
<p>一个 javascript 对象有很多属性。一个对象的属性可以被解释成一个附加到对象上的变量。对象的属性和普通的 javascript 变量基本没什么区别，仅仅是属性属于某个对象。属性定义了对象的特征。你可以通过点符号来访问一个对象的属性。</p>

<pre class="brush: js">objectName.propertyName
</pre>

<p>和其他 javascript 变量一样，对象的名字 (可以是普通的变量) 和属性的名字都是大小写敏感的。你可以在定义一个属性的时候就给它赋值。例如，我们创建一个 myCar 的对象然后给他三个属性，make，model，year。具体如下所示：</p>
...
<p>从 <a href="/zh-CN/docs/JavaScript/ECMAScript_5_support_in_Mozilla">ECMAScript 5</a> 开始，有三种原生的方法用于列出或枚举对象的属性：</p>

<ul>
 <li><a href="/zh-CN/docs/JavaScript/Reference/Statements/for...in">for...in</a> 循环<br>
  该方法依次访问一个对象及其原型链中所有可枚举的属性。</li>
 <li><a href="/zh-CN/docs/JavaScript/Reference/Global_Objects/Object/keys">Object.keys(o)</a><br>
  该方法返回对象 <code>o</code> 自身包含（不包括原型中）的所有可枚举属性的名称的数组。</li>
 <li><a href="/zh-CN/docs/JavaScript/Reference/Global_Objects/Object/getOwnPropertyNames">Object.getOwnPropertyNames(o)</a><br>
...
<p>你也可以用对象初始化器来创建数组。参见<a href="/zh-CN/docs/Web/JavaScript/Guide/Grammar_and_types#数组字面量_array_literals">数组字面量</a></p>
...
<p>这条规则的例外是从与 HTML 对应的对象，例如 <code>forms</code> 数组。对于这些数组的元素，你总是既可以通过其序号（依据其在文档中出现的顺序），也可以按照其名称（如果有的话）访问它。举例而言，如果文档中的第二个 <code>&lt;form&gt;</code> 标签有一个 <code>NAME</code> 属性且值为<code> "myForm"，访问该 form 的方式可以是 document.forms[1]，document.forms["myForm"] 或 document.myForm。</code></p>
...
<p>参见 <a href="/zh-CN/docs/JavaScript/Reference">JavaScript Reference</a> 中 Function 对象的 <a href="/zh-CN/docs/JavaScript/Reference/Global_Objects/Function/prototype"><code>prototype</code> 属性</a> 。</p>
...
<p>下面例子描述了 getters 和 setters 是如何为用户定义的对象<code> o</code> 工作的。</p>
...
<p>请注意在一个对象字面量语法中定义 getter 和 setter 使用"[gs]et property()"的方式（相比较于__define[GS]etter__) 时，并不是获取和设置某个属性自身，容易让人误以为是"[gs]et propertyName(){ }"这样错误的使用方法。定义一个 getter 或 setter 函数使用语法"[gs]et property()"，定义一个已经声明的函数作为的 getter 和 setter 方法，使用<code><a href="/zh-CN/docs/JavaScript/Reference/Global_Objects/Object/defineProperty">Object.defineProperty</a></code>(或者 <code><a href="/zh-CN/docs/JavaScript/Reference/Global_Objects/Object/defineGetter">Object.prototype.__defineGetter__</a></code> 旧语法回退)</p>

<p>下面这个例子展示使用 getter 和 setter 方法扩展 {{jsxref("Date")}}原型，为预定义好的 Date 类添加一个 year 的属性。定义属性 year 的 getter 和 setter 方法用到了 Date 类中已存在的 getFullYear 和 setFullYear 方法。</p>
...
<p>当使用 <a href="#使用对象初始化器">使用对象初始化器</a> 的方式定义 getter 和 setter 时，只需要在 getter 方法前加 get，在 setter 方法前加 set，当然，getter 方法必须是无参数的，setter 方法只接受一个参数 (设置为新值），例如：</p>
...
<p>你可以用 <a href="/zh-CN/docs/Web/JavaScript/Reference/Operators/delete"><code>delete</code></a> 操作符删除一个<strong>不是继承而来</strong>的属性。下面的例子说明如何删除一个属性：</p>
...
<p>在 JavaScript 中 objects 是一种引用类型。两个独立声明的对象永远也不会相等，即使他们有相同的属性，只有在比较一个对象和这个对象的引用时，才会返回 true.</p>
...
var fruit = {name: "apple"};
var fruitbear = fruit;  // 将 fruit 的对象<strong>引用</strong>(reference) 赋值给 fruitbear
                        // 也称为将 fruitbear“<em><strong>指向</strong></em>”fruit 对象
...
<ul>
 ...
 <li>想要学习 ECMAScript 2015 中类（一种创建对象的新方式），请阅读 <a href="https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Classes">JavaScript classes</a> 章节。</li>
</ul>
