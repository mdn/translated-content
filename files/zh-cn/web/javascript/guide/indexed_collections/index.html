...
<p>数组 (array) 是一个有序的数据集合，我们可以通过数组名称 (name) 和索引 (index) 进行访问。例如，我们定义了一个数组 emp，数组中的每个元素包含了一个雇员的名字以及其作为索引的员工号。那么 emp[1] 将会代表 1 号员工，emp[2] 将会代表 2 号员工，以此类推。</p>

<p>JavaScript 中没有明确的数组数据类型。但是，我们可以通过使用内置 Array 对象和它的方法对数组进行操作。Array 对象有很多操作数组的方法，比如合并、反转、排序等。数组对象有一个决定数组长度和使用正则表达式操作其他属性的属性。</p>
...
// 后者 4 指数组长度，所以使用字面值 (literal) 的方式应该不仅仅是便捷，同时也不易踩坑</pre>

<p><code>element0, element1, ..., elementN</code> 是数组元素的值的列表。当这些值被指定后，数组将被初始化，他们将被作为数组元素。数组的 length 属性也会被设为参数的个数。</p>

<p>括号语法被称为 "数组字面值" 或 "数组初始化器", 它比其他创建数组的方式更便捷，所以通常是首选。详细内容参见 <a href="/en-US/docs/Web/JavaScript/Guide/Grammar_and_types#Array_literals">Array literals</a> 。</p>
...
<p>如果你希望用单个元素初始化一个数组，而这个元素恰好又是数字 (<code>Number</code>)，那么你必须使用括号语法。当单个的数字 (<code>Number</code>) 传递给 Array() 构造函数时，将会被解释为数组长度，并非单个元素。</p>
...
<p>如果 N 不是一个整数，调用<code>Array(N)</code>将会报<code>RangeError</code>错误， 以下方法说明了这种行为：</p>
...
<p>在实施层面， JavaScript 实际上是将元素作为标准的对象属性来存储，把数组索引作为属性名。长度属性是特殊的，它总是返回最后一个元素的索引值加 1(下例中， Dusty 的索引是 30，所以 cats.length 返回 30 + 1)。记住， JavaScript 数组索引是基于 0 的：他们从 0 开始，而不是 1。这意味着数组长度属性将比最大的索引值大 1:</p>
...
<p>你也可以分配<code>length</code>属性。写一个小于数组元素数量的值会缩短数组，写 0 会彻底清空数组：</p>
...
<p>如果你确定数组中没有一个元素的求值是 false —— 如果你的数组只包含<a href="/en-US/docs/DOM">DOM</a>节点，如下，你可以选择一个更高效的土法子：</p>
...
<p>被传递给 forEach 的函数会在数组的每个元素像上执行一次，元素作为参数传递给该函数。未赋值的值不会在 forEach 循环迭代。</p>
...
<p><code>sort()</code> 也可以带一个回调函数来决定怎么比较数组元素。这个回调函数比较两个值，并返回 3 个值中的一个：</p>
...
<p>{{jsxref("Array.map", "map(callback[, thisObject])")}} 在数组的每个单元项上执行 callback 函数，并把返回包含回调函数返回值的新数组（译者注：也就是遍历数组，并通过 callback 对数组元素进行操作，并将所有操作结果放入数组中并返回该数组）。</p>
...
<p>{{jsxref("Array.filter", "filter(callback[, thisObject])")}} 返回一个包含所有在回调函数上返回为 true 的元素的新数组（译者注：callback 在这里担任的是过滤器的角色，当元素符合条件，过滤器就返回 true，而 filter 则会返回所有符合过滤条件的元素）。</p>
...
<p>{{jsxref("Array.every", "every(callback[, thisObject])")}} 当数组中每一个元素在 callback 上被返回 true 时就返回 true（译者注：同上，every 其实类似 filter，只不过它的功能是判断是不是数组中的所有元素都符合条件，并且返回的是布尔值）。</p>
...
<p>{{jsxref("Array.some", "some(callback[, thisObject])")}} 只要数组中有一项在 callback 上被返回 true，就返回 true（译者注：同上，类似 every，不过前者要求都符合筛选条件才返回 true，后者只要有符合条件的就返回 true）。</p>
...
<p>实际上在调用回调函数时传入了 3 个参数。第一个是当前元素项的值，第二个是它在数组中的索引，第三个是数组本身的一个引用。 JavaScript 函数忽略任何没有在参数列表中命名的参数，因此提供一个只有一个参数的回调函数是安全的，例如 <code>alert</code> 。</p>
...
<p>下面的推导式创建一个数字数组并且创建一个新的数组，数组的每个元素都是原来数值的两倍（译者注：这种形式类似于 Python 的列表推导式）。</p>
...
<p><code>map()</code> <code>和 filter()</code> 类型的操作可以被组合（等效）为单个数组推导式。这里就有一个过滤出偶数，创建一个它的倍数数组的例子：</p>
...
<p>数组推导式隐含了块作用域。新的变量 (如例子中的 i) 类似于是采用 <a href="/en-US/docs/Web/JavaScript/Reference/Statements/let"><code>let</code></a>声明的。这意味着他们不能在推导式以外访问。</p>

<p>数组推导式的输入不一定必须是数组; <a href="/en-US/docs/Web/JavaScript/Guide/Iterators_and_Generators">迭代器和生成器</a> 也是可以的。</p>
...
<p><a href="/en-US/docs/Web/JavaScript/Typed_arrays">JavaScript typed arrays</a> 是类数组对象（array-like object），其提供访问原始二进制数据的机制。 就像你知道的那样，{{jsxref("Array")}} 对象动态增长和收缩，可以有任何 JavaScript 值。但对于类型化数组，JavaScript 引擎执行优化使得这些数组访问速度快速。 随着 Web 应用程序变得越来越强大，添加音频和视频处理等功能、可以使用 <a href="/en-US/docs/WebSockets">WebSockets</a> 、使用原始数据， 这都需要访问原始的二进制数据，所以专门的优化将有助于 JavaScript 代码能够快速和容易地操纵原始二进制数据类型的数组。</p>

<h3 id="缓冲区和视图：类型化的数组结构">缓冲区和视图：类型化的数组结构</h3>

<p>为了实现最大的灵活性和效率，JavaScript 类型数组被分解为缓冲 (Buffer) 和视图 (views)。缓冲 (由{{jsxref("ArrayBuffer")}} 实现) 是代表数据块的对象，它没有格式可言，并没有提供任何机制来访问其内容。为了访问包含在缓冲区中的内存，您需要使用视图。视图提供了一个上下文，即数据类型、起始偏移量和元素数，这些元素将数据转换为实际类型数组。</p>

<p><img alt="Typed arrays in an ArrayBuffer" src="typed_arrays.png"></p>

<h3 id="ArrayBuffer">ArrayBuffer</h3>

<p> {{jsxref("ArrayBuffer")}}是一种数据类型，用于表示一个通用的、固定长度的二进制数据缓冲区。你不能直接操纵一个 ArrayBuffer 中的内容；你需要创建一个数组类型视图或{{jsxref("DataView")}}来代表特定格式的缓冲区，并从而实现读写缓冲区的内容。</p>

<h3 id="类型数组视图Typed_array_views">类型数组视图 (Typed array views)</h3>

<p>类型数组视图具有自描述性的名字，并且提供数据类型信息，例如<code>Int8</code>, <code>Uint32</code>, <code>Float64 等等。</code>如一个特定类型数组视图<code>Uint8ClampedArray</code>. 它意味着数据元素只包含 0 到 255 的整数值。它通常用于<a href="/en-US/docs/Web/API/ImageData">Canvas 数据处理</a>,例如。</p>
