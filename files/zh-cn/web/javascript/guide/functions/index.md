...
因此，闭包可以包含多个作用域；它们递归地包含了所有包含它的函数作用域。这个称之为*作用域链*。（稍后解释它被称为“链接”的原因。）
...
在下面的示例中，`multiply` 函数使用*剩余参数*收集从第二个到最后参数。然后，该函数将它们与第一个参数相乘。
...
在箭头函数出现之前，每一个新函数都定义了自己的 [`this`](/zh-CN/docs/Web/JavaScript/Reference/Operators/this) 值（在构造函数中是一个新的对象；在[严格模式](/zh-CN/docs/Web/JavaScript/Reference/Strict_mode)下是 undefined；在作为“对象方法”调用的函数中指向这个对象；等等）。以面向对象的编程风格，这样着实有点恼人。
