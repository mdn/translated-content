...
<table class="standard-table">
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>
    <p><strong>捕获组: </strong>匹配 x 并记住匹配项。例如，/(foo)/匹配并记住“foo bar”中的“foo” </p>
    ...
    <p>match() 不会返回组，如果/…但是，您仍然可以使用 String.matchAll() 来获取所有匹配项。</p>
   </td>
  </tr>
  <tr>
   <td><code>\<em>n</em></code></td>
   <td>
    <p>其中 n 是一个正整数。对正则表达式中与 n 括号匹配的最后一个子字符串的反向引用 (计算左括号)。例如，<code>/apple(,)\sorange\1/</code> 匹配 “apple，orange，cherry，peach” 中的 "apple，orange，"， 其中 <code>\1</code> 引用了 之前使用 <code>（）</code> 捕获的 <code>，</code></p>
   </td>
  </tr>
  <tr>
   <td><code>(?&lt;Name&gt;x)</code></td>
   <td>
    <p><strong>具名捕获组: </strong>匹配"x"并将其存储在返回的匹配项的 groups 属性中，该属性位于<code>&lt;Name&gt;</code>指定的名称下。尖括号 (<code>&lt;</code> 和 <code>&gt;</code>) 用于组名。</p>
