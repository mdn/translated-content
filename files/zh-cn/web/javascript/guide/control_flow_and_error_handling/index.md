...
> [!NOTE]
> 用 [`var`](/en-US/docs/Web/JavaScript/Reference/Statements/var) 声明的变量不是块级作用域的，而是受限于函数或脚本，以及设置它们的效果会持续到块本身之外。例如：
>
> ```js
> var x = 1;
> {
...
然而，在极少数情况下，你会发现自己想要这么做，[`while`](/en-US/docs/Web/JavaScript/Reference/Statements/while) 文档的[使用赋值作为条件](/en-US/docs/Web/JavaScript/Reference/Statements/while#使用赋值作为条件)小节是一篇你应该了解并遵循的通用的最佳实践语法指南。

#### 假值

下面这些值求值为 `false`（也叫做[假](/en-US/docs/Glossary/Falsy)值）：
...
`switch` 语句允许程序求一个表达式的值并且尝试将表达式的值和 `case` 标签进行匹配。如果匹配成功，这个程序执行相关的语句。
...
- 如果没有匹配的标签，程序会去找可选的 `default` 从句：
  - 如果 `default` 从句找到了，控制权转移到该从句，执行相关的语句。
  - 如果没有找到 `default` 从句，程序会继续执行 `switch` 语句后面的语句。- （`default` 语句通常是最后一个从句，当然这不是必须的。）
...
- [ECMAScript 异常](/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error#错误类型)
...
换句话说，你希望 `try` 块执行成功——但如果它没有执行成功，那么你希望将控制转移到 `catch` 块。如果 `try` 块中的语句（或者在 `try` 块中调用的函数里）一旦抛出了异常，那么控制_立马_转移到 `catch` 块。如果 `try` 块没有抛出异常，`catch` 块就会被跳过。`finally` 块总会紧跟在 `try` 和 `catch` 块之后执行，但会在 `try...catch` 语句后面的语句之前执行。

下列示例使用了 `try...catch` 语句。示例调用的函数是根据传递的值从数组中获取一个月份名称。如果该值与月份数值（`1`-`12`）不相符，会抛出带有 `"InvalidMonthNo"` 值的异常，然后在 `catch` 块的语句中设 `monthName` 变量为 `"unknown"`。

```js-nolint
function getMonthName(mo) {
  mo--; // 将月份调整为数组索引（这样的话，0 = Jan, 11 = Dec）
  const months = [
  ...
  if (months[mo]) {
    ...
    throw new Error("InvalidMonthNo"); // 在这使用 throw 关键字
  }
}
...
`catch` 块指定的标识符（上述语句中的 `exception`）会存储 `throw` 语句指定的值。你可以用这个标识符来获取抛出的异常的信息。在进入 `catch` 块时 JavaScript 会创建这个标识符。标识符只存在于 `catch` 块的存续期间里；当 `catch` 块执行完成时，标识符不再可用。
...
> [!NOTE]
> 在 `catch` 中将错误输出到控制台时，建议使用 `console.error()` 而不是 `console.log()` 进行调试。它会将消息格式化为错误，并将其添加到页面生成的错误消息列表。

#### finally 块

`finally` 块包含了在 `try` 和 `catch` 块完成*后*要执行的语句。此外，`finally` 块在 `try…catch…finally` 语句后面的代码*之前*执行。
...
下列的示例打开一个文件，然后使用这个文件。（服务器端 JavaScript 允许你访问文件。）如果在文件打开时抛出异常，`finally` 块会在脚本失败之前关闭文件。在这使用 `finally` 能*确保*文件永远不会是打开状态，即使发生了错误。
...
如果 `finally` 块返回一个值，该值会是整个 `try…catch…finally` 流程的返回值，不管在 `try` 和 `catch` 块中的语句返回了什么：
...
2. 包围的 `try...catch` 语句 `catch` 块会被检查是否能处理该异常。
...
try {
  ...
  console.error(e.message); // “The message”，或者一个 JavaScript 错误消息
}
```
