...
<p>因此，近年来，有必要开始考虑提供一种<u>将 JavaScript 程序拆分为可按需导入的单独模块</u>的机制。Node.js 已经提供这个能力很长时间了，还有很多的 JavaScript 库和框架已经开始了模块的使用（例如，<a href="https://en.wikipedia.org/wiki/CommonJS">CommonJS</a> 和基于 <a href="https://github.com/amdjs/amdjs-api/blob/master/AMD.md">AMD</a> 的其他模块系统 如 <a href="https://requirejs.org/">RequireJS</a>， 以及最新的 <a href="https://webpack.github.io/">Webpack</a> 和 <a href="https://babeljs.io/">Babel</a>）。</p>

<p>好消息是，最新的浏览器开始原生支持模块功能了，这是本文要重点讲述的。这会是一个好事情 —- 浏览器能够最优化加载模块，使它比使用库更有效率：使用库通常需要做额外的客户端处理。</p>

<h2 id="浏览器支持">浏览器支持</h2>

<p>使用 JavaScript 模块依赖于 <code>import</code> 和 <code>export</code>，浏览器兼容性如下（绿色方框中的数字对应相应平台上支持该功能的发布版本）：</p>
...
<ul>
 <li><code>canvas.mjs</code> — 包含与设置画布相关的功能：
  ...
  <ul>
   <li><code>create()</code> — 在指定 ID 的包装器 {{htmlelement("div")}} 内创建指定 <code>width</code> 和 <code>height</code> 的画布，该 ID 本身附加在指定的父元素内。返回包含画布的 2D 上下文和包装器 ID 的对象。</li>
...
<div class="note">
<p><strong>备注：</strong>在原生 JavaScript 模块中，扩展名 <code>.mjs</code> 非常重要，因为使用 MIME-type 为 <code>javascript/esm</code> 来导入文件（其他的 JavaScript 兼容 MIME-type 像 <code>application/javascript</code> 也可以），它避免了严格的 MIME 类型检查错误，像 "The server responded with a non-JavaScript MIME type"。除此之外，<code>.mjs</code> 的扩展名很明了（比如这个就是一个模块，而不是一个传统 JavaScript 文件），还能够和其他工具互相适用。看这个 <a href="https://v8.dev/features/modules#mjs">Google's note for further details</a>。</p>
</div>

<h2 id=".mjs_与_.js"><code>.mjs</code> 与 <code>.js</code></h2>

<p>纵观此文，我们使用 <code>.js</code> 扩展名的模块文件，但在其它一些文章中，你可能会看到 <code>.mjs</code> 扩展名的使用。<a href="https://v8.dev/features/modules#mjs">V8 推荐了这样的做法</a>，比如有下列理由：</p>
...
<p>使用 <code><a href="/en-US/docs/Web/JavaScript/Reference/Statements/import">import</a></code> 语句，然后你被花括号包围的用逗号分隔的你想导入的功能列表，然后是关键字 from，然后是模块文件的路径。模块文件的路径是相对于站点根目录的相对路径，对于我们的 <code>basic-modules</code> 应该是 <code>/js-examples/modules/basic-modules</code>。</p>
...
<h2 id="应用模块到你的HTML">应用模块到你的 HTML</h2>

<p>现在我们只需要将 main.mjs 模块应用到我们的 HTML 页面。 这与我们将常规脚本应用于页面的方式非常相似，但有一些显着的差异。</p>
...
<p>还有一种导出类型叫做 <strong>default export</strong> —- 这样可以很容易地使模块提供默认功能，并且还可以帮助 JavaScript 模块与现有的 CommonJS 和 AMD 模块系统进行互操作（正如 <a href="https://hacks.mozilla.org/2015/08/es6-in-depth-modules/">ES6 In Depth: Modules</a> by Jason Orendorff 的模块中所解释的那样；搜索“默认导出”）。</p>
...
<div class="note">
<p><strong>备注：</strong>重命名导出项的 as 语法在下面的 <a href="#重命名导出与导入">重命名导出与导入</a> 部分中进行了说明。</p>
</div>

<h2 id="避免命名冲突">避免命名冲突</h2>

<p>到目前为止，我们的 canvas 图形绘制模块看起来工作的很好。但是如果我们添加一个绘制其他形状的比如圆形或者矩形的模块会发生什么？这些形状可能会有相关的函数比如 <code>draw()</code>、<code>reportArea()</code>，等等；如果我们用相同的名字导入不同的函数到顶级模块文件中，我们会收到冲突和错误。</p>
...
<p>浏览器中可用的 JavaScript 模块功能的最新部分是动态模块加载。这允许您仅在需要时动态加载模块，而不必预先加载所有模块。这有一些明显的性能优势；让我们继续阅读，看看它是如何工作的。</p>
...
<p>请注意，由于 promise 履行会返回一个模块对象，因此该类成为对象的子特征，因此我们现在需要使用 <code>Module</code> 访问构造函数。在它之前，例如 <code>Module.Square( ... )</code>。</p>
