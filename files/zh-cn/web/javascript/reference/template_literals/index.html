...
<p>模板字面量 是允许嵌入表达式的字符串字面量。你可以使用多行字符串和字符串插值功能。它们在 ES2015 规范的先前版本中被称为“模板字符串”。</p>
...
<p>正如下面例子所展示的，标签函数并不一定需要返回一个字符串。</p>
...
<ul>
 <li>Unicode 字符以"\u"开头，例如<code>\u00A9</code></li>
 <li>Unicode 码位用"\u{}"表示，例如<code>\u{2F804}</code></li>
 <li>十六进制以"\x"开头，例如<code>\xA9</code></li>
 <li>八进制以"\"和数字开头，例如<code>\251</code></li>
</ul>

<p>这表示类似下面这种带标签的模版是有问题的，因为对于每一个 ECMAScript 语法，解析器都会去查找有效的转义序列，但是只能得到这是一个形式错误的语法：</p>
...
<h4 id="ES2018关于非法转义序列的修订">ES2018 关于非法转义序列的修订</h4>

<p>带标签的模版字符串应该允许嵌套支持常见转义序列的语言（例如<a href="https://en.wikipedia.org/wiki/Domain-specific_language">DSLs</a>、<a href="https://en.wikipedia.org/wiki/LaTeX">LaTeX</a>）。ECMAScript 提议<a href="https://tc39.github.io/proposal-template-literal-revision/">模版字面量修订</a>(第 4 阶段，将要集成到 ECMAScript 2018 标准) 移除对 ECMAScript 在带标签的模版字符串中转义序列的语法限制。</p>
