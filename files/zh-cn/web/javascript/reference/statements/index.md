...
- {{jsxref("Statements/break", "break")}}
  - : 终止当前的循环，switch 或标签语句，使程序跳到下一个语句执行。
...
- {{jsxref("Statements/while", "while")}}
  - : 创建一个执行指定语句的循环，直到测试条件的求值为假。先对条件进行判断，再执行语句。
...
- {{jsxref("Statements/Expression_statement", "Expression statement", "", 1)}}
  - : 表达式语句，计算表达式的值，但不使用其结果。它允许表达式执行副作用，例如执行函数或更新变量。
...
> **备注：** 有一种遗留语法允许[函数声明具有标签](/zh-CN/docs/Web/JavaScript/Reference/Statements/label#标记函数声明)，但它只是为了与网络现实兼容而进行了标准化。
