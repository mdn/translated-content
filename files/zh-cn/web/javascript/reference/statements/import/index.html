...
<p><code>name</code> 参数是“导入模块对象”的名称，它将用一种名称空间来引用导入模块的接口。export 参数指定单个的命名导出，而 <code>import * as name</code> 语法导入所有导出接口，即导入模块整体。以下示例阐明该语法。</p>
...
<p>引入模块可能有一个 <code>default</code> {{jsxref("Statements/export", "export")}}（无论它是对象，函数，类等）可用。然后可以使用 <code>import</code> 语句来导入这样的默认接口。</p>
...
<h3 id="动态import">动态 import</h3>

<p>标准用法的 import 导入的模块是静态的，会使所有被导入的模块，在加载时就被编译（无法做到按需编译，降低首页加载速度）。有些场景中，你可能希望根据条件导入模块或者按需导入模块，这时你可以使用动态导入代替静态导入。下面的是你可能会需要动态导入的场景：</p>
...
<p>关键字 import 可以像调用函数一样来动态的导入模块。以这种方式调用，将返回一个 <code>promise</code>。</p>
...
<p>下面的代码将会演示如何从辅助模块导入以协助处理 AJAX JSON 请求。</p>
...
<ul>
 ...
 <li>Axel Rauschmayer's book: <a href="https://exploringjs.com/es6/ch_modules.html">"Exploring JS: Modules"</a></li>
