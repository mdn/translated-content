...
动态模块导入并不在所有执行上下文中都可使用。例如，`import()` 可以在主线程、共享工作线程（shared worker）或专用工作线程（dedicated worker）中使用，但如果在 [service worker](/zh-CN/docs/Web/API/Service_Worker_API) 或 [worklet](/zh-CN/docs/Web/API/Worklet) 中调用，则会抛出错误。
