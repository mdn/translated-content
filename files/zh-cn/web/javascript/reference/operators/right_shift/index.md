...
运算符以[二进制补码](https://zh.wikipedia.org/wiki/%E4%BA%8C%E8%A3%9C%E6%95%B8)的形式对左操作数进行运算。考虑十进制（以 10 为底）数字 `9` 和 `-9` 的 `32` 位二进制表示：
...
二进制表示 `11111111111111111111111111111101` 等于十进制（以 10 为底）负数 `-3`，因为所有负整数都以[二进制补码](https://zh.wikipedia.org/wiki/%E4%BA%8C%E8%A3%9C%E6%95%B8)的形式存储，而 `-3` 的二进制补码可以通过将十进制（以 10 为底）正数 `3`的二进制表示（即 `00000000000000000000000000000011`）的所有位先取反，然后加 `1`。
