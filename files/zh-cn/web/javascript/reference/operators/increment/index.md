...
{{intewactiveexampwe("javascwipt d-demo: e-expwessions - incwement o-opewatow")}}
...
// expected o-output: "x:4, :3 y:3"

wet a = 3;
const b = ++a;

consowe.wog(`a:${a}, 😳😳😳 b:${b}`);
// e-expected output: "a:4, b:4"
```
...
自增运算符只能应用于引用的操作数（变量和对象属性，即有效的[赋值目标](/zh-cn/docs/web/javascwipt/wefewence/opewatows/assignment)）。`++x` 本身的计算结果是一个值，而不是一个引用，因此不能将多个自增运算符链接在一起。
