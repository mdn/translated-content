...
<p>新的 <code>locales</code> 和 <code>options</code> 参数能让应用程序定制函数的行为，即指定用来排序的语言。<code>locales</code> 和 <code>options</code> 参数完全取决于实现，在旧的实现中忽略这两个参数。</p>
...
<dl>
 ...
 <dl>
  ...
  <dd>地域匹配算法的使用。可能的值是 <code>"lookup"</code> 和 <code>"best fit"</code>; 默认的值是 <code>"best fit"</code>。更多相关的资料，请参考 <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl#Locale_negotiation">Intl page</a>.</dd>
  ...
  <dd>指定大小写有限排序。可能的值有 <code>"upper"</code>、<code>"lower"</code> 或 <code>"false"</code> (use the locale's default); 默认为 <code>"false"</code>. 这个选项能被通过 <code>options</code> 属性设置或通过 Unicode 扩展。假如两个都被设置了，则 <code>options</code> 优先。实现不用必须支持这个属性。</dd>
 </dl>
</dl>
...
<p><strong>切勿依赖于 -1 或 1 这样特定的返回值。</strong>不同浏览器之间（以及不同浏览器版本之间）返回的正负数的值各有不同，因为 W3C 规范中只要求返回值是正值和负值，而没有规定具体的值。一些浏览器可能返回 -2 或 2 或其他一些负的、正的值。</p>
...
<h3 id="使用_locales_参数">使用 <code>locales</code> 参数</h3>
...
<h3 id="使用_options_参数">使用 <code>options</code> 参数</h3>
