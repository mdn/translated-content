...
<dl>
 ...
 <dd>要被查找的字符串值。如果没有提供确切地提供字符串，<a href="https://tc39.github.io/ecma262/#sec-tostring"><var>searchValue</var> 会被强制设置为 <code>"undefined"</code></a>， 然后在当前字符串中查找这个值。举个例子：<code>'undefined'.indexOf()</code> 将会返回 0，因为 <code>undefined</code> 在位置 0 处被找到，但是 <code>'undefine'.indexOf()</code> 将会返回 -1 ，因为字符串 <code>'undefined'</code> 未被找到。</dd>
...
<p>从前面一个例子可以看出，被查找的值是空值时，JavaScript 将直接返回指定的索引值。从后面一个例子可以看出，被查找的值是空值时，JavaScript 将直接返回字符串的长度。</p>
...
<h3 id="Example_Using_indexOf_and_lastIndexOf">使用<code>indexOf()</code> 和 <code>lastIndexOf()</code></h3>
