...
一个新的[可迭代迭代器对象](/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Iterator)，它将字符串值中的 Unicode 码位以单独的字符串形式产出。
...
请注意，你很少需要直接调用该方法。`@@iterator` 方法的存在令字符串[可迭代的](/zh-CN/docs/Web/JavaScript/Reference/Iteration_protocols#可迭代协议)，而像 `for...of` 循环这样的迭代语法会自动调用该方法以获取迭代器进行循环。
...
你仍然可以手动调用返回的迭代器对象的 `next()` 方法，以实现最大程度上对迭代过程的控制。
