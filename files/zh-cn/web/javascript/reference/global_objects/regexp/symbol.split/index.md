...
- `limit` {{optional_inline}}
  - : 一个指定要查找的拆分次数的上限的整数。`[Symbol.split]()` 方法仍然会对 `this` 正则表达式模式（或上述语法中的 `regexp`）的每个匹配项进行拆分，直到拆分的元素数量达到 `limit` 限制，或者字符串中不再匹配该模式。
...
- 如果当前匹配是空字符串，或者正则表达式（由于启用了 sticky）在当前位置无法匹配，那么 `lastIndex` 仍会被推进——如果正则是 [Unicode 感知](/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/RegExp/unicode#unicode_感知模式)的，则推进一个 Unicode 码位；否则，推进一个 UTF-16 代码单元。
