...
<dl>
 <dt>radix</dt>
 <dd>指定要用于数字到字符串的转换的基数 (从 2 到 36)。如果未指定 <code>radix</code> 参数，则默认值为 10。</dd>
</dl>
...
<p>如果转换的基数大于 10，则会使用字母来表示大于 9 的数字，比如基数为 16 的情况，则使用 a 到 f 的字母来表示 10 到 15。</p>

<p>如果基数没有指定，则使用 10。</p>

<p>如果对象是负数，则会保留负号。即使 radix 是 2 时也是如此：返回的字符串包含一个负号（-）前缀和正数的二进制表示，<strong>不是</strong> 数值的二进制补码。</p>
