...
<p>一个用幂的形式 (科学记数法) 来表示{{jsxref("Number")}} 对象的字符串。小数点后以 fractionDigits 提供的值来四舍五入。如果 <code>fractionDigits</code> 参数被忽略了，小数点后的将尽可能用最多的位数来表示该数值。</p>
...
<dl>
 ...
 <dd>如果 <em>fractionDigits</em> 太小或太大将会抛出该错误。介于 0 和 20（包括 20）之间的值不会引起 <code>RangeError</code> 。 执行环境也可以支持更大或更小范围。</dd>
