...
<p>JavaScript 的<code>Number</code>类型为<a href="https://en.wikipedia.org/wiki/Floating-point_arithmetic">双精度 IEEE 754 64 位浮点</a>类型。</p>

<p>最近出了 stage3{{jsxref("BigInt")}} 任意精度数字类型，已经进入 stage3 规范</p>
...
<dl>
 ...
 <dt>{{jsxref("Number.toInteger()")}} {{Deprecated_Inline}}</dt>
...
<p>在解析序列化的 JSON 时，如果 JSON 解析器将它们强制转换为 Number 类型，那么超出此范围的整数值可能会被破坏。在工作中使用{{jsxref("String")}} 类型代替，是一个可行的解决方案。</p>

<h3 id="Example_Using_Number_to_convert_a_Date_object">使用 <code>Number</code> 转换 <code>Date</code> 对象</h3>
