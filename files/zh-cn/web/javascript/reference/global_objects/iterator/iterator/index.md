...
> **备注：** `Iterator()` 只能通过 `new`(/zh-CN/docs/Web/JavaScript/Reference/Operators/new) 构造。尝试不带 `new` 调用它会抛出一个 {{jsxref("TypeError")}}。此外，`Iterator()` 实际上不能被构造，通常是通过 `super()` 调用在子类的构造函数中隐式构造的。
...
`Iterator` 代表一个*抽象类*——一个为其子类提供通用实用程序的类，但其本身并不用于实例化。它是所有其他迭代器类的超类，用于创建实现特定迭代算法的子类——即，`Iterator` 的所有子类必须按照[迭代器协议](/zh-CN/docs/Web/JavaScript/Reference/Iteration_protocols#迭代器协议)的要求实现 `next()` 方法。由于 `Iterator` 本身并不提供 `next()` 方法，因此直接构造 `Iterator` 并不合理。
...
遗下示例定义了一个允许迭代的自定义数据结构——`Range`。使一个对象可迭代最简单的方法是提供一个生成器函数的形式的 [`[@@iterator]()`](/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Symbol/iterator) 方法：
...
- 返回的迭代器继承自 {{jsxref("Generator")}}，这意味着对 `Generator.prototype` 的修改将会影响返回的迭代器，这是一个抽象泄漏。
- 返回的迭代器没有继承自自定义原型，如果我们打算为迭代器添加额外的方法，这使其变得更加困难。
...
这种继承模式对于创建许多自定义迭代器很有用。如果你有一个现有的可迭代对象或迭代器对象，它没有继承自 `Iterator`，而你只想在它上调用迭代器辅助方法，那么你可以可以使用 {{jsxref("Iterator.from()")}} 来创建一个一次性的 `Iterator` 实例。
