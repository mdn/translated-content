...
- `end` {{optional_inline}}
  - : 提取终止处的索引（从 `0` 开始）, [转换为整数](/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Number#integer_conversion). `slice()` 会提取到不包括 `end` 的位置。
    ...
    - 如果 `end` 在标准化后小于于或等于 `start`，则不提取任何元素。
...
`slice()` 方法是一个[复制方法](/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Array#copying_methods_and_mutating_methods)。它不会改变 `this`，而是返回一个[浅拷贝](/zh-CN/docs/Glossary/Shallow_copy)，其中包含了原始数组的一部分元素。
...
`slice()` 方法会读取 `this` 对象的 `length` 属性，然后从 `start` 到 `end` 读取整数索引属性，并将它们定义在一个新创建的数组中。
