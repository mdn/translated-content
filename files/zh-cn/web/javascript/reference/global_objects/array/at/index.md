...
这不代表使用方括号符号有什么问题。例如，`array[0]` 将返回第一个项目。然而，对于后面的项目的读取，你需要用到 {{jsxref('Array.prototype.length','array.length')}}，例如，读取最后一个项目，你需要使用`array[array-1]`，对比以上方法读取最后一个项目，你可以调用 `array.at(-1)`。[(参见以下示例)](#examples)
