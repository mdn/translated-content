...
`flatMap` 方法可以用作在 `map` 方法中添加和删除元素（修改元素数量）的方法。换句话说，它允许你将*一项处理为多项（或相反）*（通过单独处理每个输入项），而不总是*一对一*的。在这个意义上，它的作用类似于 [filter](/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Array/filter) 的相反操作。如果要保留该项，则返回一个包含该项的单元素数组，如果要添加元素，则返回一个包含多个元素的数组，如果要删除该项，则返回一个空数组。
