...
{{jsxref("Array.prototype.push()")}} 有着和 `unshift()` 相同的行为，但是其应用到数组的末尾。

请注意，如果多个元素作为参数传递，它们将被插入到对象开头的块中，与它们作为参数传递的顺序完全相同。因此，调用**一次** `unshift()` 方法传递 `n` 个参数，或者 **1** 个参数调用 `n` 次（例如使用循环），不会产生相同的结果。
