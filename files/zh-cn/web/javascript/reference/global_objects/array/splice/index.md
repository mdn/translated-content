...
- `deleteCount` {{optional_inline}}
  ...
  - : 一个整数，表示数组中要从 `start` 开始删除的元素数量。
    ...
    如果省略了 `deleteCount`，或者其值大于或等于由 `start` 指定的位置到数组末尾的元素数量，那么从 `start` 到数组末尾的所有元素将被删除。但是，如果你想要传递任何 `itemN` 参数，则应将 `deleteCount` 设置为 `Infinity`，以删除 `start` 之后的所有元素，因为显式的 `undefined` 会[转换](/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Number#整数转换)为 `0`。
