...
<p><strong><code>Int8Array</code></strong> 类型数组表示二进制补码 8 位有符号整数的数组。内容初始化为 0。一旦建立，你可以使用对象的方法引用数组中的元素，或使用标准数组索引语法 ( 即，使用括号注释)。</p>
...
<dl>
 ...
 <dd>返回数组中每个元素的大小。在 <code>Int8Array</code> 中这个值为 1。</dd>
 <dt>Int8Array.length</dt>
 <dd>此属性为固定值属性，值为 3。查看 {{jsxref("TypedArray.prototype.length", "Int8Array.prototype.length")}} 获得获取数组内元素个数方法。</dd>
 <dt>{{jsxref("TypedArray.name", "Int8Array.name")}}</dt>
 <dd>返回构造器方法名称。在 Int8Array 类型中此值为 "Int8Array"。</dd>
...
<dl>
 ...
 <dd>从类数组对象或迭代器生成 int8Array 数组对象。参照 {{jsxref("Array.from()")}}</dd>
 <dt>{{jsxref("TypedArray.of", "Int8Array.of()")}}</dt>
 <dd>以多个参数构造 Int8Array 对象，参照 {{jsxref("Array.of()")}}</dd>
</dl>
...
<dl>
 ...
 <dd>这个方法会返回对象的构造原型。默认为<code>Int8Array</code> 构造函数。</dd>
...
<dl>
 ...
 <dd>返回一个以给定的初始结束位置裁剪的 Int8Array 数组。</dd>
 ...
 <dt>{{jsxref("TypedArray.@@iterator", "Int8Array.prototype[@@iterator]()")}}</dt>
  <dd>此方法返回一个带有数组内所有元素的迭代器对象，同 <code>Int8Array.prototype.values</code></dd>
</dl>
...
<p>自 ECMAScript 2015 (ES6) 施行，<code>Int8Array</code> 需要使用 {{jsxref("Operators/new", "new")}} 构造。从当前版本开始，不加 new 而便调用 <code>Int8Array</code> 构造器方法，将报出 {{jsxref("TypeError")}} 错误。</p>
