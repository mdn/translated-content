...
在松散模式下，可以在函数中返回 `this` 来获取全局对象，但是在严格模式和模块环境下，`this` 会返回 `undefined`。你也可以使用 `Function('return this')()`，但那些禁用{{jsxref("Global_Objects/eval", "eval()")}}的环境，如在浏览器中的[CSP](/zh-CN/docs/Glossary/CSP)，不允许这样使用{{jsxref("Function")}}。
