...
<p><strong><code>Intl.NumberFormat</code></strong> 是对语言敏感的格式化数字类的构造器类。</p>
...
<dl>
 ...
 <dd>
 <p>可选。缩写语言代码（BCP 47 language tag，例如: cmn-Hans-CN）的字符串或者这些字符串组成的数组. 关于参数 locales 的一般形式和解释请参见<a href="https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Intl#Locale_identification_and_negotiation">Intl page</a>. 下面的这些 Unicode 扩展键也是被允许的：</p>

 <p>译者注：下面扩展的使用方式是 language[-scripts][-region]-u-nu-*,例如:zh-u-nu-hanidec(表示中文十进制数字) </p>
 ...
 <dl>
  ...
  <dd><code>unit</code> 格式中使用的单位，可能的值为在 <a href="http://unicode.org/reports/tr35/tr35-general.html#Unit_Elements">UTS #35, Part 2, Section 6</a> 定义的核心单元标识符。已从<a href="https://github.com/unicode-org/cldr/blob/master/common/validity/unit.xml">完整列表</a>中选择了一个<a href="https://tc39.es/proposal-unified-intl-numberformat/section6/locales-currencies-tz_proposed_out.html#sec-issanctionedsimpleunitidentifier">单位子集</a>以用于 ECMAScript。可以将成对的简单单位与 <code>“ -per-”</code> 连接以组成一个复合单位。没有默认值；如果 <code>style</code> 为 <code>“unit”</code>，必须提供<code>unit</code> 属性。</dd>
 ...
 <dl>
  ...
  <dd>使用的整数数字的最小数目。可能的值是从 1 到 21，默认值是 1.</dd>
  ...
  <dd>使用的小数位数的最大数目。可能的值是从 0 到 20；纯数字格式的默认值是<code>minimumfractiondigits</code> 和 3 中大的那一个；货币格式默认值是<code>minimumfractiondigits</code> 和 <a href="http://www.currency-iso.org/en/home/tables/table-a1.html">ISO 4217 currency code list</a> 中大的那一个 (如果列表中没有提供则值为 2)；百分比格式默认值是 <code>minimumfractiondigits</code> 和 0 中大的那一个。</dd>
  <dt><code>minimumSignificantDigits</code></dt>
  <dd>使用的有效数字的最小数目。可能的值是从 1 到 21；默认值是 1。</dd>
  <dt><code>maximumSignificantDigits</code></dt>
  <dd>使用的有效数字的最大数量。可能的值是从 1 到 21；默认是 21.</dd>
...
<dl>
 ...
 <dd>返回一个数组包含的那些被提供的可以被支持的 locales，运行时的默认的 locale 不会出现在该数组中。</dd>
</dl>
...
<p>在不指定 locale 的基本用法中，一个使用默认 locale 和默认 options 的字符串被返回。</p>
...
// 德语使用逗号（<code>,</code>）作为小数点，使用句号（<code>.</code>）作为千位分隔符
