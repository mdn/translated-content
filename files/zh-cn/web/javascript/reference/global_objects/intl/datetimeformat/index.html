...
<dl>
 ...
 <p>译者注：下面这两种扩展的使用方式是 language[-scripts][-region]-u-nu-* 和 language[-scripts][-region]-u-ca-* ,例如:zh-u-nu-hanidec(表示中文十进制数字) 和 zh-u-ca-chinese(表示中国日历，比如壬辰年冬月 8 日) ,也可以 nu 和 ca 组合使用 比如 使用 zh-u-ca-chinese-nu-hanidec 格式化 Date.now() 的返回值类似于"丙申年冬月九日"</p>
 ...
 <dl>
  ...
  <dd>是否使用 12 小时时间制 (而不是 24 小时的时间). 可能的值是<code>true</code> 或 <code>false</code>; 默认值是根据 locale 来自动决定的 (译者注：中国地区的默认值为 true).</dd>
  <dt><code>formatMatcher</code></dt>
  <dd><code>format 的匹配算法。可能的值有"basic"和"best fit";默认值是"best fit".有关此属性使用的信息，参见以下段落。</code>日期时间插件被格式化输出时可以使用的属性集合描述。实现需要支持是以下子集中的其中一个 (译者注：当 weekday,year 等这些属性一个也不使用的时候，在 cmn-Hans-CN 中相当于使用集合 year, month, day, hour, minute, second 并且它们的值都是 numeric)：</dd>
 </dl>
 ...
 <p>实现可能支持其他的子集，并通过对所有可用的子集对比找到最匹配的子集。通过 formatMatcher 属性可以设置两种算法用于对比和选择子集:  <a href="http://www.ecma-international.org/ecma-402/1.0/#BasicFormatMatcher">完全匹配<code>"basic"算法</code> </a> 和 一种依赖于“best fit”算法的实现。</p>
 ...
 <p>每个日期时间组件属性的默认值都是 {{jsxref("undefined")}}，但是若所有的组件属性都是 {{jsxref("undefined")}}，那么 <code>year</code>、<code>month</code> 和 <code>day</code> 的值就都被认为是 <code>numeric</code>。</p>
 </dd>
</dl>
...
<p>不指定 locale 时，DateTimeFormat 使用默认的 locale 和 默认的 options</p>
...
<p>这个例子显示了本地化日期和时间格式的一些变化。为了获得用于您的应用程序的用户界面的语言格式，请确保使用 locales 参数确保指定语言（可能还有一些回退语言）：</p>
...
<p>可以使用 <code>options</code> 参数来自定义日期时间格式化方法返回的字符串。</p>
