...
- `options`
  ...
  - : 可选的。 拥有下面所列属性中任意几个或全部的对象：
    ...
    - `localeMatcher`
      指定要使用的本地匹配算法。可选的值有`"lookup"` 和 `"best fit"`；默认情况下使用`"best fit"`。该参数的更多信息，请参考[`Intl` page](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Intl#Locale_negotiation).
    - `type`
