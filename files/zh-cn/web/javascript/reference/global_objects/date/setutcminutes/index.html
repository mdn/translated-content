...
<dl>
 ...
 <dd>必填，表示要设置的分钟数，是一个介于 0 和 59 之间的整数。</dd>
 <dt><code>secondsValue</code></dt>
 <dd>可选参数，表示要设置的秒数，同样也是一个介于 0 和 59 之间的整数，如果你传入了这个参数，那么你就必须要传入上一个参数（<strong><code>minutesValue</code></strong>）。</dd>
 <dt><code>msValue</code></dt>
 <dd>可选参数，表示要设置的毫秒数，这是一个介于 0 和 999 之间的数字，如果你传入了这个参数，那么你就必须要传入前面两个参数（<strong><code>minutesValue</code></strong>和<strong><code>secondsValue</code></strong>）。</dd>
</dl>

<h3 id="返回值">返回值</h3>

<p>返回从 UTC 时间 1970 年 1 月 1 日 0 时 0 分 0 秒至设置后的时间的毫秒数。</p>
...
<p>如果你传入的参数值在上文所述范围之外的话，<code><strong>setUTCMinutes()</strong></code>方法会尝试修改日期对象中的其他信息，比如说你为<code><strong>secondsValue</strong></code>这个参数传入了 100（译者注：规定范围是 [0, 59]），那么第一个参数（<code><strong>minutesValue</strong></code>）就会被加 1，而秒数则变成了 40。</p>
