...
<p>{{jsxref("Global_Objects/Date", "Date")}} 对象实例引用一个具体的时间点。调用 {{jsxref("Date.toString", "toString")}} 方法会以美式英语和人类易读的形式返回日期对象的格式化字符串。在 <a href="/en-US/docs/SpiderMonkey">SpiderMonkey</a> 里，该字符串由日期部分（年月日）和其后的时间部分（时分秒及时区）组成。有时需要获取日期部分的字符串，这可以由 <code>toDateString</code> 方法完成。</p>
...
<h3 id="Example:_A_basic_usage_of_toDateString">例子：<code>toDateString</code> 方法的简单使用</h3>
