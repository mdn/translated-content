...
<p><code><strong>Date.parse()</strong></code> 方法解析一个表示某个日期的字符串，并返回从 1970-1-1 00:00:00 UTC 到该日期对象（该日期对象的 UTC 时间）的毫秒数，如果该字符串无法识别，或者一些情况下，包含了不合法的日期数值（如：2015-02-31），则返回值为 NaN。</p>

<p>不推荐在 ES5 之前使用 Date.parse 方法，因为字符串的解析完全取决于实现。直到至今，不同宿主在如何解析日期字符串上仍存在许多差异，因此最好还是手动解析日期字符串（在需要适应不同格式时库能起到很大帮助）。    </p>
...
<dl>
 ...
 <dd>一个符合 <a href="http://tools.ietf.org/html/rfc2822#page-14">RFC2822</a> 或 ISO 8601 日期格式的字符串（其他格式也许也支持，但结果可能与预期不符）。</dd>
</dl>

<h3 id="返回值">返回值</h3>

<p>一个表示从 1970-1-1 00:00:00 UTC 到给定日期字符串所表示时间的毫秒数的数值。如果参数不能解析为一个有效的日期，则返回{{jsxref("NaN")}}。</p>

<h2 id="Description">描述</h2>

<p><code>parse</code> 方法接受一个日期字符串（例如 "<code>Dec 25, 1995</code>"），并返回从 1970-1-1 00:00:00 UTC 到该日期字符串所表示日期的毫秒数。该方法在基于字符串值设置日期值时很有用，例如结合使用{{jsxref("Global_Objects/Date/setTime", "setTime()")}} 方法和 {{jsxref("Global_Objects/Date", "Date()")}} 构造函数。</p>

<p><code>parse</code> 方法接受一个表示时间的字符串，返回相应的时间值。该方法可以接受符合 RFC2822 / IETF 日期语法 (<a href="http://tools.ietf.org/html/rfc2822#page-14">RFC2822 Section 3.3</a>) 的字符串，如 "<code>Mon, 25 Dec 1995 13:30:00 GMT</code>"。该方法能够理解美国大陆时区的缩写，但是为了更通用，应该使用时区偏移，如 "<code>Mon, 25 Dec 1995 13:30:00 +0430</code>"（格林威治的子午线向东偏移 4 小时 30 分钟）。如果没有指定时区，默认使用本地时区。</p>

<p>GMT 和 UTC 被看作相等。 如果 <a href="http://tools.ietf.org/html/rfc2822#page-14">RFC2822 Section 3.3</a> 格式中不包含时区信息时，会以本地时区来解析日期字符串。</p>

<p>由于在解析日期字符串时存在偏差会导致结果不一致，因此推荐始终手动解析日期字符串，特别是不同的 ECMAScript 实现会把诸如“2015-10-12 12:00:00”的字符串解析为 NaN，UTC 或者本地时间。</p>

<h3 id="ECMAScript_5_ISO-8601_日期格式支持">ECMAScript 5 ISO-8601 日期格式支持</h3>

<p>另外，日期时间字符串也可以使用 <a href="http://www.w3.org/TR/NOTE-datetime">ISO 8601</a> 格式。例如，"<code>2011-10-10</code>"（仅日期）或 "<code>2011-10-10T14:48:00</code>"（日期和时间）能够作为参数被传递和解析。 如果参数字符串只包含日期格式，那么将会使用 UTC 时区来解析该参数。而如果是<a href="http://www.w3.org/TR/NOTE-datetime">ISO 8601</a> 格式中规定的时间加日期的格式，则将会被作为本地时区处理。</p>
...
<p>在 Firefox 30 中会被识别为本地时区的 2015 年 12 月 25 日，而在 Safari 7 中则是无效日期。但是，如果字符串被识别为 ISO 格式并且包含无效值，则在所有遵循 ES5 或者更新标准的浏览器中都会返回 {{jsxref("NaN")}} 。</p>

<pre class="brush: js">// 包含无效值的 ISO 格式字符串
new Date('2014-25-23').toISOString();
// 在所有遵循 ES5 的浏览器中返回 "RangeError: invalid date"</pre>
...
<p>如果 <code>IPOdate</code> 是一个已经存在的 {{jsxref("Date")}} 对象，则可以把其设置为本地时间 1995 年 8 月 9 日。如下：</p>
...
<p>Firefox 49 {{geckoRelease(49)}} 修改了解析 2 位数年份的方式，从和 Internet Explorer 一致改为和 Google Chrome  浏览器一致。现在，2 位数的年份小于等于 <code>50</code>  的将会被解析为 21 世纪的年份。比如， <code>04/16/17</code> ，在之前会被解析为 1917 年 4 月 16 日，现在将被解析为 2017 年 4 月 16 日。为了避免任何可能的同步问题或者有歧义的年份，推荐使用 ISO 8601 格式如 "2017-04-16" ({{bug(1265136)}})。</p>
