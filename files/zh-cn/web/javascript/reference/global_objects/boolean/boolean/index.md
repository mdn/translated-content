...
> [!NOTE]
> 调用 `Boolean()` 时可以使用或不使用 [`new`](/zh-CN/docs/Web/JavaScript/Reference/Operators/new)，但是效果不同。参见[返回值](#返回值)部分。
...
> [!NOTE]
> 当非标准属性 [`document.all`](/zh-CN/docs/Web/API/Document/all) 被用作该构造函数的参数时，结果是一个值为 `false` 的 `Boolean` 对象。这个属性是遗留的和非标准的，不应该被使用。
