...
绑定到指定属性的 getter 函数。如果找不到这样的属性或者该属性是[数据属性](/zh-CN/docs/Web/JavaScript/Guide/Data_structures#数据属性)，则返回 `undefined`。
