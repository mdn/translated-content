...
<dl>
 ...
 <dd><code>当且仅当属性的值可以被改变时为 true。(仅针对数据属性描述有效)</code></dd>
 <dt><code><strong>get</strong></code></dt>
 <dd>获取该属性的访问器函数（getter）。如果没有访问器，该值为 undefined。(仅针对包含访问器或设置器的属性描述有效)</dd>
 <dt><code><strong>set</strong></code></dt>
 <dd>获取该属性的设置器函数（setter）。如果没有设置器，该值为 undefined。(仅针对包含访问器或设置器的属性描述有效)</dd>
 <dt><code><strong>configurable</strong></code></dt>
 <dd><code>当且仅当指定对象的属性描述可以被改变或者属性可被删除时，为 true。</code></dd>
