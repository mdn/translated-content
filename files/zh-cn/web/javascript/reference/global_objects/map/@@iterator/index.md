...
与 {{jsxref("Map.prototype.entries()")}} 返回值相同，一个新的[迭代器对象](/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Iterator)，它会以插入顺序返回 map 的键值对。

## 示例

### 使用 for...of 循环进行迭代

请注意，通常你不需要直接调用此方法。`@@iterator` 方法的存在使得 `Map` 对象[可迭代](/zh-CN/docs/Web/JavaScript/Reference/Iteration_protocols#可迭代协议)，而迭代语法如 `for...of` 循环会自动调用此方法以获取用于循环的迭代器。
