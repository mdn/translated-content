...
<p>通过<strong>Error</strong>的构造器可以创建一个错误对象。当运行时错误产生时，Error 的实例对象会被抛出。Error 对象也可用于用户自定义的异常的基础对象。下面列出了各种内建的标准错误类型。</p>
...
<dl>
 ...
 <dd>可选。被创建的 Error 对象的 fileName 属性值。默认是调用 Error 构造器代码所在的文件 的名字。</dd>
 <dt><code>lineNumber </code>{{non-standard_inline}}</dt>
 <dd>可选。被创建的 Error 对象的 lineNumber 属性值。默认是调用 Error 构造器代码所在的文件的行号。</dd>
</dl>
...
<p>该页面描述了 Error 对象自身的使用，以及其构造函数的使用。关于 Error 实例的内部属性和方法，请看 {{jsxref("Error.prototype")}}。</p>
...
<p>除了通用的 Error 构造函数外，JavaScript 还有 6 个其他类型的错误构造函数。更多客户端异常，详见 <a href="/en/JavaScript/Guide/Statements#Exception_Handling_Statements" title="en/JavaScript/Guide/Statements#Exception Handling Statements">Exception Handling Statements</a>。</p>

<dl>
 <dt><strong>{{jsxref("EvalError")}}</strong></dt>
 <dd>创建一个 error 实例，表示错误的原因：与 {{jsxref("Global_Objects/eval", "eval()")}} 有关。</dd>
 <dt><strong>{{jsxref("InternalError")}} {{non-standard_inline}}</strong></dt>
 <dd>创建一个代表 Javascript 引擎内部错误的异常抛出的实例。如: "递归太多".</dd>
 <dt><strong>{{jsxref("RangeError", "RangeError")}}</strong></dt>
 <dd>创建一个 error 实例，表示错误的原因：数值变量或参数超出其有效范围。</dd>
 <dt><strong>{{jsxref("ReferenceError")}}</strong></dt>
 <dd>创建一个 error 实例，表示错误的原因：无效引用。</dd>
 <dt><strong>{{jsxref("SyntaxError")}}</strong></dt>
 <dd>创建一个 error 实例，表示错误的原因：{{jsxref("Global_Objects/eval", "eval()")}} 在解析代码的过程中发生的语法错误。</dd>
 <dt><strong>{{jsxref("TypeError")}}</strong></dt>
 <dd>创建一个 error 实例，表示错误的原因：变量或参数不属于有效类型。</dd>
 <dt><strong>{{jsxref("URIError")}}</strong></dt>
 <dd>创建一个 error 实例，表示错误的原因：给 {{jsxref("Global_Objects/encodeURI", "encodeURI()")}} 或 {{jsxref("Global_Objects/decodeURI", "decodeURI()")}} 传递的参数无效。</dd>
</dl>
...
<p>通常你会使用{{jsxref("Statements/throw", "throw")}}关键字来抛出你创建的 Error 对象。可以使用 {{jsxref("Statements/try...catch", "try...catch")}} 结构来处理异常：</p>
...
<p>你可以通过判断异常的类型来特定处理某一类的异常，即判断 {{jsxref("Object.prototype.constructor", "constructor")}} 属性，当使用现代 JavaScript 引擎时，可使用{{jsxref("Operators/instanceof", "instanceof")}} 关键字：</p>
...
<p>你可能希望自定义基于 Error 的异常类型，使得你能够 <code>throw new MyError()</code> 并可以使用 <code>instanceof MyError</code> 来检查某个异常的类型。这种需求的通用解决方法如下。</p>

<div class="warning">
<p><strong>警告：</strong>注意，在 FireFox 中抛出自定义类型的异常会显示不正确的行号和文件名。</p>
</div>
