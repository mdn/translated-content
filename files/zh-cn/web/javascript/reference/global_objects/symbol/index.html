...
<p><strong>symbol</strong> 是一种基本数据类型（{{Glossary("Primitive", "primitive data type")}}）。<code>Symbol()</code> 函数会返回 <strong>symbol</strong> 类型的值，该类型具有静态属性和静态方法。它的静态属性会暴露几个内建的成员对象；它的静态方法会暴露全局的 symbol 注册，且类似于内建对象类，但作为构造函数来说它并不完整，因为它不支持语法："<code>new Symbol()</code>"。</p>

<p>每个从 <code>Symbol()</code> 返回的 symbol 值都是唯一的。一个 symbol 值能作为对象属性的标识符；这是该数据类型仅有的目的。更进一步的解析见—— <a href="https://developer.mozilla.org/en-US/docs/Glossary/Symbol">glossary entry for Symbol</a>。</p>
...
<dl>
 ...
 <dd>可选的，字符串类型。对 symbol 的描述，可用于调试但不是访问 symbol 本身。</dd>
</dl>
...
<p>上面使用 <code>Symbol()</code> 函数的语法，不会在你的整个代码库中创建一个可用的全局的 symbol 类型。要创建跨文件可用的 symbol，甚至跨域（每个都有它自己的全局作用域），使用 {{jsxref("Symbol.for()")}} 方法和 {{jsxref("Symbol.keyFor()")}} 方法从全局的 symbol 注册表设置和取得 symbol。</p>

<h3 id="在对象中查找_Symbol_属性">在对象中查找 Symbol 属性</h3>

<p>{{jsxref("Object.getOwnPropertySymbols()")}} 方法让你在查找一个给定对象的符号属性时返回一个 symbol 类型的数组。注意，每个初始化的对象都是没有自己的 symbol 属性的，因此这个数组可能为空，除非你已经在对象上设置了 symbol 属性。</p>
...
<dl>
 ...
 <dd>一个布尔值，表明一个对象是否应该 flattened 为它的数组元素。被 {{jsxref("Array.prototype.concat()")}} 使用。</dd>
...
<dl>
 ...
 <dd>使用给定的 key 搜索现有的 symbol，如果找到则返回该 symbol。否则将使用给定的 key 在全局 symbol 注册表中创建一个新的 symbol。</dd>
 <dt>{{jsxref("Symbol.keyFor", "Symbol.keyFor(sym)")}}</dt>
 <dd>从全局 symbol 注册表中，为给定的 symbol 检索一个共享的？symbol key。</dd>
</dl>
...
<dl>
  ...
  <dd>返回该 Symbol 对象。该方法重写了 {{jsxref("Object.prototype.valueOf")}} 方法</dd>
...
<ul>
 ...
 <li>这会阻止你从一个 symbol 值隐式地创建一个新的 string 类型的属性名。例如，<code>Symbol("foo") + "bar"</code> 将抛出一个 {{jsxref("TypeError")}} (can't convert symbol to string).</li>
 <li><a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String#String_conversion">"safer" <code>String(sym)</code> conversion</a> 的作用会像 symbol 类型调用 {{jsxref("Symbol.prototype.toString()")}} 一样，但是注意 <code>new String(sym)</code> 将抛出异常。</li>
</ul>
...
<ul>
 ...
 <li><a href="https://hacks.mozilla.org/2015/06/es6-in-depth-symbols/">"ES6 In Depth: Symbols"（深入 ES6 Symbols ）on hacks.mozilla.org</a></li>
</ul>
