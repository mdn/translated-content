...
<p>检测给定的数组索引 0 的值，如果它如预期一般的等于我们给定的值 0，则这个读取线程将会睡眠等待。一旦当有一个写入线程在这个位置存储了一个新值，它将会收到写入线程的通知并且返回新值 (123) :</p>

<pre class="brush: js">Atomics.wait(int32, 0, 0);
console.log(int32[0]); // 123</pre>

<p>一旦某个写入线程存储了一个新值到<code>int32</code> 的索引 0 位置，则通知给该等待线程：</p>
