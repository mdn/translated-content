...
<p><code>该 <strong>Uint16Array</strong></code> 类型数组表示 16 位无符号整数，按平台字节顺序排列。如果需要控制字节顺序，请使用 DataView 代替。内容被初始化为 0。建立后，就可以使用对象的方法或使用标准数组索引语法（即使用括号表示法）引用数组中的元素。</p>
...
<dl>
 ...
 <dd>根据所提供的过滤函数，将所有返回 true 的所有元素创建一个新的数组。可参阅 {{jsxref("Array.prototype.filter()")}}。</dd>
 ...
 <dd>f 返回一个新的 <code>Array Iterator</code> ，其包含数组中每个索引的键。 可参阅 {{jsxref("Array.prototype.keys()")}}。</dd>
