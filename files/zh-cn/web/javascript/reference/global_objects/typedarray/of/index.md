...
- `TypedArray.of` 使用 \[\[Put]] 其中 Array.of 使用 \[\[DefineProperty]]。因此，当使用{{jsxref("Proxy")}} 对象时，它调用 {{jsxref("Global_Objects/Proxy/Proxy/set", "handler.set")}} 创建新的元素，而不是 {{jsxref("Global_Objects/Proxy/Proxy/defineProperty", "handler.defineProperty")}}。
