...
不过，这种方法会向类的用户暴露一个实现细节（`init()` 方法）。另一方面，在类外声明的任何初始化逻辑都无法访问私有静态字段。静态初始化块允许在类中声明任意初始化逻辑，并在类评估过程中执行。
...
静态代码块的作用域嵌套在类主体的词法作用域内，可以访问类内声明的 [私有名称](/en-US/docs/Web/JavaScript/Reference/Classes/Private_properties)，而不会导致语法错误。
...
下面的代码演示了一个带有静态初始化块和交错静态字段初始化器的类。输出结果显示，块和字段是按执行顺序进行评估的。

```js
class MyClass {
  static field1 = console.log("静态字段1");
  static {
    console.log("静态块1");
  }
...
请注意，超类的任何静态初始化都是先进行的，然后才是子类的初始化。

### 使用 this 和 super

静态代码块中的 `this` 是指类的构造函数对象。这段代码展示了如何访问公共静态字段。
