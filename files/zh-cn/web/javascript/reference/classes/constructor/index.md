...
1.（如果是派生类）`super()` 调用之前的 `constructor` 主体。这部分不应访问 `this`，因为它尚未初始化。2.（如果是派生类）执行 `super()` 调用，通过同样的过程初始化父类。3. 当前类的[字段](/zh-CN/docs/Web/JavaScript/Reference/Classes/Public_class_fields)将被初始化。4. 执行调用 `super()` 后的 `constructor` 主体（如果是基类，则对整个主体）。
