...
> **警告：** 在[严格模式](/zh-CN/docs/Web/JavaScript/Reference/Strict_mode)下，第 5 版 ECMAScript（**ES5**）禁止使用 `arguments.callee()`。当一个函数必须调用自身的时候，避免使用 `arguments.callee()`，通过`要么`给函数表达式一个名字，要么使用一个函数声明。

## 为什么 arguments.callee 从 ES5 严格模式中删除了？

（改编自 [a Stack Overflow answer by olliej](https://stackoverflow.com/a/235760/578288)）
...
递归函数必须能够引用它本身。很典型的，函数通过自己的名字调用自己。然而，匿名函数（通过[函数表达式](/zh-CN/docs/Web/JavaScript/Reference/Operators/function)或者 [`Function` 构造函数](/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Function)）没有名称。因此如果没有可访问的变量指向该函数，唯一能引用它的方式就是通过 `arguments.callee`。

下面的例子定义了一个函数，按流程，定义并返回了一个阶乘函数。该例并不是很实用，并且几乎都能够用[具名函数表达式](/zh-CN/docs/Web/JavaScript/Reference/Operators/function)实现同样结果的例子。
...
当你必须要使用 Function 构造函数时，下面的例子是没有可以替代 `arguments.callee` 的方案的，因此弃用它时会产生一个 BUG（参见 [Firefox bug 725398](https://bugzil.la/725398)）：
