...
<div class="warning"><p><strong>警告：</strong>在<a href="/en-US/docs/JavaScript/Reference/Functions_and_function_scope/Strict_mode">严格模式</a>下，第 5 版 ECMAScript (<strong>ES5</strong>) 禁止使用 <code>a<strong>rguments.callee()</strong>。当一个函数必须调用自身的时候，避免使用 <strong>arguments.callee(), </strong></code>通过<code>要么</code>给函数表达式一个名字，要么使用一个函数声明。</p></div>

<h2 id="为什么_arguments.callee_从_es5_严格模式中删除了？">为什么 arguments.callee 从 ES5 严格模式中删除了？</h2>

<p>(改编自 <a href="http://stackoverflow.com/a/235760/578288">a Stack Overflow answer by olliej</a>)</p>

<p>早期版本的 JavaScript 不允许使用命名函数表达式，出于这样的原因，你不能创建一个递归函数表达式。</p>
...
<p>递归函数必须能够引用它本身。很典型的，函数通过自己的名字调用自己。然而，匿名函数 (通过 <a href="/en-US/docs/JavaScript/Reference/Operators/function">函数表达式</a> 或者 <code><a href="/en-US/docs/JavaScript/Reference/Global_Objects/Function">函数构造器 </a>创建</code>) 没有名称。因此如果没有可访问的变量指向该函数，唯一能引用它的方式就是通过 <code>arguments.callee</code>。</p>

<p>下面的例子定义了一个函数，按流程，定义并返回了一个阶乘函数。该例并不是很实用，并且几乎都能够用 <a href="/en-US/docs/JavaScript/Reference/Operators/function">命名函数表达式</a> 实现同样结果的例子，and there are nearly no cases where the same result cannot be achieved with .</p>
...
<p>当你必须要使用 Function 构造函数时，下面的例子是没有可以替代 <code>arguments.callee</code> 的方案的，因此弃用它时会产生一个 BUG (参看 {{Bug("725398")}}):</p>
