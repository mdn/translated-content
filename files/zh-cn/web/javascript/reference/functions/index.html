...
<div>在 JavaScript 中，函数是<strong>头等 (</strong>first-class<strong>)</strong>对象，因为它们可以像任何其他<strong>对象</strong>一样具有属性和方法。它们与其他对象的区别在于函数可以被调用。简而言之，它们是<code><a href="/zh-cn/JavaScript/Reference/Global_Objects/Function">Function</a></code>对象。</div>

<div> </div>

<p>有关更多示例和说明，请参阅<a href="/zh-CN/docs/Web/JavaScript/Guide/Functions">有关函数的 JavaScript 指南</a>。</p>

<h2 id="描述">描述</h2>

<p>在 JavaScript 中，每个函数其实都是一个<code>Function</code>对象。查看{{jsxref("Function")}}页面<code>了解其</code>属性和方法。</p>

<p>如果一个函数中没有使用 return 语句，则它默认返回<code>undefined</code>。要想返回一个特定的值，则函数必须使用 <code><a href="/zh-cn/JavaScript/Reference/Statements/return">return</a></code> 语句来指定一个要返回的值。(使用<a href="/zh-cn/JavaScript/Reference/Operators/new">new</a>关键字调用一个<a href="/zh-cn/JavaScript/Reference/Global_Objects/Object/constructor">构造函数</a>除外)。</p>
...
<p>在函数执行时，<a href="/zh-cn/JavaScript/Reference/Operators/this"><code>this</code> 关键字</a>并不会指向正在运行的函数本身，而是指向调用该函数的对象。所以，如果你想在函数内部获取函数自身的引用，只能使用函数名或者使用<a href="/zh-cn/JavaScript/Reference/Functions_and_function_scope/arguments/callee">arguments.callee</a>属性 (<u><a href="/zh-cn/JavaScript/Strict_mode">严格模式</a></u>下不可用)，如果该函数是一个匿名函数，则你只能使用后者。</p>
...
<p>有一个特殊的语法来声明函数 (查看<a href="/zh-cn/JavaScript/Reference/Statements/function">函数语句</a>了解详情)：</p>
...
<p>可以看到，上面的两个例子都不以 function 开头。不以 function 开头的函数语句就是函数表达式定义。</p>
...
<div class="note">
<p><strong>备注：</strong> 不推荐使用 <code>Function</code> 构造函数创建函数，因为它需要的函数体作为字符串可能会阻止一些 JS 引擎优化，也会引起其他问题。</p>
</div>

<p>所有其他对象，{{jsxref("Function")}} 对象可以用 new 操作符创建：</p>

<pre class="syntaxbox">new Function (<em>arg1</em>, <em>arg2</em>, ... <em>argN</em>, <em>functionBody</em>)</pre>

<dl>
 <dt><code>arg1, arg2, ... arg<em>N</em></code></dt>
 <dd>函数使用零个或多个名称作为正式的参数名称。每一个必须是一个符合有效的 JavaScript 标识符规则的字符串或用逗号分隔的字符串列表，例如“x”,“theValue”或“a,b”。</dd>
 <dt><code>functionBody</code></dt>
 <dd>一个构成的函数定义的，包含 JavaScript 声明语句的字符串。</dd>
</dl>

<p>把 Function 的构造函数当作函数一样调用 (不使用 new 操作符) 的效果与作为 Function 的构造函数调用一样。</p>
...
<div class="note">
<p><strong>备注：</strong> 不推荐使用构造器函数的构造函数 (<code>GeneratorFunction</code> constructor) 创建函数，因为它需要的函数体作为字符串可能会阻止一些 JS 引擎优化，也会引起其他问题。</p>
</div>
...
<ul>
 ...
 <li><code><a href="https://developer.mozilla.org/en-US/docs/JavaScript/Reference/Functions_and_function_scope/arguments/caller">arguments.caller</a></code>{{Deprecated_Inline}} : 调用当前执行函数的函数。</li>
...
<p>你可以在支持添加新属性的任何标准的内置对象或用户定义的对象内定义 getter(访问方法) 和 setter(设置方法)。使用对象字面量语法定义 getters 和 setters 方法。</p>
...
<p>从 ECMAScript 6 开始，你可以用更短的语法定义自己的方法，类似于 getters 和 setters。详情请查阅 <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Method_definitions">method definitions</a> .</p>
...
<p>一个用<code>Function</code>构造函数定义的函数，被赋值给变量 multiply：</p>
...
<p>函数名和函数的变量存在着差别。函数名不能被改变，但函数的变量却能够被再分配。函数名只能在函数体内使用。倘若在函数体外使用函数名将会导致错误（如果函数之前是通过一个 var 语句声明的则是 undefined）。例如：</p>
...
<p>使用用 '<code>new Function'定义的函数没有函数名。</code> 然而，在 <a href="https://developer.mozilla.org/en-US/docs/Mozilla/Projects/SpiderMonkey">SpiderMonkey</a> JavaScript 引擎中，其函数的序列化形式表现的好像它拥有一个名叫"anonymous"的名称一样。比如，使用 <code>alert(new Function())</code> 输出：</p>
...
<p>和通过函数表达式定义或者通过 Function 构造函数定义的函数不同，函数声明定义的函数可以在它被声明之前使用。举个例子：</p>
...
<p>函数表达式定义的函数继承了当前的作用域。换言之，函数构成了闭包。另一方面，Function 构造函数定义的函数不继承任何全局作用域以外的作用域 (那些所有函数都继承的)。</p>

<p>通过函数表达式定义的函数和通过函数声明定义的函数只会被解析一次，而 Function 构造函数定义的函数却不同。也就是说，每次构造函数被调用，传递给 Function 构造函数的函数体字符串都要被解析一次 。虽然函数表达式每次都创建了一个闭包，但函数体不会被重复解析，因此函数表达式仍然要快于"<code>new Function(...)</code>"。 所以 Function 构造函数应尽可能地避免使用。</p>

<p>有一点应该要注意的，在通过解析 Function 构造函数字符串产生的函数里，内嵌的函数表达式和函数声明不会被重复解析。例如：</p>
...
<p>从 ECMAScript 6 开始，在<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Strict_mode">严格模式</a>下，块里的函数作用域为这个块。ECMAScript 6 之前不建议块级函数在严格模式下使用。</p>
...
<p>ECMAScript 6 中，如果<code>shouldDefineZero</code>是 false，则永远不会定义 zero，因为这个块从不执行。然而，这是标准的新的一部分。这是历史遗留问题，无论这个块是否执行，一些浏览器会定义 zero。</p>

<p><code>在</code><a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Strict_mode">严格模式</a><code>下，所有支持</code>ECMAScript 6 的<code>浏览器以相同的方式处理：只有在 shouldDefineZero 为 true 的情况下定义 zero，并且作用域只是这个块内。</code></p>
...
<p>下面的函数返回一个字符串，其中包含了一个格式化的、以一个由 0 开头并填充的数字。</p>

<pre class="brush: js">// 这个函数返回一个由 0 开头并填充的字符串
...
<p>你可以通过<strong> typeof </strong>操作符检测一个函数是否存在。在下面的例子中，用一个测试来演示检测 window 对象是否拥有一个 noFunc 函数的属性。如果存在，那就使用它；否则就采取其它的一些操作。</p>
...
<p>注意在 if 语句中，使用了 noFunc 的引用 -- 在函数名的后面没有括号“（）”，所以实际函数并没有被调用。</p>
...
<ul>
 ...
 <li><a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions_and_function_scope">Functions and function scope</a></li>
</ul>
