...
- 在比较两件事情时，双等号（`==`）将执行类型转换，并且会按照 IEEE 754 对 `NaN`、`-0` 和 `+0` 进行特殊处理（使 `NaN != NaN`，且 `-0 == +0`）；
...
同值相等决定了两个值在所有上下文中是否在功能上相同。（这个用例演示了[里氏替换原则](https://zh.wikipedia.org/zh-cn/%E9%87%8C%E6%B0%8F%E6%9B%BF%E6%8D%A2%E5%8E%9F%E5%88%99)的一种情况。）这一情况会在尝试修改一个不可遍属性时发生。
