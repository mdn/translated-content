...
<div class="note">
<p><strong>备注：</strong>遵循 ECMAScript 标准，<code>someObject.[[Prototype]]</code> 符号是用于指向 <code>someObject</code> 的原型。从 ECMAScript 6 开始，<code>[[Prototype]]</code> 可以通过 {{jsxref("Object.getPrototypeOf()")}} 和 {{jsxref("Object.setPrototypeOf()")}} 访问器来访问。这个等同于 JavaScript 的非标准但许多浏览器实现的属性 <code>__proto__</code>。</p>
...
<pre class="brush: js">// 让我们从一个函数里创建一个对象 o，它自身拥有属性 a 和 b 的：
let f = function () {
   this.a = 1;
   this.b = 2;
}
...
// d ---&gt; null
console.log(d.hasOwnProperty); // undefined，因为 d 没有继承 Object.prototype</pre>
...
<table>
 ...
 console.log(inst.bar_prop);</pre>
    ...
    <td>支持目前以及所有可想象到的浏览器 (IE5.5 都可以使用)。 这种方法非常快，非常符合标准，并且充分利用 JIT 优化。</td>
 ...
 console.log(inst.bar_prop)</pre>
    ...
    <td>支持当前所有非微软版本或者 IE9 以上版本的浏览器。允许一次性地直接设置 <code>__proto__</code> 属性，以便浏览器能更好地优化对象。同时允许通过 <code>Object.create(null) </code>来创建一个没有原型的对象。</td>
    <td>不支持 IE8 以下的版本。然而，随着微软不再对系统中运行的旧版本浏览器提供支持，这将不是在大多数应用中的主要问题。 另外，这个慢对象初始化在使用第二个参数的时候有可能成为一个性能黑洞，因为每个对象的描述符属性都有自己的描述对象。当以对象的格式处理成百上千的对象描述的时候，可能会造成严重的性能问题。</td>
   </tr>
   <tr>
    <td>
 ...
 console.log(inst.bar_prop)</pre>
    ...
    <td>支持所有现代浏览器和微软 IE9+ 浏览器。允许动态操作对象的原型，甚至能强制给通过 <code>Object.create(null) </code>创建出来的没有原型的对象添加一个原型。</td>
    <td>这个方式表现并不好，应该被弃用。如果你在生产环境中使用这个方法，那么快速运行 Javascript 就是不可能的，因为许多浏览器优化了原型，尝试在调用实例之前猜测方法在内存中的位置，但是动态设置原型干扰了所有的优化，甚至可能使浏览器为了运行成功，使用完全未经优化的代码进行重编译。 不支持 IE8 及以下的浏览器版本。</td>
   </tr>
   <tr>
    <td>__proto__</td>
    <td>
 ...
 console.log(inst.bar_prop)</pre>
    ...
    <td>支持所有现代非微软版本以及 IE11 以上版本的浏览器。将 <code>__proto__</code> 设置为非对象的值会静默失败，并不会抛出错误。</td>
    <td>应该完全将其抛弃因为这个行为完全不具备性能可言。 如果你在生产环境中使用这个方法，那么快速运行 Javascript 就是不可能的，因为许多浏览器优化了原型，尝试在调用实例之前猜测方法在内存中的位置，但是动态设置原型干扰了所有的优化，甚至可能使浏览器为了运行成功，使用完全未经优化的代码进行重编译。不支持 IE10 及以下的浏览器版本。</td>
   </tr>
  </tbody>
 </table>
