...
<p>首先，您需要将 WebM 视频转换为带有不同比特率的随附视频文件的 DASH 清单。根据您的需求，启动从 <a href="http://www.ffmpeg.org">ffmpeg.org</a> 的 ffmpeg 程序，就可以使用 libvpx 和 libbvorbis 支持的 WebM 视频和音频（版本 2.5 以上，3.2.5 版本已通过测试）。</p>

<h3 id="1._使用现有的WebM文件创建一个音频文件和多个视频文件。">1. 使用现有的 WebM 文件创建一个音频文件和多个视频文件。</h3>

<p>例如：</p>

<p>文件<em><strong>in.video</strong></em>可以是任何包含至少一个音频和一个视频流的容器，这些容器可以由 ffmpeg 解码，</p>
...
<p>将清单和相关的视频文件放在 Web 服务器或 CDN 上。 DASH 通过 HTTP 来完成，因此只要您的 HTTP 服务器支持字节范围请求，并且 DASH 设置为使用 mimetype="application/dash+xml" 来支持 .mpd 文件即可。</p>

<h2 id="使用DASH-客户端">使用 DASH-客户端</h2>

<p>您将需要修改网页，使其首先指向 DASH 清单，而不是直接指向特定的视频文件：</p>
...
<p>如果浏览器支持 DASH，则您的视频现在将自适应地流式传输。</p>
