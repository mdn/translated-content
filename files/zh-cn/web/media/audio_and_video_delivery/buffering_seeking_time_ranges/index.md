...
有时候知道 {{htmlelement("audio") }} 或 {{htmlelement("video") }} 已经下载了多少或有多少可以不经过延迟直接播放是有用的——音频和视频的缓冲条就是一个很好的例子。这篇文章讨论如何使用 [TimeRanges](/zh-CN/docs/Web/API/TimeRanges) 和媒体 API 的其他特性创建一个缓冲/拖动条。

## Buffered

`buffered` 属性会告诉我们媒体的哪一部分已经下载好了。它返回一个 {{domxref("TimeRanges")}} 对象，表明哪些分块已经下载。这通常是连续的但是如果用户在缓冲时跳过，就可能会有缺少的部分。
...
没有用户交互的话通常只有一个时间范围，但是如果你在媒体中跳来跳去，那么就会出现多个时间范围，下面形象地表名了这一点。这表示两个缓冲的时间范围——第一个范围从 0 到 5 秒，第二个范围从 15 到 19 秒。
...
对于该音频实例，相关联的 {{domxref("TimeRange")}} 对象会有以下可用的属性：
...
`seekable` 属性返回一个 {{domxref("TimeRanges")}} 对象，告诉我们媒体的哪一部分可以不经过延迟直接播放；这与该部分是否已被下载无关。如果服务器上启用了字节范围请求，媒体的一些部分可能是可查找（seekable）但又没有缓冲的，字节范围请求允许服务器分发媒体文件的某一部分，因此可以几乎立即准备好播放——因此它们是可查找的。有关字节范围请求的更多信息，请参见 [HTTP 范围请求](/zh-CN/docs/Web/HTTP/Range_requests)。
...
也许最好的方法是给出媒体实际下载了多少的指示——这是浏览器的原生播放器似乎显示的内容。
...
![一个简单的音频播放器，带有播放按钮、搜索栏和音量控制，下面有一个低栏，其中的绿色部分显示播放的视频，还有一个灰色的条表示缓冲的进度。](bufferedprogress.png)
