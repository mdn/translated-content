...
---
<div>
<p>我们可以通过多种方式在网络上交付音频和视频，从“静态”媒体文件到自适应实时流。 本文旨在作为探讨基于 Web 的媒体的各种传递机制以及与流行浏览器的兼容性的起点。</p>
</div>

<h2 id="音频和视频元素">音频和视频元素</h2>
...
<h3 id="HTML音频">HTML 音频</h3>

<pre class="brush: html notranslate">&lt;audio controls preload="auto"&gt;
...
<div class="note">
<p><strong>注意：</strong>preload 属性可能会被某些移动浏览器忽略。</p>
</div>

<p>有关更多信息，请参见 <a href="/en-US/Apps/Build/Manipulating_media/Cross-browser_audio_basics#HTML5_audio_in_detail">Cross Browser Audio Basics (HTML5 Audio In Detail)</a></p>

<h3 id="HTML_视频">HTML 视频</h3>

<pre class="brush: html notranslate">&lt;video controls width="640" height="480" poster="initialimage.png" autoplay muted&gt;
...
<div class="note">
<p><strong>注意：</strong>某些移动浏览器可能会忽略自动播放属性。 同样，自动播放功能在滥用时也会引起争议。 强烈建议您阅读<a href="/zh-CN/docs/Web/Media/Autoplay_guide">媒体和 Web 音频 API 的自动播放指南</a>，以了解如何明智地使用自动播放。</p>
</div>
...
<p>该过程与视频非常相似，只需记住在<code>flashvars value</code>参数中设置<code>isvideo = true</code>。<br>
 <br>
 <a href="/en-US/Apps/Build/Manipulating_media/Cross-browser_audio_basics#Fallbacks">More options for Fallbacks</a>.</p>

<h3 id="JavaScript_音频">JavaScript 音频</h3>
...
<div class="note">
<p><strong>注意</strong>: 除非由用户启动的事件发布，否则某些移动浏览器将忽略播放。</p>
</div>
...
<pre class="brush: html notranslate">&lt;audio id="player" src="data:audio/x-wav;base64,UklGRvC..."&gt;&lt;/audio&gt;</pre>
...
<pre class="brush: js notranslate">var myVideo = document.createElement('video');
...
<pre class="brush: js notranslate">  var context;
  var request;
  var source;
...
<pre class="brush: html notranslate">&lt;video id="webcam" width="480" height="360"&gt;&lt;/video&gt;</pre>

<p>Next, if supported connect the webcam source to the video element:</p>

<pre class="brush: js notranslate">navigator.getUserMedia ||
...
<p>To find out more, read our <a href="/en-US/docs/Web/API/Navigator.getUserMedia">Navigator.getUserMedia</a> page.</p>
...
<pre class="brush: js notranslate">var onSuccess = function(stream) {
...
<div class="note">
<p><strong>Note</strong>: MediaRecorder is currently only supported in Firefox and only for audio, however there are some interesting workarounds including a library called <a href="https://github.com/muaz-khan/WebRTC-Experiment/tree/master/RecordRTC">RecordRTC</a>.</p>
</div>
...
<h2 id="Debugging_Audio_Video_Issues">Debugging Audio / Video Issues</h2>

<p>Having issues playing back audio or video? Try the following check-list.</p>

<h3 id="Does_the_browser_support_the_supplied_formats">Does the browser support the supplied formats?</h3>
...
<pre class="notranslate"># AddType TYPE/SUBTYPE EXTENSION
...
<pre class="brush: html notranslate">  &lt;audio id="my-audio" src="http://jPlayer.org/audio/mp3/Miaow-01-Tempered-song.mp3"&gt;&lt;/audio&gt;
...
<dl>
 ...
 <dd>This article explains how to add captions and subtitles to HTML5 {{ htmlelement("video") }}, using <a href="https://developer.mozilla.org/en-US/docs/Web/API/Web_Video_Text_Tracks_Format">Web_Video_Text_Tracks_Format</a> and the {{ htmlelement("track") }} element.</dd>
 <dt><a href="/en-US/docs/Web/Apps/Developing/Manipulating_media/Web_Audio_API_cross_browser">Writing Web Audio API code that works in every browser</a></dt>
