...
下面所展示的视频来自[Blender Foundation](http://www.blender.org/foundation/)的 [Sintel open movie](http://www.sintel.org/)
...
> **备注：** 观众老爷可以点击这里 [查看 Github 源码](https://github.com/iandevlin/iandevlin.github.io/tree/master/mdn/video-player-with-captions), 或者[浏览在线视频](http://iandevlin.github.io/mdn/video-player-with-captions/).

## HTML5 视频字幕

在学习如何向视频添加字幕之前，我们应该了解下面这些事情

### 标题和副标题

[标题和副标题不是一回事](http://web.archive.org/web/20160117160743/http://screenfont.ca/learn/)：它们有明显不同的受众，传达的信息也不同，如果你不确定它们是什么，建议你仔细阅读这些差异。然而，它们在技术上是以相同的方式实现的，因此本文中的内容将同时适用于这两者。

在这篇文章中，我们将提到文本轨道作为字幕显示，因为它们的内容是针对影片使用的语言有困难的人，而不是聋哑人或有听力障碍的人。
...
The files that contain the actual subtitle data are simple text files that follow a specified format, in this case the [Web Video Text Tracks](/zh-CN/docs/HTML/WebVTT) (WebVTT) format. The [WebVTT specification](http://dev.w3.org/html5/webvtt/) is still being worked on, but major parts of it are stable so we can use it today.
...
This section summarises the modifications made to the previous article's code in order to facilitate the addition of subtitles to the video. If you are not interested in this, and just want to get straight into the JavaScript and more relevant CSS, skip to the [Subtitle implementation](#使用字幕) section.
...
> **备注：** IE will completely ignore WebVTT files unless you define the MIME type. This can easily be done by adding an `.htaccess` file to an appropriate directory that contains `AddType text/vtt .vtt`.
...
> **备注：** You can find an excellent list of HTML5 Video Players and their current "state" at [HTML5 Video Player Comparison](http://praegnanz.de/html5video/).
