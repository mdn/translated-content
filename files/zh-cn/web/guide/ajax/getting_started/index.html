...
<p>这篇文章旨在帮助你了解 AJAX 基础，并提供两个可上手的简单案例供你学习。</p>

<h2 id="什么是AJAX？">什么是 AJAX？</h2>

<p>AJAX 是异步的 JavaScript 和 XML（<strong>A</strong>synchronous <strong>J</strong>avaScript <strong>A</strong>nd <strong>X</strong>ML）。简单点说，就是使用 <code><a href="/en/DOM/XMLHttpRequest">XMLHttpRequest</a></code> 对象与服务器通信。 它可以使用 JSON，XML，HTML 和 text 文本等格式发送和接收数据。AJAX 最吸引人的就是它的“异步”特性，也就是说它可以在不重新刷新页面的情况下与服务器通信，交换数据，或更新页面。</p>
...
<h2 id="Step_1_–_怎样发送http请求">Step 1 – 怎样发送 http 请求</h2>

<p>为了使用 JavaScript 向服务器发送一个 http 请求，你需要一个包含必要函数功能的对象实例。这就是为什么会有 <code>XMLHttpRequest</code> 的原因。 这是 IE 浏览器的 ActiveX 对象 <code>XMLHTTP</code>的前身。 随后 Mozilla，Safari 和其他浏览器也都实现了类似的方法，被称为 <code>XMLHttpRequest</code> 。同时，微软也实现了 XMLHttpRequest 方法。</p>
...
<div class="note"><strong>Note: 上面代码只是简单版的如何创建一个 XMLHttp 实例。更实际的例子，请看本篇文章的 step 3。</strong></div>
...
<ul>
 <li><code>open()</code> 的第一个参数是 HTTP 请求方法 - 有 GET，POST，HEAD 以及服务器支持的其他方法。 保证这些方法一定要是大写字母，否则其他一些浏览器（比如 FireFox）可能无法处理这个请求。更多关于 HTTP 的请求方法，可以查看 <a href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html">W3C specs</a>。</li>
 <li>第二个参数是你要发送的 URL。由于安全原因，默认不能调用第三方 URL 域名。 确保你在页面中使用的是正确的域名，否则在调用 <code>open()</code> 方法是会有 "permission denied" 错误提示。一个容易犯的错误是你企图通过 <code>domain.tld</code> 访问网站， 而不是使用 <code>www.domain.tld</code>。如果你真的需要向另一个域名发送请求，可以查看 <a href="/En/HTTP_access_control">HTTP access control</a>。</li>
...
<p>在发送请求时，你提供的 JavaScript 函数名负责处理响应：</p>
...
<p>全部 readyState 状态值都在 <a href="/en-US/docs/Web/API/XMLHttpRequest#Properties">XMLHTTPRequest.readyState</a>，如下也是：</p>
...
<p>(<a href="http://msdn.microsoft.com/en-us//library/ms534361%28en-us,VS.85%29.aspx">Source</a>)</p>

<p>接下来，点击 HTTP 响应的 <a href="/en/HTTP#HTTP_Response_Codes">response code</a>。 可能的响应码都已经列在 <a href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html">W3C</a>这个列表里。在下面的例子中，我们通过检查响应码 <code>200 OK</code> 判断 AJAX 有没有成功。</p>
...
<p>让我们把所有的知识都集中起来做一个简单的 HTTP 请求。这个 JavaScript 会请求一个 HTML 文档 <code>test.html</code>，包含 "I'm a  test" 内容。然后我们 <code>alert()</code> 响应的内容。注意这个例子我们只是用了 JavaScript，没有用 jQuery。而且，HTML，XML 和 PHP 文件都要放在用一个目录下。</p>
...
<div class="note"><strong>Note</strong>: 如果你向一个代码片段发送请求，将返回 XML，而不是静态 XML 文件，在 IE 浏览器上则必须要设置响应头才能正常工作。如果不设置响应头为 <code>Content-Type:application/xml</code> ，IE 浏览器会在你访问 XML 元素时抛出 “Object Expected” 错误。</div>
...
<p>在上一个例子中，在收到 HTTP 请求的响应后我们会请求对象的 <code>responseText</code> 属性，包含 <code>test.html</code> 文件的内容。现在我们试试 <code>responseXML </code>属性。 </p>
...
<p>想获取更多 DOM 方法，可以查看<code> <a href="http://www.mozilla.org/docs/dom/">Mozilla's DOM implementation</a> 文档。</code></p>
