...
<table class="fullwidth-table">
 <tbody>
  ...
  <tr>
   ...
   <td>Yes</td>
   <td>No; 必须通过  <code><a href="../../../../En/DOM/Element.focus">focus()</a></code> 来聚焦该元素。可以在方向键或者其他键的响应里面做。</td>
  </tr>
  <tr>
...
<p>针对像菜单、标签面板、树等这些组合控件，父元素应该在 tab 序列里面 (tabindex="0") 而每个后代选择/标签/单元/行应该从 tab 序列里面移除 (tabindex="-1")。用户应该可通过方向键来操控这些后代元素。(关于组件一般预期的键盘支持，可以查看 <a href="http://access.aol.com/dhtml-style-guide-working-group/">DHTML Style Guide</a>.)</p>
...
<h4 id="用_element.focus()_来设置焦点_focus">用 element.focus() 来设置焦点 focus</h4>
...
<h4 id="用_onfocus_来追踪当前_focus">用 onfocus 来追踪当前 focus</h4>
...
<p>这个办法包含绑定一个单独的事件句柄到容器窗口组件上，运用 <code>aria-activedescendent 属性</code>来追踪一个 "虚拟" 焦点。（关于 ARIA 更多的信息，查看 <a href="../../../../An_Overview_of_Accessible_Web_Applications_and_Widgets">overview of accessible web applications and widgets</a>.）</p>

<p><code>aria-activedescendant</code> 属性用来标识拥有虚拟焦点的后代元素的 ID。在窗口容器的事件句柄里面在键盘和鼠标事件响应更新 aria-activedescendant 值并且确保当前 The event handler on the container must respond to key and mouse events by updating the value of <code>aria-activedescendant</code> and ensuring that the current item is styled appropriately (for example, with a border or background color). </p>

<h3 id="Tips">Tips</h3>

<h4 id="scrollIntoView">scrollIntoView</h4>

<p>Note that the use of this pattern requires the author to ensure that the current <em>focused</em> widget is scrolled into view. You should be able to use the {{ domxref("element.scrollIntoView()") }} function, but we recommend confirming this works for you in your target browsers using the <a href="http://www.quirksmode.org/dom/tests/scrollintoview.html">quirksmode test</a>.</p>

<h4 id="Issues">Issues</h4>

<ul>
 <li><a href="http://www.quirksmode.org/dom/w3c_cssom.html">quirksmode reports problems in Opera, and Konqueror</a></li>
