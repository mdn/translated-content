...
默认情况下，当人们使用 tab 键浏览网页时，只有交互式元素（如链接、表单控件）才会被聚焦。通过 `tabindex` [全局属性](/zh-CN/docs/Web/HTML/Reference/Global_attributes)，作者也可以使其他元素成为焦点。当设置为 `0` 时，元素可通过键盘和脚本聚焦。当设置为 `-1` 时，元素可被脚本聚焦，但不会成为键盘聚焦顺序的一部分。
