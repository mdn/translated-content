...
<p>首先，请阅读有关<a href="/en-US/docs/Web/Accessibility/ARIA/forms/Basic_form_hints#Required_and_invalid_fields"><code>aria-required</code> technique</a>所需的技术（如果您尚未这样做的话），因为该技术会以此为基础进行扩展。</p>
...
<h2 id="检查有效性并通知用户">检查有效性并通知用户</h2>
...
<p>下面是一个能够被插入在结束“ <code>head</code>”标签上方的示例 JavaScript 代码 </p>
...
<h2 id="checkValidity_函数"><code>checkValidity</code> 函数</h2>

<p>JavaScript 中用于表单验证的主要方法是<code>checkValidity</code>函数。 此方法采用三个参数：要验证的<code>输入</code>的 ID，要确保有效性的搜索项，以及要插入警报中的错误消息。</p>
...
<h2 id="addAlert_函数"><code>addAlert</code> 函数</h2>

<p>此功能首先删除所有旧警报。 该函数很简单：它查找 ID 为“<code>alert</code>”的元素，如果找到，则将其从<a href="/zh-CN/docs/Mozilla/Tech/XUL/Tutorial/Document_Object_Model">文档对象模型</a>中删除。</p>

<p>接下来，这个函数创建一个<code>div</code>元素来保存警告文本。 它得到了一个 “<code>alert</code>”的 ID.，并且它具有“<code>alert</code>”角色集。 这实际上是受 ARIA 启发的，即使它在属性名称中没有标明“aria”。 这是因为该角色基于<a href="http://www.w3.org/TR/xhtml-role/">XHTML 角色属性模块</a>，为简单起见，该模块简单地移植到 HTML。</p>

<p>这些文字被加入这个<code>div</code>元素，然后这个<code>div</code>元素被加入文档。</p>

<p>发生这种情况时，当此<code>div</code>出现时，Firefox 将向辅助技术触发“警报”事件。 大多数屏幕阅读器都会自动选择并说出来。 这类似于 Firefox 中的提示栏，提示您是否要保存密码。 我们刚刚创建的警报没有任何可按的按钮，只是告诉我们出了什么问题。</p>

<h2 id="修改“onblur”_事件">修改“<code>onblur</code>” 事件</h2>
...
<p>如果你在使用 Firefox 3 和一个当前支持的屏幕阅读器，试试看下面的操作：</p>
...
<p>在两种情况下，当将焦点返回到相关字段时，您的屏幕阅读器应该告诉您该字段无效。 JAWS 9 支持这个功能，但是 JAWS 8 不支持，所以这个功能不一定能够在所有版本的支持的屏幕阅读器中正常工作。</p>

<h2 id="您可能遇到的一些问题">您可能遇到的一些问题</h2>

<dl>
 <dt>Q. 为什么在某些输入中同时在标签文本和“<code>aria-required</code>”属性中都添加了“<code>(required)</code>”？
 </dt>
 <dd>A. 如果这是一个实例中的表格，并且尚不支持 ARIA 的浏览器正在访问该网站，我们仍然希望表明这是必填字段。</dd>
 <dt>Q. 为什么不将焦点自动设置回无效字段？</dt>
 <dd>A. 因为 Windows API 规范（可能还有其他规范）不允许这样做。 同样，让焦点在没有实际用户交互的情况下过于频繁地跳动通常也不是一件好事。</dd>
</dl>

<div class="warning"><p><strong>警告：</strong> TBD:让我们重新考虑一下 - 就个人而言，我认为如果不引起键盘陷阱，设置焦点可能会很好。</p></div>
