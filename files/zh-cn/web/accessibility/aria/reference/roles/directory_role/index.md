...
已弃用的 `directory` 角色用于表示一组成员的引用列表，例如静态目录。请改用 [`list`](/zh-CN/docs/Web/Accessibility/ARIA/Reference/Roles/list_role) 角色。更好的做法是使用 {{HTMLElement('ul')}} 或 {{HTMLElement('ol')}} 元素，因为使用 `directory` 并不会为辅助技术用户提供任何额外的好处。
