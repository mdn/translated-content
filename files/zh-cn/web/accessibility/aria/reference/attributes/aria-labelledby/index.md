...
5. `aria-labelledby` 属性的值可以包括不可见元素的内容。虽然你应该为辅助技术用户提供与所有其他用户相同的内容，但你可以在计算的名称字符串中包含具有 HTML [`hidden`](/zh-CN/docs/Web/HTML/Reference/Global_attributes#hidden) 属性、CSS [`display: none`](/zh-CN/docs/Web/CSS/Reference/Properties/display) 和 CSS [`visibility: hidden`](/zh-CN/docs/Web/CSS/Reference/Properties/visibility) 的元素的内容。
