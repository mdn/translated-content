...
有一部分内容在更新。更新是重要的，并且你希望在其被修改后通知用户，因此你已将其转换为具有 [`aria-live`](/zh-CN/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-live) 属性的 [ARIA 实时区域](/zh-CN/docs/Web/Accessibility/ARIA/Guides/Live_regions)。你可能希望同时更新该部分的几个组件，但是你无法确定所有内容是否会同时更新。即使它是一个非常重要的实时区域，具有 `aria-live="assertive"`，你也不希望在内容的不同部分加载时多次打断用户。这就是 `aria-busy` 可以提供帮助的地方。
...
[`aria-live`](/zh-CN/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-live) 的值决定在将值更改为 `false` 时是否立即宣布更改，或者辅助技术在当前任务完成之前是否等待打断用户。

### 在 `feed` 内部

如果具有 [`feed`](/zh-CN/docs/Web/Accessibility/ARIA/Reference/Roles/feed_role) 角色的元素的 `aria-busy` 被设置为 `true`，则消息来源（feed）内部发生的渲染更改不会被宣告，除非是用户发起的更改。
