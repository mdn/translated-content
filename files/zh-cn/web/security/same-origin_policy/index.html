...
<p>例如，<code>about:blank</code> 通常作为父脚本写入内容的新的空白弹出窗口的 URL（例如，通过  <a href="/zh-CN/docs/Web/API/Window/open">Window.open()</a>）。如果此弹出窗口也包含 JavaScript，则该脚本将从创建它的脚本那里继承对应的源。</p>
</div>
...
<p>这些差异点是不规范的，其它浏览器也未做出支持，但会助于开发基于 window RT IE 的应用程序。</p>
...
<ul>
 <li>跨源<em><strong>写操作</strong>（Cross-origin writes）</em>一般是被允许的<em>。</em>例如链接（links），重定向以及表单提交。特定少数的 HTTP 请求需要添加<a href="/zh-CN/docs/Web/HTTP/CORS#预检请求">预检请求</a>。</li>
 <li>跨源<em><strong>资源嵌入</strong>（Cross-origin embedding）</em>一般是被允许（后面会举例说明）。</li>
 <li>跨源<em><strong>读操作</strong>（Cross-origin reads）</em>一般是不被允许的<em>，</em>但常可以通过内嵌资源来巧妙的进行读取访问。例如，你可以读取嵌入图片的高度和宽度，调用内嵌脚本的方法，或<a href="https://grepular.com/Abusing_HTTP_Status_Codes_to_Expose_Private_Information">availability of an embedded resource</a>.</li>
</ul>
...
<ul>
 ...
 <li><code>&lt;link rel="stylesheet" href="..."&gt;</code> 标签嵌入 CSS。由于 CSS 的<a href="http://scarybeastsecurity.blogspot.dk/2009/12/generic-cross-browser-cross-domain.html">松散的语法规则</a>，CSS 的跨源需要一个设置正确的 HTTP 头部 <code>Content-Type</code> 。不同浏览器有不同的限制： <a href="http://msdn.microsoft.com/zh-CN/library/ie/gg622939%28v=vs.85%29.aspx">IE</a>, <a href="https://www.mozilla.org/security/announce/2010/mfsa2010-46.html">Firefox</a>, <a href="https://code.google.com/p/chromium/issues/detail?id=9877">Chrome</a>, <a href="https://support.apple.com/kb/HT4070">Safari</a> (跳至 CVE-2010-0051) 部分 和 <a href="https://www.opera.com/support/kb/view/943/">Opera</a>。</li>
 <li>通过 {{htmlelement("img")}} 展示的图片。支持的图片格式包括 PNG,JPEG,GIF,BMP,SVG,...</li>
 <li>通过 {{htmlelement("video")}} 和 {{htmlelement("audio")}} 播放的多媒体资源。</li>
 <li>通过 <code><a href="/zh-CN/docs/Web/HTML/Element/object">&lt;object&gt;</a></code>、 <a href="/zh-CN/docs/HTML/Element/embed"><code>&lt;embed&gt;</code></a> 和 <code><a href="/zh-CN/docs/HTML/Element/applet">&lt;applet&gt;</a></code> 嵌入的插件。</li>
 <li>通过 <code><a href="/zh-CN/docs/Web/CSS/@font-face">@font-face</a></code> 引入的字体。一些浏览器允许跨源字体（cross-origin fonts），一些需要同源字体（same-origin fonts）。</li>
 <li>通过 <code><a href="/zh-CN/docs/Web/HTML/Element/iframe">&lt;iframe&gt;</a></code> 载入的任何资源。站点可以使用 <a href="/zh-CN/docs/HTTP/X-Frame-Options">X-Frame-Options</a> 消息头来阻止这种形式的跨源交互。</li>
</ul>

<h3 id="如何允许跨源访问">如何允许跨源访问</h3>

<p>可以使用 <a href="/zh-CN/docs/Web/HTTP/CORS">CORS</a> 来允许跨源访问。CORS 是 {{Glossary("HTTP")}} 的一部分，它允许服务端来指定哪些主机可以从这个服务端加载资源。</p>

<h3 id="如何阻止跨源访问">如何阻止跨源访问</h3>

<ul>
 <li>阻止跨源写操作，只要检测请求中的一个不可推测的标记 (CSRF token) 即可，这个标记被称为 <a href="https://owasp.org/www-community/attacks/csrf">Cross-Site Request Forgery (CSRF)</a> 标记。你必须使用这个标记来阻止页面的跨源读操作。</li>
...
<h2 id="跨源脚本API访问">跨源脚本 API 访问</h2>

<p>JavaScript 的 API 中，如 <code><a href="/zh-CN/docs/DOM/HTMLIFrameElement">iframe.contentWindow</a></code>、 {{domxref("window.parent")}}、{{domxref("window.open")}} 和 {{domxref("window.opener")}} 允许文档间直接相互引用。当两个文档的源不同时，这些引用方式将对 <a href="https://www.whatwg.org/specs/web-apps/current-work/multipage/browsers.html#security-window">Window</a> 和 <a href="https://www.whatwg.org/specs/web-apps/current-work/multipage/history.html#security-location">Location</a>对象的访问添加限制，如下两节所述。</p>
...
<table>
 ...
 <tbody>
  <tr>
   <td>{{domxref("HTMLAnchorElement.href")}}</td>
   <td>只写。</td>
  </tr>
 </tbody>
</table>
...
<p>访问存储在浏览器中的数据，如 <a href="/zh-CN/docs/Web/API/Window/localStorage">localStorage</a> 和 <a href="/zh-CN/docs/Web/API/IndexedDB_API">IndexedDB</a>，是以源进行分割。每个源都拥有自己单独的存储空间，一个源中的 JavaScript 脚本不能对属于其它源的数据进行读写操作。</p>
