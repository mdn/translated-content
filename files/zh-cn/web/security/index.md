...
- [使用 HTTP Cookies](/zh-CN/docs/Web/HTTP/Guides/Cookies)
...
- [Referer 标头策略：隐私和安全性考虑](/zh-CN/docs/Web/Security/Referer_header:_privacy_and_security_concerns)
  - : 有一些与 [Referer HTTP 标头](/zh-CN/docs/Web/HTTP/Reference/Headers/Referer)相关的隐私和安全风险。这篇文章描述了它们，并提供避免这些风险的建议。
- Robots.txt
  - : 待补充
- Site maps
  - : 待补充
...
- [子资源完整性](/zh-CN/docs/Web/Security/Subresource_Integrity)
  - : **子资源完整性**（SRI）是一种安全功能，允许浏览器验证所获取的文件（比如，从一个{{Glossary("CDN","内容分发网络")}}中）没有遭受恶意修改。它的工作原理是允许你提供一个加密散列值，目标文件必须匹配这个值。
...
- [HTTP X-Content-Type-Options](/zh-CN/docs/Web/HTTP/Reference/Headers/X-Content-Type-Options)
  - : **`X-Content-Type-Options`** 响应的 HTTP 头是一个标记，由服务器用来表示应该遵循 {{HTTPHeader("Content-Type")}} 头中宣告的 [MIME 类型](/zh-CN/docs/Web/HTTP/Guides/MIME_types)，不应该被改变。这是一种选择退出 [MIME 类型嗅探](/zh-CN/docs/Web/HTTP/Guides/MIME_types#mime_嗅探)的方法，或者换句话说，MIME 类型是故意配置的。
