...
- `DOMAttrModified` 事件现在正确处理命名空间属性。（[Firefox 错误 362391](https://bugzil.la/362391)）
...
- 已向 XUL 元素和文档添加了 [`getElementsByAttributeNS()`](/zh-CN/docs/Mozilla/Tech/XUL/Method/getElementsByAttributeNS) 函数。（[Firefox 错误 239976](https://bugzil.la/239976))
...
- 修复了 `getElementsByTagName()`，使其在标签名包含命名空间前缀的子树上也能正常运作。（[Firefox 错误 206053](https://bugzil.la/206053)）
- `DOMNodeInserted` 和 `DOMNodeRemoved` 事件现在正确应用于正确的节点。（[Firefox 错误 367164](https://bugzil.la/367164)）
...
- 默认情况下，右键单击表单控件不会再弹出上下文菜单。（[Firefox 错误 404536](https://bugzil.la/404536))。参见[为表单控件提供上下文菜单](/zh-CN/docs/Offering%20a%20context%20menu%20for%20form%20controls)以了解如何逐个启用此功能。
