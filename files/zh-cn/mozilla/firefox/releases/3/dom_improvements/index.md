...
- 为 `Node.nodePrincipal`、`Node.baseURIObject` 和 `document.documentURIObject` 添加了仅限特权代码的 getter。Chrome 代码不得访问（获取或设置）这些属性，例如在一个未包装的内容对象上（如 [`XPCNativeWrapper`](https://web.archive.org/web/20140604075216/https://developer.mozilla.org/en-US/docs/XPCNativeWrapper) 的 `wrappedJSObject`），详情请参见 [Firefox bug 324464](https://bugzil.la/324464)。
...
- 如果按下加速键，则将 `keypress` 事件的 `charCode` 值更改为 ASCII 字符。否则，`charCode` 是未修改的字符（除 `Shift` 状态外）。参见 [Gecko Keypress Event](/zh-CN/Gecko_Keypress_Event)。
