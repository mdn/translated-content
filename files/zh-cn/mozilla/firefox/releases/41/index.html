...
<ul>
 <li>剪切、复制和粘贴命令处理已经被重新编码，并且现在允许 JS 编程实现网页内容复制和剪切。
  <ul>
   <li>使用 <code>'paste'</code>名利作为参数，{{domxref("Document.queryCommandSupported()")}}现在会返回 false 如果不充分的权限被执行 ({{bug(1161721)}}).</li>
   <li>使用<code>'cut'</code> 或 <code>'copy'</code> 命令作为参数，{{domxref("Document.queryCommandSupported()")}} 现在返回<code>true</code> 如果调用的上下文中包括用户发起的或特权代码 ({{bug(1162952)}}).</li>
...
<ul>
 <li>Improvement to our experimental <a href="/en-US/docs/Web/API/ServiceWorker_API">Service Worker</a> implementation:
...
<ul>
 ...
 <li>The <a href="/en-US/docs/Web/API/CSSFontLoading_API">CSS Font Loading API</a> has been completely implemented and is now enabled by default ({{bug(1149381)}}).</li>
...
<p>数学公式需要特殊的字体。到目前为止，这些字体被硬编码在用户默认样式 <code>mathml.css</code> (使用{{MathMLElement("math")}} 标志设置字体组) 和偏好选项<code>font.mathfont-family</code> (为拉伸和大操作数设置支持字体). Firefox 41 引入一个能够自动设置&lt;math&gt;标志和相应偏好选项的内置<code>x-math</code> 语言 (例如<code>font.name.serif.x-math</code>). 用户默认样式现在将&lt;math&gt;标志字体设置为 serif 并且<code>font.mathfont-family</code> 被替换为 <code>font.name.serif.x-math</code>.  所有平台现在基本使用相同列表的支持字体， "Latin Modern Math" 是首选。在标准的 per-language 字体偏好菜单中，默认支持字体能够被设置。更多细节，请参照{{bug(947654)}} 和 {{bug(1160456)}}.</p>

<h3 id="SVG">SVG</h3>

<ul>
 <li>站点图片 (偏爱图标，快捷方式图标) 现在支持 SVG({{bug(366324)}})</li>
</ul>
...
<ul>
 <li>The <a href="/en-US/docs/Web/Security/CSP">CSP</a> 1.1 <code>manifest-src</code> <a href="/en-US/docs/Web/Security/CSP/CSP_policy_directives">directive</a> 已经被支持 ({{bug(1089255)}}).</li>
 <li>Previous versions of Firefox incorrectly expected the <a href="/en-US/docs/Web/Security/CSP">Content Security Policy</a> <a href="/en-US/docs/Web/Security/CSP/CSP_policy_directives#referrer">referrer</a>directive's value <code>origin-when-cross-origin</code> to be spelled <code>origin-when-crossorigin</code>. This has been corrected to include the missing dash character.</li>
</ul>
