...
4. 当用户继续输入时，扩展将收到 {{WebExtAPIRef("omnibox.onInputChanged")}} 事件。事件监听器将传递用户输入的值，并可以使用建议填充地址栏下拉菜单。如果扩展使用 {{WebExtAPIRef("omnibox.setDefaultSuggestion()")}} 设置了默认建议，则该建议将显示在下拉菜单中的第一项处。
