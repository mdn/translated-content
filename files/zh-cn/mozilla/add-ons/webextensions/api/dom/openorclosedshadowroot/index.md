...
>
> 使用 {{DOMxRef("Element.attachShadow()")}} 将 Shadow DOM 添加到元素中。
...
一个 {{DOMxRef("ShadowRoot")}} 对象实例，无论其
{{DOMxRef("ShadowRoot.mode", "mode")}} 设置为 `open` 还是 `closed`，如果没有 Shadow DOM，则返回 `null`。
