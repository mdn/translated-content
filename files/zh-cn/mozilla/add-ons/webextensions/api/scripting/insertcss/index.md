...
要使用此 API，你必须拥有 `"scripting"` [权限](/zh-CN/docs/Mozilla/Add-ons/WebExtensions/manifest.json/permissions) 以及目标 URL 的权限，可以是 [主机权限](/zh-CN/docs/Mozilla/Add-ons/WebExtensions/manifest.json/permissions#主机权限) 或使用 [activeTab 权限](/zh-CN/docs/Mozilla/Add-ons/WebExtensions/manifest.json/permissions#活动标签权限)。
...
- `details`
  ...
  - : 描述要插入的 CSS 和插入位置的对象。它包含以下属性：
    ...
    - `files` {{optional_inline}}
      - : `string` 的 `array`。要注入的 CSS 文件相对于扩展根目录的路径。必须指定 `files` 或 `css` 其中之一。
...
[`Promise`](/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Promise)，当所有的 CSS 都被注入时将不带参数地兑现。如果出现了错误，该 promise 将被拒绝。

## 示例

该示例将来自字符串的 CSS 插入到活动标签页中。
