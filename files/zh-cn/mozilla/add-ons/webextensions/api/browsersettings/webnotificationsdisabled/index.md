...
请注意，这不会影响用户已为其设置了每个站点首选项的站点。例如，如果用户将<https://example.org>设置为*允许*，然后扩展程序将 `browserSettings.webNotificationsDisabled` 设置为 `true`，那么 [https://example.org](https://example.org) 下的页面仍将被允许显示通知。

将 `browserSettings.webNotificationsDisabled` 设置为 `false` 将全局默认值恢复为其默认值。
