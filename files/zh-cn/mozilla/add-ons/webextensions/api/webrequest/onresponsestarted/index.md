...
- `frameId`
  - : `integer`。如果请求发生在主框架中，则为零；正值表示请求发生的子框架的 ID。如果加载了 (子)框架的文档 (`type` 是 `main_frame` 或 `sub_frame`)，`frameId` 表示此框架的 ID，而不是外部框架的 ID。框架 ID 在标签页内是唯一的。
- `fromCache`
...
- `statusLine`
  - : `string`。响应的 HTTP 状态行，或 HTTP/0.9 响应的 'HTTP/0.9 200 OK' 字符串（即缺少状态行的响应），如果没有头，则为空字符串。
- `tabId`
...
> [!NOTE]
> 此 API 基于 Chromium 的 [`chrome.webRequest`](https://developer.chrome.google.cn/docs/extensions/reference/api/webRequest#event-onResponseStarted) API。该文档衍生自 Chromium 代码中的 [`web_request.json`](https://chromium.googlesource.com/chromium/src/+/master/extensions/common/api/web_request.json)。
