...
- `USER_SCRIPT`
  ...
  用户脚本的默认执行环境。此环境与页面的上下文和其他 `USER_SCRIPT` 世界隔离。与[内容脚本的 `ISOLATED` 世界](/zh-CN/docs/Mozilla/Add-ons/WebExtensions/API/scripting/ExecutionWorld)不同，在这一环境下扩展 API 并不可用。当脚本通过 `worldId` 注册时，可以存在多个 `USER_SCRIPT` 世界。可以通过 {{WebExtAPIRef("userScripts.configureWorld()")}} 更改 `USER_SCRIPT` 世界的配置。
