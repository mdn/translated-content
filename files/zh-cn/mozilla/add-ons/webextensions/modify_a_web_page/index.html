...
<p>浏览器附加组件 ( add-on ) 常被用于修改网页。例如更改页面的样式，隐藏特定的 DOM 节点或把 DOM 节点注入到页面中。</p>
...
<h2 id="修改匹配URL的页面">修改匹配 URL 的页面</h2>
...
<p>现在安装这个<a href="/en-US/Add-ons/WebExtensions/Temporary_Installation_in_Firefox">WebExtension</a>, 然后浏览 <a href="/">https://developer.mozilla.org/</a>：</p>

<p>{{EmbedYouTube("lxf2Tkg6U1M")}}</p>

<div class="note">
<p><strong>备注：</strong> 请注意，虽然此视频显示在 <a href="https://addons.mozilla.org/en-US/firefox/">addons.mozilla.org</a> 工作的 content scripts，但目前该网站已禁止 content scripts。</p>
</div>
...
<ul>
 ...
 <li><code><a href="/en-US/docs/Mozilla/Add-ons/WebExtensions/manifest.json/background">background</a></code>: 加载名为 "background.js" 的 <a href="/en-US/Add-ons/WebExtensions/Anatomy_of_a_WebExtension#Background_scripts">"background script"</a> （长期有效的后台脚本），在该脚本中，我们将设置注入右键菜单的内容脚本。</li>
</ul>
...
<p>在该脚本中我们创建了一个右键菜单项，给了它一个具体的 id 和标题 (将在菜单中显示的文本)。然后又设置了一个事件侦听器，当用户点击菜单项时，检查该菜单项是否就是我们的吞页菜单项。如果是，就通过<code><a href="/en-US/docs/Mozilla/Add-ons/WebExtensions/API/tabs/executeScript">tabs.executeScript()</a></code> 接口，把"page-eater.js" 注入到活动标签页中。这个接口用标签 ID 做为参数：如果省略标签 ID 参数，就默认把脚本注入当前活动标签。</p>
...
<p>重新加载<a href="/en-US/Add-ons/WebExtensions/Temporary_Installation_in_Firefox#Reloading_a_temporary_add-on">WebExtension</a>, 打开页面 (这次可以是任何一个页面) 激活右键菜单，然后选择 "Eat this page"：</p>
...
<p>内容脚本和后台脚本不能直接相互访问，但可以通过发送消息进行通信。当一端设置一个消息侦听器时，另一个端就可以发送消息了。下面的表格总结了通信时的 api 接口：</p>
...
<p>现在，不再立即执行吞页，内容脚本将先通过使用 <code><a href="/en-US/docs/Mozilla/Add-ons/WebExtensions/API/runtime/onMessage">runtime.onMessage</a></code>来监听消息。当监听到消息时，内容脚本才开始运作，除了来自<code>request.replacement</code>的替换文本不一样以外，其他的脚本运作本质上与之前的相同。</p>
...
<div class="note">
<p><strong>备注：</strong> 这些例子注入的都是 JavaScript; 想注入 CSS 可以使用 <code><a href="/en-US/docs/Mozilla/Add-ons/WebExtensions/API/tabs/insertCSS">tabs.insertCSS()</a></code> 函数。</p>
</div>
...
<ul>
 ...
 <li>使用<code>tabs.executeScript()</code>的例子：
  <ul>
   ...
   <li><a href="https://github.com/mdn/webextensions-examples/tree/master/context-menu-demo">context-menu-demo</a></li>
  </ul>
 </li>
</ul>
