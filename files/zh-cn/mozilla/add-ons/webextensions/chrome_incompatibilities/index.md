...
- **在 Chrome 中**：在使用未打包的扩展时，清单可能包含一个 [`"key"` 属性](https://developer.chrome.google.cn/docs/extensions/mv3/manifest/key/)，以便在不同机器上固定扩展 ID。这主要在使用 `web_accessible_resources` 时有用。
