...
为了解决这个问题，网络上的每台电脑需要连接到一个叫做 网络交换机（_network switch_）的特殊小电脑。交换机只干一件事：就像火车站的调度员，它要确保从一台电脑上发出的一条信息可以到达正确的电脑。为了把信息发送给电脑 B，电脑 A 必须把信息发送给交换机，交换机将收到的信息转发给电脑 B，--电脑 B 不会收到发给其他电脑的消息， 发给 电脑 B 的消息也不会传到局域网上的其他电脑上。

一旦我们把交换机加入到这个系统，我们的网络中便只需要十条网线：每台电脑一个插口，交换机上十个插口。

![十台电脑和一个交换机](internet-schema-3.png)

### 网络中的网

到目前为止一切都很好。但是我们要连接成百上千，上亿台电脑呢？当然一台*交换机*覆盖不了这么远，但是，如果你阅读得比较认真，我们曾提到*交换机*像其他电脑一样，所以我们为什么不把两个*交换机*彼此连接呢？

![两个交换机连接在一起](internet-schema-4.png)

我们把电脑连接交换机，接着交换机连接交换机，我们就会有无穷的规模。

![多个交换机连接在一起](internet-schema-5.png)

但是在现实中，这样会导致许多工程问题。数据包需要经过的交换机越多，到达目的地的时间就越长。而且你不能只依赖这种树状结构的交换机集群，因为一旦某个交换机节点故障，就会导致大面积的断网，这会使你的网络变得脆弱。为了解决这个问题，我们将每个网络保持在一个较小的规模，并使用一种设备来连接每个网络，这个设备就是 路由器（_router_）

路由器是一种负责在不同网络之间转发消息的计算机。路由器就像是邮局：当一个数据包到达是，它会读取收件人的地址，直接将数据包转发给正确的收件人，而无需经过层层中转。
...
如果你想给一台电脑发送一条信息，你必须指明它是哪台电脑。因此，任何连接到网络中的电脑都需要有一个唯一的地址来标记它，叫做“IP 地址”（IP 代表*互联网协议*）。这个地址由四部分被点分隔的数字序列组成，比如：`192.0.2.172`。
