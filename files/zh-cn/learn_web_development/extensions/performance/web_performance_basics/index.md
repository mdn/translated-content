...
- 使用*资源提示*，例如 [`rel=preconnect`](/zh-CN/docs/Web/HTML/Attributes/rel/preconnect)、[`rel=dns-prefetch`](/zh-CN/docs/Web/HTML/Attributes/rel/dns-prefetch)、[`rel=prefetch`](/zh-CN/docs/Web/HTML/Attributes/rel/prefetch) 和 [`rel=preload`](/zh-CN/docs/Web/HTML/Attributes/rel/preload)。
...
- 在超出应用视口范围的部分使用懒加载，如果你这么做了，为 SEO 制定一个后备计划（例如为 bot traffic 渲染整个页面）；例如，在 {{HTMLElement("img")}} 元素上使用 [`loading`](/zh-CN/docs/Web/HTML/Element/img#loading) 属性。
