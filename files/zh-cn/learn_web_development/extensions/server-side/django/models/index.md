...
<table>
  <tbody>
    <tr>
      ...
      <td>
        <a href="/zh-CN/docs/Learn_web_development/Extensions/Server-side/Django/skeleton_website"
...
这个教程展示了如何定义并访问 [LocalLibrary website](/zh-CN/docs/Learn_web_development/Extensions/Server-side/Django/Tutorial_local_library_website) 的模型。
...
> [!NOTE]
> 要用"抽象的"模型还有栏位说明怎么搜寻纪录可能会有点令人困惑。我们会以一个 Book 模型，其包含`title`与`genre`字段，而 genre 也是一个仅有`name`一个字段的模型。
...
> [!NOTE]
> 你可以用下划线来表示不同关系 (`ForeignKey`/`ManyToManyField`) .例如，一本书有不同的类型，用“cover“关系可能会帮助起一个参数名字 `type__cover__name__exact='hard'.`
