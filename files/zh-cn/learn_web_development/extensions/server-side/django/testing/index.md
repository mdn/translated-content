...
> [!NOTE]
> 我们构建 Django 骨架网站时，会自动创建骨架测试文件 **/catalog/tests.py**。将所有测试放入其中是完全“合法的”，但如果测试正确，你将很快得到一个非常庞大且难以管理的测试文件。
>
> 删除骨架文件，因为我们不需要它。
...
> [!NOTE]
> 测试类别还有一个我们还没有使用的`tearDown()`方法。此方法对数据库测试不是特别有用，因为`TestCase`基类会为你处理数据库拆卸。
...
> [!NOTE]
> 你通常**不应**在测试中包含**print()** 函数，如上所示。我们这样做，只是为了让你可以看到在控制台中，调用设置功能的顺序（在下一节中）。
...
> [!NOTE]
> 已省略对`last_name` 和 `date_of_birth`标签的测试，以及 `last_name`字段长度的测试。现在按照上面显示的命名约定和方法，添加你自己的版本。
...
> [!NOTE]
> 将 date_of_death 字段（/catalog/models.py）的标签更改为“death”并重新运行测试。
...
> [!NOTE]
> 这里我们实际上并没有使用数据库，或测试客户端。考虑修改这些测试，以使用[SimpleTestCase](https://docs.djangoproject.com/en/2.0/topics/testing/tools/#django.test.SimpleTestCase)。
>
...
> [!NOTE]
> 下面的`setUp()`代码，会创建一个具有指定语言`Language`的书本，但你的代码可能不包含语言模型`Language`，因为它是作为挑战创建的。如果是这种情况，只需注释掉创建或导入语言对象的代码部分。你还应该在随后的`RenewBookInstancesViewTest`部分中，执行此操作。
...
> [!WARNING]
> 全部借用的视图作为额外挑战，你的代码可能会改为重定向到主页'/'。如果是这样，请将测试代码的最后两行，修改为与下面的代码类似。请求中的`follow=True`，确保请求返回最终目标 URL（因此检查`/catalog/`而不是`/`）。
>
> ```python
...
- [Selenium](/zh-CN/docs/Learn_web_development/Extensions/Testing/Your_own_automation_environment) 是一个在真实浏览器中，自动化测试的框架。它允许你模拟与站点交互的真实用户，并为系统测试你的站点，提供了一个很好的框架（从集成测试开始的下一步）。
