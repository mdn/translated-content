...
- 第一个就是上面所讨论过的无单位比例。可以单独指定 {{cssxref("flex-grow")}} 全写属性的值。
- 第二个无单位比例值——{{cssxref("flex-shrink")}}——一般用于溢出容器的弹性项目。这指定了从每个弹性项目中取出多少溢出量，以阻止它们溢出它们的容器。这是一个相当高级的弹性盒子特性，我们不会在本文中进一步说明。
- 第三个是上面讨论的最小值。可以单独指定 {{cssxref("flex-basis")}} 全写属性的值。

我们建议不要使用 flex 全写属性，除非你真的需要（比如要去覆盖之前写的）。使用全写属性会多写很多的代码，它们也可能有点让人困惑。
...
- 相同 order 值的 flex 项按源代码顺序显示。所以假如你有四个元素，其 order 值分别是 `2`、`1`、`1` 和 `0`，那么它们的显示顺序就分别是第四、第二、第三和第一。
