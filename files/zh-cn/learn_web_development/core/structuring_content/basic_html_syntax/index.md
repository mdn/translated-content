...
字符引用的等价物可以很容易记住，因为它使用的文本可以看作是 `less than` 对应 `&lt;`，`quotation` 对应 `&quot;`，其他类似。要查找更多关于实体引用的信息，请参见 [XML 和 HTML 字符实体引用列表](https://zh.wikipedia.org/wiki/XML与HTML字符实体引用列表)（维基百科）。
