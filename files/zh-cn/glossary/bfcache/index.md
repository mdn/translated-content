...
缺点是 bfcache 条目需要更多资源，并且在表示正在进行的代码方面会创建复杂性。某些代码功能（例如 [`unload`](/zh-CN/docs/Web/API/Window/unload_event) 处理程序）不兼容，因此它们在页面上的存在会阻止页面使用 bfcache。

bfcache 对于性能非常有利，因此你有必要确保你的页面不会被阻止它被使用。你可以使用 [notRestoredReasons API](/zh-CN/docs/Web/API/Performance_API/Monitoring_bfcache_blocking_reasons) 来监视页面是否被阻止使用 bfcache 及其原因。
