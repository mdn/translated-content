...
inset 属性的解释取决于 {{cssxref("position")}} 属性的值。当设置 `position: absolute` 时，它们代表从[包含块](/zh-CN/docs/Web/CSS/CSS_display/Containing_block)或[锚点元素](/zh-CN/docs/Web/CSS/CSS_anchor_positioning/Using)插入。如果使用 `position: relative`，它们表示从盒子的默认外边距边缘位置插入。如果使用 `sticky`，则表示从{{glossary("scroll container", "滚动容器")}}的边缘嵌入。`fixed` 值与 `absolute` 类似，只是元素的位置和大小是相对于其固定定位的包含块（通常是视口）而言的。

## 参见

- [布局与包含快](/zh-CN/docs/Web/CSS/CSS_display/Containing_block)
