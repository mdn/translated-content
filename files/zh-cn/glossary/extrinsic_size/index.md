...
在 CSS 中，一个元素的**外在尺寸**（extrinsic size）由其布局上下文决定，即由该元素外部施加的约束所决定，而与其内容无关。这与元素的{{glossary("intrinsic size", "固有尺寸")}}相反，后者由其内容所决定。
...
百分比值也属于外在尺寸；它们是相对于元素的[包含块](/zh-CN/docs/Web/CSS/Guides/Display/Containing_block)计算得出的。例如，`width: 50%` 表示该元素的宽度为其包含块宽度的 `50%`，而与其内部内容无关。
