...
一个 HTTP 方法是**幂等**的，指的是同样的请求被执行一次与连续执行多次的效果是一样的，服务器的状态也是一样的。换句话说就是，

这不意味着请求不会有*任何*副作用，比如，服务器可能会对所有请求做记录。在正确实现的条件下， {{HTTPMethod("GET")}}、{{HTTPMethod("HEAD")}}、{{HTTPMethod("PUT")}} 和 {{HTTPMethod("DELETE")}} 等方法都是**幂等**的，而 {{HTTPMethod("POST")}} 方法不是。

所有的{{glossary("safe/http", "安全")}}方法也都是幂等的，包括 {{HTTPMethod("PUT")}} 和 {{HTTPMethod("DELETE")}}。而 {{HTTPMethod("POST")}} 方法不是幂等的。

幂等性只考虑服务器的状态。每次请求返回的响应可能不同：例如，第一次调用 {{HTTPMethod("DELETE")}} 可能会返回 {{HTTPStatus("200")}}，而后续的调用可能会返回 {{HTTPStatus("404")}}。另一个 {{HTTPMethod("DELETE")}} 是幂等的含义是，开发者不应该使用 `DELETE` 方法实现 RESTful API 的 _删除最后一个条目_ 功能。

需要注意的是，方法的幂等性并不是由服务器保证的，有些应用程序可能会错误地破坏幂等性约束。
