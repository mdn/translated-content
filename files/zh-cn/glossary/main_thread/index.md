...
除非故意使用 [Web Worker](/zh-CN/docs/Web/API/Web_Workers_API/Using_web_workers)，比如 [Service Worker](/zh-CN/docs/Web/API/Service_Worker_API/Using_Service_Workers)，否则 JavaScript 只在线程中运行，所以脚本的运行时，很容易导致事件处理流程或绘制的延迟。主线程中运行的工作越少，就有越多的余地来处理用户事件，页面绘制和对用户保持响应。
