...
![带对号标记的绿色小部件：基线，广泛可用。四个浏览器徽标，均带有对号标记。](widely.png)

被列为**广泛可用**（widely available）的特性在各基线浏览器中都有至少 2.5 年的稳定支持历史。

![带对号标记的蓝色小部件：基线 2022，最近可用。四个浏览器徽标，均带有对号标记。](newly.png)

被列为**最近可用**（newly available）的特性至少可在各基线浏览器的最新稳定版中使用，但可能无法在旧版浏览器和设备上使用。

![带叉号的灰色小部件：有限可用。四个浏览器图标，其中两个带对号，两个带叉号。](limited.png)

标注为**有限可用**（limited availability）的特性目前*尚未*在所有浏览器中提供。

![带虚线菱形的小部件：已弃用。四个浏览器图标，均带对号。](deprecated.png)

标注为**已弃用**（deprecated）的特性可能在一种或多种浏览器中可用，但不应在开发中使用。

![带有叉号的小部件：已弃用，即将移除。四个浏览器图标，其中两个带有对号，两个带有叉号。](removed.png)

标有**弃用并即将移除**（deprecated to be removed）的特性可能在一种或多种浏览器中可用，但即将被移除，因此不应在开发中使用。
