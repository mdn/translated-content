...
它是一个低阶、过程化的模型，更新[位图](https://zh.wikipedia.org/wiki/位图)并没有内置的[场景图](https://zh.wikipedia.org/wiki/场景图)。它提供了一个空的图形区域，允许特定的 {{Glossary("JavaScript")}} {{Glossary("API")}} 在上面绘制（比如 Canvas 2D 或 {{Glossary("WebGL")}}）。

## 参见

- 维基百科上的 [Canvas（HTML 元素）](<https://zh.wikipedia.org/wiki/Canvas_(HTML元素)>)
