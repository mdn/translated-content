...
**JavaScript 引擎**是解析和执行 {{glossary("JavaScript")}} 代码的解释器。现代 JavaScript 引擎使用即时编译（JIT）将 JavaScript 代码转换为可由计算机处理器执行的机器码。JavaScript 引擎通常由网页{{glossary("browser", "浏览器")}}开发和使用，用于运行客户端代码，也可用于如 {{glossary("Node.js")}} 这样的服务端环境。
