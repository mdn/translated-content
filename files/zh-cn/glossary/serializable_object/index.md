...
**可序列化对象**是可以在任何 JavaScript 环境（“realm”）中序列化、反序列化的对象。这允许将此类对象存储在磁盘上并在以后进行恢复，或使用 {{domxref("structuredClone()")}} 对它们进行拷贝，又或者使用 {{domxref("DedicatedWorkerGlobalScope.postMessage()")}} 在 Worker 之间共享它们。
