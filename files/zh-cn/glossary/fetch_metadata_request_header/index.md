...
有了这些信息，服务器可以实现{{Glossary("resource isolation policy", "资源隔离策略")}}，允许额外的站点仅请求用于共享并且适当的使用资源。这些方法可以帮助缓解常见的跨站点网络漏洞，例如 {{Glossary("CSRF")}}、跨站点脚本攻击（“XSSI”）、定时攻击和跨源消息攻击。

这些标头有 `Sec-` 前缀，因此有{{Glossary("Forbidden header name", "禁止的标头名称")}}。因此，它们不能从 JavaScript 进行修改。
