...
> **备注：** 虽然[规范](https://fetch.spec.whatwg.org/#forbidden-request-header)中将 {{HTTPHeader("Referer")}} 标头列为禁止修改的标头，但是用户代理并没有完全对其进行控制，因此可以通过编程的方式修改此标头。例如，当使用 [`fetch()`](/zh-CN/docs/Web/API/fetch) 时，可以通过 [`referrer` 选项](/zh-CN/docs/Web/API/fetch#referrer)对 {{HTTPHeader("Referer")}} 标头进行编程修改。
