...
> **备注：** 即使你的函数有自己的函数名称，你仍然可以用这个变量名去调用它。给函数命名只会方便你调试，_但不会影响我们调用它。_
...
> **备注：** 回调函数：我们把一个函数作为参数传递给另一个函数，那么我们就称这个函数为 _{{glossary("callback function", "回调函数")}}_。_`sayHello()` 函数就是一个回调函数_。
