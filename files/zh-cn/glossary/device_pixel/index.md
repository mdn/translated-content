...
设备像素的密度依赖于显示分辨率。_device-pixel-ratio_（设备像素比）是物理设备像素分辨率与 CSS 像素分辨率的比值。它通常不同于 {{glossary("CSS pixel", "CSS 像素")}}，后者是绝对长度，定义为 1 CSS 英寸的 1/96。被认为是高 DPI 的设备在同一物理区域内每个 CSS 像素拥有多个设备像素，而低 DPI 设备则小于 1:1 的比例。
