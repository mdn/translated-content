...
---
<p><code>flex</code> 是一个 CSS 的{{cssxref("display")}} 属性中新添加一个值。 随着<code>inline-flex</code>的使用，它将使它适用的元素成为一个{{glossary("flex container")}}（伸缩容器），而这个元素的每个子元素将成为 {{glossary("flex item")}}（伸缩项目）。伸缩项目将参与到 flex 布局中，所有由 CSS Flexible Box Layout Module（CSS 伸缩盒布局模型）定义的属性都能被它们使用。</p>

<p><code>flex</code> 属性是<code>flex-grow</code>, <code>flex-shrink</code> 和 <code>flex-basis </code>属性的简写。</p>

<p>此外，<code>&lt;flex&gt;</code> 可以作为<a href="/zh-CN/docs/Web/CSS/flex_value">弹性长度</a>被引用在 CSS Grid（栅格）布局中。</p>
