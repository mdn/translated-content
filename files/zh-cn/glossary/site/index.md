...
在某些情况下，在区分不同的站点时，也会考虑站点的方案（scheme）。这将使 `http://vpl.ca` 和 `https://vpl.ca` 成为不同的站点。包含方案的情况可以防止不安全的（HTTP）站点被视为与安全（HTTPS）站点为相同的站点。考虑方案的定义有时被称为*方案同站*。这个更严格的定义适用于处理 [`SameSite`](/zh-CN/docs/Web/HTTP/Reference/Headers/Set-Cookie#samesitesamesite-value) cookie。
