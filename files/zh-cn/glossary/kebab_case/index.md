...
烤串命名法通常用作变量命名约定。然而，在许多编程语言中，连字符表示减法，因此无法使用烤串命名法。CSS 属性如 {{cssxref("background-color")}} 和 {{cssxref("font-family")}} 采用的是烤串命名法，HTML 属性，如 [`aria-label`](/zh-CN/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-label) 和 [`data-*`](/zh-CN/docs/Web/HTML/Global_attributes/data-*) 也是如此。
